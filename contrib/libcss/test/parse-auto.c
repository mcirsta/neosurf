...
bool handle_line(const char *data, size_t datalen, void *pw)
{
    ...
    if (data[0] == '#') {
        ...
        if (ctx->indata && strncasecmp(data + 1, "errors", 6) == 0) {
            ...
            ctx->inerrors = true;
            ctx->inexp = false;
        } else if (ctx->inerrors && strncasecmp(data + 1, "expected", 8) == 0) {
            ctx->indata = false;
            ctx->inerrors = false;
            ctx->inexp = true;
            ctx->inrule = false;
...
void css__parse_expected(line_ctx *ctx, const char *data, size_t len)
{
    ...
    start_rule:
        ...
        memcpy(ctx->exp[ctx->expused].name, name, min(len - (name - data), MAX_RULE_NAME_LEN));
        ctx->exp[ctx->expused].name[min(len - (name - data), MAX_RULE_NAME_LEN - 1)] = '\0';
        ...
        while (next < data + len) {
            ...
            if (rule->bcused >= rule->bclen) {
                size_t num = rule->bcused == 0 ? 4 : rule->bcused;

                uint8_t *temp = realloc(rule->bytecode, num * 2);
                if (temp == NULL) {
            ...
            if (*next == 'P') {
                ...
                if (rule->stused >= rule->stlen) {
                    size_t num = rule->stused == 0 ? 4 : rule->stused;

                    struct stentry *temp = realloc(rule->stringtab, num * 2 * sizeof(struct stentry));
                    if (temp == NULL) {
                ...
                rule->stringtab[rule->stused].off = rule->bcused;
                rule->stringtab[rule->stused].string = malloc(next - str);
                assert(rule->stringtab[rule->stused].string != NULL);
                memcpy(rule->stringtab[rule->stused].string, str, next - str - 1);
                rule->stringtab[rule->stused].string[next - str - 1] = '\0';
                ...
                /* Append to bytecode */
                memcpy(rule->bytecode + rule->bcused, &val, sizeof(val));
...
void run_test(const uint8_t *data, size_t len, exp_entry *exp, size_t explen)
{
    ...
    while (error == CSS_IMPORTS_PENDING) {
        ...
        if (error == CSS_OK) {
            ...
            assert(css_stylesheet_create(&params, &import) == CSS_OK);

            assert(css_stylesheet_register_import(sheet, import) == CSS_OK);
