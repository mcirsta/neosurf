...
static size_t dump_css_unit(css_fixed val, css_unit unit, char *ptr, size_t len)
{
    ...
    if (unit == CSS_UNIT_MIN_CONTENT || unit == CSS_UNIT_MAX_CONTENT || unit == CSS_UNIT_FIT_CONTENT) {
        switch (unit) {
...
static void dump_computed_style(const css_computed_style *style, char *buf, size_t *len, css_unit_ctx *unit_ctx)
{
    char *ptr = buf;
    size_t wrote = 0;
    uint8_t val;
    ...
    *len -= wrote;
    ...
    case CSS_BORDER_COLOR_CURRENT_COLOR:
        ...
        break;
    case CSS_BORDER_COLOR_COLOR:
        wrote = snprintf(ptr, *len, "border-right-color: #%08x\n", color);
        break;
    default:
        wrote = 0;
        break;
    ...
    *len -= wrote;
    ...
    case CSS_BORDER_COLOR_CURRENT_COLOR:
        ...
        break;
    case CSS_BORDER_COLOR_COLOR:
        wrote = snprintf(ptr, *len, "border-bottom-color: #%08x\n", color);
        break;
    default:
        wrote = 0;
        break;
    ...
    *len -= wrote;
    ...
    case CSS_BORDER_COLOR_CURRENT_COLOR:
        ...
        break;
    case CSS_BORDER_COLOR_COLOR:
        wrote = snprintf(ptr, *len, "border-left-color: #%08x\n", color);
        break;
    default:
        wrote = 0;
        break;
    ...
    case CSS_CLIP_RECT:
        ...
        *len -= wrote;
        ...
        if (rect.right_auto)
            ...
            wrote = dump_css_unit(rect.right, rect.runit, ptr, *len);
        wrote += snprintf(ptr + wrote, *len - wrote, ", ");
        ptr += wrote;
        *len -= wrote;
        ...
        if (rect.bottom_auto)
            ...
            wrote = dump_css_unit(rect.bottom, rect.bunit, ptr, *len);
        wrote += snprintf(ptr + wrote, *len - wrote, ", ");
        ptr += wrote;
        *len -= wrote;
        ...
        *len -= wrote;
        ...
        while (content->type != CSS_COMPUTED_CONTENT_NONE) {
            ...
            case CSS_COMPUTED_CONTENT_STRING:
                wrote += snprintf(ptr + wrote, *len - wrote, "\"%.*s\"", (int)lwc_string_length(content->data.string),
                    lwc_string_data(content->data.string));
                break;
            case CSS_COMPUTED_CONTENT_URI:
                wrote += snprintf(ptr + wrote, *len - wrote, "uri(\"%.*s\")", (int)lwc_string_length(content->data.uri),
                    lwc_string_data(content->data.uri));
                break;
            case CSS_COMPUTED_CONTENT_COUNTER:
                wrote += snprintf(ptr + wrote, *len - wrote, "counter(%.*s)",
                    (int)lwc_string_length(content->data.counter.name), lwc_string_data(content->data.counter.name));
                break;
            case CSS_COMPUTED_CONTENT_COUNTERS:
                wrote += snprintf(ptr + wrote, *len - wrote,
                    "counters(%.*s, "
                    "\"%.*s\")",
                    (int)lwc_string_length(content->data.counters.name), lwc_string_data(content->data.counters.name),
                    (int)lwc_string_length(content->data.counters.sep), lwc_string_data(content->data.counters.sep));
                break;
            case CSS_COMPUTED_CONTENT_ATTR:
                wrote += snprintf(ptr + wrote, *len - wrote, "attr(%.*s)", (int)lwc_string_length(content->data.attr),
                    lwc_string_data(content->data.attr));
                break;
            case CSS_COMPUTED_CONTENT_OPEN_QUOTE:
                wrote += snprintf(ptr + wrote, *len - wrote, "open-quote");
                break;
            case CSS_COMPUTED_CONTENT_CLOSE_QUOTE:
                wrote += snprintf(ptr + wrote, *len - wrote, "close-quote");
                break;
            case CSS_COMPUTED_CONTENT_NO_OPEN_QUOTE:
                wrote += snprintf(ptr + wrote, *len - wrote, "no-open-quote");
                break;
            case CSS_COMPUTED_CONTENT_NO_CLOSE_QUOTE:
                wrote += snprintf(ptr + wrote, *len - wrote, "no-close-quote");
                break;
            }
    ...
    if (val == CSS_COUNTER_INCREMENT_INHERIT) {
        ...
        *len -= wrote;
        ...
        while (counter->name != NULL) {
            wrote = snprintf(
                ptr, *len, " %.*s ", (int)lwc_string_length(counter->name), lwc_string_data(counter->name));
            ptr += wrote;
            *len -= wrote;
    ...
    *len -= wrote;
    ...
    if (string_list != NULL) {
        while (*string_list != NULL) {
            wrote = snprintf(
                ptr, *len, " url('%.*s')", (int)lwc_string_length(*string_list), lwc_string_data(*string_list));
            ptr += wrote;
            *len -= wrote;
    ...
    if (val == CSS_FONT_FAMILY_INHERIT) {
        ...
        *len -= wrote;
        ...
        if (string_list != NULL) {
            while (*string_list != NULL) {
                wrote = snprintf(
                    ptr, *len, " \"%.*s\"", (int)lwc_string_length(*string_list), lwc_string_data(*string_list));
                ptr += wrote;
                *len -= wrote;
            ...
            *len -= wrote;
            ...
            if (tracks[i].unit == CSS_UNIT_MINMAX) {
                ...
                *len -= wrote;
                ...
                wrote = dump_css_unit(tracks[i].value, tracks[i].min_unit, ptr, *len);
                ptr += wrote;
                *len -= wrote;
                ...
                *len -= wrote;
                ...
                wrote = dump_css_unit(tracks[i].max_value, tracks[i].max_unit, ptr, *len);
                ptr += wrote;
                *len -= wrote;
                ...
                *len -= wrote;
                ...
                wrote = dump_css_unit(tracks[i].value, tracks[i].unit, ptr, *len);
                ptr += wrote;
                *len -= wrote;
            }
        }
    ...
    *len -= wrote;
    ...
    if (val == CSS_LIST_STYLE_IMAGE_INHERIT) {
        ...
        wrote = snprintf(
            ptr, *len, "list-style-image: url('%.*s')\n", (int)lwc_string_length(url), lwc_string_data(url));
    ...
    *len -= wrote;
    ...
    case CSS_LIST_STYLE_TYPE_DECIMAL_LEADING_ZERO:
        wrote = snprintf(ptr, *len, "list-style-type: decimal-leading-zero\n");
        break;
