...
void css__parse_expected(line_ctx *ctx, const char *data, size_t len)
{
    ...
    if (colon != data + len) {
        ...
        for (len = len - (colon + 1 - data); len > 0; len--, p++) {
            ...
            assert(ctx->exp[ctx->expused].textLen < EXP_ENTRY_TEXT_LEN);
        }
    }
    ctx->expused++;
}
...
void run_test(const uint8_t *data, size_t len, exp_entry *exp, size_t explen)
{
    ...
    assert(
        parserutils_inputstream_create("UTF-8", CSS_CHARSET_DICTATED, css__charset_extract, &input) == PARSERUTILS_OK);

    assert(css__lexer_create(input, &lexer) == CSS_OK);

    assert(parserutils_inputstream_append(input, data, len) == PARSERUTILS_OK);

    assert(parserutils_inputstream_append(input, NULL, 0) == PARSERUTILS_OK);
    ...
    while ((error = css__lexer_get_token(lexer, &tok)) == CSS_OK) {
        if (tok->type != exp[e].type) {
            printf("%d: Got token %s, Expected %s [%d, %d]\n", testnum, string_from_type(tok->type),
                string_from_type(exp[e].type), tok->line, tok->col);
        ...
        if (exp[e].hasText) {
            ...
            if (strncmp((char *)tok->data.data, exp[e].text, tok->data.len) != 0) {
                printf("%d: Got data '%.*s', Expected '%.*s'\n", testnum, (int)tok->data.len, tok->data.data,
