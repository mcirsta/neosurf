...
static css_error node_classes(void *pw, void *n, lwc_string ***classes, uint32_t *n_classes)
{
    unsigned int i;
    node *node = n;
    UNUSED(pw);
...
static css_error node_id(void *pw, void *n, lwc_string **id)
{
    ...
    for (i = 0; i < node->n_attrs; i++) {
        bool amatch = false;
        assert(lwc_string_caseless_isequal(node->attrs[i].name, lc->attr_id, &amatch) == lwc_error_ok);
        if (amatch == true)
            break;
    }
...
static css_error named_ancestor_node(void *pw, void *n, const css_qname *qname, void **ancestor)
{
    ...
    for (node = node->parent; node != NULL; node = node->parent) {
        bool match = false;
        assert(lwc_string_caseless_isequal(qname->name, node->name, &match) == lwc_error_ok);
        if (match == true)
            break;
    }
...
static css_error named_parent_node(void *pw, void *n, const css_qname *qname, void **parent)
{
    ...
    *parent = NULL;
    if (node->parent != NULL) {
        bool match = false;
        assert(lwc_string_caseless_isequal(qname->name, node->parent->name, &match) == lwc_error_ok);
        if (match == true)
...
static css_error named_sibling_node(void *pw, void *n, const css_qname *qname, void **sibling)
{
    ...
    *sibling = NULL;
    if (node->prev != NULL) {
        bool match = false;
        assert(lwc_string_caseless_isequal(qname->name, node->prev->name, &match) == lwc_error_ok);
        if (match == true)
...
static css_error node_has_name(void *pw, void *n, const css_qname *qname, bool *match)
{
    ...
    if (lwc_string_length(qname->name) == 1 && lwc_string_data(qname->name)[0] == '*')
        *match = true;
        ...
        assert(lwc_string_caseless_isequal(node->name, qname->name, match) == lwc_error_ok);
...
static css_error node_has_class(void *pw, void *n, lwc_string *name, bool *match)
{
    ...
    for (i = 0; i < node->n_attrs; i++) {
        bool amatch = false;
        assert(lwc_string_caseless_isequal(node->attrs[i].name, ctx->attr_class, &amatch) == lwc_error_ok);
        if (amatch == true)
            break;
    }
...
static css_error node_has_id(void *pw, void *n, lwc_string *name, bool *match)
{
    ...
    for (i = 0; i < node->n_attrs; i++) {
        bool amatch = false;
        assert(lwc_string_caseless_isequal(node->attrs[i].name, ctx->attr_id, &amatch) == lwc_error_ok);
        if (amatch == true)
            break;
    }
...
static css_error node_has_attribute(void *pw, void *n, const css_qname *qname, bool *match)
{
    ...
    *match = false;
    for (i = 0; i < node->n_attrs; i++) {
        assert(lwc_string_caseless_isequal(node->attrs[i].name, qname->name, match) == lwc_error_ok);
        if (*match == true)
            break;
    }
    ...
    *match = false;
    ...
    if (*match == true) {
        ...
        if (len < vlen)
            ...
            *match = (strncasecmp(data + suffix_start, vdata, vlen) == 0);
        }
    }
...
static css_error node_count_siblings(void *pw, void *n, bool same_name, bool after, int32_t *count)
{
    ...
    if (after) {
        while (node->next != NULL) {
            if (same_name) {
                assert(lwc_string_caseless_isequal(name, node->next->name, &match) == lwc_error_ok);
        ...
        while (node->prev != NULL) {
            if (same_name) {
                assert(lwc_string_caseless_isequal(name, node->prev->name, &match) == lwc_error_ok);
...
static css_error css_font_resolution_func(void *pw, lwc_string *name, css_system_font *system_font)
{
    lwc_error err;
    ...
    if (strncmp(lwc_string_data(name), "special-system-font", lwc_string_length(name)) != 0) {
        return CSS_INVALID;
    }
    ...
    err = lwc_intern_string("special-system-font", strlen("special-system-font"), &system_font->family);
    if (err != lwc_error_ok) {
        return CSS_NOMEM;
    }
...
static css_error test_error_handler(void *pw, css_stylesheet *sheet, css_error error, const char *msg)
{
    UNUSED(pw);
    UNUSED(sheet);
...
static void css__parse_media_list(const char **data, size_t *len, css_media *media)
{
    ...
    while (p < end) {
        ...
        if (p - start == 10 && strncasecmp(start, "projection", 10) == 0)
            result |= CSS_MEDIA_PROJECTION;
        else if (p - start == 8 && strncasecmp(start, "handheld", 8) == 0)
            result |= CSS_MEDIA_HANDHELD;
        else if (p - start == 8 && strncasecmp(start, "embossed", 8) == 0)
            result |= CSS_MEDIA_EMBOSSED;
        else if (p - start == 7 && strncasecmp(start, "braille", 7) == 0)
...
static void css__parse_pseudo_list(const char **data, size_t *len, uint32_t *element)
{
    ...
    *element = CSS_PSEUDO_ELEMENT_NONE;
    ...
    while (p < end) {
        ...
        /* Pseudo elements */
        if (p - start == 12 && strncasecmp(start, "first-letter", 12) == 0)
            *element = CSS_PSEUDO_ELEMENT_FIRST_LETTER;
        else if (p - start == 10 && strncasecmp(start, "first-line", 10) == 0)
...
static void css__parse_tree_data(line_ctx *ctx, const char *data, size_t len)
{
    ...
    if (value == NULL) {
        ...
        attribute *temp = realloc(n->attrs, (n->n_attrs + 1) * sizeof(attribute));
        assert(temp != NULL);
        ...
        assert(lwc_string_caseless_isequal(n->attrs[n->n_attrs].name, ctx->attr_class, &amatch) == lwc_error_ok);
        if (amatch == true) {
            n->classes = realloc(NULL, sizeof(lwc_string **));
            assert(n->classes != NULL);

            n->classes[0] = lwc_string_ref(n->attrs[n->n_attrs].value);
            n->n_classes = 1;
        }
...
static void run_test_select_tree(css_select_ctx *select, node *node, line_ctx *ctx, char *buf, size_t *buflen)
{
    css_select_results *sr;
    struct node *n = NULL;
    ...
    assert(css_select_style(select, node, &unit_ctx, &ctx->media, NULL, &select_handler, ctx, &sr) == CSS_OK);

    if (node->parent != NULL) {
        css_computed_style *composed;
        assert(css_computed_style_compose(node->parent->sr->styles[ctx->pseudo_element],
                   sr->styles[ctx->pseudo_element], &unit_ctx, &composed) == CSS_OK);
    ...
    if (node == ctx->target) {
        dump_computed_style(sr->styles[ctx->pseudo_element], buf, buflen, &unit_ctx);
    }
...
static void show_differences(size_t len, const char *exp, const char *res)
{
    ...
    while (pos_exp < exp + len && pos_res < res + len) {
        if (*pos_exp == '\n' && *pos_res == '\n') {
            if (pos_exp - opos_exp != pos_res - opos_res || memcmp(opos_exp, opos_res, pos_exp - opos_exp) != 0) {
                printf("Expected:\t%.*s\n", (int)(pos_exp - opos_exp), opos_exp);
                printf("  Result:\t%.*s\n", (int)(pos_res - opos_res), opos_res);
                printf("\n");
            }
            opos_exp = ++pos_exp;
            opos_res = ++pos_res;
...
static void destroy_tree(node *root)
{
    ...
    if (root->libcss_node_data != NULL) {
        css_libcss_node_data_handler(&select_handler, CSS_NODE_DELETED, NULL, root, NULL, root->libcss_node_data);
    }
...
static void run_test(line_ctx *ctx, const char *exp, size_t explen)
{
    ...
    for (i = 0; i < ctx->n_sheets; i++) {
        assert(css_select_ctx_append_sheet(select, ctx->sheets[i].sheet, ctx->sheets[i].origin, ctx->sheets[i].media) ==
            CSS_OK);
    }
    ...
    if (8192 - buflen != explen || memcmp(buf, exp, explen) != 0) {
        ...
        printf("Result (%u):\n%.*s\n", (int)(8192 - buflen), (int)(8192 - buflen), buf);
...
static bool handle_line(const char *data, size_t datalen, void *pw)
{
    ...
    if (data[0] == '#') {
        if (ctx->intree) {
            ...
            if (strncasecmp(data + 1, "errors", 6) == 0) {
                assert(css_stylesheet_data_done(ctx->sheets[ctx->n_sheets - 1].sheet) == CSS_OK);

                ctx->intree = false;
                ctx->insheet = false;
                ctx->inerrors = true;
                ctx->inexp = false;
            } else if (strncasecmp(data + 1, "ua", 2) == 0 || strncasecmp(data + 1, "user", 4) == 0 ||
                ...
                error = css_stylesheet_append_data(
                    ctx->sheets[ctx->n_sheets - 1].sheet, (const uint8_t *)data, datalen);
        ...
        if (ctx->intree) {
            ...
            error = css_stylesheet_append_data(ctx->sheets[ctx->n_sheets - 1].sheet, (const uint8_t *)data, datalen);
            assert(error == CSS_OK || error == CSS_NEEDDATA);
        } else if (ctx->inexp) {
