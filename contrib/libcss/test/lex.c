...
static void printToken(const css_token *token)
{
    ...
    case CSS_TOKEN_ATKEYWORD:
        printf("ATKEYWORD(%.*s)", (int)token->data.len, token->data.data);
        break;
    ...
    case CSS_TOKEN_PERCENTAGE:
        printf("PERCENTAGE(%.*s)", (int)token->data.len, token->data.data);
        break;
    case CSS_TOKEN_DIMENSION:
        printf("DIMENSION(%.*s)", (int)token->data.len, token->data.data);
        break;
    case CSS_TOKEN_URI:
        printf("URI(%.*s)", (int)token->data.len, token->data.data);
        break;
    case CSS_TOKEN_UNICODE_RANGE:
        printf("UNICODE-RANGE(%.*s)", (int)token->data.len, token->data.data);
        break;
    case CSS_TOKEN_CDO:
        printf("CDO");
        break;
    ...
    case CSS_TOKEN_FUNCTION:
        printf("FUNCTION(%.*s)", (int)token->data.len, token->data.data);
        break;
...
#define CHUNK_SIZE (4096)
    ...
    for (i = 0; i < ITERATIONS; i++) {
        assert(parserutils_inputstream_create("UTF-8", CSS_CHARSET_DICTATED, css__charset_extract, &stream) ==
            PARSERUTILS_OK);
