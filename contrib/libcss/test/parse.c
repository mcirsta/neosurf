...
#if DUMP_EVENTS
static const char *event_names[] = {"START_STYLESHEET", "END_STYLESHEET", "START_RULESET", "END_RULESET",
    "START_ATRULE", "END_ATRULE", "START_BLOCK", "END_BLOCK", "BLOCK_CONTENT", "DECLARATION"};
#endif

static css_error event_handler(css_parser_event type, const parserutils_vector *tokens, void *pw)
{
#if !DUMP_EVENTS
    UNUSED(type);
    UNUSED(tokens);
...
#define CHUNK_SIZE (4096)
    ...
    for (i = 0; i < ITERATIONS; i++) {
        assert(css__parser_create("UTF-8", CSS_CHARSET_DICTATED, &parser) == CSS_OK);

        params.event_handler.handler = event_handler;
        params.event_handler.pw = NULL;
        assert(css__parser_setopt(parser, CSS_PARSER_EVENT_HANDLER, &params) == CSS_OK);
