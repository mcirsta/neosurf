...
#define APPEND(lexer, data, len)                                                                                       \
    do {                                                                                                               \
        css_error error;                                                                                               \
        error = appendToTokenData((lexer), (const uint8_t *)(data), (len));                                            \
        if (error != CSS_OK)                                                                                           \
            return error;                                                                                              \
...
static css_error appendToTokenData(css_lexer *lexer, const uint8_t *data, size_t len);
static css_error emitToken(css_lexer *lexer, css_token_type type, css_token **token);
...
static css_error NumberOrPercentageOrDimension(css_lexer *lexer, css_token **token);
...
static css_error URIOrUnicodeRangeOrIdentOrFunction(css_lexer *lexer, css_token **token);
...
css_error css__lexer_get_token(css_lexer *lexer, css_token **token)
{
    ...
    case sCOMMENT:
        ...
        if (!lexer->emit_comments && error == CSS_OK && (*token)->type == CSS_TOKEN_COMMENT)
            goto start;
        return error;
    case sMATCH:
...
css_error emitToken(css_lexer *lexer, css_token_type type, css_token **token)
{
    ...
    if (lexer->escapeSeen) {
        ...
        error = parserutils_inputstream_peek(lexer->input, 0, &data, &clen);
    ...
    case CSS_TOKEN_STRING:
        ...
        if (t->data.len > 0 && (t->data.data[t->data.len - 1] == '"' || t->data.data[t->data.len - 1] == '\'')) {
            t->data.len -= 1;
        }
        break;
    ...
    case CSS_TOKEN_URI:
        ...
        while (t->data.len > 0 && isSpace(t->data.data[t->data.len - 1])) {
            t->data.len--;
        }
...
css_error NumberOrPercentageOrDimension(css_lexer *lexer, css_token **token)
{
    ...
    css_error error;
    parserutils_error perror;
    enum { Initial = 0, Dot = 1, MoreDigits = 2, Suffix = 3, NMChars = 4, Escape = 5 };
    ...
    case Dot:
        ...
        if (perror == PARSERUTILS_EOF) {
            if (t->data.len == 1 && (lexer->context.first == '.' || lexer->context.first == '+'))
                return emitToken(lexer, CSS_TOKEN_CHAR, token);
            else
...
css_error String(css_lexer *lexer, css_token **token)
{
    ...
    return emitToken(lexer, error == CSS_OK ? CSS_TOKEN_STRING : CSS_TOKEN_INVALID_STRING, token);
}

css_error URIOrUnicodeRangeOrIdentOrFunction(css_lexer *lexer, css_token **token)
{
    const uint8_t *cptr;
    uint8_t c;
    size_t clen;
...
css_error URI(css_lexer *lexer, css_token **token)
{
    ...
    css_error error;
    parserutils_error perror;
    enum { Initial = 0, LParen = 1, W1 = 2, Quote = 3, URL = 4, W2 = 5, RParen = 6, String = 7 };
...
css_error consumeEscape(css_lexer *lexer, bool nl)
{
    ...
    if (!lexer->escapeSeen) {
        if (lexer->bytesReadForToken > 1) {
            perror = parserutils_inputstream_peek(lexer->input, 0, &sdata, &slen);

            assert(perror == PARSERUTILS_OK);

            /* -1 to skip '\\' */
            perror = parserutils_buffer_append(lexer->unescapedTokenData, sdata, lexer->bytesReadForToken - 1);
            if (perror != PARSERUTILS_OK)
                return css_error_from_parserutils_error(perror);
        }
    ...
    if (nl && c == '\r') {
        perror = parserutils_inputstream_peek(lexer->input, lexer->bytesReadForToken + clen, &cptr, &clen);
        if (perror != PARSERUTILS_OK && perror != PARSERUTILS_EOF)
            return css_error_from_parserutils_error(perror);
...
css_error consumeUnicode(css_lexer *lexer, uint32_t ucs)
{
    ...
    if (ucs > 0x10FFFF || ucs <= 0x0008 || ucs == 0x000B || (0x000E <= ucs && ucs <= 0x001F) ||
        (0x007F <= ucs && ucs <= 0x009F) || (0xD800 <= ucs && ucs <= 0xDFFF) || (0xFDD0 <= ucs && ucs <= 0xFDEF) ||
        (ucs & 0xFFFE) == 0xFFFE) {
        ucs = 0xFFFD;
    } else if (ucs == 0x000D) {
        ucs = 0x000A;
    ...
    if (perror == PARSERUTILS_OK && *cptr == '\r') {
        ...
        perror = parserutils_inputstream_peek(lexer->input, lexer->bytesReadForToken + 1, &cptr, &clen);
        if (perror != PARSERUTILS_OK && perror != PARSERUTILS_EOF) {
            /* Rewind what we've read */
            lexer->bytesReadForToken = bytesReadInit;
            return css_error_from_parserutils_error(perror);
...
{
    return c == '_' || ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || ('0' <= c && c <= '9') || c == '-' ||
        c >= 0x80 || c == '\\';
}

bool startNMStart(uint8_t c)
{
    return c == '_' || ('a' <= c && c <= 'z') || ('A' <= c && c <= 'Z') || c >= 0x80 || c == '\\';
}
