...
static parserutils_error try_utf32_charset(const uint8_t *data, size_t len, uint16_t *result);
static parserutils_error try_utf16_charset(const uint8_t *data, size_t len, uint16_t *result);
static parserutils_error try_ascii_compatible_charset(const uint8_t *data, size_t len, uint16_t *result);
 ...
 * \return PARSERUTILS_OK on success, appropriate error otherwise
 */
parserutils_error css_charset_read_bom_or_charset(const uint8_t *data, size_t len, uint16_t *mibenum)
{
    parserutils_error error;
    uint16_t charset = 0;
    ...
    if (data[0] == 0x00 && data[1] == 0x00 && data[2] == 0xFE && data[3] == 0xFF) {
        ...
        charset = parserutils_charset_mibenum_from_name("UTF-16BE", SLEN("UTF-16BE"));
    } else if (data[0] == 0xFF && data[1] == 0xFE) {
        charset = parserutils_charset_mibenum_from_name("UTF-16LE", SLEN("UTF-16LE"));
...
#define CHARSET_LE "@\0\0\0c\0\0\0h\0\0\0a\0\0\0r\0\0\0s\0\0\0e\0\0\0t\0\0\0 \0\0\0\"\0\0\0"
    ...
    if (memcmp(data, CHARSET_LE, SLEN(CHARSET_LE)) == 0) {
        ...
        for (end = start; end < data + len - 4; end += 4) {
            uint32_t c = end[0] | (end[1] << 8) | (end[2] << 16) | (end[3] << 24);
            ...
            /* Reached the end? */
            if (c == '"' && end < data + len - 8) {
                uint32_t d = end[4] | (end[5] << 8) | (end[6] << 16) | (end[7] << 24);
        ...
        if ((ptr - buf == SLEN("UTF-32LE") && memcmp(buf, "UTF-32LE", ptr - buf) == 0) ||
            (ptr - buf == SLEN("UTF-32") && memcmp(buf, "UTF-32", ptr - buf) == 0)) {
            /* Convert to MIB enum */
            charset = parserutils_charset_mibenum_from_name("UTF-32LE", SLEN("UTF-32LE"));
        }
        ...
        /* Look for "; at end of charset declaration */
        for (end = start; end < data + len - 4; end += 4) {
            uint32_t c = end[3] | (end[2] << 8) | (end[1] << 16) | (end[0] << 24);
            ...
            /* Reached the end? */
            if (c == '"' && end < data + len - 8) {
                uint32_t d = end[7] | (end[6] << 8) | (end[5] << 16) | (end[4] << 24);
        ...
        if ((ptr - buf == SLEN("UTF-32BE") && memcmp(buf, "UTF-32BE", ptr - buf) == 0) ||
            (ptr - buf == SLEN("UTF-32") && memcmp(buf, "UTF-32", ptr - buf) == 0)) {
            /* Convert to MIB enum */
            charset = parserutils_charset_mibenum_from_name("UTF-32BE", SLEN("UTF-32BE"));
        }
...
#define CHARSET_LE "@\0c\0h\0a\0r\0s\0e\0t\0 \0\"\0"
    ...
    if (memcmp(data, CHARSET_LE, SLEN(CHARSET_LE)) == 0) {
        ...
        if ((ptr - buf == SLEN("UTF-16LE") && memcmp(buf, "UTF-16LE", ptr - buf) == 0) ||
            (ptr - buf == SLEN("UTF-16") && memcmp(buf, "UTF-16", ptr - buf) == 0)) {
            /* Convert to MIB enum */
            charset = parserutils_charset_mibenum_from_name("UTF-16LE", SLEN("UTF-16LE"));
        }
    } else if (memcmp(data, CHARSET_BE, SLEN(CHARSET_BE)) == 0) {
        ...
        if ((ptr - buf == SLEN("UTF-16BE") && memcmp(buf, "UTF-16BE", ptr - buf) == 0) ||
            (ptr - buf == SLEN("UTF-16") && memcmp(buf, "UTF-16", ptr - buf) == 0)) {
            /* Convert to MIB enum */
            charset = parserutils_charset_mibenum_from_name("UTF-16BE", SLEN("UTF-16BE"));
        }
...
parserutils_error try_ascii_compatible_charset(const uint8_t *data, size_t len, uint16_t *result)
{
    uint16_t charset = 0;
...
#define CHARSET "@charset \""
    ...
    if (memcmp(data, CHARSET, SLEN(CHARSET)) == 0) {
        ...
        /* Convert to MIB enum */
        charset = parserutils_charset_mibenum_from_name((const char *)start, end - start);
        ...
        if (charset == parserutils_charset_mibenum_from_name("UTF-32", SLEN("UTF-32")) ||
            charset == parserutils_charset_mibenum_from_name("UTF-32LE", SLEN("UTF-32LE")) ||
            charset == parserutils_charset_mibenum_from_name("UTF-32BE", SLEN("UTF-32BE")) ||
            charset == parserutils_charset_mibenum_from_name("UTF-16", SLEN("UTF-16")) ||
            charset == parserutils_charset_mibenum_from_name("UTF-16LE", SLEN("UTF-16LE")) ||
            charset == parserutils_charset_mibenum_from_name("UTF-16BE", SLEN("UTF-16BE"))) {
