...
#ifndef NDEBUG
#include <stdio.h>
extern void parserutils_stack_dump(parserutils_stack *stack, const char *prefix, void (*printer)(void *item));
extern void parserutils_vector_dump(parserutils_vector *vector, const char *prefix, void (*printer)(void *item));
#ifdef DEBUG_STACK
...
typedef struct parser_state {
    unsigned int state : 16, substate : 16;
} parser_state;
...
static css_error css__parser_create_internal(
    const char *charset, css_charset_source cs_source, parser_state initial, css_parser **parser);

static css_error transition(css_parser *parser, parser_state to, parser_state subsequent);
...
css_error css__parser_create(const char *charset, css_charset_source cs_source, css_parser **parser)
{
    ...
    return css__parser_create_internal(charset, cs_source, initial, parser);
}
 ...
 *         CSS_NOMEM on memory exhaustion
 */
css_error css__parser_create_internal(
    const char *charset, css_charset_source cs_source, parser_state initial, css_parser **parser)
{
    css_parser *p;
    parserutils_error perror;
    ...
    perror = parserutils_inputstream_create(charset, cs_source, css__charset_extract, &p->stream);
    if (perror != PARSERUTILS_OK) {
        free(p);
        return css_error_from_parserutils_error(perror);
    }
    ...
    perror = parserutils_stack_create(sizeof(parser_state), STACK_CHUNK, &p->states);
    if (perror != PARSERUTILS_OK) {
    ...
    perror = parserutils_vector_create(sizeof(css_token), STACK_CHUNK, &p->tokens);
    if (perror != PARSERUTILS_OK) {
    ...
    perror = parserutils_stack_create(sizeof(char), STACK_CHUNK, &p->open_items);
    if (perror != PARSERUTILS_OK) {
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error transition(css_parser *parser, parser_state to, parser_state subsequent)
{
...
css_error getToken(css_parser *parser, const css_token **token)
{
    ...
    if (parser->pushback != NULL) {
        *token = parser->pushback;
        ...
        if (t->type < CSS_TOKEN_LAST_INTERN && t->data.data != NULL) {
            ...
            lerror = lwc_intern_string((char *)t->data.data, t->data.len, &t->idata);
    ...
    perror = parserutils_vector_append(parser->tokens, (css_token *)(*token));
    if (perror != PARSERUTILS_OK)
        return css_error_from_parserutils_error(perror);
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
static css_error emit(css_parser *parser, css_parser_event type, const parserutils_vector *tokens, bool graceful)
{
    css_error error = CSS_OK;
...
css_error parseRuleset(css_parser *parser)
{
...
#endif
            error = emit(parser, CSS_PARSER_START_RULESET, NULL, false);
    ...
    case Brace:
        ...
        if (parser->parseError == false) {
            error = emit(parser, CSS_PARSER_START_RULESET, parser->tokens, false);
            if (error == CSS_INVALID) {
                parser->parseError = true;
            } else if (error != CSS_OK) {
                return error;
...
css_error parseAtRule(css_parser *parser)
{
    ...
    case AfterAny:
        ...
        if (token->type == CSS_TOKEN_CHAR && lwc_string_length(token->idata) == 1) {
            if (lwc_string_data(token->idata)[0] == ')' || lwc_string_data(token->idata)[0] == ']') {
                parser_state to = {sAny0, Initial};
                parser_state subsequent = {sAtRule, AfterAny};
...
css_error parseAtRuleEnd(css_parser *parser)
{
    ...
    case Initial:
        ...
        parserutils_vector_dump(parser->tokens, __func__, tprinter);
#endif
        error = emit(parser, CSS_PARSER_START_ATRULE, parser->tokens, false);
        if (error == CSS_INVALID) {
            parser_state to = {sMalformedAtRule, Initial};
        ...
        case Initial:
            ...
            if (token->type == CSS_TOKEN_ATKEYWORD) {
                ...
                if (lwc_string_length(token->idata) == 1 && lwc_string_data(token->idata)[0] == '{') {
                    ...
                    parserutils_vector_dump(parser->tokens, __func__, tprinter);
#endif
                    error = emit(parser, CSS_PARSER_BLOCK_CONTENT, parser->tokens, true);
                    if (error != CSS_OK) {
                        return error;
                    }
                    ...
                    return transition(parser, to, subsequent);
                } else if (lwc_string_length(token->idata) == 1 && lwc_string_data(token->idata)[0] == ';') {
                    ...
                    parserutils_vector_dump(parser->tokens, __func__, tprinter);
#endif
                    error = emit(parser, CSS_PARSER_END_BLOCK_CONTENT, parser->tokens, true);
                    if (error != CSS_OK) {
                        return error;
                    }
...
css_error parseDeclListEnd(css_parser *parser)
{
    ...
    case Initial:
        ...
        if (token->type != CSS_TOKEN_CHAR || lwc_string_length(token->idata) != 1 ||
            (lwc_string_data(token->idata)[0] != ';' && lwc_string_data(token->idata)[0] != '}')) {
            parser_state to = {sDeclaration, Initial};
            parser_state subsequent = {sDeclListEnd, AfterDeclaration};
...
css_error parseAny(css_parser *parser)
{
    ...
    case Initial:
        ...
        if (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_NUMBER && token->type != CSS_TOKEN_PERCENTAGE &&
            token->type != CSS_TOKEN_DIMENSION && token->type != CSS_TOKEN_STRING && token->type != CSS_TOKEN_CHAR &&
        ...
        if (token->type == CSS_TOKEN_FUNCTION) {
            ...
            (lwc_string_data(token->idata)[0] == '(' || lwc_string_data(token->idata)[0] == '[')) {
            parserutils_stack_push(parser->open_items, &(lwc_string_data(token->idata)[0] == '(' ? ")" : "]")[0]);
            state->substate = WS;
        } else {
            state->substate = WS2;
        }
    ...
    case AfterAny0: {
        ...
        if (token->type == CSS_TOKEN_CHAR && lwc_string_length(token->idata) == 1 &&
            lwc_string_data(token->idata)[0] == ((uint8_t *)parserutils_stack_get_current(parser->open_items))[0]) {
...
css_error parseMalformedSelector(css_parser *parser)
{
    ...
    case Go:
        while (1) {
            ...
            if (want == '{' && match != NULL && parserutils_stack_get_current(parser->open_items) == NULL)
                break;
        }
    }
...
static void tprinter(void *token)
{
    ...
    if (t->idata) {
        printf("%d: %.*s", t->type, (int)lwc_string_length(t->idata), lwc_string_data(t->idata));
    } else if (t->data.data)
