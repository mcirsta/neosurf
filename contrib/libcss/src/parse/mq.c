...
static void css__mq_cond_or_feature_destroy(css_mq_cond_or_feature *cond_or_feature);
...
static css_error mq_parse_condition(
    lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, bool permit_or, css_mq_cond **cond);

static css_error
mq_parse_ratio(const parserutils_vector *vector, int32_t *ctx, const css_token *numerator, css_fixed *ratio)
{
...
static css_error mq_populate_value(css_mq_value *value, const css_token *token)
{
    if (token->type == CSS_TOKEN_NUMBER) {
        ...
        value->data.num_or_ratio = css__number_from_lwc_string(token->idata, false, &num_len);
...
static css_error mq_parse_range__convert_to_level_4(css_mq_feature *feature)
{
    ...
    if (feature->op != CSS_MQ_FEATURE_OP_EQ || lwc_string_length(feature->name) <= 4) {
        return CSS_OK;
    }

    if (name[0] == 'm' && name[3] == '-') {
        if (name[1] == 'i' && name[2] == 'n') {
            if (lwc_intern_substring(feature->name, 4, lwc_string_length(feature->name) - 4, &new_name) !=
                lwc_error_ok) {
                return CSS_NOMEM;
            }
            lwc_string_unref(feature->name);
...
static css_error mq_parse_range(lwc_string **strings, const parserutils_vector *vector, int32_t *ctx,
    const css_token *name_or_value, css_mq_feature **feature)
{
    ...
    if (name_or_value == NULL ||
        (name_or_value->type != CSS_TOKEN_NUMBER && name_or_value->type != CSS_TOKEN_DIMENSION &&
    ...
    /* Name-or-value */
    if (name_or_value->type == CSS_TOKEN_NUMBER && tokenIsChar(parserutils_vector_peek(vector, *ctx), '/')) {
        /* ratio */
        ...
        lwc_string_caseless_isequal(name_or_value->idata, strings[INFINITE], &match) == lwc_error_ok &&
        match == false) {
    ...
    if (value_or_name == NULL ||
        (value_or_name->type != CSS_TOKEN_NUMBER && value_or_name->type != CSS_TOKEN_DIMENSION &&
    ...
    if (value_or_name->type == CSS_TOKEN_NUMBER && tokenIsChar(parserutils_vector_peek(vector, *ctx), '/')) {
        /* ratio */
    ...
    if (name_first == false && token != NULL && tokenIsChar(token, ')') == false) {
        ...
        if (value2 == NULL ||
            (value2->type != CSS_TOKEN_NUMBER && value2->type != CSS_TOKEN_DIMENSION &&
...
static css_error
mq_parse_media_feature(lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, css_mq_feature **feature)
{
    ...
    if (name_or_value->type == CSS_TOKEN_IDENT) {
        ...
        if (tokenIsChar(token, ')')) {
            ...
            if (token->type == CSS_TOKEN_NUMBER && tokenIsChar(parserutils_vector_peek(vector, *ctx), '/')) {
                /* ratio */
                css_fixed ratio;
            ...
            /* mf-range */
            error = mq_parse_range(strings, vector, ctx, name_or_value, &result);
            if (error != CSS_OK) {
                return error;
            }
 ...
 */
static css_error mq_parse_consume_any_value(
    lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, bool until, const char until_char)
{
    const css_token *token;
    css_error error;
    ...
    while (true) {
        ...
        case CSS_TOKEN_CHAR:
            ...
            if (tokenIsChar(token, '(')) {
                ...
                error = mq_parse_consume_any_value(strings, vector, ctx, true, ')');
                if (error != CSS_OK) {
                    return error;
                }
            } else if (tokenIsChar(token, '[')) {
...
static css_error mq_parse_general_enclosed(lwc_string **strings, const parserutils_vector *vector, int32_t *ctx)
{
    const css_token *token;
    css_error error;
    ...
    case CSS_TOKEN_IDENT:
        error = mq_parse_consume_any_value(strings, vector, ctx, false, '\0');
        if (error != CSS_OK) {
            return error;
        }
        break;
...
static css_error mq_parse_media_in_parens(
    lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, css_mq_cond_or_feature **cond_or_feature)
{
    const css_token *token;
    bool match;
    int old_ctx;
    ...
    if (tokenIsChar(token, '(') ||
        (token->type == CSS_TOKEN_IDENT &&
            lwc_string_caseless_isequal(token->idata, strings[NOT], &match) == lwc_error_ok && match)) {
        css_mq_cond *cond;
    ...
    } else if (token->type == CSS_TOKEN_IDENT || token->type == CSS_TOKEN_NUMBER ||
    ...
    lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, bool permit_or, css_mq_cond **cond)
{
    ...
    token = parserutils_vector_peek(vector, *ctx);
    if (token == NULL ||
        (tokenIsChar(token, '(') == false && token->type != CSS_TOKEN_IDENT &&
            lwc_string_caseless_isequal(token->idata, strings[NOT], &match) != lwc_error_ok && match == false)) {
        return CSS_INVALID;
    }
    ...
    if (tokenIsChar(token, '(') == false) {
        ...
        error = mq_parse_media_in_parens(strings, vector, ctx, &cond_or_feature);
        if (error != CSS_OK) {
    ...
    while (token != NULL && tokenIsChar(token, ')') == false && tokenIsChar(token, ',') == false) {
        error = mq_parse_media_in_parens(strings, vector, ctx, &cond_or_feature);
        if (error != CSS_OK) {
            css__mq_cond_destroy(result);
            return CSS_INVALID;
        ...
        parts = realloc(result->parts, (result->nparts + 1) * sizeof(*result->parts));
        if (parts == NULL) {
            css__mq_cond_or_feature_destroy(cond_or_feature);
            css__mq_cond_destroy(result);
            return CSS_NOMEM;
        ...
        token = parserutils_vector_peek(vector, *ctx);
        if (token != NULL && tokenIsChar(token, ')') == false && tokenIsChar(token, ',') == false) {
            if (token->type != CSS_TOKEN_IDENT) {
                css__mq_cond_destroy(result);
                return CSS_INVALID;
            } else if (lwc_string_caseless_isequal(token->idata, strings[AND], &match) == lwc_error_ok && match) {
                if (op != 0 && op != AND) {
                    css__mq_cond_destroy(result);
                    return CSS_INVALID;
                }
                op = AND;
            } else if (lwc_string_caseless_isequal(token->idata, strings[OR], &match) == lwc_error_ok && match) {
 ...
 */
static css_error mq_parse_type(lwc_string **strings, lwc_string *type, uint64_t *result)
{
    bool match;
    css_error error = CSS_OK;
    ...
    } else if (lwc_string_caseless_isequal(type, strings[AURAL], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_AURAL;
    } else if (lwc_string_caseless_isequal(type, strings[BRAILLE], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_BRAILLE;
    } else if (lwc_string_caseless_isequal(type, strings[EMBOSSED], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_EMBOSSED;
    } else if (lwc_string_caseless_isequal(type, strings[HANDHELD], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_HANDHELD;
    } else if (lwc_string_caseless_isequal(type, strings[PRINT], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_PRINT;
    } else if (lwc_string_caseless_isequal(type, strings[PROJECTION], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_PROJECTION;
    } else if (lwc_string_caseless_isequal(type, strings[SCREEN], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_SCREEN;
    } else if (lwc_string_caseless_isequal(type, strings[SPEECH], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_SPEECH;
    } else if (lwc_string_caseless_isequal(type, strings[TTY], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_TTY;
    } else if (lwc_string_caseless_isequal(type, strings[TV], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_TV;
    } else if (lwc_string_caseless_isequal(type, strings[ALL], &match) == lwc_error_ok && match) {
        *result = CSS_MEDIA_ALL;
    } else if (lwc_string_caseless_isequal(type, strings[NOT], &match) == lwc_error_ok && match) {
        error = CSS_INVALID;
    } else if (lwc_string_caseless_isequal(type, strings[AND], &match) == lwc_error_ok && match) {
        error = CSS_INVALID;
    } else if (lwc_string_caseless_isequal(type, strings[OR], &match) == lwc_error_ok && match) {
        error = CSS_INVALID;
    } else if (lwc_string_caseless_isequal(type, strings[ONLY], &match) == lwc_error_ok && match) {
        error = CSS_INVALID;
    } else {
...
static css_error
mq_parse_media_query(lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, css_mq_query **query)
{
    const css_token *token;
    ...
    if (is_condition) {
        /* media-condition */
        error = mq_parse_condition(strings, vector, ctx, true, &result->cond);
        if (error != CSS_OK) {
            free(result);
            return error;
        }
    ...
    if (lwc_string_caseless_isequal(token->idata, strings[NOT], &match) == lwc_error_ok && match) {
        result->negate_type = 1;
        consumeWhitespace(vector, ctx);
        token = parserutils_vector_iterate(vector, ctx);
    } else if (lwc_string_caseless_isequal(token->idata, strings[ONLY], &match) == lwc_error_ok && match) {
        consumeWhitespace(vector, ctx);
        token = parserutils_vector_iterate(vector, ctx);
    }
    ...
    if (token != NULL) {
        if (token->type != CSS_TOKEN_IDENT ||
            lwc_string_caseless_isequal(token->idata, strings[AND], &match) != lwc_error_ok || match == false) {
            free(result);
            return CSS_INVALID;
        }
        ...
        error = mq_parse_condition(strings, vector, ctx, false, &result->cond);
        if (error != CSS_OK) {
            free(result);
            return error;
        }
...
css_error
css__mq_parse_media_list(lwc_string **strings, const parserutils_vector *vector, int32_t *ctx, css_mq_query **media)
{
...
static css_error css_parse_media_query_handle_event(css_parser_event type, const parserutils_vector *tokens, void *pw)
{
    int32_t idx = 0;
    css_error err;
    css_mq_query *media;
...
css_error css_parse_media_query(lwc_string **strings, const uint8_t *mq, size_t len, css_mq_query **media_out)
{
    ...
    err = css__parser_create_for_media_query(NULL, CSS_CHARSET_DEFAULT, &parser);
    if (err != CSS_OK) {
        return err;
    }
    ...
    err = css__parser_setopt(parser, CSS_PARSER_EVENT_HANDLER, &params_handler);
    if (err != CSS_OK) {
        css__parser_destroy(parser);
        return err;
    }

    err = css__parser_parse_chunk(parser, (const uint8_t *)"@media ", strlen("@media "));
