...
/* Event handlers */
static css_error language_handle_event(css_parser_event type, const parserutils_vector *tokens, void *pw);
static css_error handleStartStylesheet(css_language *c, const parserutils_vector *vector);
static css_error handleEndStylesheet(css_language *c, const parserutils_vector *vector);
static css_error handleStartRuleset(css_language *c, const parserutils_vector *vector);
static css_error handleEndRuleset(css_language *c, const parserutils_vector *vector);
static css_error handleStartAtRule(css_language *c, const parserutils_vector *vector);
static css_error handleEndAtRule(css_language *c, const parserutils_vector *vector);
static css_error handleStartBlock(css_language *c, const parserutils_vector *vector);
static css_error handleEndBlock(css_language *c, const parserutils_vector *vector);
static css_error handleBlockContent(css_language *c, const parserutils_vector *vector);
static css_error handleEndBlockContent(css_language *c, const parserutils_vector *vector);
static css_error handleDeclaration(css_language *c, const parserutils_vector *vector);

/* At-rule parsing */
static css_error addNamespace(css_language *c, lwc_string *prefix, lwc_string *uri);
static css_error lookupNamespace(css_language *c, lwc_string *prefix, lwc_string **uri);

/* Selector list parsing */
static css_error
parseClass(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail *specific);
static css_error
parseAttrib(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail *specific);
static css_error
parseNth(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail_value *value);
static css_error parsePseudo(
    css_language *c, const parserutils_vector *vector, int32_t *ctx, bool in_not, css_selector_detail *specific);
static css_error parseSpecific(
    css_language *c, const parserutils_vector *vector, int32_t *ctx, bool in_not, css_selector_detail *specific);
static css_error
parseAppendSpecific(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **parent);
static css_error
parseSelectorSpecifics(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **parent);
static css_error parseTypeSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_qname *qname);
static css_error
parseSimpleSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **result);
static css_error
parseCombinator(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_combinator *result);
static css_error parseSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **result);
...
css_error css__language_create(css_stylesheet *sheet, css_parser *parser, void **language)
{
    ...
    perror = parserutils_stack_create(sizeof(context_entry), STACK_CHUNK, &c->context);
 ...
 */
css_error language_handle_event(css_parser_event type, const parserutils_vector *tokens, void *pw)
{
...
css_error handleStartStylesheet(css_language *c, const parserutils_vector *vector)
{
    parserutils_error perror;
...
css_error handleStartAtRule(css_language *c, const parserutils_vector *vector)
{
    ...
    if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[CHARSET], &match) == lwc_error_ok && match) {
        if (c->state == CHARSET_PERMITTED) {
            ...
            if (charset == NULL || charset->type != CSS_TOKEN_STRING)
                return CSS_INVALID;
            ...
            error = css__stylesheet_rule_create(c->sheet, CSS_RULE_CHARSET, &rule);
            if (error != CSS_OK)
                return error;

            error = css__stylesheet_rule_set_charset(c->sheet, rule, charset->idata);
            if (error != CSS_OK) {
                css__stylesheet_rule_destroy(c->sheet, rule);
                return error;
            }
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[LIBCSS_IMPORT], &match) == lwc_error_ok &&
        match) {
        if (c->state <= IMPORT_PERMITTED) {
            ...
            if (uri == NULL || (uri->type != CSS_TOKEN_STRING && uri->type != CSS_TOKEN_URI))
                return CSS_INVALID;
            ...
            /* Create rule */
            error = css__stylesheet_rule_create(c->sheet, CSS_RULE_IMPORT, &rule);
            if (error != CSS_OK) {
            ...
            /* Inform rule of it */
            error = css__stylesheet_rule_set_nascent_import(c->sheet, rule, url, media);
            if (error != CSS_OK) {
            ...
            if (c->sheet->import != NULL) {
                error = c->sheet->import(c->sheet->import_pw, c->sheet, url);
                if (error != CSS_OK) {
                    lwc_string_unref(url);
                    css__stylesheet_rule_destroy(c->sheet, rule);
                    return error;
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[NAMESPACE], &match) == lwc_error_ok && match) {
        if (c->state <= NAMESPACE_PERMITTED) {
            ...
            if (token == NULL || (token->type != CSS_TOKEN_STRING && token->type != CSS_TOKEN_URI)) {
                return CSS_INVALID;
            }
    ...
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[MEDIA], &match) == lwc_error_ok && match) {
        css_mq_query *media = NULL;
        ...
        c->state = HAD_RULE;
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[FONT_FACE], &match) == lwc_error_ok && match) {
        error = css__stylesheet_rule_create(c->sheet, CSS_RULE_FONT_FACE, &rule);
        if (error != CSS_OK)
            return error;
        ...
        c->state = HAD_RULE;
    } else if (lwc_string_caseless_isequal(atkeyword->idata, c->strings[PAGE], &match) == lwc_error_ok && match) {
        const css_token *token;

        /* any0 = (':' IDENT)? ws */

        error = css__stylesheet_rule_create(c->sheet, CSS_RULE_PAGE, &rule);
        if (error != CSS_OK)
            return error;
        ...
        if (token != NULL) {
            ...
            error = css__stylesheet_rule_set_page_selector(c->sheet, rule, sel);
            if (error != CSS_OK) {
...
css_error handleEndBlock(css_language *c, const parserutils_vector *vector)
{
    ...
    if (rule != NULL && rule->type == CSS_RULE_FONT_FACE) {
        ...
        if (c->sheet->font_face != NULL && ff_rule->font_face != NULL) {
            css_error error = c->sheet->font_face(c->sheet->font_face_pw, c->sheet, ff_rule->font_face);
            if (error != CSS_OK)
                return error;
        }
    }
...
css_error handleDeclaration(css_language *c, const parserutils_vector *vector)
{
    ...
    rule = entry->data;
    if (rule == NULL ||
        (rule->type != CSS_RULE_SELECTOR && rule->type != CSS_RULE_PAGE && rule->type != CSS_RULE_FONT_FACE))
        return CSS_INVALID;
...
css_error addNamespace(css_language *c, lwc_string *prefix, lwc_string *uri)
{
    if (prefix == NULL) {
        ...
        for (idx = 0; idx < c->num_namespaces; idx++) {
            if (lwc_string_isequal(c->namespaces[idx].prefix, prefix, &match) == lwc_error_ok && match)
                break;
        }

        if (idx == c->num_namespaces) {
            /* Not found, create a new mapping */
            css_namespace *ns = realloc(c->namespaces, sizeof(css_namespace) * (c->num_namespaces + 1));
...
css_error parseClass(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail *specific)
{
    css_qname qname;
    ...
    if (qname.name->insensitive == NULL && lwc__intern_caseless_string(qname.name) != lwc_error_ok)
        return CSS_NOMEM;

    return css__stylesheet_selector_detail_init(
        c->sheet, CSS_SELECTOR_CLASS, &qname, detail_value, CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
}

css_error parseAttrib(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail *specific)
{
    css_qname qname;
    css_selector_detail_value detail_value;
    ...
    token = parserutils_vector_iterate(vector, ctx);
    if (token == NULL ||
        (token->type != CSS_TOKEN_IDENT && tokenIsChar(token, '*') == false && tokenIsChar(token, '|') == false))
        return CSS_INVALID;
    ...
    return css__stylesheet_selector_detail_init(
        c->sheet, type, &qname, detail_value, CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
}

css_error parseNth(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector_detail_value *value)
{
    const css_token *token;
    bool match;
    ...
    token = parserutils_vector_iterate(vector, ctx);
    if (token == NULL ||
        (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_NUMBER && token->type != CSS_TOKEN_DIMENSION))
        return CSS_INVALID;

    if (token->type == CSS_TOKEN_IDENT &&
        lwc_string_caseless_isequal(token->idata, c->strings[ODD], &match) == lwc_error_ok && match) {
        /* Odd */
        value->nth.a = 2;
        value->nth.b = 1;
    } else if (token->type == CSS_TOKEN_IDENT &&
        lwc_string_caseless_isequal(token->idata, c->strings[EVEN], &match) == lwc_error_ok && match) {
        /* Even */
        value->nth.a = 2;
        value->nth.b = 0;
        ...
        val = css__number_from_lwc_string(token->idata, true, &consumed);
        ...
        if (token->type == CSS_TOKEN_IDENT) {
            ...
            if (len > 0) {
                ...
                if (len > 1) {
                    ...
                    /* -n-b */
                    b = css__number_from_string((const uint8_t *)data + 1, len - 1, true, &consumed);
                    if (consumed != len - 1)
                        return CSS_INVALID;
            ...
            /* 2n */
            a = css__number_from_lwc_string(token->idata, true, &consumed);
            if (consumed == 0 || (data[consumed] != 'n' && data[consumed] != 'N'))
                return CSS_INVALID;
            ...
            if (++consumed < len) {
                ...
                if (++consumed < len) {
                    ...
                    b = css__number_from_string((const uint8_t *)data + bstart, len - bstart, true, &consumed);
        ...
        if (had_b == false) {
            ...
            if (had_sign == false && token != NULL && (tokenIsChar(token, '-') || tokenIsChar(token, '+'))) {
                parserutils_vector_iterate(vector, ctx);
            ...
            if (token != NULL && token->type == CSS_TOKEN_NUMBER) {
                 ...
                 */
                if (had_sign && lwc_string_length(token->idata) > 0) {
                    data = lwc_string_data(token->idata);
                ...
                b = css__number_from_lwc_string(token->idata, true, &consumed);
                if (consumed != lwc_string_length(token->idata))
                    return CSS_INVALID;
            }
        }
...
css_error
parsePseudo(css_language *c, const parserutils_vector *vector, int32_t *ctx, bool in_not, css_selector_detail *specific)
{
    static const struct {
        int index;
        css_selector_type type;
    } pseudo_lut[] = {{FIRST_CHILD, CSS_SELECTOR_PSEUDO_CLASS}, {LINK, CSS_SELECTOR_PSEUDO_CLASS},
        ...
        {LAST_OF_TYPE, CSS_SELECTOR_PSEUDO_CLASS}, {ONLY_CHILD, CSS_SELECTOR_PSEUDO_CLASS},
        {ONLY_OF_TYPE, CSS_SELECTOR_PSEUDO_CLASS}, {EMPTY, CSS_SELECTOR_PSEUDO_CLASS},
        {TARGET, CSS_SELECTOR_PSEUDO_CLASS}, {ENABLED, CSS_SELECTOR_PSEUDO_CLASS},
    ...
    if (token == NULL || (token->type != CSS_TOKEN_IDENT && token->type != CSS_TOKEN_FUNCTION))
        return CSS_INVALID;
    ...
    for (lut_idx = 0; lut_idx < N_ELEMENTS(pseudo_lut); lut_idx++) {
        if ((lwc_string_caseless_isequal(qname.name, c->strings[pseudo_lut[lut_idx].index], &match) == lwc_error_ok) &&
            match) {
    ...
    if (in_not && (type == CSS_SELECTOR_PSEUDO_ELEMENT || pseudo_lut[lut_idx].index == NOT))
        return CSS_INVALID;
    ...
    if (token->type == CSS_TOKEN_FUNCTION) {
        ...
        if (fun_type == LANG) {
            ...
            if (token->type == CSS_TOKEN_IDENT || tokenIsChar(token, '*') || tokenIsChar(token, '|')) {
                /* Have type selector */
                ...
                error = parseSpecific(c, vector, ctx, true, &det);
                if (error != CSS_OK)
                    return error;
    ...
    return css__stylesheet_selector_detail_init(c->sheet, type, &qname, detail_value, value_type, negate, specific);
}

css_error parseSpecific(
    css_language *c, const parserutils_vector *vector, int32_t *ctx, bool in_not, css_selector_detail *specific)
{
    css_error error;
    const css_token *token;
    ...
    if (token->type == CSS_TOKEN_HASH) {
        ...
        error = css__stylesheet_selector_detail_init(
            c->sheet, CSS_SELECTOR_ID, &qname, detail_value, CSS_SELECTOR_DETAIL_VALUE_STRING, false, specific);
        if (error != CSS_OK)
            return error;
...
css_error parseAppendSpecific(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **parent)
{
    css_error error;
    ...
    return css__stylesheet_selector_append_specific(c->sheet, parent, &specific);
}

css_error parseSelectorSpecifics(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **parent)
{
    css_error error;
    const css_token *token;
    ...
    while ((token = parserutils_vector_peek(vector, *ctx)) != NULL && token->type != CSS_TOKEN_S &&
        tokenIsChar(token, '+') == false && tokenIsChar(token, '>') == false && tokenIsChar(token, '~') == false &&
...
css_error parseTypeSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_qname *qname)
{
    const css_token *token;
    css_error error;
    lwc_string *prefix = NULL;
    ...
    if (token != NULL && tokenIsChar(token, '|')) {
        ...
        if (token == NULL || (token->type != CSS_TOKEN_IDENT && tokenIsChar(token, '*') == false)) {
            return CSS_INVALID;
        }
...
css_error parseSimpleSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **result)
{
    ...
    if (token->type == CSS_TOKEN_IDENT || tokenIsChar(token, '*') || tokenIsChar(token, '|')) {
        ...
        error = css__stylesheet_selector_create(c->sheet, &qname, &selector);
        if (error != CSS_OK) {
            *ctx = orig_ctx;
            return error;
        }
...
css_error parseCombinator(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_combinator *result)
{
    const css_token *token;
...
css_error parseSelector(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_selector **result)
{
    css_error error;
    const css_token *token = NULL;
    ...
    while ((token = parserutils_vector_peek(vector, *ctx)) != NULL && tokenIsChar(token, ',') == false) {
        css_combinator comb = CSS_COMBINATOR_NONE;
        ...
        if (comb == CSS_COMBINATOR_ANCESTOR &&
            ((token = parserutils_vector_peek(vector, *ctx)) == NULL || tokenIsChar(token, ',')))
            continue;
        ...
        error = css__stylesheet_selector_combine(c->sheet, comb, selector, other);
        if (error != CSS_OK) {
            css__stylesheet_selector_destroy(c->sheet, selector);
...
css_error parseSelectorList(css_language *c, const parserutils_vector *vector, css_rule *rule)
{
    css_error error;
    const css_token *token = NULL;
    css_selector *selector = NULL;
