...
css_error css__parse_border_spacing(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        ...
        /* inherit */
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BORDER_SPACING);
    } else {
        int num_lengths = 0;
        ...
        if (unit[0] & UNIT_ANGLE || unit[0] & UNIT_TIME || unit[0] & UNIT_FREQ || unit[0] & UNIT_PCT) {
            *ctx = orig_ctx;
            return CSS_INVALID;
        }
        ...
        if (token != NULL) {
             ...
             */
            error = css__parse_unit_specifier(c, vector, ctx, UNIT_PX, &length[1], &unit[1]);
            if (error == CSS_OK) {
                if (unit[1] & UNIT_ANGLE || unit[1] & UNIT_TIME || unit[1] & UNIT_FREQ || unit[1] & UNIT_PCT) {
                    *ctx = orig_ctx;
                    return CSS_INVALID;
                }
        ...
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_BORDER_SPACING, 0, BORDER_SPACING_SET);
        ...
        error = css__stylesheet_style_vappend(result, 4, length[0], unit[0], length[1], unit[1]);
    }
