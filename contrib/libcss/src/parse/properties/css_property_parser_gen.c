...
void output_token_type_check(
    FILE *outputf, bool do_token_check, struct keyval_list *IDENT, struct keyval_list *URI, struct keyval_list *NUMBER)
{
    fprintf(outputf,
...
cleanup:
    ...
    for (int i = 0; i < IDENT.count; i++) {
        if (IDENT.item[i] != &ident_inherit && IDENT.item[i] != &ident_initial && IDENT.item[i] != &ident_revert &&
