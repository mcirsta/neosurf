...
css_error css__parse_column_rule(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_COLUMN_RULE_COLOR);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_COLUMN_RULE_STYLE);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_COLUMN_RULE_WIDTH);
        if (error == CSS_OK)
    ...
    do {
        ...
        if ((color) && (error = css__parse_column_rule_color(c, vector, ctx, color_style)) == CSS_OK) {
            color = false;
        } else if ((style) && (error = css__parse_column_rule_style(c, vector, ctx, style_style)) == CSS_OK) {
            style = false;
        } else if ((width) && (error = css__parse_column_rule_width(c, vector, ctx, width_style)) == CSS_OK) {
            width = false;
        }
    ...
    if (color) {
        error = css__stylesheet_style_appendOPV(color_style, CSS_PROP_COLUMN_RULE_COLOR, 0, COLUMN_RULE_COLOR_SET);
        if (error != CSS_OK)
            goto css__parse_column_rule_cleanup;
    ...
    if (style) {
        error = css__stylesheet_style_appendOPV(style_style, CSS_PROP_COLUMN_RULE_STYLE, 0, COLUMN_RULE_STYLE_NONE);
        if (error != CSS_OK)
            goto css__parse_column_rule_cleanup;
    }

    if (width) {
        error = css__stylesheet_style_appendOPV(width_style, CSS_PROP_COLUMN_RULE_WIDTH, 0, COLUMN_RULE_WIDTH_MEDIUM);
        if (error != CSS_OK)
            goto css__parse_column_rule_cleanup;
    }
