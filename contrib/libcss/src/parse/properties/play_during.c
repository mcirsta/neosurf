...
css_error css__parse_play_during(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (token->type == CSS_TOKEN_IDENT) {
        ...
        for (modifiers = 0; modifiers < 2; modifiers++) {
            ...
            if (token != NULL && token->type == CSS_TOKEN_IDENT) {
                if ((lwc_string_caseless_isequal(token->idata, c->strings[MIX], &match) == lwc_error_ok && match)) {
                ...
                } else if (lwc_string_caseless_isequal(token->idata, c->strings[REPEAT], &match) == lwc_error_ok &&
                    match) {
    ...
    if ((flags & FLAG_INHERIT) == false && (value & PLAY_DURING_TYPE_MASK) == PLAY_DURING_URI) {
