...
css_error css__parse_flex(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FLEX_GROW);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FLEX_SHRINK);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FLEX_BASIS);
        if (error == CSS_OK)
    ...
    /* defaults */
    if (grow) {
        error = css__stylesheet_style_appendOPV(grow_style, CSS_PROP_FLEX_GROW, 0, FLEX_GROW_SET);
        if (error != CSS_OK)
            goto css__parse_flex_cleanup;
    ...
    if (shrink) {
        error = css__stylesheet_style_appendOPV(shrink_style, CSS_PROP_FLEX_SHRINK, 0, FLEX_SHRINK_SET);
        if (error != CSS_OK)
            goto css__parse_flex_cleanup;
    ...
    if (basis) {
        ...
        if (!grow || !shrink) {
            error = css__stylesheet_style_appendOPV(basis_style, CSS_PROP_FLEX_BASIS, 0, FLEX_BASIS_SET);
            if (error != CSS_OK)
                goto css__parse_flex_cleanup;

            error = css__stylesheet_style_vappend(basis_style, 2, 0, UNIT_PX);
            if (error != CSS_OK)
                goto css__parse_flex_cleanup;

        } else {
            error = css__stylesheet_style_appendOPV(basis_style, CSS_PROP_FLEX_BASIS, 0, FLEX_BASIS_AUTO);
            if (error != CSS_OK)
                goto css__parse_flex_cleanup;
        }
    }
