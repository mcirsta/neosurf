...
css_error css__parse_content(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_CONTENT);
...
#define CSS_APPEND(CSSVAL) css__stylesheet_style_append(result, first ? buildOPV(CSS_PROP_CONTENT, 0, CSSVAL) : CSSVAL)
        ...
        while (token != NULL) {
            if ((token->type == CSS_TOKEN_IDENT) &&
                (lwc_string_caseless_isequal(token->idata, c->strings[OPEN_QUOTE], &match) == lwc_error_ok && match)) {

                error = CSS_APPEND(CONTENT_OPEN_QUOTE);

            } else if (token->type == CSS_TOKEN_IDENT &&
                (lwc_string_caseless_isequal(token->idata, c->strings[CLOSE_QUOTE], &match) == lwc_error_ok && match)) {

                error = CSS_APPEND(CONTENT_CLOSE_QUOTE);
            } else if (token->type == CSS_TOKEN_IDENT &&
                (lwc_string_caseless_isequal(token->idata, c->strings[NO_OPEN_QUOTE], &match) == lwc_error_ok &&
                    match)) {
                error = CSS_APPEND(CONTENT_NO_OPEN_QUOTE);
            } else if (token->type == CSS_TOKEN_IDENT &&
                (lwc_string_caseless_isequal(token->idata, c->strings[NO_CLOSE_QUOTE], &match) == lwc_error_ok &&
                    match)) {
                ...
                (lwc_string_caseless_isequal(token->idata, c->strings[ATTR], &match) == lwc_error_ok && match)) {
                uint32_t snumber;
                ...
                (lwc_string_caseless_isequal(token->idata, c->strings[COUNTER], &match) == lwc_error_ok && match)) {
                lwc_string *name;
                uint32_t snumber;
                uint32_t opv;
                ...
                if (tokenIsChar(token, ',')) {
                    ...
                    opv |= LIST_STYLE_TYPE_DECIMAL << CONTENT_COUNTER_STYLE_SHIFT;
                }
                ...
                (lwc_string_caseless_isequal(token->idata, c->strings[COUNTERS], &match) == lwc_error_ok && match)) {
                lwc_string *name;
                lwc_string *sep;
                ...
                if (tokenIsChar(token, ',')) {
                    ...
                    opv |= LIST_STYLE_TYPE_DECIMAL << CONTENT_COUNTERS_STYLE_SHIFT;
                }
