...
css_error css__parse_flex_flow(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FLEX_DIRECTION);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FLEX_WRAP);
        if (error == CSS_OK)
    ...
    do {
        ...
        if ((wrap) && (error = css__parse_flex_wrap(c, vector, ctx, wrap_style)) == CSS_OK) {
            wrap = false;
        } else if ((direction) && (error = css__parse_flex_direction(c, vector, ctx, direction_style)) == CSS_OK) {
            direction = false;
        }
    ...
    /* defaults */
    if (direction) {
        error = css__stylesheet_style_appendOPV(direction_style, CSS_PROP_FLEX_DIRECTION, 0, FLEX_DIRECTION_ROW);
        if (error != CSS_OK) {
            goto css__parse_flex_flow_cleanup;
        }
    }
    ...
    if (wrap) {
        error = css__stylesheet_style_appendOPV(wrap_style, CSS_PROP_FLEX_WRAP, 0, FLEX_WRAP_NOWRAP);
        if (error != CSS_OK) {
            goto css__parse_flex_flow_cleanup;
        }
    }
