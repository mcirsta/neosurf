			      ...
			      css_style *result)
{
	...
	if (flag_value != FLAG_VALUE__NONE) {
		...
		error = css_stylesheet_style_flag_value(
			result, flag_value, CSS_PROP_GRID_ROW_START);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
	...
	error = parse_grid_line_value(
		c, vector, ctx, &start_value, &start_integer);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
	...
	if (token != NULL && tokenIsChar(token, '/')) {
		...
		error = parse_grid_line_value(
			c, vector, ctx, &end_value, &end_integer);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
	...
	error = css__stylesheet_style_appendOPV(
		result, CSS_PROP_GRID_ROW_START, 0, start_value);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
	...
	/* Emit grid-row-end */
	error = css__stylesheet_style_appendOPV(
		result, CSS_PROP_GRID_ROW_END, 0, end_value);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
