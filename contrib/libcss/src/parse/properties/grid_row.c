 ...
 *
 * If only one value is given, grid-row-end is set to auto.
 */
css_error css__parse_grid_row(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        parserutils_vector_iterate(vector, ctx);
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_GRID_ROW_START);
        if (error != CSS_OK) {
            *ctx = orig_ctx;
            return error;
        }
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_GRID_ROW_END);
        if (error != CSS_OK)
            *ctx = orig_ctx;
        return error;
    }
    ...
    /* Emit grid-row-start */
    error = css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_ROW_START, 0, start_value);
    if (error != CSS_OK) {
        *ctx = orig_ctx;
        return error;
    }
    ...
    /* Emit grid-row-end */
    error = css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_ROW_END, 0, end_value);
    if (error != CSS_OK) {
        *ctx = orig_ctx;
        return error;
    }
