...
css_error css__parse_background(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BACKGROUND_ATTACHMENT);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BACKGROUND_COLOR);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BACKGROUND_IMAGE);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BACKGROUND_POSITION);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BACKGROUND_REPEAT);
        if (error == CSS_OK)
    ...
    do {
         ...
         */
        if ((attachment) && (error = css__parse_background_attachment(c, vector, ctx, attachment_style)) == CSS_OK) {
            attachment = false;
        } else if ((color) && (error = css__parse_background_color(c, vector, ctx, color_style)) == CSS_OK) {
            color = false;
        } else if ((image) && (error = css__parse_background_image(c, vector, ctx, image_style)) == CSS_OK) {
            image = false;
        } else if ((position) && (error = css__parse_background_position(c, vector, ctx, position_style)) == CSS_OK) {
            position = false;
        } else if ((repeat) && (error = css__parse_background_repeat(c, vector, ctx, repeat_style)) == CSS_OK) {
            repeat = false;
        }
    ...
    if (attachment) {
        error = css__stylesheet_style_appendOPV(
            attachment_style, CSS_PROP_BACKGROUND_ATTACHMENT, 0, BACKGROUND_ATTACHMENT_SCROLL);
        if (error != CSS_OK)
            goto css__parse_background_cleanup;
    }
    ...
    if (color) {
        error = css__stylesheet_style_appendOPV(
            color_style, CSS_PROP_BACKGROUND_COLOR, 0, BACKGROUND_COLOR_TRANSPARENT);
        if (error != CSS_OK)
            goto css__parse_background_cleanup;
    }

    if (image) {
        error = css__stylesheet_style_appendOPV(image_style, CSS_PROP_BACKGROUND_IMAGE, 0, BACKGROUND_IMAGE_NONE);
        if (error != CSS_OK)
            goto css__parse_background_cleanup;
    }

    if (position) {
        error = css__stylesheet_style_appendOPV(position_style, CSS_PROP_BACKGROUND_POSITION, 0,
    ...
    if (repeat) {
        error = css__stylesheet_style_appendOPV(repeat_style, CSS_PROP_BACKGROUND_REPEAT, 0, BACKGROUND_REPEAT_REPEAT);
        if (error != CSS_OK)
            goto css__parse_background_cleanup;
    }
