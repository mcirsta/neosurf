    ...
    css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result, enum css_properties_e id)
{
    ...
    if (token->type == CSS_TOKEN_IDENT) {
        if ((lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match)) {
            return css_stylesheet_style_inherit(result, CSS_PROP_GRID_TEMPLATE_ROWS);
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[INITIAL], &match) == lwc_error_ok && match)) {
            return css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_TEMPLATE_ROWS, 0, GRID_TEMPLATE_NONE);
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[REVERT], &match) == lwc_error_ok && match)) {
            return css_stylesheet_style_revert(result, CSS_PROP_GRID_TEMPLATE_ROWS);
        } else if ((lwc_string_caseless_isequal(token->idata, c->strings[UNSET], &match) == lwc_error_ok && match)) {
            return css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_TEMPLATE_ROWS, FLAG_VALUE_UNSET, 0);
    ...
    *ctx = orig_ctx;
    ...
    error = css__stylesheet_style_appendOPV(result, CSS_PROP_GRID_TEMPLATE_ROWS, 0, GRID_TEMPLATE_SET);
    if (error != CSS_OK) {
        *ctx = orig_ctx;
        return error;
    }
