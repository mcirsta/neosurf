				  ...
				  grid_track_t *track)
{
	...
	fprintf(stderr,
		"DEBUG: parse_track_size peeked token type %d\n",
		token->type);
	...
	if (token->type == CSS_TOKEN_FUNCTION) {
		if ((lwc_string_caseless_isequal(token->idata,
			...
			track->unit =
				CSS_UNIT_MINMAX; /* Mark as minmax track */
					  ...
					  enum css_properties_e id)
{
	...
	if (token->type == CSS_TOKEN_IDENT) {
		if ((lwc_string_caseless_isequal(token->idata,
			...
			return css_stylesheet_style_inherit(
				result, CSS_PROP_GRID_TEMPLATE_COLUMNS);
		} else if ((lwc_string_caseless_isequal(token->idata,
							c->strings[INITIAL],
							&match) ==
				    lwc_error_ok &&
	...
	*ctx = orig_ctx;
	...
	while (num_tracks < MAX_GRID_TRACKS) {
		...
		/* Also check for special chars that terminate property ( like ;
		 * or } ) */
		/* But unit parser will fail if it sees them, breaking the loop,
		 * which is fine. */
		/* However, if it returns CSS_INVALID, we need to decide if
		 * property is valid so far */
		/* For grid-template-columns, stray tokens invalidate the whole
		 * property */

		error = parse_track_size(c, vector, ctx, &track);
		if (error == CSS_OK) {
			fprintf(stderr,
				"DEBUG: Track %d parsed OK. Type=%d\n",
				num_tracks,
				track.type);
			tracks[num_tracks++] = track;
		} else {
			fprintf(stderr,
				"DEBUG: Track parsing failed for index %d\n",
				num_tracks);
	...
	error = css__stylesheet_style_appendOPV(
		result, CSS_PROP_GRID_TEMPLATE_COLUMNS, 0, GRID_TEMPLATE_SET);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
