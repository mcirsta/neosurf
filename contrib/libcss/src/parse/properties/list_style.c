...
css_error css__parse_list_style(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_LIST_STYLE_IMAGE);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_LIST_STYLE_POSITION);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_LIST_STYLE_TYPE);
        if (error == CSS_OK)
    ...
    do {
        ...
        if ((type) && (error = css__parse_list_style_type(c, vector, ctx, type_style)) == CSS_OK) {
            type = false;
        } else if ((position) && (error = css__parse_list_style_position(c, vector, ctx, position_style)) == CSS_OK) {
            position = false;
        } else if ((image) && (error = css__parse_list_style_image(c, vector, ctx, image_style)) == CSS_OK) {
            image = false;
        }
    ...
    /* defaults */
    if (image) {
        error = css__stylesheet_style_appendOPV(image_style, CSS_PROP_LIST_STYLE_IMAGE, 0, LIST_STYLE_IMAGE_NONE);
        if (error != CSS_OK) {
            goto css__parse_list_style_cleanup;
        }
    }
    ...
    if (position) {
        error = css__stylesheet_style_appendOPV(
            position_style, CSS_PROP_LIST_STYLE_POSITION, 0, LIST_STYLE_POSITION_OUTSIDE);
        if (error != CSS_OK) {
            goto css__parse_list_style_cleanup;
        }
    }
    ...
    if (type) {
        error = css__stylesheet_style_appendOPV(type_style, CSS_PROP_LIST_STYLE_TYPE, 0, LIST_STYLE_TYPE_DISC);
        if (error != CSS_OK) {
            goto css__parse_list_style_cleanup;
        }
    }
