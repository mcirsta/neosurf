			 ...
			 css_style *result)
{
	...
	if (token->type == CSS_TOKEN_IDENT &&
	    lwc_string_caseless_isequal(
		    token->idata, c->strings[NORMAL], &match) == lwc_error_ok &&
	    match) {
		parserutils_vector_iterate(vector, ctx);
		...
		error = css__stylesheet_style_appendOPV(
			result, CSS_PROP_ROW_GAP, 0, value[0]);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
		...
		error = css__stylesheet_style_appendOPV(
			result, CSS_PROP_COLUMN_GAP, 0, value[1]);
		if (error != CSS_OK)
			*ctx = orig_ctx;
		return error;
	}
	...
	if (token != NULL && token->type != CSS_TOKEN_EOF) {
		...
		error = css__parse_unit_specifier(
			c, vector, &temp_ctx, UNIT_PX, &length[1], &unit[1]);
		if (error == CSS_OK) {
