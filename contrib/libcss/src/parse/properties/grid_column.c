				 ...
				 css_style *result)
{
	...
	if (flag_value != FLAG_VALUE__NONE) {
		...
		error = css_stylesheet_style_flag_value(
			result, flag_value, CSS_PROP_GRID_COLUMN_START);
		if (error != CSS_OK) {
			*ctx = orig_ctx;
			return error;
		}
		error = css_stylesheet_style_flag_value(
			result, flag_value, CSS_PROP_GRID_COLUMN_END);
		if (error != CSS_OK)
			*ctx = orig_ctx;
		return error;
	}
	...
	error = css__stylesheet_style_appendOPV(
		result, CSS_PROP_GRID_COLUMN_START, 0, start_value);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
	...
	error = css__stylesheet_style_appendOPV(
		result, CSS_PROP_GRID_COLUMN_END, 0, end_value);
	if (error != CSS_OK) {
		*ctx = orig_ctx;
		return error;
	}
