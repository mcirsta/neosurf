...
css_error css__parse_grid_area(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    css_error error;
    const css_token *token;
    uint16_t values[4] = {CSS_GRID_LINE_AUTO, CSS_GRID_LINE_AUTO, CSS_GRID_LINE_AUTO, CSS_GRID_LINE_AUTO};
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        parserutils_vector_iterate(vector, ctx);
        for (int i = 0; i < 4; i++) {
            error = css_stylesheet_style_flag_value(result, flag_value, props[i]);
            if (error != CSS_OK) {
                *ctx = orig_ctx;
                return error;
            }
    ...
    for (int i = 0; i < 4; i++) {
        error = parse_grid_line_value(c, vector, ctx, &values[i], &integers[i]);
        if (error != CSS_OK) {
            if (i == 0) {
    ...
    for (int i = 0; i < 4; i++) {
        error = css__stylesheet_style_appendOPV(result, props[i], 0, values[i]);
        if (error != CSS_OK) {
            *ctx = orig_ctx;
            return error;
        }
        if (values[i] == CSS_GRID_LINE_SET) {
            error = css__stylesheet_style_append(result, integers[i]);
            if (error != CSS_OK) {
                *ctx = orig_ctx;
                return error;
            }
