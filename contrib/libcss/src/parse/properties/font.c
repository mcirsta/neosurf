...
static css_error parse_system_font(css_language *c, css_style *result, css_system_font *system_font)
{
    css_error error;
    bool match;
    ...
    error = css__stylesheet_style_vappend(
        result, 2, system_font->size.size, css__to_parse_unit(system_font->size.unit));
    if (error != CSS_OK)
        return error;
    ...
    error = css__stylesheet_style_vappend(
        result, 2, system_font->line_height.size, css__to_parse_unit(system_font->line_height.unit));
    if (error != CSS_OK)
        return error;
    ...
    else if ((lwc_string_caseless_isequal(system_font->family, c->strings[SANS_SERIF], &match) == lwc_error_ok &&
                 match))
...
css_error css__parse_font(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FONT_STYLE);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FONT_VARIANT);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FONT_WEIGHT);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FONT_SIZE);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_LINE_HEIGHT);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_FONT_FAMILY);
        if (error == CSS_OK)
    ...
    if ((token->type == CSS_TOKEN_IDENT) && (c->sheet->font != NULL) &&
        (c->sheet->font(c->sheet->font_pw, token->idata, &system_font) == CSS_OK)) {
    ...
    for (svw = 0; svw < 3; svw++) {
        ...
        if ((style) && (error = css__parse_font_style(c, vector, ctx, style_style)) == CSS_OK) {
            style = false;
        } else if ((variant) && (error = css__parse_font_variant(c, vector, ctx, variant_style)) == CSS_OK) {
            variant = false;
        } else if ((weight) && (error = css__parse_font_weight(c, vector, ctx, weight_style)) == CSS_OK) {
            weight = false;
        }
    ...
    /* defaults */
    if (style) {
        error = css__stylesheet_style_appendOPV(style_style, CSS_PROP_FONT_STYLE, 0, FONT_STYLE_NORMAL);
        if (error != CSS_OK)
            goto css__parse_font_cleanup;
    }

    if (variant) {
        error = css__stylesheet_style_appendOPV(variant_style, CSS_PROP_FONT_VARIANT, 0, FONT_VARIANT_NORMAL);
        if (error != CSS_OK)
            goto css__parse_font_cleanup;
    }

    if (weight) {
        error = css__stylesheet_style_appendOPV(weight_style, CSS_PROP_FONT_WEIGHT, 0, FONT_WEIGHT_NORMAL);
        if (error != CSS_OK)
            goto css__parse_font_cleanup;
    }

    if (line_height) {
        error = css__stylesheet_style_appendOPV(line_height_style, CSS_PROP_LINE_HEIGHT, 0, LINE_HEIGHT_NORMAL);
        if (error != CSS_OK)
            goto css__parse_font_cleanup;
    }
