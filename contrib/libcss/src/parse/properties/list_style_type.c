...
css_error css__parse_list_style_type(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (ident->type == CSS_TOKEN_IDENT &&
        ...
        (lwc_string_caseless_isequal(ident->idata, c->strings[REVERT], &match) == lwc_error_ok && match)) {
        flags = FLAG_REVERT;
    } else if (ident->type == CSS_TOKEN_IDENT &&
        (lwc_string_caseless_isequal(ident->idata, c->strings[UNSET], &match) == lwc_error_ok && match)) {
        flags = FLAG_UNSET;
    } else {
