...
css_error css__parse_border_color(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BORDER_TOP_COLOR);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BORDER_RIGHT_COLOR);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BORDER_BOTTOM_COLOR);
        if (error != CSS_OK)
            return error;

        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_BORDER_LEFT_COLOR);
        if (error == CSS_OK)
