...
css_error css__parse_text_decoration(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (lwc_string_caseless_isequal(token->idata, c->strings[INHERIT], &match) == lwc_error_ok && match) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_TEXT_DECORATION);

    } else if (lwc_string_caseless_isequal(token->idata, c->strings[NONE], &match) == lwc_error_ok && match) {
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_TEXT_DECORATION, 0, TEXT_DECORATION_NONE);
    } else {
        uint16_t value = 0;
        while (token != NULL) {
            if ((lwc_string_caseless_isequal(token->idata, c->strings[UNDERLINE], &match) == lwc_error_ok && match)) {
            ...
            } else if ((lwc_string_caseless_isequal(token->idata, c->strings[OVERLINE], &match) == lwc_error_ok &&
                           match)) {
            ...
            } else if ((lwc_string_caseless_isequal(token->idata, c->strings[LINE_THROUGH], &match) == lwc_error_ok &&
                           match)) {
            ...
            } else if ((lwc_string_caseless_isequal(token->idata, c->strings[BLINK], &match) == lwc_error_ok &&
                           match)) {
        ...
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_TEXT_DECORATION, 0, value);
    }
