...
css_error css__parse_quotes(css_language *c, const parserutils_vector *vector, int32_t *ctx, css_style *result)
{
    ...
    if (flag_value != FLAG_VALUE__NONE) {
        error = css_stylesheet_style_flag_value(result, flag_value, CSS_PROP_QUOTES);

    } else if ((token->type == CSS_TOKEN_IDENT) &&
        (lwc_string_caseless_isequal(token->idata, c->strings[NONE], &match) == lwc_error_ok && match)) {
        error = css__stylesheet_style_appendOPV(result, CSS_PROP_QUOTES, 0, QUOTES_NONE);
...
#define CSS_FIRST_APPEND(CSSVAL)                                                                                       \
    css__stylesheet_style_append(result, first ? buildOPV(CSS_PROP_QUOTES, 0, CSSVAL) : CSSVAL)
        ...
        while ((token != NULL) && (token->type == CSS_TOKEN_STRING)) {
            ...
            error = css__stylesheet_string_add(c->sheet, lwc_string_ref(token->idata), &close_snumber);
            if (error != CSS_OK)
                break;
