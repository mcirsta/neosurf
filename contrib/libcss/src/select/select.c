...
static css_error set_initial(css_select_state *state, uint32_t prop, css_pseudo_element pseudo, void *parent);

static css_error
select_from_sheet(css_select_ctx *ctx, const css_stylesheet *sheet, css_origin origin, css_select_state *state);
static css_error match_selectors_in_sheet(css_select_ctx *ctx, const css_stylesheet *sheet, css_select_state *state);
static css_error match_selector_chain(css_select_ctx *ctx, const css_selector *selector, css_select_state *state);
static css_error match_named_combinator(css_select_ctx *ctx, css_combinator type, const css_selector *selector,
    css_select_state *state, void *node, void **next_node);
static css_error match_universal_combinator(css_select_ctx *ctx, css_combinator type, const css_selector *selector,
    css_select_state *state, void *node, bool may_optimise, bool *rejected_by_cache, void **next_node);
static css_error match_details(css_select_ctx *ctx, void *node, const css_selector_detail *detail,
...
static void css__destroy_node_data(struct css_node_data *node_data)
{
    ...
    if (node_data->bloom != NULL && node_data->bloom != css__get_empty_bloom()) {
        free(node_data->bloom);
    }

    for (i = 0; i < CSS_PSEUDO_ELEMENT_COUNT; i++) {
        if (node_data->partial.styles[i] != NULL) {
            css_computed_style_destroy(node_data->partial.styles[i]);
        }
    }
...
css_error css_libcss_node_data_handler(css_select_handler *handler, css_node_data_action action, void *pw, void *node,
    void *clone_node, void *libcss_node_data)
{
    ...
    if (handler == NULL || libcss_node_data == NULL || handler->handler_version != CSS_SELECT_HANDLER_VERSION_1) {
        return CSS_BADPARM;
    }
 ...
 */
css_error
css_select_ctx_append_sheet(css_select_ctx *ctx, const css_stylesheet *sheet, css_origin origin, const char *media)
{
    if (ctx == NULL || sheet == NULL)
        return CSS_BADPARM;

    return css_select_ctx_insert_sheet(ctx, sheet, ctx->n_sheets, origin, media);
}
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
css_error css_select_ctx_insert_sheet(
    css_select_ctx *ctx, const css_stylesheet *sheet, uint32_t index, css_origin origin, const char *media)
{
    ...
    temp = realloc(ctx->sheets, (ctx->n_sheets + 1) * sizeof(css_select_sheet));
    if (temp == NULL)
        return CSS_NOMEM;
    ...
    if (index < ctx->n_sheets) {
        memmove(&ctx->sheets[index + 1], &ctx->sheets[index], (ctx->n_sheets - index) * sizeof(css_select_sheet));
    }

    error = css_parse_media_query(sheet->propstrings, (const uint8_t *)media, (media == NULL) ? 0 : strlen(media), &mq);
    if (error == CSS_NOMEM) {
        return error;
...
css_error css_select_ctx_remove_sheet(css_select_ctx *ctx, const css_stylesheet *sheet)
{
    ...
    memmove(&ctx->sheets[index], &ctx->sheets[index + 1], (ctx->n_sheets - index) * sizeof(css_select_sheet));
 ...
 * \return CSS_OK on success, appropriate error otherwise
 */
static css_error css__select_ctx_create_default_style(css_select_ctx *ctx, css_select_handler *handler, void *pw)
{
 ...
 */
css_error
css_select_default_style(css_select_ctx *ctx, css_select_handler *handler, void *pw, css_computed_style **style)
{
    css_error error;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
static css_error css__get_parent_bloom(void *parent, css_select_handler *handler, void *pw, css_bloom **parent_bloom)
{
    ...
    if (parent != NULL) {
         ...
         * we build for. */
        error = handler->get_libcss_node_data(pw, parent, (void **)(void *)&node_data);
        if (error != CSS_OK) {
            return error;
        }
        if (node_data != NULL) {
    ...
    if (bloom == NULL) {
        ...
        if (parent != NULL) {
            ...
            if (node_data == NULL) {
                ...
                error = handler->set_libcss_node_data(pw, parent, node_data);
                if (error != CSS_OK) {
                    css__destroy_node_data(node_data);
                    return error;
                }
...
static css_error css__create_node_bloom(css_bloom **node_bloom, css_select_state *state)
{
    css_error error;
    css_bloom *bloom;
    lwc_hash hash;
    ...
    *node_bloom = NULL;
    ...
    if (lwc_string_caseless_hash_value(state->element.name, &hash) != lwc_error_ok) {
        error = CSS_NOMEM;
        goto cleanup;
    }
    ...
    if (state->id != NULL) {
        if (lwc_string_caseless_hash_value(state->id, &hash) != lwc_error_ok) {
            error = CSS_NOMEM;
            goto cleanup;
        }
        css_bloom_add_hash(bloom, hash);
    ...
    if (state->classes != NULL) {
        for (uint32_t i = 0; i < state->n_classes; i++) {
            ...
            if (lwc_string_caseless_hash_value(s, &hash) != lwc_error_ok) {
                error = CSS_NOMEM;
                goto cleanup;
            }
            css_bloom_add_hash(bloom, hash);
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
static css_error css__set_node_data(void *node, css_select_state *state, css_select_handler *handler, void *pw)
{
    int i;
    css_error error;
    css_bloom *bloom;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
static css_error css_select_style__get_sharable_node_data_for_candidate(css_select_state *state,
    void *share_candidate_node, enum share_candidate_type type, struct css_node_data **sharable_node_data)
{
    css_error error;
    ...
    *sharable_node_data = NULL;
    ...
    error = state->handler->get_libcss_node_data(state->pw, share_candidate_node, (void **)(void *)&node_data);
    ...
    if (node_data->flags &
        (CSS_NODE_FLAGS_TAINT_PSEUDO_CLASS | CSS_NODE_FLAGS_TAINT_ATTRIBUTE | CSS_NODE_FLAGS_TAINT_SIBLING |
    ...
    error = state->handler->node_id(state->pw, share_candidate_node, &share_candidate_id);
    if (error != CSS_OK) {
        return error;
    ...
    error = state->handler->node_classes(
        state->pw, share_candidate_node, &share_candidate_classes, &share_candidate_n_classes);
    if (error != CSS_OK) {
        return error;
    }
    ...
    for (uint32_t i = 0; i < share_candidate_n_classes; i++) {
        bool match;
        if (lwc_string_caseless_isequal(state->classes[i], share_candidate_classes[i], &match) == lwc_error_ok &&
            match == false) {
#ifdef DEBUG_STYLE_SHARING
 ...
 */
static css_error css_select_style__get_named_cousin(css_select_state *state, void *node, void **cousin_out)
{
    /* TODO:
     *
 ...
 */
static css_error
css_select_style__get_sharable_node_data(void *node, css_select_state *state, struct css_node_data **sharable_node_data)
{
    css_error error;
    ...
    *sharable_node_data = NULL;
    ...
    while (true) {
        ...
        error = state->handler->named_generic_sibling_node(state->pw, node, &state->element, &share_candidate_node);
        if (error != CSS_OK) {
            return error;
        } else {
            if (share_candidate_node == NULL) {
                error = css_select_style__get_named_cousin(state, node, &share_candidate_node);
                if (error != CSS_OK) {
                    return error;
                } else {
                    if (share_candidate_node == NULL) {
 ...
 */
static css_error css_select__initialise_selection_state(css_select_state *state, void *node, void *parent,
    const css_media *media, const css_unit_ctx *unit_ctx, css_select_handler *handler, void *pw)
{
    css_error error;
    bool match;
    ...
    error = handler->node_classes(pw, node, &state->classes, &state->n_classes);
    if (error != CSS_OK) {
        goto failed;
    }
...
static css_error css__select_revert_property_to_origin(css_select_state *select_state, prop_state *prop_state,
    css_origin origin, enum css_pseudo_element pseudo, enum css_properties_e property)
{
    css_error error;
    ...
    error = prop_dispatch[property].copy(
        select_state->revert[origin].style[pseudo], select_state->results->styles[pseudo]);
    if (error != CSS_OK) {
        return error;
    }
...
static css_error css__select_revert_property(css_select_state *select_state, prop_state *prop_state,
    enum css_pseudo_element pseudo, enum css_properties_e property)
{
    css_error error;

    switch (prop_state->origin) {
    case CSS_ORIGIN_AUTHOR:
        error = css__select_revert_property_to_origin(select_state, prop_state, CSS_ORIGIN_USER, pseudo, property);
        if (error != CSS_OK) {
            return error;
        }
    ...
    case CSS_ORIGIN_USER:
        error = css__select_revert_property_to_origin(select_state, prop_state, CSS_ORIGIN_UA, pseudo, property);
        if (error != CSS_OK) {
            return error;
        }
        if (prop_state->explicit_default != FLAG_VALUE_REVERT) {
 ...
 */
css_error css_select_style(css_select_ctx *ctx, void *node, const css_unit_ctx *unit_ctx, const css_media *media,
    const css_stylesheet *inline_style, css_select_handler *handler, void *pw, css_select_results **result)
{
    ...
    if (ctx->uses_revert || (inline_style != NULL && inline_style->uses_revert)) {
     ...
     */
    error = css__computed_style_create(&state.results->styles[CSS_PSEUDO_ELEMENT_NONE], ctx->calc);
    if (error != CSS_OK) {
        goto cleanup;
    }
    ...
    for (i = 0; i < ctx->n_sheets; i++) {
        ...
        if (state.revert != NULL && s.origin != origin) {
            for (j = 0; j < CSS_PSEUDO_ELEMENT_COUNT; j++) {
                ...
                error = css__computed_style_clone(state.results->styles[j], &state.revert[origin].style[j]);
                if (error != CSS_OK) {
                    goto cleanup;
                }
                memcpy(state.revert[origin].props, state.props, sizeof(state.props));
            }
            origin = s.origin;
        }

        if (mq__list_match(s.media, unit_ctx, media, &ctx->str) && s.sheet->disabled == false) {
    ...
    if (inline_style != NULL) {
        ...
        /* Sanity check style */
        if (inline_style->rule_count != 1 || inline_style->rule_list->type != CSS_RULE_SELECTOR ||
    ...
    for (i = 0; i < CSS_N_PROPERTIES; i++) {
        ...
        if (prop->explicit_default == FLAG_VALUE_REVERT) {
            error = css__select_revert_property(&state, prop, CSS_PSEUDO_ELEMENT_NONE, i);
            if (error != CSS_OK) {
                goto cleanup;
            }
        }
         ...
         * value. */
        if (prop->explicit_default == FLAG_VALUE_INITIAL || prop->set == false ||
            (parent == NULL && prop->explicit_default == FLAG_VALUE_INHERIT)) {
            error = set_initial(&state, i, CSS_PSEUDO_ELEMENT_NONE, parent);
            if (error != CSS_OK)
                goto cleanup;
        }
    }
    ...
    for (j = CSS_PSEUDO_ELEMENT_NONE + 1; j < CSS_PSEUDO_ELEMENT_COUNT; j++) {
        ...
        for (i = 0; i < CSS_N_PROPERTIES; i++) {
            ...
            if (prop->explicit_default == FLAG_VALUE_REVERT) {
                error = css__select_revert_property(&state, prop, j, i);
                if (error != CSS_OK) {
                    goto cleanup;
                }
            }
    ...
    if (parent == NULL) {
        ...
        error = css__compute_absolute_values(NULL, state.results->styles[CSS_PSEUDO_ELEMENT_NONE], unit_ctx);
        if (error != CSS_OK)
            goto cleanup;
    }
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
css_error css_select_font_faces(css_select_ctx *ctx, const css_media *media, const css_unit_ctx *unit_ctx,
    lwc_string *font_family, css_select_font_faces_results **result)
{
    uint32_t i;
    css_error error;
    ...
    for (i = 0; i < ctx->n_sheets; i++) {
        ...
        if (mq__list_match(s.media, unit_ctx, media, &ctx->str) && s.sheet->disabled == false) {
            error = select_font_faces_from_sheet(s.sheet, s.origin, &state, &ctx->str);
            if (error != CSS_OK)
                goto cleanup;
        }
    }
    ...
    if (n_font_faces > 0) {
        ...
        results->font_faces = malloc(n_font_faces * sizeof(css_font_face *));
        ...
        if (state.ua_font_faces.count != 0) {
            memcpy(results->font_faces, state.ua_font_faces.font_faces,
        ...
        if (state.user_font_faces.count != 0) {
            memcpy(results->font_faces + i, state.user_font_faces.font_faces,
                sizeof(css_font_face *) * state.user_font_faces.count);
            i += state.user_font_faces.count;
        }

        if (state.author_font_faces.count != 0) {
            memcpy(results->font_faces + i, state.author_font_faces.font_faces,
...
css_error set_initial(css_select_state *state, uint32_t prop, css_pseudo_element pseudo, void *parent)
{
    css_error error;
...
select_from_sheet(css_select_ctx *ctx, const css_stylesheet *sheet, css_origin origin, css_select_state *state)
{
    ...
    do {
        ...
        if (rule != NULL && rule->type == CSS_RULE_IMPORT) {
            ...
            if (import->sheet != NULL && mq__list_match(import->media, state->unit_ctx, state->media, &ctx->str)) {
...
static css_error _select_font_face_from_rule(const css_rule_font_face *rule, css_origin origin,
    css_select_font_faces_state *state, const css_select_strings *str)
{
    ...
    const css_stylesheet *sheet, css_origin origin, css_select_font_faces_state *state, const css_select_strings *str)
{
    ...
    do {
        ...
        if (rule != NULL && rule->type == CSS_RULE_IMPORT) {
            ...
            if (import->sheet != NULL && mq__list_match(import->media, state->unit_ctx, state->media, str)) {
                /* It's applicable, so process it */
                if (sp >= IMPORT_STACK_SIZE)
                    return CSS_NOMEM;
            ...
            error = _select_font_face_from_rule((const css_rule_font_face *)rule, origin, state, str);
...
static inline bool _selectors_pending(const css_selector **node, const css_selector **id, const css_selector ***classes,
...
static inline bool _selector_less_specific(const css_selector *ref, const css_selector *cand)
{
    bool result = true;
...
static const css_selector *_selector_next(const css_selector **node, const css_selector **id,
    const css_selector ***classes, uint32_t n_classes, const css_selector **univ, css_select_rule_source *src)
{
...
css_error match_selectors_in_sheet(css_select_ctx *ctx, const css_stylesheet *sheet, css_select_state *state)
{
    ...
    error = css__selector_hash_find(sheet->selectors, &req, &node_iterator, &node_selectors);
    if (error != CSS_OK)
        goto cleanup;
    ...
    if (state->classes != NULL && n_classes > 0) {
        ...
        for (i = 0; i < n_classes; i++) {
            ...
            error = css__selector_hash_find_by_class(sheet->selectors, &req, &class_iterator, &class_selectors[i]);
            if (error != CSS_OK)
                goto cleanup;
        }
    }
    ...
    if (state->id != NULL) {
        ...
        req.id = state->id;
        error = css__selector_hash_find_by_id(sheet->selectors, &req, &id_iterator, &id_selectors);
        if (error != CSS_OK)
            goto cleanup;
    }

    /* Find hash chain for universal selector */
    error = css__selector_hash_find_universal(sheet->selectors, &req, &univ_iterator, &univ_selectors);
    if (error != CSS_OK)
        goto cleanup;

    /* Process matching selectors, if any */
    while (_selectors_pending(node_selectors, id_selectors, class_selectors, n_classes, univ_selectors)) {
         ...
         */
        selector = _selector_next(node_selectors, id_selectors, class_selectors, n_classes, univ_selectors, &src);
        ...
        case CSS_SELECT_RULE_SRC_ELEMENT:
            error = node_iterator(&req, node_selectors, &node_selectors);
            break;
...
static void update_reject_cache(css_select_state *state, css_combinator comb, const css_selector *s)
{
    ...
    if (state->next_reject < state->reject_cache || comb != CSS_COMBINATOR_ANCESTOR || next_detail == NULL ||
        next_detail->next != 0 || (next_detail->type != CSS_SELECTOR_CLASS && next_detail->type != CSS_SELECTOR_ID))
        return;
...
css_error match_selector_chain(css_select_ctx *ctx, const css_selector *selector, css_select_state *state)
{
    ...
    do {
        ...
        if (s->data.comb != CSS_COMBINATOR_NONE && s->combinator->data.qname.name != ctx->str.universal) {
            /* Named combinator */
            may_optimise &= (s->data.comb == CSS_COMBINATOR_ANCESTOR || s->data.comb == CSS_COMBINATOR_PARENT);

            error = match_named_combinator(ctx, s->data.comb, s->combinator, state, node, &next_node);
            if (error != CSS_OK)
                return error;
            ...
            error = match_universal_combinator(
                ctx, s->data.comb, s->combinator, state, node, may_optimise, &rejected_by_cache, &next_node);
            if (error != CSS_OK)
                return error;
            ...
            if (next_node == NULL) {
                if (may_optimise && s == selector && rejected_by_cache == false) {
                    update_reject_cache(state, s->data.comb, s->combinator);
                }
    ...
    if (state->results->styles[pseudo] == NULL) {
        error = css__computed_style_create(&state->results->styles[pseudo], ctx->calc);
        if (error != CSS_OK)
            return error;
    }
    ...
    css_select_state *state, void *node, void **next_node)
{
    ...
    do {
        ...
        case CSS_COMBINATOR_ANCESTOR:
            error = state->handler->named_ancestor_node(state->pw, n, &selector->data.qname, &n);
            if (error != CSS_OK)
                return error;
            break;
        case CSS_COMBINATOR_PARENT:
            error = state->handler->named_parent_node(state->pw, n, &selector->data.qname, &n);
            if (error != CSS_OK)
                return error;
            break;
        case CSS_COMBINATOR_SIBLING:
            error = state->handler->named_sibling_node(state->pw, n, &selector->data.qname, &n);
            if (error != CSS_OK)
                return error;
            if (node == state->node) {
        ...
        case CSS_COMBINATOR_GENERIC_SIBLING:
            error = state->handler->named_generic_sibling_node(state->pw, n, &selector->data.qname, &n);
            if (error != CSS_OK)
                return error;
            if (node == state->node) {
                state->node_data->flags |= CSS_NODE_FLAGS_TAINT_SIBLING;
        ...
        if (n != NULL) {
            /* Match its details */
            error = match_details(ctx, n, detail, state, &match, NULL);
            if (error != CSS_OK)
                return error;
...
static inline void add_node_flags(const void *node, const css_select_state *state, css_node_flags flags)
{
...
css_error match_universal_combinator(css_select_ctx *ctx, css_combinator type, const css_selector *selector,
    css_select_state *state, void *node, bool may_optimise, bool *rejected_by_cache, void **next_node)
{
    const css_selector_detail *detail = &selector->data;
    const css_selector_detail *next_detail = NULL;
    ...
    *rejected_by_cache = false;
    ...
    if (may_optimise && (type == CSS_COMBINATOR_ANCESTOR || type == CSS_COMBINATOR_PARENT) && next_detail != NULL &&
        (next_detail->type == CSS_SELECTOR_CLASS || next_detail->type == CSS_SELECTOR_ID)) {
        ...
        while (reject <= last) {
            ...
            if (reject->type == next_detail->type &&
                lwc_string_isequal(reject->value, next_detail->qname.name, &match) == lwc_error_ok && match) {
    ...
    do {
        ...
        case CSS_COMBINATOR_GENERIC_SIBLING:
            ...
            if (error != CSS_OK)
                return error;
            add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_SIBLING);
            break;
        case CSS_COMBINATOR_NONE:
            break;
        }
...
css_error match_details(css_select_ctx *ctx, void *node, const css_selector_detail *detail, css_select_state *state,
...
css_error match_detail(css_select_ctx *ctx, void *node, const css_selector_detail *detail, css_select_state *state,
    bool *match, css_pseudo_element *pseudo_element)
{
    bool is_root = false;
    ...
    case CSS_SELECTOR_ELEMENT:
        if (detail->negate != 0) {
             ...
             * otherwise. */
            error = state->handler->node_has_name(state->pw, node, &detail->qname, match);
        }
        break;
    case CSS_SELECTOR_CLASS:
        error = state->handler->node_has_class(state->pw, node, detail->qname.name, match);
        break;
    case CSS_SELECTOR_ID:
        error = state->handler->node_has_id(state->pw, node, detail->qname.name, match);
        break;
    case CSS_SELECTOR_PSEUDO_CLASS:
        ...
        if (is_root == false && detail->qname.name == ctx->str.first_child) {
            ...
            error = state->handler->node_count_siblings(state->pw, node, false, true, &num_after);
            if (error == CSS_OK) {
                int32_t a = detail->value.nth.a;
                int32_t b = detail->value.nth.b;
        ...
        } else if (is_root == false && detail->qname.name == ctx->str.nth_of_type) {
            int32_t num_before = 0;

            error = state->handler->node_count_siblings(state->pw, node, true, false, &num_before);
            if (error == CSS_OK) {
                int32_t a = detail->value.nth.a;
                int32_t b = detail->value.nth.b;
        ...
        } else if (is_root == false && detail->qname.name == ctx->str.nth_last_of_type) {
            int32_t num_after = 0;

            error = state->handler->node_count_siblings(state->pw, node, true, true, &num_after);
            if (error == CSS_OK) {
                int32_t a = detail->value.nth.a;
                int32_t b = detail->value.nth.b;
        ...
        } else if (is_root == false && detail->qname.name == ctx->str.last_child) {
            int32_t num_after = 0;

            error = state->handler->node_count_siblings(state->pw, node, false, true, &num_after);
            if (error == CSS_OK)
                *match = (num_after == 0);
        } else if (is_root == false && detail->qname.name == ctx->str.first_of_type) {
            int32_t num_before = 0;

            error = state->handler->node_count_siblings(state->pw, node, true, false, &num_before);
            if (error == CSS_OK)
                *match = (num_before == 0);
        } else if (is_root == false && detail->qname.name == ctx->str.last_of_type) {
            int32_t num_after = 0;

            error = state->handler->node_count_siblings(state->pw, node, true, true, &num_after);
            if (error == CSS_OK)
                *match = (num_after == 0);
        } else if (is_root == false && detail->qname.name == ctx->str.only_child) {
        ...
        } else if (is_root == false && detail->qname.name == ctx->str.only_of_type) {
            int32_t num_before = 0, num_after = 0;
            ...
            *match = is_root;
            ...
            error = state->handler->node_is_empty(state->pw, node, match);
        } else if (detail->qname.name == ctx->str.link) {
            error = state->handler->node_is_link(state->pw, node, match);
            flags = CSS_NODE_FLAGS_NONE;
        } else if (detail->qname.name == ctx->str.visited) {
            error = state->handler->node_is_visited(state->pw, node, match);
            flags = CSS_NODE_FLAGS_NONE;
        } else if (detail->qname.name == ctx->str.hover) {
            error = state->handler->node_is_hover(state->pw, node, match);
            flags = CSS_NODE_FLAGS_NONE;
        } else if (detail->qname.name == ctx->str.active) {
            error = state->handler->node_is_active(state->pw, node, match);
            flags = CSS_NODE_FLAGS_NONE;
        } else if (detail->qname.name == ctx->str.focus) {
            error = state->handler->node_is_focus(state->pw, node, match);
            flags = CSS_NODE_FLAGS_NONE;
        } else if (detail->qname.name == ctx->str.target) {
            error = state->handler->node_is_target(state->pw, node, match);
        } else if (detail->qname.name == ctx->str.lang) {
            error = state->handler->node_is_lang(state->pw, node, detail->value.string, match);
        } else if (detail->qname.name == ctx->str.enabled) {
            error = state->handler->node_is_enabled(state->pw, node, match);
        } else if (detail->qname.name == ctx->str.disabled) {
            error = state->handler->node_is_disabled(state->pw, node, match);
        } else if (detail->qname.name == ctx->str.checked) {
            error = state->handler->node_is_checked(state->pw, node, match);
        } else {
            *match = false;
        }
    ...
    case CSS_SELECTOR_ATTRIBUTE:
        error = state->handler->node_has_attribute(state->pw, node, &detail->qname, match);
        add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_ATTRIBUTE);
        break;
    case CSS_SELECTOR_ATTRIBUTE_EQUAL:
        error = state->handler->node_has_attribute_equal(state->pw, node, &detail->qname, detail->value.string, match);
        add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_ATTRIBUTE);
        break;
    case CSS_SELECTOR_ATTRIBUTE_DASHMATCH:
        error = state->handler->node_has_attribute_dashmatch(
            state->pw, node, &detail->qname, detail->value.string, match);
        add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_ATTRIBUTE);
        break;
    ...
    case CSS_SELECTOR_ATTRIBUTE_PREFIX:
        error = state->handler->node_has_attribute_prefix(state->pw, node, &detail->qname, detail->value.string, match);
        add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_ATTRIBUTE);
        break;
    case CSS_SELECTOR_ATTRIBUTE_SUFFIX:
        error = state->handler->node_has_attribute_suffix(state->pw, node, &detail->qname, detail->value.string, match);
        add_node_flags(node, state, CSS_NODE_FLAGS_TAINT_ATTRIBUTE);
        break;
