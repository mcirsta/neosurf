...
static inline css_error set_align_content(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~ALIGN_CONTENT_MASK) | (((uint32_t)type & 0x7) << ALIGN_CONTENT_SHIFT);
...
static inline css_error set_align_items(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~ALIGN_ITEMS_MASK) | (((uint32_t)type & 0x7) << ALIGN_ITEMS_SHIFT);
...
static inline css_error set_align_self(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~ALIGN_SELF_MASK) | (((uint32_t)type & 0x7) << ALIGN_SELF_SHIFT);
...
static inline css_error set_background_attachment(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BACKGROUND_ATTACHMENT_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BACKGROUND_ATTACHMENT_MASK) | (((uint32_t)type & 0x3) << BACKGROUND_ATTACHMENT_SHIFT);
...
static inline css_error set_background_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[BACKGROUND_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BACKGROUND_COLOR_MASK) | (((uint32_t)type & 0x3) << BACKGROUND_COLOR_SHIFT);
...
#define BACKGROUND_IMAGE_INDEX 16
#define BACKGROUND_IMAGE_SHIFT 16
#define BACKGROUND_IMAGE_MASK 0x10000

static inline css_error set_background_image(css_computed_style *style, uint8_t type, lwc_string *string)
{
    uint32_t *bits = &style->i.bits[BACKGROUND_IMAGE_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~BACKGROUND_IMAGE_MASK) | (((uint32_t)type & 0x1) << BACKGROUND_IMAGE_SHIFT);
...
#define BACKGROUND_POSITION_SHIFT 6
#define BACKGROUND_POSITION_MASK 0x7ffc0

static inline css_error set_background_position(
    css_computed_style *style, uint8_t type, css_fixed length_a, css_unit unit_a, css_fixed length_b, css_unit unit_b)
{
    ...
    *bits = (*bits & ~BACKGROUND_POSITION_MASK) |
        ((((uint32_t)type & 0x1) | (unit_b << 1) | (unit_a << 7)) << BACKGROUND_POSITION_SHIFT);
...
static inline css_error set_background_repeat(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BACKGROUND_REPEAT_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~BACKGROUND_REPEAT_MASK) | (((uint32_t)type & 0x7) << BACKGROUND_REPEAT_SHIFT);
...
#define BORDER_BOTTOM_COLOR_INDEX 13
#define BORDER_BOTTOM_COLOR_SHIFT 0
#define BORDER_BOTTOM_COLOR_MASK 0x3

static inline css_error set_border_bottom_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[BORDER_BOTTOM_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BORDER_BOTTOM_COLOR_MASK) | (((uint32_t)type & 0x3) << BORDER_BOTTOM_COLOR_SHIFT);
...
#define BORDER_BOTTOM_STYLE_SHIFT 12
#define BORDER_BOTTOM_STYLE_MASK 0xf000

static inline css_error set_border_bottom_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BORDER_BOTTOM_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BORDER_BOTTOM_STYLE_MASK) | (((uint32_t)type & 0xf) << BORDER_BOTTOM_STYLE_SHIFT);
...
#define BORDER_BOTTOM_WIDTH_INDEX 14
#define BORDER_BOTTOM_WIDTH_SHIFT 14
#define BORDER_BOTTOM_WIDTH_MASK 0x7fc000

static inline css_error
set_border_bottom_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[BORDER_BOTTOM_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~BORDER_BOTTOM_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << BORDER_BOTTOM_WIDTH_SHIFT);
...
static inline css_error set_border_collapse(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BORDER_COLLAPSE_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BORDER_COLLAPSE_MASK) | (((uint32_t)type & 0x3) << BORDER_COLLAPSE_SHIFT);
...
static inline css_error set_border_left_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[BORDER_LEFT_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BORDER_LEFT_COLOR_MASK) | (((uint32_t)type & 0x3) << BORDER_LEFT_COLOR_SHIFT);
...
static inline css_error set_border_left_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BORDER_LEFT_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BORDER_LEFT_STYLE_MASK) | (((uint32_t)type & 0xf) << BORDER_LEFT_STYLE_SHIFT);
...
#define BORDER_LEFT_WIDTH_INDEX 14
#define BORDER_LEFT_WIDTH_SHIFT 23
#define BORDER_LEFT_WIDTH_MASK 0xff800000

static inline css_error set_border_left_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[BORDER_LEFT_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~BORDER_LEFT_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << BORDER_LEFT_WIDTH_SHIFT);
...
static inline css_error set_border_right_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[BORDER_RIGHT_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BORDER_RIGHT_COLOR_MASK) | (((uint32_t)type & 0x3) << BORDER_RIGHT_COLOR_SHIFT);
...
#define BORDER_RIGHT_STYLE_SHIFT 20
#define BORDER_RIGHT_STYLE_MASK 0xf00000

static inline css_error set_border_right_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BORDER_RIGHT_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BORDER_RIGHT_STYLE_MASK) | (((uint32_t)type & 0xf) << BORDER_RIGHT_STYLE_SHIFT);
...
#define BORDER_RIGHT_WIDTH_INDEX 6
#define BORDER_RIGHT_WIDTH_SHIFT 5
#define BORDER_RIGHT_WIDTH_MASK 0x3fe0

static inline css_error set_border_right_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[BORDER_RIGHT_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~BORDER_RIGHT_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << BORDER_RIGHT_WIDTH_SHIFT);
...
#define BORDER_SPACING_INDEX 5
#define BORDER_SPACING_SHIFT 19
#define BORDER_SPACING_MASK 0xfff80000
    ...
    css_computed_style *style, uint8_t type, css_fixed length_a, css_unit unit_a, css_fixed length_b, css_unit unit_b)
{
    ...
    *bits = (*bits & ~BORDER_SPACING_MASK) |
        ((((uint32_t)type & 0x1) | (unit_b << 1) | (unit_a << 7)) << BORDER_SPACING_SHIFT);
...
static inline css_error set_border_top_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[BORDER_TOP_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~BORDER_TOP_COLOR_MASK) | (((uint32_t)type & 0x3) << BORDER_TOP_COLOR_SHIFT);
...
static inline css_error set_border_top_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BORDER_TOP_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BORDER_TOP_STYLE_MASK) | (((uint32_t)type & 0xf) << BORDER_TOP_STYLE_SHIFT);
...
#define BORDER_TOP_WIDTH_INDEX 6
#define BORDER_TOP_WIDTH_SHIFT 14
#define BORDER_TOP_WIDTH_MASK 0x7fc000

static inline css_error set_border_top_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[BORDER_TOP_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~BORDER_TOP_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << BORDER_TOP_WIDTH_SHIFT);
...
#define BOTTOM_MASK 0xff

static inline css_error set_bottom(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[BOTTOM_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~BOTTOM_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << BOTTOM_SHIFT);
...
static inline css_error set_box_sizing(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~BOX_SIZING_MASK) | (((uint32_t)type & 0x3) << BOX_SIZING_SHIFT);
...
static inline css_error set_break_after(css_computed_style *style, uint8_t type)
{
    ...
    /* 4bits: tttt : type */
    *bits = (*bits & ~BREAK_AFTER_MASK) | (((uint32_t)type & 0xf) << BREAK_AFTER_SHIFT);
...
#define BREAK_BEFORE_INDEX 8
#define BREAK_BEFORE_SHIFT 0
#define BREAK_BEFORE_MASK 0xf

static inline css_error set_break_before(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BREAK_BEFORE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BREAK_BEFORE_MASK) | (((uint32_t)type & 0xf) << BREAK_BEFORE_SHIFT);
...
static inline css_error set_break_inside(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[BREAK_INSIDE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~BREAK_INSIDE_MASK) | (((uint32_t)type & 0xf) << BREAK_INSIDE_SHIFT);
...
static inline css_error set_caption_side(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[CAPTION_SIDE_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~CAPTION_SIDE_MASK) | (((uint32_t)type & 0x3) << CAPTION_SIDE_SHIFT);
...
static inline css_error set_clip(css_computed_style *style, uint8_t type, css_computed_clip_rect *rect)
{
    uint32_t *bits;
    ...
    */
    *bits = (*bits & ~CLIP_MASK) | (((uint32_t)type & 0x3) << CLIP_SHIFT);

    if (type == CSS_CLIP_RECT) {
        *bits |= (((rect->top_auto ? 0x20 : 0) | (rect->right_auto ? 0x10 : 0) | (rect->bottom_auto ? 0x8 : 0) |
                      (rect->left_auto ? 0x4 : 0))
            << CLIP_SHIFT);

        *bits |= (((rect->tunit << 5) | rect->runit) << (CLIP_SHIFT + 16));

        *bits |= (((rect->bunit << 5) | rect->lunit) << (CLIP_SHIFT + 6));
...
#define COLOR_INDEX 16
#define COLOR_SHIFT 17
#define COLOR_MASK 0x20000

static inline css_error set_color(css_computed_style *style, uint8_t type, css_color color)
{
...
static inline css_error set_column_count(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[COLUMN_COUNT_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~COLUMN_COUNT_MASK) | (((uint32_t)type & 0x3) << COLUMN_COUNT_SHIFT);
...
static inline css_error set_column_fill(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~COLUMN_FILL_MASK) | (((uint32_t)type & 0x3) << COLUMN_FILL_SHIFT);
...
#define COLUMN_GAP_SHIFT 8
#define COLUMN_GAP_MASK 0xff00

static inline css_error set_column_gap(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[COLUMN_GAP_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~COLUMN_GAP_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << COLUMN_GAP_SHIFT);
...
static inline css_error set_column_rule_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[COLUMN_RULE_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~COLUMN_RULE_COLOR_MASK) | (((uint32_t)type & 0x3) << COLUMN_RULE_COLOR_SHIFT);
...
#define COLUMN_RULE_STYLE_SHIFT 8
#define COLUMN_RULE_STYLE_MASK 0xf00

static inline css_error set_column_rule_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[COLUMN_RULE_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~COLUMN_RULE_STYLE_MASK) | (((uint32_t)type & 0xf) << COLUMN_RULE_STYLE_SHIFT);
...
#define COLUMN_RULE_WIDTH_INDEX 6
#define COLUMN_RULE_WIDTH_SHIFT 23
#define COLUMN_RULE_WIDTH_MASK 0xff800000

static inline css_error set_column_rule_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[COLUMN_RULE_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~COLUMN_RULE_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << COLUMN_RULE_WIDTH_SHIFT);
...
static inline css_error set_column_span(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~COLUMN_SPAN_MASK) | (((uint32_t)type & 0x3) << COLUMN_SPAN_SHIFT);
...
#define COLUMN_WIDTH_SHIFT 16
#define COLUMN_WIDTH_MASK 0xff0000

static inline css_error set_column_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[COLUMN_WIDTH_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~COLUMN_WIDTH_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << COLUMN_WIDTH_SHIFT);
...
static inline css_error set_content(css_computed_style *style, uint8_t type, css_computed_content_item *content)
{
    uint32_t *bits;
    ...
    *bits = (*bits & ~CONTENT_MASK) | (((uint32_t)type & 0x3) << CONTENT_SHIFT);

    for (c = content; c != NULL && c->type != CSS_COMPUTED_CONTENT_NONE; c++) {
        switch (c->type) {
    ...
    /* Free existing array */
    if (oldcontent != NULL) {
        for (c = oldcontent; c->type != CSS_COMPUTED_CONTENT_NONE; c++) {
            switch (c->type) {
            case CSS_COMPUTED_CONTENT_STRING:
...
#define COUNTER_INCREMENT_INDEX 16
#define COUNTER_INCREMENT_SHIFT 18
#define COUNTER_INCREMENT_MASK 0x40000

static inline css_error
set_counter_increment(css_computed_style *style, uint8_t type, css_computed_counter *counter_arr)
{
    uint32_t *bits = &style->i.bits[COUNTER_INCREMENT_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~COUNTER_INCREMENT_MASK) | (((uint32_t)type & 0x1) << COUNTER_INCREMENT_SHIFT);
...
static inline css_error set_counter_reset(css_computed_style *style, uint8_t type, css_computed_counter *counter_arr)
{
    uint32_t *bits = &style->i.bits[COUNTER_RESET_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~COUNTER_RESET_MASK) | (((uint32_t)type & 0x1) << COUNTER_RESET_SHIFT);
...
#define CURSOR_SHIFT 20
#define CURSOR_MASK 0x1f00000

static inline css_error set_cursor(css_computed_style *style, uint8_t type, lwc_string **string_arr)
{
    uint32_t *bits = &style->i.bits[CURSOR_INDEX];

    /* 5bits: ttttt : type */
    *bits = (*bits & ~CURSOR_MASK) | (((uint32_t)type & 0x1f) << CURSOR_SHIFT);
...
static inline css_error set_direction(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~DIRECTION_MASK) | (((uint32_t)type & 0x3) << DIRECTION_SHIFT);
...
#define DISPLAY_INDEX 14
#define DISPLAY_SHIFT 1
#define DISPLAY_MASK 0x3e
...
static inline css_error set_display(css_computed_style *style, uint8_t type)
{
    ...
    /* 5bits: ttttt : type */
    *bits = (*bits & ~DISPLAY_MASK) | (((uint32_t)type & 0x1f) << DISPLAY_SHIFT);
...
static inline css_error set_empty_cells(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~EMPTY_CELLS_MASK) | (((uint32_t)type & 0x3) << EMPTY_CELLS_SHIFT);
...
#define FILL_OPACITY_INDEX 16
#define FILL_OPACITY_SHIFT 20
#define FILL_OPACITY_MASK 0x100000

static inline css_error set_fill_opacity(css_computed_style *style, uint8_t type, css_fixed fixed)
{
    uint32_t *bits = &style->i.bits[FILL_OPACITY_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~FILL_OPACITY_MASK) | (((uint32_t)type & 0x1) << FILL_OPACITY_SHIFT);
...
#define FLEX_BASIS_SHIFT 24
#define FLEX_BASIS_MASK 0xff000000

static inline css_error set_flex_basis(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[FLEX_BASIS_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~FLEX_BASIS_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << FLEX_BASIS_SHIFT);
...
static inline css_error set_flex_direction(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[FLEX_DIRECTION_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~FLEX_DIRECTION_MASK) | (((uint32_t)type & 0x7) << FLEX_DIRECTION_SHIFT);
...
#define FLEX_GROW_INDEX 16
#define FLEX_GROW_SHIFT 21
#define FLEX_GROW_MASK 0x200000

static inline css_error set_flex_grow(css_computed_style *style, uint8_t type, css_fixed fixed)
{
    uint32_t *bits = &style->i.bits[FLEX_GROW_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~FLEX_GROW_MASK) | (((uint32_t)type & 0x1) << FLEX_GROW_SHIFT);
...
#define FLEX_SHRINK_INDEX 16
#define FLEX_SHRINK_SHIFT 22
#define FLEX_SHRINK_MASK 0x400000

static inline css_error set_flex_shrink(css_computed_style *style, uint8_t type, css_fixed fixed)
{
    uint32_t *bits = &style->i.bits[FLEX_SHRINK_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~FLEX_SHRINK_MASK) | (((uint32_t)type & 0x1) << FLEX_SHRINK_SHIFT);
...
static inline css_error set_flex_wrap(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~FLEX_WRAP_MASK) | (((uint32_t)type & 0x3) << FLEX_WRAP_SHIFT);
...
static inline css_error set_font_family(css_computed_style *style, uint8_t type, lwc_string **string_arr)
{
    uint32_t *bits = &style->i.bits[FONT_FAMILY_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~FONT_FAMILY_MASK) | (((uint32_t)type & 0x7) << FONT_FAMILY_SHIFT);
...
#define FONT_SIZE_INDEX 9
#define FONT_SIZE_SHIFT 12
#define FONT_SIZE_MASK 0x3ff000

static inline css_error set_font_size(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[FONT_SIZE_INDEX];

    /* 10bits: uuuuuutttt : unit | type */
    *bits = (*bits & ~FONT_SIZE_MASK) | ((((uint32_t)type & 0xf) | (unit << 4)) << FONT_SIZE_SHIFT);
...
#define FONT_STYLE_SHIFT 1
#define FONT_STYLE_MASK 0x6
...
static inline css_error set_font_style(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~FONT_STYLE_MASK) | (((uint32_t)type & 0x3) << FONT_STYLE_SHIFT);
...
static inline css_error set_font_variant(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[FONT_VARIANT_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~FONT_VARIANT_MASK) | (((uint32_t)type & 0x3) << FONT_VARIANT_SHIFT);
...
#define FONT_WEIGHT_SHIFT 12
#define FONT_WEIGHT_MASK 0xf000
...
static inline css_error set_font_weight(css_computed_style *style, uint8_t type)
{
    ...
    /* 4bits: tttt : type */
    *bits = (*bits & ~FONT_WEIGHT_MASK) | (((uint32_t)type & 0xf) << FONT_WEIGHT_SHIFT);
...
#define GRID_AUTO_FLOW_SHIFT 20
#define GRID_AUTO_FLOW_MASK 0x700000

static inline css_error set_grid_auto_flow(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[GRID_AUTO_FLOW_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~GRID_AUTO_FLOW_MASK) | (((uint32_t)type & 0x7) << GRID_AUTO_FLOW_SHIFT);
...
#define GRID_COLUMN_END_SHIFT 5
#define GRID_COLUMN_END_MASK 0x60

static inline css_error set_grid_column_end(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[GRID_COLUMN_END_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_COLUMN_END_MASK) | (((uint32_t)type & 0x3) << GRID_COLUMN_END_SHIFT);
...
#define GRID_COLUMN_START_SHIFT 7
#define GRID_COLUMN_START_MASK 0x180

static inline css_error set_grid_column_start(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[GRID_COLUMN_START_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_COLUMN_START_MASK) | (((uint32_t)type & 0x3) << GRID_COLUMN_START_SHIFT);
...
#define GRID_ROW_END_SHIFT 9
#define GRID_ROW_END_MASK 0x600

static inline css_error set_grid_row_end(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[GRID_ROW_END_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_ROW_END_MASK) | (((uint32_t)type & 0x3) << GRID_ROW_END_SHIFT);
...
#define GRID_ROW_START_MASK 0x1800

static inline css_error set_grid_row_start(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[GRID_ROW_START_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_ROW_START_MASK) | (((uint32_t)type & 0x3) << GRID_ROW_START_SHIFT);
...
#define GRID_TEMPLATE_COLUMNS_SHIFT 13
#define GRID_TEMPLATE_COLUMNS_MASK 0x6000

static inline css_error
set_grid_template_columns(css_computed_style *style, uint8_t type, css_computed_grid_track *grid_track_arr)
{
    uint32_t *bits = &style->i.bits[GRID_TEMPLATE_COLUMNS_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_TEMPLATE_COLUMNS_MASK) | (((uint32_t)type & 0x3) << GRID_TEMPLATE_COLUMNS_SHIFT);

    css_computed_grid_track *old_grid_track_arr = style->grid_template_columns;
...
#define GRID_TEMPLATE_ROWS_SHIFT 15
#define GRID_TEMPLATE_ROWS_MASK 0x18000

static inline css_error
set_grid_template_rows(css_computed_style *style, uint8_t type, css_computed_grid_track *grid_track_arr)
{
    uint32_t *bits = &style->i.bits[GRID_TEMPLATE_ROWS_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~GRID_TEMPLATE_ROWS_MASK) | (((uint32_t)type & 0x3) << GRID_TEMPLATE_ROWS_SHIFT);
...
static inline css_error set_height(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[HEIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~HEIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << HEIGHT_SHIFT);
...
static inline css_error set_justify_content(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~JUSTIFY_CONTENT_MASK) | (((uint32_t)type & 0x7) << JUSTIFY_CONTENT_SHIFT);
...
static inline css_error set_left(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[LEFT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~LEFT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << LEFT_SHIFT);
...
#define LETTER_SPACING_SHIFT 16
#define LETTER_SPACING_MASK 0xff0000

static inline css_error set_letter_spacing(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[LETTER_SPACING_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~LETTER_SPACING_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << LETTER_SPACING_SHIFT);
...
#define LINE_HEIGHT_SHIFT 24
#define LINE_HEIGHT_MASK 0xff000000

static inline css_error set_line_height(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[LINE_HEIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~LINE_HEIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << LINE_HEIGHT_SHIFT);
...
#define LIST_STYLE_IMAGE_INDEX 16
#define LIST_STYLE_IMAGE_SHIFT 23
#define LIST_STYLE_IMAGE_MASK 0x800000

static inline css_error set_list_style_image(css_computed_style *style, uint8_t type, lwc_string *string)
{
    uint32_t *bits = &style->i.bits[LIST_STYLE_IMAGE_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~LIST_STYLE_IMAGE_MASK) | (((uint32_t)type & 0x1) << LIST_STYLE_IMAGE_SHIFT);
...
#define LIST_STYLE_POSITION_SHIFT 17
#define LIST_STYLE_POSITION_MASK 0x60000

static inline css_error set_list_style_position(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[LIST_STYLE_POSITION_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~LIST_STYLE_POSITION_MASK) | (((uint32_t)type & 0x3) << LIST_STYLE_POSITION_SHIFT);
...
static inline css_error set_list_style_type(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[LIST_STYLE_TYPE_INDEX];

    /* 6bits: tttttt : type */
    *bits = (*bits & ~LIST_STYLE_TYPE_MASK) | (((uint32_t)type & 0x3f) << LIST_STYLE_TYPE_SHIFT);
...
#define MARGIN_BOTTOM_SHIFT 0
#define MARGIN_BOTTOM_MASK 0xff

static inline css_error set_margin_bottom(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MARGIN_BOTTOM_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MARGIN_BOTTOM_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MARGIN_BOTTOM_SHIFT);
...
#define MARGIN_LEFT_SHIFT 8
#define MARGIN_LEFT_MASK 0xff00

static inline css_error set_margin_left(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MARGIN_LEFT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MARGIN_LEFT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MARGIN_LEFT_SHIFT);
...
#define MARGIN_RIGHT_SHIFT 16
#define MARGIN_RIGHT_MASK 0xff0000

static inline css_error set_margin_right(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MARGIN_RIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MARGIN_RIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MARGIN_RIGHT_SHIFT);
...
#define MARGIN_TOP_SHIFT 24
#define MARGIN_TOP_MASK 0xff000000

static inline css_error set_margin_top(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MARGIN_TOP_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MARGIN_TOP_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MARGIN_TOP_SHIFT);
...
#define MAX_HEIGHT_SHIFT 0
#define MAX_HEIGHT_MASK 0xff

static inline css_error set_max_height(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MAX_HEIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MAX_HEIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MAX_HEIGHT_SHIFT);
...
#define MAX_WIDTH_SHIFT 8
#define MAX_WIDTH_MASK 0xff00

static inline css_error set_max_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MAX_WIDTH_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MAX_WIDTH_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MAX_WIDTH_SHIFT);
...
#define MIN_HEIGHT_SHIFT 16
#define MIN_HEIGHT_MASK 0xff0000

static inline css_error set_min_height(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MIN_HEIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MIN_HEIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MIN_HEIGHT_SHIFT);
...
#define MIN_WIDTH_SHIFT 24
#define MIN_WIDTH_MASK 0xff000000

static inline css_error set_min_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[MIN_WIDTH_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~MIN_WIDTH_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << MIN_WIDTH_SHIFT);
...
static inline css_error set_opacity(css_computed_style *style, uint8_t type, css_fixed fixed)
{
    uint32_t *bits = &style->i.bits[OPACITY_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~OPACITY_MASK) | (((uint32_t)type & 0x1) << OPACITY_SHIFT);
...
#define ORDER_INDEX 16
#define ORDER_SHIFT 25
#define ORDER_MASK 0x2000000

static inline css_error set_order(css_computed_style *style, uint8_t type, int32_t integer)
{
...
#define ORPHANS_INDEX 16
#define ORPHANS_SHIFT 26
#define ORPHANS_MASK 0x4000000

static inline css_error set_orphans(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[ORPHANS_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~ORPHANS_MASK) | (((uint32_t)type & 0x1) << ORPHANS_SHIFT);
...
#define OUTLINE_COLOR_INDEX 15
#define OUTLINE_COLOR_SHIFT 19
#define OUTLINE_COLOR_MASK 0x180000

static inline css_error set_outline_color(css_computed_style *style, uint8_t type, css_color color)
{
    uint32_t *bits = &style->i.bits[OUTLINE_COLOR_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~OUTLINE_COLOR_MASK) | (((uint32_t)type & 0x3) << OUTLINE_COLOR_SHIFT);
...
#define OUTLINE_STYLE_INDEX 8
#define OUTLINE_STYLE_SHIFT 16
#define OUTLINE_STYLE_MASK 0xf0000

static inline css_error set_outline_style(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[OUTLINE_STYLE_INDEX];

    /* 4bits: tttt : type */
    *bits = (*bits & ~OUTLINE_STYLE_MASK) | (((uint32_t)type & 0xf) << OUTLINE_STYLE_SHIFT);
...
#define OUTLINE_WIDTH_INDEX 9
#define OUTLINE_WIDTH_SHIFT 3
#define OUTLINE_WIDTH_MASK 0xff8

static inline css_error set_outline_width(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[OUTLINE_WIDTH_INDEX];

    /* 9bits: uuuuuuttt : unit | type */
    *bits = (*bits & ~OUTLINE_WIDTH_MASK) | ((((uint32_t)type & 0x7) | (unit << 3)) << OUTLINE_WIDTH_SHIFT);
...
static inline css_error set_overflow_x(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~OVERFLOW_X_MASK) | (((uint32_t)type & 0x7) << OVERFLOW_X_SHIFT);
...
static inline css_error set_overflow_y(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~OVERFLOW_Y_MASK) | (((uint32_t)type & 0x7) << OVERFLOW_Y_SHIFT);
...
#define PADDING_BOTTOM_SHIFT 25
#define PADDING_BOTTOM_MASK 0xfe000000

static inline css_error set_padding_bottom(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[PADDING_BOTTOM_INDEX];

    /* 7bits: uuuuuut : unit | type */
    *bits = (*bits & ~PADDING_BOTTOM_MASK) | ((((uint32_t)type & 0x1) | (unit << 1)) << PADDING_BOTTOM_SHIFT);
...
#define PADDING_LEFT_SHIFT 4
#define PADDING_LEFT_MASK 0x7f0

static inline css_error set_padding_left(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[PADDING_LEFT_INDEX];

    /* 7bits: uuuuuut : unit | type */
    *bits = (*bits & ~PADDING_LEFT_MASK) | ((((uint32_t)type & 0x1) | (unit << 1)) << PADDING_LEFT_SHIFT);
...
#define PADDING_RIGHT_SHIFT 11
#define PADDING_RIGHT_MASK 0x3f800

static inline css_error set_padding_right(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[PADDING_RIGHT_INDEX];

    /* 7bits: uuuuuut : unit | type */
    *bits = (*bits & ~PADDING_RIGHT_MASK) | ((((uint32_t)type & 0x1) | (unit << 1)) << PADDING_RIGHT_SHIFT);
...
#define PADDING_TOP_SHIFT 18
#define PADDING_TOP_MASK 0x1fc0000

static inline css_error set_padding_top(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[PADDING_TOP_INDEX];

    /* 7bits: uuuuuut : unit | type */
    *bits = (*bits & ~PADDING_TOP_MASK) | ((((uint32_t)type & 0x1) | (unit << 1)) << PADDING_TOP_SHIFT);
...
static inline css_error set_page_break_after(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[PAGE_BREAK_AFTER_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~PAGE_BREAK_AFTER_MASK) | (((uint32_t)type & 0x7) << PAGE_BREAK_AFTER_SHIFT);
...
#define PAGE_BREAK_BEFORE_INDEX 10
#define PAGE_BREAK_BEFORE_SHIFT 3
#define PAGE_BREAK_BEFORE_MASK 0x38

static inline css_error set_page_break_before(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[PAGE_BREAK_BEFORE_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~PAGE_BREAK_BEFORE_MASK) | (((uint32_t)type & 0x7) << PAGE_BREAK_BEFORE_SHIFT);
...
#define PAGE_BREAK_INSIDE_INDEX 15
#define PAGE_BREAK_INSIDE_SHIFT 21
#define PAGE_BREAK_INSIDE_MASK 0x600000

static inline css_error set_page_break_inside(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[PAGE_BREAK_INSIDE_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~PAGE_BREAK_INSIDE_MASK) | (((uint32_t)type & 0x3) << PAGE_BREAK_INSIDE_SHIFT);
...
#define POSITION_SHIFT 6
#define POSITION_MASK 0x1c0
...
static inline css_error set_position(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~POSITION_MASK) | (((uint32_t)type & 0x7) << POSITION_SHIFT);
...
static inline css_error set_quotes(css_computed_style *style, uint8_t type, lwc_string **string_arr)
{
    uint32_t *bits = &style->i.bits[QUOTES_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~QUOTES_MASK) | (((uint32_t)type & 0x1) << QUOTES_SHIFT);
...
static inline css_error set_right(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[RIGHT_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~RIGHT_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << RIGHT_SHIFT);
...
#define ROW_GAP_INDEX 0
#define ROW_GAP_SHIFT 8
#define ROW_GAP_MASK 0xff00

static inline css_error set_row_gap(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[ROW_GAP_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~ROW_GAP_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << ROW_GAP_SHIFT);
...
static inline css_error set_stroke_opacity(css_computed_style *style, uint8_t type, css_fixed fixed)
{
    uint32_t *bits = &style->i.bits[STROKE_OPACITY_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~STROKE_OPACITY_MASK) | (((uint32_t)type & 0x1) << STROKE_OPACITY_SHIFT);
...
#define TABLE_LAYOUT_INDEX 15
#define TABLE_LAYOUT_SHIFT 23
#define TABLE_LAYOUT_MASK 0x1800000

static inline css_error set_table_layout(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[TABLE_LAYOUT_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~TABLE_LAYOUT_MASK) | (((uint32_t)type & 0x3) << TABLE_LAYOUT_SHIFT);
...
static inline css_error set_text_align(css_computed_style *style, uint8_t type)
{
    ...
    /* 4bits: tttt : type */
    *bits = (*bits & ~TEXT_ALIGN_MASK) | (((uint32_t)type & 0xf) << TEXT_ALIGN_SHIFT);
...
static inline css_error set_text_decoration(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[TEXT_DECORATION_INDEX];

    /* 5bits: ttttt : type */
    *bits = (*bits & ~TEXT_DECORATION_MASK) | (((uint32_t)type & 0x1f) << TEXT_DECORATION_SHIFT);
...
#define TEXT_INDENT_SHIFT 25
#define TEXT_INDENT_MASK 0xfe000000

static inline css_error set_text_indent(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[TEXT_INDENT_INDEX];

    /* 7bits: uuuuuut : unit | type */
    *bits = (*bits & ~TEXT_INDENT_MASK) | ((((uint32_t)type & 0x1) | (unit << 1)) << TEXT_INDENT_SHIFT);
...
#define TEXT_TRANSFORM_SHIFT 9
#define TEXT_TRANSFORM_MASK 0xe00

static inline css_error set_text_transform(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[TEXT_TRANSFORM_INDEX];

    /* 3bits: ttt : type */
    *bits = (*bits & ~TEXT_TRANSFORM_MASK) | (((uint32_t)type & 0x7) << TEXT_TRANSFORM_SHIFT);
...
#define TOP_INDEX 0
#define TOP_SHIFT 16
#define TOP_MASK 0xff0000

static inline css_error set_top(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[TOP_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~TOP_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << TOP_SHIFT);
...
#define UNICODE_BIDI_SHIFT 25
#define UNICODE_BIDI_MASK 0x6000000

static inline css_error set_unicode_bidi(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[UNICODE_BIDI_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~UNICODE_BIDI_MASK) | (((uint32_t)type & 0x3) << UNICODE_BIDI_SHIFT);
...
#define VERTICAL_ALIGN_INDEX 9
#define VERTICAL_ALIGN_SHIFT 22
#define VERTICAL_ALIGN_MASK 0xffc00000

static inline css_error set_vertical_align(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[VERTICAL_ALIGN_INDEX];

    /* 10bits: uuuuuutttt : unit | type */
    *bits = (*bits & ~VERTICAL_ALIGN_MASK) | ((((uint32_t)type & 0xf) | (unit << 4)) << VERTICAL_ALIGN_SHIFT);
...
#define VISIBILITY_INDEX 15
#define VISIBILITY_SHIFT 27
#define VISIBILITY_MASK 0x18000000
...
static inline css_error set_visibility(css_computed_style *style, uint8_t type)
{
    ...
    /* 2bits: tt : type */
    *bits = (*bits & ~VISIBILITY_MASK) | (((uint32_t)type & 0x3) << VISIBILITY_SHIFT);
...
static inline css_error set_white_space(css_computed_style *style, uint8_t type)
{
    ...
    /* 3bits: ttt : type */
    *bits = (*bits & ~WHITE_SPACE_MASK) | (((uint32_t)type & 0x7) << WHITE_SPACE_SHIFT);
...
static inline css_error set_widows(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[WIDOWS_INDEX];

    /* 1bit: t : type */
    *bits = (*bits & ~WIDOWS_MASK) | (((uint32_t)type & 0x1) << WIDOWS_SHIFT);
...
#define WIDTH_MASK 0xff000000

static inline css_error set_width(css_computed_style *style, uint8_t type, css_fixed_or_calc length, css_unit unit)
{
    uint32_t orig_bits = (style->i.bits[WIDTH_INDEX] & WIDTH_MASK) >> WIDTH_SHIFT;
    ...
    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~WIDTH_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << WIDTH_SHIFT);
...
#define WORD_SPACING_INDEX 14
#define WORD_SPACING_SHIFT 6
#define WORD_SPACING_MASK 0x3fc0

static inline css_error set_word_spacing(css_computed_style *style, uint8_t type, css_fixed length, css_unit unit)
{
    uint32_t *bits = &style->i.bits[WORD_SPACING_INDEX];

    /* 8bits: uuuuuutt : unit | type */
    *bits = (*bits & ~WORD_SPACING_MASK) | ((((uint32_t)type & 0x3) | (unit << 2)) << WORD_SPACING_SHIFT);
...
static inline css_error set_writing_mode(css_computed_style *style, uint8_t type)
{
    uint32_t *bits = &style->i.bits[WRITING_MODE_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~WRITING_MODE_MASK) | (((uint32_t)type & 0x3) << WRITING_MODE_SHIFT);
...
static inline css_error set_z_index(css_computed_style *style, uint8_t type, int32_t integer)
{
    uint32_t *bits = &style->i.bits[Z_INDEX_INDEX];

    /* 2bits: tt : type */
    *bits = (*bits & ~Z_INDEX_MASK) | (((uint32_t)type & 0x3) << Z_INDEX_SHIFT);
