 ...
 */
static inline css_unit
css_unit__map_viewport_units(const css_computed_style *style,
			     ...
			     const css_unit unit)
{
	...
	case CSS_UNIT_VMIN:
		return (viewport_height < viewport_width) ? CSS_UNIT_VH
							  : CSS_UNIT_VW;
...
css_unit__absolute_len2pt(const css_computed_style *style,
			  const css_fixed viewport_height,
			  const css_fixed viewport_width,
			  ...
			  const css_unit unit)
{
	...
	assert(unit != CSS_UNIT_EM && unit != CSS_UNIT_EX &&
	       unit != CSS_UNIT_CH && unit != CSS_UNIT_IC &&
	       unit != CSS_UNIT_REM);

	switch (css_unit__map_viewport_units(
		style, viewport_height, viewport_width, unit)) {
	case CSS_UNIT_PX:
				    ...
				    const css_unit unit)
{
	return css_unit__absolute_len2pt(
		style, ctx->viewport_height, ctx->viewport_width, length, unit);
}
 ...
 */
static inline css_fixed
css_unit__font_size_px(const css_computed_style *style,
 ...
 */
static inline css_fixed
css_unit__px_per_unit(const css_unit_len_measure measure,
...
{
	switch (css_unit__map_viewport_units(
		ref_style, viewport_height, viewport_width, unit)) {
	case CSS_UNIT_EM:
		return css_unit__font_size_px(ref_style,
					      font_size_default,
					      font_size_minimum,
					      viewport_height,
					      viewport_width);
	...
	case CSS_UNIT_EX:
		...
		return FMUL(css_unit__font_size_px(ref_style,
						   font_size_default,
						   font_size_minimum,
						   viewport_height,
						   viewport_width),
	...
	case CSS_UNIT_REM:
		return css_unit__font_size_px(root_style,
					      font_size_default,
					      font_size_minimum,
					      viewport_height,
					      viewport_width);
			     ...
			     const css_unit unit)
{
	...
	css_fixed px_per_unit = css_unit__px_per_unit(ctx->measure,
						      NULL,
						      NULL,
 ...
 */
static inline css_hint_length
css_unit__get_font_size(const css_computed_style *style,
...
/* Exported function, documented in unit.h. */
css_error
css_unit_compute_absolute_font_size(const css_hint_length *ref_length,
				    const css_computed_style *root_style,
				    css_fixed font_size_default,
				    css_hint *size)
{
	...
	case CSS_FONT_SIZE_DIMENSION:
		...
		case CSS_UNIT_PCT:
			size->data.length.value = FDIV(
				FMUL(size->data.length.value, ref_len.value),
				INTTOFIX(100));
			size->data.length.unit = ref_len.unit;
			break;
		...
		case CSS_UNIT_IC:
			...
			case CSS_UNIT_EX:
				size->data.length.value = FMUL(
					size->data.length.value, FLTTOFIX(0.6));
				break;

			case CSS_UNIT_CH:
				size->data.length.value = FMUL(
					size->data.length.value, FLTTOFIX(0.4));
				break;
