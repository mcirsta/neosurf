 ...
 */
static inline css_unit css_unit__map_viewport_units(const css_computed_style *style, const css_fixed viewport_height,
    const css_fixed viewport_width, const css_unit unit)
{
    switch (unit) {
    case CSS_UNIT_VI:
        return (style != NULL && get_writing_mode(style) != CSS_WRITING_MODE_HORIZONTAL_TB) ? CSS_UNIT_VH : CSS_UNIT_VW;

    case CSS_UNIT_VB:
        return (style != NULL && get_writing_mode(style) != CSS_WRITING_MODE_HORIZONTAL_TB) ? CSS_UNIT_VW : CSS_UNIT_VH;

    case CSS_UNIT_CQI:
        return (style != NULL && get_writing_mode(style) != CSS_WRITING_MODE_HORIZONTAL_TB) ? CSS_UNIT_CQH
                                                                                            : CSS_UNIT_CQW;

    case CSS_UNIT_CQB:
        return (style != NULL && get_writing_mode(style) != CSS_WRITING_MODE_HORIZONTAL_TB) ? CSS_UNIT_CQW
                                                                                            : CSS_UNIT_CQH;

    case CSS_UNIT_VMIN:
        return (viewport_height < viewport_width) ? CSS_UNIT_VH : CSS_UNIT_VW;
    ...
    const css_fixed viewport_width, const css_fixed length, const css_unit unit)
{
    ...
    switch (css_unit__map_viewport_units(style, viewport_height, viewport_width, unit)) {
    case CSS_UNIT_PX:
    ...
    case CSS_UNIT_VH:
        return FDIV(FMUL(FDIV(FMUL(length, viewport_height), F_100), F_72), F_96);
 ...
 */
static inline css_hint_length css_unit__get_font_size(const css_computed_style *style, css_fixed font_size_default)
{
...
/* Exported function, documented in unit.h. */
css_error css_unit_compute_absolute_font_size(const css_hint_length *ref_length, const css_computed_style *root_style,
    css_fixed font_size_default, css_hint *size)
{
    ...
    case CSS_FONT_SIZE_XX_LARGE: {
        ...
        size->data.length.value = FMUL(factors[size->status - 1], font_size_default);
    ...
    case CSS_FONT_SIZE_DIMENSION:
        ...
        case CSS_UNIT_PCT:
            size->data.length.value = FDIV(FMUL(size->data.length.value, ref_len.value), INTTOFIX(100));
            size->data.length.unit = ref_len.unit;
            break;
        ...
        case CSS_UNIT_IC:
            ...
            size->data.length.value = FMUL(size->data.length.value, ref_len.value);
            ...
            case CSS_UNIT_CH:
                size->data.length.value = FMUL(size->data.length.value, FLTTOFIX(0.4));
                break;
        ...
        case CSS_UNIT_REM:
            ...
            ref_len = css_unit__get_font_size(root_style, font_size_default);
