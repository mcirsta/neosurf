...
static inline bool arena__compare_computed_content_item(
    const struct css_computed_content_item *a, const struct css_computed_content_item *b)
{
    if (a == NULL && b == NULL) {
        return true;
...
static inline bool
arena__compare_css_computed_counter(const struct css_computed_counter *a, const struct css_computed_counter *b)
{
    bool match;
    ...
    if (a->value == b->value && lwc_string_isequal(a->name, b->name, &match) == lwc_error_ok && match == true) {
        return true;
    }
...
static inline bool arena__compare_string_list(lwc_string **a, lwc_string **b)
{
    ...
    while (*a != NULL && *b != NULL) {
        ...
        if (lwc_string_isequal(*a, *b, &match) != lwc_error_ok || match == false) {
            return false;
        }
...
static inline bool css__arena_style_is_equal(struct css_computed_style *a, struct css_computed_style *b)
{
    ...
    if (!arena__compare_css_computed_counter(a->counter_increment, b->counter_increment)) {
        return false;
    }

    if (!arena__compare_css_computed_counter(a->counter_reset, b->counter_reset)) {
        return false;
    }
