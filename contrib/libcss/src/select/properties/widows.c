...
css_error
css__cascade_widows(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error
css__copy_widows(const css_computed_style *from, css_computed_style *to)
{
	int32_t count = 0;
			      ...
			      css_computed_style *result)
{
	...
	return css__copy_widows(type == CSS_WIDOWS_INHERIT ? parent : child,
				result);
}
