...
css_error css__cascade_border_top_color(uint32_t opv, css_style *style, css_select_state *state)
{
    return css__cascade_bg_border_color(opv, style, state, set_border_top_color);
}

css_error css__set_border_top_color_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_border_top_color(const css_computed_style *from, css_computed_style *to)
{
    css_color color;
...
css_error css__compose_border_top_color(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_border_top_color(type == CSS_BORDER_COLOR_INHERIT ? parent : child, result);
}
