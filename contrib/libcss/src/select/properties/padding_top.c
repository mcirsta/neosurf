...
css_error css__cascade_padding_top(uint32_t opv,
				   css_style *style,
...
css_error
css__set_padding_top_from_hint(const css_hint *hint, css_computed_style *style)
{
	return set_padding_top(style,
			       hint->status,
...
{
	return set_padding_top(
		state->computed, CSS_PADDING_SET, 0, CSS_UNIT_PX);
}

css_error
css__copy_padding_top(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
