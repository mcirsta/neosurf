...
css_error css__cascade_width(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__copy_width(const css_computed_style *from, css_computed_style *to)
{
...
css_error
css__compose_width(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    css_fixed_or_calc length = (css_fixed_or_calc)0;
    css_unit unit = CSS_UNIT_PX;
    uint8_t type = get_width(child, &length, &unit);

    return css__copy_width(type == CSS_WIDTH_INHERIT ? parent : child, result);
}
