...
css_error css__cascade_grid_template_rows(uint32_t opv, css_style *style, css_select_state *state)
{
        ...
        case GRID_TEMPLATE_SET:
            ...
            if (n_tracks > 0) {
                ...
                for (int32_t i = 0; i < n_tracks; i++) {
                    ...
                    /* First read value */
                    css_fixed track_value = *style->bytecode;
                    advance_bytecode(style, sizeof(css_code_t));
                    ...
                    if (css__to_css_unit(raw_unit) == CSS_UNIT_MINMAX) {
                        ...
                        /* Read min unit */
                        tracks[i].min_unit = css__to_css_unit(*style->bytecode);
                        advance_bytecode(style, sizeof(css_code_t));

                        /* Read max value */
                        tracks[i].max_value = *style->bytecode;
                        advance_bytecode(style, sizeof(css_code_t));
...
css_error css__set_grid_template_rows_from_hint(const css_hint *hint, css_computed_style *style)
{
...
{
    return set_grid_template_rows(state->computed, CSS_GRID_TEMPLATE_NONE, NULL);
}

css_error css__copy_grid_template_rows(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error css__compose_grid_template_rows(
