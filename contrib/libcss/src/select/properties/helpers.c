 ...
 ******************************************************************************/
css_error css__cascade_bg_border_color(
    uint32_t opv, css_style *style, css_select_state *state, css_error (*fun)(css_computed_style *, uint8_t, css_color))
{
...
css_error css__cascade_uri_none(uint32_t opv, css_style *style, css_select_state *state,
    css_error (*fun)(css_computed_style *, uint8_t, lwc_string *))
{
...
css_error css__cascade_border_style(
    ...
    if (hasFlagValue(opv) == false) {
        ...
        case BOTTOM_CALC:
            ...
            advance_bytecode(style,
                sizeof(unit)); // TODO: Skip unit, not
                               // sure what to do
...
css_error css__cascade_number(
    uint32_t opv, css_style *style, css_select_state *state, css_error (*fun)(css_computed_style *, uint8_t, css_fixed))
{
    uint16_t value = 0;
    css_fixed length = 0;
...
css_error css__cascade_break_after_before_inside(
    uint32_t opv, css_style *style, css_select_state *state, css_error (*fun)(css_computed_style *, uint8_t))
{
...
css_error css__cascade_counter_increment_reset(uint32_t opv, css_style *style, css_select_state *state,
    css_error (*fun)(css_computed_style *, uint8_t, css_computed_counter *))
{
    ...
    if (hasFlagValue(opv) == false) {
        ...
        case COUNTER_INCREMENT_NAMED: {
            ...
            while (v != COUNTER_INCREMENT_NONE) {
                ...
                temp = realloc(counters, (n_counters + 1) * sizeof(css_computed_counter));
                if (temp == NULL) {
                    if (counters != NULL) {
                        free(counters);
                    }
