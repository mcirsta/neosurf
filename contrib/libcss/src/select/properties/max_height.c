...
css_error
css__cascade_max_height(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__initial_max_height(css_select_state *state)
{
	return set_max_height(
		state->computed, CSS_MAX_HEIGHT_NONE, 0, CSS_UNIT_PX);
}

css_error
css__copy_max_height(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				  ...
				  css_computed_style *result)
{
	...
	return css__copy_max_height(
		type == CSS_MAX_HEIGHT_INHERIT ? parent : child, result);
}
