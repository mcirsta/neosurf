...
css_error css__cascade_word_spacing(uint32_t opv,
				    css_style *style,
...
css_error
css__set_word_spacing_from_hint(const css_hint *hint, css_computed_style *style)
{
	return set_word_spacing(style,
				hint->status,
...
css_error css__initial_word_spacing(css_select_state *state)
{
	return set_word_spacing(
		state->computed, CSS_WORD_SPACING_NORMAL, 0, CSS_UNIT_PX);
}

css_error
css__copy_word_spacing(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_word_spacing(
		type == CSS_WORD_SPACING_INHERIT ? parent : child, result);
}
