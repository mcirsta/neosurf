...
css_error css__cascade_azimuth(uint32_t opv, css_style *style, css_select_state *state)
{
    css_fixed val = 0;
...
css_error css__set_azimuth_from_hint(const css_hint *hint, css_computed_style *style)
{
    UNUSED(hint);
    UNUSED(style);
...
css_error css__copy_azimuth(const css_computed_style *from, css_computed_style *to)
{
    UNUSED(from);
    UNUSED(to);
...
css_error
css__compose_azimuth(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    UNUSED(parent);
    UNUSED(child);
    UNUSED(result);
