...
css_error css__cascade_grid_row_end(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_grid_row_end_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_grid_row_end(const css_computed_style *from, css_computed_style *to)
{
    css_fixed integer;
    uint8_t type;
...
css_error
css__compose_grid_row_end(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    css_fixed integer;
    uint8_t type = get_grid_row_end(child, &integer);

    return css__copy_grid_row_end(type == CSS_GRID_LINE_INHERIT ? parent : child, result);
}
