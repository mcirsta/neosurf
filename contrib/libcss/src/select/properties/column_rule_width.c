...
css_error css__cascade_column_rule_width(uint32_t opv,
					 css_style *style,
					 css_select_state *state)
{
	return css__cascade_border_width(
		opv, style, state, set_column_rule_width);
}

css_error css__set_column_rule_width_from_hint(const css_hint *hint,
					       css_computed_style *style)
{
	return set_column_rule_width(style,
				     hint->status,
...
{
	return set_column_rule_width(
		state->computed, CSS_COLUMN_RULE_WIDTH_MEDIUM, 0, CSS_UNIT_PX);
}

css_error css__copy_column_rule_width(const css_computed_style *from,
					 ...
					 css_computed_style *result)
{
	...
	return css__copy_column_rule_width(
		type == CSS_COLUMN_RULE_WIDTH_INHERIT ? parent : child, result);
}
