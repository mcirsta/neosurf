...
css_error css__cascade_letter_spacing(uint32_t opv,
				      css_style *style,
				      css_select_state *state)
{
	return css__cascade_length_normal(
...
{
	return set_letter_spacing(style,
				  hint->status,
...
css_error css__initial_letter_spacing(css_select_state *state)
{
	return set_letter_spacing(
		state->computed, CSS_LETTER_SPACING_NORMAL, 0, CSS_UNIT_PX);
}

css_error
css__copy_letter_spacing(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				      ...
				      css_computed_style *result)
{
	...
	return css__copy_letter_spacing(
		type == CSS_LETTER_SPACING_INHERIT ? parent : child, result);
}
