...
css_error css__cascade_border_collapse(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_border_collapse_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_border_collapse(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error css__compose_border_collapse(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_border_collapse(type == CSS_BORDER_COLLAPSE_INHERIT ? parent : child, result);
}
