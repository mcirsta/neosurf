...
css_error css__cascade_opacity(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_opacity_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_opacity(const css_computed_style *from, css_computed_style *to)
{
    css_fixed opacity = 0;
...
css_error
css__compose_opacity(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    css_fixed opacity = 0;
    uint8_t type = get_opacity(child, &opacity);

    return css__copy_opacity(type == CSS_OPACITY_INHERIT ? parent : child, result);
}
