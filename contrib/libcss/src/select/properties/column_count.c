...
css_error css__cascade_column_count(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_column_count_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_column_count(const css_computed_style *from, css_computed_style *to)
{
    int32_t count = 0;
...
css_error
css__compose_column_count(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    int32_t count = 0;
    uint8_t type = get_column_count(child, &count);

    return css__copy_column_count(type == CSS_COLUMN_COUNT_INHERIT ? parent : child, result);
}
