...
css_error css__cascade_font_family(uint32_t opv, css_style *style, css_select_state *state)
{
    ...
    if (n_fonts > 0) {
        ...
        if (value == CSS_FONT_FAMILY_INHERIT) {
            ...
            error = state->handler->ua_default_for_property(state->pw, CSS_PROP_FONT_FAMILY, &hint);
            if (error == CSS_OK) {
                lwc_string **item;
...
css_error css__set_font_family_from_hint(const css_hint *hint, css_computed_style *style)
{
    lwc_string **item;
    css_error error;
...
css_error css__copy_font_family(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error
css__compose_font_family(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    lwc_string **font_family = NULL;
    uint8_t type = get_font_family(child, &font_family);

    return css__copy_font_family(type == CSS_FONT_FAMILY_INHERIT ? parent : child, result);
}
