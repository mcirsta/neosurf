...
{
    return set_vertical_align(style, hint->status, hint->data.length.value, hint->data.length.unit);
}

css_error css__initial_vertical_align(css_select_state *state)
{
    return set_vertical_align(state->computed, CSS_VERTICAL_ALIGN_BASELINE, 0, CSS_UNIT_PX);
}
