...
css_error css__cascade_font_style(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_font_style_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_font_style(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_font_style(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_font_style(child);

    return css__copy_font_style(type == CSS_FONT_STYLE_INHERIT ? parent : child, result);
}
