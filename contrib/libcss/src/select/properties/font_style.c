...
css_error
css__cascade_font_style(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error
css__set_font_style_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error
css__copy_font_style(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				  ...
				  css_computed_style *result)
{
	...
	return css__copy_font_style(
		type == CSS_FONT_STYLE_INHERIT ? parent : child, result);
}
