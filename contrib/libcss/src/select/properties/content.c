...
css_error css__cascade_content(uint32_t opv, css_style *style, css_select_state *state)
{
    ...
    if (hasFlagValue(opv) == false) {
        ...
        if (v == CONTENT_NORMAL) {
            ...
            while (v != CONTENT_NORMAL) {
                ...
                temp = realloc(content, (n_contents + 1) * sizeof(css_computed_content_item));
                if (temp == NULL) {
                    if (content != NULL) {
                        free(content);
                    }
                ...
                case CONTENT_COUNTER:
                    ...
                    content[n_contents].type = CSS_COMPUTED_CONTENT_COUNTER;
                ...
                case CONTENT_COUNTERS: {
                    ...
                    content[n_contents].type = CSS_COMPUTED_CONTENT_COUNTERS;
                ...
                case CONTENT_URI:
                    ...
                    content[n_contents].type = CSS_COMPUTED_CONTENT_URI;
                    content[n_contents].data.uri = he;
                    break;
                case CONTENT_ATTR:
                    advance_bytecode(style, sizeof(css_code_t));

                    content[n_contents].type = CSS_COMPUTED_CONTENT_ATTR;
                    content[n_contents].data.attr = he;
                    break;
                case CONTENT_STRING:
                    advance_bytecode(style, sizeof(css_code_t));

                    content[n_contents].type = CSS_COMPUTED_CONTENT_STRING;
                    content[n_contents].data.string = he;
                    break;
                case CONTENT_OPEN_QUOTE:
                    content[n_contents].type = CSS_COMPUTED_CONTENT_OPEN_QUOTE;
                    break;
                case CONTENT_CLOSE_QUOTE:
                    content[n_contents].type = CSS_COMPUTED_CONTENT_CLOSE_QUOTE;
                    break;
                case CONTENT_NO_OPEN_QUOTE:
                    content[n_contents].type = CSS_COMPUTED_CONTENT_NO_OPEN_QUOTE;
                    break;
                case CONTENT_NO_CLOSE_QUOTE:
                    content[n_contents].type = CSS_COMPUTED_CONTENT_NO_CLOSE_QUOTE;
                    break;
                }
...
css_error css__set_content_from_hint(const css_hint *hint, css_computed_style *style)
{
    ...
    for (item = hint->data.content; item != NULL && item->type != CSS_COMPUTED_CONTENT_NONE; item++) {
        switch (item->type) {
...
css_error css__copy_content(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error
css__compose_content(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    const css_computed_content_item *items = NULL;
    uint8_t type = get_content(child, &items);

    return css__copy_content(type == CSS_CONTENT_INHERIT ? parent : child, result);
}
