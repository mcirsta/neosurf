...
css_error css__cascade_bg_border_color(uint32_t opv, css_style *style, css_select_state *state,
...
static inline css_error css__copy_lwc_string_array(bool ref, lwc_string *const *orig, lwc_string ***copy_out)
{
    size_t count = 0;
...
css__copy_computed_counter_array(bool ref, const css_computed_counter *orig, css_computed_counter **copy_out)
{
    size_t count = 0;
    css_computed_counter *copy = NULL;

    if (orig != NULL) {
        for (const css_computed_counter *i = orig; i->name != NULL; i++) {
            count++;
        }
...
static inline css_error css__copy_computed_content_item_array(
    bool ref, const css_computed_content_item *orig, css_computed_content_item **copy_out)
{
    size_t count = 0;
    css_computed_content_item *copy = NULL;

    if (orig != NULL) {
        for (const css_computed_content_item *i = orig; i->type != CSS_COMPUTED_CONTENT_NONE; i++) {
            count++;
        }
        ...
        if (ref) {
            for (size_t i = 0; i < count; i++) {
                ...
                case CSS_COMPUTED_CONTENT_STRING:
                    copy[i].data.string = lwc_string_ref(orig[i].data.string);
                    break;
                case CSS_COMPUTED_CONTENT_URI:
                    copy[i].data.uri = lwc_string_ref(orig[i].data.uri);
                    break;
                case CSS_COMPUTED_CONTENT_ATTR:
                    copy[i].data.attr = lwc_string_ref(orig[i].data.attr);
                    break;
                case CSS_COMPUTED_CONTENT_COUNTER:
                    copy[i].data.counter.name = lwc_string_ref(orig[i].data.counter.name);
                    copy[i].data.counter.style = orig[i].data.counter.style;
                    break;
                case CSS_COMPUTED_CONTENT_COUNTERS:
                    copy[i].data.counters.name = lwc_string_ref(orig[i].data.counters.name);
                    copy[i].data.counters.sep = lwc_string_ref(orig[i].data.counters.sep);
                    copy[i].data.counters.style = orig[i].data.counters.style;
                    break;
                default:
                    break;
                }
...
static inline css_error
css__copy_grid_track_array(const css_computed_grid_track *orig, css_computed_grid_track **copy_out)
{
    size_t count = 0;
    ...
    if (orig != NULL) {
         ...
         */
        for (const css_computed_grid_track *t = orig; t->value != 0 || t->unit != 0; t++) {
            count++;
        }
