...
css__cascade_break_after_before_inside(uint32_t opv,
				       ...
				       css_error (*fun)(css_computed_style *,
							uint8_t));
css_error
css__cascade_counter_increment_reset(uint32_t opv,
				     css_style *style,
				     css_select_state *state,
				     css_error (*fun)(css_computed_style *,
						      uint8_t,
...
static inline css_error css__copy_lwc_string_array(bool ref,
...
css__copy_computed_counter_array(bool ref,
				 ...
				 css_computed_counter **copy_out)
{
	...
	if (orig != NULL) {
		for (const css_computed_counter *i = orig; i->name != NULL;
		     i++) {
			count++;
		}
...
static inline css_error
css__copy_computed_content_item_array(bool ref,
				      ...
				      css_computed_content_item **copy_out)
{
	...
	if (orig != NULL) {
		for (const css_computed_content_item *i = orig;
		     i->type != CSS_COMPUTED_CONTENT_NONE;
		     i++) {
			count++;
		}
