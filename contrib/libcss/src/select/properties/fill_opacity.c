...
css_error css__cascade_fill_opacity(uint32_t opv,
				    css_style *style,
...
{
	return set_fill_opacity(state->computed,
				CSS_FILL_OPACITY_SET,
				INTTOFIX(1));
}

css_error
css__copy_fill_opacity(const css_computed_style *from, css_computed_style *to)
{
	css_fixed fill_opacity = 0;
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_fill_opacity(
		type == CSS_FILL_OPACITY_INHERIT ? parent : child, result);
}
