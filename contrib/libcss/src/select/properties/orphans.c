...
css_error
css__cascade_orphans(uint32_t opv, css_style *style, css_select_state *state)
{
	return css__cascade_number(opv, style, state, set_orphans);
}

css_error
css__set_orphans_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error
css__copy_orphans(const css_computed_style *from, css_computed_style *to)
{
	int32_t count = 0;
			       ...
			       css_computed_style *result)
{
	...
	return css__copy_orphans(type == CSS_ORPHANS_INHERIT ? parent : child,
				 result);
}
