...
css_error css__cascade_text_transform(uint32_t opv,
				      css_style *style,
...
css_error
css__copy_text_transform(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				      ...
				      css_computed_style *result)
{
	...
	return css__copy_text_transform(
		type == CSS_TEXT_TRANSFORM_INHERIT ? parent : child, result);
}
