...
css_error css__cascade_outline_width(uint32_t opv,
				     css_style *style,
...
{
	return set_outline_width(style,
				 hint->status,
...
{
	return set_outline_width(
		state->computed, CSS_OUTLINE_WIDTH_MEDIUM, 0, CSS_UNIT_PX);
}

css_error
css__copy_outline_width(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				     ...
				     css_computed_style *result)
{
	...
	return css__copy_outline_width(
		type == CSS_OUTLINE_WIDTH_INHERIT ? parent : child, result);
}
