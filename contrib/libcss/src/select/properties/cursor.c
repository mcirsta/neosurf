...
css_error css__cascade_cursor(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_cursor_from_hint(const css_hint *hint, css_computed_style *style)
{
    lwc_string **item;
    css_error error;
...
css_error css__copy_cursor(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error
css__compose_cursor(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    lwc_string **cursor = NULL;
    uint8_t type = get_cursor(child, &cursor);

    return css__copy_cursor(type == CSS_CURSOR_INHERIT ? parent : child, result);
}
