...
css_error
css__cascade_column_gap(uint32_t opv, css_style *style, css_select_state *state)
{
...
{
	return set_column_gap(state->computed,
...
css_error
css__copy_column_gap(const css_computed_style *from, css_computed_style *to)
{
				  ...
				  css_computed_style *result)
{
	...
	return css__copy_column_gap(
		type == CSS_COLUMN_GAP_INHERIT ? parent : child, result);
}
