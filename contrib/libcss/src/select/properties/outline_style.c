...
css_error css__cascade_outline_style(uint32_t opv,
				     css_style *style,
...
css_error
css__copy_outline_style(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				     ...
				     css_computed_style *result)
{
	...
	return css__copy_outline_style(
		type == CSS_OUTLINE_STYLE_INHERIT ? parent : child, result);
}
