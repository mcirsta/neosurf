...
css_error css__cascade_break_inside(uint32_t opv,
				    css_style *style,
...
css_error
css__copy_break_inside(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_break_inside(
		type == CSS_BREAK_INSIDE_INHERIT ? parent : child, result);
}
