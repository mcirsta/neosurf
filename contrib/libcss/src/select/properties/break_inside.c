...
css_error css__cascade_break_inside(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_break_inside_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_break_inside(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_break_inside(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_break_inside(child);

    return css__copy_break_inside(type == CSS_BREAK_INSIDE_INHERIT ? parent : child, result);
}
