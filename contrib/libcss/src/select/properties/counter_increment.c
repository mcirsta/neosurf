...
css_error css__cascade_counter_increment(uint32_t opv, css_style *style, css_select_state *state)
{
    return css__cascade_counter_increment_reset(opv, style, state, set_counter_increment);
}

css_error css__set_counter_increment_from_hint(const css_hint *hint, css_computed_style *style)
{
    ...
    if (hint->status == CSS_COUNTER_INCREMENT_NAMED && hint->data.counter != NULL) {
...
{
    return set_counter_increment(state->computed, CSS_COUNTER_INCREMENT_NONE, NULL);
}

css_error css__copy_counter_increment(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error css__compose_counter_increment(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_counter_increment(type == CSS_COUNTER_INCREMENT_INHERIT ? parent : child, result);
}
