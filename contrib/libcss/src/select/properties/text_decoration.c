...
css_error css__cascade_text_decoration(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_text_decoration_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_text_decoration(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error css__compose_text_decoration(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_text_decoration(type == CSS_TEXT_DECORATION_INHERIT ? parent : child, result);
}
