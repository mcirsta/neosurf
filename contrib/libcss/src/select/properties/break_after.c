...
css_error css__cascade_break_after(uint32_t opv,
				   css_style *style,
				   css_select_state *state)
{
	return css__cascade_break_after_before_inside(
		opv, style, state, set_break_after);
}

css_error
css__set_break_after_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error
css__copy_break_after(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				   ...
				   css_computed_style *result)
{
	...
	return css__copy_break_after(
		type == CSS_BREAK_AFTER_INHERIT ? parent : child, result);
}
