...
css_error css__cascade_align_items(uint32_t opv,
				   css_style *style,
...
css_error
css__copy_align_items(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				   ...
				   css_computed_style *result)
{
	...
	return css__copy_align_items(
		type == CSS_ALIGN_ITEMS_INHERIT ? parent : child, result);
}
