...
css_error css__cascade_margin_right(uint32_t opv,
				    css_style *style,
...
css_error
css__set_margin_right_from_hint(const css_hint *hint, css_computed_style *style)
{
	return set_margin_right(style,
				hint->status,
...
css_error css__initial_margin_right(css_select_state *state)
{
	return set_margin_right(
		state->computed, CSS_MARGIN_SET, 0, CSS_UNIT_PX);
}

css_error
css__copy_margin_right(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_margin_right(
		type == CSS_MARGIN_INHERIT ? parent : child, result);
}
