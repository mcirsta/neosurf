...
css_error css__cascade_align_content(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_align_content_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_align_content(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error css__compose_align_content(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_align_content(type == CSS_ALIGN_CONTENT_INHERIT ? parent : child, result);
}
