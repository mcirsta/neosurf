...
css_error css__cascade_padding_bottom(uint32_t opv,
				      css_style *style,
...
{
	return set_padding_bottom(style,
				  hint->status,
...
{
	return set_padding_bottom(
		state->computed, CSS_PADDING_SET, 0, CSS_UNIT_PX);
}

css_error
css__copy_padding_bottom(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
