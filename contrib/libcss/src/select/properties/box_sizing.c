...
css_error
css__cascade_box_sizing(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error
css__set_box_sizing_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error
css__copy_box_sizing(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				  ...
				  css_computed_style *result)
{
	...
	return css__copy_box_sizing(
		type == CSS_BOX_SIZING_INHERIT ? parent : child, result);
}
