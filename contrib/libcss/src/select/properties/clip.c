...
css_error css__cascade_clip(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_clip_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_clip(const css_computed_style *from, css_computed_style *to)
{
    css_computed_clip_rect rect = {
        0, 0, 0, 0, CSS_UNIT_PX, CSS_UNIT_PX, CSS_UNIT_PX, CSS_UNIT_PX, false, false, false, false};
...
css_error
css__compose_clip(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    css_computed_clip_rect rect = {
        0, 0, 0, 0, CSS_UNIT_PX, CSS_UNIT_PX, CSS_UNIT_PX, CSS_UNIT_PX, false, false, false, false};
    uint8_t type = get_clip(child, &rect);

    return css__copy_clip(type == CSS_CLIP_INHERIT ? parent : child, result);
}
