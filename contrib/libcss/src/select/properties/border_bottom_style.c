...
css_error css__cascade_border_bottom_style(uint32_t opv,
					   css_style *style,
					   css_select_state *state)
{
	return css__cascade_border_style(
		opv, style, state, set_border_bottom_style);
}
...
css_error css__copy_border_bottom_style(const css_computed_style *from,
					   ...
					   css_computed_style *result)
{
	...
	return css__copy_border_bottom_style(
		type == CSS_BORDER_STYLE_INHERIT ? parent : child, result);
}
