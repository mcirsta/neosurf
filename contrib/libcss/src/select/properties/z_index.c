...
css_error css__cascade_z_index(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_z_index_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_z_index(const css_computed_style *from, css_computed_style *to)
{
    int32_t index = 0;
...
css_error
css__compose_z_index(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    int32_t index = 0;
    uint8_t type = get_z_index(child, &index);

    return css__copy_z_index(type == CSS_Z_INDEX_INHERIT ? parent : child, result);
}
