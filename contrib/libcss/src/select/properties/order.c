...
css_error css__cascade_order(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_order_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_order(const css_computed_style *from, css_computed_style *to)
{
    int32_t order = 0;
...
css_error
css__compose_order(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    int32_t order = 0;
    uint8_t type = get_order(child, &order);

    return css__copy_order(type == CSS_ORDER_INHERIT ? parent : child, result);
}
