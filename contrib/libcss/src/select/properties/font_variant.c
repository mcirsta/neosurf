...
css_error css__cascade_font_variant(uint32_t opv,
				    css_style *style,
...
css_error
css__copy_font_variant(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_font_variant(
		type == CSS_FONT_VARIANT_INHERIT ? parent : child, result);
}
