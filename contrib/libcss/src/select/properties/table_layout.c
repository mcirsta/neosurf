...
css_error css__cascade_table_layout(uint32_t opv,
				    css_style *style,
...
css_error
css__copy_table_layout(const css_computed_style *from, css_computed_style *to)
{
	if (from == to) {
		return CSS_OK;
	}
				    ...
				    css_computed_style *result)
{
	...
	return css__copy_table_layout(
		type == CSS_TABLE_LAYOUT_INHERIT ? parent : child, result);
}
