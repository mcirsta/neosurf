...
css_error css__cascade_table_layout(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_table_layout_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_table_layout(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_table_layout(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_table_layout(child);

    return css__copy_table_layout(type == CSS_TABLE_LAYOUT_INHERIT ? parent : child, result);
}
