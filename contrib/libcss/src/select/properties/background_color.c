...
css_error css__cascade_background_color(uint32_t opv,
					css_style *style,
					css_select_state *state)
{
	return css__cascade_bg_border_color(
		opv, style, state, set_background_color);
}
...
css_error css__copy_background_color(const css_computed_style *from,
					...
					css_computed_style *result)
{
	...
	return css__copy_background_color(
		type == CSS_BACKGROUND_COLOR_INHERIT ? parent : child, result);
}
