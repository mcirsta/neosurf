...
css_error css__cascade_text_align(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_text_align_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_text_align(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_text_align(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    if (type == CSS_TEXT_ALIGN_INHERIT_IF_NON_MAGIC) {
         ...
         * inherit as normal. */
        if (type == CSS_TEXT_ALIGN_LIBCSS_LEFT || type == CSS_TEXT_ALIGN_LIBCSS_CENTER ||
    ...
    return css__copy_text_align(type == CSS_TEXT_ALIGN_INHERIT ? parent : child, result);
}
