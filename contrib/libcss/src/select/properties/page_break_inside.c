...
css_error css__cascade_page_break_inside(uint32_t opv,
					 css_style *style,
					 css_select_state *state)
{
	return css__cascade_page_break_after_before_inside(
		opv, style, state, set_page_break_inside);
}
...
css_error css__copy_page_break_inside(const css_computed_style *from,
					 ...
					 css_computed_style *result)
{
	...
	return css__copy_page_break_inside(
		type == CSS_PAGE_BREAK_INSIDE_INHERIT ? parent : child, result);
}
