...
css_error css__cascade_flex_direction(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_flex_direction_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_flex_direction(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error css__compose_flex_direction(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_flex_direction(type == CSS_FLEX_DIRECTION_INHERIT ? parent : child, result);
}
