...
css_error css__cascade_grid_template_columns(uint32_t opv, css_style *style, css_select_state *state)
{
    ...
    if (css__outranks_existing(getOpcode(opv), isImportant(opv), state, getFlagValue(opv))) {
        return set_grid_template_columns(state->computed, value, tracks);
    }
...
css_error css__set_grid_template_columns_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_grid_template_columns(const css_computed_style *from, css_computed_style *to)
{
    css_error error;
...
css_error css__compose_grid_template_columns(
