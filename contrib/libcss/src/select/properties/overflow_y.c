...
css_error css__cascade_overflow_y(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_overflow_y_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_overflow_y(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_overflow_y(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_overflow_y(child);

    return css__copy_overflow_y(type == CSS_OVERFLOW_INHERIT ? parent : child, result);
}
