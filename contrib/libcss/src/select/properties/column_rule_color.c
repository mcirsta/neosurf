...
css_error css__cascade_column_rule_color(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_column_rule_color_from_hint(const css_hint *hint, css_computed_style *style)
{
...
{
    return set_column_rule_color(state->computed, CSS_COLUMN_RULE_COLOR_CURRENT_COLOR, 0);
}

css_error css__copy_column_rule_color(const css_computed_style *from, css_computed_style *to)
{
    css_color color;
...
css_error css__compose_column_rule_color(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_column_rule_color(type == CSS_COLUMN_RULE_COLOR_INHERIT ? parent : child, result);
}
