...
css_error css__cascade_list_style_type(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_list_style_type_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_list_style_type(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error css__compose_list_style_type(
    const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    ...
    return css__copy_list_style_type(type == CSS_LIST_STYLE_TYPE_INHERIT ? parent : child, result);
}
