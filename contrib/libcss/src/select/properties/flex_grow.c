...
css_error css__cascade_flex_grow(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_flex_grow_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_flex_grow(const css_computed_style *from, css_computed_style *to)
{
    css_fixed flex_grow = 0;
...
css_error
css__compose_flex_grow(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    css_fixed flex_grow = 0;
    uint8_t type = get_flex_grow(child, &flex_grow);

    return css__copy_flex_grow(type == CSS_FLEX_GROW_INHERIT ? parent : child, result);
}
