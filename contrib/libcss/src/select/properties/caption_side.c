...
css_error css__cascade_caption_side(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_caption_side_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_caption_side(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_caption_side(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_caption_side(child);

    return css__copy_caption_side(type == CSS_CAPTION_SIDE_INHERIT ? parent : child, result);
}
