...
css_error css__cascade_column_span(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_column_span_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_column_span(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_column_span(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_column_span(child);

    return css__copy_column_span(type == CSS_COLUMN_SPAN_INHERIT ? parent : child, result);
}
