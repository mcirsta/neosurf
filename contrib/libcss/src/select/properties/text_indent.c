...
css_error css__cascade_text_indent(uint32_t opv,
				   css_style *style,
...
css_error
css__set_text_indent_from_hint(const css_hint *hint, css_computed_style *style)
{
	return set_text_indent(style,
			       hint->status,
...
{
	return set_text_indent(
		state->computed, CSS_TEXT_INDENT_SET, 0, CSS_UNIT_PX);
}

css_error
css__copy_text_indent(const css_computed_style *from, css_computed_style *to)
{
	css_fixed length = 0;
				   ...
				   css_computed_style *result)
{
	...
	return css__copy_text_indent(
		type == CSS_TEXT_INDENT_INHERIT ? parent : child, result);
}
