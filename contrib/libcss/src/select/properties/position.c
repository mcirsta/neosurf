...
css_error css__cascade_position(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_position_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_position(const css_computed_style *from, css_computed_style *to)
{
    if (from == to) {
        return CSS_OK;
    }
...
css_error
css__compose_position(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_position(child);

    return css__copy_position(type == CSS_POSITION_INHERIT ? parent : child, result);
}
