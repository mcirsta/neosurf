...
css_error css__cascade_flex_wrap(uint32_t opv, css_style *style, css_select_state *state)
{
...
css_error css__set_flex_wrap_from_hint(const css_hint *hint, css_computed_style *style)
{
...
css_error css__copy_flex_wrap(const css_computed_style *from, css_computed_style *to)
{
...
css_error
css__compose_flex_wrap(const css_computed_style *parent, const css_computed_style *child, css_computed_style *result)
{
    uint8_t type = get_flex_wrap(child);

    return css__copy_flex_wrap(type == CSS_FLEX_WRAP_INHERIT ? parent : child, result);
}
