...
static css_error _insert_into_chain(css_selector_hash *ctx, hash_entry *head, const css_selector *selector);
 ...
 */
static inline bool
_chain_good_for_element_name(const css_selector *selector, const css_qname *qname, const lwc_string *uni)
{
    if (selector->data.qname.name != uni) {
        bool match;
        if (lwc_string_caseless_isequal(selector->data.qname.name, qname->name, &match) == lwc_error_ok &&
            match == false) {
            return false;
        }
    }
...
css_error css__selector_hash_create(css_selector_hash **hash)
{
    ...
    h->hash_size = sizeof(css_selector_hash) + DEFAULT_SLOTS * sizeof(hash_entry) + DEFAULT_SLOTS * sizeof(hash_entry) +
...
css_error css__selector_hash_insert(css_selector_hash *hash, const css_selector *selector)
{
    ...
    if ((name = _id_name(selector)) != NULL) {
        ...
        error = _insert_into_chain(hash, &hash->ids.slots[index], selector);
    } else if ((name = _class_name(selector)) != NULL) {
        /* Named class */
        mask = hash->classes.n_slots - 1;
        index = _hash_name(name) & mask;

        error = _insert_into_chain(hash, &hash->classes.slots[index], selector);
        ...
        error = _insert_into_chain(hash, &hash->elements.slots[index], selector);
    } else {
        /* Universal chain */
...
css_error css__selector_hash_remove(css_selector_hash *hash, const css_selector *selector)
{
    ...
    if ((name = _id_name(selector)) != NULL) {
        ...
        error = _remove_from_chain(hash, &hash->ids.slots[index], selector);
    } else if ((name = _class_name(selector)) != NULL) {
        /* Named class */
        mask = hash->classes.n_slots - 1;
        index = _hash_name(name) & mask;

        error = _remove_from_chain(hash, &hash->classes.slots[index], selector);
    } else if (lwc_string_length(selector->data.qname.name) != 1 ||
        lwc_string_data(selector->data.qname.name)[0] != '*') {
        /* Named element */
        mask = hash->elements.n_slots - 1;
        ...
        error = _remove_from_chain(hash, &hash->elements.slots[index], selector);
    } else {
        /* Universal chain */
    ...
    css_selector_hash_iterator *iterator, const css_selector ***matched)
{
    ...
    if (lwc_string_caseless_hash_value(req->qname.name, &name_hash) != lwc_error_ok) {
        return CSS_NOMEM;
    }
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            lerror = lwc_string_isequal(req->qname.name->insensitive, head->sel->data.qname.name->insensitive, &match);
            ...
            if (match && RULE_HAS_BYTECODE(head)) {
                if (css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&
                    mq_rule_good_for_media(head->sel->rule, req->unit_ctx, req->media, req->str)) {
                    /* Found a match */
                    break;
                }
            }
    ...
    css_selector_hash_iterator *iterator, const css_selector ***matched)
{
    ...
    if (hash == NULL || req == NULL || req->class == NULL || iterator == NULL || matched == NULL)
        return CSS_BADPARM;

    /* Find index */
    mask = hash->classes.n_slots - 1;

    if (lwc_string_caseless_hash_value(req->class, &class_hash) != lwc_error_ok) {
        return CSS_NOMEM;
    }
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            if (n != NULL) {
                ...
                if (match && RULE_HAS_BYTECODE(head)) {
                    if (css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&
                        _chain_good_for_element_name(head->sel, &(req->qname), req->str->universal) &&
                        mq_rule_good_for_media(head->sel->rule, req->unit_ctx, req->media, req->str)) {
                        /* Found a match */
                        break;
                    }
    ...
    css_selector_hash_iterator *iterator, const css_selector ***matched)
{
    ...
    if (hash == NULL || req == NULL || req->id == NULL || iterator == NULL || matched == NULL)
        return CSS_BADPARM;
    ...
    if (head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            n = _id_name(head->sel);
            if (n != NULL) {
                lerror = lwc_string_isequal(req->id->insensitive, n->insensitive, &match);
                if (lerror != lwc_error_ok)
                    return css_error_from_lwc_error(lerror);
    ...
    css_selector_hash_iterator *iterator, const css_selector ***matched)
{
    ...
    if (head->sel != NULL) {
        /* Search through chain for first match */
        while (head != NULL) {
            if (RULE_HAS_BYTECODE(head) && css_bloom_in_bloom(head->sel_chain_bloom, req->node_bloom) &&
                mq_rule_good_for_media(head->sel->rule, req->unit_ctx, req->media, req->str)) {
                /* Found a match */
                break;
            }
 ...
 */
static inline void _chain_bloom_add_detail(const css_selector_detail *d, css_bloom bloom[CSS_BLOOM_SIZE])
{
    ...
    case CSS_SELECTOR_ELEMENT:
        ...
        if (lwc_string_length(d->qname.name) == 1 && lwc_string_data(d->qname.name)[0] == '*') {
            return;
        }
        /* Fall through */
 ...
 */
static void _chain_bloom_generate(const css_selector *s, css_bloom bloom[CSS_BLOOM_SIZE])
{
    ...
    do {
        ...
        if (s->data.comb == CSS_COMBINATOR_ANCESTOR || s->data.comb == CSS_COMBINATOR_PARENT) {
 ...
 */
css_error _insert_into_chain(css_selector_hash *ctx, hash_entry *head, const css_selector *selector)
{
    if (head->sel == NULL) {
        head->sel = selector;
        head->next = NULL;
        ...
        do {
            ...
            if (search->sel->specificity == selector->specificity && search->sel->rule->index > selector->rule->index)
                break;
 ...
 */
css_error _remove_from_chain(css_selector_hash *ctx, hash_entry *head, const css_selector *selector)
{
    ...
    const struct css_hash_selection_requirments *req, const css_selector **current, const css_selector ***next)
{
    ...
    if (head != NULL && head->sel != NULL) {
        /* Search through chain for first match */
        while (head != NULL) {
            lerror = lwc_string_caseless_isequal(name, head->sel->data.qname.name, &match);
            if (lerror != lwc_error_ok)
                return css_error_from_lwc_error(lerror);
    ...
    const struct css_hash_selection_requirments *req, const css_selector **current, const css_selector ***next)
{
    ...
    if (head != NULL && head->sel != NULL) {
        ...
        while (head != NULL) {
            ...
            if (name != NULL) {
                lerror = lwc_string_caseless_isequal(ref, name, &match);
                if (lerror != lwc_error_ok)
                    return css_error_from_lwc_error(lerror);
