    ...
    css_mq_feature_op op, const css_mq_value *value, const css_fixed client_len, const css_unit_ctx *unit_ctx)
{
    ...
    if (value->data.dim.unit != UNIT_PX) {
        v = css_unit_len2px_mq(unit_ctx, value->data.dim.len, css__to_css_unit(value->data.dim.unit));
    } else {
...
static inline bool
mq_match_feature_eq_ident_op1(css_mq_feature_op op, const css_mq_value *value, const lwc_string *client_value)
{
    bool is_match;
    ...
    switch (op) {
    case CSS_MQ_FEATURE_OP_EQ:
        return (lwc_string_isequal(value->data.ident, client_value, &is_match) == lwc_error_ok) && is_match;
    default:
        return false;
    }
}
...
static inline bool mq_match_feature(
    const css_mq_feature *feat, const css_unit_ctx *unit_ctx, const css_media *media, const css_select_strings *str)
{
    bool match;

    /* TODO: Use interned string for comparison. */
    if (lwc_string_isequal(feat->name, str->width, &match) == lwc_error_ok && match == true) {
        if (!mq_match_feature_range_length_op1(feat->op, &feat->value, media->width, unit_ctx)) {
            return false;
        }
        return mq_match_feature_range_length_op2(feat->op2, &feat->value2, media->width, unit_ctx);

    } else if (lwc_string_isequal(feat->name, str->height, &match) == lwc_error_ok && match == true) {
        if (!mq_match_feature_range_length_op1(feat->op, &feat->value, media->height, unit_ctx)) {
            return false;
        }

        return mq_match_feature_range_length_op2(feat->op2, &feat->value2, media->height, unit_ctx);

    } else if (lwc_string_isequal(feat->name, str->prefers_color_scheme, &match) == lwc_error_ok && match == true) {
        if (mq_match_feature_eq_ident_op1(feat->op, &feat->value, media->prefers_color_scheme) ||
...
static inline bool mq_match_condition(
    const css_mq_cond *cond, const css_unit_ctx *unit_ctx, const css_media *media, const css_select_strings *str)
{
    ...
    for (uint32_t i = 0; i < cond->nparts; i++) {
        ...
        if (cond->parts[i]->type == CSS_MQ_FEATURE) {
            part_matched = mq_match_feature(cond->parts[i]->data.feat, unit_ctx, media, str);
        } else {
            assert(cond->parts[i]->type == CSS_MQ_COND);
            part_matched = mq_match_condition(cond->parts[i]->data.cond, unit_ctx, media, str);
        }
...
static inline bool mq__list_match(
    const css_mq_query *m, const css_unit_ctx *unit_ctx, const css_media *media, const css_select_strings *str)
{
    for (; m != NULL; m = m->next) {
        /* Check type */
        if (!!(m->type & media->type) != m->negate_type) {
            if (m->cond == NULL || mq_match_condition(m->cond, unit_ctx, media, str)) {
...
static inline bool mq_rule_good_for_media(
    const css_rule *rule, const css_unit_ctx *unit_ctx, const css_media *media, const css_select_strings *str)
{
    bool applies = true;
    ...
    while (ancestor != NULL) {
        ...
        if (ancestor->type == CSS_RULE_MEDIA) {
            applies = mq__list_match(m->media, unit_ctx, media, str);
            if (applies == false) {
                break;
            }
        }
