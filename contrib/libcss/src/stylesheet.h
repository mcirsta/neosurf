...
struct css_stylesheet {
    ...
    css_font_face_notification_fn font_face; /**< Font-face notification function */
...
css__stylesheet_style_appendOPV(css_style *style, opcode_t opcode, uint8_t flags, uint16_t value)
{
...
static inline css_error css_stylesheet_style_initial(css_style *style, opcode_t opcode)
{
    return css__stylesheet_style_append(style, buildOPV(opcode, FLAG_INITIAL, 0));
}

/** Helper function to set inherit generic value */
static inline css_error css_stylesheet_style_revert(css_style *style, opcode_t opcode)
{
    style->sheet->uses_revert = true;
    return css__stylesheet_style_append(style, buildOPV(opcode, FLAG_REVERT, 0));
}

/** Helper function to set initial generic value */
static inline css_error css_stylesheet_style_unset(css_style *style, opcode_t opcode)
{
    return css__stylesheet_style_append(style, buildOPV(opcode, FLAG_UNSET, 0));
}

/** Helper function to set initial generic value */
static inline css_error css_stylesheet_style_flag_value(css_style *style, enum flag_value flag_value, opcode_t opcode)
{
    enum flag flag = flag_value << 1;
    if (flag == FLAG_REVERT) {
        style->sheet->uses_revert = true;
    }
    return css__stylesheet_style_append(style, buildOPV(opcode, flag, 0));
}
...
css_error css__stylesheet_selector_append_specific(
    css_stylesheet *sheet, css_selector **parent, const css_selector_detail *specific);

css_error
css__stylesheet_selector_combine(css_stylesheet *sheet, css_combinator type, css_selector *a, css_selector *b);
