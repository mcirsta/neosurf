...
css_error css__stylesheet_string_add(css_stylesheet *sheet, lwc_string *string, uint32_t *string_number)
{
    ...
    for (new_string_number = 0; new_string_number < sheet->string_vector_c; new_string_number++) {
        lwc_error res;
        bool isequal;
        res = lwc_string_isequal(string, sheet->string_vector[new_string_number], &isequal);
    ...
    if (sheet->string_vector_c >= sheet->string_vector_l) {
        ...
        new_vector = realloc(sheet->string_vector, new_vector_len * sizeof(lwc_string *));
...
css_error css_stylesheet_create(const css_stylesheet_params *params, css_stylesheet **stylesheet)
{
    ...
    if (params == NULL ||
        ...
        params->url == NULL || params->resolve == NULL || stylesheet == NULL)
        return CSS_BADPARM;
    ...
    if (params->inline_style) {
        error = css__parser_create_for_inline_style(
            params->charset, (params->charset != NULL) ? CSS_CHARSET_DICTATED : CSS_CHARSET_DEFAULT, &sheet->parser);
    } else {
        error = css__parser_create(
            params->charset, (params->charset != NULL) ? CSS_CHARSET_DICTATED : CSS_CHARSET_DEFAULT, &sheet->parser);
    }
    ...
    error = css__language_create(sheet, sheet->parser, &sheet->parser_frontend);
    if (error != CSS_OK) {
        css__parser_destroy(sheet->parser);
        css__propstrings_unref();
        free(sheet);
...
css_error css_stylesheet_append_data(css_stylesheet *sheet, const uint8_t *data, size_t len)
{
    ...
    fprintf(stderr, "DEBUG: css_stylesheet_append_data called with %zu bytes\n", len);
...
css_error css_stylesheet_data_done(css_stylesheet *sheet)
{
    ...
    for (r = sheet->rule_list; r != NULL; r = r->next) {
        ...
        if (r->type != CSS_RULE_UNKNOWN && r->type != CSS_RULE_CHARSET && r->type != CSS_RULE_IMPORT)
            break;
...
css_error css__stylesheet_merge_style(css_style *target, css_style *style)
{
    ...
    if (newcode_len > target->allocated) {
        ...
        newcode_len &= ~(CSS_STYLE_DEFAULT_SIZE - 1);
        newcode = realloc(target->bytecode, newcode_len * sizeof(css_code_t));
...
css_error css__stylesheet_style_append(css_style *style, css_code_t css_code)
{
    ...
    if (style->allocated == style->used) {
        ...
        newcode = realloc(style->bytecode, sizeof(css_code_t) * newcode_len);
        if (newcode == NULL)
            return CSS_NOMEM;
...
css_error css__stylesheet_selector_create(css_stylesheet *sheet, css_qname *qname, css_selector **selector)
{
    ...
    if (sheet == NULL || qname == NULL || qname->name == NULL || selector == NULL)
        return CSS_BADPARM;

    if (sheet->error) {
        char buf[256];
        snprintf(buf, sizeof(buf), "Creating selector: %.*s", (int)lwc_string_length(qname->name),
    ...
    for (c = selector->combinator; c != NULL; c = d) {
        ...
        for (detail = &c->data; detail;) {
            ...
            if (detail->qname.name == NULL) {
                if (sheet->error != NULL) {
                    sheet->error(sheet->error_pw, sheet, CSS_INVALID,
 ...
 */
css_error css__stylesheet_selector_detail_init(css_stylesheet *sheet, css_selector_type type, css_qname *qname,
    css_selector_detail_value value, css_selector_detail_value_type value_type, bool negate,
    css_selector_detail *detail)
{
    if (sheet == NULL || qname == NULL || qname->name == NULL || detail == NULL)
        return CSS_BADPARM;
 ...
 * \return CSS_OK on success, appropriate error otherwise.
 */
css_error css__stylesheet_selector_append_specific(
    css_stylesheet *sheet, css_selector **parent, const css_selector_detail *detail)
{
    css_selector *temp;
    css_selector_detail *d;
    size_t num_details = 0;

    if (sheet == NULL || parent == NULL || *parent == NULL || detail == NULL)
        return CSS_BADPARM;
    ...
    temp = realloc((*parent), sizeof(css_selector) + (num_details + 1) * sizeof(css_selector_detail));
    if (temp == NULL)
        return CSS_NOMEM;
...
css_error css__stylesheet_rule_destroy(css_stylesheet *sheet, css_rule *rule)
{
    ...
    assert(rule->parent == NULL && rule->next == NULL && rule->prev == NULL);
...
css_error css__stylesheet_rule_add_selector(css_stylesheet *sheet, css_rule *rule, css_selector *selector)
{
    ...
    sels = realloc(r->selectors, (r->base.items + 1) * sizeof(css_selector *));
    if (sels == NULL)
        return CSS_NOMEM;
 ...
 */
css_error
css__stylesheet_rule_set_nascent_import(css_stylesheet *sheet, css_rule *rule, lwc_string *url, css_mq_query *media)
{
        ...
        for (i = 0; i < rule->items; i++) {
            ...
            if (error != CSS_OK) {
                ...
                for (i--; i >= 0; i--) {
                    ...
                    /* Ignore errors */
                    css__selector_hash_remove(sheet->selectors, sel);
                }
