...
static bmp_result bmp_info_header_parse(bmp_image *bmp, uint8_t *data)
{
	...
	if (bmp->bpp < 16) {
		...
		/* boundary checking */
		if (bmp->buffer_size <
		    (14 + header_size + ((uint64_t)4 * bmp->colours)))
			return BMP_INSUFFICIENT_DATA;
		...
		for (i = 0; i < bmp->colours; i++) {
			uint32_t colour = data[2] | (data[1] << 8) |
					  (data[0] << 16);
			if (bmp->opaque)
				colour |= ((uint32_t)0xff << 24);
			data += palette_size;
			bmp->colour_table[i] = read_uint32((uint8_t *)&colour,
							   0);
		}
	...
	bmp->bitmap = bmp->bitmap_callbacks.bitmap_create(bmp->width,
							  bmp->height,
							  flags);
	if (!bmp->bitmap) {
		if (bmp->colour_table)
...
static bmp_result ico_header_parse(ico_collection *ico, uint8_t *data)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (image->bmp.bmp_data - ico->ico_data >= 0 &&
		    (uint32_t)(image->bmp.bmp_data - ico->ico_data) >=
			    ico->buffer_size)
			return BMP_DATA_ERROR;
	...
	for (y = 0; y < bmp->height; y++) {
		...
		if (bmp->encoding == BMP_ENCODING_BITFIELDS) {
			for (x = 0; x < bmp->width; x++) {
				...
				data += 4;
				scanline[x] = read_uint32(
					(uint8_t *)&scanline[x], 0);
			}
		} else {
			for (x = 0; x < bmp->width; x++) {
