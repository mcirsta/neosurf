...
{
    return (int32_t)((unsigned)data[o] | ((unsigned)data[o + 1] << 8) | ((unsigned)data[o + 2] << 16) |
        ((unsigned)data[o + 3] << 24));
}

static inline uint32_t read_uint32(uint8_t *data, unsigned int o)
{
    return (uint32_t)((unsigned)data[o] | ((unsigned)data[o + 1] << 8) | ((unsigned)data[o + 2] << 16) |
        ((unsigned)data[o + 3] << 24));
}
...
static bmp_result bmp_info_header_parse(bmp_image *bmp, uint8_t *data)
{
    ...
    if (header_size == BITMAPCOREHEADER) {
        /* the following header is for os/2 and windows 2.x and consists
         * of:
         *
         ...
         * added height of XOR-Bitmap and AND-Bitmap (double the visible
         * height) Technically we could remove this check and ICOs with
         * bitmaps of any size could be processed; this is to conform to
         * the spec.
         */
        if (bmp->ico) {
         ...
         */
        if ((bmp->bpp != 1) && (bmp->bpp != 4) && (bmp->bpp != 8) && (bmp->bpp != 16) && (bmp->bpp != 24) &&
            (bmp->bpp != 32))
            return BMP_DATA_ERROR;
        if (bmp->bpp < 16)
         ...
         * minimum of 40 bytes and (as of Windows 95) a maximum of 108
         * bytes.
         *
         *	+0	UINT32	size of this header (in bytes)
         ...
         *	+24	UINT32	horizontal resolution (in pixels per
         * meter) +28	UINT32	vertical resolution (in pixels per
         * meter) +32	UINT32	number of colours in the image +36
         *	UINT32	number of important colours +40	UINT32	mask
         * identifying bits of red component +44	UINT32	mask
         * identifying bits of green component +48	UINT32	mask
         * identifying bits of blue component +52	UINT32	mask
         * identifying bits of alpha component +56	UINT32	color
         * space type +60	UINT32	x coordinate of red endpoint +64
         *	UINT32	y coordinate of red endpoint +68	UINT32
         *	z coordinate of red endpoint +72	UINT32	x
         * coordinate of green endpoint +76	UINT32	y coordinate of
         * green endpoint +80	UINT32	z coordinate of green endpoint
         ...
         * masks. Here we acquire the masks and determine the required
         * bit shift to align them in our 24-bit color 8-bit alpha
         * format.
         */
    ...
    if (bmp->bpp < 16) {
         ...
         *	+2	BYTE	red
         *
         * if the palette is from an OS/2 or Win2.x file then the
         * entries are padded with an extra byte.
         */
    ...
    /* BMPs within ICOs don't have BMP file headers, so the image data
     * should always be right after the colour table.
     */
    if (bmp->ico)
...
static bmp_result ico_header_parse(ico_collection *ico, uint8_t *data)
{
     ...
     *	+12	UINT32	Offset (points to the BMP info header, not the
     * bitmap data)
     */
    for (i = 0; i < count; i++) {
        ico_image *image;
        ...
        if (image->bmp.bmp_data + image->bmp.buffer_size > ico->ico_data + ico->buffer_size)
            return BMP_INSUFFICIENT_DATA;
        image->bmp.ico = true;
        ...
        if (image->bmp.buffer_size - ICO_DIR_ENTRY_SIZE >= ico->buffer_size - (ico->ico_data - data))
            return BMP_INSUFFICIENT_DATA;
        ...
        if (bmp->encoding == BMP_ENCODING_BITFIELDS) {
            ...
            for (x = 0; x < bmp->width; x++) {
                ...
                if ((bmp->limited_trans) && (word == bmp->transparent_index))
                    ...
                    scanline[x] = ((word & (31 << 0)) << 19) | ((word & (31 << 5)) << 6) | ((word & (31 << 10)) >> 7);
                }
                if (bmp->opaque)
                    scanline[x] |= ((unsigned)0xff << 24);
                data += 2;
...
bmp_result bmp_create(bmp_image *bmp, bmp_bitmap_callback_vt *bitmap_callbacks)
{
...
/* exported interface documented in libnsbmp.h */
bmp_result ico_collection_create(ico_collection *ico, bmp_bitmap_callback_vt *bitmap_callbacks)
{
