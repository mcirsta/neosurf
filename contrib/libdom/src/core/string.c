 ...
 */
static const dom_string_internal empty_string = {{0}, {{(uint8_t *)"", 0}}, DOM_STRING_CDATA};
...
dom_exception dom_string_create_interned(const uint8_t *ptr, size_t len, dom_string **str)
{
    ...
    if (lwc_intern_string((const char *)ptr, len, &ret->data.intern) != lwc_error_ok) {
        free(ret);
        return DOM_NO_MEM_ERR;
    }
...
dom_exception dom_string_intern(dom_string *str, struct lwc_string_s **lwcstr)
{
    ...
    if (istr->type != DOM_STRING_INTERNED) {
        ...
        lerr = lwc_intern_string((const char *)istr->data.cdata.ptr, istr->data.cdata.len, &ret);
...
bool dom_string_isequal(const dom_string *s1, const dom_string *s2)
{
    ...
    if (is1->type == DOM_STRING_INTERNED && is2->type == DOM_STRING_INTERNED) {
        bool match;

        (void)lwc_string_isequal(is1->data.intern, is2->data.intern, &match);
...
bool dom_string_caseless_isequal(const dom_string *s1, const dom_string *s2)
{
    ...
    if (is1->type == DOM_STRING_INTERNED && is2->type == DOM_STRING_INTERNED) {
        ...
        if (lwc_string_caseless_isequal(is1->data.intern, is2->data.intern, &match) != lwc_error_ok)
            return false;
...
uint32_t dom_string_rindex(dom_string *str, uint32_t chr)
{
    ...
    while (slen > 0) {
        ...
        if (err == PARSERUTILS_OK) {
            err = parserutils_charset_utf8_to_ucs4(s + coff, slen - clen, &c, &clen);
        }
