...
dom_exception dom_implementation_create_document_type(
    const char *qname, const char *public_id, const char *system_id, struct dom_document_type **doctype)
{
    ...
    err = dom_string_create((const uint8_t *)qname, strlen(qname), &qname_s);
    if (err != DOM_NO_ERR)
        return err;
    ...
    if (public_id != NULL) {
        err = dom_string_create((const uint8_t *)public_id, strlen(public_id), &public_id_s);
        if (err != DOM_NO_ERR) {
    ...
    if (system_id != NULL) {
        err = dom_string_create((const uint8_t *)system_id, strlen(system_id), &system_id_s);
        if (err != DOM_NO_ERR) {
 ...
 * finished with it.
 */
dom_exception dom_implementation_create_document(uint32_t impl_type, const char *namespace, const char *qname,
    struct dom_document_type *doctype, dom_events_default_action_fetcher daf, void *daf_ctx, struct dom_document **doc)
{
    ...
    if (namespace != NULL) {
        err = dom_string_create((const uint8_t *)namespace, strlen(namespace), &namespace_s);
        if (err != DOM_NO_ERR)
            return err;
    }
    ...
    if (doctype != NULL) {
        ...
        err = dom_node_append_child((struct dom_node *)d, (struct dom_node *)doctype, &ins_doctype);
        if (err != DOM_NO_ERR) {
    ...
    if (qname_s != NULL) {
        ...
        err = dom_node_append_child((struct dom_node *)d, (struct dom_node *)e, &inserted);
        if (err != DOM_NO_ERR) {
