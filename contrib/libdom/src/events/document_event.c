...
static const char *__event_types[] = {"Event", "CustomEvent", "UIEvent", "TextEvent", "KeyboardEvent", "MouseEvent",
    "MouseMultiWheelEvent", "MouseWheelEvent", "MutationEvent", "MutationNameEvent"};
    ...
    dom_document_event_internal *dei, dom_events_default_action_fetcher actions, void *actions_ctx)
{
    ...
    for (i = 0; i < DOM_EVENT_COUNT; i++) {
        err = lwc_intern_string(__event_types[i], strlen(__event_types[i]), &dei->event_types[i]);
        if (err != lwc_error_ok)
...
dom_exception _dom_document_event_create_event(dom_document_event *de, dom_string *type, struct dom_event **evt)
{
    ...
    case DOM_KEYBOARD_EVENT:
        err = _dom_keyboard_event_create((dom_keyboard_event **)evt);
        break;
    case DOM_MOUSE_EVENT:
        err = _dom_mouse_event_create((dom_mouse_event **)evt);
        break;
    case DOM_MOUSE_MULTI_WHEEL_EVENT:
        err = _dom_mouse_multi_wheel_event_create((dom_mouse_multi_wheel_event **)evt);
        break;
    case DOM_MOUSE_WHEEL_EVENT:
        err = _dom_mouse_wheel_event_create((dom_mouse_wheel_event **)evt);
        break;
    case DOM_MUTATION_EVENT:
        err = _dom_mutation_event_create((dom_mutation_event **)evt);
        break;
    case DOM_MUTATION_NAME_EVENT:
        err = _dom_mutation_name_event_create((dom_mutation_name_event **)evt);
        break;
    }
