...
static const struct dom_element_protected_vtable _dom_html_element_protect_vtable = {
    {DOM_HTML_ELEMENT_PROTECT_VTABLE}, DOM_ELEMENT_PROTECT_VTABLE};
...
dom_exception _dom_html_element_initialise(struct dom_html_element_create_params *params, struct dom_html_element *el)
{
    ...
    err = _dom_element_initialise(&params->doc->base, &el->base, params->name, params->namespace, params->prefix);
    if (err != DOM_NO_ERR)
        return err;
...
#define SIMPLE_GET_SET(fattr, attr)                                                                                    \
    dom_exception _dom_html_element_get_##fattr(dom_html_element *element, dom_string **fattr)                         \
    {                                                                                                                  \
        dom_exception ret;                                                                                             \
        ...
        return ret;                                                                                                    \
                                                                                                                       ...
                                                                                                                       \
    dom_exception _dom_html_element_set_##fattr(dom_html_element *element, dom_string *fattr)                          \
    {                                                                                                                  \
        dom_exception ret;                                                                                             \
        dom_string *_memo_##attr;                                                                                      \
                                                                                                                       \
...
_dom_html_element_get_elements_by_tag_name(struct dom_element *element, dom_string *name, struct dom_nodelist **result)
{
    ...
    err = _dom_document_get_nodelist(
        base->owner, DOM_NODELIST_BY_NAME_CASELESS, (struct dom_node_internal *)element, name, NULL, NULL, result);
    ...
    struct dom_element *element, dom_string *namespace, dom_string *localname, struct dom_nodelist **result)
{
    ...
    err = _dom_document_get_nodelist(element->base.owner, DOM_NODELIST_BY_NAMESPACE_CASELESS,
...
dom_html_element_get_int32_t_property(dom_html_element *ele, const char *name, uint32_t len, int32_t *value)
{
    ...
    if (a != NULL) {
        ...
        if (err == DOM_NO_ERR) {
            char *s3 = _strndup(dom_string_data(s2), dom_string_byte_length(s2));
            if (s3 != NULL) {
...
dom_html_element_set_int32_t_property(dom_html_element *ele, const char *name, uint32_t len, uint32_t value)
{
    ...
    err = dom_string_create((const uint8_t *)numbuffer, strlen(numbuffer), &svalue);
    if (err != DOM_NO_ERR)
        goto cleanup;
