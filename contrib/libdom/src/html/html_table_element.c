 ...
 */
dom_exception
_dom_html_table_element_create(struct dom_html_element_create_params *params,
...
dom_exception
_dom_html_table_element_copy(dom_node_internal *old, dom_node_internal **copy)
{
...
#define SIMPLE_GET(attr)                                                       \
	dom_exception dom_html_table_element_get_##attr(                       \
		dom_html_table_element *element, dom_string **attr)            \
	{                                                                      \
		dom_exception ret;                                             \
 ...
 */
dom_exception
dom_html_table_element_get_caption(dom_html_table_element *table,
				   dom_html_table_caption_element **caption)
{
	...
	for (node_tmp = node_tmp->first_child; node_tmp != NULL;
 ...
 */
dom_exception
dom_html_table_element_set_caption(dom_html_table_element *table,
				   dom_html_table_caption_element *caption)
{
	dom_node_internal *check_node = ((dom_node_internal *)caption);
	dom_html_document *doc =
		(dom_html_document *)(((dom_node_internal *)table)->owner);
	dom_exception exp;
	...
	return dom_node_append_child(table, caption, &new_caption);
}
 ...
 */
dom_exception
dom_html_table_element_get_t_head(dom_html_table_element *table,
 ...
 */
dom_exception
dom_html_table_element_set_t_head(dom_html_table_element *table,
				  dom_html_table_section_element *t_head)
{
	...
	return dom_node_append_child(table, t_head, &new_t_head);
}
 ...
 */
dom_exception
dom_html_table_element_get_t_foot(dom_html_table_element *table,
 ...
 */
dom_exception
dom_html_table_element_set_t_foot(dom_html_table_element *table,
				  dom_html_table_section_element *t_foot)
{
	dom_node_internal *check_node = ((
		dom_node_internal *)t_foot); /*< temporary node to check for
						raised exceptions */
	dom_html_document *doc =
		(dom_html_document *)(((dom_node_internal *)table)->owner);
	dom_exception exp;
	...
	return dom_node_append_child(table, t_foot, &new_t_foot);
}
...
static bool table_t_bodies_callback(struct dom_node_internal *node, void *ctx)
{
	...
	if (node->type == DOM_ELEMENT_NODE &&
	    dom_string_caseless_isequal(
		    node->name, doc->elements[DOM_HTML_ELEMENT_TYPE_TBODY])) {
		return true;
	}
	return false;
}
...
dom_exception
dom_html_table_element_get_t_bodies(dom_html_table_element *element,
 ...
 */
dom_exception
dom_html_table_element_create_caption(dom_html_table_element *element,
				      dom_html_element **caption)
{
	dom_exception exp;
	if ((exp = dom_html_table_element_get_caption(
		     element, (dom_html_table_caption_element **)caption)) !=
	    DOM_NO_ERR) {
	...
	if ((*caption) == NULL) {
		dom_node *new_caption;
		dom_html_document *doc =
			(dom_html_document *)((dom_node_internal *)element)
				->owner;
		...
		exp = _dom_html_table_caption_element_create(
			&params, (dom_html_table_caption_element **)caption);
		if (exp != DOM_NO_ERR) {
			dom_node_unref(*caption);
			return exp;
		}

		exp = dom_node_append_child(element, *caption, &new_caption);
		dom_node_unref(*caption);
		if (exp == DOM_NO_ERR)
 ...
 */
dom_exception
dom_html_table_element_delete_caption(dom_html_table_element *element)
{
	dom_html_table_caption_element *caption;
 ...
 */
dom_exception
dom_html_table_element_create_t_foot(dom_html_table_element *element,
				     dom_html_element **t_foot)
{
	dom_exception exp;
	exp = dom_html_table_element_get_t_foot(
		element, (dom_html_table_section_element **)t_foot);
	if (exp != DOM_NO_ERR)
		return exp;
	...
	if ((*t_foot) == NULL) {
		...
		exp = _dom_html_table_section_element_create(
			&params, (dom_html_table_section_element **)t_foot);
		if (exp != DOM_NO_ERR) {
			dom_node_unref(*t_foot);
			return exp;
		}

		exp = dom_node_append_child(element, *t_foot, &new_t_foot);
		dom_node_unref(*t_foot);
		if (exp == DOM_NO_ERR)
 ...
 */
dom_exception
dom_html_table_element_delete_t_foot(dom_html_table_element *element)
{
	dom_html_table_section_element *t_foot;
	dom_node *old_t_foot;
	dom_exception err;
 ...
 */
dom_exception
dom_html_table_element_create_t_head(dom_html_table_element *element,
				     dom_html_element **t_head)
{
	dom_exception exp;
	exp = dom_html_table_element_get_t_head(
		element, (dom_html_table_section_element **)t_head);
	if (exp != DOM_NO_ERR) {
	...
	if ((*t_head) == NULL) {
		...
		exp = _dom_html_table_section_element_create(
			&params, (dom_html_table_section_element **)t_head);
		if (exp != DOM_NO_ERR) {
			dom_node_unref(*t_head);
			return exp;
		}

		exp = dom_node_append_child(element, *t_head, &new_t_head);
		if (exp == DOM_NO_ERR) {
			dom_node_unref(*t_head);
 ...
 */
dom_exception
dom_html_table_element_delete_t_head(dom_html_table_element *element)
{
	dom_html_table_section_element *t_head;
	dom_node *old_t_head;
	dom_exception err;
...
dom_html_table_element_create_t_body(dom_html_table_element *element,
				     dom_html_table_section_element **t_body)
{
	dom_html_collection *t_bodies;
	uint32_t len;
	dom_exception exp;
	exp = dom_html_table_element_get_t_bodies(element, &t_bodies);
	if (exp != DOM_NO_ERR) {
		return exp;
	}
	exp = dom_html_collection_get_length(t_bodies, &len);
	if (exp != DOM_NO_ERR) {
	...
	if (len == 0) {
		...
		exp = dom_node_append_child(element, *t_body, &new_t_body);
		if (exp == DOM_NO_ERR) {
			dom_node_unref(*t_body);
						...
						dom_html_element **row_out)
{
	...
	if (index > (int32_t)len || index < -1) {
		...
		exp = dom_node_append_child(new_body,
						...
						int32_t index)
{
	...
	if (index >= (int32_t)len || index < -1 || len == 0) {
		...
		if (window_len + section_len > (uint32_t)index) {
			exp = dom_html_table_section_element_delete_row(
				t_head, index - window_len);
			dom_node_unref(t_head);
			return exp;
		}
		dom_node_unref(t_head);
