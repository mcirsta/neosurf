...
static hubbub_error create_comment(void *parser, const hubbub_string *data, void **result)
{
    ...
    *result = NULL;
    ...
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create comment node text");
    ...
    if (err != DOM_NO_ERR) {
        ...
        dom_parser->msg(
            DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create comment node with text '%.*s'", data->len, data->ptr);
        return HUBBUB_UNKNOWN;
    }
...
static hubbub_error create_doctype(void *parser, const hubbub_doctype *doctype, void **result)
{
    dom_hubbub_parser *dom_parser = (dom_hubbub_parser *)parser;
    dom_exception err;
    ...
    *result = NULL;
    ...
    qname = parser_strndup((const char *)doctype->name.ptr, (size_t)doctype->name.len);
    if (qname == NULL) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create doctype name");
        goto fail;
    }

    if (doctype->public_missing == false) {
        public_id = parser_strndup((const char *)doctype->public_id.ptr, (size_t)doctype->public_id.len);
    } else {
        public_id = strdup("");
    }
    if (public_id == NULL) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create doctype public id");
        goto clean1;
    }

    if (doctype->system_missing == false) {
        system_id = parser_strndup((const char *)doctype->system_id.ptr, (size_t)doctype->system_id.len);
    } else {
        system_id = strdup("");
    }
    if (system_id == NULL) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create doctype system id");
        goto clean2;
    }

    err = dom_implementation_create_document_type(qname, public_id, system_id, &dtype);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create the document type");
        goto clean3;
    }
...
static hubbub_error create_element(void *parser, const hubbub_tag *tag, void **result)
{
    ...
    *result = NULL;
    ...
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create element name");
        goto fail;
    }

    if (tag->ns == HUBBUB_NS_NULL) {
        err = dom_document_create_element(dom_parser->doc, name, &element);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create the DOM element");
            goto clean1;
        }
    } else {
        err = dom_document_create_element_ns(dom_parser->doc, dom_namespaces[tag->ns], name, &element);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create the DOM element");
            goto clean1;
        }
    ...
    if (tag->n_attributes > 0) {
        herr = add_attributes(parser, element, tag->attributes, tag->n_attributes);
        if (herr != HUBBUB_OK)
            goto clean1;
    }
    ...
    case DOM_HTML_ELEMENT_TYPE_SCRIPT: {
        ...
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't get flags out of script element");
            goto clean1;
        }
        flags |= DOM_HTML_SCRIPT_ELEMENT_FLAG_PARSER_INSERTED;
        err = dom_html_script_element_set_flags(script, flags);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't set flags into script element");
            goto clean1;
        }
        break;
    }
...
static hubbub_error create_text(void *parser, const hubbub_string *data, void **result)
{
    ...
    *result = NULL;
    ...
    err = dom_string_create(data->ptr, data->len, &str);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create text '%.*s'", data->len, data->ptr);
        goto fail;
    }

    err = dom_document_create_text_node(dom_parser->doc, str, &text);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create the DOM text node");
        goto clean1;
    }
...
static hubbub_error insert_before(void *parser, void *parent, void *child, void *ref_child, void **result)
{
    dom_hubbub_parser *dom_parser = (dom_hubbub_parser *)parser;
    dom_exception err;

    err = dom_node_insert_before(
        (struct dom_node *)parent, (struct dom_node *)child, (struct dom_node *)ref_child, (struct dom_node **)result);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(
            DOM_MSG_CRITICAL, dom_parser->mctx, "Can't insert node '%p' before node '%p'", child, ref_child);
        return HUBBUB_UNKNOWN;
    }
...
static hubbub_error remove_child(void *parser, void *parent, void *child, void **result)
{
    ...
    err = dom_node_remove_child((struct dom_node *)parent, (struct dom_node *)child, (struct dom_node **)result);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't remove child '%p'", child);
        return HUBBUB_UNKNOWN;
    }
...
static hubbub_error clone_node(void *parser, void *node, bool deep, void **result)
{
    ...
    err = dom_node_clone_node((struct dom_node *)node, deep, (struct dom_node **)result);
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't clone node '%p'", node);
        return HUBBUB_UNKNOWN;
    }
...
static hubbub_error reparent_children(void *parser, void *node, void *new_parent)
{
    ...
    while (true) {
        ...
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_note_get_first_child");
            return HUBBUB_UNKNOWN;
        }
        if (child == NULL)
            break;

        err = dom_node_remove_child(node, (struct dom_node *)child, &result);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_node_remove_child");
            goto fail;
        }
        dom_node_unref(result);

        err = dom_node_append_child((struct dom_node *)new_parent, (struct dom_node *)child, &result);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_node_append_child");
            goto fail;
        }
        dom_node_unref(result);
...
static hubbub_error get_parent(void *parser, void *node, bool element_only, void **result)
{
    dom_hubbub_parser *dom_parser = (dom_hubbub_parser *)parser;
    dom_exception err;
    ...
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_node_get_parent");
        return HUBBUB_UNKNOWN;
    }
    ...
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_node_get_type");
        goto fail;
    }
...
static hubbub_error has_children(void *parser, void *node, bool *result)
{
    ...
    if (err != DOM_NO_ERR) {
        dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in dom_node_has_child_nodes");
        return HUBBUB_UNKNOWN;
    }
    return HUBBUB_OK;
}
...
static hubbub_error form_associate(void *parser, void *form, void *node)
{
    ...
    if (dom_string_caseless_isequal(ele->name, doc->elements[DOM_HTML_ELEMENT_TYPE_BUTTON])) {
        ...
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in form_associate");
            return HUBBUB_UNKNOWN;
        }
    } else if (dom_string_caseless_isequal(ele->name, doc->elements[DOM_HTML_ELEMENT_TYPE_INPUT])) {
        err = _dom_html_input_element_set_form((dom_html_input_element *)node, form_ele);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in form_associate");
            return HUBBUB_UNKNOWN;
        }
    } else if (dom_string_caseless_isequal(ele->name, doc->elements[DOM_HTML_ELEMENT_TYPE_SELECT])) {
        err = _dom_html_select_element_set_form((dom_html_select_element *)node, form_ele);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in form_associate");
            return HUBBUB_UNKNOWN;
        }
    } else if (dom_string_caseless_isequal(ele->name, doc->elements[DOM_HTML_ELEMENT_TYPE_TEXTAREA])) {
        err = _dom_html_text_area_element_set_form((dom_html_text_area_element *)node, form_ele);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Error in form_associate");
            return HUBBUB_UNKNOWN;
        }
...
static hubbub_error add_attributes(void *parser, void *node, const hubbub_attribute *attributes, uint32_t n_attributes)
{
    ...
    for (i = 0; i < n_attributes; i++) {
        ...
        err = dom_string_create_interned(attributes[i].name.ptr, attributes[i].name.len, &name);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create attribute name");
            goto fail;
        }

        err = dom_string_create(attributes[i].value.ptr, attributes[i].value.len, &value);
        if (err != DOM_NO_ERR) {
            dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't create attribute value");
            dom_string_unref(name);
            goto fail;
        }

        if (attributes[i].ns == HUBBUB_NS_NULL) {
            err = dom_element_set_attribute((struct dom_element *)node, name, value);
            dom_string_unref(name);
            dom_string_unref(value);
            if (err != DOM_NO_ERR) {
                dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't add attribute");
            }
        } else {
            err = dom_element_set_attribute_ns(
                (struct dom_element *)node, dom_namespaces[attributes[i].ns], name, value);
            dom_string_unref(name);
            dom_string_unref(value);
            if (err != DOM_NO_ERR) {
                dom_parser->msg(DOM_MSG_CRITICAL, dom_parser->mctx, "Can't add attribute ns");
            }
        }
    }
...
static hubbub_error set_quirks_mode(void *parser, hubbub_quirks_mode mode)
{
    ...
    case HUBBUB_QUIRKS_MODE_NONE:
        dom_document_set_quirks_mode(dom_parser->doc, DOM_DOCUMENT_QUIRKS_MODE_NONE);
        break;
    case HUBBUB_QUIRKS_MODE_LIMITED:
        dom_document_set_quirks_mode(dom_parser->doc, DOM_DOCUMENT_QUIRKS_MODE_LIMITED);
        break;
    case HUBBUB_QUIRKS_MODE_FULL:
        dom_document_set_quirks_mode(dom_parser->doc, DOM_DOCUMENT_QUIRKS_MODE_FULL);
        break;
    }
 ...
 */
static void dom_hubbub_parser_default_msg(uint32_t severity, void *ctx, const char *msg, ...)
{
    UNUSED(severity);
    UNUSED(ctx);
    UNUSED(msg);
...
dom_hubbub_parser_create(dom_hubbub_parser_params *params, dom_hubbub_parser **parser, dom_document **document)
{
    ...
    err = dom_implementation_create_document(
        DOM_IMPLEMENTATION_HTML, NULL, NULL, NULL, params->daf, params->ctx, &binding->doc);
    if (err != DOM_NO_ERR) {
    ...
    hubbub_parser_setopt(binding->parser, HUBBUB_PARSER_TREE_HANDLER, &optparams);

    /* set document node*/
    optparams.document_node = dom_node_ref((struct dom_node *)binding->doc);
    hubbub_parser_setopt(binding->parser, HUBBUB_PARSER_DOCUMENT_NODE, &optparams);
...
dom_hubbub_error dom_hubbub_parser_completed(dom_hubbub_parser *parser)
{
    ...
    if (err != HUBBUB_OK) {
        parser->msg(DOM_MSG_ERROR, parser->mctx, "hubbub_parser_completed failed: %d", err);
        return DOM_HUBBUB_HUBBUB_ERR | err;
    }
