...
static void expat_xmlparser_start_element_handler(void *_parser, const XML_Char *name, const XML_Char **atts)
{
    ...
    if (ns_sep != NULL) {
        err = dom_string_create_interned((const uint8_t *)name, ns_sep - name, &namespace);
        if (err != DOM_NO_ERR) {
    ...
    err = dom_string_create_interned((const uint8_t *)name, strlen(name), &tag_name);
    if (err != DOM_NO_ERR) {
    ...
    if (namespace == NULL)
        ...
        err = dom_document_create_element_ns(parser->doc, namespace, tag_name, &elem);
    if (err != DOM_NO_ERR) {
        if (namespace != NULL)
            dom_string_unref(namespace);
        dom_string_unref(tag_name);
        parser->msg(DOM_MSG_CRITICAL, parser->mctx, "Failed to create element '%s'", name);
        return;
    }
    ...
    while (*atts) {
        ...
        ns_sep = strchr(*atts, '\n');
        if (ns_sep != NULL) {
            err = dom_string_create_interned((const uint8_t *)(*atts), ns_sep - (*atts), &namespace);
            if (err != DOM_NO_ERR) {
        ...
        if (ns_sep == NULL)
            ...
            err = dom_string_create_interned((const uint8_t *)(ns_sep + 1), strlen(ns_sep + 1), &key);
        if (err != DOM_NO_ERR) {
        ...
        atts++;
        err = dom_string_create((const uint8_t *)(*atts), strlen(*atts), &value);
        if (err != DOM_NO_ERR) {
            dom_node_unref(elem);
            if (namespace != NULL)
                dom_string_unref(namespace);
        ...
        if (namespace == NULL)
            ...
            err = dom_element_set_attribute_ns(elem, namespace, key, value);
        if (namespace != NULL)
            dom_string_unref(namespace);
        dom_string_unref(key);
        ...
        if (err != DOM_NO_ERR) {
            dom_node_unref(elem);
            parser->msg(DOM_MSG_CRITICAL, parser->mctx, "No memory for setting attribute");
            return;
        }
    }

    err = dom_node_append_child(parser->current, (struct dom_node *)elem, (struct dom_node **)(void *)&ins_elem);
    if (err != DOM_NO_ERR) {
        dom_node_unref(elem);
        parser->msg(DOM_MSG_CRITICAL, parser->mctx, "No memory for appending child node");
        return;
    }
...
static void expat_xmlparser_end_element_handler(void *_parser, const XML_Char *name)
{
    dom_xml_parser *parser = _parser;
    dom_exception err;
    dom_node *parent;
    ...
    if (parent == NULL || parent == (dom_node *)parser->doc) {
        ...
        parser->msg(DOM_MSG_CRITICAL, parser->mctx, "Attempted to close more than was opened.");
        return;
    }
...
static void expat_xmlparser_start_cdata_handler(void *_parser)
{
    dom_xml_parser *parser = _parser;
...
static void expat_xmlparser_end_cdata_handler(void *_parser)
{
    dom_xml_parser *parser = _parser;
...
static void expat_xmlparser_cdata_handler(void *_parser, const XML_Char *s, int len)
{
    dom_xml_parser *parser = _parser;
    dom_string *data;
    dom_exception err;
    ...
    if (ntype == DOM_TEXT_NODE && parser->is_cdata == false) {
        ...
        err = dom_characterdata_append_data((dom_characterdata *)lastchild, data);
        dom_string_unref(data);
        if (lastchild != NULL)
            dom_node_unref(lastchild);
        if (err != DOM_NO_ERR) {
    ...
    err = parser->is_cdata ? dom_document_create_cdata_section(parser->doc, data, (dom_cdata_section **)(void *)&cdata)
                           : dom_document_create_text_node(parser->doc, data, (dom_text **)(void *)&cdata);
    if (err != DOM_NO_ERR) {
        dom_string_unref(data);
        parser->msg(DOM_MSG_CRITICAL, parser->mctx, "No memory for cdata section");
        return;
...
static int expat_xmlparser_external_entity_ref_handler(XML_Parser parser, const XML_Char *context, const XML_Char *base,
    const XML_Char *system_id, const XML_Char *public_id)
{
    ...
    if ((strncmp(system_id, "http://", 7) == 0) || (strncmp(system_id, "https://", 8) == 0)) {
        return XML_STATUS_OK;
    }
...
static void expat_xmlparser_comment_handler(void *_parser, const XML_Char *_comment)
{
    dom_xml_parser *parser = _parser;
    ...
    err = dom_string_create((const uint8_t *)_comment, strlen((const char *)_comment), &data);
    if (err != DOM_NO_ERR) {
    ...
    err = dom_node_append_child(parser->current, (struct dom_node *)comment, (struct dom_node **)(void *)&ins_comment);
    if (err != DOM_NO_ERR) {
...
static void expat_xmlparser_start_doctype_decl_handler(void *_parser, const XML_Char *doctype_name,
    const XML_Char *system_id, const XML_Char *public_id, int has_internal_subset)
{
    dom_xml_parser *parser = _parser;
    ...
    err = dom_implementation_create_document_type(
        doctype_name, system_id ? system_id : "", public_id ? public_id : "", &doctype);
    ...
    err = dom_node_append_child(parser->doc, (struct dom_node *)doctype, (struct dom_node **)(void *)&ins_doctype);
    if (err != DOM_NO_ERR) {
...
static void expat_xmlparser_unknown_data_handler(void *_parser, const XML_Char *s, int len)
{
    UNUSED(_parser);
    UNUSED(s);
    UNUSED(len);
...
dom_xml_parser_create(const char *enc, const char *int_enc, dom_msg msg, void *mctx, dom_document **document)
{
    ...
    if (err != DOM_NO_ERR) {
        parser->msg(DOM_MSG_CRITICAL, parser->mctx, "Failed creating document");
    ...
    XML_SetElementHandler(parser->parser, expat_xmlparser_start_element_handler, expat_xmlparser_end_element_handler);

    XML_SetCdataSectionHandler(parser->parser, expat_xmlparser_start_cdata_handler, expat_xmlparser_end_cdata_handler);

    XML_SetCharacterDataHandler(parser->parser, expat_xmlparser_cdata_handler);

    XML_SetParamEntityParsing(parser->parser, XML_PARAM_ENTITY_PARSING_ALWAYS);

    XML_SetExternalEntityRefHandler(parser->parser, expat_xmlparser_external_entity_ref_handler);

    XML_SetCommentHandler(parser->parser, expat_xmlparser_comment_handler);

    XML_SetStartDoctypeDeclHandler(parser->parser, expat_xmlparser_start_doctype_decl_handler);
...
dom_xml_error dom_xml_parser_parse_chunk(dom_xml_parser *parser, uint8_t *data, size_t len)
{
    ...
    if (status != XML_STATUS_OK) {
        parser->msg(DOM_MSG_ERROR, parser->mctx, "XML_Parse failed: %d", status);
