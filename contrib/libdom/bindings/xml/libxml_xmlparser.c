...
static void xml_parser_start_element_ns(void *ctx,
...
static void xml_parser_end_element_ns(void *ctx,
				      const xmlChar *localname,
				      const xmlChar *prefix,
				      const xmlChar *URI);
...
static xmlEntityPtr
xml_parser_get_parameter_entity(void *ctx, const xmlChar *name);
...
static void xml_parser_internal_subset(void *ctx,
				       const xmlChar *name,
				       const xmlChar *ExternalID,
				       const xmlChar *SystemID);
static void xml_parser_external_subset(void *ctx,
				       const xmlChar *name,
				       const xmlChar *ExternalID,
				       const xmlChar *SystemID);
static void xml_parser_entity_decl(void *ctx,
				   const xmlChar *name,
				   int type,
				   const xmlChar *publicId,
				   const xmlChar *systemId,
				   xmlChar *content);
static void xml_parser_notation_decl(void *ctx,
				     const xmlChar *name,
				     const xmlChar *publicId,
				     const xmlChar *systemId);
static void xml_parser_attribute_decl(void *ctx,
				      const xmlChar *elem,
...
static void xml_parser_element_decl(void *ctx,
				    const xmlChar *name,
				    int type,
				    xmlElementContentPtr content);
static void xml_parser_unparsed_entity_decl(void *ctx,
					    const xmlChar *name,
					    const xmlChar *publicId,
					    const xmlChar *systemId,
				      ...
				      dom_document **document)
{
	...
	parser->xml_ctx = xmlCreatePushParserCtxt(
		&sax_handler, parser, "", 0, NULL);
	...
	ret = xmlCtxtUseOptions(parser->xml_ctx,
				XML_PARSE_DTDATTR | XML_PARSE_DTDLOAD);
	if (ret != 0) {
...
static dom_exception xml_parser_add_attribute(dom_xml_parser *parser,
					      dom_element *elem,
					      const xmlChar *localname,
					      const xmlChar *prefix,
					      const xmlChar *URI,
					      ...
					      size_t len)
{
	...
	if (URI != NULL) {
		err = dom_string_create_interned((const uint8_t *)URI,
	...
	if (prefix != NULL) {
		...
		if ((size_t)snprintf(qname, qlen, "%s:%s", prefix, localname) >=
		    qlen) {
			parser->msg(DOM_MSG_CRITICAL,
				    parser->mctx,
		...
		err = dom_string_create_interned((const uint8_t *)qname,
						 qlen - 1,
						 &name);
		if (err != DOM_NO_ERR) {
			parser->msg(DOM_MSG_CRITICAL,
 ...
 */
void xml_parser_start_element_ns(void *ctx,
				 const xmlChar *localname,
				 const xmlChar *prefix,
				 const xmlChar *URI,
				 int nb_namespaces,
				 ...
				 const xmlChar **attributes)
{
	...
	if (prefix != NULL) {
		...
		if ((size_t)snprintf(qname, len, "%s:%s", prefix, localname) >=
		    len) {
			parser->msg(DOM_MSG_CRITICAL,
				    parser->mctx,
		...
		parser->err = dom_string_create_interned((const uint8_t *)qname,
							 len - 1,
							 &tag_name);
		if (parser->err != DOM_NO_ERR) {
			parser->msg(DOM_MSG_CRITICAL,
	...
	if (namespace == NULL) {
		...
		parser->err = dom_document_create_element_ns(
			parser->doc, namespace, tag_name, &elem);
	}
	...
	for (attr = 0; attr < nb_attributes; attr++) {
		...
		parser->err = xml_parser_add_attribute(parser,
						       elem,
						       a_localname,
						       a_prefix,
						       a_URI,
 ...
 * \param URI        The element namespace URI
 */
void xml_parser_end_element_ns(void *ctx,
			       const xmlChar *localname,
			       const xmlChar *prefix,
			       const xmlChar *URI)
{
...
void xml_parser_reference(void *ctx, const xmlChar *name)
{
	...
	parser->err = dom_document_create_entity_reference(parser->doc,
							   data,
							   &ref);
...
void xml_parser_cdata_block(void *ctx, const xmlChar *value, int len)
{
	...
	parser->err = dom_document_create_cdata_section(parser->doc,
							data,
							&cdata);
	dom_string_unref(data);
...
#ifdef DEBUG_XML_PARSER
	parser->msg(
		DOM_MSG_DEBUG, parser->mctx, "GetParameterEntity(%s)", name);
#endif
...
void xml_parser_internal_subset(void *ctx,
				const xmlChar *name,
				const xmlChar *ExternalID,
				const xmlChar *SystemID)
{
...
void xml_parser_external_subset(void *ctx,
				const xmlChar *name,
				const xmlChar *ExternalID,
				const xmlChar *SystemID)
{
...
void xml_parser_entity_decl(void *ctx,
			    const xmlChar *name,
			    int type,
			    const xmlChar *publicId,
			    const xmlChar *systemId,
			    xmlChar *content)
{
	...
	xmlSAX2EntityDecl(
		parser->xml_ctx, name, type, publicId, systemId, content);
}

void xml_parser_notation_decl(void *ctx,
			      const xmlChar *name,
			      const xmlChar *publicId,
			      const xmlChar *systemId)
{
...
void xml_parser_attribute_decl(void *ctx,
			       const xmlChar *elem,
			       const xmlChar *fullname,
			       int type,
			       int def,
			       ...
			       xmlEnumerationPtr tree)
{
	...
	xmlSAX2AttributeDecl(
		parser->xml_ctx, elem, fullname, type, def, defaultValue, tree);
}

void xml_parser_element_decl(void *ctx,
			     const xmlChar *name,
			     int type,
			     xmlElementContentPtr content)
{
...
#ifdef DEBUG_XML_PARSER
	parser->msg(
		DOM_MSG_DEBUG, parser->mctx, "ElementDecl(%s, %d)", name, type);
#endif
...
void xml_parser_unparsed_entity_decl(void *ctx,
				     const xmlChar *name,
				     const xmlChar *publicId,
				     const xmlChar *systemId,
				     const xmlChar *notationName)
{
	...
	xmlSAX2UnparsedEntityDecl(
		parser->xml_ctx, name, publicId, systemId, notationName);
}
