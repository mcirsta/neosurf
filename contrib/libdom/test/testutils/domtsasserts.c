...
bool is_equals_string(const char *expected, dom_string *actual, bool ignoreCase)
{
    ...
    err = dom_string_create((const uint8_t *)expected, strlen(expected), &exp);
    if (err != DOM_NO_ERR)
        return false;
    ...
    const char *query, const char *fragment, const char *isAbsolute, dom_string *actual)
{
    ...
    if (host != NULL) {
        if (actualPath[0] == '/' && actualPath[1] == '/') {
            ...
            actualHost = domts_strndup(actualPath, termslash - actualPath);
        } else {
            actualHost = strdup("");
        }
    ...
    if (name != NULL) {
        ...
        if (finalPeriod != NULL) {
            actualName = domts_strndup(actualFile, finalPeriod - actualFile);
        } else {
