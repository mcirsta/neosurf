...
dom_document *create_doc_dom_from_file(char *file)
{
    ...
    while (chunk_length == buffer_size) {
        ...
        error = dom_hubbub_parser_parse_chunk(parser, buffer, chunk_length);
        if (error != DOM_HUBBUB_OK) {
            dom_hubbub_parser_destroy(parser);
    ...
    if (!close) {
        if (length == 5 && strncmp(string, "title", 5) == 0) {
            ...
            if (exc == DOM_NO_ERR && s != NULL) {
                printf(" $%.*s$", (int)dom_string_byte_length(s), dom_string_data(s));
                dom_string_unref(s);
            }
        }

        /* Print the element's id & class, if it has them */
        if (dump_dom_element_attribute(node, "id") == false || dump_dom_element_attribute(node, "class") == false) {
            /* Error occured */
            printf(">\n");
            return false;
        }
...
bool dump_dom_structure(dom_node *node, int depth)
{
    ...
    exc = libdom_treewalk(DOM_WALK_ENABLE_ALL, dump_dom_structure__cb, node, &depth);
    if (exc != DOM_NO_ERR) {
        return false;
    }
