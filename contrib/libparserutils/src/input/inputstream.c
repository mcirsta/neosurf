...
static inline parserutils_error parserutils_inputstream_refill_buffer(parserutils_inputstream_private *stream);
static inline parserutils_error parserutils_inputstream_strip_bom(uint16_t *mibenum, parserutils_buffer *buffer);
 ...
 */
parserutils_error parserutils_inputstream_create(
    const char *enc, uint32_t encsrc, parserutils_charset_detect_func csdetect, parserutils_inputstream **stream)
{
    ...
    if (enc != NULL) {
        ...
        error = parserutils__filter_setopt(s->input, PARSERUTILS_FILTER_SET_ENCODING, &params);
        if (error != PARSERUTILS_OK) {
            parserutils__filter_destroy(s->input);
            parserutils_buffer_destroy(s->public.utf8);
            parserutils_buffer_destroy(s->raw);
...
{
    parserutils_inputstream_private *s = (parserutils_inputstream_private *)stream;
...
parserutils_error parserutils_inputstream_insert(parserutils_inputstream *stream, const uint8_t *data, size_t len)
{
    ...
    return parserutils_buffer_insert(s->public.utf8, s->public.cursor, data, len);
}
 ...
 */
parserutils_error
parserutils_inputstream_peek_slow(parserutils_inputstream *stream, size_t offset, const uint8_t **ptr, size_t *length)
{
    parserutils_inputstream_private *s = (parserutils_inputstream_private *)stream;
    ...
    if (s->raw->length == 0) {
        ...
        return s->public.had_eof ? PARSERUTILS_EOF : PARSERUTILS_NEEDDATA;
    }
    ...
    if (IS_ASCII(s->public.utf8->data[s->public.cursor + offset])) {
        ...
        error = parserutils_charset_utf8_char_byte_length(s->public.utf8->data + s->public.cursor + offset, &len);
 ...
 */
parserutils_error
parserutils_inputstream_change_charset(parserutils_inputstream *stream, const char *enc, uint32_t source)
{
    parserutils_inputstream_private *s = (parserutils_inputstream_private *)stream;
    parserutils_filter_optparams params;
    uint16_t temp;
...
parserutils_error parserutils_inputstream_refill_buffer(parserutils_inputstream_private *stream)
{
    ...
    if (stream->done_first_chunk == false) {
        ...
        if (stream->csdetect != NULL) {
            error = stream->csdetect(stream->raw->data, stream->raw->length, &stream->mibenum, &stream->encsrc);
            if (error != PARSERUTILS_OK) {
                if (error != PARSERUTILS_NEEDDATA || stream->public.had_eof == false)
                    return error;
         ...
         */
        if (stream->mibenum == 0) {
            stream->mibenum = parserutils_charset_mibenum_from_name("UTF-8", SLEN("UTF-8"));
            stream->encsrc = 0;
        }
        ...
        error = parserutils_inputstream_strip_bom(&stream->mibenum, stream->raw);
        if (error != PARSERUTILS_OK)
            return error;

        /* Ensure filter is using the correct encoding */
        params.encoding.name = parserutils_charset_mibenum_to_name(stream->mibenum);

        error = parserutils__filter_setopt(stream->input, PARSERUTILS_FILTER_SET_ENCODING, &params);
        if (error != PARSERUTILS_OK)
            return error;
    ...
    if (stream->public.cursor == stream->public.utf8->length) {
         ...
         * extend it. */
        memmove(stream->public.utf8->data, stream->public.utf8->data + stream->public.cursor,
            stream->public.utf8->length - stream->public.cursor);

        stream->public.utf8->length -= stream->public.cursor;

        if (stream->public.utf8->length > stream->public.utf8->allocated / 2) {
    ...
    error = parserutils__filter_process_chunk(stream->input, &raw, &raw_length, &utf8, &utf8_space);
    ...
    error = parserutils_buffer_discard(stream->raw, 0, stream->raw->length - raw_length);
    if (error != PARSERUTILS_OK)
        return error;

    /* Fix up the utf8 buffer information */
    stream->public.utf8->length = stream->public.utf8->allocated - utf8_space;
 ...
 */
parserutils_error parserutils_inputstream_strip_bom(uint16_t *mibenum, parserutils_buffer *buffer)
{
    ...
    if (utf8 == 0) {
        utf8 = parserutils_charset_mibenum_from_name("UTF-8", SLEN("UTF-8"));
        ...
        utf32 = parserutils_charset_mibenum_from_name("UTF-32", SLEN("UTF-32"));
        utf32be = parserutils_charset_mibenum_from_name("UTF-32BE", SLEN("UTF-32BE"));
        utf32le = parserutils_charset_mibenum_from_name("UTF-32LE", SLEN("UTF-32LE"));
    }
...
#define UTF8_BOM_LEN (3)
    ...
    if (*mibenum == utf8) {
        if (buffer->length >= UTF8_BOM_LEN && buffer->data[0] == 0xEF && buffer->data[1] == 0xBB &&
            buffer->data[2] == 0xBF) {
            return parserutils_buffer_discard(buffer, 0, UTF8_BOM_LEN);
        }
    } else if (*mibenum == utf16be) {
        if (buffer->length >= UTF16_BOM_LEN && buffer->data[0] == 0xFE && buffer->data[1] == 0xFF) {
            return parserutils_buffer_discard(buffer, 0, UTF16_BOM_LEN);
        }
    } else if (*mibenum == utf16le) {
        if (buffer->length >= UTF16_BOM_LEN && buffer->data[0] == 0xFF && buffer->data[1] == 0xFE) {
            return parserutils_buffer_discard(buffer, 0, UTF16_BOM_LEN);
        }
        ...
        *mibenum = utf16be;
        ...
        if (buffer->length >= UTF16_BOM_LEN) {
            if (buffer->data[0] == 0xFE && buffer->data[1] == 0xFF) {
                return parserutils_buffer_discard(buffer, 0, UTF16_BOM_LEN);
            } else if (buffer->data[0] == 0xFF && buffer->data[1] == 0xFE) {
                *mibenum = utf16le;
                return parserutils_buffer_discard(buffer, 0, UTF16_BOM_LEN);
            }
        }
    } else if (*mibenum == utf32be) {
        if (buffer->length >= UTF32_BOM_LEN && buffer->data[0] == 0x00 && buffer->data[1] == 0x00 &&
            buffer->data[2] == 0xFE && buffer->data[3] == 0xFF) {
            return parserutils_buffer_discard(buffer, 0, UTF32_BOM_LEN);
        }
    } else if (*mibenum == utf32le) {
        if (buffer->length >= UTF32_BOM_LEN && buffer->data[0] == 0xFF && buffer->data[1] == 0xFE &&
        ...
        *mibenum = utf32be;
        ...
        if (buffer->length >= UTF32_BOM_LEN) {
            if (buffer->data[0] == 0x00 && buffer->data[1] == 0x00 && buffer->data[2] == 0xFE &&
                buffer->data[3] == 0xFF) {
                return parserutils_buffer_discard(buffer, 0, UTF32_BOM_LEN);
            } else if (buffer->data[0] == 0xFF && buffer->data[1] == 0xFE && buffer->data[2] == 0x00 &&
                buffer->data[3] == 0x00) {
                *mibenum = utf32le;
                return parserutils_buffer_discard(buffer, 0, UTF32_BOM_LEN);
            }
