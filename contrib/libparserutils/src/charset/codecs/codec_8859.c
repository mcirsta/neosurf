...
static struct {
	...
	size_t len;
	uint32_t *table;
} known_charsets[] = {{0, "ISO-8859-1", SLEN("ISO-8859-1"), t1},
...
charset_8859_codec_create(const char *charset,
			  parserutils_charset_codec **codec);
static parserutils_error
charset_8859_codec_destroy(parserutils_charset_codec *codec);
static parserutils_error
charset_8859_codec_encode(parserutils_charset_codec *codec,
			  const uint8_t **source,
			  size_t *sourcelen,
			  uint8_t **dest,
			  size_t *destlen);
static parserutils_error
charset_8859_codec_decode(parserutils_charset_codec *codec,
			  const uint8_t **source,
			  size_t *sourcelen,
			  uint8_t **dest,
			  size_t *destlen);
static parserutils_error
charset_8859_codec_reset(parserutils_charset_codec *codec);
static inline parserutils_error
charset_8859_codec_read_char(charset_8859_codec *c,
			     const uint8_t **source,
			     size_t *sourcelen,
			     uint8_t **dest,
			     size_t *destlen);
static inline parserutils_error
charset_8859_codec_output_decoded_char(charset_8859_codec *c,
				       uint32_t ucs4,
				       uint8_t **dest,
				       size_t *destlen);
...
bool charset_8859_codec_handles_charset(const char *charset)
{
	...
	if (known_charsets[0].mib == 0) {
		for (i = 0; i < N_ELEMENTS(known_charsets); i++) {
			known_charsets[i]
				.mib = parserutils_charset_mibenum_from_name(
					    ...
					    size_t *destlen)
{
	...
	if (c->write_len > 0) {
		...
		while (c->write_len > 0) {
			error = charset_8859_from_ucs4(
				c, pwrite[0], dest, destlen);
					    ...
					    size_t *destlen)
{
	...
	while (*sourcelen > 0) {
		error = charset_8859_codec_read_char(
					       ...
					       size_t *destlen)
{
	...
	if (error == PARSERUTILS_OK) {
		/* Read a character */
		error = charset_8859_codec_output_decoded_char(
			c, ucs4, dest, destlen);
