...
static struct {
    ...
    size_t len;
    uint32_t *table;
} known_charsets[] = {{0, "ISO-8859-1", SLEN("ISO-8859-1"), t1}, {0, "ISO-8859-2", SLEN("ISO-8859-2"), t2},
    {0, "ISO-8859-3", SLEN("ISO-8859-3"), t3}, {0, "ISO-8859-4", SLEN("ISO-8859-4"), t4},
    {0, "ISO-8859-5", SLEN("ISO-8859-5"), t5}, {0, "ISO-8859-6", SLEN("ISO-8859-6"), t6},
    {0, "ISO-8859-7", SLEN("ISO-8859-7"), t7}, {0, "ISO-8859-8", SLEN("ISO-8859-8"), t8},
    {0, "ISO-8859-9", SLEN("ISO-8859-9"), t9}, {0, "ISO-8859-10", SLEN("ISO-8859-10"), t10},
    {0, "ISO-8859-11", SLEN("ISO-8859-11"), t11}, {0, "ISO-8859-13", SLEN("ISO-8859-13"), t13},
    {0, "ISO-8859-14", SLEN("ISO-8859-14"), t14}, {0, "ISO-8859-15", SLEN("ISO-8859-15"), t15},
...
static inline parserutils_error charset_8859_codec_read_char(
    charset_8859_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen);
static inline parserutils_error
charset_8859_codec_output_decoded_char(charset_8859_codec *c, uint32_t ucs4, uint8_t **dest, size_t *destlen);
static inline parserutils_error charset_8859_from_ucs4(charset_8859_codec *c, uint32_t ucs4, uint8_t **s, size_t *len);
static inline parserutils_error
charset_8859_to_ucs4(charset_8859_codec *c, const uint8_t *s, size_t len, uint32_t *ucs4);
...
bool charset_8859_codec_handles_charset(const char *charset)
{
    ...
    if (known_charsets[0].mib == 0) {
        for (i = 0; i < N_ELEMENTS(known_charsets); i++) {
            known_charsets[i].mib = parserutils_charset_mibenum_from_name(
                known_charsets[i].name, known_charsets[i].len);
        }
    }
 ...
 */
parserutils_error charset_8859_codec_create(const char *charset, parserutils_charset_codec **codec)
{
    uint32_t i;
 ...
 */
parserutils_error charset_8859_codec_encode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    ...
    if (c->write_len > 0) {
        ...
        while (c->write_len > 0) {
            error = charset_8859_from_ucs4(c, pwrite[0], dest, destlen);
    ...
    while (*sourcelen > 0) {
        ...
        while (towritelen > 0) {
            error = charset_8859_from_ucs4(c, towrite[0], dest, destlen);
            if (error != PARSERUTILS_OK) {
                uint32_t len;
 ...
 */
parserutils_error charset_8859_codec_decode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    charset_8859_codec *c = (charset_8859_codec *)codec;
    parserutils_error error;
    ...
    while (*sourcelen > 0) {
        error = charset_8859_codec_read_char(c, source, sourcelen, dest, destlen);
        if (error != PARSERUTILS_OK) {
            return error;
        }
    }
 ...
 * ::destlen will be reduced appropriately on exit.
 */
parserutils_error charset_8859_codec_read_char(
    charset_8859_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    uint32_t ucs4;
    parserutils_error error;
    ...
    if (error == PARSERUTILS_OK) {
        /* Read a character */
        error = charset_8859_codec_output_decoded_char(c, ucs4, dest, destlen);
        ...
        error = charset_8859_codec_output_decoded_char(c, 0xFFFD, dest, destlen);
        if (error == PARSERUTILS_OK || error == PARSERUTILS_NOMEM) {
            /* output succeeded; update source pointers */
            *source += 1;
            *sourcelen -= 1;
 ...
 */
parserutils_error charset_8859_from_ucs4(charset_8859_codec *c, uint32_t ucs4, uint8_t **s, size_t *len)
{
    uint8_t out = 0;
 ...
 *         PARSERUTILS_INVALID if the character cannot be represented
 */
parserutils_error charset_8859_to_ucs4(charset_8859_codec *c, const uint8_t *s, size_t len, uint32_t *ucs4)
{
    uint32_t out;
...
const parserutils_charset_handler charset_8859_codec_handler = {
    charset_8859_codec_handles_charset, charset_8859_codec_create};
