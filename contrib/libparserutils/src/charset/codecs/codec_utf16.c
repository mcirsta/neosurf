...
static inline parserutils_error charset_utf16_codec_read_char(
    charset_utf16_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen);
static inline parserutils_error
charset_utf16_codec_output_decoded_char(charset_utf16_codec *c, uint32_t ucs4, uint8_t **dest, size_t *destlen);
 ...
 */
parserutils_error charset_utf16_codec_create(const char *charset, parserutils_charset_codec **codec)
{
 ...
 */
parserutils_error charset_utf16_codec_encode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    ...
    if (c->write_len > 0) {
        ...
        while (c->write_len > 0) {
            error = parserutils_charset_utf16_from_ucs4(pwrite[0], buf, &len);
    ...
    while (*sourcelen > 0) {
        ...
        while (towritelen > 0) {
            ...
            error = parserutils_charset_utf16_from_ucs4(towrite[0], buf, &len);
            assert(error == PARSERUTILS_OK);
 ...
 */
parserutils_error charset_utf16_codec_decode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    charset_utf16_codec *c = (charset_utf16_codec *)codec;
    parserutils_error error;
    ...
    if (c->inval_len > 0) {
        ...
        error = charset_utf16_codec_read_char(c, (const uint8_t **)&in, &l, dest, destlen);
    ...
    while (*sourcelen > 0) {
        error = charset_utf16_codec_read_char(c, source, sourcelen, dest, destlen);
        if (error != PARSERUTILS_OK) {
            return error;
        }
    }
 ...
 * ::destlen will be reduced appropriately on exit.
 */
parserutils_error charset_utf16_codec_read_char(
    charset_utf16_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    uint32_t ucs4;
    size_t sucs4;
    ...
    error = parserutils_charset_utf16_to_ucs4(*source, *sourcelen, &ucs4, &sucs4);
    if (error == PARSERUTILS_OK) {
        /* Read a character */
        error = charset_utf16_codec_output_decoded_char(c, ucs4, dest, destlen);
        if (error == PARSERUTILS_OK || error == PARSERUTILS_NOMEM) {
        ...
        *sourcelen = 0;
        ...
        error = parserutils_charset_utf16_next_paranoid(*source, *sourcelen, 0, &nextchar);
        if (error != PARSERUTILS_OK) {
        ...
        error = charset_utf16_codec_output_decoded_char(c, 0xFFFD, dest, destlen);
        if (error == PARSERUTILS_OK || error == PARSERUTILS_NOMEM) {
            /* output succeeded; update source pointers */
            *source += nextchar;
            *sourcelen -= nextchar;
