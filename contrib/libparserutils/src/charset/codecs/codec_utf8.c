...
static inline parserutils_error charset_utf8_codec_read_char(
    charset_utf8_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen);
static inline parserutils_error
charset_utf8_codec_output_decoded_char(charset_utf8_codec *c, uint32_t ucs4, uint8_t **dest, size_t *destlen);
...
bool charset_utf8_codec_handles_charset(const char *charset)
{
    return parserutils_charset_mibenum_from_name(charset, strlen(charset)) ==
        parserutils_charset_mibenum_from_name("UTF-8", SLEN("UTF-8"));
}
 ...
 */
parserutils_error charset_utf8_codec_create(const char *charset, parserutils_charset_codec **codec)
{
 ...
 */
parserutils_error charset_utf8_codec_encode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
 ...
 */
parserutils_error charset_utf8_codec_decode(
    parserutils_charset_codec *codec, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    charset_utf8_codec *c = (charset_utf8_codec *)codec;
    parserutils_error error;
    ...
    if (c->inval_len > 0) {
        ...
        error = charset_utf8_codec_read_char(c, (const uint8_t **)&in, &l, dest, destlen);
    ...
    while (*sourcelen > 0) {
        error = charset_utf8_codec_read_char(c, source, sourcelen, dest, destlen);
        if (error != PARSERUTILS_OK) {
            return error;
        }
    }
 ...
 * ::destlen will be reduced appropriately on exit.
 */
parserutils_error charset_utf8_codec_read_char(
    charset_utf8_codec *c, const uint8_t **source, size_t *sourcelen, uint8_t **dest, size_t *destlen)
{
    uint32_t ucs4 = 0;
    size_t sucs4 = 0;
    ...
    if (error == PARSERUTILS_OK) {
        /* Read a character */
        error = charset_utf8_codec_output_decoded_char(c, ucs4, dest, destlen);
        if (error == PARSERUTILS_OK || error == PARSERUTILS_NOMEM) {
        ...
        *sourcelen = 0;
        ...
        error = charset_utf8_codec_output_decoded_char(c, 0xFFFD, dest, destlen);
        if (error == PARSERUTILS_OK || error == PARSERUTILS_NOMEM) {
            /* output succeeded; update source pointers */
            *source += nextchar;
            *sourcelen -= nextchar;
...
const parserutils_charset_handler charset_utf8_codec_handler = {
    charset_utf8_codec_handles_charset, charset_utf8_codec_create};
