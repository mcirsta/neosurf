...
parserutils_error parserutils_vector_append(parserutils_vector *vector, void *item)
{
    ...
    if ((size_t)slot >= vector->items_allocated) {
        ...
        /* Overflow check */
        if (new_allocated < vector->items_allocated || new_allocated > SIZE_MAX / vector->item_size)
            return PARSERUTILS_NOMEM;
    ...
    memcpy((uint8_t *)vector->items + (slot * vector->item_size), item, vector->item_size);
