 ...
 * \return PARSERUTILS_OK on success, appropriate error otherwise.
 */
static inline parserutils_error ensure_space(parserutils_buffer *buffer, size_t len)
{
    try_rebase(buffer);
...
parserutils_error parserutils_buffer_insert(parserutils_buffer *buffer, size_t offset, const uint8_t *data, size_t len)
{
    ...
    memmove(buffer->data + offset + len, buffer->data + offset, buffer->length - offset);
...
parserutils_error parserutils_buffer_discard(parserutils_buffer *buffer, size_t offset, size_t len)
{
    ...
    memmove(buffer->data + offset, buffer->data + offset + len, buffer->length - (len + offset));
