...
parserutils_error parserutils_stack_push(parserutils_stack *stack, const void *item)
{
    ...
    if ((size_t)slot >= stack->items_allocated) {
        void *temp = realloc(stack->items, (stack->items_allocated + stack->chunk_size) * stack->item_size);
        if (temp == NULL)
            return PARSERUTILS_NOMEM;
...
parserutils_error parserutils_stack_pop(parserutils_stack *stack, void *item)
{
    ...
    if (item != NULL) {
        memcpy(item, (uint8_t *)stack->items + (stack->current_item * stack->item_size), stack->item_size);
    }
