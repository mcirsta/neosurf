...
typedef parserutils_error (*parserutils_charset_detect_func)(
    const uint8_t *data, size_t len, uint16_t *mibenum, uint32_t *source);
...
parserutils_inputstream_peek(parserutils_inputstream *stream, size_t offset, const uint8_t **ptr, size_t *length)
{
...
#define IS_ASCII(x) (((x) & 0x80) == 0)
    ...
    if (off < utf8_len) {
        if (IS_ASCII(utf8_data[off])) {
            ...
            error = parserutils_charset_utf8_char_byte_length(utf8_data + off, &len);
 ...
 */
static inline void parserutils_inputstream_advance(parserutils_inputstream *stream, size_t bytes)
{
    if (stream == NULL)
        return;
