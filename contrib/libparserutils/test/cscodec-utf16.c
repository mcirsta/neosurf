...
int main(int argc, char **argv)
{
    ...
    assert(parserutils_charset_codec_create("NATS-SEFI-ADD", &codec) == PARSERUTILS_BADENCODING);

    assert(parserutils_charset_codec_create("UTF-16", &ctx.codec) == PARSERUTILS_OK);
    ...
    if (data[0] == '#') {
        ...
        if (strncasecmp(data + 1, "data", 4) == 0) {
            ...
            if (strncasecmp(ptr, "LOOSE", 5) == 0) {
                ...
                params.error_mode.mode = PARSERUTILS_CHARSET_CODEC_ERROR_TRANSLIT;
                ptr += 9;
            }

            assert(parserutils_charset_codec_setopt(ctx->codec, PARSERUTILS_CHARSET_CODEC_ERROR_MODE,
                       (parserutils_charset_codec_optparams *)&params) == PARSERUTILS_OK);
        } else if (strncasecmp(data + 1, "expected", 8) == 0) {
            ctx->indata = false;
            ctx->inexp = true;

            ctx->exp_ret = parserutils_error_from_string(data + 10, datalen - 10 - 1 /* \n */);
        ...
        if (ctx->indata) {
            ...
            while (datalen) {
                ...
                assert(datalen >= sizeof("&#xNNNN") - 1 && data[0] == '&' && data[1] == '#' && data[2] == 'x' &&
                    isxdigit(data[3]) && isxdigit(data[4]) && isxdigit(data[5]) && isxdigit(data[6]));
                /* UTF-16 code is always host endian (different
                   than UCS-32 !).  */
                nCodePoint = (hex2digit(data[3]) << 12) | (hex2digit(data[4]) << 8) | (hex2digit(data[5]) << 4) |
                    hex2digit(data[6]);
                *((uint16_t *)(void *)(ctx->buf + ctx->bufused)) = nCodePoint;
                ctx->bufused += 2;
                data += sizeof("&#xNNNN") - 1;
        ...
        if (ctx->inexp) {
            ...
            while (datalen) {
                ...
                assert(datalen >= sizeof("&#xXXXXYYYY") - 1 && data[0] == '&' && data[1] == '#' && data[2] == 'x' &&
                    isxdigit(data[3]) && isxdigit(data[4]) && isxdigit(data[5]) && isxdigit(data[6]) &&
                    isxdigit(data[7]) && isxdigit(data[8]) && isxdigit(data[9]) && isxdigit(data[10]));
                /* UCS-4 code is always big endian, so convert
                   host endian to big endian.  */
                nCodePoint = htonl((hex2digit(data[3]) << 28) | (hex2digit(data[4]) << 24) |
                    (hex2digit(data[5]) << 20) | (hex2digit(data[6]) << 16) | (hex2digit(data[7]) << 12) |
                    (hex2digit(data[8]) << 8) | (hex2digit(data[9]) << 4) | hex2digit(data[10]));
                *((uint32_t *)(void *)(ctx->exp + ctx->expused)) = nCodePoint;
                ctx->expused += 4;
...
void run_test(line_ctx *ctx)
{
    ...
    if (ctx->dir == DECODE) {
        assert(parserutils_charset_codec_decode(ctx->codec, &psrc, &srclen, &pdest, &destlen) == ctx->exp_ret);
    } else if (ctx->dir == ENCODE) {
        assert(parserutils_charset_codec_encode(ctx->codec, &psrc, &srclen, &pdest, &destlen) == ctx->exp_ret);
    } else {
        ...
        assert(parserutils_charset_codec_decode(ctx->codec, &psrc, &srclen, &ptemp, &templen) == ctx->exp_ret);
        ...
        assert(parserutils_charset_codec_encode(ctx->codec, &ptemp2, &templen2, &pdest, &destlen) == ctx->exp_ret);
        if (ctx->exp_ret == PARSERUTILS_OK) {
            assert(templen2 == 0);
    ...
    for (i = 0; i < ctx->expused; i++) {
        printf("%c%c ", "0123456789abcdef"[(dest[i] >> 4) & 0xf], "0123456789abcdef"[dest[i] & 0xf]);
    }
    printf("' Expected '");
    for (i = 0; i < ctx->expused; i++) {
        printf("%c%c ", "0123456789abcdef"[(ctx->exp[i] >> 4) & 0xf], "0123456789abcdef"[ctx->exp[i] & 0xf]);
    }
    printf("'\n");
