...
hubbub_error handle_in_caption(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    case HUBBUB_TOKEN_START_TAG: {
        ...
        if (type == CAPTION || type == COL || type == COLGROUP || type == TBODY || type == TD || type == TFOOT ||
    ...
    case HUBBUB_TOKEN_END_TAG: {
        ...
        if (type == CAPTION) {
            ...
            err = HUBBUB_REPROCESS;
        } else if (type == BODY || type == COL || type == COLGROUP || type == HTML || type == TBODY || type == TD ||
            type == TFOOT || type == TH || type == THEAD || type == TR) {
            /** \todo parse error */
        } else {
            /* Process as if "in body" */
            err = handle_in_body(treebuilder, token);
