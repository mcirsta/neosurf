...
static hubbub_error
process_dd_dt_li_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token, element_type type);
...
static hubbub_error
process_presentational_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token, element_type type);
...
static hubbub_error
process_applet_marquee_object_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token, element_type type);
...
static hubbub_error aa_find_and_validate_formatting_element(
    hubbub_treebuilder *treebuilder, element_type type, formatting_list_entry **element);
static formatting_list_entry *aa_find_formatting_element(hubbub_treebuilder *treebuilder, element_type type);
static hubbub_error aa_find_furthest_block(
    hubbub_treebuilder *treebuilder, formatting_list_entry *formatting_element, uint32_t *furthest_block);
static hubbub_error aa_reparent_node(hubbub_treebuilder *treebuilder, void *node, void *new_parent, void **reparented);
static hubbub_error aa_find_bookmark_location_reparenting_misnested(hubbub_treebuilder *treebuilder,
    uint32_t formatting_element, uint32_t *furthest_block, bookmark *bookmark, uint32_t *last_node);
...
hubbub_error handle_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    if (treebuilder->context.strip_leading_lr && token->type != HUBBUB_TOKEN_CHARACTER) {
    ...
    case HUBBUB_TOKEN_EOF:
        for (i = treebuilder->context.current_node; i > 0; i--) {
            element_type type = treebuilder->context.element_stack[i].type;

            if (!(type == DD || type == DT || type == LI || type == P || type == TBODY || type == TD || type == TFOOT ||
                    type == TH || type == THEAD || type == TR || type == BODY)) {
                /** \todo parse error */
                break;
            }
        }
 ...
 * \param token        The token to process
 */
hubbub_error process_character(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err = HUBBUB_OK;
    ...
    if (treebuilder->context.frameset_ok) {
        for (p = dummy.ptr; p < dummy.ptr + dummy.len; p++) {
            if (*p != 0x0009 && *p != 0x000a && *p != 0x000c && *p != 0x0020) {
 ...
 */
hubbub_error process_start_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err = HUBBUB_OK;
    ...
    if (type == HTML) {
    ...
    } else if (type == BASE || type == COMMAND || type == LINK || type == META || type == NOFRAMES || type == SCRIPT ||
    ...
    } else if (type == ADDRESS || type == ARTICLE || type == ASIDE || type == BLOCKQUOTE || type == CENTER ||
        type == DATAGRID || type == DETAILS || type == DIALOG || type == DIR || type == DIV || type == DL ||
        type == FIELDSET || type == FIGCAPTION || type == FIGURE || type == FOOTER || type == HEADER || type == MAIN ||
        type == MENU || type == NAV || type == OL || type == P || type == SECTION || type == SUMMARY || type == UL) {
    ...
    } else if (type == B || type == BIG || type == CODE || type == EM || type == FONT || type == I || type == S ||
        type == SMALL || type == STRIKE || type == STRONG || type == TT || type == U) {
        err = process_presentational_in_body(treebuilder, token, type);
    } else if (type == NOBR) {
        err = process_nobr_in_body(treebuilder, token);
    } else if (type == BUTTON) {
        err = process_button_in_body(treebuilder, token);
    } else if (type == APPLET || type == MARQUEE || type == OBJECT) {
        err = process_applet_marquee_object_in_body(treebuilder, token, type);
    } else if (type == XMP) {
        err = reconstruct_active_formatting_list(treebuilder);
        if (err != HUBBUB_OK)
            return err;
        ...
        if (err == HUBBUB_OK) {
            ...
            treebuilder->context.element_stack[current_table(treebuilder)].tainted = false;
            treebuilder->context.mode = IN_TABLE;
        }
    } else if (type == AREA || type == BASEFONT || type == BGSOUND || type == BR || type == EMBED || type == IMG ||
        type == INPUT || type == PARAM || type == SPACER || type == WBR) {
        err = reconstruct_active_formatting_list(treebuilder);
        if (err != HUBBUB_OK)
            return err;
    ...
    } else if (type == IFRAME || type == NOEMBED || type == NOFRAMES ||
        (treebuilder->context.enable_scripting && type == NOSCRIPT)) {
        if (type == IFRAME)
            treebuilder->context.frameset_ok = false;
        err = parse_generic_rcdata(treebuilder, token, false);
    } else if (type == SELECT) {
        ...
        } else if (treebuilder->context.mode == IN_TABLE || treebuilder->context.mode == IN_CAPTION ||
            treebuilder->context.mode == IN_COLUMN_GROUP || treebuilder->context.mode == IN_TABLE_BODY ||
            treebuilder->context.mode == IN_ROW || treebuilder->context.mode == IN_CELL) {
        ...
        if (token->data.tag.self_closing) {
            ...
            if (err == HUBBUB_OK) {
                treebuilder->context.second_mode = treebuilder->context.mode;
                treebuilder->context.mode = IN_FOREIGN_CONTENT;
            }
        }
    } else if (type == CAPTION || type == COL || type == COLGROUP || type == FRAME || type == HEAD || type == TBODY ||
        type == TD || type == TFOOT || type == TH || type == THEAD || type == TR) {
        /** \todo parse error */
    } else {
 ...
 */
hubbub_error process_end_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err = HUBBUB_OK;
    element_type type = element_type_from_name(treebuilder, &token->data.tag.name);
    ...
    if (type == BODY) {
        ...
        type == DETAILS || type == DIALOG || type == DIR || type == DATAGRID || type == DIV || type == DL ||
        type == FIELDSET || type == FIGCAPTION || type == FIGURE || type == FOOTER || type == HEADER ||
        type == LISTING || type == MAIN || type == MENU || type == NAV || type == OL || type == PRE ||
    ...
    } else if (type == A || type == B || type == BIG || type == CODE || type == EM || type == FONT || type == I ||
        type == NOBR || type == S || type == SMALL || type == STRIKE || type == STRONG || type == TT || type == U) {
        err = process_0presentational_in_body(treebuilder, type);
    } else if (type == APPLET || type == BUTTON || type == MARQUEE || type == OBJECT) {
        err = process_0applet_button_marquee_object_in_body(treebuilder, type);
    } else if (type == BR) {
        err = process_0br_in_body(treebuilder);
    } else if (type == AREA || type == BASEFONT || type == BGSOUND || type == EMBED || type == HR || type == IFRAME ||
        type == IMAGE || type == IMG || type == INPUT || type == ISINDEX || type == NOEMBED || type == NOFRAMES ||
        type == PARAM || type == SELECT || type == SPACER || type == TABLE || type == TEXTAREA || type == WBR ||
        (treebuilder->context.enable_scripting && type == NOSCRIPT)) {
        /** \todo parse error */
    } else {
 ...
 * \param token        The token to process
 */
hubbub_error process_html_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    /** \todo parse error */

    return treebuilder->tree_handler->add_attributes(treebuilder->tree_handler->ctx,
        treebuilder->context.element_stack[0].node, token->data.tag.attributes, token->data.tag.n_attributes);
}
 ...
 * \param token        The token to process
 */
hubbub_error process_body_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    /** \todo parse error */

    if (treebuilder->context.current_node < 1 || treebuilder->context.element_stack[1].type != BODY)
        return HUBBUB_OK;

    return treebuilder->tree_handler->add_attributes(treebuilder->tree_handler->ctx,
        treebuilder->context.element_stack[1].node, token->data.tag.attributes, token->data.tag.n_attributes);
}
 ...
 * \param token        The token to process
 */
hubbub_error process_frameset_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err = HUBBUB_OK;
    ...
    err = remove_node_from_dom(treebuilder, treebuilder->context.element_stack[1].node);
    if (err != HUBBUB_OK)
        return err;
 ...
 * \param token        The token to process
 */
hubbub_error process_container_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
 ...
 * \param token        The token to process
 */
hubbub_error process_hN_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    element_type type;
    ...
    type = treebuilder->context.element_stack[treebuilder->context.current_node].type;
 ...
 * \param token        The token to process
 */
hubbub_error process_form_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    ...
    if (treebuilder->context.form_element != NULL) {
        ...
        treebuilder->context.form_element = treebuilder->context.element_stack[treebuilder->context.current_node].node;
    }
 ...
 */
hubbub_error process_dd_dt_li_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token, element_type type)
{
    hubbub_error err;
    ...
    for (node = treebuilder->context.current_node; node > 0; node--) {
        ...
        if (((type == DD || type == DT) && (ntype == DD || ntype == DT)))
            break;

        if (!is_formatting_element(ntype) && !is_phrasing_element(ntype) && ntype != ADDRESS && ntype != DIV)
            break;
    }

    /* If we found one, then pop all nodes up to and including it */
    if (stack[node].type == LI || stack[node].type == DD || stack[node].type == DT) {
 ...
 * \param token        The token to process
 */
hubbub_error process_plaintext_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    ...
    err = hubbub_tokeniser_setopt(treebuilder->tokeniser, HUBBUB_TOKENISER_CONTENT_MODEL, &params);
    assert(err == HUBBUB_OK);
 ...
 * \param token        The token to process
 */
hubbub_error process_a_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    formatting_list_entry *entry = aa_find_formatting_element(treebuilder, A);
    ...
    if (entry != NULL) {
        ...
        if (entry2 == entry && entry2->details.node == node) {
            ...
            err = formatting_list_remove(treebuilder, entry, &ons, &otype, &onode, &oindex);
            assert(err == HUBBUB_OK);

            treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, onode);
        }

        /* Remove from the stack of open elements, if still there */
        if (index <= treebuilder->context.current_node && treebuilder->context.element_stack[index].node == node) {
            hubbub_ns ns;
            element_type otype;
            void *onode;

            err = element_stack_remove(treebuilder, index, &ns, &otype, &onode);
            assert(err == HUBBUB_OK);
    ...
    err = formatting_list_append(treebuilder, token->data.tag.ns, A,
        treebuilder->context.element_stack[treebuilder->context.current_node].node, treebuilder->context.current_node);
    if (err != HUBBUB_OK) {
        hubbub_ns ns;
        element_type type;
        void *node;

        remove_node_from_dom(treebuilder, treebuilder->context.element_stack[treebuilder->context.current_node].node);
 ...
 * \param token        The token to process
 */
hubbub_error process_nobr_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
 ...
 * \param token        The token to process
 */
hubbub_error process_button_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    ...
    if (element_in_scope(treebuilder, BUTTON, false)) {
        ...
        err = process_0applet_button_marquee_object_in_body(treebuilder, BUTTON);
        assert(err == HUBBUB_OK);
    }
 ...
 * \param token        The token to process
 */
hubbub_error process_hr_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
 ...
 * \param token        The token to process
 */
hubbub_error process_image_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    hubbub_tag tag;
 ...
 * \param token        The token to process
 */
hubbub_error process_isindex_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
    hubbub_token dummy;
    ...
    if (token->data.tag.n_attributes > 0) {
        uint32_t i;
        attrs = malloc((token->data.tag.n_attributes + 1) * sizeof(hubbub_attribute));
        if (attrs == NULL)
            return HUBBUB_NOMEM;
        ...
        for (i = 0; i < token->data.tag.n_attributes; i++) {
            ...
            if (strncmp((const char *)name, "action", attr->name.len) == 0) {
                action = attr;
            } else if (strncmp((const char *)name, "prompt", attr->name.len) == 0) {
                prompt = attr;
            } else if (strncmp((const char *)name, "name", attr->name.len) == 0) {
            } else {
                attrs[n_attrs++] = *attr;
            }
        }
 ...
 * \param token        The token to process
 */
hubbub_error process_textarea_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    treebuilder->context.strip_leading_lr = true;
    treebuilder->context.frameset_ok = false;
 ...
 * \param token        The token to process
 */
hubbub_error process_select_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
 ...
 * \param token        The token to process
 */
hubbub_error process_opt_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
 ...
 * \param token        The token to process
 */
hubbub_error process_phrasing_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    hubbub_error err;
...
hubbub_error process_0body_in_body(hubbub_treebuilder *treebuilder)
{
    ...
    if (!element_in_scope(treebuilder, BODY, false)) {
        ...
        for (node = treebuilder->context.current_node; node > 0; node--) {
            ...
            if (ntype != DD && ntype != DT && ntype != LI && ntype != OPTGROUP && ntype != OPTION && ntype != P &&
                ntype != RP && ntype != RT && ntype != TBODY && ntype != TD && ntype != TFOOT && ntype != TH &&
                ntype != THEAD && ntype != TR && ntype != BODY) {
                /** \todo parse error */
            }
        }
 ...
 * \param type         The element type
 */
hubbub_error process_0container_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
...
hubbub_error process_0form_in_body(hubbub_treebuilder *treebuilder)
{
    ...
    if (treebuilder->context.form_element != NULL)
        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, treebuilder->context.form_element);
    treebuilder->context.form_element = NULL;

    idx = element_in_scope(treebuilder, FORM, false);

    if (idx == 0 || node == NULL || treebuilder->context.element_stack[idx].node != node) {
        /** \todo parse error */
    } else {
        hubbub_ns ns;
        element_type otype;
        ...
        if (treebuilder->context.element_stack[treebuilder->context.current_node].node != node) {
            /** \todo parse error */
        }

        element_stack_remove(treebuilder, idx, &ns, &otype, &onode);
...
hubbub_error process_0p_in_body(hubbub_treebuilder *treebuilder)
{
    ...
    if (treebuilder->context.element_stack[treebuilder->context.current_node].type != P) {
        /** \todo parse error */
    }
 ...
 * \param type         The element type
 */
hubbub_error process_0dd_dt_li_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
    if (!element_in_scope(treebuilder, type, false)) {
        /** \todo parse error */
    } else {
 ...
 * \param type         The element type
 */
hubbub_error process_0h_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
    UNUSED(type);

    /** \todo optimise this */
    if (element_in_scope(treebuilder, H1, false) || element_in_scope(treebuilder, H2, false) ||
        element_in_scope(treebuilder, H3, false) || element_in_scope(treebuilder, H4, false) ||
        element_in_scope(treebuilder, H5, false) || element_in_scope(treebuilder, H6, false)) {
        uint32_t popped = 0;
        element_type otype;
        ...
        do {
            ...
            popped++;
        } while (otype != H1 && otype != H2 && otype != H3 && otype != H4 && otype != H5 && otype != H6);
 ...
 * \param type         The element type
 */
hubbub_error process_0presentational_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
    hubbub_error err;
    ...
    while (true) {
        ...
        /* 1 */
        err = aa_find_and_validate_formatting_element(treebuilder, type, &entry);
        ...
        /* 2 & 3 */
        err = aa_find_furthest_block(treebuilder, entry, &furthest_block);
        assert(err == HUBBUB_OK || err == HUBBUB_REPROCESS);
        if (err == HUBBUB_OK)
            return err;
        ...
        err = aa_find_bookmark_location_reparenting_misnested(
            treebuilder, formatting_element, &furthest_block, &bookmark, &last_node);
        if (err != HUBBUB_OK)
            return err;

        /* 7 */
        if (stack[common_ancestor].type == TABLE || stack[common_ancestor].type == TBODY ||
            stack[common_ancestor].type == TFOOT || stack[common_ancestor].type == THEAD ||
            stack[common_ancestor].type == TR) {
            err = aa_insert_into_foster_parent(treebuilder, stack[last_node].node, &reparented);
        } else {
            err = aa_reparent_node(treebuilder, stack[last_node].node, stack[common_ancestor].node, &reparented);
        }
        if (err != HUBBUB_OK)
            return err;

        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, stack[last_node].node);
        ...
        if (reparented != stack[last_node].node) {
            ...
            for (node_entry = treebuilder->context.formatting_list_end; node_entry != NULL;
                node_entry = node_entry->prev) {
                if (node_entry->stack_index == last_node) {
                    treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx, reparented);
        ...
        err = treebuilder->tree_handler->clone_node(
            treebuilder->tree_handler->ctx, entry->details.node, false, &fe_clone);
        if (err != HUBBUB_OK)
            return err;

        /* 9 */
        err = treebuilder->tree_handler->reparent_children(
            treebuilder->tree_handler->ctx, stack[furthest_block].node, fe_clone);
        if (err != HUBBUB_OK) {
            treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, fe_clone);
            return err;
        }

        /* 10 */
        err = treebuilder->tree_handler->append_child(
            treebuilder->tree_handler->ctx, stack[furthest_block].node, fe_clone, &clone_appended);
        if (err != HUBBUB_OK) {
            treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, fe_clone);
            return err;
        }
        ...
        if (clone_appended != fe_clone) {
            ...
            treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx, clone_appended);
        }
        ...
        /* 12 */
        err = aa_remove_element_stack_item(treebuilder, formatting_element, furthest_block);
        assert(err == HUBBUB_OK);
        ...
        err = formatting_list_insert(
            treebuilder, bookmark.prev, bookmark.next, ons, otype, clone_appended, furthest_block + 1);
        if (err != HUBBUB_OK) {
            treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, clone_appended);
            return err;
        }
    ...
    hubbub_treebuilder *treebuilder, element_type type, formatting_list_entry **element)
{
    ...
    if (entry == NULL ||
        (entry->stack_index != 0 && element_in_scope(treebuilder, entry->details.type, false) != entry->stack_index)) {
        /** \todo parse error */
        return HUBBUB_OK;
    }
    ...
    if (entry->stack_index == 0) {
        ...
        formatting_list_remove(treebuilder, entry, &ns, &type, &node, &index);
 ...
 */
formatting_list_entry *aa_find_formatting_element(hubbub_treebuilder *treebuilder, element_type type)
{
    formatting_list_entry *entry;

    for (entry = treebuilder->context.formatting_list_end; entry != NULL; entry = entry->prev) {

        /* Assumption: HTML and TABLE elements are not in the list */
        if (is_scoping_element(entry->details.type) || entry->details.type == type)
            break;
    }
 ...
 *         HUBBUB_OK to stop.
 */
hubbub_error aa_find_furthest_block(
    hubbub_treebuilder *treebuilder, formatting_list_entry *formatting_element, uint32_t *furthest_block)
{
    ...
    if (fb > treebuilder->context.current_node) {
        ...
        formatting_list_remove(treebuilder, formatting_element, &ns, &type, &node, &index);
 ...
 */
hubbub_error aa_reparent_node(hubbub_treebuilder *treebuilder, void *node, void *new_parent, void **reparented)
{
    hubbub_error err;
    ...
    return treebuilder->tree_handler->append_child(treebuilder->tree_handler->ctx, new_parent, node, reparented);
}
    ...
    uint32_t formatting_element, uint32_t *furthest_block, bookmark *bookmark, uint32_t *last_node)
{
    ...
    while (true) {
        ...
        /* ii */
        for (node_entry = treebuilder->context.formatting_list_end; node_entry != NULL; node_entry = node_entry->prev) {
        ...
        if (node_entry == NULL) {
            err = aa_remove_element_stack_item(treebuilder, node, treebuilder->context.current_node);
            assert(err == HUBBUB_OK);
        ...
        /* vi */
        err = aa_reparent_node(treebuilder, stack[last].node, stack[node].node, &reparented);
        if (err != HUBBUB_OK)
            return err;

        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, stack[last].node);
 ...
 */
hubbub_error aa_remove_element_stack_item(hubbub_treebuilder *treebuilder, uint32_t index, uint32_t limit)
{
    element_context *stack = treebuilder->context.element_stack;
    uint32_t n;
    ...
    for (n = index + 1; n <= limit; n++) {
        if (is_formatting_element(stack[n].type) ||
            (is_scoping_element(stack[n].type) && stack[n].type != HTML && stack[n].type != TABLE)) {
            formatting_list_entry *e;

            for (e = treebuilder->context.formatting_list_end; e != NULL; e = e->prev) {
                if (e->stack_index == n)
                    e->stack_index--;
            }
        }
    ...
    treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, stack[index].node);

    /* Now, shuffle the stack up one, removing node in the process */
    memmove(&stack[index], &stack[index + 1], (limit - index) * sizeof(element_context));
 ...
 */
hubbub_error aa_clone_and_replace_entries(hubbub_treebuilder *treebuilder, formatting_list_entry *element)
{
    hubbub_error err;
    hubbub_ns ons;
    element_type otype;
    ...
    err = treebuilder->tree_handler->clone_node(treebuilder->tree_handler->ctx, element->details.node, false, &clone);
    if (err != HUBBUB_OK)
        return err;

    /* Replace formatting list entry for node with clone */
    err = formatting_list_replace(treebuilder, element, element->details.ns, element->details.type, clone,
        element->stack_index, &ons, &otype, &onode, &oindex);
    assert(err == HUBBUB_OK);

    treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, onode);

    treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx, clone);
...
hubbub_error aa_insert_into_foster_parent(hubbub_treebuilder *treebuilder, void *node, void **inserted)
{
    ...
    if (cur_table == 0) {
        treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx, stack[0].node);

        foster_parent = stack[0].node;
    } else {
        void *t_parent = NULL;

        treebuilder->tree_handler->get_parent(treebuilder->tree_handler->ctx, stack[cur_table].node, true, &t_parent);

        if (t_parent != NULL) {
            foster_parent = t_parent;
            insert = true;
        } else {
            treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx, stack[cur_table - 1].node);
    ...
    err = remove_node_from_dom(treebuilder, node);
    if (err != HUBBUB_OK) {
        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, foster_parent);
        return err;
    }

    if (insert) {
        err = treebuilder->tree_handler->insert_before(
            treebuilder->tree_handler->ctx, foster_parent, node, stack[cur_table].node, inserted);
    } else {
        err = treebuilder->tree_handler->append_child(treebuilder->tree_handler->ctx, foster_parent, node, inserted);
    }
    if (err != HUBBUB_OK) {
        treebuilder->tree_handler->unref_node(treebuilder->tree_handler->ctx, foster_parent);
        return err;
 ...
 * \param type         The element type
 */
hubbub_error process_0applet_button_marquee_object_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
    if (!element_in_scope(treebuilder, type, false)) {
        /** \todo parse error */
    } else {
 ...
 * \param type         The element type
 */
hubbub_error process_0generic_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
    element_context *stack = treebuilder->context.element_stack;
    ...
    do {
        if (stack[node].type == type) {
            ...
            break;
        } else if (!is_formatting_element(stack[node].type) && !is_phrasing_element(stack[node].type)) {
            /** \todo parse error */
            break;
        }
    } while (--node > 0);
