...
static hubbub_error
process_character(hubbub_treebuilder *treebuilder, const hubbub_token *token);
static hubbub_error
process_start_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token);
static hubbub_error
process_end_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token);
...
static hubbub_error process_container_in_body(hubbub_treebuilder *treebuilder,
					      const hubbub_token *token);
static hubbub_error
process_hN_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token);
...
static hubbub_error process_plaintext_in_body(hubbub_treebuilder *treebuilder,
					      const hubbub_token *token);
static hubbub_error
process_a_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token);
static hubbub_error
process_presentational_in_body(hubbub_treebuilder *treebuilder,
			       const hubbub_token *token,
			       element_type type);
...
static hubbub_error process_button_in_body(hubbub_treebuilder *treebuilder,
					   const hubbub_token *token);
static hubbub_error
process_applet_marquee_object_in_body(hubbub_treebuilder *treebuilder,
				      const hubbub_token *token,
				      element_type type);
static hubbub_error
process_hr_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token);
...
static hubbub_error process_select_in_body(hubbub_treebuilder *treebuilder,
					   const hubbub_token *token);
static hubbub_error
process_opt_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token);
...
static hubbub_error
process_0container_in_body(hubbub_treebuilder *treebuilder, element_type type);
...
static hubbub_error
process_0dd_dt_li_in_body(hubbub_treebuilder *treebuilder, element_type type);
static hubbub_error
process_0h_in_body(hubbub_treebuilder *treebuilder, element_type type);
static hubbub_error
process_0presentational_in_body(hubbub_treebuilder *treebuilder,
				element_type type);
static hubbub_error
process_0applet_button_marquee_object_in_body(hubbub_treebuilder *treebuilder,
					      element_type type);
static hubbub_error process_0br_in_body(hubbub_treebuilder *treebuilder);
static hubbub_error
process_0generic_in_body(hubbub_treebuilder *treebuilder, element_type type);

static hubbub_error
aa_find_and_validate_formatting_element(hubbub_treebuilder *treebuilder,
					element_type type,
					formatting_list_entry **element);
static formatting_list_entry *
aa_find_formatting_element(hubbub_treebuilder *treebuilder, element_type type);
static hubbub_error
aa_find_furthest_block(hubbub_treebuilder *treebuilder,
...
static hubbub_error aa_reparent_node(hubbub_treebuilder *treebuilder,
				     ...
				     void **reparented);
static hubbub_error
aa_find_bookmark_location_reparenting_misnested(hubbub_treebuilder *treebuilder,
						uint32_t formatting_element,
						uint32_t *furthest_block,
						bookmark *bookmark,
						uint32_t *last_node);
static hubbub_error
aa_remove_element_stack_item(hubbub_treebuilder *treebuilder,
			     uint32_t index,
			     uint32_t limit);
static hubbub_error
aa_clone_and_replace_entries(hubbub_treebuilder *treebuilder,
			     formatting_list_entry *element);
 ...
 */
hubbub_error
handle_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
...
process_start_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	...
	if (type == HTML) {
		...
		err = process_frameset_in_body(treebuilder, token);
		   ...
		   type == BLOCKQUOTE || type == CENTER || type == DATAGRID ||
		   type == DETAILS || type == DIALOG || type == DIR ||
		   type == DIV || type == DL || type == FIELDSET ||
		   type == FIGCAPTION || type == FIGURE || type == FOOTER ||
		   type == HEADER || type == MAIN || type == MENU ||
		   type == NAV || type == OL || type == P || type == SECTION ||
		...
		err = process_a_in_body(treebuilder, token);
		   ...
		   type == FONT || type == I || type == S || type == SMALL ||
		   type == STRIKE || type == STRONG || type == TT ||
		   type == U) {
		...
		if (err == HUBBUB_OK) {
			...
			treebuilder->context
				.element_stack[current_table(treebuilder)]
				.tainted = false;
			treebuilder->context.mode = IN_TABLE;
		}
	} else if (type == AREA || type == BASEFONT || type == BGSOUND ||
		   type == BR || type == EMBED || type == IMG ||
		   type == INPUT || type == PARAM || type == SPACER ||
		   type == WBR) {
		err = reconstruct_active_formatting_list(treebuilder);
		if (err != HUBBUB_OK)
			return err;
		...
		if (token->data.tag.self_closing) {
		   ...
		   type == TD || type == TFOOT || type == TH || type == THEAD ||
		   type == TR) {
		/** \todo parse error */
	} else {
...
process_end_tag(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	...
	if (type == BODY) {
		...
		err = process_0body_in_body(treebuilder);
		   ...
		   type == BLOCKQUOTE || type == CENTER || type == DETAILS ||
		   type == DIALOG || type == DIR || type == DATAGRID ||
		   type == DIV || type == DL || type == FIELDSET ||
		   type == FIGCAPTION || type == FIGURE || type == FOOTER ||
		   type == HEADER || type == LISTING || type == MAIN ||
		   type == MENU || type == NAV || type == OL || type == PRE ||
		...
		err = process_0h_in_body(treebuilder, type);
		   ...
		   type == EM || type == FONT || type == I || type == NOBR ||
		   type == S || type == SMALL || type == STRIKE ||
		   type == STRONG || type == TT || type == U) {
		err = process_0presentational_in_body(treebuilder, type);
	} else if (type == APPLET || type == BUTTON || type == MARQUEE ||
		   type == OBJECT) {
		err = process_0applet_button_marquee_object_in_body(treebuilder,
								    type);
	} else if (type == BR) {
		err = process_0br_in_body(treebuilder);
	} else if (type == AREA || type == BASEFONT || type == BGSOUND ||
		   type == EMBED || type == HR || type == IFRAME ||
		   type == IMAGE || type == IMG || type == INPUT ||
		   type == ISINDEX || type == NOEMBED || type == NOFRAMES ||
		   type == PARAM || type == SELECT || type == SPACER ||
 ...
 */
hubbub_error
process_html_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	/** \todo parse error */
 ...
 */
hubbub_error
process_body_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	/** \todo parse error */
...
process_hN_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	...
	type = treebuilder->context
		       .element_stack[treebuilder->context.current_node]
		       .type;
 ...
 */
hubbub_error
process_form_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	hubbub_error err;
				      ...
				      element_type type)
{
	...
	for (node = treebuilder->context.current_node; node > 0; node--) {
		...
		if (!is_formatting_element(ntype) &&
		    !is_phrasing_element(ntype) && ntype != ADDRESS &&
		    ntype != DIV)
			break;
	}
	...
	if (stack[node].type == LI || stack[node].type == DD ||
		...
		do {
			...
			err = element_stack_pop(
				treebuilder, &ns, &otype, &node);
			assert(err == HUBBUB_OK);
...
{
	hubbub_error err;
	formatting_list_entry *entry = aa_find_formatting_element(treebuilder,
								  A);
	...
	if (entry != NULL) {
		...
		if (entry2 == entry && entry2->details.node == node) {
			...
			err = formatting_list_remove(treebuilder,
						     entry,
						     &ons,
						     &otype,
						     &onode,
		...
		if (index <= treebuilder->context.current_node &&
		    treebuilder->context.element_stack[index].node == node) {
			hubbub_ns ns;
			element_type otype;
			void *onode;

			err = element_stack_remove(
				treebuilder, index, &ns, &otype, &onode);
			assert(err == HUBBUB_OK);
 ...
 */
hubbub_error
process_nobr_in_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	hubbub_error err;
	...
	if (token->data.tag.n_attributes > 0) {
		...
		for (i = 0; i < token->data.tag.n_attributes; i++) {
			...
			if (strncmp((const char *)name,
				    ...
				    attr->name.len) == 0) {
				action = attr;
			} else if (strncmp((const char *)name,
					   "prompt",
					   attr->name.len) == 0) {
				prompt = attr;
			} else if (strncmp((const char *)name,
	...
	if (!element_in_scope(treebuilder, BODY, false)) {
		...
		for (node = treebuilder->context.current_node; node > 0;
			...
			if (ntype != DD && ntype != DT && ntype != LI &&
			    ...
			    ntype != P && ntype != RP && ntype != RT &&
			    ntype != TBODY && ntype != TD && ntype != TFOOT &&
			    ntype != TH && ntype != THEAD && ntype != TR &&
			    ntype != BODY) {
				/** \todo parse error */
			}
		}
...
process_0h_in_body(hubbub_treebuilder *treebuilder, element_type type)
{
	...
	if (element_in_scope(treebuilder, H1, false) ||
		...
		do {
			...
			popped++;
		} while (otype != H1 && otype != H2 && otype != H3 &&
			 otype != H4 && otype != H5 && otype != H6);
					     ...
					     element_type type)
{
	...
	while (true) {
		...
		if (stack[common_ancestor].type == TABLE ||
			...
			err = aa_insert_into_foster_parent(
				treebuilder,
		...
		treebuilder->tree_handler->unref_node(
			treebuilder->tree_handler->ctx, stack[last_node].node);
		...
		err = treebuilder->tree_handler->reparent_children(
			treebuilder->tree_handler->ctx,
			stack[furthest_block].node,
			fe_clone);
		if (err != HUBBUB_OK) {
			treebuilder->tree_handler->unref_node(
				treebuilder->tree_handler->ctx, fe_clone);
			return err;
		}
		...
		if (clone_appended != fe_clone) {
			...
			treebuilder->tree_handler->ref_node(
				treebuilder->tree_handler->ctx, clone_appended);
		}
		...
		/* 11 */
		err = formatting_list_remove(
			treebuilder, entry, &ons, &otype, &onode, &oindex);
		assert(err == HUBBUB_OK);
					...
					formatting_list_entry **element)
{
	...
	if (entry == NULL ||
	    (entry->stack_index != 0 &&
	     element_in_scope(treebuilder, entry->details.type, false) !=
		     entry->stack_index)) {
		/** \todo parse error */
		return HUBBUB_OK;
	}
	...
	if (entry->stack_index == 0) {
		...
		formatting_list_remove(
			treebuilder, entry, &ns, &type, &node, &index);
...
aa_find_formatting_element(hubbub_treebuilder *treebuilder, element_type type)
{
	...
	for (entry = treebuilder->context.formatting_list_end; entry != NULL;
	     entry = entry->prev) {
			      ...
			      void **reparented)
{
	...
	return treebuilder->tree_handler->append_child(
		treebuilder->tree_handler->ctx, new_parent, node, reparented);
}
						...
						uint32_t *last_node)
{
	...
	while (true) {
		...
		if (node_entry == NULL) {
			err = aa_remove_element_stack_item(
				treebuilder,
				node,
				treebuilder->context.current_node);
			assert(err == HUBBUB_OK);
		...
		treebuilder->tree_handler->unref_node(
			treebuilder->tree_handler->ctx, stack[last].node);
	...
	for (n = index + 1; n <= limit; n++) {
		if (is_formatting_element(stack[n].type) ||
		    (is_scoping_element(stack[n].type) &&
		     stack[n].type != HTML && stack[n].type != TABLE)) {
					  ...
					  formatting_list_entry *element)
{
	...
	err = formatting_list_replace(treebuilder,
				      element,
				      element->details.ns,
					  ...
					  void **inserted)
{
	...
	if (cur_table == 0) {
		treebuilder->tree_handler->ref_node(
			treebuilder->tree_handler->ctx, stack[0].node);
	...
	if (err != HUBBUB_OK) {
		treebuilder->tree_handler->unref_node(
			treebuilder->tree_handler->ctx, foster_parent);
		return err;
	}
