...
static void table_clear_stack(hubbub_treebuilder *treebuilder)
{
	...
	while (cur_node != TBODY && cur_node != TFOOT && cur_node != THEAD &&
	       cur_node != HTML) {
		hubbub_ns ns;
		element_type type;
		void *node;
 ...
 */
static hubbub_error
table_sub_start_or_table_end(hubbub_treebuilder *treebuilder)
{
 ...
 */
hubbub_error
handle_in_table_body(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
	...
	case HUBBUB_TOKEN_START_TAG: {
		...
		if (type == TR) {
			...
			if (err == HUBBUB_OK) {
			   ...
			   type == TBODY || type == TFOOT || type == THEAD) {
	...
	case HUBBUB_TOKEN_END_TAG: {
		...
		if (type == TBODY || type == TFOOT || type == THEAD) {
			...
			err = table_sub_start_or_table_end(treebuilder);
			   ...
			   type == COLGROUP || type == HTML || type == TD ||
			   type == TH || type == TR) {
			/** \todo parse error */
			/* Ignore the token */
		} else {
