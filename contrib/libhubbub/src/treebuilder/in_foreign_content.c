...
void adjust_foreign_attributes(hubbub_treebuilder *treebuilder, hubbub_tag *tag)
{
	...
	for (i = 0; i < tag->n_attributes; i++) {
		...
		if (attr->name.len >= 10 &&
		    strncmp((char *)name, "xlink:", SLEN("xlink:")) == 0) {
			size_t len = attr->name.len - 6;
			name += 6;

			if (hubbub_string_match(name, len, S("actuate")) ||
			    hubbub_string_match(name, len, S("arcrole")) ||
			    hubbub_string_match(name, len, S("href")) ||
			    hubbub_string_match(name, len, S("role")) ||
			    hubbub_string_match(name, len, S("show")) ||
			    hubbub_string_match(name, len, S("title")) ||
	...
	case HUBBUB_TOKEN_START_TAG: {
		...
		if (cur_node_ns == HUBBUB_NS_HTML ||
		    (cur_node_ns == HUBBUB_NS_MATHML &&
		     ...
		     (cur_node == MI || cur_node == MO || cur_node == MN ||
		      cur_node == MS || cur_node == MTEXT)) ||
		    (type == SVG && (cur_node_ns == HUBBUB_NS_MATHML &&
				     cur_node == ANNOTATION_XML)) ||
		    (cur_node_ns == HUBBUB_NS_SVG &&
		     (cur_node == FOREIGNOBJECT || cur_node == DESC ||
		      cur_node == TITLE))) {
			err = process_as_in_secondary(treebuilder, token);
			   ...
			   type == LISTING || type == MENU || type == META ||
			   type == NOBR || type == OL || type == P ||
			   type == PRE || type == RUBY || type == S ||
			   type == SMALL || type == SPAN || type == STRONG ||
