...
hubbub_error handle_in_row(hubbub_treebuilder *treebuilder, const hubbub_token *token)
{
    ...
    case HUBBUB_TOKEN_START_TAG: {
        ...
        if (type == TH || type == TD) {
            ...
            treebuilder->tree_handler->ref_node(treebuilder->tree_handler->ctx,
                treebuilder->context.element_stack[treebuilder->context.current_node].node);
            ...
            if (err != HUBBUB_OK) {
                ...
                remove_node_from_dom(
                    treebuilder, treebuilder->context.element_stack[treebuilder->context.current_node].node);
                element_stack_pop(treebuilder, &ns, &type, &node);

                return err;
            }
        } else if (type == CAPTION || type == COL || type == COLGROUP || type == TBODY || type == TFOOT ||
            type == THEAD || type == TR) {
    ...
    case HUBBUB_TOKEN_END_TAG: {
        ...
        if (type == TR) {
            ...
            err = act_as_if_end_tag_tr(treebuilder);
        } else if (type == BODY || type == CAPTION || type == COL || type == COLGROUP || type == HTML || type == TD ||
            type == TH) {
            /** \todo parse error */
            /* Ignore the token */
        } else {
