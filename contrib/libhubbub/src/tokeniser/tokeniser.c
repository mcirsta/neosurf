 ...
 */
static const uint32_t cp1252Table[32] = {0x20AC, 0xFFFD, 0x201A, 0x0192, 0x201E, 0x2026, 0x2020, 0x2021, 0x02C6, 0x2030,
...
static inline hubbub_error emit_character_token(hubbub_tokeniser *tokeniser, const hubbub_string *chars);
...
static inline hubbub_error emit_current_doctype(hubbub_tokeniser *tokeniser, bool force_quirks);
static hubbub_error hubbub_tokeniser_emit_token(hubbub_tokeniser *tokeniser, const hubbub_token *token);
 ...
 */
hubbub_error
hubbub_tokeniser_setopt(hubbub_tokeniser *tokeniser, hubbub_tokeniser_opttype type, hubbub_tokeniser_optparams *params)
{
            ...
            if (tokeniser->paused == true) {
                ...
                if (tokeniser->insert_buf->length > 0) {
                    parserutils_inputstream_insert(
                        tokeniser->input, tokeniser->insert_buf->data, tokeniser->insert_buf->length);
                    parserutils_buffer_discard(tokeniser->insert_buf, 0, tokeniser->insert_buf->length);
                }
...
#define state(x) case x:
    ...
    while (cont == HUBBUB_OK) {
        switch (tokeniser->state) {
            state(STATE_DATA) cont = hubbub_tokeniser_handle_data(tokeniser);
            break;
            state(STATE_CHARACTER_REFERENCE_DATA) cont = hubbub_tokeniser_handle_character_reference_data(tokeniser);
            break;
            state(STATE_TAG_OPEN) cont = hubbub_tokeniser_handle_tag_open(tokeniser);
            break;
            state(STATE_CLOSE_TAG_OPEN) cont = hubbub_tokeniser_handle_close_tag_open(tokeniser);
            break;
            state(STATE_TAG_NAME) cont = hubbub_tokeniser_handle_tag_name(tokeniser);
            break;
            state(STATE_BEFORE_ATTRIBUTE_NAME) cont = hubbub_tokeniser_handle_before_attribute_name(tokeniser);
            break;
            state(STATE_ATTRIBUTE_NAME) cont = hubbub_tokeniser_handle_attribute_name(tokeniser);
            break;
            state(STATE_AFTER_ATTRIBUTE_NAME) cont = hubbub_tokeniser_handle_after_attribute_name(tokeniser);
            break;
            state(STATE_BEFORE_ATTRIBUTE_VALUE) cont = hubbub_tokeniser_handle_before_attribute_value(tokeniser);
            break;
            state(STATE_ATTRIBUTE_VALUE_DQ) cont = hubbub_tokeniser_handle_attribute_value_dq(tokeniser);
            break;
            state(STATE_ATTRIBUTE_VALUE_SQ) cont = hubbub_tokeniser_handle_attribute_value_sq(tokeniser);
            break;
            state(STATE_ATTRIBUTE_VALUE_UQ) cont = hubbub_tokeniser_handle_attribute_value_uq(tokeniser);
            break;
            state(STATE_CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE)
                cont = hubbub_tokeniser_handle_character_reference_in_attribute_value(tokeniser);
            break;
            state(STATE_AFTER_ATTRIBUTE_VALUE_Q) cont = hubbub_tokeniser_handle_after_attribute_value_q(tokeniser);
            break;
            state(STATE_SELF_CLOSING_START_TAG) cont = hubbub_tokeniser_handle_self_closing_start_tag(tokeniser);
            break;
            state(STATE_BOGUS_COMMENT) cont = hubbub_tokeniser_handle_bogus_comment(tokeniser);
            break;
            state(STATE_MARKUP_DECLARATION_OPEN) cont = hubbub_tokeniser_handle_markup_declaration_open(tokeniser);
            break;
            state(STATE_MATCH_COMMENT) cont = hubbub_tokeniser_handle_match_comment(tokeniser);
            break;
        ...
        case STATE_COMMENT_END:
            ...
            break;
            state(STATE_MATCH_DOCTYPE) cont = hubbub_tokeniser_handle_match_doctype(tokeniser);
            break;
            state(STATE_DOCTYPE) cont = hubbub_tokeniser_handle_doctype(tokeniser);
            break;
            state(STATE_BEFORE_DOCTYPE_NAME) cont = hubbub_tokeniser_handle_before_doctype_name(tokeniser);
            break;
            state(STATE_DOCTYPE_NAME) cont = hubbub_tokeniser_handle_doctype_name(tokeniser);
            break;
            state(STATE_AFTER_DOCTYPE_NAME) cont = hubbub_tokeniser_handle_after_doctype_name(tokeniser);
            break;

            state(STATE_MATCH_PUBLIC) cont = hubbub_tokeniser_handle_match_public(tokeniser);
            break;
            state(STATE_BEFORE_DOCTYPE_PUBLIC) cont = hubbub_tokeniser_handle_before_doctype_public(tokeniser);
            break;
            state(STATE_DOCTYPE_PUBLIC_DQ) cont = hubbub_tokeniser_handle_doctype_public_dq(tokeniser);
            break;
            state(STATE_DOCTYPE_PUBLIC_SQ) cont = hubbub_tokeniser_handle_doctype_public_sq(tokeniser);
            break;
            state(STATE_AFTER_DOCTYPE_PUBLIC) cont = hubbub_tokeniser_handle_after_doctype_public(tokeniser);
            break;
            state(STATE_MATCH_SYSTEM) cont = hubbub_tokeniser_handle_match_system(tokeniser);
            break;
            state(STATE_BEFORE_DOCTYPE_SYSTEM) cont = hubbub_tokeniser_handle_before_doctype_system(tokeniser);
            break;
            state(STATE_DOCTYPE_SYSTEM_DQ) cont = hubbub_tokeniser_handle_doctype_system_dq(tokeniser);
            break;
            state(STATE_DOCTYPE_SYSTEM_SQ) cont = hubbub_tokeniser_handle_doctype_system_sq(tokeniser);
            break;
            state(STATE_AFTER_DOCTYPE_SYSTEM) cont = hubbub_tokeniser_handle_after_doctype_system(tokeniser);
            break;
            state(STATE_BOGUS_DOCTYPE) cont = hubbub_tokeniser_handle_bogus_doctype(tokeniser);
            break;
            state(STATE_MATCH_CDATA) cont = hubbub_tokeniser_handle_match_cdata(tokeniser);
            break;
            state(STATE_CDATA_BLOCK) cont = hubbub_tokeniser_handle_cdata_block(tokeniser);
            break;
            state(STATE_NUMBERED_ENTITY) cont = hubbub_tokeniser_handle_numbered_entity(tokeniser);
            break;
            state(STATE_NAMED_ENTITY) cont = hubbub_tokeniser_handle_named_entity(tokeniser);
            break;
        }
    }
...
#define START_BUF(str, cptr, length)                                                                                   \
    do {                                                                                                               \
        parserutils_error perror;                                                                                      \
        perror = parserutils_buffer_append(tokeniser->buffer, (uint8_t *)(cptr), (length));                            \
        if (perror != PARSERUTILS_OK)                                                                                  \
            return hubbub_error_from_parserutils_error(perror);                                                        \
        (str).len = (length);                                                                                          \
    } while (0)
...
hubbub_error hubbub_tokeniser_handle_data(hubbub_tokeniser *tokeniser)
{
    ...
    while ((error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.pending, &cptr, &len)) ==
        PARSERUTILS_OK) {
        ...
        if (c == '&' &&
             ...
             * consumption */
            break;
        } else if (c == '-' && tokeniser->escape_flag == false &&
            (tokeniser->content_model == HUBBUB_CONTENT_MODEL_RCDATA ||
             ...
             * characters. */
            error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.pending - 2, &cptr, &len);
            ...
            error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.pending + len, &cptr, &len);
    ...
    if (tokeniser->state != STATE_TAG_OPEN && (tokeniser->state != STATE_DATA || error == PARSERUTILS_EOF) &&
...
hubbub_error hubbub_tokeniser_handle_character_reference_data(hubbub_tokeniser *tokeniser)
{
    ...
    if (tokeniser->context.match_entity.complete == false) {
        return hubbub_tokeniser_consume_character_reference(tokeniser, tokeniser->context.pending);
    } else {
        hubbub_token token;
        ...
        if (tokeniser->context.match_entity.codepoint) {
            parserutils_charset_utf8_from_ucs4(tokeniser->context.match_entity.codepoint, &utf8ptr, &len);
            ...
            /* +1 for ampersand */
            parserutils_inputstream_advance(tokeniser->input, tokeniser->context.match_entity.length + 1);
        } else {
            parserutils_error error;
...
hubbub_error hubbub_tokeniser_handle_tag_open(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '/') {
        ...
        if (c == '!') {
            ...
            ctag->n_attributes = 0;
            tokeniser->context.current_tag_type = HUBBUB_TOKEN_START_TAG;
...
hubbub_error hubbub_tokeniser_handle_close_tag_open(hubbub_tokeniser *tokeniser)
{
    ...
    if (tokeniser->content_model == HUBBUB_CONTENT_MODEL_RCDATA ||
        tokeniser->content_model == HUBBUB_CONTENT_MODEL_CDATA) {
        uint8_t *start_tag_name = tokeniser->context.last_start_tag_name;
        size_t start_tag_len = tokeniser->context.last_start_tag_len;

        while ((error = parserutils_inputstream_peek(
                    tokeniser->input, ctx->pending + ctx->close_tag_match.count, &cptr, &len)) == PARSERUTILS_OK) {
            c = *cptr;

            if ((start_tag_name[ctx->close_tag_match.count] & ~0x20) != (c & ~0x20)) {
                break;
            }
        ...
        if (ctx->close_tag_match.match == true) {
            error = parserutils_inputstream_peek(
                tokeniser->input, ctx->pending + ctx->close_tag_match.count, &cptr, &len);
    ...
    if (ctx->close_tag_match.match == false && tokeniser->content_model != HUBBUB_CONTENT_MODEL_PCDATA) {
        ...
        if ('A' <= c && c <= 'Z') {
            uint8_t lc = (c + 0x20);
            START_BUF(tokeniser->context.current_tag.name, &lc, len);
            ...
            START_BUF(tokeniser->context.current_tag.name, cptr, len);
            tokeniser->context.current_tag.n_attributes = 0;
            ...
            parserutils_inputstream_advance(tokeniser->input, tokeniser->context.pending);
            tokeniser->context.pending = 0;
...
hubbub_error hubbub_tokeniser_handle_before_attribute_name(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '\t' || c == '\n' || c == '\f' || c == ' ' || c == '\r') {
        ...
        if ('A' <= c && c <= 'Z') {
            ...
            START_BUF(attr[ctag->n_attributes].name, u_fffd, sizeof(u_fffd));
        } else {
...
hubbub_error hubbub_tokeniser_handle_attribute_name(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '\t' || c == '\n' || c == '\f' || c == ' ' || c == '\r') {
        ...
        COLLECT(ctag->attributes[ctag->n_attributes - 1].name, u_fffd, sizeof(u_fffd));
        tokeniser->context.pending += len;
    } else if ('A' <= c && c <= 'Z') {
        uint8_t lc = (c + 0x20);
        COLLECT(ctag->attributes[ctag->n_attributes - 1].name, &lc, len);
        tokeniser->context.pending += len;
    } else {
        COLLECT(ctag->attributes[ctag->n_attributes - 1].name, cptr, len);
        tokeniser->context.pending += len;
    }
...
hubbub_error hubbub_tokeniser_handle_before_attribute_value(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '\t' || c == '\n' || c == '\f' || c == ' ' || c == '\r') {
        ...
        START_BUF(ctag->attributes[ctag->n_attributes - 1].value, u_fffd, sizeof(u_fffd));
        tokeniser->context.pending += len;
        tokeniser->state = STATE_ATTRIBUTE_VALUE_UQ;
    } else {
        if (c == '=') {
        ...
        START_BUF(ctag->attributes[ctag->n_attributes - 1].value, cptr, len);
...
hubbub_error hubbub_tokeniser_handle_attribute_value_dq(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '"') {
        ...
        COLLECT_MS(ctag->attributes[ctag->n_attributes - 1].value, u_fffd, sizeof(u_fffd));
        tokeniser->context.pending += len;
    } else if (c == '\r') {
        error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.pending + len, &cptr, &len);
        ...
        if (error != PARSERUTILS_OK && error != PARSERUTILS_EOF) {
            ...
            COLLECT_MS(ctag->attributes[ctag->n_attributes - 1].value, &lf, sizeof(lf));
        }

        /* Consume '\r' */
        tokeniser->context.pending += 1;
    } else {
        COLLECT_MS(ctag->attributes[ctag->n_attributes - 1].value, cptr, len);
        tokeniser->context.pending += len;
    }
...
hubbub_error hubbub_tokeniser_handle_attribute_value_uq(hubbub_tokeniser *tokeniser)
{
    ...
    assert(c == '&' || ctag->attributes[ctag->n_attributes - 1].value.len >= 1);
    ...
    if (c == '\t' || c == '\n' || c == '\f' || c == ' ' || c == '\r') {
        ...
        COLLECT(ctag->attributes[ctag->n_attributes - 1].value, u_fffd, sizeof(u_fffd));
        tokeniser->context.pending += len;
    } else {
        if (c == '"' || c == '\'' || c == '=') {
            /** \todo parse error */
        ...
        COLLECT(ctag->attributes[ctag->n_attributes - 1].value, cptr, len);
        tokeniser->context.pending += len;
    }
...
hubbub_error hubbub_tokeniser_handle_character_reference_in_attribute_value(hubbub_tokeniser *tokeniser)
{
    if (tokeniser->context.match_entity.complete == false) {
        ...
        hubbub_tag *ctag = &tokeniser->context.current_tag;
        hubbub_attribute *attr = &ctag->attributes[ctag->n_attributes - 1];
        ...
        if (tokeniser->context.match_entity.codepoint) {
            ...
            /* +1 for the ampersand */
            tokeniser->context.pending += tokeniser->context.match_entity.length + 1;
        } else {
            size_t len = 0;
            const uint8_t *cptr = NULL;
            parserutils_error error;
...
hubbub_error hubbub_tokeniser_handle_bogus_comment(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '>') {
        ...
        error = parserutils_buffer_append(tokeniser->buffer, u_fffd, sizeof(u_fffd));
        if (error != PARSERUTILS_OK)
            return hubbub_error_from_parserutils_error(error);
        ...
        if (error != PARSERUTILS_OK && error != PARSERUTILS_EOF) {
            ...
            error = parserutils_buffer_append(tokeniser->buffer, &lf, sizeof(lf));
            if (error != PARSERUTILS_OK) {
                return hubbub_error_from_parserutils_error(error);
            }
        }
        ...
        error = parserutils_buffer_append(tokeniser->buffer, (uint8_t *)cptr, len);
        if (error != PARSERUTILS_OK)
            return hubbub_error_from_parserutils_error(error);
...
hubbub_error hubbub_tokeniser_handle_comment(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == '>' &&
        (tokeniser->state == STATE_COMMENT_START_DASH || tokeniser->state == STATE_COMMENT_START ||
        ...
        if (tokeniser->state == STATE_COMMENT_START) {
            ...
            error = parserutils_buffer_append(tokeniser->buffer, (uint8_t *)"-", SLEN("-"));
            if (error != PARSERUTILS_OK) {
                return hubbub_error_from_parserutils_error(error);
            }
        }
        ...
        if (tokeniser->state == STATE_COMMENT_START_DASH || tokeniser->state == STATE_COMMENT_END_DASH) {
            error = parserutils_buffer_append(tokeniser->buffer, (uint8_t *)"-", SLEN("-"));
            if (error != PARSERUTILS_OK) {
                return hubbub_error_from_parserutils_error(error);
            }
        ...
        if (c == '\0') {
            ...
            size_t next_len;
            error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.pending + len, &cptr, &next_len);
            if (error != PARSERUTILS_OK && error != PARSERUTILS_EOF) {
                return hubbub_error_from_parserutils_error(error);
            } else if (error != PARSERUTILS_EOF && *cptr != '\n') {
                error = parserutils_buffer_append(tokeniser->buffer, &lf, sizeof(lf));
            ...
            error = parserutils_buffer_append(tokeniser->buffer, cptr, len);
            if (error != PARSERUTILS_OK) {
                return hubbub_error_from_parserutils_error(error);
            }
        }
...
hubbub_error hubbub_tokeniser_handle_match_doctype(hubbub_tokeniser *tokeniser)
{
    ...
    error = parserutils_inputstream_peek(tokeniser->input, tokeniser->context.match_doctype.count, &cptr, &len);

    if (error != PARSERUTILS_OK) {
        if (error == PARSERUTILS_EOF) {
            tokeniser->context.current_comment.len = tokeniser->context.pending = 0;
            tokeniser->state = STATE_BOGUS_COMMENT;
            return HUBBUB_OK;
        } else {
            return hubbub_error_from_parserutils_error(error);
    ...
    if (tokeniser->context.match_doctype.count == DOCTYPE_LEN) {
        ...
        memset(&tokeniser->context.current_doctype, 0, sizeof tokeniser->context.current_doctype);
...
hubbub_error hubbub_tokeniser_handle_match_cdata(hubbub_tokeniser *tokeniser)
{
    ...
    if (tokeniser->context.match_cdata.count == CDATA_LEN) {
        parserutils_inputstream_advance(tokeniser->input, tokeniser->context.match_cdata.count + len);
        tokeniser->context.pending = 0;
...
hubbub_error hubbub_tokeniser_handle_cdata_block(hubbub_tokeniser *tokeniser)
{
    ...
    if (c == ']' && (tokeniser->context.match_cdata.end == 0 || tokeniser->context.match_cdata.end == 1)) {
        tokeniser->context.pending += len;
...
hubbub_error hubbub_tokeniser_consume_character_reference(hubbub_tokeniser *tokeniser, size_t pos)
{
    ...
    error = parserutils_inputstream_peek(tokeniser->input, pos, &cptr, &len);
    ...
    error = parserutils_inputstream_peek(tokeniser->input, off, &cptr, &len);
...
hubbub_error hubbub_tokeniser_handle_numbered_entity(hubbub_tokeniser *tokeniser)
{
    ...
    error = parserutils_inputstream_peek(
        tokeniser->input, ctx->match_entity.offset + ctx->match_entity.length, &cptr, &len);
    ...
    while ((error = parserutils_inputstream_peek(tokeniser->input, ctx->match_entity.offset + ctx->match_entity.length,
                &cptr, &len)) == PARSERUTILS_OK) {
        uint8_t c = *cptr;

        if (ctx->match_entity.base == 10 && ('0' <= c && c <= '9')) {
            ctx->match_entity.had_data = true;
            ctx->match_entity.codepoint = ctx->match_entity.codepoint * 10 + (c - '0');

            ctx->match_entity.length += len;
        } else if (ctx->match_entity.base == 16 &&
            (('0' <= c && c <= '9') || ('A' <= (c & ~0x20) && (c & ~0x20) <= 'F'))) {
            ctx->match_entity.had_data = true;
            ...
            if ('0' <= c && c <= '9') {
                ...
                ctx->match_entity.codepoint += ((c & ~0x20) - 'A' + 10);
            }
    ...
    if (ctx->match_entity.had_data) {
        ...
        if (0x80 <= cp && cp <= 0x9F) {
            ...
            cp = 0x000A;
        } else if (ctx->match_entity.overflow || cp <= 0x0008 || cp == 0x000B || (0x000E <= cp && cp <= 0x001F) ||
            (0x007F <= cp && cp <= 0x009F) || (0xD800 <= cp && cp <= 0xDFFF) || (0xFDD0 <= cp && cp <= 0xFDEF) ||
            (cp & 0xFFFE) == 0xFFFE) {
...
hubbub_error hubbub_tokeniser_handle_named_entity(hubbub_tokeniser *tokeniser)
{
    ...
    while ((error = parserutils_inputstream_peek(tokeniser->input,
                ctx->match_entity.offset + ctx->match_entity.poss_length, &cptr, &len)) == PARSERUTILS_OK) {
        uint32_t cp;
        ...
        error = hubbub_entities_search_step(c, &cp, &ctx->match_entity.context);
        if (error == HUBBUB_OK) {
            /* Had a match - store it for later */
            ctx->match_entity.codepoint = cp;

            ctx->match_entity.length = ctx->match_entity.poss_length + len;
            ctx->match_entity.poss_length = ctx->match_entity.length;
    ...
    if (ctx->match_entity.length > 0) {
        uint8_t c;
        error = parserutils_inputstream_peek(
            tokeniser->input, ctx->match_entity.offset + ctx->match_entity.length - 1, &cptr, &len);
        ...
        if ((tokeniser->context.match_entity.return_state == STATE_CHARACTER_REFERENCE_IN_ATTRIBUTE_VALUE) &&
            c != ';') {
            error = parserutils_inputstream_peek(
                tokeniser->input, ctx->match_entity.offset + ctx->match_entity.length, &cptr, &len);
             ...
             * occur here. */
            assert(error == PARSERUTILS_OK || error == PARSERUTILS_EOF);
 ...
 * \return	true
 */
hubbub_error emit_character_token(hubbub_tokeniser *tokeniser, const hubbub_string *chars)
{
    hubbub_token token;
...
hubbub_error emit_current_tag(hubbub_tokeniser *tokeniser)
{
    ...
    if (token.type == HUBBUB_TOKEN_START_TAG) {
        ...
        if (token.data.tag.name.len < sizeof(tokeniser->context.last_start_tag_name)) {
            strncpy((char *)tokeniser->context.last_start_tag_name, (const char *)token.data.tag.name.ptr,
                token.data.tag.name.len);
            tokeniser->context.last_start_tag_len = token.data.tag.name.len;
        } else {
 ...
 * \return	true
 */
hubbub_error emit_current_doctype(hubbub_tokeniser *tokeniser, bool force_quirks)
{
    hubbub_token token;
    ...
    if (token.data.doctype.public_missing == false) {
        token.data.doctype.public_id.ptr = tokeniser->buffer->data + token.data.doctype.name.len;
    }

    if (token.data.doctype.system_missing == false) {
        token.data.doctype.system_id.ptr = tokeniser->buffer->data + token.data.doctype.name.len +
 ...
 */
hubbub_error hubbub_tokeniser_emit_token(hubbub_tokeniser *tokeniser, const hubbub_token *token)
{
    hubbub_error err = HUBBUB_OK;
    ...
    case HUBBUB_TOKEN_DOCTYPE:
        assert(memchr(token->data.doctype.name.ptr, 0xff, token->data.doctype.name.len) == NULL);
        if (token->data.doctype.public_missing == false)
            assert(memchr(token->data.doctype.public_id.ptr, 0xff, token->data.doctype.public_id.len) == NULL);
        if (token->data.doctype.system_missing == false)
            assert(memchr(token->data.doctype.system_id.ptr, 0xff, token->data.doctype.system_id.len) == NULL);
        break;
    case HUBBUB_TOKEN_START_TAG:
    case HUBBUB_TOKEN_END_TAG: {
        uint32_t i;
        assert(memchr(token->data.tag.name.ptr, 0xff, token->data.tag.name.len) == NULL);
    ...
    case HUBBUB_TOKEN_COMMENT:
        assert(memchr(token->data.comment.ptr, 0xff, token->data.comment.len) == NULL);
        break;
    case HUBBUB_TOKEN_CHARACTER:
        assert(memchr(token->data.character.ptr, 0xff, token->data.character.len) == NULL);
        break;
    ...
    if (tokeniser->buffer->length) {
        parserutils_buffer_discard(tokeniser->buffer, 0, tokeniser->buffer->length);
    }
    ...
    if (tokeniser->insert_buf->length > 0) {
        parserutils_inputstream_insert(tokeniser->input, tokeniser->insert_buf->data, tokeniser->insert_buf->length);
        parserutils_buffer_discard(tokeniser->insert_buf, 0, tokeniser->insert_buf->length);
    }
