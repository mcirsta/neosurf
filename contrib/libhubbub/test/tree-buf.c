...
static hubbub_parser *setup_parser(void)
{
    ...
    /* Don't enable scripting -- we want the same behaviour as NetSurf.
       ...
       HUBBUB_PARSER_ENABLE_SCRIPTING, &params) == HUBBUB_OK);
    */
...
int main(int argc, char **argv)
{
    ...
    for (i = 0; i < n_chunks; i++) {
        ...
        assert(hubbub_parser_parse_chunk(parser, (uint8_t *)buf, chunks[i]) == HUBBUB_OK);
    }
...
static void node_print(buf_t *buf, node_t *node, unsigned depth)
{
    ...
    case ELEMENT:
        ...
        qsort(node->data.element.attrs, node->data.element.n_attrs, sizeof *node->data.element.attrs, compare_attrs);
