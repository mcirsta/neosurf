...
int main(int argc, char **argv)
{
    ...
    assert(strcmp((char *)((json_object_get_object(json)->head)->k), "tests") == 0);

    /* Get array of tests */
    tests = json_object_get_array((struct json_object *)(json_object_get_object(json)->head)->v);

    for (i = 0; i < (int)array_list_length(tests); i++) {
        /* Get test */
        struct json_object *test = (struct json_object *)array_list_get_idx(tests, i);
        ...
        /* Extract settings */
        for (entry = json_object_get_object(test)->head; entry; entry = entry->next) {
            key = (char *)entry->k;
            ...
            if (strcmp(key, "description") == 0) {
                ...
                ctx.input = (const uint8_t *)json_object_get_string(val);
...
void run_test(context *ctx)
{
    ...
    for (i = 0; i < max_i; i++) {
        ...
        assert(parserutils_inputstream_create("UTF-8", 0, NULL, &stream) == PARSERUTILS_OK);
        ...
        if (ctx->last_start_tag != NULL) {
            ...
            assert(parserutils_inputstream_append(stream, buf, len - 1) == PARSERUTILS_OK);
        ...
        if (ctx->process_cdata) {
            ...
            assert(hubbub_tokeniser_setopt(tok, HUBBUB_TOKENISER_PROCESS_CDATA, &params) == HUBBUB_OK);
        }

        params.token_handler.handler = token_handler;
        params.token_handler.pw = ctx;
        assert(hubbub_tokeniser_setopt(tok, HUBBUB_TOKENISER_TOKEN_HANDLER, &params) == HUBBUB_OK);

        if (ctx->content_model == NULL) {
            params.content_model.model = HUBBUB_CONTENT_MODEL_PCDATA;
        } else {
            const char *cm = json_object_get_string((struct json_object *)array_list_get_idx(ctx->content_model, i));

            if (strcmp(cm, "PCDATA") == 0) {
                params.content_model.model = HUBBUB_CONTENT_MODEL_PCDATA;
            } else if (strcmp(cm, "RCDATA") == 0) {
                params.content_model.model = HUBBUB_CONTENT_MODEL_RCDATA;
        ...
        assert(hubbub_tokeniser_setopt(tok, HUBBUB_TOKENISER_CONTENT_MODEL, &params) == HUBBUB_OK);

        assert(parserutils_inputstream_append(stream, ctx->input, ctx->input_len) == PARSERUTILS_OK);

        assert(parserutils_inputstream_append(stream, NULL, 0) == PARSERUTILS_OK);

        printf("Input: '%.*s' (%d)\n", (int)ctx->input_len, (const char *)ctx->input, (int)ctx->input_len);
...
hubbub_error token_handler(const hubbub_token *token, void *pw)
{
    ...
    for (; ctx->output_index < (int)array_list_length(ctx->output); ctx->output_index++) {
        /* Get object for index */
        obj = (struct json_object *)array_list_get_idx(ctx->output, ctx->output_index);
    ...
    assert("too many tokens" &&
        (ctx->output_index < (int)array_list_length(ctx->output) || token->type == HUBBUB_TOKEN_EOF));
    ...
    printf("got %s: expected %s\n", token_names[token->type],
        json_object_get_string((struct json_object *)array_list_get_idx(items, 0)));

    /* Make sure we got the token we expected */
    assert(strcmp(token_names[token->type],
               json_object_get_string((struct json_object *)array_list_get_idx(items, 0))) == 0);

    switch (token->type) {
    case HUBBUB_TOKEN_DOCTYPE: {
        const char *expname = json_object_get_string(array_list_get_idx(items, 1));
        const char *exppub = json_object_get_string(array_list_get_idx(items, 2));
        const char *expsys = json_object_get_string(array_list_get_idx(items, 3));
        bool expquirks = !json_object_get_boolean(array_list_get_idx(items, 4));
        const char *gotname = (const char *)token->data.doctype.name.ptr;
        const char *gotpub, *gotsys;

        printf("'%.*s' %sids:\n", (int)token->data.doctype.name.len, gotname,
        ...
        if (token->data.doctype.public_missing) {
            ...
            printf("\tpublic: '%.*s' (%d)\n", (int)token->data.doctype.public_id.len, gotpub,
        ...
        if (token->data.doctype.system_missing) {
            ...
            printf("\tsystem: '%.*s' (%d)\n", (int)token->data.doctype.system_id.len, gotsys,
        ...
        assert((exppub == NULL) == (token->data.doctype.public_missing == true));
        if (exppub) {
            assert(token->data.doctype.public_id.len == strlen(exppub));
            assert(strncmp(gotpub, exppub, strlen(exppub)) == 0);
        }

        assert((expsys == NULL) == (token->data.doctype.system_missing == true));
        if (gotsys) {
    ...
    case HUBBUB_TOKEN_START_TAG: {
        const char *expname = json_object_get_string(array_list_get_idx(items, 1));
        struct lh_entry *expattrs = json_object_get_object(array_list_get_idx(items, 2))->head;
        bool self_closing = json_object_get_boolean(array_list_get_idx(items, 3));

        const char *tagname = (const char *)token->data.tag.name.ptr;

        printf("expected: '%s' %s\n", expname, (self_closing) ? "(self-closing) " : "");

        printf("     got: '%.*s' %s\n", (int)token->data.tag.name.len, tagname,
        ...
        assert((token->data.tag.n_attributes == 0) == (expattrs == NULL));

        assert(self_closing == token->data.tag.self_closing);

        for (i = 0; i < token->data.tag.n_attributes; i++) {
            char *expname = (char *)expattrs->k;
            const char *expval = json_object_get_string((struct json_object *)expattrs->v);
            const char *gotname = (const char *)token->data.tag.attributes[i].name.ptr;
            size_t namelen = token->data.tag.attributes[i].name.len;
            const char *gotval = (const char *)token->data.tag.attributes[i].value.ptr;
            size_t vallen = token->data.tag.attributes[i].value.len;

            printf("\t'%.*s' = '%.*s'\n", (int)namelen, gotname, (int)vallen, gotval);
    ...
    case HUBBUB_TOKEN_COMMENT: {
        const char *expstr = json_object_get_string(array_list_get_idx(items, 1));
    ...
    case HUBBUB_TOKEN_CHARACTER: {
        int expstrlen = json_object_get_string_len(array_list_get_idx(items, 1));
        const char *expstr = json_object_get_string(array_list_get_idx(items, 1));
        const char *gotstr = (const char *)token->data.character.ptr;
        size_t len = min(token->data.character.len, expstrlen - ctx->char_off);
        ...
        } else if (strlen(expstr + ctx->char_off) > token->data.character.len) {
