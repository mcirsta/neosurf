...
const char *const ns_names[NUM_NAMESPACES] = {NULL, NULL /*html*/, "math", "svg", "xlink", "xml", "xmlns"};
...
static hubbub_tree_handler tree_handler = {create_comment, create_doctype, create_element, create_text, ref_node,
    ...
    form_associate, add_attributes, set_quirks_mode, NULL, complete_script, NULL};
...
static hubbub_parser *setup_parser(void)
{
    ...
    assert(hubbub_parser_setopt(parser, HUBBUB_PARSER_TREE_HANDLER, &params) == HUBBUB_OK);

    params.document_node = (void *)1;
    assert(hubbub_parser_setopt(parser, HUBBUB_PARSER_DOCUMENT_NODE, &params) == HUBBUB_OK);

    params.enable_scripting = true;
    assert(hubbub_parser_setopt(parser, HUBBUB_PARSER_ENABLE_SCRIPTING, &params) == HUBBUB_OK);
        ...
        case READING_DATA_AFTER_FIRST:
            if (strcmp(line, "#errors\n") == 0) {
                ...
                if (state == READING_DATA_AFTER_FIRST) {
                    assert(hubbub_parser_parse_chunk(parser, (uint8_t *)"\n", 1) == HUBBUB_OK);
                } else {
                    state = READING_DATA_AFTER_FIRST;
                }

                printf(": %s", line);
                assert(hubbub_parser_parse_chunk(parser, (uint8_t *)line, len - 1) == HUBBUB_OK);
            }
            break;
...
hubbub_error create_doctype(void *ctx, const hubbub_doctype *doctype, void **result)
{
    ...
    node->type = DOCTYPE;
    node->data.doctype.name = strndup((const char *)doctype->name.ptr, doctype->name.len);

    if (!doctype->public_missing) {
        node->data.doctype.public_id = strndup((const char *)doctype->public_id.ptr, doctype->public_id.len);
    }

    if (!doctype->system_missing) {
        node->data.doctype.system_id = strndup((const char *)doctype->system_id.ptr, doctype->system_id.len);
    }
    node->refcnt = 1;
...
hubbub_error create_element(void *ctx, const hubbub_tag *tag, void **result)
{
    ...
    node->data.element.name = strndup((const char *)tag->name.ptr, tag->name.len);
    node->data.element.n_attrs = tag->n_attributes;

    node->data.element.attrs = calloc(node->data.element.n_attrs, sizeof *node->data.element.attrs);
    ...
    for (i = 0; i < tag->n_attributes; i++) {
        ...
        attr->name = strndup((const char *)tag->attributes[i].name.ptr, tag->attributes[i].name.len);

        attr->value = strndup((const char *)tag->attributes[i].value.ptr, tag->attributes[i].value.len);
    }
    node->refcnt = 1;
    ...
    if (insert) {
        ...
        if (tchild->type == CHARACTER && insert->type == CHARACTER) {
            insert->data.content = realloc(
                insert->data.content, strlen(insert->data.content) + strlen(tchild->data.content) + 1);
...
hubbub_error insert_before(void *ctx, void *parent, void *child, void *ref_child, void **result)
{
    ...
    if (tchild->type == CHARACTER && tref->prev && tref->prev->type == CHARACTER) {
...
hubbub_error clone_node(void *ctx, void *node, bool deep, void **result)
{
    ...
    case DOCTYPE:
        new_node->data.doctype.name = strdup(old_node->data.doctype.name);
        if (old_node->data.doctype.public_id)
            new_node->data.doctype.public_id = strdup(old_node->data.doctype.public_id);
        if (old_node->data.doctype.system_id)
            new_node->data.doctype.system_id = strdup(old_node->data.doctype.system_id);
        break;
    case COMMENT:
    case CHARACTER:
    ...
    case ELEMENT:
        ...
        new_node->data.element.name = strdup(old_node->data.element.name);
        new_node->data.element.attrs = calloc(old_node->data.element.n_attrs, sizeof *new_node->data.element.attrs);
        for (i = 0; i < old_node->data.element.n_attrs; i++) {
            ...
            attr->name = strdup(old_node->data.element.attrs[i].name);
            attr->value = strdup(old_node->data.element.attrs[i].value);
        }
    ...
    *result = new_node;
    ...
    new_node->child = new_node->parent = new_node->next = new_node->prev = NULL;
...
hubbub_error add_attributes(void *ctx, void *vnode, const hubbub_attribute *attributes, uint32_t n_attributes)
{
    ...
    node->data.element.attrs = realloc(
        node->data.element.attrs, node->data.element.n_attrs * sizeof *node->data.element.attrs);
    ...
    for (i = 0; i < n_attributes; i++) {
        ...
        attr->name = strndup((const char *)attributes[i].name.ptr, attributes[i].name.len);

        attr->value = strndup((const char *)attributes[i].value.ptr, attributes[i].value.len);
    }
...
static void node_print(buf_t *buf, node_t *node, unsigned depth)
{
    ...
    case DOCTYPE:
        ...
        if (node->data.doctype.public_id || node->data.doctype.system_id) {
    ...
    case ELEMENT:
        ...
        if (node->data.element.n_attrs > 0) {
            qsort(
                node->data.element.attrs, node->data.element.n_attrs, sizeof *node->data.element.attrs, compare_attrs);
        }
