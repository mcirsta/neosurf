...
{
    while ((*cursor) < textend) {
        if ((**cursor == 0x20) || (**cursor == 0x09) || (**cursor == 0x0A) || (**cursor == 0x0D) || (**cursor == ')')) {
            break;
        }
        (*cursor)++;
    }
...
static inline void advance_property_name(const char **cursor, const char *textend)
{
    while ((*cursor) < textend) {
        if (((**cursor < 0x30 /* 0 */) || (**cursor > 0x39 /* 9 */)) &&
            ...
            ((**cursor < 0x61 /* a */) || (**cursor > 0x7A /* z */)) && (**cursor != '-') && (**cursor != '_')) {
            break;
        }
        (*cursor)++;
    }
 ...
 */
static inline svgtiny_code parse_url_fragment(const char **text, const char *textend, const char **idout, int *idlenout)
{
 ...
 */
static svgtiny_code
element_from_url(const char **url, size_t urllen, struct svgtiny_parse_state *state, dom_element **element)
{
    svgtiny_code res;
    dom_exception exc;
...
apply_transform(enum transform_type transform, int paramc, float *paramv, struct svgtiny_transformation_matrix *tm)
{
    ...
    case TRANSFORM_ROTATE:
        ...
        if (paramc == 3) {
            e = -paramv[1] * cos(angle) + paramv[2] * sin(angle) + paramv[1];
            f = -paramv[1] * sin(angle) - paramv[2] * cos(angle) + paramv[2];
        } else if (paramc == 2) {
...
static inline svgtiny_code
parse_transform_function(const char **cursor, const char *textend, enum transform_type *transformout)
{
    const char *tokstart;
    size_t toklen;
    ...
    tokstart = *cursor;
    while ((*cursor) < textend) {
        if ((**cursor != 0x61 /* a */) && (**cursor != 0x65 /* e */) && (**cursor != 0x74 /* t */) &&
            (**cursor != 0x73 /* s */) && (**cursor != 0x72 /* r */) && (**cursor != 0x6B /* k */) &&
            (**cursor != 0x6C /* l */) && (**cursor != 0x77 /* w */) && (**cursor != 0x63 /* c */) &&
            (**cursor != 0x69 /* i */) && (**cursor != 0x6D /* m */) && (**cursor != 0x6E /* n */) &&
            (**cursor != 0x6F /* o */) && (**cursor != 0x78 /* x */) && (**cursor != 0x58 /* X */) &&
            (**cursor != 0x59 /* Y */)) {
            break;
        }
        (*cursor)++;
 ...
 */
static inline svgtiny_code
parse_transform_parameters(const char **cursor, const char *textend, int *paramc, float *paramv)
{
    int param_idx = 0;
    int param_max;
    const char *tokend;
 ...
 */
static inline svgtiny_code parse_hex_color(const char **cursor, const char *textend, svgtiny_colour *c)
{
 ...
 */
static inline svgtiny_code parse_color_function(const char **cursorout, const char *textend, svgtiny_colour *c)
{
    ...
    if (((cursor[0] != 'r') && (cursor[0] != 'R')) || ((cursor[1] != 'g') && (cursor[1] != 'G')) ||
    ...
    *cursorout = cursor;
    *c = svgtiny_RGB((unsigned int)argf[0], (unsigned int)argf[1], (unsigned int)argf[2]);
    return svgtiny_OK;
}
 ...
 */
static inline svgtiny_code parse_paint_url(const char **cursorout, const char *textend,
    struct svgtiny_parse_state_gradient *grad, struct svgtiny_parse_state *state, svgtiny_colour *c)
{
    const char *cursor = *cursorout;
    svgtiny_code res;
    ...
    if (((cursor[0] != 'u') && (cursor[0] != 'U')) || ((cursor[1] != 'r') && (cursor[1] != 'R')) ||
...
svgtiny_code svgtiny_parse_none(const char *cursor, const char *textend)
{
    ...
    if (cursor[0] != 'n' || cursor[1] != 'o' || cursor[2] != 'n' || cursor[3] != 'e') {
 ...
 *
 */
static svgtiny_code svgtiny_parse_paint(const char *text, size_t textlen, struct svgtiny_parse_state_gradient *grad,
    struct svgtiny_parse_state *state, svgtiny_colour *c)
{
 ...
 * parse an offset
 */
static svgtiny_code svgtiny_parse_offset(const char *text, size_t textlen, float *offset)
{
    svgtiny_code err;
    float number;
    const char *numend;
 ...
 */
static inline svgtiny_code dispatch_op(const char *value, size_t value_len, struct svgtiny_parse_state *state,
    struct svgtiny_parse_internal_operation *styleop)
{
    ...
    case SVGTIOP_PAINT:
        res = svgtiny_parse_paint(value, value_len, styleop->param, state, styleop->value);
        break;
 ...
 */
static inline svgtiny_code parse_declaration(const char *declaration, const char *end,
    struct svgtiny_parse_state *state, struct svgtiny_parse_internal_operation *styleops)
{
 ...
 *
 */
svgtiny_code svgtiny_parse_poly_points(const char *text, size_t textlen, float *pointv, unsigned int *pointc)
{
    const char *textend = text + textlen;
...
svgtiny_code svgtiny_parse_attributes(
    dom_element *node, struct svgtiny_parse_state *state, struct svgtiny_parse_internal_operation *styleops)
{
    struct svgtiny_parse_internal_operation *styleop;
    dom_string *attr;
    dom_exception exc;
    ...
    for (styleop = styleops; styleop->key != NULL; styleop++) {
        ...
        if (attr != NULL) {
            dispatch_op(dom_string_data(attr), dom_string_byte_length(attr), state, styleop);
            dom_string_unref(attr);
        }
    }
    return svgtiny_OK;
...
svgtiny_parse_element_from_href(dom_element *node, struct svgtiny_parse_state *state, dom_element **element)
{
    ...
    if (attr == NULL) {
        ...
        exc = dom_string_create_interned((const uint8_t *)XLINK_NS, strlen(XLINK_NS), &xmlns_xlink);
        if (exc != DOM_NO_ERR || xmlns_xlink == NULL) {
            return svgtiny_LIBDOM_ERROR;
        }

        /* attempt to get href attribute in xlink namespace */
        exc = dom_element_get_attribute_ns(node, xmlns_xlink, state->interned_href, &attr);
    ...
    res = element_from_url(&url, dom_string_byte_length(attr), state, element);
