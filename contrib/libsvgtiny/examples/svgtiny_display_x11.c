...
int main(int argc, char *argv[])
{
    ...
    if (code != svgtiny_OK) {
        ...
        case svgtiny_SVG_ERROR:
            fprintf(stderr, "svgtiny_SVG_ERROR: line %i: %s", diagram->error_line, diagram->error_message);
            break;
        default:
...
void gui_init(void)
{
    ...
    diagram_window = XCreateSimpleWindow(
        display, DefaultRootWindow(display), 0, 0, diagram->width, diagram->height, 0, 0, 0);

    update_window_title();

    XMapWindow(display, diagram_window);
    XSelectInput(display, diagram_window, KeyPressMask | ButtonPressMask | ExposureMask | StructureNotifyMask);
...
void update_window_title(void)
{
    ...
    snprintf(title, sizeof title, "%s (%i%%) - svgtiny", base_name, (int)roundf(scale * 100.0));
...
void gui_poll(void)
{
    ...
    case Expose:
        ...
        break;
    case ClientMessage:
        if (event.xclient.message_type == wm_protocols_atom && event.xclient.format == 32 &&
...
void event_diagram_expose(const XExposeEvent *expose_event)
{
    ...
    surface = cairo_xlib_surface_create(display, diagram_window, DefaultVisual(display, DefaultScreen(display)),
    ...
    for (i = 0; i != diagram->shape_count; i++) {
        if (diagram->shape[i].path) {
            ...
            cairo_set_source_rgb(cr, svgtiny_RED(diagram->shape[i].stroke) / 255.0,
                svgtiny_GREEN(diagram->shape[i].stroke) / 255.0, svgtiny_BLUE(diagram->shape[i].stroke) / 255.0);
            cairo_move_to(cr, scale * diagram->shape[i].text_x, scale * diagram->shape[i].text_y);
...
void render_path(cairo_t *cr, float scale, struct svgtiny_shape *path)
{
    ...
    for (j = 0; j != path->path_length;) {
        ...
        case svgtiny_PATH_MOVE:
            cairo_move_to(cr, scale * path->path[j + 1], scale * path->path[j + 2]);
            j += 3;
            break;
        ...
        case svgtiny_PATH_LINE:
            cairo_line_to(cr, scale * path->path[j + 1], scale * path->path[j + 2]);
            j += 3;
            break;
        case svgtiny_PATH_BEZIER:
            cairo_curve_to(cr, scale * path->path[j + 1], scale * path->path[j + 2], scale * path->path[j + 3],
                scale * path->path[j + 4], scale * path->path[j + 5], scale * path->path[j + 6]);
            j += 7;
            break;
        default:
            printf("error ");
    ...
    if (path->fill != svgtiny_TRANSPARENT) {
        cairo_set_source_rgb(
            cr, svgtiny_RED(path->fill) / 255.0, svgtiny_GREEN(path->fill) / 255.0, svgtiny_BLUE(path->fill) / 255.0);
        cairo_fill_preserve(cr);
    }
    if (path->stroke != svgtiny_TRANSPARENT) {
        cairo_set_source_rgb(cr, svgtiny_RED(path->stroke) / 255.0, svgtiny_GREEN(path->stroke) / 255.0,
