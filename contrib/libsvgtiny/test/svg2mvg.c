...
static void write_mvg(FILE *fh, float scale, struct svgtiny_diagram *diagram)
{
    ...
    for (i = 0; i != diagram->shape_count; i++) {
        ...
        fprintf(fh, "stroke-width %g ", scale * diagram->shape[i].stroke_width);

        if (diagram->shape[i].stroke_linecap_set) {
            const char *kw = "butt";
            switch (diagram->shape[i].stroke_linecap) {
            case svgtiny_CAP_ROUND:
                kw = "round";
                break;
            case svgtiny_CAP_SQUARE:
                kw = "square";
                break;
            case svgtiny_CAP_BUTT:
            default:
                kw = "butt";
                break;
            }
        ...
        if (diagram->shape[i].stroke_linejoin_set) {
            ...
            case svgtiny_JOIN_ROUND:
                kwj = "round";
                break;
            case svgtiny_JOIN_BEVEL:
                kwj = "bevel";
                break;
            case svgtiny_JOIN_MITER:
            default:
                kwj = "miter";
                break;
            }
        ...
        if (diagram->shape[i].stroke_dasharray_set && diagram->shape[i].stroke_dasharray_count > 0 &&
            diagram->shape[i].stroke_dasharray) {
            unsigned int di;
        ...
        if (diagram->shape[i].path) {
            ...
            for (j = 0; j != diagram->shape[i].path_length;) {
                ...
                case svgtiny_PATH_MOVE:
                    fprintf(
                        fh, "M %g %g ", scale * diagram->shape[i].path[j + 1], scale * diagram->shape[i].path[j + 2]);
                    j += 3;
                    break;
                ...
                case svgtiny_PATH_LINE:
                    fprintf(
                        fh, "L %g %g ", scale * diagram->shape[i].path[j + 1], scale * diagram->shape[i].path[j + 2]);
                    j += 3;
                    break;

                case svgtiny_PATH_BEZIER:
                    fprintf(fh, "C %g %g %g %g %g %g ", scale * diagram->shape[i].path[j + 1],
                        scale * diagram->shape[i].path[j + 2], scale * diagram->shape[i].path[j + 3],
                        scale * diagram->shape[i].path[j + 4], scale * diagram->shape[i].path[j + 5],
            ...
            fprintf(fh, "text %g %g '%s'", scale * diagram->shape[i].text_x, scale * diagram->shape[i].text_y,
