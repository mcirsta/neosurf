...
static void write_mvg(FILE *fh, float scale, struct svgtiny_diagram *diagram)
{
	...
	for (i = 0; i != diagram->shape_count; i++) {
		...
		if (diagram->shape[i].fill_opacity_set) {
			fprintf(fh,
				"fill-opacity %g ",
				diagram->shape[i].fill_opacity);
		}
		if (diagram->shape[i].fill_rule_set && diagram->shape[i].path) {
			if (diagram->shape[i].fill_rule ==
		...
		if (diagram->shape[i].stroke_miterlimit_set) {
			fprintf(fh,
				"stroke-miterlimit %g ",
				diagram->shape[i].stroke_miterlimit);
		}
		if (diagram->shape[i].stroke_dasharray_set &&
		    diagram->shape[i].stroke_dasharray_count > 0 &&
		    diagram->shape[i].stroke_dasharray) {
			unsigned int di;
			fprintf(fh, "stroke-dasharray ");
			for (di = 0;
			     di < diagram->shape[i].stroke_dasharray_count;
			     di++) {
				if (di != 0)
					fprintf(fh, ",");
				fprintf(fh,
					"%g",
					diagram->shape[i].stroke_dasharray[di]);
			}
			fprintf(fh, " ");
		}
		if (diagram->shape[i].stroke_dashoffset_set) {
			fprintf(fh,
				"stroke-dashoffset %g ",
				diagram->shape[i].stroke_dashoffset);
		}
