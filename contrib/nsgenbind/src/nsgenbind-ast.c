...
struct genbind_node *genbind_node_find_type(struct genbind_node *node,
					    ...
					    enum genbind_node_type type)
{
	return genbind_node_find(
		node, prev, genbind_cmp_node_type, (void *)type);
}
						  ...
						  const char *ident)
{
	...
	while (found_node != NULL) {
		...
		ident_node = genbind_node_find_type(genbind_node_getnode(
							    found_node),
						    NULL,
			 ...
			 enum genbind_method_type methodtype)
{
	...
	res_node = genbind_node_find_type(genbind_node_getnode(node),
					  prev,
	...
	while (res_node != NULL) {
		...
		type = (enum genbind_method_type *)genbind_node_getint(
			type_node);
		if (*type == methodtype) {
			break;
		}
			       ...
			       const char *ident)
{
	...
	while (res_node != NULL) {
		method_ident = genbind_node_gettext(
			genbind_node_find_type(genbind_node_getnode(res_node),
					       NULL,
					       GENBIND_NODE_TYPE_IDENT));

		if ((ident != NULL) && (method_ident != NULL) &&
...
FILE *genbindopen(const char *filename)
{
	...
	if (genfile != NULL) {
		...
		if (prevfilepath == NULL) {
			...
			if (slash && backslash) {
				fullname = (slash > backslash) ? slash
							       : backslash;
			} else if (slash) {
				fullname = slash;
			} else {
	...
	if (options->idlpath != NULL) {
		...
		fullname = malloc(fulllen);
		snprintf(
			fullname, fulllen, "%s/%s", options->idlpath, filename);
		genfile = fopen(fullname, "r");
