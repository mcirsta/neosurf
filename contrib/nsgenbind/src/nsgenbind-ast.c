...
{
    int count = 0;
    genbind_node_foreach_type(node, type, genbind_enumerate_node, &count);
    return count;
}
...
struct genbind_node *
genbind_node_find_type(struct genbind_node *node, struct genbind_node *prev, enum genbind_node_type type)
{
    return genbind_node_find(node, prev, genbind_cmp_node_type, (void *)type);
}

/* exported interface documented in nsgenbind-ast.h */
struct genbind_node *genbind_node_find_type_ident(
    struct genbind_node *node, struct genbind_node *prev, enum genbind_node_type type, const char *ident)
{
    ...
    while (found_node != NULL) {
        ...
        ident_node = genbind_node_find_type(genbind_node_getnode(found_node), NULL, GENBIND_NODE_TYPE_IDENT);
        ...
        while (ident_node != NULL) {
            ...
            ident_node = genbind_node_find_type(genbind_node_getnode(found_node), ident_node, GENBIND_NODE_TYPE_IDENT);
        }
...
genbind_node_find_method(struct genbind_node *node, struct genbind_node *prev, enum genbind_method_type methodtype)
{
    ...
    res_node = genbind_node_find_type(genbind_node_getnode(node), prev, GENBIND_NODE_TYPE_METHOD);
    while (res_node != NULL) {
        struct genbind_node *type_node;
        enum genbind_method_type *type;

        type_node = genbind_node_find_type(genbind_node_getnode(res_node), NULL, GENBIND_NODE_TYPE_METHOD_TYPE);
        ...
        res_node = genbind_node_find_type(genbind_node_getnode(node), res_node, GENBIND_NODE_TYPE_METHOD);
    }
...
struct genbind_node *genbind_node_find_method_ident(
    struct genbind_node *node, struct genbind_node *prev, enum genbind_method_type nodetype, const char *ident)
{
    struct genbind_node *res_node;
    char *method_ident;
    ...
    while (res_node != NULL) {
        method_ident = genbind_node_gettext(
            genbind_node_find_type(genbind_node_getnode(res_node), NULL, GENBIND_NODE_TYPE_IDENT));

        if ((ident != NULL) && (method_ident != NULL) && strcmp(ident, method_ident) == 0) {
            break;
        }
...
static int genbind_ast_dump(FILE *dfile, struct genbind_node *node, int indent)
{
    ...
    while (node != NULL) {
        fprintf(dfile, "%.*s%s", indent, SPACES, genbind_node_type_to_str(node->type));
        ...
        if (txt == NULL) {
            ...
            if (val == NULL) {
                fprintf(dfile, "\n");
                genbind_ast_dump(dfile, genbind_node_getnode(node), indent + 2);
            } else {
                fprintf(dfile, ": %d\n", *val);
            }
        } else {
...
void genbind_free_ast(struct genbind_node *node)
{
    ...
    while (node != NULL) {
        ...
        default:
            /* other types contain int or nothing in r, no freeing
             * needed */
            break;
        }
