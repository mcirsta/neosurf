...
static int enumerate_interface_type(struct webidl_node *interface_node, enum webidl_node_type node_type)
{
    int count = 0;
    struct webidl_node *members_node;

    members_node = webidl_node_find_type(webidl_node_getnode(interface_node), NULL, WEBIDL_NODE_TYPE_LIST);
    while (members_node != NULL) {
        count += webidl_node_enumerate_type(webidl_node_getnode(members_node), node_type);

        members_node = webidl_node_find_type(webidl_node_getnode(interface_node), members_node, WEBIDL_NODE_TYPE_LIST);
    }
 ...
 */
static void compute_inherit_refcount(struct ir_entry *entries, int entryc)
{
    int idx;
    int inf;
    ...
    for (idx = 0; idx < entryc; idx++) {
        ...
        for (inf = 0; inf < entryc; inf++) {
            ...
            if ((inf != idx) && (entries[idx].inherit_name != NULL) &&
                (strcmp(entries[idx].inherit_name, entries[inf].name) == 0)) {
 ...
 */
static struct ir_entry *entry_topoligical_sort(struct ir_entry *srcinf, int infc)
{
    ...
    for (idx = infc - 1; idx >= 0; idx--) {
        ...
        inf = 0;
        while ((inf < infc) && ((srcinf[inf].name == NULL) || (srcinf[inf].refcount > 0))) {
            inf++;
        }
        if (inf == infc) {
            free(dstinf);
...
find_operation_name(struct ir_operation_entry *operationv, int operationc, const char *name)
{
    ...
    for (opc = 0; opc < operationc; opc++) {
        ...
        if (cure->name == name) {
            ...
            if ((cure->name != NULL) && (name != NULL) && (strcmp(cure->name, name) == 0)) {
                return cure;
            }
        }
    }
...
static int argument_map_new(
    struct webidl_node *arg_list_node, int *argumentc_out, struct ir_operation_argument_entry **argumentv_out)
{
    int argumentc;
    ...
    argumentc = webidl_node_enumerate_type(webidl_node_getnode(arg_list_node), WEBIDL_NODE_TYPE_ARGUMENT);
    if (argumentc == 0) {
        *argumentc_out = 0;
        *argumentv_out = NULL;
        return 0;
    ...
    argument = webidl_node_find_type(webidl_node_getnode(arg_list_node), NULL, WEBIDL_NODE_TYPE_ARGUMENT);

    while (argument != NULL) {

        cure->name = webidl_node_gettext(
            webidl_node_find_type(webidl_node_getnode(argument), NULL, WEBIDL_NODE_TYPE_IDENT));

        cure->node = argument;

        cure->optionalc = webidl_node_enumerate_type(webidl_node_getnode(argument), WEBIDL_NODE_TYPE_OPTIONAL);

        cure->elipsisc = webidl_node_enumerate_type(webidl_node_getnode(argument), WEBIDL_NODE_TYPE_ELLIPSIS);

        cure++;

        argument = webidl_node_find_type(webidl_node_getnode(arg_list_node), argument, WEBIDL_NODE_TYPE_ARGUMENT);
    }
 ...
 */
static int
overload_map_new(struct webidl_node *op_node, int *overloadc_out, struct ir_operation_overload_entry **overloadv_out)
{
    ...
    /* return type */
    cure->type = webidl_node_find_type(webidl_node_getnode(op_node), NULL, WEBIDL_NODE_TYPE_TYPE);

    arg_list_node = webidl_node_find_type(webidl_node_getnode(op_node), NULL, WEBIDL_NODE_TYPE_LIST);
    if (arg_list_node != NULL) {
        argument_map_new(arg_list_node, &cure->argumentc, &cure->argumentv);
    }
...
static int operation_map_new(struct webidl_node *interface, struct genbind_node *class, int *operationc_out,
    struct ir_operation_entry **operationv_out)
{
    ...
    operationc = enumerate_interface_type(interface, WEBIDL_NODE_TYPE_OPERATION);
    ...
    operationv = calloc(operationc, sizeof(struct ir_operation_entry));
    if (operationv == NULL) {
        return -1;
    };
    cure = operationv;
    ...
    list_node = webidl_node_find_type(webidl_node_getnode(interface), NULL, WEBIDL_NODE_TYPE_LIST);

    while (list_node != NULL) {
        /* iterate through operations on list */
        op_node = webidl_node_find_type(webidl_node_getnode(list_node), NULL, WEBIDL_NODE_TYPE_OPERATION);
        ...
        while (op_node != NULL) {
            ...
            operation_name = webidl_node_gettext(
                webidl_node_find_type(webidl_node_getnode(op_node), NULL, WEBIDL_NODE_TYPE_IDENT));
            /* if this operation is already an entry in the list
             * augment that entry else create a new one
             */
            finde = find_operation_name(operationv, operationc, operation_name);
            if (finde == NULL) {
                ...
                cure->method = genbind_node_find_method_ident(class, NULL, GENBIND_METHOD_TYPE_METHOD, cure->name);

                overload_map_new(op_node, &cure->overloadc, &cure->overloadv);

                cure++; /* advance to next entry */
            } else {
                overload_map_new(op_node, &finde->overloadc, &finde->overloadv);
            ...
            op_node = webidl_node_find_type(webidl_node_getnode(list_node), op_node, WEBIDL_NODE_TYPE_OPERATION);
        }

        list_node = webidl_node_find_type(webidl_node_getnode(interface), list_node, WEBIDL_NODE_TYPE_LIST);
    }
 ...
 */
static char *get_extended_value(struct webidl_node *node, const char *key)
{
    char *ident;
    ...
    ext_attr = webidl_node_find_type(webidl_node_getnode(node), NULL, WEBIDL_NODE_TYPE_EXTENDED_ATTRIBUTE);
    while (ext_attr != NULL) {

        elem = webidl_node_find_type(webidl_node_getnode(ext_attr), NULL, WEBIDL_NODE_TYPE_IDENT);
        ident = webidl_node_gettext(elem);

        if ((ident != NULL) && (strcmp(ident, key) == 0)) {
            /* first identifier matches */

            elem = webidl_node_find_type(webidl_node_getnode(ext_attr), elem, WEBIDL_NODE_TYPE_IDENT);
            ident = webidl_node_gettext(elem);

            if ((ident != NULL) && (*ident == '=')) {
                return webidl_node_gettext(
                    webidl_node_find_type(webidl_node_getnode(ext_attr), elem, WEBIDL_NODE_TYPE_IDENT));
            }
        }

        ext_attr = webidl_node_find_type(webidl_node_getnode(node), ext_attr, WEBIDL_NODE_TYPE_EXTENDED_ATTRIBUTE);
    }
 ...
 */
static int type_map_new(struct webidl_node *node, int *typec_out, struct ir_type_entry **typev_out)
{
    int typec;
    ...
    typec = webidl_node_enumerate_type(webidl_node_getnode(node), WEBIDL_NODE_TYPE_TYPE);
    if (typec == 0) {
        *typec_out = 0;
        *typev_out = NULL;
        return 0;
    ...
    type_node = webidl_node_find_type(webidl_node_getnode(node), NULL, WEBIDL_NODE_TYPE_TYPE);
    ...
    while (type_node != NULL) {
        ...
        modifier = (enum webidl_type_modifier *)webidl_node_getint(
            webidl_node_find_type(webidl_node_getnode(type_node), NULL, WEBIDL_NODE_TYPE_MODIFIER));
        if (modifier != NULL) {
        ...
        /* type nullability */
        cure->nullable = (webidl_node_find_type(webidl_node_getnode(type_node), NULL, WEBIDL_NODE_TYPE_TYPE_NULLABLE) !=
            NULL);
        ...
        type_node = webidl_node_find_type(webidl_node_getnode(node), type_node, WEBIDL_NODE_TYPE_TYPE);
    }
 ...
 */
static int attribute_map_new(struct webidl_node *interface, struct genbind_node *class, int *attributec_out,
    struct ir_attribute_entry **attributev_out)
{
    ...
    attributec = enumerate_interface_type(interface, WEBIDL_NODE_TYPE_ATTRIBUTE);
    *attributec_out = attributec;
    ...
    while (list_node != NULL) {
        ...
        at_node = webidl_node_find_type(webidl_node_getnode(list_node), NULL, WEBIDL_NODE_TYPE_ATTRIBUTE);
        ...
        while (at_node != NULL) {
            ...
            cure->name = webidl_node_gettext(
                webidl_node_find_type(webidl_node_getnode(at_node), NULL, WEBIDL_NODE_TYPE_IDENT));

            cure->getter = genbind_node_find_method_ident(class, NULL, GENBIND_METHOD_TYPE_GETTER, cure->name);
            ...
            modifier = (enum webidl_type_modifier *)webidl_node_getint(
                webidl_node_find_type(webidl_node_getnode(at_node), NULL, WEBIDL_NODE_TYPE_MODIFIER));
            if ((modifier != NULL) && (*modifier == WEBIDL_TYPE_MODIFIER_READONLY)) {
                cure->modifier = WEBIDL_TYPE_MODIFIER_READONLY;
            } else {
                cure->modifier = WEBIDL_TYPE_MODIFIER_NONE;
                cure->setter = genbind_node_find_method_ident(class, NULL, GENBIND_METHOD_TYPE_SETTER, cure->name);
            }

            /* check for putforwards extended attribute */
            cure->putforwards = get_extended_value(at_node, "PutForwards");

            if ((cure->putforwards != NULL) && (cure->modifier != WEBIDL_TYPE_MODIFIER_READONLY)) {
                WARN(WARNING_WEBIDL, "putforwards on a writable attribute (%s) is prohibited", cure->name);
            }

            /* check for treatnullas extended attribute */
            cure->treatnullas = get_extended_value(at_node, "TreatNullAs");

            /* move to next attribute */
            cure++;

            at_node = webidl_node_find_type(webidl_node_getnode(list_node), at_node, WEBIDL_NODE_TYPE_ATTRIBUTE);
        }
...
static int constant_map_new(struct webidl_node *interface, int *constantc_out, struct ir_constant_entry **constantv_out)
{
    struct webidl_node *list_node;
    ...
    /* enumerate constants */
    constantc = enumerate_interface_type(interface, WEBIDL_NODE_TYPE_CONST);
    ...
    *constantc_out = constantc;
    ...
    constantv = calloc(constantc, sizeof(struct ir_constant_entry));
    if (constantv == NULL) {
        return -1;
    };
    cure = constantv;
    ...
    while (list_node != NULL) {
        ...
        constant_node = webidl_node_find_type(webidl_node_getnode(list_node), NULL, WEBIDL_NODE_TYPE_CONST);

        while (constant_node != NULL) {
            cure->node = constant_node;

            cure->name = webidl_node_gettext(
                webidl_node_find_type(webidl_node_getnode(constant_node), NULL, WEBIDL_NODE_TYPE_IDENT));

            cure++;

            /* move to next constant */
            constant_node = webidl_node_find_type(
                webidl_node_getnode(list_node), constant_node, WEBIDL_NODE_TYPE_CONST);
        }
...
static int
member_map_new(struct webidl_node *dictionary, int *memberc_out, struct ir_operation_argument_entry **memberv_out)
{
    struct webidl_node *list_node;
    ...
    /* enumerate members */
    memberc = enumerate_interface_type(dictionary, WEBIDL_NODE_TYPE_ARGUMENT);
    ...
    list_node = webidl_node_find_type(webidl_node_getnode(dictionary), NULL, WEBIDL_NODE_TYPE_LIST);

    while (list_node != NULL) {
        /* iterate through members on list */
        member_node = webidl_node_find_type(webidl_node_getnode(list_node), NULL, WEBIDL_NODE_TYPE_ARGUMENT);

        while (member_node != NULL) {
            cure->node = member_node;

            cure->name = webidl_node_gettext(
                webidl_node_find_type(webidl_node_getnode(member_node), NULL, WEBIDL_NODE_TYPE_IDENT));

            cure->optionalc = webidl_node_enumerate_type(webidl_node_getnode(member_node), WEBIDL_NODE_TYPE_OPTIONAL);

            cure->elipsisc = webidl_node_enumerate_type(webidl_node_getnode(member_node), WEBIDL_NODE_TYPE_ELLIPSIS);

            cure++;

            /* move to next member */
            member_node = webidl_node_find_type(webidl_node_getnode(list_node), member_node, WEBIDL_NODE_TYPE_ARGUMENT);
        }

        list_node = webidl_node_find_type(webidl_node_getnode(dictionary), list_node, WEBIDL_NODE_TYPE_LIST);
    }
...
static int entry_map_new(
    struct genbind_node *genbind, struct webidl_node *interface, int *interfacec_out, struct ir_entry **interfacev_out)
{
    int interfacec;
    int dictionaryc;
    int entryc;
    ...
    interfacec = webidl_node_enumerate_type(interface, WEBIDL_NODE_TYPE_INTERFACE);
    dictionaryc = webidl_node_enumerate_type(interface, WEBIDL_NODE_TYPE_DICTIONARY);
    if (options->verbose) {
    ...
    cure = entries;
    node = webidl_node_find_type(interface, NULL, WEBIDL_NODE_TYPE_INTERFACE);
    while (node != NULL) {
        ...
        cure->inherit_name = webidl_node_gettext(
            webidl_node_find_type(webidl_node_getnode(node), NULL, WEBIDL_NODE_TYPE_INHERITANCE));

        /* matching class from binding  */
        cure->class = genbind_node_find_type_ident(genbind, NULL, GENBIND_NODE_TYPE_CLASS, cure->name);
        ...
        if (webidl_node_find_type_ident(
                webidl_node_getnode(node), WEBIDL_NODE_TYPE_EXTENDED_ATTRIBUTE, "NoInterfaceObject") != NULL) {
        ...
        if (webidl_node_find_type_ident(
                webidl_node_getnode(node), WEBIDL_NODE_TYPE_EXTENDED_ATTRIBUTE, "PrimaryGlobal") != NULL) {
        ...
        operation_map_new(node, cure->class, &cure->u.interface.operationc, &cure->u.interface.operationv);

        /* enumerate and map the interface attributes */
        attribute_map_new(node, cure->class, &cure->u.interface.attributec, &cure->u.interface.attributev);

        /* enumerate and map the interface constants */
        constant_map_new(node, &cure->u.interface.constantc, &cure->u.interface.constantv);

        /* move to next interface */
        node = webidl_node_find_type(interface, node, WEBIDL_NODE_TYPE_INTERFACE);
        cure++;
    }

    /* for each dictionary populate an entry in the map */
    node = webidl_node_find_type(interface, NULL, WEBIDL_NODE_TYPE_DICTIONARY);
    while (node != NULL) {
        ...
        member_map_new(node, &cure->u.dictionary.memberc, &cure->u.dictionary.memberv);

        /* move to next interface */
        node = webidl_node_find_type(interface, node, WEBIDL_NODE_TYPE_DICTIONARY);
        cure++;
    }
...
int ir_new(struct genbind_node *genbind, struct webidl_node *webidl, struct ir **map_out)
{
    ...
    map->webidl = webidl;
    map->binding_node = genbind_node_find_type(genbind, NULL, GENBIND_NODE_TYPE_BINDING);

    /* interfaces */
    ret = entry_map_new(genbind, webidl, &map->entryc, &map->entries);
    if (ret != 0) {
        free(map);
        return ret;
    }
...
static int ir_dump_dictionary(FILE *dumpf, struct ir_entry *ecur)
{
    if (ecur->u.dictionary.memberc > 0) {
        ...
        for (argc = 0; argc < ecur->u.dictionary.memberc; argc++) {
            ...
            if (arge->optionalc != 0) {
                fprintf(dumpf, "\t\t\toptional:%d\n", arge->optionalc);
            }

            if (arge->elipsisc != 0) {
                fprintf(dumpf, "\t\t\telipsis:%d\n", arge->elipsisc);
            }
        }
    }
    return 0;
...
static int ir_dump_interface(FILE *dumpf, struct ir_entry *ecur)
{
    if (ecur->u.interface.operationc > 0) {
        ...
        for (opc = 0; opc < ecur->u.interface.operationc; opc++) {
            ...
            fprintf(dumpf, "\t\t%s\n", ope->name);
            fprintf(dumpf, "\t\t\tmethod:%p\n", ope->method);
            for (ovlc = 0; ovlc < ope->overloadc; ovlc++) {
                int argc;
                struct ir_operation_overload_entry *ovle;
                ovle = ope->overloadv + ovlc;

                fprintf(dumpf, "\t\t\toverload:%d\n", ovlc);

                fprintf(dumpf, "\t\t\t\treturn type:%p\n", ovle->type);

                fprintf(dumpf, "\t\t\t\targuments:%d\n", ovle->argumentc);

                fprintf(dumpf, "\t\t\t\toptionals:%d\n", ovle->optionalc);
    ...
    if (ecur->u.interface.attributec > 0) {
        ...
        while (attre != NULL) {
            fprintf(dumpf, "\t\t%s %p", attre->name, attre->getter);
...
int ir_dumpdot(struct ir *index)
{
    ...
    for (eidx = 0; eidx < index->entryc; eidx++) {
        ...
        if ((ecur == IR_ENTRY_TYPE_INTERFACE) && (ecur->u.interface.noobject == true)) {
...
struct ir_entry *ir_inherit_entry(struct ir *map, struct ir_entry *entry)
{
    ...
    if ((entry != NULL) && (entry->inherit_idx != -1)) {
...
static void ir_free_entry(struct ir_entry *entry)
{
    ...
    /* we don't free entry->class_name, it's not dynamically allocated in
     * ir.c */
    if (entry->class_name) {
