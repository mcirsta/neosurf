...
static int enumerate_interface_type(struct webidl_node *interface_node,
				    enum webidl_node_type node_type)
{
	int count = 0;
	struct webidl_node *members_node;

	members_node = webidl_node_find_type(webidl_node_getnode(
						     interface_node),
					     NULL,
					     WEBIDL_NODE_TYPE_LIST);
	while (members_node != NULL) {
		count += webidl_node_enumerate_type(
			webidl_node_getnode(members_node), node_type);
...
entry_topoligical_sort(struct ir_entry *srcinf, int infc)
{
	...
	for (idx = infc - 1; idx >= 0; idx--) {
		...
		inf = 0;
		while ((inf < infc) && ((srcinf[inf].name == NULL) ||
...
static int argument_map_new(struct webidl_node *arg_list_node,
			    int *argumentc_out,
			    struct ir_operation_argument_entry **argumentv_out)
{
	...
	argumentc = webidl_node_enumerate_type(
		webidl_node_getnode(arg_list_node), WEBIDL_NODE_TYPE_ARGUMENT);
	if (argumentc == 0) {
		*argumentc_out = 0;
		*argumentv_out = NULL;
		return 0;
	...
	while (argument != NULL) {
		...
		cure->name = webidl_node_gettext(
			webidl_node_find_type(webidl_node_getnode(argument),
					      NULL,
		...
		argument = webidl_node_find_type(webidl_node_getnode(
							 arg_list_node),
						 argument,
						 WEBIDL_NODE_TYPE_ARGUMENT);
	}
 ...
 * avoided.
 */
static int overload_map_new(struct webidl_node *op_node,
			    int *overloadc_out,
...
static int operation_map_new(struct webidl_node *interface,
			     ...
			     struct ir_operation_entry **operationv_out)
{
	...
	list_node = webidl_node_find_type(webidl_node_getnode(interface),
					  NULL,
					  WEBIDL_NODE_TYPE_LIST);

	while (list_node != NULL) {
		/* iterate through operations on list */
		op_node = webidl_node_find_type(webidl_node_getnode(list_node),
						NULL,
						WEBIDL_NODE_TYPE_OPERATION);
		...
		while (op_node != NULL) {
			...
			operation_name = webidl_node_gettext(
				webidl_node_find_type(webidl_node_getnode(
							      op_node),
						      NULL,
						      WEBIDL_NODE_TYPE_IDENT));
		...
		list_node = webidl_node_find_type(webidl_node_getnode(
							  interface),
						  list_node,
						  WEBIDL_NODE_TYPE_LIST);
	}
...
static char *get_extended_value(struct webidl_node *node, const char *key)
{
	...
	ext_attr = webidl_node_find_type(webidl_node_getnode(node),
					 NULL,
					 WEBIDL_NODE_TYPE_EXTENDED_ATTRIBUTE);
	while (ext_attr != NULL) {

		elem = webidl_node_find_type(webidl_node_getnode(ext_attr),
					     NULL,
					     WEBIDL_NODE_TYPE_IDENT);
		...
		if ((ident != NULL) && (strcmp(ident, key) == 0)) {
			...
			elem = webidl_node_find_type(webidl_node_getnode(
							     ext_attr),
						     elem,
						     WEBIDL_NODE_TYPE_IDENT);
			ident = webidl_node_gettext(elem);
 ...
 */
static int type_map_new(struct webidl_node *node,
			int *typec_out,
			struct ir_type_entry **typev_out)
{
	...
	typec = webidl_node_enumerate_type(webidl_node_getnode(node),
					   WEBIDL_NODE_TYPE_TYPE);
	if (typec == 0) {
		*typec_out = 0;
		*typev_out = NULL;
 ...
 */
static int attribute_map_new(struct webidl_node *interface,
			     struct genbind_node *class,
			     int *attributec_out,
			     struct ir_attribute_entry **attributev_out)
{
	...
	*attributec_out = attributec;
	...
	while (list_node != NULL) {
		...
		at_node = webidl_node_find_type(webidl_node_getnode(list_node),
						NULL,
						WEBIDL_NODE_TYPE_ATTRIBUTE);
		...
		while (at_node != NULL) {
			...
			cure->name = webidl_node_gettext(webidl_node_find_type(
			...
			modifier = (enum webidl_type_modifier *)
				webidl_node_getint(webidl_node_find_type(
					webidl_node_getnode(at_node),
					NULL,
					WEBIDL_NODE_TYPE_MODIFIER));
			if ((modifier != NULL) &&
...
static int constant_map_new(struct webidl_node *interface,
			    int *constantc_out,
			    struct ir_constant_entry **constantv_out)
{
	...
	*constantc_out = constantc;
	...
	while (list_node != NULL) {
		...
		constant_node = webidl_node_find_type(webidl_node_getnode(
							      list_node),
						      NULL,
...
static int member_map_new(struct webidl_node *dictionary,
			  int *memberc_out,
			  struct ir_operation_argument_entry **memberv_out)
{
	...
	list_node = webidl_node_find_type(webidl_node_getnode(dictionary),
					  NULL,
					  WEBIDL_NODE_TYPE_LIST);

	while (list_node != NULL) {
		/* iterate through members on list */
		member_node = webidl_node_find_type(webidl_node_getnode(
							    list_node),
						    NULL,
						    WEBIDL_NODE_TYPE_ARGUMENT);
...
static int entry_map_new(struct genbind_node *genbind,
			 ...
			 struct ir_entry **interfacev_out)
{
	...
	while (node != NULL) {
		...
		/* name of interface */
		cure->name = webidl_node_gettext(
			webidl_node_find_type(webidl_node_getnode(node),
					      NULL,
					      WEBIDL_NODE_TYPE_IDENT));
		...
		cure->class = genbind_node_find_type_ident(
			genbind, NULL, GENBIND_NODE_TYPE_CLASS, cure->name);
	   ...
	   struct ir **map_out)
{
	...
	map->webidl = webidl;
	map->binding_node = genbind_node_find_type(genbind,
						   NULL,
	...
	for (eidx = 0; eidx < index->entryc; eidx++) {
		if (index->entries[eidx].inherit_idx != -1) {
			fprintf(dumpf,
				"%04d -> %04d;\n",
				eidx,
				index->entries[eidx].inherit_idx);
		}
	}
...
static void ir_free_entry(struct ir_entry *entry)
{
	...
	case IR_ENTRY_TYPE_INTERFACE:
		...
		for (i = 0; i < entry->u.interface.operationc; i++) {
			struct ir_operation_entry *op =
				&entry->u.interface.operationv[i];
			int j;
			/* free overloads */
			for (j = 0; j < op->overloadc; j++) {
				struct ir_operation_overload_entry *ovl =
					&op->overloadv[j];
				/* free arguments */
				if (ovl->argumentv) {
					free(ovl->argumentv);
		...
		for (i = 0; i < entry->u.interface.attributec; i++) {
			struct ir_attribute_entry *attr =
				&entry->u.interface.attributev[i];
			if (attr->typev) {
				free(attr->typev);
			}
