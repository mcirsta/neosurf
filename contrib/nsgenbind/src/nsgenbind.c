...
static struct options *process_cmdline(int argc, char **argv)
{
    ...
    while ((opt = getopt(argc, argv, "vngDW::I:")) != -1) {
        ...
        default: /* '?' */
            fprintf(stderr, "Usage: %s [-v] [-g] [-D] [-W] [-I idlpath] inputfile outputdir\n", argv[0]);
            free(options);
            return NULL;
        }
    }
    ...
    if (optind > (argc - 2)) {
        fprintf(stderr, "Error: expected input filename and output directory\n");
        free(options);
        return NULL;
    }
...
static int genbind_load_idl(struct genbind_node *genbind, struct webidl_node **webidl_out)
{
    int res;
    struct genbind_node *binding_node;

    binding_node = genbind_node_find_type(genbind, NULL, GENBIND_NODE_TYPE_BINDING);

    /* walk AST and load any web IDL files required */
    res = genbind_node_foreach_type(
        genbind_node_getnode(binding_node), GENBIND_NODE_TYPE_WEBIDL, webidl_file_cb, webidl_out);
    if (res != 0) {
...
static enum bindingtype_e genbind_get_type(struct genbind_node *node)
{
    ...
    binding_node = genbind_node_find_type(node, NULL, GENBIND_NODE_TYPE_BINDING);
    ...
    binding_type = genbind_node_gettext(
        genbind_node_find_type(genbind_node_getnode(binding_node), NULL, GENBIND_NODE_TYPE_NAME));
