 ...
 * \todo tool should generate warning for interfaces with empty private structs
 ...
 */
static int output_private_header(struct ir *ir)
{
    int idx;
    struct opctx *privc;
    ...
    for (idx = 0; idx < ir->entryc; idx++) {
         ...
         * output
         */
        if ((interfacee->type == IR_ENTRY_TYPE_INTERFACE) && (interfacee->u.interface.noobject)) {
            continue;
        }

        switch (interfacee->type) {
        case IR_ENTRY_TYPE_INTERFACE:
            outputf(privc, "/* Private data for %s interface */\n", interfacee->name);
            break;

        case IR_ENTRY_TYPE_DICTIONARY:
            outputf(privc, "/* Private data for %s dictionary */\n", interfacee->name);
            break;
        }
        ...
        priv_node = genbind_node_find_type(genbind_node_getnode(interfacee->class), NULL, GENBIND_NODE_TYPE_PRIVATE);
        while (priv_node != NULL) {
            ...
            priv_node = genbind_node_find_type(
                genbind_node_getnode(interfacee->class), priv_node, GENBIND_NODE_TYPE_PRIVATE);
        }

        outputf(privc, "} __attribute__((aligned)) %s_private_t;\n\n", interfacee->class_name);
    }
 ...
 */
static int output_prototype_header(struct ir *ir)
{
    int idx;
    struct opctx *protoc;
 ...
 */
static int output_makefile(struct ir *ir)
{
    int idx;
    FILE *makef;
 ...
 */
static int output_binding_header(struct ir *ir)
{
    struct opctx *bindc;
    ...
    outputf(bindc, "duk_bool_t %s_instanceof(duk_context *ctx, duk_idx_t index, const char *klass);\n", DLPFX);

    outputf(bindc, "duk_ret_t %s_create_prototypes(duk_context *ctx);\n", DLPFX);
 ...
 * implementations.
 */
static int output_binding_src(struct ir *ir)
{
    int idx;
    ...
    outputf(bindc, "static duk_ret_t %s_create_prototype(duk_context *ctx,\n", DLPFX);
    outputf(bindc,
    ...
    for (idx = 0; idx < ir->entryc; idx++) {
        ...
        outputf(bindc, "\t%s_create_prototype(ctx, %s_%s___proto, \"%s\", \"%s\");\n", DLPFX, DLPFX,
            interfacee->class_name, proto_name, interfacee->name);
    ...
    if (pglobale != NULL) {
        ...
        outputf(bindc, "\t%s_create_prototype(ctx, %s_%s___proto, \"%s\", \"%s\");\n", DLPFX, DLPFX,
            pglobale->class_name, proto_name, pglobale->name);
        free(proto_name);
    }
    ...
    /* binding postface */
    output_method_cdata(bindc, ir->binding_node, GENBIND_METHOD_TYPE_POSTFACE);
...
int duk_libdom_output(struct ir *ir)
{
    ...
    for (idx = 0; idx < ir->entryc; idx++) {
        ...
        if (irentry->class_name != NULL) {
            ...
            snprintf(irentry->filename, ifacenamelen, "%s.c", irentry->class_name);
        }
    }
