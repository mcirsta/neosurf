...
int output_cdata(struct opctx *outc, struct genbind_node *node, enum genbind_node_type nodetype)
{
    ...
    cdata = genbind_node_gettext(genbind_node_find_type(genbind_node_getnode(node), NULL, nodetype));
    if (cdata != NULL) {
...
int output_ccode(struct opctx *outc, struct genbind_node *node)
{
    ...
    line = genbind_node_getint(genbind_node_find_type(genbind_node_getnode(node), NULL, GENBIND_NODE_TYPE_LINE));

    filename = genbind_node_gettext(genbind_node_find_type(genbind_node_getnode(node), NULL, GENBIND_NODE_TYPE_FILE));
...
int output_ctype(struct opctx *outc, struct genbind_node *node, bool identifier)
{
    ...
    typename_node = genbind_node_find_type(genbind_node_getnode(node), NULL, GENBIND_NODE_TYPE_NAME);
    while (typename_node != NULL) {
        type_cdata = genbind_node_gettext(typename_node);

        outputf(outc, "%s", type_cdata);

        typename_node = genbind_node_find_type(genbind_node_getnode(node), typename_node, GENBIND_NODE_TYPE_NAME);
    ...
    if (identifier) {
        if ((type_cdata != NULL) && (type_cdata[0] != '*') && (type_cdata[0] != ' ')) {
            outputc(outc, ' ');
        }
...
int output_method_cdata(struct opctx *outc, struct genbind_node *node, enum genbind_method_type sel_method_type)
{
    ...
    method = genbind_node_find_type(genbind_node_getnode(node), NULL, GENBIND_NODE_TYPE_METHOD);

    while (method != NULL) {
        enum genbind_method_type *method_type;

        method_type = (enum genbind_method_type *)genbind_node_getint(
            genbind_node_find_type(genbind_node_getnode(method), NULL, GENBIND_NODE_TYPE_METHOD_TYPE));
        if ((method_type != NULL) && (*method_type == sel_method_type)) {
            output_ccode(outc, method);
        }

        method = genbind_node_find_type(genbind_node_getnode(node), method, GENBIND_NODE_TYPE_METHOD);
    }
...
char *gen_idl2c_name(const char *idlname)
{
    ...
    *outc++ = tolower(*inc++);
    ...
    while (*inc != 0) {
         ...
         * an underscore there
         */
        if ((inc == (idlname + 4)) && (idlname[0] == 'H') && (idlname[1] == 'T') && (idlname[2] == 'M') &&
            (idlname[3] == 'L') && (islower(inc[1]) == 0)) {
            *outc++ = '_';
        }
        if (islower(*inc) != 0) {
            if (waslower == 0) {
                /* high to lower case transition */
                if (((outc - name) <= 3) || (*(outc - 3) != '_')) {
                    *outc = *(outc - 1);
                    *(outc - 1) = '_';
                    outc++;
                }
