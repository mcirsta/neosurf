		 ...
		 enum genbind_node_type nodetype)
{
	...
	cdata = genbind_node_gettext(genbind_node_find_type(
		genbind_node_getnode(node), NULL, nodetype));
	if (cdata != NULL) {
...
int output_ccode(struct opctx *outc, struct genbind_node *node)
{
	...
	line = genbind_node_getint(genbind_node_find_type(
		genbind_node_getnode(node), NULL, GENBIND_NODE_TYPE_LINE));

	filename = genbind_node_gettext(genbind_node_find_type(
...
int output_ctype(struct opctx *outc, struct genbind_node *node, bool identifier)
{
	...
	if (identifier) {
		if ((type_cdata != NULL) && (type_cdata[0] != '*') &&
		    (type_cdata[0] != ' ')) {
			outputc(outc, ' ');
		}
			...
			enum genbind_method_type sel_method_type)
{
	...
	while (method != NULL) {
		...
		method_type = (enum genbind_method_type *)genbind_node_getint(
			genbind_node_find_type(genbind_node_getnode(method),
					       NULL,
...
char *gen_idl2c_name(const char *idlname)
{
	...
	*outc++ = tolower(*inc++);
	...
	while (*inc != 0) {
		 ...
		 * an underscore there
		 */
		if ((inc == (idlname + 4)) && (idlname[0] == 'H') &&
		    (idlname[1] == 'T') && (idlname[2] == 'M') &&
