...
/* exported interface defined in webidl-ast.h */
struct webidl_node *
webidl_node_find(struct webidl_node *node, struct webidl_node *prev, webidl_callback_t *cb, void *ctx)
{
...
/* exported interface defined in webidl-ast.h */
struct webidl_node *
webidl_node_find_type(struct webidl_node *node, struct webidl_node *prev, enum webidl_node_type type)
{
...
/* exported interface defined in webidl-ast.h */
struct webidl_node *
webidl_node_find_type_ident(struct webidl_node *root_node, enum webidl_node_type type, const char *ident)
{
...
static int webidl_ast_dump_int(FILE *dumpf, struct webidl_node *node)
{
    ...
    case WEBIDL_NODE_TYPE_TYPE_BASE:
        fprintf(dumpf, ": %s\n", webidl_type_to_str(WEBIDL_TYPE_MODIFIER_NONE, node->r.number));
        break;
...
static int webidl_ast_dump(FILE *dumpf, struct webidl_node *node, int indent)
{
    ...
    char *txt;
    while (node != NULL) {
        fprintf(dumpf, "%.*s%s", indent, SPACES, webidl_node_type_to_str(node->type));
...
static struct webidl_node *webidl_node_clone(struct webidl_node *node)
{
    ...
    if (node->type == WEBIDL_NODE_TYPE_MODIFIER || node->type == WEBIDL_NODE_TYPE_TYPE_BASE ||
        node->type == WEBIDL_NODE_TYPE_LITERAL_INT || node->type == WEBIDL_NODE_TYPE_SPECIAL ||
...
static int implements_copy_nodes(struct webidl_node *src_node, struct webidl_node *dst_node)
{
...
static int intercalate_implements(struct webidl_node *interface_node, void *ctx)
{
    ...
    implements_node = webidl_node_find_type(
        webidl_node_getnode(interface_node), NULL, WEBIDL_NODE_TYPE_INTERFACE_IMPLEMENTS);
    while (implements_node != NULL) {

        implements_interface_node = webidl_node_find_type_ident(
            webidl_ast, WEBIDL_NODE_TYPE_INTERFACE, webidl_node_gettext(implements_node));
...
int webidl_intercalate_implements(struct webidl_node *webidl_ast)
{
    ...
    if (webidl_ast != NULL) {
         ...
         */
        res = webidl_node_for_each_type(webidl_ast, WEBIDL_NODE_TYPE_INTERFACE, intercalate_implements, webidl_ast);
    }
    return res;
}
