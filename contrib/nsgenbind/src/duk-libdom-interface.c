...
static bool compare_ctypes(struct genbind_node *a, struct genbind_node *b)
{
    ...
    ta = genbind_node_find_type(genbind_node_getnode(a), NULL, GENBIND_NODE_TYPE_NAME);
    tb = genbind_node_find_type(genbind_node_getnode(b), NULL, GENBIND_NODE_TYPE_NAME);
    ...
    while ((ta != NULL) && (tb != NULL)) {
        ...
        ta = genbind_node_find_type(genbind_node_getnode(a), ta, GENBIND_NODE_TYPE_NAME);
        tb = genbind_node_find_type(genbind_node_getnode(b), tb, GENBIND_NODE_TYPE_NAME);
    }
    if (ta != tb) {
        return false;
    }
 ...
 */
static int output_safe_get_private(struct opctx *outc, char *class_name, int idx)
{
    outputf(outc, "\t%s_private_t *priv;\n", class_name);
    outputf(outc, "\tduk_get_prop_string(ctx, %d, %s_magic_string_private);\n", idx, DLPFX);
...
static int output_get_prototype(struct opctx *outc, const char *interface_name)
{
    ...
    outputf(outc, "\t/* get prototype */\n");
    outputf(outc, "\tduk_get_global_string(ctx, %s_magic_string_prototypes);\n", DLPFX);
    outputf(outc, "\tduk_get_prop_string(ctx, -1, \"%s\");\n", proto_name);
    outputf(outc, "\tduk_replace(ctx, -2);\n");
...
static int output_set_destructor(struct opctx *outc, char *class_name, int idx)
{
    ...
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s___destructor, 1);\n", DLPFX, class_name);
 ...
 */
static int output_set_constructor(struct opctx *outc, char *class_name, int idx, int argc)
{
    outputf(outc, "\t/* Set the constructor */\n");
    outputf(outc, "\tduk_dup(ctx, %d);\n", idx);
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s___constructor, %d);\n", DLPFX, class_name, 1 + argc);
 ...
 */
static int output_dump_stack(struct opctx *outc)
{
    if (options->dbglog) {
        /* dump stack */
 ...
 */
static int output_add_method(struct opctx *outc, const char *class_name, const char *method)
{
    outputf(outc, "\t/* Add a method */\n");
    outputf(outc, "\tduk_dup(ctx, 0);\n");
    outputf(outc, "\tduk_push_string(ctx, \"%s\");\n", method);
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s_%s, DUK_VARARGS);\n", DLPFX, class_name, method);
    output_dump_stack(outc);
    outputf(outc, "\tduk_def_prop(ctx, -3,\n");
 ...
 */
static int output_populate_rw_property(struct opctx *outc, const char *class_name, const char *property)
{
    outputf(outc, "\t/* Add read/write property */\n");
    outputf(outc, "\tduk_dup(ctx, 0);\n");
    outputf(outc, "\tduk_push_string(ctx, \"%s\");\n", property);
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s_%s_getter, 0);\n", DLPFX, class_name, property);
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s_%s_setter, 1);\n", DLPFX, class_name, property);
    output_dump_stack(outc);
    outputf(outc, "\tduk_def_prop(ctx, -4, DUK_DEFPROP_HAVE_GETTER |\n");
 ...
 */
static int output_populate_ro_property(struct opctx *outc, const char *class_name, const char *property)
{
    ...
    outputf(outc, "\tduk_push_c_function(ctx, %s_%s_%s_getter, 0);\n", DLPFX, class_name, property);
    output_dump_stack(outc);
    outputf(outc, "\tduk_def_prop(ctx, -3, DUK_DEFPROP_HAVE_GETTER |\n");
    outputf(outc, "\t\tDUK_DEFPROP_HAVE_ENUMERABLE | DUK_DEFPROP_ENUMERABLE |\n");
    outputf(outc, "\t\tDUK_DEFPROP_HAVE_CONFIGURABLE);\n");
    outputf(outc, "\tduk_pop(ctx);\n\n");
 ...
 */
static int output_prototype_constant_int(struct opctx *outc, const char *constant_name, int value)
{
    outputf(outc, "\tduk_dup(ctx, 0);\n");
    outputf(outc, "\tduk_push_string(ctx, \"%s\");\n", constant_name);
    outputf(outc, "\tduk_push_int(ctx, %d);\n", value);
    outputf(outc, "\tduk_def_prop(ctx, -3, DUK_DEFPROP_HAVE_VALUE |\n");
    outputf(outc, "\t\t     DUK_DEFPROP_HAVE_WRITABLE |\n");
    outputf(outc, "\t\t     DUK_DEFPROP_HAVE_ENUMERABLE |\n");
    outputf(outc, "\t\t     DUK_DEFPROP_ENUMERABLE |\n");
    outputf(outc, "\t\t     DUK_DEFPROP_HAVE_CONFIGURABLE);\n");
 ...
 */
static int output_get_method_private(struct opctx *outc, char *class_name, bool is_global)
{
    outputf(outc, "\t/* Get private data for method */\n");
 ...
 */
static int output_interface_constructor(struct opctx *outc, struct ir_entry *interfacee)
{
    int init_argc;

    /* constructor definition */
    outputf(outc, "static duk_ret_t %s_%s___constructor(duk_context *ctx)\n", DLPFX, interfacee->class_name);
    outputf(outc, "{\n");

    output_create_private(outc, interfacee->class_name);

    /* generate call to initialisor */
    outputf(outc, "\t%s_%s___init(ctx, priv", DLPFX, interfacee->class_name);

    for (init_argc = 1; init_argc <= interfacee->class_init_argc; init_argc++) {
        switch (interfacee->class_init_argt[init_argc - 1]) {
        case IR_INIT_ARG_BOOL:
            outputf(outc, ", duk_get_bool(ctx, %d)", init_argc);
            break;
        case IR_INIT_ARG_UNSIGNED:
        case IR_INIT_ARG_INT:
            outputf(outc, ", duk_get_int(ctx, %d)", init_argc);
            break;
        case IR_INIT_ARG_POINTER:
            outputf(outc, ", duk_get_pointer(ctx, %d)", init_argc);
            break;
        }
    }
    outputf(outc, ");\n");
 ...
 */
static int output_interface_destructor(struct opctx *outc, struct ir_entry *interfacee)
{
    /* destructor definition */
    outputf(outc, "static duk_ret_t %s_%s___destructor(duk_context *ctx)\n", DLPFX, interfacee->class_name);
    outputf(outc, "{\n");

    output_safe_get_private(outc, interfacee->class_name, 0);

    /* generate call to finaliser */
    outputf(outc, "\t%s_%s___fini(ctx, priv);\n", DLPFX, interfacee->class_name);
 ...
 */
static int output_interface_inherit_init(struct opctx *outc, struct ir_entry *interfacee, struct ir_entry *inherite)
{
    ...
    init_node = genbind_node_find_method(interfacee->class, NULL, GENBIND_METHOD_TYPE_INIT);


    inh_init_node = genbind_node_find_method(inherite->class, NULL, GENBIND_METHOD_TYPE_INIT);


    outputf(outc, "\t%s_%s___init(ctx, &priv->parent", DLPFX, inherite->class_name);

    /* for each parameter in the parent find a matching named
     * parameter to pass and cast if necessary
     */

    inh_param_node = genbind_node_find_type(genbind_node_getnode(inh_init_node), NULL, GENBIND_NODE_TYPE_PARAMETER);
    while (inh_param_node != NULL) {
        char *param_name;
        param_name = genbind_node_gettext(
            genbind_node_find_type(genbind_node_getnode(inh_param_node), NULL, GENBIND_NODE_TYPE_IDENT));

        param_node = genbind_node_find_type_ident(
            genbind_node_getnode(init_node), NULL, GENBIND_NODE_TYPE_PARAMETER, param_name);
        if (param_node == NULL) {
            fprintf(stderr,
                "class \"%s\" (interface %s) parent class \"%s\" (interface %s) initialisor requires a parameter \"%s\" with compatible identifier\n",
                interfacee->class_name, interfacee->name, inherite->class_name, inherite->name, param_name);
            return -1;
        } else {
            outputf(outc, ", ");

            /* cast the parameter if required */
            if (compare_ctypes(param_node, inh_param_node) == false) {
                outputc(outc, '(');
        ...
        inh_param_node = genbind_node_find_type(
            genbind_node_getnode(inh_init_node), inh_param_node, GENBIND_NODE_TYPE_METHOD);
    }
...
static enum ir_init_argtype guess_argtype_from(struct genbind_node *param_node)
{
    ...
    typename_node = genbind_node_find_type(genbind_node_getnode(param_node), NULL, GENBIND_NODE_TYPE_NAME);
    while (typename_node != NULL) {
        ...
        typename_node = genbind_node_find_type(genbind_node_getnode(param_node), typename_node, GENBIND_NODE_TYPE_NAME);
    }
...
static int
output_interface_init_declaration(struct opctx *outc, struct ir_entry *interfacee, struct genbind_node *init_node)
{
    struct genbind_node *param_node;
    ...
    outputf(outc, "void %s_%s___init(duk_context *ctx, %s_private_t *priv", DLPFX, interfacee->class_name,
    ...
    param_node = genbind_node_find_type(genbind_node_getnode(init_node), NULL, GENBIND_NODE_TYPE_PARAMETER);
    while (param_node != NULL) {
        interfacee->class_init_argc++;
        interfacee->class_init_argt = realloc(
            interfacee->class_init_argt, interfacee->class_init_argc * sizeof(enum ir_init_argtype));
        interfacee->class_init_argt[interfacee->class_init_argc - 1] = guess_argtype_from(param_node);
        outputf(outc, ", ");

        output_ctype(outc, param_node, true);

        param_node = genbind_node_find_type(genbind_node_getnode(init_node), param_node, GENBIND_NODE_TYPE_PARAMETER);
    }
 ...
 */
static int output_interface_init(struct opctx *outc, struct ir_entry *interfacee, struct ir_entry *inherite)
{
    struct genbind_node *init_node;
    int res;
    ...
    if (options->dbglog) {
        outputf(outc, "\tNSLOG(dukky, INFO, \"Initialise %%p (priv=%%p)\", duk_get_heapptr(ctx, 0), priv);\n");
    }
 ...
 */
static int output_interface_fini(struct opctx *outc, struct ir_entry *interfacee, struct ir_entry *inherite)
{
    struct genbind_node *fini_node;

    /* find the finaliser method on the class (if any) */
    fini_node = genbind_node_find_method(interfacee->class, NULL, GENBIND_METHOD_TYPE_FINI);

    /* finaliser definition */
    if (interfacee->refcount == 0) {
        outputf(outc, "static ");
    }
    outputf(outc, "void %s_%s___fini(duk_context *ctx, %s_private_t *priv)\n", DLPFX, interfacee->class_name,
        interfacee->class_name);
    outputf(outc, "{\n");

    /* generate log statement */
    if (options->dbglog) {
        outputf(outc, "\tNSLOG(dukky, INFO, \"Finalise %%p\", duk_get_heapptr(ctx, 0));\n");
    }
    ...
    if (inherite != NULL) {
        outputf(outc, "\t%s_%s___fini(ctx, &priv->parent);\n", DLPFX, inherite->class_name);
    }
    outputf(outc, "}\n\n");
 ...
 */
static int
output_prototype_method(struct opctx *outc, struct ir_entry *interfacee, struct ir_operation_entry *operatione)
{

    if (operatione->name != NULL) {
        /* normal method on prototype */
        output_add_method(outc, interfacee->class_name, operatione->name);
    } else {
        /* special method on prototype */
        outputf(outc, "\t/* Special method on prototype - UNIMPLEMENTED */\n\n");
    }
 ...
 */
static int output_prototype_methods(struct opctx *outc, struct ir_entry *entry)
{
    int opc;
    int res = 0;

    for (opc = 0; opc < entry->u.interface.operationc; opc++) {
        res = output_prototype_method(outc, entry, entry->u.interface.operationv + opc);
        if (res != 0) {
            break;
        }
    }
...
static int
output_prototype_attribute(struct opctx *outc, struct ir_entry *interfacee, struct ir_attribute_entry *attributee)
{
    if ((attributee->putforwards == NULL) && (attributee->modifier == WEBIDL_TYPE_MODIFIER_READONLY)) {
        return output_populate_ro_property(outc, interfacee->class_name, attributee->name);
    }
    return output_populate_rw_property(outc, interfacee->class_name, attributee->name);
}
 ...
 */
static int output_prototype_attributes(struct opctx *outc, struct ir_entry *entry)
{
    int attrc;
    int res = 0;

    for (attrc = 0; attrc < entry->u.interface.attributec; attrc++) {
        res = output_prototype_attribute(outc, entry, entry->u.interface.attributev + attrc);
        if (res != 0) {
            break;
        }
    }
 ...
 */
static int output_prototype_constant(struct opctx *outc, struct ir_constant_entry *constante)
{
    int *value;

    value = webidl_node_getint(
        webidl_node_find_type(webidl_node_getnode(constante->node), NULL, WEBIDL_NODE_TYPE_LITERAL_INT));
 ...
 */
static int output_prototype_constants(struct opctx *outc, struct ir_entry *entry)
{
    int attrc;
    int res = 0;

    for (attrc = 0; attrc < entry->u.interface.constantc; attrc++) {
        res = output_prototype_constant(outc, entry->u.interface.constantv + attrc);
        if (res != 0) {
            break;
        }
    }
...
static int output_global_create_prototype(struct opctx *outc, struct ir *ir, struct ir_entry *interfacee)
{
    int idx;
    ...
    for (idx = 0; idx < ir->entryc; idx++) {
        ...
        if (entry->type == IR_ENTRY_TYPE_INTERFACE) {
            ...
            outputf(outc, "\tdukky_inject_not_ctr(ctx, 0, \"%s\");\n", entry->name);
        }
    }
    return 0;
}
 ...
 */
static int
output_interface_prototype(struct opctx *outc, struct ir *ir, struct ir_entry *interfacee, struct ir_entry *inherite)
{
    struct genbind_node *proto_node;

    /* find the prototype method on the class */
    proto_node = genbind_node_find_method(interfacee->class, NULL, GENBIND_METHOD_TYPE_PROTOTYPE);
    ...
    if (inherite != NULL) {
        outputf(outc, "\t/* Set this prototype's prototype (left-parent) */\n");
        output_get_prototype(outc, inherite->name);
        outputf(outc, "\tduk_set_prototype(ctx, 0);\n\n");
    }
    ...
    output_set_constructor(outc, interfacee->class_name, 0, interfacee->class_init_argc);
...
static int output_interface_elipsis_operation(
    struct opctx *outc, struct ir_entry *interfacee, struct ir_operation_entry *operatione)
{
    int cdatac; /* cdata blocks output */

    /* overloaded method definition */
    outputf(outc, "static duk_ret_t %s_%s_%s(duk_context *ctx)\n", DLPFX, interfacee->class_name, operatione->name);
    outputf(outc, "{\n");
     ...
     */
    WARN(WARNING_UNIMPLEMENTED, "Elipsis parameters not checked: method %s::%s();", interfacee->name, operatione->name);

    output_get_method_private(outc, interfacee->class_name, interfacee->u.interface.primary_global);

    cdatac = output_ccode(outc, operatione->method);
    if (cdatac == 0) {
        /* no implementation so generate default */
        WARN(WARNING_UNIMPLEMENTED, "Unimplemented: method %s::%s();", interfacee->name, operatione->name);
        outputf(outc, "\treturn 0;\n");
    }
 ...
 */
static int output_interface_overloaded_operation(
    struct opctx *outc, struct ir_entry *interfacee, struct ir_operation_entry *operatione)
{
    int cdatac; /* cdata blocks output */
 ...
 */
static int output_interface_special_operation(
    struct opctx *outc, struct ir_entry *interfacee, struct ir_operation_entry *operatione)
{
    /* special method definition */
    outputf(outc, "/* Special method definition - UNIMPLEMENTED */\n\n");

    WARN(WARNING_UNIMPLEMENTED, "Special operation on interface %s (operation entry %p)", interfacee->name, operatione);
 ...
 */
static int
output_operation_optional_defaults(struct opctx *outc, struct ir_operation_argument_entry *argumentv, int argumentc)
{
    int argc;
    for (argc = 0; argc < argumentc; argc++) {
        ...
        lit_node = webidl_node_getnode(
            webidl_node_find_type(webidl_node_getnode(cure->node), NULL, WEBIDL_NODE_TYPE_OPTIONAL));
        ...
        if (lit_node != NULL) {
            ...
            case WEBIDL_NODE_TYPE_LITERAL_BOOL:
                lit_int = webidl_node_getint(lit_node);
                outputf(outc, "\t\tduk_push_boolean(ctx, %d);\n", *lit_int);
                break;
...
static int output_operation_argument_type_check(struct opctx *outc, struct ir_entry *interfacee,
    struct ir_operation_entry *operatione, struct ir_operation_overload_entry *overloade, int argidx)
{
    ...
    type_node = webidl_node_find_type(webidl_node_getnode(argumente->node), NULL, WEBIDL_NODE_TYPE_TYPE);

    if (type_node == NULL) {
        fprintf(stderr, "%s:%s %dth argument %s has no type\n", interfacee->name, operatione->name, argidx,
            argumente->name);
        return -1;
    }

    argument_type = (enum webidl_type *)webidl_node_getint(
        webidl_node_find_type(webidl_node_getnode(type_node), NULL, WEBIDL_NODE_TYPE_TYPE_BASE));

    if (argument_type == NULL) {
        fprintf(stderr, "%s:%s %dth argument %s has no type base\n", interfacee->name, operatione->name, argidx,
            argumente->name);
        return -1;
    }
    ...
    default:
        outputf(outc, "\t\t/* unhandled type check */\n");
    }
 ...
 */
static int
output_interface_operation(struct opctx *outc, struct ir_entry *interfacee, struct ir_operation_entry *operatione)
{
    int cdatac; /* cdata blocks output */
    ...
    if (operatione->name == NULL) {
        return output_interface_special_operation(outc, interfacee, operatione);
    }

    if (operatione->overloadc != 1) {
        return output_interface_overloaded_operation(outc, interfacee, operatione);
    }

    if (operatione->overloadv->elipsisc != 0) {
        return output_interface_elipsis_operation(outc, interfacee, operatione);
    }
    ...
    if (fixedargc > 0) {
        outputf(outc, "if (%s_argc < %d) {\n", DLPFX, fixedargc);
        outputf(outc, "\t\t/* not enough arguments */\n");
        outputf(outc, "\t\treturn duk_error(ctx, DUK_RET_TYPE_ERROR, %s_error_fmt_argument, %d, %s_argc);\n", DLPFX,
            fixedargc, DLPFX);
        outputf(outc, "\t} else ");
    }

    for (optargc = fixedargc; optargc < overloade->argumentc; optargc++) {
        outputf(outc,
            "if (%s_argc == %d) {\n"
            "\t\t/* %d optional arguments need adding */\n",
            DLPFX, optargc, overloade->argumentc - optargc);
        output_operation_optional_defaults(outc, overloade->argumentv + optargc, overloade->argumentc - optargc);
        outputf(outc, "\t} else ");
    }
    ...
    outputf(outc,
        ...
        "\t\tduk_set_top(ctx, %d);\n"
        "\t}\n",
        DLPFX, overloade->argumentc, overloade->argumentc);
    outputf(outc, "\n");
    ...
    for (argidx = 0; argidx < overloade->argumentc; argidx++) {
        output_operation_argument_type_check(outc, interfacee, operatione, overloade, argidx);
    }
 ...
 */
static int output_interface_operations(struct opctx *outc, struct ir_entry *ife)
{
    int opc;
    int res = 0;

    for (opc = 0; opc < ife->u.interface.operationc; opc++) {
        res = output_interface_operation(outc, ife, ife->u.interface.operationv + opc);
        if (res != 0) {
            break;
        }
    }
 ...
 */
static int
output_attribute_getter(struct opctx *outc, struct ir_entry *interfacee, struct ir_attribute_entry *atributee)
{
    /* getter definition */
    outputf(
        outc, "static duk_ret_t %s_%s_%s_getter(duk_context *ctx)\n", DLPFX, interfacee->class_name, atributee->name);
    outputf(outc, "{\n");
    ...
    if (atributee->getter != NULL) {
        ...
        if (res == 0) {
            ...
            res = output_generated_attribute_getter(outc, interfacee, atributee);
        }
        if (res >= 0) {
            outputf(outc, "}\n\n");
            return res;
    ...
    if (atributee->typec == 0) {
        ...
        type_str = webidl_type_to_str(atributee->typev[0].modifier, atributee->typev[0].base);
    } else {
        type_str = "multiple";
    }

    WARN(WARNING_UNIMPLEMENTED, "Unimplemented: getter %s::%s(%s);", interfacee->name, atributee->name, type_str);
 ...
 */
static int
output_putforwards_setter(struct opctx *outc, struct ir_entry *interfacee, struct ir_attribute_entry *atributee)
{
    /* generate autogenerated putforwards */

    outputf(outc, "\tduk_ret_t get_ret;\n\n");

    outputf(outc, "\tget_ret = %s_%s_%s_getter(ctx);\n", DLPFX, interfacee->class_name, atributee->name);
    ...
    outputf(outc, "\tduk_put_prop_string(ctx, -2, \"%s\");\n\n", atributee->putforwards);
 ...
 */
static int
output_attribute_setter(struct opctx *outc, struct ir_entry *interfacee, struct ir_attribute_entry *atributee)
{
    int res = -1;

    /* setter definition */
    outputf(
        outc, "static duk_ret_t %s_%s_%s_setter(duk_context *ctx)\n", DLPFX, interfacee->class_name, atributee->name);
    outputf(outc, "{\n");
    ...
    if (atributee->setter != NULL) {
        ...
        if (res == 0) {
            ...
            res = output_generated_attribute_setter(outc, interfacee, atributee);
        }
    } else if (atributee->putforwards != NULL) {
        res = output_putforwards_setter(outc, interfacee, atributee);
    }
    ...
    if (res < 0) {
        ...
        WARN(WARNING_UNIMPLEMENTED, "Unimplemented: setter %s::%s(%s);", interfacee->name, atributee->name, type_str);
        if (options->dbglog) {
            outputf(outc, "\tNSLOG(dukky, WARNING, \"Unimplemented\");\n");
        }
 ...
 */
static int
output_interface_attribute(struct opctx *outc, struct ir_entry *interfacee, struct ir_attribute_entry *atributee)
{
    int res;
    ...
    if ((atributee->modifier != WEBIDL_TYPE_MODIFIER_READONLY) || (atributee->putforwards != NULL)) {
 ...
 */
static int output_interface_attributes(struct opctx *outc, struct ir_entry *ife)
{
    int attrc;

    for (attrc = 0; attrc < ife->u.interface.attributec; attrc++) {
        output_interface_attribute(outc, ife, ife->u.interface.attributev + attrc);
    }
...
int output_interface(struct ir *ir, struct ir_entry *interfacee)
{
    ...
    /* class preface */
    output_method_cdata(ifop, interfacee->class, GENBIND_METHOD_TYPE_PREFACE);
    ...
    /* class prologue */
    output_method_cdata(ifop, interfacee->class, GENBIND_METHOD_TYPE_PROLOGUE);
    ...
    /* class epilogue */
    output_method_cdata(ifop, interfacee->class, GENBIND_METHOD_TYPE_EPILOGUE);

    /* binding epilogue */
    output_method_cdata(ifop, ir->binding_node, GENBIND_METHOD_TYPE_EPILOGUE);

    /* class postface */
    output_method_cdata(ifop, interfacee->class, GENBIND_METHOD_TYPE_POSTFACE);

    /* binding postface */
    output_method_cdata(ifop, ir->binding_node, GENBIND_METHOD_TYPE_POSTFACE);
