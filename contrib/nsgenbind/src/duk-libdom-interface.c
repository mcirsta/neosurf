...
static char *get_prototype_name(const char *interface_name)
{
	...
	for (pnamelen--; pnamelen >= 0; pnamelen--) {
		proto_name[pnamelen + pfxlen] = toupper(
			interface_name[pnamelen]);
	}
	return proto_name;
}
 ...
 */
static int output_add_method(struct opctx *outc,
 ...
 */
static int output_populate_rw_property(struct opctx *outc,
				       const char *class_name,
				       const char *property)
{
 ...
 */
static int output_populate_ro_property(struct opctx *outc,
				       const char *class_name,
				       const char *property)
{
 ...
 */
static int output_prototype_constant_int(struct opctx *outc,
...
output_interface_constructor(struct opctx *outc, struct ir_entry *interfacee)
{
	...
	for (init_argc = 1; init_argc <= interfacee->class_init_argc;
	     init_argc++) {
 ...
 */
static int output_interface_inherit_init(struct opctx *outc,
					 ...
					 struct ir_entry *inherite)
{
	...
	inh_param_node = genbind_node_find_type(genbind_node_getnode(
							inh_init_node),
						NULL,
						GENBIND_NODE_TYPE_PARAMETER);
	while (inh_param_node != NULL) {
		char *param_name;
		param_name = genbind_node_gettext(genbind_node_find_type(
		...
		if (param_node == NULL) {
			...
			if (compare_ctypes(param_node, inh_param_node) ==
			    false) {
				outputc(outc, '(');
...
static enum ir_init_argtype guess_argtype_from(struct genbind_node *param_node)
{
	...
	while (typename_node != NULL) {
		...
		typename_node = genbind_node_find_type(genbind_node_getnode(
							       param_node),
						       typename_node,
						       GENBIND_NODE_TYPE_NAME);
	}
...
static int output_interface_init_declaration(struct opctx *outc,
					     struct ir_entry *interfacee,
					     struct genbind_node *init_node)
{
	struct genbind_node *param_node;
	...
	param_node = genbind_node_find_type(genbind_node_getnode(init_node),
					    NULL,
					    GENBIND_NODE_TYPE_PARAMETER);
 ...
 */
static int output_interface_init(struct opctx *outc,
				 struct ir_entry *interfacee,
				 struct ir_entry *inherite)
{
	struct genbind_node *init_node;
 ...
 */
static int output_interface_fini(struct opctx *outc,
				 struct ir_entry *interfacee,
				 struct ir_entry *inherite)
{
 ...
 */
static int output_prototype_method(struct opctx *outc,
				   struct ir_entry *interfacee,
...
static int output_prototype_methods(struct opctx *outc, struct ir_entry *entry)
{
	...
	for (opc = 0; opc < entry->u.interface.operationc; opc++) {
		res = output_prototype_method(
			outc, entry, entry->u.interface.operationv + opc);
		if (res != 0) {
			break;
		}
	}
...
static int output_prototype_attribute(struct opctx *outc,
				      struct ir_entry *interfacee,
...
output_prototype_attributes(struct opctx *outc, struct ir_entry *entry)
{
	...
	for (attrc = 0; attrc < entry->u.interface.attributec; attrc++) {
		res = output_prototype_attribute(
			outc, entry, entry->u.interface.attributev + attrc);
		if (res != 0) {
			break;
		}
	}
 ...
 */
static int output_prototype_constant(struct opctx *outc,
				     struct ir_constant_entry *constante)
{
	int *value;

	value = webidl_node_getint(
		webidl_node_find_type(webidl_node_getnode(constante->node),
				      NULL,
...
output_prototype_constants(struct opctx *outc, struct ir_entry *entry)
{
	...
	for (attrc = 0; attrc < entry->u.interface.constantc; attrc++) {
		res = output_prototype_constant(
			outc, entry->u.interface.constantv + attrc);
		if (res != 0) {
			break;
		}
	}
...
static int output_global_create_prototype(struct opctx *outc,
					  struct ir *ir,
					  struct ir_entry *interfacee)
{
	int idx;
 ...
 */
static int output_interface_prototype(struct opctx *outc,
				      struct ir *ir,
				      struct ir_entry *interfacee,
				      struct ir_entry *inherite)
{
	...
	output_set_constructor(
		outc, interfacee->class_name, 0, interfacee->class_init_argc);
	...
	int argumentc)
{
	...
	for (argc = 0; argc < argumentc; argc++) {
		...
		lit_node = webidl_node_getnode(
			webidl_node_find_type(webidl_node_getnode(cure->node),
					      NULL,
...
static int output_operation_argument_type_check(
	struct opctx *outc,
	struct ir_entry *interfacee,
	struct ir_operation_entry *operatione,
	...
	int argidx)
{
	...
	type_node = webidl_node_find_type(webidl_node_getnode(argumente->node),
					  NULL,
	...
	argument_type = (enum webidl_type *)webidl_node_getint(
		webidl_node_find_type(webidl_node_getnode(type_node),
				      NULL,
 ...
 */
static int output_interface_operation(struct opctx *outc,
				      struct ir_entry *interfacee,
				      struct ir_operation_entry *operatione)
{
	int cdatac; /* cdata blocks output */
	...
	for (argidx = 0; argidx < overloade->argumentc; argidx++) {
		output_operation_argument_type_check(
			outc, interfacee, operatione, overloade, argidx);
	}
...
static int output_interface_operations(struct opctx *outc, struct ir_entry *ife)
{
	...
	for (opc = 0; opc < ife->u.interface.operationc; opc++) {
		res = output_interface_operation(
			outc, ife, ife->u.interface.operationv + opc);
		if (res != 0) {
			break;
		}
	}
 ...
 */
static int output_attribute_getter(struct opctx *outc,
				   struct ir_entry *interfacee,
 ...
 */
static int output_putforwards_setter(struct opctx *outc,
				     struct ir_entry *interfacee,
				     struct ir_attribute_entry *atributee)
{
 ...
 */
static int output_attribute_setter(struct opctx *outc,
				   struct ir_entry *interfacee,
				   struct ir_attribute_entry *atributee)
{
	int res = -1;
 ...
 */
static int output_interface_attribute(struct opctx *outc,
				      struct ir_entry *interfacee,
				      struct ir_attribute_entry *atributee)
{
	int res;
...
static int output_interface_attributes(struct opctx *outc, struct ir_entry *ife)
{
	...
	for (attrc = 0; attrc < ife->u.interface.attributec; attrc++) {
		output_interface_attribute(outc,
					   ife,
