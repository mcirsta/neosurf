...
static lwc_error
lwc__intern(const char *s, size_t slen, lwc_string **ret, lwc_hasher hasher, lwc_strncmp compare, lwc_memcpy copy)
{
    lwc_hash h;
    lwc_hash bucket;
    lwc_string *str;
...
{
    return lwc__intern(s, slen, ret, lwc__calculate_hash, strncmp, (lwc_memcpy)memcpy);
}
...
static void *lwc__lcase_memcpy(void *restrict _target, const void *restrict _source, size_t n)
{
...
lwc_error lwc__intern_caseless_string(lwc_string *str)
{
    ...
    return lwc__intern(
        CSTR_OF(str), str->len, &(str->insensitive), lwc__calculate_lcase_hash, lwc__lcase_strncmp, lwc__lcase_memcpy);
}
