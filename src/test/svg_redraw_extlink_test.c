...
static nserror
cap_clip(const struct redraw_context *ctx, const struct rect *clip)
{
	(void)ctx;
	(void)clip;
	return NSERROR_OK;
...
static nserror cap_rectangle(const struct redraw_context *ctx,
...
{
	const char *svg_path =
		"../contrib/libsvgtiny/test/data/link-external-small-ltr-progressive.svg";
	...
	struct rect clip = {
		.x0 = -100000, .y0 = -100000, .x1 = 100000, .y1 = 100000};
	capture_t cap;
	...
	bool ok = svg_redraw_diagram(
		diagram, 0, 0, 1000, 1000, &clip, &ctx, 1.0f, 0xFFFFFF);
	ck_assert_msg(
		ok == true,
		"Expected svg_redraw_diagram to succeed; path_calls=%d max_len=%u",
		cap.path_calls,
		cap.max_len);
