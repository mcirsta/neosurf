...
static void *key_clone(void *key)
{
...
static uint32_t key_hash(void *key)
{
...
static void *value_alloc(void *key)
{
...
static bool hashmap_test_iterator_cb(void *key, void *value, void *ctx)
{
...
static void basic_fixture_create(void)
{
	corestring_create();
...
static void basic_fixture_teardown(void)
{
...
{
	hashmap_test_value_t *value = hashmap_insert(
		test_hashmap, corestring_nsurl_about_blank);
...
START_TEST(iterate_empty)
{
	...
	ck_assert(hashmap_iterate(test_hashmap,
				  hashmap_test_iterator_cb,
				  &iteration_ctx) == false);
 ...
 */
static case_pair chain_pairs[] = {{"https://www.google.com/", NULL},
...
static void chain_fixture_create(void)
{
...
static void chain_fixture_teardown(void)
{
	case_pair *chain_case = chain_pairs;
