...
static css_error
resolve_font(void *pw, lwc_string *name, css_system_font *system_font)
{
	return CSS_OK;
}
...
int main(int argc, char **argv)
{
	...
	if (css_stylesheet_append_data(
		    sheet, (const uint8_t *)css, strlen(css)) != CSS_OK) {
	...
	if (css_select_ctx_append_sheet(
		    select_ctx, sheet, CSS_ORIGIN_AUTHOR, NULL) != CSS_OK) {
