...
const char all_chars[] = "\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0a\x0b\x0c\x0d\x0e\x0f"
...
const char most_escaped_upper[] = "%01%02%03%04%05%06%07%08%09%0A%0B%0C%0D%0E%0F"
...
static const struct test_pairs url_unescape_test_vec[] = {
    ...
    {&simple_string[0], SLEN(simple_string), &simple_string[0], SLEN(simple_string)}, /* normal string with no percent
                                                                             encoded characters */
    {&most_escaped_upper[0], SLEN(most_escaped_upper), &all_chars[0],
        SLEN(all_chars)}, /* all characters that must be percent encoded */
    {&all_escaped_lower[0], SLEN(all_escaped_lower), &all_chars[0],
...
static TCase *url_unescape_case_create(void)
{
    ...
    tcase_add_loop_test(tc, url_unescape_simple_test, 0, NELEMS(url_unescape_test_vec));

    tcase_add_loop_test(tc, url_unescape_length_test, 0, NELEMS(url_unescape_test_vec));
