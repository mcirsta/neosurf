...
static nserror cap_polygon(const struct redraw_context *ctx,
...
START_TEST(test_borders_left_only_geometry)
{
    ...
    bool ok = html_redraw_borders(&b, 0, 0, /* p_width */ 100, /* p_height */ 50, &clip, 1.0f, &ctx);
    ...
    for (int i = 0; i < cap.rectangle_count; i++) {
        ...
        if (rr.x0 < minx) minx = rr.x0;
        if (rr.x1 < minx) minx = rr.x1;
        if (rr.x0 > maxx) maxx = rr.x0;
        if (rr.x1 > maxx) maxx = rr.x1;
        if (rr.y0 < miny) miny = rr.y0;
        if (rr.y1 < miny) miny = rr.y1;
        if (rr.y0 > maxy) maxy = rr.y0;
        if (rr.y1 > maxy) maxy = rr.y1;
    }

    /* Expected bounds for left border only: x in [b.x - left, b.x], y in [b.y, b.y + p_height] */
