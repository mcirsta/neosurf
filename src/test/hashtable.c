...
static void match_hashtable_create(void)
{
    ...
    for (idx = 0; idx < NELEMS(match_tests); idx++) {
        hash_add(match_hash_a, match_tests[idx].test, match_tests[idx].res);
        hash_add(match_hash_b, match_tests[idx].res, match_tests[idx].test);
    }
}
...
static Suite *hashtable_suite(void)
{
    ...
    tcase_add_checked_fixture(tc_match, match_hashtable_create, match_hashtable_teardown);

    tcase_add_loop_test(tc_match, hashtable_matcha_test, 0, NELEMS(match_tests));
    tcase_add_loop_test(tc_match, hashtable_matchb_test, 0, NELEMS(match_tests));
    ...
    tcase_add_checked_fixture(tc_dict_s, dicts_hashtable_create, dict_hashtable_teardown);
    ...
    tcase_add_checked_fixture(tc_dict_l, dictl_hashtable_create, dict_hashtable_teardown);
