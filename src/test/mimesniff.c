...
content_type content_factory_type_from_mime_type(lwc_string *mime_type)
{
    ...
    if (lwc_string_caseless_isequal(mime_type, corestring_lwc_image_gif, &match) == lwc_error_ok && match) {
        type = CONTENT_IMAGE;
    }
    if (lwc_string_caseless_isequal(mime_type, corestring_lwc_image_jpeg, &match) == lwc_error_ok && match) {
        type = CONTENT_IMAGE;
    }
    return type;
}
...
START_TEST(mimesniff_api_test)
{
    ...
    err = mimesniff_compute_effective_type(NULL, NULL, 0, false, false, &effective_type);
    ck_assert(err == NSERROR_NOT_FOUND);

    /* no header type, no data and sniffing allowed */
    err = mimesniff_compute_effective_type(NULL, NULL, 0, true, false, &effective_type);
...
START_TEST(mimesniff_match_unknown_exact_test)
{
    ...
    err = mimesniff_compute_effective_type(NULL, tst->data, tst->len, true, false, &effective_type);
    ck_assert(err == NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, *(tst->mime_type), &match) == lwc_error_ok && match);
...
static TCase *mimesniff_match_unknown_case_create(void)
{
    ...
    tcase_add_loop_test(tc, mimesniff_match_unknown_exact_test, 0, NELEMS(match_unknown_exact_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_riff_test, 0, NELEMS(match_unknown_riff_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_bom_test, 0, NELEMS(match_unknown_bom_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_ws_test, 0, NELEMS(match_unknown_ws_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_mp4_test, 0, NELEMS(match_unknown_mp4_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_bad_mp4_test, 0, NELEMS(match_unknown_bad_mp4_tests));

    tcase_add_loop_test(tc, mimesniff_match_unknown_txtbin_test, 0, NELEMS(match_unknown_txtbin_tests));
...
START_TEST(mimesniff_unparsable_header_test)
{
    ...
    err = mimesniff_compute_effective_type("badheader", NULL, 0, false, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_NOT_FOUND);

    /* unparsable header type, no data and sniffing allowed */
    err = mimesniff_compute_effective_type("badheader", NULL, 0, true, false, &effective_type);
...
START_TEST(mimesniff_parsable_header_nosniff_test)
{
    ...
    /* unparsable header type, no data and sniffing not allowed */
    err = mimesniff_compute_effective_type("text/plain", NULL, 0, false, false, &effective_type);
    ck_assert(err == NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, corestring_lwc_text_plain, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);
}
END_TEST
...
START_TEST(mimesniff_svg_header_sniff_imageonly_test)
{
    ...
    err = mimesniff_compute_effective_type("image/svg+xml", NULL, 0, true, true, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, corestring_lwc_image_svg, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);
}
END_TEST
...
START_TEST(mimesniff_image_header_sniff_imageonly_test)
{
    ...
    err = mimesniff_compute_effective_type("image/jpeg", NULL, 0, true, true, &effective_type);
    ck_assert_int_eq(err, NSERROR_NEED_DATA);

    /* svg header type, unsniffable data and sniffing allowed images only */
    err = mimesniff_compute_effective_type(
        "image/jpeg", (const uint8_t *)"notsniffablejpeg", 12, true, true, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, corestring_lwc_image_jpeg, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);

    /* svg header type, gif data and sniffing allowed images only */
    err = mimesniff_compute_effective_type("image/jpeg", (const uint8_t *)"GIF87a", 6, true, true, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, corestring_lwc_image_gif, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);
}
END_TEST
...
START_TEST(mimesniff_text_header_nodata_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("text/plain", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_NEED_DATA);

    /* svg header type, unsniffable data and sniffing allowed images only */
    err = mimesniff_compute_effective_type(
        "text/plain", (const uint8_t *)"a\nb\tc \x01", 7, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(
        lwc_string_caseless_isequal(effective_type, corestring_lwc_application_octet_stream, &match) == lwc_error_ok &&
        match);
    lwc_string_unref(effective_type);
}
END_TEST
...
START_TEST(mimesniff_text_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type(
        (const char *)tst->data, (const uint8_t *)"text", 4, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);
...
START_TEST(mimesniff_unknown_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("unknown/unknown", match_unknown_riff_tests[0].data,
        match_unknown_riff_tests[0].len, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(
        lwc_string_caseless_isequal(effective_type, *match_unknown_riff_tests[0].mime_type, &match) == lwc_error_ok &&
        match);
    lwc_string_unref(effective_type);

    /* unknown header type, sniffable data and sniffing allowed */
    err = mimesniff_compute_effective_type("application/unknown", match_unknown_riff_tests[1].data,
        match_unknown_riff_tests[1].len, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(
        lwc_string_caseless_isequal(effective_type, *match_unknown_riff_tests[1].mime_type, &match) == lwc_error_ok &&
        match);
    lwc_string_unref(effective_type);

    /* unknown header type, sniffable data and sniffing allowed */
    err = mimesniff_compute_effective_type(
        "*/*", match_unknown_riff_tests[0].data, match_unknown_riff_tests[0].len, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);
...
START_TEST(mimesniff_plusxml_header_sniff_test)
{
    ...
    /* unknown header type, sniffable data and sniffing allowed */
    err = mimesniff_compute_effective_type("image/svg+xml", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);
...
START_TEST(mimesniff_xml_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("text/xml", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, corestring_lwc_text_xml, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);

    err = mimesniff_compute_effective_type("application/xml", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(
        lwc_string_caseless_isequal(effective_type, corestring_lwc_application_xml, &match) == lwc_error_ok && match);
    lwc_string_unref(effective_type);
}
END_TEST
...
START_TEST(mimesniff_supported_image_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("image/gif", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_NEED_DATA);

    err = mimesniff_compute_effective_type(
        "image/gif", match_unknown_exact_tests[0].data, match_unknown_exact_tests[0].len, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);
...
START_TEST(mimesniff_html_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("text/html", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_NEED_DATA);
}
END_TEST
...
static struct test_mimetype text_html_header_tests[] = {
    ...
    SIG("<rdf:RDF xmlns:rdf=\"http://www.w3.org/1999/02/22-rdf-syntax-ns#\" xmlns=\"http://purl.org/rss/1.0\">",
...
START_TEST(mimesniff_text_html_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("text/html", tst->data, tst->len, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);
...
START_TEST(mimesniff_text_fancy_header_sniff_test)
{
    ...
    err = mimesniff_compute_effective_type("text/fancy", NULL, 0, true, false, &effective_type);
    ck_assert_int_eq(err, NSERROR_OK);

    ck_assert(lwc_string_caseless_isequal(effective_type, text_fancy, &match) == lwc_error_ok && match);
...
static TCase *mimesniff_header_case_create(void)
{
    ...
    tcase_add_loop_test(tc, mimesniff_text_header_sniff_test, 0, NELEMS(text_header_tests));
    ...
    tcase_add_loop_test(tc, mimesniff_text_html_header_sniff_test, 0, NELEMS(text_html_header_tests));
