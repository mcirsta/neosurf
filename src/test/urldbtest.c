...
static void verify_cookie(const char *url, const char *expected)
{
    char *res = test_urldb_get_cookie(url);
    if (expected) {
        ck_assert(res != NULL);
...
static void verify_cookie_present(const char *url)
{
    char *res = test_urldb_get_cookie(url);
    ck_assert(res != NULL);
    free(res);
...
START_TEST(urldb_original_test)
{
    ...
    free(urldb_get_cookie(url, true));
    nsurl_unref(url);
    ...
    verify_cookie_present("http://www.example.org/foo/bar/baz/quux.htm");

    /* Defaulted path with no non-leaf path segments */
    ck_assert(test_urldb_set_cookie("name=value\r\n", "http://no-non-leaf.example.org/index.html", NULL));
    verify_cookie_present("http://no-non-leaf.example.org/page2.html");
    verify_cookie_present("http://no-non-leaf.example.org/");
    ...
    verify_cookie_present("http://example.org/index.cgi");

    /* Valid path (includes leafname in non-root directory) */
    ck_assert(
        test_urldb_set_cookie("name=value;Path=/foo/index.html\r\n", "http://www.example.org/foo/index.html", NULL));
    /* Should _not_ match the above, as the leafnames differ */
    verify_cookie("http://www.example.org/foo/bar.html", NULL);
    ...
    verify_cookie("http://www.example.com/", "foo=value");
    ...
    verify_cookie("http://example.com/", "foo=\"hello\"; bar=bat; name=value");
    ...
    verify_cookie("http://expires.com/", NULL);
...
static const struct test_urls add_set_get_tests[] = {
    ...
    {"http://tree.example.com/this_url_has_a_ridiculously_long_path/made_up_from_a_number_of_inoranately_long_elments_some_of_well_over_forty/characters_in_length/the_whole_path_comes_out_well_in_excess_of_two_hundred_characters_in_length/this_is_intended_to_try_and_drive/the_serialisation_code_mad/foo.png",
        NULL, CONTENT_IMAGE, false},
    {"https://tree.example.com:8080/example.png", "fishy port       ", CONTENT_HTML, false},
    {"http://tree.example.com/bar.png", "\t     ", CONTENT_IMAGE, false}, /* silly title */
    {"http://[2001:db8:1f70::999:de8:7648:6e8]:100/", "ipv6 with port", CONTENT_TEXTPLAIN, false},
    {"file:///home/", NULL, CONTENT_HTML, false}, /* no title */
    {"http://foo@moose.com/", NULL, CONTENT_HTML, false}, /* Mantis bug #996 */
    {"http://a.xn--11b4c3d/a", "a title", CONTENT_HTML, false},
    {"https://smog.大众汽车/test", "unicode title 大众汽车", CONTENT_HTML, false},
};
...
static TCase *urldb_add_get_case_create(void)
{
    ...
    tcase_add_loop_test(tc, urldb_add_set_get_test, 0, NELEMS(add_set_get_tests));
...
START_TEST(urldb_iterate_partial_numeric_v6_test)
{
    ...
    cb_count = 0;
    urldb_iterate_partial("[2001:db8:1f70::999:de8:7648:6e8]", urldb_iterate_entries_cb);
    ck_assert_int_eq(cb_count, 0);
    ...
    cb_count = 0;
    urldb_iterate_partial("[2001:db8:1f70::999:de8:7648:6e8]/index.wrong", urldb_iterate_entries_cb);
    ck_assert_int_eq(cb_count, 0);
    ...
    cb_count = 0;
    urldb_iterate_partial("[2001:db8:1f70::999:de8:7648:6e8]/in", urldb_iterate_entries_cb);
    ck_assert_int_eq(cb_count, 1);

    /* double path separators are ignored */
    cb_count = 0;
    urldb_iterate_partial("[2001:db8:1f70::999:de8:7648:6e8]//index.html", urldb_iterate_entries_cb);
    ck_assert_int_eq(cb_count, 1);

    /* bad ipv6 address inet_pton should fail with this */
    cb_count = 0;
    urldb_iterate_partial("[2001::1f70::999::7648:8]", urldb_iterate_entries_cb);
    ck_assert_int_eq(cb_count, 0);
}
END_TEST
