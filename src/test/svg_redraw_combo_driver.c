...
static bool svg_plot_combo_diagram(const struct svgtiny_diagram *diagram,
				   int x,
				   int y,
				   int width,
				   int height,
...
#define BGR(c)                                                                 \
	...
	for (unsigned int i = 0; i != diagram->shape_count; i++) {
		if (diagram->shape[i].path) {
			...
			int px = (int)(diagram->shape[i].text_x * sx) +
				 transform[4];
			int py = (int)(diagram->shape[i].text_y * sy) +
				 transform[5];
...
bool svg_redraw_diagram(struct svgtiny_diagram *diagram,
			...
			colour background_colour)
{
	return svg_plot_combo_diagram(diagram,
				      x,
				      y,
				      width,
				      height,
