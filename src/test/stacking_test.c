...
START_TEST(stacking_context_add_many_test)
{
	...
	for (i = 0; i < 100; i++) {
		result = stacking_context_add(&ctx,
					      (struct box *)(uintptr_t)(0x1000 +
									i),
					      i - 50,
					      i * 10,
					      i * 20);
...
START_TEST(stacking_context_negative_zindex_test)
{
	...
	stacking_context_add(
		&ctx, (struct box *)0x1000, 10, 0, 0); /* positive */
	stacking_context_add(
		&ctx, (struct box *)0x2000, -5, 0, 0); /* negative */
	stacking_context_add(
		&ctx, (struct box *)0x3000, 5, 0, 0); /* positive */
	stacking_context_add(
		&ctx, (struct box *)0x4000, -100, 0, 0); /* negative */
	stacking_context_add(
		&ctx, (struct box *)0x5000, 0, 0, 0); /* zero (positive side) */
	stacking_context_add(
		&ctx, (struct box *)0x6000, -1, 0, 0); /* negative */
	...
	for (i = 0; i < (int)ctx.count; i++) {
		if (ctx.entries[i].z_index < 0) {
			...
			ck_assert_msg(
				!found_transition,
				"Negative z-index found after positive at index %d",
				i);
		} else {
			if (!found_transition) {
