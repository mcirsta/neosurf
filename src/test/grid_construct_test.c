...
static css_computed_style *MOCK_STYLE_BLOCK = (css_computed_style *)
	mock_style_block_storage;
static css_computed_style *MOCK_STYLE_GRID = (css_computed_style *)
	mock_style_grid_storage;
static uint8_t mock_style_inline_storage[4096];
static css_computed_style *MOCK_STYLE_INLINE = (css_computed_style *)
	mock_style_inline_storage;
...
{
	fprintf(stderr,
		"DEBUG: box_add_child parent=%p child=%p\n",
		parent,
		child);
	if (!parent || !child)
		return;
...
static struct neosurf_table mock_gui = {.misc = &mock_misc,
...
{
	fprintf(stderr,
		"DEBUG: box_complete_cb called with status=%d\n",
		status);
...
START_TEST(test_grid_construction)
{
	 ...
	 */
	fprintf(fp,
		"<html><body><div id=\"grid\"><div id=\"child\">Child</div></div></body></html>");
	fclose(fp);

	dom_document *doc = NULL;
	nserror err = libdom_parse_file("/tmp/ns_test_grid.html",
					"UTF-8",
					&doc);
...
#define INIT_STR(x, v) dom_string_create((const uint8_t *)(v), strlen(v), &x)
	...
	INIT_STR(corestring_dom___ns_key_box_node_data,
	...
	ctx->content = &htmlc;
	ctx->n = (dom_node *)
		root_el; /* Start construction at Root element (HTML) */
	ctx->root_box = NULL;
	...
	*/
	...
	while (htmlc.conversion_begun) {
		fprintf(stderr,
			"DEBUG: Calling convert for ctx->n=%p\n",
			ctx->n);
