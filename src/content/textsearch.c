 ...
 */
static inline void textsearch_broadcast(struct textsearch_context *textsearch, int type, bool state, const char *string)
{
    ...
    for (a = context->found->next; a; a = a->next) {
        ...
        if (add && !a->sel) {
            ...
            if (a->sel != NULL) {
                ...
                selection_set_position(a->sel, a->start_idx, a->end_idx);
            }
        }
    }
}
...
static nserror search_text(struct textsearch_context *context, const char *string, int string_len, search_flags_t flags)
{
    ...
    case_sensitive = ((flags & SEARCH_FLAG_CASE_SENSITIVE) != 0) ? true : false;
    ...
    if ((context->newsearch) || (context->prev_case_sens != case_sensitive)) {
        ...
        res = context->c->handler->textsearch_find(context->c, context, string, string_len, case_sensitive);
    ...
    /* update match state */
    textsearch_broadcast(context, CONTENT_TEXTSEARCH_MATCH, (context->current != NULL), NULL);

    search_show_all(showall, context);

    /* update back state */
    textsearch_broadcast(
        context, CONTENT_TEXTSEARCH_BACK, ((context->current != NULL) && (context->current->prev != NULL)), NULL);

    /* update forward state */
    textsearch_broadcast(
        context, CONTENT_TEXTSEARCH_FORWARD, ((context->current != NULL) && (context->current->next != NULL)), NULL);
    ...
    res = context->c->handler->textsearch_bounds(context->c, context->current->start_idx, context->current->end_idx,
        context->current->start_box, context->current->end_box, &bounds);
    if (res == NSERROR_OK) {
...
static nserror content_textsearch_step(struct textsearch_context *textsearch, search_flags_t flags, const char *string)
{
    ...
    textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_RECENT, false, string);
    ...
    if (i < string_len) {
        ...
        /* update match state */
        textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_MATCH, true, NULL);

        /* update back state */
        textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_BACK, false, NULL);

        /* update forward state */
        textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_FORWARD, false, NULL);
...
nserror content_textsearch_add_match(struct textsearch_context *context, unsigned start_idx, unsigned end_idx,
    struct box *start_box, struct box *end_box)
{
    ...
    unsigned *start_idx, unsigned *end_idx)
{
    ...
    for (cur = textsearch->found->next; cur != NULL; cur = cur->next) {
        if (cur->sel && selection_highlighted(cur->sel, start_offset, end_offset, start_idx, end_idx)) {
            return true;
        }
    }
...
nserror content_textsearch_destroy(struct textsearch_context *textsearch)
{
    ...
    if (textsearch->string != NULL) {
        /* broadcast recent query string */
        textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_RECENT, false, textsearch->string);
    ...
    /* update forward state */
    textsearch_broadcast(textsearch, CONTENT_TEXTSEARCH_FORWARD, true, NULL);
...
nserror content_textsearch(struct hlcache_handle *h, void *context, search_flags_t flags, const char *string)
{
    ...
    if (string != NULL && c->textsearch.string != NULL && c->textsearch.context != NULL &&
