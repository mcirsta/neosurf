 ...
 */
static nserror textplain_create(const content_handler *handler, lwc_string *imime_type,
    ...
    struct content **c)
{
    ...
    error = content__init(&text->base, handler, imime_type, params, llcache, fallback_charset, quirks);
    if (error != NSERROR_OK) {
        free(text);
        return error;
    }

    error = http_parameter_list_find_item(params, corestring_lwc_charset, &encoding);
    if (error != NSERROR_OK) {
...
static bool textplain_drain_input(textplain_content *c, parserutils_inputstream *stream, parserutils_error terminator)
{
    ...
    while (parserutils_inputstream_peek(stream, offset, &ch, &chlen) != terminator) {
...
static bool textplain_process_data(struct content *c, const char *data, unsigned int size)
{
    ...
    error = parserutils_inputstream_append(stream, (const uint8_t *)data, size);
    if (error != PARSERUTILS_OK) {
        goto no_memory;
    }
...
static float textplain_line_height(void)
{
     ...
     */
    return FIXTOFLT(
        FDIV((FMUL(FLTTOFIX(1.2), FMUL(nscss_screen_dpi, INTTOFIX((textplain_style.size / PLOT_STYLE_SCALE))))), F_72));
}
    ...
    while (i < utf8_data_size) {
        ...
        if (term || next_col >= columns) {
            if (line_count % 1024 == 0) {
                line1 = realloc(line, sizeof(struct textplain_line) * (line_count + 1024 + 3));
                if (!line1)
                    goto no_memory;
            ...
            if (term) {
                ...
                if (i + 1 < utf8_data_size && utf8_data[i + 1] != utf8_data[i] &&
                    (utf8_data[i + 1] == '\n' || utf8_data[i + 1] == '\r')) {
                    i++;
                }
            } else {
...
static size_t textplain_offset_from_coords(struct content *c, int x, int y, int dir)
{
    ...
    while (x > 0) {
        ...
        if (next_offset < length) {
            guit->layout->width(&textplain_style, text, next_offset, &width);
        }
        ...
        if (x <= width) {
            ...
            guit->layout->position(&textplain_style, text, next_offset, x, &char_offset, &pixel_offset);
 ...
 */
static nserror
textplain_mouse_action(struct content *c, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    textplain_content *text = (textplain_content *)c;
    ...
    if (selection_click(text->sel, text->bw, mouse, idx)) {
        ...
        if (selection_dragging(text->sel)) {
            browser_window_set_drag_type(bw, DRAGGING_SELECTION, NULL);
 ...
 */
static nserror
textplain_mouse_track(struct content *c, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    textplain_content *text = (textplain_content *)c;
 ...
 */
static bool text_draw(const char *utf8_text, size_t utf8_len, size_t offset, int x, int y, const struct rect *clip,
    int height, float scale, textplain_content *text, const struct selection *sel, const struct redraw_context *ctx)
{
    ...
    if (ctx->interactive == true) {
        ...
        if (!highlighted && (text->base.textsearch.context != NULL) &&
            content_textsearch_ishighlighted(
                text->base.textsearch.context, offset, offset + len, &start_idx, &end_idx)) {
            highlighted = true;
        }
        ...
        if (highlighted) {
            ...
            res = guit->layout->width(&textplain_style, utf8_text, start_idx, &startx);
            if (res != NSERROR_OK) {
                startx = 0;
            }

            res = guit->layout->width(&textplain_style, utf8_text, endtxt_idx, &endx);
            if (res != NSERROR_OK) {
                endx = 0;
            }
    ...
    struct content *c, struct content_redraw_data *data, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    for (lineno = line0; lineno != line1; lineno++) {
        ...
        while (offset < length) {
            ...
            if (!text_draw(text_d + offset, next_offset - offset, line[lineno].start + offset, tx,
                    y + (lineno * scaled_line_height), clip, line_height, data->scale, text, text->sel, ctx)) {
                return false;
            }
            ...
            res = guit->layout->width(&textplain_style, &text_d[offset], next_offset - offset, &width);
            ...
            if (bw) {
                ...
                if (selection_highlighted(sel, tab_ofst, tab_ofst + 1, &start_idx, &end_idx)) {
                    highlighted = true;
                }
                ...
                if (!highlighted && (c->textsearch.context != NULL)) {
                    ...
                    if (content_textsearch_ishighlighted(
                            c->textsearch.context, tab_ofst, tab_ofst + 1, &start_idx, &end_idx)) {
                        highlighted = true;
                    }
                }
                ...
                if (highlighted) {
                    ...
                    res = ctx->plot->rectangle(ctx, plot_style_highlight, &rect);
                    if (res != NSERROR_OK) {
                        return false;
                    }
                }
 ...
 */
static nserror
textplain_open(struct content *c, struct browser_window *bw, struct content *page, struct object_params *params)
{
    textplain_content *text = (textplain_content *)c;
 ...
 */
static nserror textplain_textsearch_find(
    struct content *c, struct textsearch_context *context, const char *pattern, int p_len, bool case_sens)
{
    ...
    for (line = 0; line < nlines; line++) {
        ...
        if (text) {
            while (length > 0) {
                ...
                res = content_textsearch_add_match(context, start_idx, start_idx + match_length, NULL, NULL);
                if (res != NSERROR_OK) {
                    return res;
                }
 ...
 */
static nserror textplain_textsearch_bounds(struct content *c, unsigned start_idx, unsigned end_idx,
    struct box *start_box, struct box *end_box, struct rect *bounds)
{
...
static nserror
textplain_textselection_copy(struct content *c, unsigned start_idx, unsigned end_idx, struct selection_string *selstr)
{
    const char *text;
    size_t length;
    bool res = false;
...
nserror textplain_init(void)
{
    ...
    lerror = lwc_intern_string("Windows-1252", SLEN("Windows-1252"), &textplain_default_charset);
