 ...
 */
static nserror nsjpegxl_create(const content_handler *handler, lwc_string *imime_type,
...
static bool nsjpegxl_convert(struct content *c)
{
    ...
    title = messages_get_buff(
        "JPEGXLTitle", nsurl_access_leaf(llcache_handle_get_url(c->llcache)), c->width, c->height);
    if (title != NULL) {
