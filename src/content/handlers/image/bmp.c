...
static nserror nsbmp_create(const struct content_handler *handler, lwc_string *imime_type,
    ...
    struct content **c)
{
    ...
    error = content__init(&bmp->base, handler, imime_type, params, llcache, fallback_charset, quirks);
    if (error != NSERROR_OK) {
        free(bmp);
        return error;
    }
...
static bool nsbmp_convert(struct content *c)
{
    ...
    /* set title text */
    title = messages_get_buff("BMPTitle", nsurl_access_leaf(llcache_handle_get_url(c->llcache)), c->width, c->height);
    if (title != NULL) {
    ...
    struct content *c, struct content_redraw_data *data, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    if (bmp->bmp->decoded == false) {
        ...
        if ((res != BMP_OK) && (res != BMP_INSUFFICIENT_DATA) && (res != BMP_DATA_ERROR)) {
            return false;
        }

        bitmap_format_to_client(bmp->bitmap,
            &(bitmap_fmt_t){
    ...
    return (ctx->plot->bitmap(ctx, bmp->bitmap, data->x, data->y, data->width, data->height, data->background_colour,
                flags) == NSERROR_OK);
}
...
static const char *nsbmp_types[] = {"application/bmp", "application/preview", "application/x-bmp",
    "application/x-win-bitmap", "image/bmp", "image/ms-bmp", "image/x-bitmap", "image/x-bmp", "image/x-ms-bmp",
    "image/x-win-bitmap", "image/x-windows-bmp", "image/x-xbitmap"};
