...
static nserror nssprite_create(const content_handler *handler, lwc_string *imime_type,
    ...
    struct content **c)
{
    ...
    error = content__init(&sprite->base, handler, imime_type, params, llcache, fallback_charset, quirks);
    if (error != NSERROR_OK) {
        free(sprite);
        return error;
    }
...
static bool nssprite_convert(struct content *c)
{
    ...
    /* set title text */
    title = messages_get_buff(
        "SpriteTitle", nsurl_access_leaf(llcache_handle_get_url(c->llcache)), c->width, c->height);
    if (title != NULL) {
    ...
    struct content *c, struct content_redraw_data *data, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    return (ctx->plot->bitmap(ctx, nssprite->bitmap, data->x, data->y, data->width, data->height,
