...
static void row_callback(png_structp png_s, png_bytep new_row, png_uint_32 row_num, int pass)
{
    ...
    if (png_c->interlace) {
        ...
        row_num = interlace_row_start[pass] + interlace_row_step[pass] * row_num;
...
static nserror nspng_create_png_data(nspng_content *png_c)
{
    ...
    png_set_progressive_read_fn(png_c->png, png_c, info_callback, row_callback, end_callback);

    return NSERROR_OK;
}

static nserror nspng_create(const content_handler *handler, lwc_string *imime_type, const struct http_parameter *params,
    struct llcache_handle *llcache, const char *fallback_charset, bool quirks, struct content **c)
{
    ...
    error = content__init(&png_c->base, handler, imime_type, params, llcache, fallback_charset, quirks);
    if (error != NSERROR_OK) {
        free(png_c);
        return error;
    }
...
static struct bitmap *png_cache_convert(struct content *c)
{
    ...
    png_cache_read_data.data = content__get_source_data(c, &png_cache_read_data.size);
...
static bool nspng_convert(struct content *c)
{
    ...
    /* set title text */
    title = messages_get_buff("PNGTitle", nsurl_access_leaf(llcache_handle_get_url(c->llcache)), c->width, c->height);
    if (title != NULL) {
...
static nserror nspng_clone(const struct content *old_c, struct content **new_c)
{
    ...
    if ((old_c->status == CONTENT_STATUS_READY) || (old_c->status == CONTENT_STATUS_DONE)) {
