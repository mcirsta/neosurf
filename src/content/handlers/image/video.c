...
static gboolean
nsvideo_bus_call(GstBus *bus, GstMessage *msg, nsvideo_content *video)
{
...
static void
nsvideo_need_data_event(GstElement *playbin, guint size, nsvideo_content *video)
{
}

static void
nsvideo_enough_data_event(GstElement *playbin, nsvideo_content *video)
{
}
				 ...
				 nsvideo_content *video)
{
	...
	g_signal_connect(video->appsrc,
			 "need-data",
			 G_CALLBACK(nsvideo_need_data_event),
			 video);
	g_signal_connect(video->appsrc,
			 "enough-data",
			 G_CALLBACK(nsvideo_enough_data_event),
			 video);
}
			      ...
			      struct content **c)
{
	...
	error = content__init(&video->base,
			      handler,
			      imime_type,
			      params,
			      llcache,
	...
	g_signal_connect(video->playbin,
			 "deep-notify::source",
			 G_CALLBACK(nsvideo_source_event),
			 video);
...
static bool
nsvideo_process_data(struct content *c, const char *data, unsigned int size)
{
