...
static nserror rsvg_create(const content_handler *handler, lwc_string *imime_type, const struct http_parameter *params,
...
static void rsvg__get_demensions(const rsvg_content *svgc, int *width, int *height)
{
    ...
    gotsize = rsvg_handle_get_intrinsic_size_in_pixels(svgc->rsvgh, &rwidth, &rheight);
    if (gotsize == TRUE) {
        *width = rwidth;
        *height = rheight;
    } else {
        ...
        rsvg_handle_get_geometry_for_element(svgc->rsvgh, NULL, &ink_rect, &logical_rect, NULL);
