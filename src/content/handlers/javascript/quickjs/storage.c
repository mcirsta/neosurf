...
static JSValue js_storage_getItem(JSContext *ctx,
				  ...
				  JSValueConst *argv)
{
	if (argc > 0) {
		...
		NSLOG(neosurf,
		      INFO,
		      "storage.getItem called with key: %s",
		      key);
...
static JSValue js_storage_setItem(JSContext *ctx,
				  JSValueConst this_val,
				  int argc,
				  JSValueConst *argv)
{
	if (argc >= 2) {
		...
		NSLOG(neosurf,
		      INFO,
		      "storage.setItem called with key: %s, value: %s",
		      key,
		      value);
		JS_FreeCString(ctx, key);
...
static JSValue js_storage_removeItem(JSContext *ctx,
				     JSValueConst this_val,
				     int argc,
				     JSValueConst *argv)
{
	if (argc > 0) {
		...
		NSLOG(neosurf,
		      INFO,
		      "storage.removeItem called with key: %s",
		      key);
		JS_FreeCString(ctx, key);
	}
	return JS_UNDEFINED;
...
static JSValue js_storage_clear(JSContext *ctx,
				JSValueConst this_val,
				int argc,
				JSValueConst *argv)
{
...
static JSValue js_storage_key(JSContext *ctx,
			      JSValueConst this_val,
			      int argc,
			      JSValueConst *argv)
{
...
static JSValue js_storage_length_getter(JSContext *ctx,
					JSValueConst this_val,
					int argc,
					JSValueConst *argv)
{
...
int qjs_init_storage(JSContext *ctx)
{
	...
	/* Create localStorage */
	JS_SetPropertyStr(
		ctx, global_obj, "localStorage", create_storage_object(ctx));

	/* Create sessionStorage */
	JS_SetPropertyStr(
		ctx, global_obj, "sessionStorage", create_storage_object(ctx));
