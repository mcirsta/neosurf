...
static JSValue js_addEventListener(JSContext *ctx,
...
static JSValue js_removeEventListener(JSContext *ctx,
				      JSValueConst this_val,
				      int argc,
				      JSValueConst *argv)
{
...
static JSValue js_dispatchEvent(JSContext *ctx,
				JSValueConst this_val,
				int argc,
				JSValueConst *argv)
{
...
int qjs_init_event_target(JSContext *ctx)
{
	...
	JS_SetPropertyStr(
		...
		global_obj,
		"dispatchEvent",
		JS_NewCFunction(ctx, js_dispatchEvent, "dispatchEvent", 1));
