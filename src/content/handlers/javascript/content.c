...
static nserror javascript_create(const content_handler *handler, lwc_string *imime_type,
    ...
    struct content **c)
{
    ...
    error = content__init(&script->base, handler, imime_type, params, llcache, fallback_charset, quirks);
    if (error != NSERROR_OK) {
        free(script);
        return error;
    }
