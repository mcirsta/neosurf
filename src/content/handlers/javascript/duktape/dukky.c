...
static duk_ret_t dukky_populate_object(duk_context *ctx, void *udata)
{
    ...
    if (duk_is_undefined(ctx, -1)) {
        NSLOG(dukky, WARNING, "Unable to find dukky prototype for `%s` - falling back to HTMLUnknownElement",
...
duk_ret_t dukky_create_object(duk_context *ctx, const char *name, int args)
{
    ...
    /* ... obj args name nargs */
    if ((ret = duk_safe_call(ctx, dukky_populate_object, NULL, args + 3, 1)) != DUK_EXEC_SUCCESS)
        return ret;
...
duk_bool_t dukky_push_node_stacked(duk_context *ctx)
{
    ...
    if (duk_is_undefined(ctx, -1)) {
        ...
        if (duk_safe_call(ctx, dukky_populate_object, NULL, 4, 1) != DUK_EXEC_SUCCESS) {
...
#define SET_HTML_CLASS(CLASS)                                                                                          \
    ...
    *html_class_len = SLEN(PROTO_NAME(HTML)) + SLEN(#CLASS) + SLEN("ELEMENT");

static void
dukky_html_element_class_from_tag_type(dom_html_element_type type, const char **html_class, size_t *html_class_len)
{
    switch (type) {
...
static void dukky_push_node_klass(duk_context *ctx, struct dom_node *node)
{
    ...
    case DOM_ELEMENT_NODE: {
        ...
        dukky_html_element_class_from_tag_type(type, &html_class, &html_class_len);
...
nserror js_newheap(int timeout, jsheap **heap)
{
    ...
    *heap = NULL;
    ...
    ctx = ret->ctx = duk_create_heap(dukky_alloc_function, dukky_realloc_function, dukky_free_function, ret, NULL);
    if (ret->ctx == NULL) {
        free(ret);
        return NSERROR_NOMEM;
    }
...
nserror js_newthread(jsheap *heap, void *win_priv, void *doc_priv, jsthread **thread)
{
    ...
    NSLOG(dukky, DEBUG, "New javascript/duktape thread, win_priv=%p, doc_priv=%p", win_priv, doc_priv);
    ...
    /* ..., polyfill.js */
    if (duk_pcompile_lstring_filename(CTX, DUK_COMPILE_EVAL, (const char *)polyfill_js, polyfill_js_len) != 0) {
    ...
    /* ..., generics.js */
    if (duk_pcompile_lstring_filename(CTX, DUK_COMPILE_EVAL, (const char *)generics_js, generics_js_len) != 0) {
        NSLOG(dukky, CRITICAL, "%s", duk_safe_to_string(CTX, -1));
...
#define JS_EXEC_TIMEOUT_MS 10000 /* 10 seconds */
     ...
     * start time.
     */
    return heap->exec_start_time != 0 && now > (heap->exec_start_time + JS_EXEC_TIMEOUT_MS);
}
...
bool js_exec(jsthread *thread, const uint8_t *txt, size_t txtlen, const char *name)
{
    ...
    if (duk_pcompile_lstring_filename(CTX, DUK_COMPILE_EVAL, (const char *)txt, txtlen) != 0) {
    ...
    NSLOG(dukky, DEEPDEBUG, "Returning %s", duk_get_boolean(CTX, 0) ? "true" : "false");
...
bool dukky_get_current_value_of_event_handler(duk_context *ctx, dom_string *name, dom_event_target *et)
{
    ...
    if (duk_is_undefined(ctx, -1)) {
        ...
        if (duk_pcompile(ctx, DUK_COMPILE_FUNCTION) != 0) {
            /* ... node err */
            NSLOG(dukky, DEBUG, "Unable to proceed with handler, could not compile");
            duk_pop_2(ctx);
            return false;
        }
        /* ... node handler */
...
static void dukky_generic_event_handler(dom_event *evt, void *pw)
{
    ...
    NSLOG(dukky, DEBUG, "Event's name is %*s", dom_string_length(name), dom_string_data(name));
    ...
    /* ... node */
    if (dukky_get_current_value_of_event_handler(ctx, name, (dom_event_target *)targ) == false) {
        /* ... */
        goto handle_extras;
    }
    /* ... handler node */
    ...
    if (duk_pcall_method(ctx, 1) != 0) {
        ...
        /* ... err */
        NSLOG(dukky, DEBUG, "OH NOES! An error running a callback.  Meh.");
        exc = dom_event_stop_immediate_propagation(evt);
        if (exc != DOM_NO_ERR)
            NSLOG(dukky, DEBUG, "WORSE! could not stop propagation");
        ...
        NSLOG(dukky, DEBUG, "Uncaught error in JS: %s: %s", duk_safe_to_string(ctx, -5), duk_safe_to_string(ctx, -4));
        NSLOG(
            dukky, INFO, "              was at: %s line %s", duk_safe_to_string(ctx, -3), duk_safe_to_string(ctx, -2));
        NSLOG(dukky, INFO, "         Stack trace: %s", duk_safe_to_string(ctx, -1));
...
handle_extras:
    ...
    while (duk_get_prop_index(ctx, -1, idx++)) {
        ...
        if (duk_pcall_method(ctx, 1) != 0) {
            ...
            /* ... err name message fileName lineNumber stack */
            NSLOG(
                dukky, DEBUG, "Uncaught error in JS: %s: %s", duk_safe_to_string(ctx, -5), duk_safe_to_string(ctx, -4));
            NSLOG(dukky, DEBUG, "              was at: %s line %s", duk_safe_to_string(ctx, -3),
                duk_safe_to_string(ctx, -2));
            NSLOG(dukky, DEBUG, "         Stack trace: %s", duk_safe_to_string(ctx, -1));
...
void dukky_register_event_listener_for(duk_context *ctx, struct dom_element *ele, dom_string *name, bool capture)
{
    ...
    exc = dom_event_listener_create(dukky_generic_event_handler, ctx, &listen);
    if (exc != DOM_NO_ERR)
        return;
    exc = dom_event_target_add_event_listener(ele, name, listen, capture);
    if (exc != DOM_NO_ERR) {
        NSLOG(dukky, DEBUG, "Unable to register listener for %p.%*s", ele, dom_string_length(name),
            dom_string_data(name));
    } else {
        NSLOG(dukky, DEBUG, "have registered listener for %p.%*s", ele, dom_string_length(name), dom_string_data(name));
    }
...
void js_handle_new_element(jsthread *thread, struct dom_element *node)
{
    ...
    for (idx = 0; idx < siz; idx++) {
        ...
        if (exc != DOM_NO_ERR)
            goto out;
        if (is_body &&
            (key == corestring_dom_onblur || key == corestring_dom_onerror || key == corestring_dom_onfocus ||
                key == corestring_dom_onload || key == corestring_dom_onresize || key == corestring_dom_onscroll)) {
        ...
        if (dom_string_length(key) > 2) {
            ...
            if (data[0] == 'o' && data[1] == 'n') {
                dom_string *sub = NULL;
                exc = dom_string_substr(key, 2, dom_string_length(key), &sub);
                if (exc == DOM_NO_ERR) {
                    dukky_register_event_listener_for(CTX, node, sub, false);
...
bool js_fire_event(jsthread *thread, const char *type, struct dom_document *doc, struct dom_node *target)
{
    ...
    if (duk_is_undefined(CTX, -1)) {
        ...
        /* ... handlers bodynode */
        if (dukky_get_current_value_of_event_handler(CTX, corestring_dom_load, body) == false) {
    ...
    if (duk_pcall_method(CTX, 1) != 0) {
        ...
        /* ... err */
        NSLOG(dukky, DEBUG, "OH NOES! An error running a handler.  Meh.");
        duk_get_prop_string(CTX, -1, "name");
        duk_get_prop_string(CTX, -2, "message");
        duk_get_prop_string(CTX, -3, "fileName");
        duk_get_prop_string(CTX, -4, "lineNumber");
        ...
        NSLOG(dukky, DEBUG, "Uncaught error in JS: %s: %s", duk_safe_to_string(CTX, -5), duk_safe_to_string(CTX, -4));
        NSLOG(
            dukky, DEBUG, "              was at: %s line %s", duk_safe_to_string(CTX, -3), duk_safe_to_string(CTX, -2));
        NSLOG(dukky, DEBUG, "         Stack trace: %s", duk_safe_to_string(CTX, -1));
