...
static void html_css_fetcher_poll(lwc_string *scheme)
{
    ...
    do {
        ...
        if (c->aborted) {
             ...
             */
            snprintf(header, sizeof header, "Content-Type: text/css; charset=utf-8");
            ...
            if (c->aborted == false) {
                snprintf(header, sizeof header, "Content-Length: %" PRIsizet, dom_string_byte_length(c->item->data));
                msg.type = FETCH_HEADER;
                msg.data.header_or_data.buf = (const uint8_t *)header;
                msg.data.header_or_data.len = strlen(header);
                html_css_fetcher_send_callback(&msg, c);
            ...
            if (c->aborted == false) {
                snprintf(header, sizeof header, "X-NS-Base: %.*s", (int)nsurl_length(c->item->base_url),
            ...
            if (c->aborted == false) {
                msg.type = FETCH_DATA;
                msg.data.header_or_data.buf = (const uint8_t *)dom_string_data(c->item->data);
                msg.data.header_or_data.len = dom_string_byte_length(c->item->data);
                html_css_fetcher_send_callback(&msg, c);
            }
...
{
    const struct fetcher_operation_table html_css_fetcher_ops = {.initialise = html_css_fetcher_initialise,
