...
static inline nserror plot_clipped_rectangle(
    const struct redraw_context *ctx, const plot_style_t *style, const struct rect *clip, struct rect *rect)
{
    nserror res;
 ...
 */
static nserror html_redraw_border_plot(const int side, const int *p, colour c, enum css_border_style_e style,
    int thickness, bool rectangular, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    case CSS_BORDER_STYLE_RIDGE:
        ...
        if ((rectangular || thickness == 2) && thickness != 1) {
            ...
            if (side == TOP || side == RIGHT) {
                ...
                rect.x1 = p[6];
                rect.y1 = p[7];
                rect.x1 = ((side == TOP) && (p[4] - p[6] != 0)) ? rect.x1 + p[4] - p[6] : rect.x1;
                ...
                rect.x1 = p[2];
                rect.y1 = p[3];
                rect.y1 = ((side == LEFT) && (p[1] - p[3] != 0)) ? rect.y1 + p[1] - p[3] : rect.y1;
                res = plot_clipped_rectangle(ctx, plot_style_bdr_out, clip, &rect);
            }
        } else {
    ...
    const struct rect *clip, float scale, const struct redraw_context *ctx)
{
    ...
    for (i = 0; i != 4; i++) {
        ...
        if (box->border[side].width == 0 || nscss_color_is_transparent(box->border[side].c)) {
            continue;
        }
        ...
        case LEFT:
            ...
            res = html_redraw_border_plot(side, z, col, box->border[side].style, box->border[side].width * scale,
                square_end_1 && square_end_2, clip, ctx);
            if (res != NSERROR_OK) {
                return false;
            }
            break;
        ...
        case TOP:
            ...
            if (box->border[TOP].style == CSS_BORDER_STYLE_SOLID && box->border[TOP].c == box->border[LEFT].c) {
            ...
            if (box->border[TOP].style == CSS_BORDER_STYLE_SOLID && box->border[TOP].c == box->border[RIGHT].c) {
        ...
        case BOTTOM:
            ...
            if (box->border[BOTTOM].style == CSS_BORDER_STYLE_SOLID && box->border[BOTTOM].c == box->border[LEFT].c) {
                /* don't bother overlapping left corner if
                 * it's the same colour anyway
                 */
                z[4] += left;
            ...
            if (box->border[BOTTOM].style == CSS_BORDER_STYLE_SOLID && box->border[BOTTOM].c == box->border[RIGHT].c) {
                /* don't bother overlapping right corner if
                 * it's the same colour anyway
                 */
                z[2] -= right;
        ...
        default:
            assert(side == TOP || side == BOTTOM || side == LEFT || side == RIGHT);
            break;
        }
    }
    ...
    bool last, const struct redraw_context *ctx)
{
    ...
    square_end_1 = (top == 0);
    square_end_2 = (bottom == 0);
    if (left != 0 && first && nscss_color_is_transparent(box->border[LEFT].c) == false) {
        ...
        res = html_redraw_border_plot(
            LEFT, z, col, box->border[LEFT].style, left, square_end_1 && square_end_2, clip, ctx);
        if (res != NSERROR_OK) {
            return false;
        }
    }
    ...
    square_end_1 = (top == 0);
    square_end_2 = (bottom == 0);
    if (right != 0 && last && nscss_color_is_transparent(box->border[RIGHT].c) == false) {
        ...
        res = html_redraw_border_plot(
            RIGHT, z, col, box->border[RIGHT].style, right, square_end_1 && square_end_2, clip, ctx);
        if (res != NSERROR_OK) {
            return false;
        }
    }
    ...
    square_end_1 = (left == 0);
    square_end_2 = (right == 0);
    if (top != 0 && nscss_color_is_transparent(box->border[TOP].c) == false) {
        ...
        if (first && box->border[TOP].style == CSS_BORDER_STYLE_SOLID && box->border[TOP].c == box->border[LEFT].c) {
            /* don't bother overlapping left corner if
             * it's the same colour anyway
             */
            z[2] += left;
        ...
        if (last && box->border[TOP].style == CSS_BORDER_STYLE_SOLID && box->border[TOP].c == box->border[RIGHT].c) {
            /* don't bother overlapping right corner if
             * it's the same colour anyway
             */
            z[4] -= right;
        ...
        res = html_redraw_border_plot(
            TOP, z, col, box->border[TOP].style, top, square_end_1 && square_end_2, clip, ctx);
        if (res != NSERROR_OK) {
            return false;
        }
    }
    ...
    square_end_1 = (left == 0);
    square_end_2 = (right == 0);
    if (bottom != 0 && nscss_color_is_transparent(box->border[BOTTOM].c) == false) {
        ...
        if (first && box->border[BOTTOM].style == CSS_BORDER_STYLE_SOLID &&
        ...
        if (last && box->border[BOTTOM].style == CSS_BORDER_STYLE_SOLID &&
        ...
        res = html_redraw_border_plot(
            BOTTOM, z, col, box->border[BOTTOM].style, bottom, square_end_1 && square_end_2, clip, ctx);
        if (res != NSERROR_OK) {
            return false;
        }
    }
