				 ...
				 bool do_marker)
{
	...
	if (box->list_marker) {
		 ...
		 * list element */
		res = coords_from_range(
			box->list_marker, start_idx, end_idx, rdwi, true);
		if (res != NSERROR_OK) {
			return res;
		}
	}
	...
	if ((box->type != BOX_BR) &&
		...
		if (selected_part(
			    box, start_idx, end_idx, &start_off, &end_off)) {
			int width, height;
			int x, y;
	...
	if (child) {
		...
		while (child) {
			...
			res = coords_from_range(
				child, start_idx, end_idx, rdwi, false);
			if (res != NSERROR_OK) {
				return res;
			}
				  ...
				  size_t whitespace_length)
{
	...
	if (box != NULL) {
		...
		if (box->style != NULL) {
			...
			font_plot_style_from_css(unit_len_ctx,
						 box->style,
						 &style);
			pstyle = &style;
		} else {
