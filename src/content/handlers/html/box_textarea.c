...
nserror box_textarea_keypress(html_content *html, struct box *box, uint32_t key)
{
    ...
    case NS_KEY_CR:
        if (form) {
            res = form_submit(content_get_url(c), html->bw, form, NULL);
        }
        break;
    ...
    case NS_KEY_TAB: {
        ...
        for (next_input = gadget->next; next_input &&
            ((next_input->type != GADGET_TEXTBOX && next_input->type != GADGET_TEXTAREA &&
    ...
    case NS_KEY_SHIFT_TAB: {
         ...
         * displayed (i.e. has an associated box) */
        for (prev_input = gadget->prev; prev_input &&
            ((prev_input->type != GADGET_TEXTBOX && prev_input->type != GADGET_TEXTAREA &&
...
static void box_textarea_callback(void *data, struct textarea_msg *msg)
{
    ...
    case TEXTAREA_MSG_DRAG_REPORT:
        if (msg->data.drag == TEXTAREA_DRAG_NONE) {
            ...
            struct rect rect = {.x0 = INT_MIN, .y0 = INT_MIN, .x1 = INT_MAX, .y1 = INT_MAX};
            union html_drag_owner drag_owner;
            drag_owner.textarea = box;

            switch (msg->data.drag) {
            case TEXTAREA_DRAG_SCROLLBAR:
                html_set_drag_type(html, HTML_DRAG_TEXTAREA_SCROLLBAR, drag_owner, &rect);
                break;

            case TEXTAREA_DRAG_SELECTION:
                html_set_drag_type(html, HTML_DRAG_TEXTAREA_SELECTION, drag_owner, &rect);
                break;
    ...
    case TEXTAREA_MSG_REDRAW_REQUEST: {
        ...
        content__request_redraw((struct content *)html, x + msg->data.redraw.x0, y + msg->data.redraw.y0,
            msg->data.redraw.x1 - msg->data.redraw.x0, msg->data.redraw.y1 - msg->data.redraw.y0);
    } break;
    ...
    case TEXTAREA_MSG_SELECTION_REPORT:
        if (msg->data.selection.have_selection) {
            ...
            html_set_selection(html, HTML_SELECTION_TEXTAREA, sel_owner, msg->data.selection.read_only);
        } else {
    ...
    case TEXTAREA_MSG_CARET_UPDATE:
        ...
        if (msg->data.caret.type == TEXTAREA_CARET_HIDE) {
            ...
            html_set_focus(html, HTML_FOCUS_TEXTAREA, focus_owner, true, 0, 0, 0, NULL);
        } else {
            union html_focus_owner focus_owner;
            focus_owner.textarea = box;
            html_set_focus(html, HTML_FOCUS_TEXTAREA, focus_owner, false, msg->data.caret.pos.x, msg->data.caret.pos.y,
                msg->data.caret.pos.height, msg->data.caret.pos.clip);
        }
        break;
...
bool box_textarea_create_textarea(html_content *html, struct box *box, struct dom_node *node)
{
    ...
    assert(gadget != NULL);
    assert(gadget->type == GADGET_TEXTAREA || gadget->type == GADGET_TEXTBOX || gadget->type == GADGET_PASSWORD);

    if (gadget->type == GADGET_TEXTAREA) {
        dom_html_text_area_element *textarea = (dom_html_text_area_element *)node;
        ta_flags = TEXTAREA_MULTILINE;

        err = dom_html_text_area_element_get_read_only(textarea, &read_only);
        if (err != DOM_NO_ERR)
            return false;

        err = dom_html_text_area_element_get_disabled(textarea, &disabled);
        if (err != DOM_NO_ERR)
            return false;
    ...
    gadget->data.text.ta = textarea_create(ta_flags, &ta_setup, box_textarea_callback, &gadget->data.text.data);
