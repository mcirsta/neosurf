...
static struct form *parse_form_element(const char *docenc, dom_node *node)
{
	...
	if (dom_html_form_element_get_accept_charset(formele, &ds_charset) !=
	    DOM_NO_ERR)
		goto out;
	...
	if (dom_html_form_element_get_enctype(formele, &ds_enctype) !=
	    DOM_NO_ERR)
		goto out;
...
parse_button_element(struct form *forms, dom_html_button_element *button)
{
	...
	if (ds_value != NULL) {
		control->value = strndup(dom_string_data(ds_value),
	...
	if (ds_name != NULL) {
		control->name = strndup(dom_string_data(ds_name),
...
parse_input_element(struct form *forms, dom_html_input_element *input)
{
	...
	if (control->type == GADGET_CHECKBOX || control->type == GADGET_RADIO) {
		bool selected;
		if (dom_html_input_element_get_checked(input, &selected) ==
		    DOM_NO_ERR) {
	...
	if (control->type == GADGET_PASSWORD ||
		...
		int32_t maxlength;
		if (dom_html_input_element_get_max_length(input, &maxlength) !=
		    DOM_NO_ERR) {
			maxlength = -1;
		}
	...
	if (control->type != GADGET_FILE && control->type != GADGET_IMAGE) {
		if (dom_html_input_element_get_value(input, &ds_value) ==
		    DOM_NO_ERR) {
			if (ds_value != NULL) {
...
html_forms_get_control_for_node(struct form *forms, dom_node *node)
{
	...
	for (f = forms; f != NULL; f = f->prev) {
		if (f->control_index != NULL) {
			...
			if (slot != NULL) {
				struct form_control *mapped = *(
					struct form_control **)slot;
				if (mapped != NULL) {
					return mapped;
				}
	...
	if (err == DOM_NO_ERR && ds_name != NULL) {
		...
		if (dom_string_caseless_lwc_isequal(ds_name,
			...
			ctl = parse_button_element(
				forms, (dom_html_button_element *)node);
		} else if (dom_string_caseless_lwc_isequal(
				   ds_name, corestring_lwc_input)) {
			ctl = parse_input_element(
				forms, (dom_html_input_element *)node);
		} else if (dom_string_caseless_lwc_isequal(
				   ds_name, corestring_lwc_textarea)) {
			ctl = parse_textarea_element(
				forms, (dom_html_text_area_element *)node);
		} else if (dom_string_caseless_lwc_isequal(
				   ds_name, corestring_lwc_select)) {
			ctl = parse_select_element(
				forms, (dom_html_select_element *)node);
		}
	}
