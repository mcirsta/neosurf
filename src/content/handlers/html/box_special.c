...
static bool box_create_frameset(struct content_html_frames *f, dom_node *n, html_content *content)
{
    ...
    if (err == DOM_NO_ERR && s != NULL) {
        if ((dom_string_data(s)[0] == '0') && (dom_string_data(s)[1] == '\0'))
    ...
    if (err == DOM_NO_ERR && s != NULL) {
        if (dom_string_caseless_lwc_isequal(s, corestring_lwc_no) == 0)
            default_border = false;
        dom_string_unref(s);
    }
        ...
        for (col = 0; c != NULL && col < cols; col++) {
            while (c != NULL) {
                ...
                if (type != DOM_ELEMENT_NODE ||
                    (!dom_string_caseless_lwc_isequal(name, corestring_lwc_frame) &&
                        !dom_string_caseless_lwc_isequal(name, corestring_lwc_frameset))) {
            ...
            if (dom_string_caseless_lwc_isequal(s, corestring_lwc_frameset)) {
                dom_string_unref(s);
                frame->border = 0;
                if (box_create_frameset(frame, c, content) == false) {
                    dom_node_unref(c);
                    return false;
                }
            ...
            if (dom_element_has_attribute(c, corestring_dom_noresize, &frame->no_resize) != DOM_NO_ERR) {
                /* If we can't read the attribute for some
                 * reason, assume we didn't have it.
                 */
                frame->no_resize = false;
            }
            ...
            if (err == DOM_NO_ERR && s != NULL) {
                if (dom_string_caseless_lwc_isequal(s, corestring_lwc_yes))
                    frame->scrolling = BW_SCROLLING_YES;
                else if (dom_string_caseless_lwc_isequal(s, corestring_lwc_no))
            ...
            err = dom_element_get_attribute(c, corestring_dom_bordercolor, &s);
            if (err == DOM_NO_ERR && s != NULL) {
                css_color color;

                if (nscss_parse_colour(dom_string_data(s), &color))
                    frame->border_colour = nscss_color_to_ns(color);
...
static bool box_get_attribute(dom_node *n, const char *attribute, void *context, char **value)
{
    ...
    err = dom_string_create_interned((const uint8_t *)attribute, strlen(attribute), &attr_name);
    if (err != DOM_NO_ERR)
        return false;
...
static bool box_input_text(html_content *html, struct box *box, struct dom_node *node)
{
    ...
    inline_box = box_create(NULL, box->style, false, 0, 0, box->title, 0, html->bctx);
    if (!inline_box)
        return false;
...
static bool box_a(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
    ...
    if (err == DOM_NO_ERR && s != NULL) {
        if (dom_string_caseless_lwc_isequal(s, corestring_lwc__blank))
            box->target = "_blank";
        else if (dom_string_caseless_lwc_isequal(s, corestring_lwc__top))
            box->target = "_top";
        else if (dom_string_caseless_lwc_isequal(s, corestring_lwc__parent))
            box->target = "_parent";
        else if (dom_string_caseless_lwc_isequal(s, corestring_lwc__self))
...
static bool box_embed(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
    ...
    if (err != DOM_NO_ERR || src == NULL)
        return true;
    if (box_extract_link(content, src, content->base_url, &params->data) == false) {
    ...
    for (idx = 0; idx < num_attrs; idx++) {
        ...
        if (param->name == NULL || param->value == NULL || param->valuetype == NULL) {
            dom_namednodemap_unref(attrs);
            return false;
        }
...
static bool box_input(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
    ...
    if (dom_string_caseless_lwc_isequal(type, corestring_lwc_password)) {
    ...
    } else if (dom_string_caseless_lwc_isequal(type, corestring_lwc_hidden)) {
        /* no box for hidden inputs */
        box->type = BOX_NONE;

    } else if ((dom_string_caseless_lwc_isequal(type, corestring_lwc_checkbox) ||
                   dom_string_caseless_lwc_isequal(type, corestring_lwc_radio))) {

    } else if (dom_string_caseless_lwc_isequal(type, corestring_lwc_submit) ||
        dom_string_caseless_lwc_isequal(type, corestring_lwc_reset) ||
        dom_string_caseless_lwc_isequal(type, corestring_lwc_button)) {
        struct box *inline_container, *inline_box;
        ...
        inline_box = box_create(NULL, box->style, false, 0, 0, box->title, 0, content->bctx);
        if (inline_box == NULL)
            goto no_memory;
    ...
    } else if (dom_string_caseless_lwc_isequal(type, corestring_lwc_image)) {
        ...
        if (box->style && ns_computed_display(box->style, box_is_root(n)) != CSS_DISPLAY_NONE &&
            ...
            err = dom_element_get_attribute(n, corestring_dom_src, &s);
            if (err == DOM_NO_ERR && s != NULL) {
                error = nsurl_join(content->base_url, dom_string_data(s), &url);
                dom_string_unref(s);
                if (error != NSERROR_OK)
                    goto no_memory;
                 ...
                 */
                if (nsurl_compare(url, content->base_url, NSURL_COMPLETE) == false) {
                    if (!html_fetch_object(content, url, box, image_types, false)) {
                        nsurl_unref(url);
                        goto no_memory;
                    }
                }
...
static bool box_object(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
    ...
    if (box_get_attribute(n, "usemap", content->bctx, &box->usemap) == false)
        return false;
    if (box->usemap && box->usemap[0] == '#')
        box->usemap++;
    ...
    if (err == DOM_NO_ERR && codebase != NULL) {
        if (box_extract_link(content, codebase, content->base_url, &params->codebase) == false) {
    ...
    if (err == DOM_NO_ERR && classid != NULL) {
        if (box_extract_link(content, classid, params->codebase, &params->classid) == false) {
    ...
    if (err == DOM_NO_ERR && data != NULL) {
        if (box_extract_link(content, data, params->codebase, &params->data) == false) {
    ...
    /* Don't include ourself */
    if (params->classid != NULL && nsurl_compare(content->base_url, params->classid, NSURL_COMPLETE))
        return true;

    if (params->data != NULL && nsurl_compare(content->base_url, params->data, NSURL_COMPLETE))
        return true;

    /* codetype and type are MIME types */
    if (box_get_attribute(n, "codetype", params, &params->codetype) == false)
        return false;
    ...
    if (params->classid != NULL && params->data == NULL && params->codetype != NULL) {
        lwc_string *icodetype;
        lwc_error lerror;

        lerror = lwc_intern_string(params->codetype, strlen(params->codetype), &icodetype);
        if (lerror != lwc_error_ok)
            return false;

        if (content_factory_type_from_mime_type(icodetype) == CONTENT_NONE) {
    ...
    if (params->data != NULL && params->type != NULL) {
        ...
        lerror = lwc_intern_string(params->type, strlen(params->type), &itype);
        if (lerror != lwc_error_ok)
            return false;
    ...
    while (c != NULL) {
        ...
        if (type == DOM_ELEMENT_NODE) {
            ...
            if (!dom_string_caseless_lwc_isequal(name, corestring_lwc_param)) {
            ...
            if (box_get_attribute(c, "name", param, &param->name) == false) {
                dom_node_unref(c);
                return false;
            }

            if (box_get_attribute(c, "value", param, &param->value) == false) {
                dom_node_unref(c);
                return false;
            }

            if (box_get_attribute(c, "type", param, &param->type) == false) {
                dom_node_unref(c);
                return false;
            }

            if (box_get_attribute(c, "valuetype", param, &param->valuetype) == false) {
                dom_node_unref(c);
                return false;
            }
    ...
    box->flags |= IS_REPLACED;
    if (!html_fetch_object(content, params->data ? params->data : params->classid, box, CONTENT_ANY, false))
        return false;
...
static bool box_select(dom_node *n, html_content *content, struct box *box, bool *convert_children)
{
    ...
    while (c != NULL) {
        ...
        if (dom_string_caseless_lwc_isequal(name, corestring_lwc_option)) {
            dom_string_unref(name);
        ...
        } else if (dom_string_caseless_lwc_isequal(name, corestring_lwc_optgroup)) {
            dom_string_unref(name);
            ...
            while (c2 != NULL) {
                ...
                if (dom_string_caseless_lwc_isequal(c2_name, corestring_lwc_option)) {
                    dom_string_unref(c2_name);

                    if (box_select_add_option(gadget, c2) == false) {
                        dom_node_unref(c2);
                        dom_node_unref(c);
                        form_free_control(gadget);
                        return false;
    ...
    if (gadget->data.select.multiple == false && gadget->data.select.num_selected == 0) {
        gadget->data.select.current = gadget->data.select.items;
        gadget->data.select.current->initial_selected = gadget->data.select.current->selected = true;
        gadget->data.select.num_selected = 1;
        dom_html_option_element_set_selected(gadget->data.select.current->node, true);
    }
