...
{
	return b->type == BOX_INLINE || b->type == BOX_INLINE_FLEX ||
	       b->type == BOX_INLINE_BLOCK || b->type == BOX_TEXT ||
...
static inline bool
lh__have_border(enum box_side side, const css_computed_style *style)
{
...
{
	return b->padding[TOP] + b->padding[BOTTOM] + b->border[TOP].width +
	       b->border[BOTTOM].width + lh__non_auto_margin(b, TOP) +
	       lh__non_auto_margin(b, BOTTOM);
}

static inline int lh__delta_outer_width(const struct box *b)
{
	return b->padding[LEFT] + b->padding[RIGHT] + b->border[LEFT].width +
	       b->border[RIGHT].width + lh__non_auto_margin(b, LEFT) +
	       lh__non_auto_margin(b, RIGHT);
}

static inline int
lh__delta_outer_main(const struct box *flex, const struct box *b)
{
...
static inline int
lh__delta_outer_cross(const struct box *flex, const struct box *b)
{
...
static inline bool lh__box_size_cross_is_auto(bool horizontal, struct box *b)
{
	...
	if (horizontal) {
		return css_computed_height(b->style, &length, &unit) ==
		       CSS_HEIGHT_AUTO;
	} else {
		return css_computed_width(b->style, &length, &unit) ==
		       CSS_WIDTH_AUTO;
	}
}

static inline enum css_align_self_e
lh__box_align_self(const struct box *flex, const struct box *item)
{
 ...
 */
static inline void calculate_mbp_width(const css_unit_ctx *unit_len_ctx,
 ...
 *				gadget properties.
 */
static inline void layout_handle_box_sizing(const css_unit_ctx *unit_len_ctx,
 ...
 */
static inline void layout_find_dimensions(const css_unit_ctx *unit_len_ctx,
					  int available_width,
					  int viewport_height,
					  const struct box *box,
					  const css_computed_style *style,
					  ...
					  struct box_border border[4])
{
	...
	if (width) {
		if (css_computed_width_px(
			    style, unit_len_ctx, available_width, width) ==
		    CSS_WIDTH_SET) {
			layout_handle_box_sizing(unit_len_ctx,
						 box,
						 available_width,
						 true,
						 width);
	...
	for (i = 0; i != 4; i++) {
		...
		if (border && box->type != BOX_TABLE_CELL) {
			...
			if (bstyle == CSS_BORDER_STYLE_HIDDEN ||
				...
				border[i].width = FIXTOINT(
					css_unit_len2device_px(style,
							       unit_len_ctx,
							       value,
							       unit));
