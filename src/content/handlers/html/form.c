 ...
 */
static nserror fetch_data_list_add_sname(
    const char *name, const char *ksfx, int value, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    if (fetch_data->name == NULL) {
        free(fetch_data);
        NSLOG(neosurf, INFO, "keyname allocation failure for %s%s", name, ksfx);
        return NSERROR_NOMEM;
    }
 ...
 */
static nserror fetch_data_list_add(dom_string *name, dom_string *value, const char *rawfile, const char *form_charset,
    const char *docu_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    struct fetch_multipart_data *fetch_data;
    ...
    if (rawfile != NULL) {
        ...
        if (fetch_data->rawfile == NULL) {
            NSLOG(neosurf, INFO, "Could not encode rawfile value for fetch data");
 ...
 * \return NSERROR_OK on success or appropriate error code.
 */
static nserror form_dom_to_data_textarea(dom_html_text_area_element *text_area_element, const char *form_charset,
    const char *doc_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    exp = dom_html_text_area_element_get_disabled(text_area_element, &element_disabled);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get text area disabled property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    exp = dom_html_text_area_element_get_name(text_area_element, &inputname);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get text area name property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    exp = dom_html_text_area_element_get_value(text_area_element, &inputvalue);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get text area content. exp %d", exp);
        dom_string_unref(inputname);
        return NSERROR_DOM;
    }

    /* add key/value pair to fetch data list */
    res = fetch_data_list_add(inputname, inputvalue, NULL, form_charset, doc_charset, fetch_data_next_ptr);
...
static nserror form_dom_to_data_select_option(dom_html_option_element *option_element, dom_string *keyname,
    const char *form_charset, const char *docu_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    nserror res;
    dom_exception exp; /* the result from DOM operations */
    dom_string *value;
    ...
    /* add key/value pair to fetch data list */
    res = fetch_data_list_add(keyname, value, NULL, form_charset, docu_charset, fetch_data_next_ptr);
 ...
 * \return NSERROR_OK on success or appropriate error code.
 */
static nserror form_dom_to_data_select(dom_html_select_element *select_element, const char *form_charset,
    const char *doc_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    /* check if element is disabled */
    exp = dom_html_select_element_get_disabled(select_element, &element_disabled);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get select disabled property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get select name property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get select options collection length");
    ...
    for (option_index = 0; option_index < options_count; ++option_index) {
        exp = dom_html_options_collection_item(options, option_index, &option_element);
        if (exp != DOM_NO_ERR) {
            NSLOG(neosurf, INFO, "Could not get options item %d", option_index);
            res = NSERROR_DOM;
        } else {
            res = form_dom_to_data_select_option(
                (dom_html_option_element *)option_element, inputname, form_charset, doc_charset, fetch_data_next_ptr);
...
static nserror form_dom_to_data_input_submit(dom_html_input_element *input_element, dom_string *inputname,
    const char *charset, const char *document_charset, dom_html_element **submit_button,
    struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    /* add key/value pair to fetch data list */
    res = fetch_data_list_add(inputname, inputvalue, NULL, charset, document_charset, fetch_data_next_ptr);
...
static nserror form_dom_to_data_input_image(dom_html_input_element *input_element, dom_string *inputname,
    const char *charset, const char *document_charset, dom_html_element **submit_button,
    struct fetch_multipart_data ***fetch_data_next_ptr)
{
    nserror res;
    ...
    exp = dom_node_get_user_data((dom_node *)input_element, corestring_dom___ns_key_image_coords_node_data, &coords);
    if (exp != DOM_NO_ERR) {
    ...
    res = fetch_data_list_add_sname(basename, ".x", coords->x, fetch_data_next_ptr);

    if (res == NSERROR_OK) {
        res = fetch_data_list_add_sname(basename, ".y", coords->y, fetch_data_next_ptr);
    }
...
static nserror form_dom_to_data_input_checkbox(dom_html_input_element *input_element, dom_string *inputname,
    ...
    const char *charset, const char *document_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    /* add key/value pair to fetch data list */
    res = fetch_data_list_add(inputname, inputvalue, rawfile, charset, document_charset, fetch_data_next_ptr);
...
static nserror form_dom_to_data_input_text(dom_html_input_element *input_element, dom_string *inputname,
    const char *charset, const char *document_charset, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    nserror res;
    dom_exception exp; /* the result from DOM operations */
    ...
    const char *document_charset, dom_html_element **submit_button, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    ...
    /* check if element is disabled */
    exp = dom_html_input_element_get_disabled(input_element, &element_disabled);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get input disabled property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get input name property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    if (dom_string_caseless_isequal(inputtype, corestring_dom_submit)) {
        ...
        res = form_dom_to_data_input_submit(
            input_element, inputname, charset, document_charset, submit_button, fetch_data_next_ptr);

    } else if (dom_string_caseless_isequal(inputtype, corestring_dom_image)) {

        res = form_dom_to_data_input_image(
            input_element, inputname, charset, document_charset, submit_button, fetch_data_next_ptr);

    } else if (dom_string_caseless_isequal(inputtype, corestring_dom_radio) ||
        dom_string_caseless_isequal(inputtype, corestring_dom_checkbox)) {

        res = form_dom_to_data_input_checkbox(input_element, inputname, charset, document_charset, fetch_data_next_ptr);
        ...
        res = form_dom_to_data_input_text(input_element, inputname, charset, document_charset, fetch_data_next_ptr);
    }
 ...
 * \return NSERROR_OK on success or appropriate error code.
 */
static nserror form_dom_to_data_button(dom_html_button_element *button_element, const char *form_charset,
    const char *doc_charset, dom_html_element **submit_button, struct fetch_multipart_data ***fetch_data_next_ptr)
{
    dom_exception exp; /* the result from DOM operations */
    bool element_disabled;
    dom_string *inputname;
    ...
    /* check if element is disabled */
    exp = dom_html_button_element_get_disabled(button_element, &element_disabled);
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Unable to get disabled property. exp %d", exp);
        return NSERROR_DOM;
    }
    ...
    if (exp != DOM_NO_ERR) {
        NSLOG(neosurf, INFO, "Could not get button name property. exp %d", exp);
        return NSERROR_DOM;
    }
...
form_dom_to_data(struct form *form, struct form_control *submit_control, struct fetch_multipart_data **fetch_data_out)
{
    ...
    for (element_idx = 0; element_idx < element_count; element_idx++) {
        ...
        if (exp != DOM_NO_ERR) {
            NSLOG(neosurf, INFO, "retrieving form element %d failed with %d", element_idx, exp);
            res = NSERROR_DOM;
            goto form_dom_to_data_error;
        }
        ...
        if (exp != DOM_NO_ERR) {
            NSLOG(neosurf, INFO, "getting element node name %d failed with %d", element_idx, exp);
        ...
        if (dom_string_isequal(nodename, corestring_dom_TEXTAREA)) {
            ...
            res = form_dom_to_data_textarea(
                (dom_html_text_area_element *)element, charset, form->document_charset, &fetch_data_next);

        } else if (dom_string_isequal(nodename, corestring_dom_SELECT)) {
            /* Form element is HTMLSelectElement */
            res = form_dom_to_data_select(
                (dom_html_select_element *)element, charset, form->document_charset, &fetch_data_next);
            ...
            res = form_dom_to_data_button(
                (dom_html_button_element *)element, charset, form->document_charset, &submit_button, &fetch_data_next);

        } else {
            /* Form element is not handled */
            NSLOG(neosurf, INFO, "Unhandled element type: %*s", (int)dom_string_byte_length(nodename),
...
static void form_select_menu_scroll_callback(void *client_data, struct scrollbar_msg_data *scrollbar_data)
{
    ...
    case SCROLLBAR_MSG_SCROLL_START: {
        ...
        browser_window_set_drag_type(html->bw, DRAGGING_CONTENT_SCROLLBAR, &rect);
...
void form_free_control(struct form_control *control)
{
    ...
    NSLOG(neosurf, INFO, "Control:%p name:%p value:%p initial:%p", control, control->name, control->value,
    ...
    if (control->type == GADGET_SELECT) {
        ...
        for (option = control->data.select.items; option; option = next) {
            next = option->next;
            NSLOG(neosurf, INFO, "select option:%p text:%p value:%p", option, option->text, option->value);
            free(option->text);
            free(option->value);
            free(option);
        }
    ...
    if (control->type == GADGET_TEXTAREA || control->type == GADGET_TEXTBOX || control->type == GADGET_PASSWORD) {
    ...
    if (control->form != NULL) {
        for (c = control->form->controls; c != NULL; c = c->next) {
            ...
            if (c == control) {
                ...
                if (control->form->last_control == control)
                    control->form->controls = control->form->last_control = NULL;
                break;
            }
        }
...
bool form_add_option(struct form_control *control, char *value, char *text, bool selected, void *node)
{
    ...
    /* set selected */
    if (selected && (control->data.select.num_selected == 0 || control->data.select.multiple)) {
...
nserror form_open_select_menu(
    void *client_data, struct form_control *control, select_menu_redraw_callback callback, struct content *c)
{
    ...
    if (control->data.select.menu == NULL) {
        ...
        menu->width = box->width + box->border[RIGHT].width + box->padding[RIGHT] + box->border[LEFT].width +
            box->padding[LEFT];

        font_plot_style_from_css(&html->unit_len_ctx, control->box->style, &fstyle);
        menu->f_size = fstyle.size;

        menu->line_height = FIXTOINT(
            FDIV((FMUL(FLTTOFIX(1.2), FMUL(html->unit_len_ctx.device_dpi, INTTOFIX(fstyle.size / PLOT_STYLE_SCALE)))),
                F_72));

        line_height_with_spacing = menu->line_height + menu->line_height * SELECT_LINE_SPACING;

        total_height = control->data.select.num_items * line_height_with_spacing;
        ...
        res = scrollbar_create(false, menu->height, total_height, menu->height, control,
            form_select_menu_scroll_callback, &(menu->scrollbar));
        if (res != NSERROR_OK) {
    ...
    struct form_control *control, int x, int y, float scale, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    item_y -= line_height_with_spacing;
    text_pos_offset = y - scroll + (int)(line_height * (0.75 + SELECT_LINE_SPACING));
    ...
    res = scrollbar_redraw(menu->scrollbar, x_cp + menu->width - SCROLLBAR_WIDTH, y_cp, clip, scale, ctx);
    if (res != NSERROR_OK) {
        return false;
    }
...
bool form_clip_inside_select_menu(struct form_control *control, float scale, const struct rect *clip)
{
    ...
    if (clip->x0 >= 0 && clip->x1 <= width && clip->y0 >= 0 && clip->y1 <= height)
        return true;
...
const char *form_select_mouse_action(struct form_control *control, browser_mouse_state mouse, int x, int y)
{
    ...
    if (menu->scroll_capture || (x > scrollbar_x && x < x1 && y > y0 && y < y1)) {
        /* The scroll is currently capturing all events or the mouse
         * event is taking place on the scrollbar widget area
         */
        x -= scrollbar_x;
        return scrollbar_mouse_status_to_message(scrollbar_mouse_action(menu->scrollbar, mouse, x, y));
    }
    ...
    if (x > x0 && x < scrollbar_x && y > y0 && y < y1) {
        ...
        if (!(mouse & BROWSER_MOUSE_CLICK_1 && !multiple))
            /* anything but a button 1 click over a single select
               menu */
            status = messages_get(control->data.select.multiple ? "SelectMClick" : "SelectClick");
...
void form_select_mouse_drag_end(struct form_control *control, browser_mouse_state mouse, int x, int y)
{
    ...
    box_y += box->height + box->border[BOTTOM].width + box->padding[BOTTOM] + box->padding[TOP];
...
void form_select_menu_callback(void *client_data, int x, int y, int width, int height)
{
    ...
    content__request_redraw((struct content *)html, menu_x + x, menu_y + y, width, height);
}
...
void form_radio_set(struct form_control *radio)
{
    ...
    for (control = radio->form->controls; control != NULL; control = control->next) {
         ...
         */
        if ((control->name == NULL) || (radio->name == NULL) || (control->name[0] == '\0') ||
            (radio->name[0] == '\0') || strcmp(control->name, radio->name) != 0)
            continue;
...
/* private interface described in html/form_internal.h */
nserror
form_submit(nsurl *page_url, struct browser_window *target, struct form *form, struct form_control *submit_button)
{
    nserror res;
    ...
    case method_GET:
        ...
        if (res == NSERROR_OK) {
            ...
            if (res == NSERROR_OK) {
                res = browser_window_navigate(target, query_url, page_url, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
    ...
    case method_POST_URLENC:
        res = form_url_encode(form, success, &data);
        if (res == NSERROR_OK) {
            res = browser_window_navigate(target, action_url, page_url, BW_NAVIGATE_HISTORY, data, NULL, NULL);
            free(data);
        }
        break;

    case method_POST_MULTIPART:
        res = browser_window_navigate(target, action_url, page_url, BW_NAVIGATE_HISTORY, NULL, success, NULL);
        ...
        if (control->node != NULL) {
            ...
            if (err == DOM_NO_ERR) {
                if (control->type == GADGET_TEXTAREA)
                    err = dom_html_text_area_element_set_value((dom_html_text_area_element *)(control->node), str);
                else
                    err = dom_html_input_element_set_value((dom_html_input_element *)(control->node), str);
...
void form_gadget_sync_with_dom(struct form_control *control)
{
    ...
    if (control->syncing ||
        (control->type != GADGET_TEXTBOX && control->type != GADGET_PASSWORD && control->type != GADGET_HIDDEN &&
    ...
    if ((control->last_synced_value == NULL && control->value != NULL && control->value[0] != '\0') ||
        ...
        exc = dom_string_create((uint8_t *)(control->value), strlen(control->value), &value);
        if (exc != DOM_NO_ERR) {
            goto out;
        }
        if (control->node_value != NULL) {
...
static hashmap_parameters_t form_node_map_params = {
