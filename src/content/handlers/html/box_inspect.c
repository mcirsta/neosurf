...
#define box_is_float(box) (box->type == BOX_FLOAT_LEFT || box->type == BOX_FLOAT_RIGHT)
...
static bool box_contains_point(const css_unit_ctx *unit_len_ctx, const struct box *box, int x, int y, bool *physically)
{
    ...
    if (box->style != NULL && css_computed_position(box->style) == CSS_POSITION_ABSOLUTE &&
        ...
        struct rect r = {.x0 = box->border[LEFT].width,
            .y0 = box->border[TOP].width,
            .x1 = box->padding[LEFT] + box->width + box->border[RIGHT].width + box->padding[RIGHT],
            .y1 = box->padding[TOP] + box->height + box->border[BOTTOM].width + box->padding[BOTTOM]};
        ...
        if (css_rect.left_auto == false) {
            r.x0 += FIXTOINT(css_unit_len2device_px(box->style, unit_len_ctx, css_rect.left, css_rect.lunit));
        }
        if (css_rect.top_auto == false) {
            r.y0 += FIXTOINT(css_unit_len2device_px(box->style, unit_len_ctx, css_rect.top, css_rect.tunit));
        }
        if (css_rect.right_auto == false) {
            r.x1 = box->border[LEFT].width +
                FIXTOINT(css_unit_len2device_px(box->style, unit_len_ctx, css_rect.right, css_rect.runit));
        }
        if (css_rect.bottom_auto == false) {
            r.y1 = box->border[TOP].width +
                FIXTOINT(css_unit_len2device_px(box->style, unit_len_ctx, css_rect.bottom, css_rect.bunit));
        }
    ...
    if (x >= -box->border[LEFT].width &&
        x < box->padding[LEFT] + box->width + box->padding[RIGHT] + box->border[RIGHT].width &&
        y >= -box->border[TOP].width &&
        y < box->padding[TOP] + box->height + box->padding[BOTTOM] + box->border[BOTTOM].width) {
        *physically = true;
        return true;
    }
    if (box->list_marker && box->list_marker->x - box->x <= x + box->list_marker->border[LEFT].width &&
        x < box->list_marker->x - box->x + box->list_marker->padding[LEFT] + box->list_marker->width +
                box->list_marker->border[RIGHT].width + box->list_marker->padding[RIGHT] &&
        box->list_marker->y - box->y <= y + box->list_marker->border[TOP].width &&
        y < box->list_marker->y - box->y + box->list_marker->padding[TOP] + box->list_marker->height +
                box->list_marker->border[BOTTOM].width + box->list_marker->padding[BOTTOM]) {
        *physically = true;
        return true;
    }
    if ((box->style && css_computed_overflow_x(box->style) == CSS_OVERFLOW_VISIBLE) || !box->style) {
        if (box->descendant_x0 <= x && x < box->descendant_x1) {
            *physically = false;
            return true;
        }
    }
    if ((box->style && css_computed_overflow_y(box->style) == CSS_OVERFLOW_VISIBLE) || !box->style) {
 ...
 */
static inline struct box *box_next_xy(struct box *b, int *x, int *y, bool skip_children)
{
    struct box *n;
    int tx, ty;
 ...
 */
static bool box_nearer_text_box(struct box *box, int bx, int by, int x, int y, int dir, struct box **nearest, int *tx,
    int *ty, int *nr_xd, int *nr_yd)
{
 ...
 */
static bool box_nearest_text_box(struct box *box, int bx, int by, int fx, int fy, int x, int y, int dir,
    struct box **nearest, int *tx, int *ty, int *nr_xd, int *nr_yd)
{
    ...
    while (child) {
        if (child->type == BOX_FLOAT_LEFT || child->type == BOX_FLOAT_RIGHT) {
            c_bx = fx + child->x - scrollbar_get_offset(child->scroll_x);
            c_by = fy + child->y - scrollbar_get_offset(child->scroll_y);
        } else {
            c_bx = bx + child->x - scrollbar_get_offset(child->scroll_x);
            c_by = by + child->y - scrollbar_get_offset(child->scroll_y);
        }
        ...
        if (in_box && child->text && !child->object) {
            if (box_nearer_text_box(child, c_bx, c_by, x, y, dir, nearest, tx, ty, nr_xd, nr_yd))
                return true;
        } else {
            if (child->list_marker) {
                if (box_nearer_text_box(child->list_marker, c_bx + child->list_marker->x, c_by + child->list_marker->y,
                        x, y, dir, nearest, tx, ty, nr_xd, nr_yd))
                    return true;
            }
            if (box_nearest_text_box(child, c_bx, c_by, c_fx, c_fy, x, y, dir, nearest, tx, ty, nr_xd, nr_yd))
                return true;
        }
        child = child->next;
    }
...
struct box *box_find_by_id(struct box *box, lwc_string *id)
{
    ...
    if (box->id != NULL && lwc_string_isequal(id, box->id, &m) == lwc_error_ok && m == true) {
        return box;
    }
...
void box_dump(FILE *stream, struct box *box, unsigned int depth, bool style)
{
    ...
    fprintf(stream, "x%i y%i w%i h%i ", box->x, box->y, box->width, box->height);
    if (box->max_width != UNKNOWN_MAX_WIDTH) {
        fprintf(stream, "min%i max%i ", box->min_width.value, box->max_width);
    }
    fprintf(
        stream, "desc(%i %i %i %i) ", box->descendant_x0, box->descendant_y0, box->descendant_x1, box->descendant_y1);

    fprintf(stream, "m(%i %i %i %i) ", box->margin[TOP], box->margin[LEFT], box->margin[BOTTOM], box->margin[RIGHT]);
    ...
    if (box->object) {
        fprintf(stream, "(object '%s') ", nsurl_access(hlcache_handle_get_url(box->object)));
    }
    if (box->iframe) {
    ...
    if (box->col) {
        ...
        for (i = 0; i != box->columns; i++) {
            fprintf(stream, " (%s %s %i %i %i)",
                ((const char *[]){"UNKNOWN", "FIXED", "AUTO", "PERCENT", "RELATIVE"})[box->col[i].type],
                ((const char *[]){"normal", "positioned"})[box->col[i].positioned], box->col[i].width, box->col[i].min,
                box->col[i].max);
        }
        fprintf(stream, ")");
    }
...
{
    return box->padding[TOP] + box->height + box->padding[BOTTOM] + box->border[BOTTOM].width < box->descendant_y1;
}
...
{
    return box->padding[LEFT] + box->width + box->padding[RIGHT] + box->border[RIGHT].width < box->descendant_x1;
}
...
struct box *box_pick_text_box(struct html_content *html, int x, int y, int dir, int *dx, int *dy)
{
    ...
    if (!box_nearest_text_box(box, bx, by, fx, fy, x, y, dir, &text_box, &tx, &ty, &nr_xd, &nr_yd)) {
        if (text_box && text_box->text && !text_box->object) {
            int w = (text_box->padding[LEFT] + text_box->width + text_box->padding[RIGHT]);
            int h = (text_box->padding[TOP] + text_box->height + text_box->padding[BOTTOM]);
            int x1, y1;
