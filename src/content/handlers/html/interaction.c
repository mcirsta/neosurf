...
static browser_pointer_shape get_pointer_shape(struct box *box, bool imagemap)
{
    ...
    case CSS_CURSOR_AUTO:
        if (box->href || (box->gadget && (box->gadget->type == GADGET_IMAGE || box->gadget->type == GADGET_SUBMIT)) ||
            imagemap) {
            /* link */
...
static void html_box_drag_start(struct box *box, int x, int y)
{
    ...
    if (box->scroll_x != NULL) {
        ...
        scroll_mouse_y = y - (box_y + box->padding[TOP] + box->height + box->padding[BOTTOM] - SCROLLBAR_WIDTH);
        scrollbar_start_content_drag(box->scroll_x, scroll_mouse_x, scroll_mouse_y);
    } else if (box->scroll_y != NULL) {
        scroll_mouse_x = x - (box_x + box->padding[LEFT] + box->width + box->padding[RIGHT] - SCROLLBAR_WIDTH);
        scroll_mouse_y = y - box_y;

        scrollbar_start_content_drag(box->scroll_y, scroll_mouse_x, scroll_mouse_y);
    }
}
...
static size_t html_selection_drag_end(struct html_content *html, browser_mouse_state mouse, int x, int y, int dir)
{
    ...
    if (box) {
        ...
        guit->layout->position(&fstyle, box->text, box->length, dx, &idx, &pixel_offset);
 ...
 */
static void html__image_coords_dom_user_data_handler(
    dom_node_operation operation, dom_string *key, void *_data, struct dom_node *src, struct dom_node *dst)
{
    ...
    if (!dom_string_isequal(corestring_dom___ns_key_image_coords_node_data, key) || coords == NULL) {
        return;
    }
    ...
    case DOM_NODE_CLONED:
        ...
        if (newcoords != NULL) {
            *newcoords = *coords;
            if (dom_node_set_user_data(dst, corestring_dom___ns_key_image_coords_node_data, newcoords,
                    html__image_coords_dom_user_data_handler, &oldcoords) == DOM_NO_ERR) {
                free(oldcoords);
            }
        }
        break;
...
static void html_overflow_scroll_drag_end(struct scrollbar *scrollbar, browser_mouse_state mouse, int x, int y)
{
    ...
    if (scrollbar_is_horizontal(scrollbar)) {
        scroll_mouse_x = x - box_x;
        scroll_mouse_y = y - (box_y + box->padding[TOP] + box->height + box->padding[BOTTOM] - SCROLLBAR_WIDTH);
        scrollbar_mouse_drag_end(scrollbar, mouse, scroll_mouse_x, scroll_mouse_y);
    } else {
        scroll_mouse_x = x - (box_x + box->padding[LEFT] + box->width + box->padding[RIGHT] - SCROLLBAR_WIDTH);
        scroll_mouse_y = y - box_y;
        scrollbar_mouse_drag_end(scrollbar, mouse, scroll_mouse_x, scroll_mouse_y);
 ...
 */
static nserror
mouse_action_select_menu(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    struct box *box;
    int box_x = 0;
    int box_y = 0;
    ...
    status = form_select_mouse_action(html->visible_select_menu, mouse, x - box_x, y - box_y);
    if (status != NULL) {
    ...
    content_request_redraw(bw_content, box_x, box_y, width, height);
    return NSERROR_OK;
}
 ...
 */
static nserror
mouse_action_drag_selection(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    struct box *box;
    int dir = -1;
    int dx, dy;
 ...
 */
static nserror
mouse_action_drag_scrollbar(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    ...
    if (scrollbar_is_horizontal(scr)) {
        scroll_mouse_x = x - box_x;
        scroll_mouse_y = y - (box_y + box->padding[TOP] + box->height + box->padding[BOTTOM] - SCROLLBAR_WIDTH);
        scrollbar_status = scrollbar_mouse_action(scr, mouse, scroll_mouse_x, scroll_mouse_y);
    } else {
        scroll_mouse_x = x - (box_x + box->padding[LEFT] + box->width + box->padding[RIGHT] - SCROLLBAR_WIDTH);
        scroll_mouse_y = y - box_y;
 ...
 */
static nserror
mouse_action_drag_textarea(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    struct box *box;
    int box_x = 0;
    int box_y = 0;
    ...
    assert(box->gadget->type == GADGET_TEXTAREA || box->gadget->type == GADGET_PASSWORD ||
 ...
 */
static nserror
mouse_action_drag_content(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    struct box *box;
    int box_x = 0;
    int box_y = 0;
    ...
    box_coords(box, &box_x, &box_y);
    content_mouse_track(box->object, bw, mouse, x - box_x, y - box_y);
    return NSERROR_OK;
}
...
static nserror get_mouse_action_node(html_content *html, int x, int y, struct mouse_action_state *man)
{
    ...
    do {
        ...
        if (box->usemap) {
            man->link.url = imagemap_get(html, box->usemap, box_x, box_y, x, y, &man->link.target);
            man->link.box = box;
        ...
        if ((box->scroll_x != NULL) || (box->scroll_y != NULL)) {
            ...
            padding_left = box_x + scrollbar_get_offset(box->scroll_x);
            padding_right = padding_left + box->padding[LEFT] + box->width + box->padding[RIGHT];
            padding_top = box_y + scrollbar_get_offset(box->scroll_y);
            padding_bottom = padding_top + box->padding[TOP] + box->height + box->padding[BOTTOM];

            if ((x > padding_left) && (x < padding_right) && (y > padding_top) && (y < padding_bottom)) {
                /* mouse inside padding box */

                if ((box->scroll_y != NULL) && (x > (padding_right - SCROLLBAR_WIDTH))) {
                ...
                } else if ((box->scroll_x != NULL) && (y > (padding_bottom - SCROLLBAR_WIDTH))) {
 ...
 */
static nserror
gadget_mouse_action(html_content *html, browser_mouse_state mouse, int x, int y, struct mouse_action_state *mas)
{
    ...
    bool click;
    click = mouse &
        (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_CLICK_1 | BROWSER_MOUSE_CLICK_2 |
    ...
    case GADGET_SELECT:
        ...
        if (mouse & BROWSER_MOUSE_CLICK_1 && nsoption_bool(core_select_menu)) {
            ...
            content_broadcast(c, CONTENT_MSG_SELECTMENU, &msg_data);
        }
        break;
    ...
    case GADGET_CHECKBOX:
        ...
        if (mouse & BROWSER_MOUSE_CLICK_1) {
            ...
            dom_html_input_element_set_checked(
                (dom_html_input_element *)(mas->gadget.control->node), mas->gadget.control->selected);
    ...
    case GADGET_TEXTAREA:
        ...
        if (click &&
            (html->selection_type != HTML_SELECTION_TEXTAREA || html->selection_owner.textarea != mas->gadget.box)) {
            union html_selection_owner sel_owner;
            sel_owner.none = true;
            html_set_selection(html, HTML_SELECTION_NONE, sel_owner, true);
        }

        ta_status = textarea_mouse_action(
            mas->gadget.control->data.text.ta, mouse, x - mas->gadget.box_x, y - mas->gadget.box_y);
 ...
 */
static nserror
iframe_mouse_action(struct browser_window *bw, browser_mouse_state mouse, int x, int y, struct mouse_action_state *mas)
{
    int pos_x, pos_y;
    float scale;
    ...
    if (mouse & BROWSER_MOUSE_CLICK_1 || mouse & BROWSER_MOUSE_CLICK_2) {
        browser_window_mouse_click(mas->iframe, mouse, (x * scale) - pos_x, (y * scale) - pos_y);
    } else {
        browser_window_mouse_track(mas->iframe, mouse, (x * scale) - pos_x, (y * scale) - pos_y);
    }
 ...
 */
static nserror html_object_mouse_action(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x,
    int y, struct mouse_action_state *mas)
{
    bool click;
    click = mouse &
        (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_CLICK_1 | BROWSER_MOUSE_CLICK_2 |
    ...
    if (click &&
        (html->selection_type != HTML_SELECTION_CONTENT || html->selection_owner.content != mas->html_object.box)) {
        union html_selection_owner sel_owner;
        sel_owner.none = true;
        html_set_selection(html, HTML_SELECTION_NONE, sel_owner, true);
    }
    ...
    if (mouse & BROWSER_MOUSE_CLICK_1 || mouse & BROWSER_MOUSE_CLICK_2) {
        content_mouse_action(
            mas->html_object.box->object, bw, mouse, x - mas->html_object.pos_x, y - mas->html_object.pos_y);
    } else {
        content_mouse_track(
            mas->html_object.box->object, bw, mouse, x - mas->html_object.pos_x, y - mas->html_object.pos_y);
    }
 ...
 */
static nserror link_mouse_action(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y,
    struct mouse_action_state *mas)
{
    ...
    if (mas->title) {
        snprintf(
            status_buffer, sizeof status_buffer, "%s: %s", url_s ? url_s : nsurl_access(mas->link.url), mas->title);
    } else {
        snprintf(status_buffer, sizeof status_buffer, "%s", url_s ? url_s : nsurl_access(mas->link.url));
    }
    ...
    mas->result.pointer = get_pointer_shape(mas->link.box, mas->link.is_imagemap);

    if (mouse & BROWSER_MOUSE_CLICK_1 && mouse & BROWSER_MOUSE_MOD_1) {
        /* force download of link */
        browser_window_navigate(
            bw, mas->link.url, content_get_url((struct content *)html), BW_NAVIGATE_DOWNLOAD, NULL, NULL, NULL);

    } else if (mouse & BROWSER_MOUSE_CLICK_2 && mouse & BROWSER_MOUSE_MOD_1) {
        msg_data.savelink.url = mas->link.url;
        msg_data.savelink.title = mas->title;
        content_broadcast((struct content *)html, CONTENT_MSG_SAVELINK, &msg_data);
 ...
 */
static nserror default_mouse_action(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x,
    int y, struct mouse_action_state *mas)
{
    struct content *c = (struct content *)html;
    ...
    if (mas->text.box) {
        ...
        font_plot_style_from_css(&html->unit_len_ctx, mas->text.box->style, &fstyle);

        guit->layout->position(
            &fstyle, mas->text.box->text, mas->text.box->length, x - mas->text.box_x, &idx, &pixel_offset);

        if (selection_click(html->sel, html->bw, mouse, mas->text.box->byte_offset + idx)) {
 ...
 */
static nserror
mouse_action_drag_none(html_content *html, struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    nserror res;
    struct content *c = (struct content *)html;
    union content_msg_data msg_data;
    ...
    if (mouse & BROWSER_MOUSE_CLICK_4) {
        ...
        mas.result.status = scrollbar_mouse_status_to_message(
            scrollbar_mouse_action(mas.scroll.bar, mouse, mas.scroll.mouse_x, mas.scroll.mouse_y));
        mas.result.pointer = BROWSER_POINTER_DEFAULT;
    ...
    case ACTION_SUBMIT:
        res = form_submit(content_get_url(c), browser_window_find_target(bw, mas.gadget.target, mouse),
            mas.gadget.control->form, mas.gadget.control);
        break;

    case ACTION_NAVIGATE:
        res = browser_window_navigate(browser_window_find_target(bw, mas.link.target, mouse), mas.link.url,
            content_get_url(c), BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
        break;

    case ACTION_JS:
        path = nsurl_get_component(mas.link.url, NSURL_PATH);
        if (path != NULL) {
            html_exec(c, lwc_string_data(path), lwc_string_length(path));
...
bool html_keypress(struct content *c, uint32_t key)
{
    ...
    if (html->layout != NULL && html->layout->node != NULL) {
        fire_dom_keyboard_event(corestring_dom_keydown, html->layout->node, true, true, key);
    }
    ...
    int x, int y, int height, const struct rect *clip)
{
    ...
    int y_off = 0;
    struct rect cr;
    bool textarea_lost_focus = html->focus_type == HTML_FOCUS_TEXTAREA && focus_type != HTML_FOCUS_TEXTAREA;
    ...
    html_content *html, html_selection_type selection_type, union html_selection_owner selection_owner, bool read_only)
{
    ...
    if ((selection_type == HTML_SELECTION_NONE && html->selection_type != HTML_SELECTION_NONE) ||
        (selection_type != HTML_SELECTION_NONE && html->selection_type == HTML_SELECTION_NONE))
    ...
    if (html->selection_type != HTML_SELECTION_NONE) {
        ...
        case HTML_SELECTION_TEXTAREA:
            if (same_type && html->selection_owner.textarea == selection_owner.textarea)
                break;
            box = html->selection_owner.textarea;
            textarea_clear_selection(box->gadget->data.text.ta);
            break;
        case HTML_SELECTION_CONTENT:
            if (same_type && html->selection_owner.content == selection_owner.content)
                break;
    ...
    content_broadcast((struct content *)html, CONTENT_MSG_SELECTION, &msg_data);
}
