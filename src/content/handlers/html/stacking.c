...
bool box_creates_stacking_context(const struct box *box)
{
	...
	bool is_positioned = (pos == CSS_POSITION_RELATIVE ||
			  ...
			  int y_parent)
{
	...
	if (ctx->count >= ctx->capacity) {
		size_t new_capacity = (ctx->capacity == 0) ? 16
							   : ctx->capacity * 2;
		struct zindex_entry *new_entries = realloc(
			ctx->entries, new_capacity * sizeof(*new_entries));
		if (new_entries == NULL) {
			return false;
		}
...
void stacking_context_sort(struct stacking_context *ctx)
{
	...
	stable_sort(ctx->entries,
		    ctx->count,
		    sizeof(*ctx->entries),
		    zindex_entry_compare);
}
				      ...
				      int y_parent)
{
	...
	if (z != Z_INDEX_AUTO) {
		...
		struct stacking_context *ctx = (z < 0) ? negative_ctx
						       : positive_ctx;
	...
	for (c = box->children; c; c = c->next) {
		...
		if (!stacking_context_collect_subtree(negative_ctx,
						      positive_ctx,
						      c,
						      x_offset,
						      y_offset)) {
