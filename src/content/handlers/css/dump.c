...
void nscss_dump_computed_style(FILE *stream, const css_computed_style *style)
{
    ...
    if (val == CSS_BACKGROUND_IMAGE_IMAGE && url != NULL) {
        fprintf(stream, "background-image: url('%.*s') ", (int)lwc_string_length(url), lwc_string_data(url));
    ...
    case CSS_CONTENT_SET:
        ...
        while (content->type != CSS_COMPUTED_CONTENT_NONE) {
            ...
            case CSS_COMPUTED_CONTENT_STRING:
                fprintf(stream, "\"%.*s\"", (int)lwc_string_length(content->data.string),
                    lwc_string_data(content->data.string));
                break;
            case CSS_COMPUTED_CONTENT_URI:
                fprintf(stream, "uri(\"%.*s\")", (int)lwc_string_length(content->data.uri),
                    lwc_string_data(content->data.uri));
                break;
            case CSS_COMPUTED_CONTENT_COUNTER:
                fprintf(stream, "counter(%.*s)", (int)lwc_string_length(content->data.counter.name),
                    lwc_string_data(content->data.counter.name));
                break;
            case CSS_COMPUTED_CONTENT_COUNTERS:
                fprintf(stream, "counters(%.*s, \"%.*s\")", (int)lwc_string_length(content->data.counters.name),
                    lwc_string_data(content->data.counters.name), (int)lwc_string_length(content->data.counters.sep),
                    lwc_string_data(content->data.counters.sep));
                break;
            case CSS_COMPUTED_CONTENT_ATTR:
                fprintf(stream, "attr(%.*s)", (int)lwc_string_length(content->data.attr),
    ...
    if ((val == CSS_COUNTER_INCREMENT_NONE) || (counter == NULL)) {
        ...
        while (counter->name != NULL) {
            fprintf(stream, " %.*s ", (int)lwc_string_length(counter->name), lwc_string_data(counter->name));
    ...
    if (string_list != NULL) {
        while (*string_list != NULL) {
            fprintf(stream, " url\"%.*s\")", (int)lwc_string_length(*string_list), lwc_string_data(*string_list));
    ...
    if (val != CSS_FONT_FAMILY_INHERIT) {
        ...
        if (string_list != NULL) {
            while (*string_list != NULL) {
                fprintf(stream, " \"%.*s\"", (int)lwc_string_length(*string_list), lwc_string_data(*string_list));
    ...
    if (url != NULL) {
        fprintf(stream, "list-style-image: url('%.*s') ", (int)lwc_string_length(url), lwc_string_data(url));
