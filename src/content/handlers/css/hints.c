...
static bool parse_number(const char *data, bool maybe_negative, bool real, css_fixed *value, size_t *consumed)
{
    ...
    *consumed = 0;
    ...
    if (real && len > 1 && ptr[0] == '.' && ('0' <= ptr[1] && ptr[1] <= '9')) {
        ptr++;
        len--;
...
{
    static const struct colour_map named_colours[] = {{"aliceblue", 0xfff0f8ff}, {"antiquewhite", 0xfffaebd7},
        {"aqua", 0xff00ffff}, {"aquamarine", 0xff7fffd4}, {"azure", 0xfff0ffff}, {"beige", 0xfff5f5dc},
        {"bisque", 0xffffe4c4}, {"black", 0xff000000}, {"blanchedalmond", 0xffffebcd}, {"blue", 0xff0000ff},
        {"blueviolet", 0xff8a2be2}, {"brown", 0xffa52a2a}, {"burlywood", 0xffdeb887}, {"cadetblue", 0xff5f9ea0},
        {"chartreuse", 0xff7fff00}, {"chocolate", 0xffd2691e}, {"coral", 0xffff7f50}, {"cornflowerblue", 0xff6495ed},
        {"cornsilk", 0xfffff8dc}, {"crimson", 0xffdc143c}, {"cyan", 0xff00ffff}, {"darkblue", 0xff00008b},
        {"darkcyan", 0xff008b8b}, {"darkgoldenrod", 0xffb8860b}, {"darkgray", 0xffa9a9a9}, {"darkgreen", 0xff006400},
        ...
        {"dimgrey", 0xff696969}, {"dodgerblue", 0xff1e90ff}, {"feldspar", 0xffd19275}, {"firebrick", 0xffb22222},
        {"floralwhite", 0xfffffaf0}, {"forestgreen", 0xff228b22}, {"fuchsia", 0xffff00ff}, {"gainsboro", 0xffdcdcdc},
        {"ghostwhite", 0xfff8f8ff}, {"gold", 0xffffd700}, {"goldenrod", 0xffdaa520}, {"gray", 0xff808080},
        {"green", 0xff008000}, {"greenyellow", 0xffadff2f}, {"grey", 0xff808080}, {"honeydew", 0xfff0fff0},
        ...
        {"lightseagreen", 0xff20b2aa}, {"lightskyblue", 0xff87cefa}, {"lightslateblue", 0xff8470ff},
        ...
        {"skyblue", 0xff87ceeb}, {"slateblue", 0xff6a5acd}, {"slategray", 0xff708090}, {"slategrey", 0xff708090},
        {"snow", 0xfffffafa}, {"springgreen", 0xff00ff7f}, {"steelblue", 0xff4682b4}, {"tan", 0xffd2b48c},
        {"teal", 0xff008080}, {"thistle", 0xffd8bfd8}, {"tomato", 0xffff6347}, {"turquoise", 0xff40e0d0},
        {"violet", 0xffee82ee}, {"violetred", 0xffd02090}, {"wheat", 0xfff5deb3}, {"white", 0xffffffff},
        {"whitesmoke", 0xfff5f5f5}, {"yellow", 0xffffff00}, {"yellowgreen", 0xff9acd32}};
    const struct colour_map *entry;

    entry = bsearch(name, named_colours, sizeof(named_colours) / sizeof(named_colours[0]), sizeof(named_colours[0]),
        cmp_colour_name);
...
{
    hint_ctx.hints = malloc(sizeof(struct css_hint) * MAX_HINTS_PER_ELEMENT);
...
static void css_hint_table_cell_border_padding(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        ...
        if (parse_dimension(dom_string_data(attr), false, &hint_length.value, &hint_length.unit) &&
            INTTOFIX(0) != hint_length.value) {

            for (hint_prop = CSS_PROP_BORDER_TOP_STYLE; hint_prop <= CSS_PROP_BORDER_LEFT_STYLE; hint_prop++) {
                hint->prop = hint_prop;
                hint->status = CSS_BORDER_STYLE_INSET;
                css_hint_advance(&hint);
            }

            for (hint_prop = CSS_PROP_BORDER_TOP_WIDTH; hint_prop <= CSS_PROP_BORDER_LEFT_WIDTH; hint_prop++) {
                hint->prop = hint_prop;
    ...
    exc = dom_element_get_attribute(tablenode, corestring_dom_bordercolor, &attr);

    if (exc == DOM_NO_ERR && attr != NULL) {
        uint32_t hint_prop;
        css_color hint_color;

        if (nscss_parse_colour((const char *)dom_string_data(attr), &hint_color)) {

            for (hint_prop = CSS_PROP_BORDER_TOP_COLOR; hint_prop <= CSS_PROP_BORDER_LEFT_COLOR; hint_prop++) {
                hint->prop = hint_prop;
    ...
    exc = dom_element_get_attribute(tablenode, corestring_dom_cellpadding, &attr);
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        ...
        if (parse_dimension(dom_string_data(attr), false, &hint_length.value, &hint_length.unit)) {
            ...
            for (hint_prop = CSS_PROP_PADDING_TOP; hint_prop <= CSS_PROP_PADDING_LEFT; hint_prop++) {
                hint->prop = hint_prop;
...
static void css_hint_vertical_align_table_cells(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        ...
        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_middle)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
            hint->status = CSS_VERTICAL_ALIGN_MIDDLE;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_bottom)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
            hint->status = CSS_VERTICAL_ALIGN_BOTTOM;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_baseline)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
...
static void css_hint_vertical_align_replaced(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_top)) {
            ...
            dom_string_caseless_lwc_isequal(attr, corestring_lwc_baseline)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
            hint->status = CSS_VERTICAL_ALIGN_BASELINE;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_texttop)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
            hint->status = CSS_VERTICAL_ALIGN_TEXT_TOP;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_absmiddle) ||
            dom_string_caseless_lwc_isequal(attr, corestring_lwc_abscenter)) {
            hint->prop = CSS_PROP_VERTICAL_ALIGN;
            hint->status = CSS_VERTICAL_ALIGN_MIDDLE;
            css_hint_advance(&hint);
        }
...
static void css_hint_text_align_normal(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && align != NULL) {
        if (dom_string_caseless_lwc_isequal(align, corestring_lwc_left)) {
            hint->prop = CSS_PROP_TEXT_ALIGN;
            hint->status = CSS_TEXT_ALIGN_LEFT;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(align, corestring_lwc_center)) {
            hint->prop = CSS_PROP_TEXT_ALIGN;
            hint->status = CSS_TEXT_ALIGN_CENTER;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(align, corestring_lwc_right)) {
            hint->prop = CSS_PROP_TEXT_ALIGN;
...
static void css_hint_margin_left_right_align_center(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        ...
        if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_center) ||
            dom_string_caseless_lwc_isequal(attr, corestring_lwc_abscenter) ||
            dom_string_caseless_lwc_isequal(attr, corestring_lwc_middle) ||
            dom_string_caseless_lwc_isequal(attr, corestring_lwc_absmiddle)) {
...
static void css_hint_text_align_special(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && align != NULL) {
        if (dom_string_caseless_lwc_isequal(align, corestring_lwc_center)) {
            hint->prop = CSS_PROP_TEXT_ALIGN;
            hint->status = CSS_TEXT_ALIGN_LIBCSS_CENTER;
            css_hint_advance(&hint);

        } else if (dom_string_caseless_lwc_isequal(align, corestring_lwc_left)) {
            hint->prop = CSS_PROP_TEXT_ALIGN;
...
static void css_hint_margin_left_right_hr(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (exc == DOM_NO_ERR && attr != NULL) {
        memset(hint, 0, sizeof(*hint) * 2);
        if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_left)) {
            hint->prop = CSS_PROP_MARGIN_LEFT;
            hint->data.length.value = 0;
            hint->data.length.unit = CSS_UNIT_PX;
            hint->status = CSS_MARGIN_SET;
        ...
        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_center)) {
            hint->prop = CSS_PROP_MARGIN_LEFT;
            hint->status = CSS_MARGIN_AUTO;
            css_hint_advance(&hint);
        ...
        } else if (dom_string_caseless_lwc_isequal(attr, corestring_lwc_right)) {
            hint->prop = CSS_PROP_MARGIN_LEFT;
            hint->status = CSS_MARGIN_AUTO;
            css_hint_advance(&hint);
...
static void css_hint_table_spacing_border(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    exc = dom_element_get_attribute(node, corestring_dom_bordercolor, &attr);
    ...
    exc = dom_element_get_attribute(node, corestring_dom_cellspacing, &attr);

    if (exc == DOM_NO_ERR && attr != NULL) {
        if (parse_dimension((const char *)dom_string_data(attr), false, &hint->data.position.h.value,
...
static void css_hint_height(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        if (parse_dimension(
                (const char *)dom_string_data(attr), false, &hint->data.length.value, &hint->data.length.unit)) {
...
static void css_hint_height_width_canvas(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        if (parse_dimension(
                (const char *)dom_string_data(attr), true, &hint->data.length.value, &hint->data.length.unit)) {
            hint->prop = CSS_PROP_HEIGHT;
            hint->data.length.unit = CSS_UNIT_PX;
            hint->status = CSS_HEIGHT_SET;
            css_hint_advance(&hint);
...
static void css_hint_width_input(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        if (parse_dimension(
            ...
            err = dom_element_get_attribute(node, corestring_dom_type, &attr2);
            if (err == DOM_NO_ERR) {

                hint->prop = CSS_PROP_WIDTH;
                hint->status = CSS_WIDTH_SET;

                if (attr2 == NULL || dom_string_caseless_lwc_isequal(attr2, corestring_lwc_text) ||
                    dom_string_caseless_lwc_isequal(attr2, corestring_lwc_search) ||
                    dom_string_caseless_lwc_isequal(attr2, corestring_lwc_password) ||
                    dom_string_caseless_lwc_isequal(attr2, corestring_lwc_file)) {
                    hint->data.length.unit = CSS_UNIT_EX;
                }
                if (attr2 != NULL) {
...
static void css_hint_anchor_color(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (is_visited) {
        ...
        err = dom_element_get_attribute(bodynode, corestring_dom_link, &color);
    }

    if (err == DOM_NO_ERR && color != NULL) {
        if (nscss_parse_colour((const char *)dom_string_data(color), &hint->data.color)) {
...
static void css_hint_font_size(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && size != NULL) {
        if (parse_font_size((const char *)dom_string_data(size), &hint->status, &hint->data.length.value,
...
static void css_hint_bg_color(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && bgcolor != NULL) {
        if (nscss_parse_colour((const char *)dom_string_data(bgcolor), &hint->data.color)) {
...
static void css_hint_bg_image(nscss_select_ctx *ctx, dom_node *node)
{
    ...
    if (err == DOM_NO_ERR && attr != NULL) {
        nsurl *url;
        nserror error = nsurl_join(ctx->base_url, (const char *)dom_string_data(attr), &url);
        dom_string_unref(attr);

        if (error == NSERROR_OK) {
            lwc_string *iurl;
            lwc_error lerror = lwc_intern_string(nsurl_access(url), nsurl_length(url), &iurl);
            nsurl_unref(url);
