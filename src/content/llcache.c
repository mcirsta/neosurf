...
static nserror llcache_object_user_new(llcache_handle_callback cb, void *pw, llcache_object_user **user)
{
    ...
    NSLOG(llcache, DEBUG, "Created user %p (%p, %p, %p)", u, h, (void *)cb, pw);
...
static nserror llcache_send_event_to_users(llcache_object *object, llcache_event *event)
{
    ...
    if (event->type == LLCACHE_EVENT_ERROR && event->data.error.code == NSERROR_OK) {
        NSLOG(llcache, ERROR,
            "llcache_send_event_to_users: LLCACHE_EVENT_ERROR with NSERROR_OK! Msg: %s. Object state: %d",
            event->data.error.msg ? event->data.error.msg : "NULL", object->fetch.state);
    }
...
static nserror llcache_post_data_clone(const llcache_post_data *orig, llcache_post_data **clone)
{
    ...
    if (orig->type == LLCACHE_POST_URL_ENCODED) {
        ...
        post_clone->data.multipart = fetch_multipart_data_clone(orig->data.multipart);
...
static nserror llcache_fetch_header_cache_control(llcache_object *object, char *name, char *value)
{
    ...
    case 4:
        if (strcasecmp(name, "Date") == 0) {
            /* extract Date header */
            res = nsc_strntimet(value, strlen(value), &object->cache.date);
            if (res != NSERROR_OK) {
                NSLOG(llcache, INFO, "Processing Date header value \"%s\" returned %d", value, res);
            }
            ...
            if (object->cache.etag == NULL) {
                NSLOG(llcache, INFO, "No memory to duplicate ETag");
                return NSERROR_NOMEM;
            }
        }
        break;
    ...
    case 13:
        if (strcasecmp(name, "Cache-Control") == 0) {
            ...
            nsc_strntimet(value, strlen(value), &object->cache.last_modified);
        }
        break;
    }
...
static nserror llcache_fetch_process_header(llcache_object *object, const uint8_t *data, size_t len)
{
    ...
    temp = realloc(object->headers, (object->num_headers + 1) * sizeof(llcache_header));
    if (temp == NULL) {
        free(name);
        free(value);
        return NSERROR_NOMEM;
...
static nserror get_referer_header(nsurl *url, nsurl *referer, char **header_out)
{
    ...
    if (lwc_string_isequal(scheme, corestring_lwc_https, &match1) != lwc_error_ok) {
        match1 = false;
    }
    if (lwc_string_isequal(ref_scheme, corestring_lwc_http, &match2) != lwc_error_ok) {
        match2 = false;
    }
    if (match == true || (match1 == true && match2 == true)) {
        const size_t len = SLEN("Referer: ") + nsurl_length(referer) + 1;

        header = malloc(len);
        if (header == NULL) {
            res = NSERROR_NOMEM;
        } else {
            snprintf(header, len, "Referer: %s", nsurl_access(referer));
...
static nserror llcache_object_refetch(llcache_object *object)
{
    ...
    if (object->cache.etag != NULL) {
        const size_t len = SLEN("If-None-Match: ") + strlen(object->cache.etag) + 1;
        ...
        snprintf(headers[header_idx], len, "If-None-Match: %s", object->cache.etag);
    ...
    if (object->cache.last_modified != 0) {
        ...
        snprintf(headers[header_idx], len, "If-Modified-Since: %s", rfc1123_date(object->cache.last_modified));
    ...
    if (object->fetch.referer != NULL) {
        if (get_referer_header(object->url, object->fetch.referer, &headers[header_idx]) == NSERROR_OK) {
            header_idx++;
        }
    }
    ...
    /* Kick off fetch */
    res = fetch_start(object->url, object->fetch.referer, llcache_fetch_callback, object,
        object->fetch.flags & LLCACHE_RETRIEVE_NO_ERROR_PAGES, &postdata,
        object->fetch.flags & LLCACHE_RETRIEVE_VERIFIABLE, object->fetch.tried_with_tls_downgrade,
        (const char **)headers, &object->fetch.fetch);
 ...
 * \return NSERROR_OK on success, appropriate error otherwise
 */
static nserror llcache_object_fetch(llcache_object *object, uint32_t flags, nsurl *referer,
    const llcache_post_data *post, uint32_t redirect_count, bool hsts_in_use)
{
    nserror error;
...
static nserror llcache_object_destroy(llcache_object *object)
{
    ...
    if (object->fetch.post != NULL) {
        if (object->fetch.post->type == LLCACHE_POST_URL_ENCODED) {
            ...
            fetch_multipart_data_destroy(object->fetch.post->data.multipart);
        }
...
static int llcache_object_rfc2616_remaining_lifetime(const llcache_cache_control *cd)
{
    ...
    if ((cd->no_cache == LLCACHE_VALIDATE_FRESH) && (freshness_lifetime > current_age)) {
...
static bool llcache_object_is_fresh(const llcache_object *object)
{
    ...
    NSLOG(llcache, DEBUG, "%p: (%d > 0 || %d != %d)", object, remaining_lifetime, object->fetch.state,
    ...
    return ((cd->no_cache == LLCACHE_VALIDATE_FRESH) &&
        ((remaining_lifetime > 0) || (object->fetch.state != LLCACHE_FETCH_COMPLETE)));
}
...
static nserror llcache_retrieve_persisted_data(llcache_object *object)
{
    ...
    if ((object->source_data != NULL) || (object->store_state != LLCACHE_STATE_DISC)) {
...
static nserror llcache_serialise_metadata(llcache_object *object, uint8_t **data_out, size_t *datasize_out)
{
    ...
    /* headers */
    for (hloop = 0; hloop < object->num_headers; hloop++) {
        use = snprintf(op, datasize, "%s:%s", object->headers[hloop].name, object->headers[hloop].value);
        if (use < 0) {
            goto operror;
        }
        use++; /* does not count the null */
    ...
    for (hloop = 0; hloop < cert_chain_depth; hloop++) {
        ...
        use = snprintf(op, datasize, "%d", (int)(object->chain->certs[hloop].err));
        if (use < 0) {
            goto operror;
        }
        use++; /* does not count the null */
        ...
        if (object->chain->certs[hloop].der != NULL) {
            ...
            res = nsu_base64_encode(
                object->chain->certs[hloop].der, object->chain->certs[hloop].der_length, (uint8_t *)op, &output_length);
            if (res != NSUERROR_OK) {
                goto operror;
            }
            use = output_length;
...
static nserror llcache_process_metadata(llcache_object *object)
{
    ...
    res = guit->llcache->fetch(object->url, BACKING_STORE_META, &metadata, &metadatalen);
    if (res != NSERROR_OK) {
        return res;
    }
    ...
    if (nsurl_compare(object->url, metadataurl, NSURL_COMPLETE) != true) {
        ...
        NSLOG(llcache, INFO, "Got metadata for %s instead of %s", nsurl_access(metadataurl), nsurl_access(object->url));
    ...
    for (hloop = 0; hloop < ssl_cert_count; hloop++) {
        ...
        if (errcode < SSL_CERT_ERR_OK || errcode > SSL_CERT_ERR_MAX_KNOWN) {
        ...
        remaining -= lnsize + 1;
        if (lnsize > 0) {
            nsures = nsu_base64_decode_alloc(
                (const uint8_t *)ln, lnsize, &chain->certs[hloop].der, &chain->certs[hloop].der_length);
...
static inline bool llcache__scheme_is_persistable(const nsurl *url)
{
    ...
    if ((lwc_string_isequal(scheme, corestring_lwc_http, &match) == lwc_error_ok && (match == true)) ||
        (lwc_string_isequal(scheme, corestring_lwc_https, &match) == lwc_error_ok && (match == true))) {
        persistable = true;
    }
 ...
 */
static nserror llcache_object_fetch_persistent(
    llcache_object *object, uint32_t flags, nsurl *referer, const llcache_post_data *post, uint32_t redirect_count)
{
    nserror error;
    nsurl *referer_clone = NULL;
    llcache_post_data *post_clone = NULL;
 ...
 * \return NSERROR_OK on success, appropriate error otherwise
 */
static nserror llcache_object_retrieve_from_cache(nsurl *url, uint32_t flags, nsurl *referer,
    const llcache_post_data *post, uint32_t redirect_count, bool hsts_in_use, llcache_object **result)
{
    nserror error;
    llcache_object *obj, *newest = NULL;

    NSLOG(llcache, DEBUG, "Searching cache for %s flags:%" PRIx32 " referer:%s post:%p", nsurl_access(url), flags,
    ...
    for (obj = llcache->cached_objects; obj != NULL; obj = obj->next) {
        ...
        if ((newest == NULL || obj->cache.req_time > newest->cache.req_time) &&
            nsurl_compare(obj->url, url, NSURL_COMPLETE) == true) {
            newest = obj;
        }
    }
    ...
    if ((newest != NULL) && (llcache_object_is_fresh(newest))) {
        ...
        if (error == NSERROR_OK) {
            ...
            /* Record candidate, so we can fall back if it is still
             * fresh */
            newest->candidate_count++;
            obj->candidate = newest;

            /* Attempt to kick-off fetch */
            error = llcache_object_fetch(obj, flags, referer, post, redirect_count, hsts_in_use);
            if (error != NSERROR_OK) {
                newest->candidate_count--;
                llcache_object_destroy(obj);
                return error;
    ...
    uint32_t redirect_count, bool hsts_in_use, llcache_object **result)
{
    ...
    if (uncachable) {
        ...
        error = llcache_object_retrieve_from_cache(
            defragmented_url, flags, referer, post, redirect_count, hsts_in_use, &obj);
        if (error != NSERROR_OK) {
            nsurl_unref(defragmented_url);
            return error;
        }
...
static nserror llcache_hsts_transform_url(nsurl *url, nsurl **result, bool *hsts_in_use)
{
    ...
    scheme = nsurl_get_component(url, NSURL_SCHEME);
    if (lwc_string_caseless_isequal(scheme, corestring_lwc_http, &match) != lwc_error_ok || match == false) {
        /* Non-HTTP fetch: no transform required */
        if (lwc_string_caseless_isequal(scheme, corestring_lwc_https, &match) == lwc_error_ok && match) {
    ...
    if (urldb_get_hsts_enabled(url)) {
         ...
         * it's redundant) */
        error = nsurl_replace_scheme(url, corestring_lwc_https, result);
...
static nserror llcache_hsts_update_policy(llcache_object *object)
{
    ...
    if (lwc_string_caseless_isequal(scheme, corestring_lwc_https, &match) != lwc_error_ok || match == false) {
    ...
    for (i = 0; i < object->num_headers; i++) {
        if (strcasecmp("Strict-Transport-Security", object->headers[i].name) == 0) {
            urldb_set_hsts_policy(object->url, object->headers[i].value);
...
static nserror llcache_fetch_notmodified(llcache_object *object, llcache_object **replacement)
{
    ...
    if (object->candidate != NULL) {
        ...
        if (object->candidate->cache.no_cache == LLCACHE_VALIDATE_ONCE) {
    ...
    if (object->fetch.state != LLCACHE_FETCH_DATA) {
        ...
        if ((http_code != 200 && http_code != 203) ||
            (nsurl_has_component(object->url, NSURL_QUERY) &&
                (object->cache.max_age == INVALID_AGE && object->cache.expires == 0))) {
            /* Invalidate cache control data */
            llcache_invalidate_cache_control_data(object);
        }
...
static nserror llcache_fetch_ssl_error(llcache_object *object)
{
    ...
    if (object->fetch.hsts_in_use || object->fetch.tried_with_tls_downgrade) {
...
static nserror write_backing_store(struct llcache_object *object, size_t *written_out, unsigned long *elapsed)
{
    ...
    ret = guit->llcache->store(object->url, BACKING_STORE_NONE, object->source_data, object->source_len);
    if (ret != NSERROR_OK) {
...
static void llcache_persist_slowcheck(void *p)
{
    ...
    if (llcache->total_written > (2 * llcache->minimum_bandwidth)) {
        ...
        if (total_bandwidth < llcache->minimum_bandwidth) {
            NSLOG(llcache, INFO, "Current bandwidth %" PRIu64 " less than minimum %" PRIsizet, total_bandwidth,
...
static void llcache_persist(void *p)
{
    ...
    for (idx = 0; idx < lst_count; idx++) {
        ...
        NSLOG(llcache, DEBUG, "Wrote %" PRIsizet " bytes in %lums bw:%" PRIsizet " %s", written, elapsed,
            (written * 1000) / elapsed, nsurl_access(lst[idx]->url));
        ...
        if (total_elapsed > llcache->time_quantum) {
            ...
            if (total_bandwidth < llcache->minimum_bandwidth) {
                 ...
                 *  overall performance is too slow to be
                 * useful.
                 */
                guit->misc->schedule(llcache->time_quantum * 100, llcache_persist_slowcheck, NULL);
                break;
            } else {
    ...
    NSLOG(llcache, DEBUG, "writeout size:%" PRIsizet " time:%lu bandwidth:%lubytes/s", total_written, total_elapsed,
        total_bandwidth);
...
static void llcache_fetch_callback(const fetch_msg *msg, void *p)
{
    ...
    case FETCH_HEADER:
        ...
        error = llcache_fetch_process_header(object, msg->data.header_or_data.buf, msg->data.header_or_data.len);
        break;
    ...
    case FETCH_DATA:
        /* Received some data */
        error = llcache_fetch_process_data(object, msg->data.header_or_data.buf, msg->data.header_or_data.len);
        break;
    ...
    case FETCH_FINISHED:
        /* Finished fetching */
        {
            ...
            temp = realloc(object->source_data, object->source_len);
    ...
    case FETCH_CERTS:
        ...
        if (error != NSERROR_OK) {
            NSLOG(llcache, ERROR, "Unable to duplicate cert chain into cache: %s", messages_get_errorcode(error));
        }
    ...
    for (user = object->users; user != NULL; user = next_user) {
        ...
        if (handle->state != objstate) {
            if (emitted_notify == false) {
                NSLOG(llcache, DEBUG, "Notifying users of %p", object);
                emitted_notify = true;
            }

            NSLOG(llcache, DEBUG, "User %p state: %d Object state: %d", user, handle->state, objstate);
        }

        /* User: INIT, Obj: HEADERS, DATA, COMPLETE => User->HEADERS */
        if (handle->state == LLCACHE_FETCH_INIT && objstate > LLCACHE_FETCH_INIT) {
        ...
        if (handle->state == LLCACHE_FETCH_HEADERS && objstate > LLCACHE_FETCH_HEADERS) {
        ...
        if (handle->state == LLCACHE_FETCH_DATA && objstate >= LLCACHE_FETCH_DATA &&
            ...
            event.data.data.buf = object->source_data + handle->bytes;
...
static nserror llcache_object_snapshot(llcache_object *object, llcache_object **snapshot)
{
    ...
    if (object->num_headers > 0) {
        newobj->headers = calloc(object->num_headers, sizeof(llcache_header));
 ...
 */
static inline uint32_t total_object_size(llcache_object *object)
{
    uint32_t tot;
    size_t hdrc;
...
static void llcache_catch_up_all_users(void *ignored)
{
    ...
    for (object = llcache->cached_objects; object != NULL; object = object->next) {
...
void llcache_clean(bool purge)
{
    ...
    for (object = llcache->uncached_objects; object != NULL; object = next) {
        ...
        if ((object->users == NULL) && (object->candidate_count == 0) && (object->fetch.fetch == NULL)) {
            NSLOG(llcache, DEBUG, "Discarding uncachable object with no users (%p) %s", object,
                nsurl_access(object->url));

            llcache_object_remove_from_list(object, &llcache->uncached_objects);
     ...
     */
    for (object = llcache->cached_objects; ((limit < llcache_size) && (object != NULL)); object = next) {
        next = object->next;
        if ((object->users == NULL) && (object->candidate_count == 0) && (object->fetch.fetch == NULL) &&
            (object->store_state == LLCACHE_STATE_DISC)) {
            guit->llcache->release(object->url, BACKING_STORE_NONE);
    ...
    for (object = llcache->cached_objects; ((limit < llcache_size) && (object != NULL)); object = next) {
        next = object->next;
        if ((object->users == NULL) && (object->candidate_count == 0) && (object->fetch.fetch == NULL) &&
            (object->store_state == LLCACHE_STATE_DISC) && (object->source_data == NULL)) {
...
nserror llcache_initialise(const struct llcache_parameters *prm)
{
    ...
    NSLOG(llcache, INFO, "llcache initialising with a limit of %" PRIu32 " bytes", llcache->limit);

    NSLOG(llcache, INFO, "llcache backing store init: path=%s limit=%" PRIsizet " hyst=%" PRIsizet,
        prm->store.path ? prm->store.path : "(null)", prm->store.limit, prm->store.hysteresis);
...
void llcache_finalise(void)
{
    ...
    NSLOG(llcache, INFO,
        "Backing store wrote %" PRIu64 " bytes in %" PRIu64 " ms "
...
/* Exported interface documented in content/llcache.h */
nserror llcache_handle_retrieve(nsurl *url, uint32_t flags, nsurl *referer, const llcache_post_data *post,
...
nserror llcache_handle_abort(llcache_handle *handle)
{
    ...
    if (all_alone == false) {
        ...
        if (user->iterator_target) {
            ...
            llcache_object_user *newuser = calloc(1, sizeof(llcache_object_user));
            if (newuser == NULL) {
        ...
        /* Add new object to uncached list */
        llcache_object_add_to_list(newobject, &llcache->uncached_objects);
    } else {
