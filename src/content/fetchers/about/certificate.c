...
static int ns_EVP_PKEY_get_bn_param(const EVP_PKEY *pkey, const char *key_name, BIGNUM **bn)
{
    RSA *rsa;
    BIGNUM *result = NULL;
...
static int ns_EVP_PKEY_get_utf8_string_param(
    const EVP_PKEY *pkey, const char *key_name, char *str, size_t max_len, size_t *out_len)
{
...
static int ns_EVP_PKEY_get_octet_string_param(
    const EVP_PKEY *pkey, const char *key_name, unsigned char *buf, size_t max_len, size_t *out_len)
{
    const EC_GROUP *ecgroup;
    ...
    if (ecgroup != NULL) {
        ...
        if (ecpoint != NULL) {
            bnctx = BN_CTX_new();
            len = EC_POINT_point2oct(ecgroup, ecpoint, POINT_CONVERSION_UNCOMPRESSED, NULL, 0, bnctx);
            if (len != 0 && len <= max_len) {
                if (EC_POINT_point2oct(ecgroup, ecpoint, POINT_CONVERSION_UNCOMPRESSED, buf, len, bnctx) == len)
                    ret = 1;
            }
            if (out_len != NULL)
                *out_len = len;
...
static nserror ec_to_info(EVP_PKEY *pkey, struct ns_cert_pkey *ikey)
{
    ...
    if (len != 0) {
        ...
        if (ikey->curve != NULL) {
            if (ns_EVP_PKEY_get_utf8_string_param(pkey, "group", ikey->curve, len + 1, NULL) == 0) {
                free(ikey->curve);
                ikey->curve = NULL;
            }
        }
    ...
    len = 0;
    ns_EVP_PKEY_get_octet_string_param(pkey, "encoded-pub-key", NULL, 0, &len);
    if (len != 0) {
        unsigned char *point = malloc(len);
        if (point != NULL) {
            if (ns_EVP_PKEY_get_octet_string_param(pkey, "encoded-pub-key", point, len, NULL) == 1) {
...
static nserror der_to_certinfo(const uint8_t *der, size_t der_length, struct ns_cert_info *info)
{
    BIO *mem;
    BUF_MEM *buf;
...
{
    nserror res;
    res = fetch_about_ssenddataf(ctx, "<tr><th>Common Name</th><td>%s</td></tr>\n", cert_name->common_name);
    if (res != NSERROR_OK) {
        return res;
    }

    if (cert_name->organisation != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Organisation</th><td>%s</td></tr>\n", cert_name->organisation);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (cert_name->organisation_unit != NULL) {
        res = fetch_about_ssenddataf(
            ctx, "<tr><th>Organisation Unit</th><td>%s</td></tr>\n", cert_name->organisation_unit);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (cert_name->locality != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Locality</th><td>%s</td></tr>\n", cert_name->locality);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (cert_name->province != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Province</th><td>%s</td></tr>\n", cert_name->province);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (cert_name->country != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Country</th><td>%s</td></tr>\n", cert_name->country);
        if (res != NSERROR_OK) {
            return res;
        }
    }
...
static nserror format_certificate_san(struct fetch_about_context *ctx, struct ns_cert_san *san)
{
    ...
    while (san != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>DNS Name</th><td>%s</td></tr>\n", san->name);
        if (res != NSERROR_OK) {
            return res;
        }
...
static nserror format_certificate_public_key(struct fetch_about_context *ctx, struct ns_cert_pkey *public_key)
{
    ...
    res = fetch_about_ssenddataf(ctx,
        ...
        public_key->algor, public_key->size);
    if (res != NSERROR_OK) {
        return res;
    }
    ...
    if (public_key->exponent != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Exponent</th><td>%s</td></tr>\n", public_key->exponent);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (public_key->modulus != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Modulus</th><td class=\"data\">%s</td></tr>\n", public_key->modulus);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (public_key->curve != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Curve</th><td>%s</td></tr>\n", public_key->curve);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (public_key->public != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Public Value</th><td>%s</td></tr>\n", public_key->public);
        if (res != NSERROR_OK) {
            return res;
        }
    }
...
static nserror format_certificate_fingerprint(struct fetch_about_context *ctx, struct ns_cert_info *cert_info)
{
    ...
    if ((cert_info->sha1fingerprint == NULL) && (cert_info->sha256fingerprint == NULL)) {
    ...
    if (cert_info->sha256fingerprint != NULL) {
        res = fetch_about_ssenddataf(
            ctx, "<tr><th>SHA-256</th><td class=\"data\">%s</td></tr>\n", cert_info->sha256fingerprint);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    if (cert_info->sha1fingerprint != NULL) {
        res = fetch_about_ssenddataf(
            ctx, "<tr><th>SHA-1</th><td class=\"data\">%s</td></tr>\n", cert_info->sha1fingerprint);
        if (res != NSERROR_OK) {
            return res;
        }
    }
...
static nserror format_certificate(struct fetch_about_context *ctx, struct ns_cert_info *cert_info, size_t depth)
{
    ...
    if (cert_info->serialnum != NULL) {
        res = fetch_about_ssenddataf(ctx, "<tr><th>Serial Number</th><td>%s</td></tr>\n", cert_info->serialnum);
        if (res != NSERROR_OK) {
            return res;
        }
    }
