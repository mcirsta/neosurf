...
bool fetch_about_query_privacy_handler(struct fetch_about_context *ctx)
{
    ...
    res = get_query_description(siteurl, "PrivacyDescription", &description);
    if (res == NSERROR_OK) {
    ...
    if (chainurl == NULL) {
        res = fetch_about_ssenddataf(ctx,
            "<div><p>%s</p></div>"
            "<div><p>%s</p></div>",
            reason, messages_get("ViewCertificatesNotPossible"));
    } else {
        res = fetch_about_ssenddataf(ctx,
            "<div><p>%s</p></div>"
            "<div><p><a href=\"%s\" target=\"_blank\">%s</a></p></div>",
            reason, chainurl, messages_get("ViewCertificates"));
    }
    if (res != NSERROR_OK) {
        goto fetch_about_query_ssl_handler_aborted;
    }
    res = fetch_about_ssenddataf(ctx,
        ...
        "value=\"%s\">"
        "</div>",
        messages_get("Backtosafety"), messages_get("Proceed"));
    if (res != NSERROR_OK) {
        goto fetch_about_query_ssl_handler_aborted;
    }
