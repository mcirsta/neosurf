...
bool fetch_about_srverror(struct fetch_about_context *ctx)
{
    ...
    res = fetch_about_ssenddataf(ctx, "%s 500 %s", messages_get("FetchErrorCode"), messages_get("HTTP500"));
    if (res != NSERROR_OK) {
        return false;
    }
...
struct about_handlers about_handler_list[] = {{"credits", SLEN("credits"), NULL, fetch_about_credits_handler, false},
    ...
    {"nscolours.css", SLEN("nscolours.css"), NULL, fetch_about_nscolours_handler, true},
    {"logo", SLEN("logo"), NULL, fetch_about_logo_handler, true},
    {/* details about the image cache */
        "imagecache", SLEN("imagecache"), NULL, fetch_about_imagecache_handler, true},
    {/* The default blank page */
        "blank", SLEN("blank"), NULL, fetch_about_blank_handler, true},
    {/* details about a certificate */
        "certificate", SLEN("certificate"), NULL, fetch_about_certificate_handler, true},
    {/* chart generator */
        "chart", SLEN("chart"), NULL, fetch_about_chart_handler, true},
    {"query/auth", SLEN("query/auth"), NULL, fetch_about_query_auth_handler, true},
    {"query/ssl", SLEN("query/ssl"), NULL, fetch_about_query_privacy_handler, true},
    {"query/timeout", SLEN("query/timeout"), NULL, fetch_about_query_timeout_handler, true},
    {"query/fetcherror", SLEN("query/fetcherror"), NULL, fetch_about_query_fetcherror_handler, true},
    {/* web search using the configured provider */
        "websearch", SLEN("websearch"), NULL, fetch_about_websearch_handler, true}};
...
static bool fetch_about_404_handler(struct fetch_about_context *ctx)
{
    ...
    res = fetch_about_ssenddataf(ctx,
        ...
        "<p>%s %d %s %s</p>\n"
        "</body>\n</html>\n",
        title, title, messages_get("FetchErrorCode"), 404, messages_get("FetchFile"), nsurl_access(ctx->url));
    if (res != NSERROR_OK) {
        return false;
    }
...
static bool fetch_about_initialise(lwc_string *scheme)
{
    ...
    for (abt_loop = 0; abt_loop < about_handler_list_len; abt_loop++) {
        error = lwc_intern_string(about_handler_list[abt_loop].name, about_handler_list[abt_loop].name_len,
...
nserror fetch_about_register(void)
{
    ...
    const struct fetcher_operation_table fetcher_ops = {.initialise = fetch_about_initialise,
