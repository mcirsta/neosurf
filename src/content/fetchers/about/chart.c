...
static nserror ensure_label_count(struct chart_param *chart, unsigned int count)
{
    ...
    nlabels = realloc(chart->data.label, count * sizeof(struct chart_label));
    if (nlabels == NULL) {
 ...
 */
static nserror
extract_series_values(struct chart_param *chart, unsigned int series_num, const char *valstr, size_t valstrlen)
{
    nserror res;
    unsigned int valcur;
    ...
    for (valcur = 0, vallen = 0, valstart = 0; (valstart < valstrlen) && (valcur < series->len);
        valstart += vallen, valcur++) {
        /* get query section length */
        vallen = 0;
        while (((valstart + vallen) < valstrlen) && (valstr[valstart + vallen] != ',')) {
            vallen++;
        }
 ...
 */
static nserror extract_next_series_values(struct chart_param *chart, const char *valstr, size_t valstrlen)
{
    nserror res;
    ...
    res = extract_series_values(chart, chart->data.series_len, valstr, valstrlen);
    if (res == NSERROR_OK) {
 ...
 * extract label title
 */
static nserror extract_series_labels(struct chart_param *chart, const char *valstr, size_t valstrlen)
{
    nserror res;
    ...
    for (valcur = 0, vallen = 0, valstart = 0; (valstart < valstrlen) && (valcur < chart->data.label_len);
        valstart += vallen, valcur++) {
        /* get query section length */
        vallen = 0;
        while (((valstart + vallen) < valstrlen) && (valstr[valstart + vallen] != ',')) {
 ...
 */
static nserror extract_series_colours(struct chart_param *chart, const char *valstr, size_t valstrlen)
{
    return NSERROR_OK;
}
 ...
 */
static nserror process_query_section(const char *str, size_t len, struct chart_param *chart)
{
...
static nserror output_pie_legend(struct fetch_about_context *ctx, struct chart_param *chart)
{
    nserror res;
    unsigned int lblidx;
    ...
    case CHART_KEY_RIGHT:
        ...
        for (lblidx = 0; lblidx < chart->data.label_len; lblidx++) {
            res = fetch_about_ssenddataf(ctx, "<rect  x=\"%d\" y=\"%d\" width=\"%d\" height=\"%d\" fill=\"#%06x\" />",
                chart->width - legend_width, (vertical_spacing * lblidx) + (vertical_spacing / 2),
                vertical_spacing * 2 / 3, vertical_spacing * 2 / 3, chart->data.label[lblidx].colour);
            if (res != NSERROR_OK) {
                return res;
            }
            res = fetch_about_ssenddataf(ctx, "<text x=\"%d\" y=\"%d\" fill=\"#%06x\" >%s</text>",
                chart->width - legend_width + vertical_spacing, vertical_spacing * (lblidx + 1),
                chart->data.label[lblidx].colour, chart->data.label[lblidx].title);
            if (res != NSERROR_OK) {
                return res;
            }
        }
...
static float compute_series_total(struct chart_param *chart, unsigned int series)
{
    float total;
    unsigned int curdata;

    for (total = 0, curdata = 0; curdata < chart->data.series[series].len; curdata++) {
 ...
 */
static bool pie_chart(struct fetch_about_context *ctx, struct chart_param *chart)
{
    nserror res;
    ...
    /* content type */
    if (fetch_about_send_header(ctx, "Content-Type: image/svg; charset=utf-8")) {
        goto aborted;
    }
    ...
    for (curdata = 0; curdata < chart->data.series[0].len; curdata++) {
        ...
        res = fetch_about_ssenddataf(ctx,
            ...
            "A %g %g 0 %d 1 %g %g\n"
            "L %g %g Z\" fill=\"#%06x\" />\n",
            circle_centre_x + last_x, circle_centre_y + last_y, ra, rb, large ? 1 : 0, circle_centre_x + end_x,
            circle_centre_y + end_y, circle_centre_x, circle_centre_y, chart->data.label[curdata].colour);
        if (res != NSERROR_OK) {
            goto aborted;
        }
        last_x = end_x;
