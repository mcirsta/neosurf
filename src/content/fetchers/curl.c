...
#define CIPHER_LIST                                                                                                    \
    /* disable everything */                                                                                           \
    "-ALL:" /* enable TLSv1.2 ECDHE AES GCM suites */                                                                  \
    "EECDH+AESGCM+TLSv1.2:" /* enable ECDHE CHACHA20/POLY1305 suites */                                                \
    "EECDH+CHACHA20:" /* Sort above by strength */                                                                     \
    "@STRENGTH:" /* enable ECDHE (auth=RSA, mac=SHA1) AES CBC suites */                                                \
    "EECDH+aRSA+AES+SHA1"
...
#define CIPHER_LIST_LEGACY                                                                                             \
    /* as above */                                                                                                     \
    CIPHER_LIST ":" /* enable (non-PFS) RSA AES GCM suites */                                                          \
                "RSA+AESGCM:" /* enable (non-PFS) RSA (mac=SHA1) AES CBC                                               \
                         suites */                                                                                     \
                "RSA+AES+SHA1"
...
static bool curl_fetch_ssl_key_eq(void *key1, void *key2)
{
    ...
    if (lwc_string_isequal(hostname1, hostname2, &iseq) != lwc_error_ok || iseq == false)
        goto out;
...
static void curl_fetch_ssl_value_destroy(void *value)
{
    ...
    if (cert_chain_free(chain) != NSERROR_OK) {
        NSLOG(wisp, WARNING, "Problem freeing SSL certificate chain");
    }
}
...
struct curl_fetch_info {
    ...
    NSCURL_POSTDATA_T
    *curl_postdata; /**< POST data in curl representation */
...
{
    NSLOG(wisp, INFO, "Initialise cURL fetcher for %s", lwc_string_data(scheme));
...
static void fetch_curl_finalise(lwc_string *scheme)
{
    ...
    curl_fetchers_registered--;
    NSLOG(wisp, INFO, "Finalise cURL fetcher %s", lwc_string_data(scheme));
    if (curl_fetchers_registered == 0) {
        CURLMcode codem;
        /* All the fetchers have been finalised. */
        NSLOG(wisp, INFO, "All cURL fetchers finalised, closing down cURL");
        ...
        NSLOG(wisp, DEBUG, "Cleaning up SSL cert chain hashmap");
 ...
 * clone postdata
 */
static struct fetch_postdata *fetch_curl_clone_postdata(const struct fetch_postdata *src)
{
    ...
    const struct fetch_postdata *postdata, const char **headers)
{
    ...
    if (fetch->postdata == NULL && postdata != NULL && postdata->type != FETCH_POSTDATA_NONE) {
        /* Only fail if allocation failed for actual data.
           If postdata was NULL or NONE, fetch->postdata being NULL is
           fine/expected if clone returns NULL for that case. However,
           fetch_curl_clone_postdata returns a struct even for NONE,
           unless src is NULL. If src is NULL, we get NULL. If src is
           NONE, we get a struct with NONE. So checking result against
           NULL is enough if we expect a struct always unless src was
           NULL. If src is NULL, fetch->postdata is NULL.
        */
        goto failed;
    }
    /* Use a simpler check matching original logic which allocated postdata
     * even for NONE */
...
#define APPEND(list, value)                                                                                            \
    ...
    if ((nsoption_charp(accept_language) != NULL) && (nsoption_charp(accept_language)[0] != '\0')) {
        char s[80];
        snprintf(s, sizeof s, "Accept-Language: %s, *;q=0.1", nsoption_charp(accept_language));
        s[sizeof s - 1] = 0;
    ...
    if (nsoption_charp(accept_charset) != NULL && nsoption_charp(accept_charset)[0] != '\0') {
        char s[80];
        snprintf(s, sizeof s, "Accept-Charset: %s, *;q=0.1", nsoption_charp(accept_charset));
        s[sizeof s - 1] = 0;
        APPEND(fetch->headers, s);
    }
 ...
 */
static struct cert_chain *fetch_curl_get_cached_chain(struct curl_fetch_info *f)
{
 ...
 */
static void fetch_curl_store_certs_in_cache(struct curl_fetch_info *f)
{
    size_t depth;
    BIO *mem;
    ...
    if (depth > 0) {
        ...
        for (depth = fetch->cert_depth; depth > 0; depth--) {
            if (fetch->cert_data[depth].err != 0) {
                /* error in previous certificate so fail
                 * verification */
                verify_ok = 0;
 ...
 * \return 1 to indicate verification success and 0 to indicate verification
 * failure.
 */
static int fetch_curl_cert_verify_callback(X509_STORE_CTX *x509_ctx, void *parm)
{
    ...
    ok = X509_VERIFY_PARAM_set1_host(vparam, lwc_string_data(f->host), lwc_string_length(f->host));
 ...
 */
static CURLcode fetch_curl_sslctxfun(CURL *curl_handle, void *_sslctx, void *parm)
{
    struct curl_fetch_info *f = (struct curl_fetch_info *)parm;
 ...
 */
static curl_mime *fetch_curl_postdata_convert(CURL *chandle, const struct fetch_multipart_data *multipart)
{
    curl_mime *cmime;
    curl_mimepart *part;
    ...
    if (cmime == NULL) {
        NSLOG(wisp, WARNING, "postdata conversion failed to curl mime context");
        return NULL;
    }
    ...
    for (; multipart != NULL; multipart = multipart->next) {
        ...
        if (multipart->file && value_len == 0) {
            /* file entries with no filename require special
             * handling */
            ...
            cb_ctx->position = 0;
            code = curl_mime_data_cb(
                part, value_len, mime_data_read_callback, mime_data_seek_callback, mime_data_free_callback, cb_ctx);
            if (code != CURLE_OK) {
                free(cb_ctx);
                goto convert_failed;
            }
...
convert_failed:
    NSLOG(wisp, WARNING, "postdata conversion failed with curl code: %d", code);
    ...
    for (; control; control = control->next) {
        if (control->file) {
            ...
            if (control->value[0] == '\0') {
                ...
                code = curl_formadd(&post, &last, CURLFORM_COPYNAME, control->name, CURLFORM_FILE, control->rawfile,
                    CURLFORM_FILENAME, leafname, CURLFORM_CONTENTTYPE, (mimetype != 0 ? mimetype : "text/plain"),
                    CURLFORM_END);
                if (code != CURL_FORMADD_OK)
                    NSLOG(wisp, INFO, "curl_formadd: %d (%s=%s)", code, control->name, control->value);
                free(mimetype);
            }
            free(leafname);
        } else {
            code = curl_formadd(
                &post, &last, CURLFORM_COPYNAME, control->name, CURLFORM_COPYCONTENTS, control->value, CURLFORM_END);
            if (code != CURL_FORMADD_OK)
                NSLOG(wisp, INFO, "curl_formadd: %d (%s=%s)", code, control->name, control->value);
        }
    }
...
#define SETOPT(option, value)                                                                                          \
    ...
    if (nsoption_bool(http_proxy) && (nsoption_charp(http_proxy_host) != NULL) &&
        ...
        if (nsoption_int(http_proxy_auth) != OPTION_HTTP_PROXY_AUTH_NONE) {
            SETOPT(CURLOPT_PROXYAUTH,
                nsoption_int(http_proxy_auth) == OPTION_HTTP_PROXY_AUTH_BASIC ? (long)CURLAUTH_BASIC
                                                                              : (long)CURLAUTH_NTLM);
            snprintf(fetch_proxy_userpwd, sizeof fetch_proxy_userpwd, "%s:%s", nsoption_charp(http_proxy_auth_user),
    ...
    if (curl_with_openssl) {
        SETOPT(CURLOPT_SSL_CIPHER_LIST, f->downgrade_tls ? CIPHER_LIST_LEGACY : CIPHER_LIST);
    }
...
static bool fetch_curl_initiate_fetch(struct curl_fetch_info *fetch, CURL *handle)
{
    ...
    if (code != CURLE_OK) {
        ...
        NSLOG(wisp, WARNING, "cURL handle maybe went bad, retry later");
...
static bool fetch_curl_start(void *vfetch)
{
    ...
    NSLOG(wisp, DEBUG, "PROFILER: STOP Fetch queue %p", fetch);
    NSLOG(wisp, DEBUG, "PROFILER: START Fetch latency %p", fetch);
...
static void fetch_curl_cache_handle(CURL *handle, lwc_string *host)
{
    ...
    /* We do not have a handle cached, first up determine if the cache is
     * full */
    ...
    /* The table isn't full yet, so make a shiny new handle to add to the
     * ring */
...
static void fetch_curl_stop(struct curl_fetch_info *f)
{
    ...
    if (f->curl_handle) {
        ...
        codem = curl_multi_remove_handle(fetch_curl_multi, f->curl_handle);
...
static void fetch_curl_abort(void *vf)
{
    ...
    if (f->curl_handle) {
        if (inside_curl) {
            ...
            NSLOG(wisp, DEBUG, "Immediate abort");
            fetch_curl_stop(f);
...
static bool fetch_curl_process_headers(struct curl_fetch_info *f)
{
    ...
    if (f->only_2xx && strncmp(nsurl_access(f->url), "http", 4) == 0 && (http_code < 200 || 299 < http_code)) {
...
static void fetch_curl_done(CURL *curl_handle, CURLcode result)
{
    ...
    if (f->profiled_response_started) {
        ...
        NSLOG(wisp, DEBUG, "PROFILER: STOP Fetch latency %p", f);
    }

    if ((abort_fetch == false) && (result == CURLE_OK || ((result == CURLE_WRITE_ERROR) && (f->stopped == false)))) {
         ...
         */
        if (f->stopped || (!f->had_headers && fetch_curl_process_headers(f))) {
         ...
         */
        ;
    } else if (result == CURLE_SSL_PEER_CERTIFICATE || result == CURLE_SSL_CACERT) {
        ...
        NSLOG(wisp, INFO, "cURL error %d: %s", result, curl_easy_strerror(result));
        error = true;
    }
...
static void fetch_curl_poll(lwc_string *scheme_ignored)
{
    ...
    if (nsoption_bool(suppress_curl_debug) == false) {
        ...
        codem = curl_multi_fdset(fetch_curl_multi, &read_fd_set, &write_fd_set, &exc_fd_set, &max_fd);
        assert(codem == CURLM_OK);

        NSLOG(wisp, DEEPDEBUG, "Curl file descriptor states (maxfd=%i):", max_fd);
        for (i = 0; i <= max_fd; i++) {
            bool read = false;
            bool write = false;
            bool error = false;
            ...
            if (read || write || error) {
                NSLOG(wisp, DEEPDEBUG, "  fd %i: %s %s %s", i, read ? "read" : "    ", write ? "write" : "     ",
                    error ? "error" : "     ");
            }
        }
    }
    ...
    while (curl_msg) {
        switch (curl_msg->msg) {
        case CURLMSG_DONE:
            fetch_curl_done(curl_msg->easy_handle, curl_msg->data.result);
            break;
        default:
            break;
        }
...
#undef UPDATE_DELAY_MS
    ...
    if (dltotal > 0) {
        snprintf(fetch_progress_buffer, 255, messages_get("Progress"), human_friendly_bytesize(dlnow),
...
static size_t fetch_curl_header(char *data, size_t size, size_t nmemb, void *_f)
{
    ...
    if (f->profiled_response_started == false) {
        NSLOG(wisp, DEBUG, "PROFILER: STOP Fetch latency %p", f);
        NSLOG(wisp, DEBUG, "PROFILER: START Fetch download %p", f);
...
static int fetch_curl_fdset(lwc_string *scheme, fd_set *read_set, fd_set *write_set, fd_set *error_set)
{
    ...
    code = curl_multi_fdset(fetch_curl_multi, read_set, write_set, error_set, &maxfd);
...
nserror fetch_curl_register(void)
{
    ...
    int i;
    lwc_string *scheme;
    const struct fetcher_operation_table fetcher_ops = {.initialise = fetch_curl_initialise,
    ...
    NSLOG(wisp, INFO, "curl_version %s", curl_version());
...
#define SETOPT(option, value)                                                                                          \
    ...
    if (mcode != CURLM_OK) {                                                                                           \
        NSLOG(wisp, ERROR, "attempting curl_multi_setopt(%s, ...)", #option);                                       \
...
#define SETOPT(option, value)                                                                                          \
    ...
    if (code != CURLE_OK) {                                                                                            \
        NSLOG(wisp, ERROR, "attempting curl_easy_setopt(%s, ...)", #option);                                        \
    ...
    if (nsoption_charp(ca_bundle) && strcmp(nsoption_charp(ca_bundle), "")) {
        NSLOG(wisp, INFO, "ca_bundle: '%s'", nsoption_charp(ca_bundle));
        SETOPT(CURLOPT_CAINFO, nsoption_charp(ca_bundle));
    }
    if (nsoption_charp(ca_path) && strcmp(nsoption_charp(ca_path), "")) {
        NSLOG(wisp, INFO, "ca_path: '%s'", nsoption_charp(ca_path));
    ...
    if (curl_with_openssl) {
         ...
         * TLS1.3 support */
        code = curl_easy_setopt(fetch_blank_curl, CURLOPT_TLS13_CIPHERS, CIPHER_SUITES);
    ...
    NSLOG(wisp, INFO, "cURL %slinked against openssl", curl_with_openssl ? "" : "not ");
    ...
    if (curl_fetch_ssl_hashmap == NULL) {
        NSLOG(wisp, CRITICAL, "Unable to initialise SSL certificate hashmap");
    ...
    for (i = 0; data->protocols[i]; i++) {
        ...
        if (fetcher_add(scheme, &fetcher_ops) != NSERROR_OK) {
            NSLOG(wisp, INFO, "Unable to register cURL fetcher for %s", data->protocols[i]);
        }
    }
...
curl_easy_setopt_failed:
    NSLOG(wisp, INFO, "curl_easy_setopt failed.");
    return NSERROR_INIT_FAILED;
