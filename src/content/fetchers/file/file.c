...
static inline bool fetch_file_send_callback(const fetch_msg *msg, struct fetch_file_context *ctx)
{
    ctx->locked = true;
...
static void fetch_file_process_plain(struct fetch_file_context *ctx, struct stat *fdstat)
{
    ...
    /* content type */
    if (fetch_file_send_header(ctx, "Content-Type: %s", guit->fetch->filetype(ctx->path))) {
        goto fetch_file_process_aborted;
    }

    /* content length */
    if (fetch_file_send_header(ctx, "Content-Length: %" PRIsizet, fdstat->st_size)) {
        goto fetch_file_process_aborted;
    }
...
fetch_file_process_aborted:
    ...
    if (buf == NULL) {
        msg.type = FETCH_ERROR;
        msg.data.error = "Unable to allocate memory for file data buffer";
        fetch_file_send_callback(&msg, ctx);
        fclose(infile);
        return;
    }
 ...
 */
static nserror
process_dir_ent(struct fetch_file_context *ctx, struct dirent *ent, bool even, char *buffer, size_t buffer_len)
{
    nserror ret;
    ...
    if (S_ISREG(ent_stat.st_mode)) {
        ...
        dirlist_generate_row(
            even, true, url, ent->d_name, messages_get("FileDirectory"), -1, datebuf, timebuf, buffer, buffer_len);
    } else {
        /* something else */
        dirlist_generate_row(even, false, url, ent->d_name, "", -1, datebuf, timebuf, buffer, buffer_len);
    }
...
static void fetch_file_process_dir(struct fetch_file_context *ctx, struct stat *fdstat)
{
    ...
    for (i = 0; i < n; i++) {
        ...
        err = process_dir_ent(ctx, listing[i], even, buffer, sizeof(buffer));
...
nserror fetch_file_register(void)
{
    ...
    const struct fetcher_operation_table fetcher_ops = {.initialise = fetch_file_initialise,
