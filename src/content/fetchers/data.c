...
static bool fetch_data_process(struct fetch_data_context *c)
{
    ...
    if (c->mimetype == NULL) {
        msg.type = FETCH_ERROR;
        msg.data.error = "Unable to allocate memory for mimetype in data: URL";
        fetch_data_send_callback(&msg, c);
        return false;
    }
    ...
    if (c->base64) {
        if ((nsu_base64_decode_alloc((uint8_t *)unescaped, unescaped_len, (uint8_t **)&c->data, &c->datalen) !=
                NSUERROR_OK) ||
            (c->data == NULL)) {
            msg.type = FETCH_ERROR;
...
static void fetch_data_poll(lwc_string *scheme)
{
    ...
    while (ring != NULL) {
        ...
        if (c->aborted == false && fetch_data_process(c) == true) {
             ...
             */
            fetch_data_send_header(c, "Content-Type: %s", c->mimetype);

            if (c->aborted == false) {
                fetch_data_send_header(c, "Content-Length: %" PRIsizet, c->datalen);
            }
...
nserror fetch_data_register(void)
{
    ...
    const struct fetcher_operation_table fetcher_ops = {.initialise = fetch_data_initialise,
