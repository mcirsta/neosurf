...
static inline bool fetch_resource_send_callback(const fetch_msg *msg, struct fetch_resource_context *ctx)
{
    ctx->locked = true;
...
static bool fetch_resource_data_handler(struct fetch_resource_context *ctx)
{
    ...
    /* content length */
    if (fetch_resource_send_header(ctx, "Content-Length: %" PRIsizet, ctx->entry->data_len)) {
        goto fetch_resource_data_aborted;
    }

    /* create etag */
    if (fetch_resource_send_header(ctx, "ETag: \"%10" PRId64 "\"", (int64_t)DIRECT_ETAG_VALUE)) {
        goto fetch_resource_data_aborted;
    }

    /* create max-age of 1 year */
    if (fetch_resource_send_header(ctx, "Cache-Control: max-age=31536000")) {
        goto fetch_resource_data_aborted;
    }
...
static bool fetch_resource_notfound_handler(struct fetch_resource_context *ctx)
{
    ...
    snprintf(buffer, sizeof buffer,
        ...
        "<p>%s %d %s %s</p>\n"
        "</body>\n</html>\n",
        title, title, code, messages_get("FetchErrorCode"), code, messages_get("FetchFile"), nsurl_access(ctx->url));
    ...
    const struct fetch_postdata *postdata, const char **headers)
{
    ...
    if ((path = nsurl_get_component(url, NSURL_PATH)) != NULL) {
        ...
        for (i = 0; i < fetch_resource_path_count; i++) {
            if (lwc_string_isequal(path, fetch_resource_map[i].path, &match) == lwc_error_ok && match) {
...
nserror fetch_resource_register(void)
{
    ...
    const struct fetcher_operation_table fetcher_ops = {.initialise = fetch_resource_initialise,
