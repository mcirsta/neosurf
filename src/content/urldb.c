...
static struct search_node *search_trees[NUM_SEARCH_TREES] = {&empty, &empty, &empty, &empty, &empty, &empty, &empty,
 ...
 */
static void urldb_write_paths(const struct path_data *parent, const char *host, FILE *fp, char **path, int *path_alloc,
    int *path_used, time_t expiry)
{
    ...
    do {
        ...
        if (*path_alloc < len) {
            char *temp;
            temp = realloc(*path, (len > 64) ? len : *path_alloc + 64);
            if (!temp) {
                return;
            }
            *path = temp;
        ...
        *path_used = len;
        ...
        if (p->children != NULL) {
            ...
            /* leaf node */
            if (p->persistent || ((p->urld.last_visit > expiry) && (p->urld.visits > 0))) {
 ...
 * \return true to continue, false otherwise
 */
static bool urldb_iterate_partial_host(
    struct search_node *root, const char *prefix, bool (*callback)(nsurl *url, const struct url_data *data))
{
    int c;
    ...
    if (c > 0) {
        ...
        return urldb_iterate_partial_host(root->left, prefix, callback);
    } else if (c < 0) {
        /* No match => look in right subtree */
        return urldb_iterate_partial_host(root->right, prefix, callback);
    } else {
        /* Match => iterate over l/r subtrees & process this node */
        if (!urldb_iterate_partial_host(root->left, prefix, callback)) {
            return false;
        }

        if (root->data->paths.children) {
            /* and extract all paths attached to this host */
            if (!urldb_iterate_entries_path(&root->data->paths, callback, NULL)) {
                return false;
            }
        }

        if (!urldb_iterate_partial_host(root->right, prefix, callback)) {
            return false;
        }
    }
...
static bool urldb_iterate_partial_path(
    const struct path_data *parent, const char *prefix, bool (*callback)(nsurl *url, const struct url_data *data))
{
    ...
    bool (*cookie_callback)(const struct cookie_data *data))
{
    ...
    if ((parent->data->paths.children) || ((cookie_callback) && (parent->data->paths.cookies))) {
        ...
        if (!urldb_iterate_entries_path(&parent->data->paths, url_callback, cookie_callback)) {
            return false;
        }
    }
...
static struct path_data *urldb_add_path_fragment(struct path_data *segment, lwc_string *fragment)
{
    ...
    temp = realloc(segment->fragment, (segment->frag_cnt + 1) * sizeof(char *));
    if (!temp)
        return NULL;

    segment->fragment = temp;
    segment->fragment[segment->frag_cnt] = strdup(lwc_string_data(fragment));
    ...
    qsort(segment->fragment, segment->frag_cnt, sizeof(char *), urldb_add_path_fragment_cmp);
 ...
 * \return Pointer to added node, or NULL on memory exhaustion
 */
static struct path_data *urldb_add_path_node(
    lwc_string *scheme, unsigned int port, const char *segment, lwc_string *fragment, struct path_data *parent)
{
...
urldb_match_path(const struct path_data *parent, const char *path, lwc_string *scheme, unsigned short port)
{
    ...
    while (p != NULL) {
        ...
        if (strncmp(p->segment, path + 1, slash - path - 1) == 0 &&
            lwc_string_isequal(p->scheme, scheme, &match) == lwc_error_ok && match == true && p->port == port) {
            if (*slash == '\0') {
                /* Complete match */
...
static struct path_data *urldb_find_url(nsurl *url)
{
    ...
    if (lwc_string_isequal(scheme, corestring_lwc_mailto, &match) == lwc_error_ok && match == true) {
    ...
    } else if (lwc_string_isequal(scheme, corestring_lwc_file, &match) == lwc_error_ok && match == true) {
        host_str = "localhost";
...
static int urldb_search_match_host(const struct host_part *a, const struct host_part *b)
{
    ...
    for (; a && a != &db_root && b && b != &db_root; a = a->parent, b = b->parent) {
...
static struct search_node *urldb_search_insert_internal(struct search_node *root, struct search_node *n)
{
    ...
    if (root == &empty) {
        ...
        if (c > 0) {
            root->left = urldb_search_insert_internal(root->left, n);
        } else if (c < 0) {
            root->right = urldb_search_insert_internal(root->right, n);
        } else {
            /* exact match */
            free(n);
            return root;
...
static bool urldb_parse_avpair(struct cookie_internal_data *c, char *n, char *v, bool was_quoted)
{
    ...
    if (!c->comment && strcasecmp(n, "Comment") == 0) {
         ...
         * useful purpose) */
        for (datenoday = v; *datenoday && !ascii_is_digit(*datenoday); datenoday++) {
            /* do nothing */
        }
 ...
 */
static struct path_data *urldb_add_path(lwc_string *scheme, unsigned int port, const struct host_part *host,
    char *path_query, lwc_string *fragment, nsurl *url)
{
    struct path_data *d, *e;
    char *buf = path_query;
    char *segment, *slash;
    ...
    do {
        ...
        if (!slash) {
            ...
            for (e = d->children; e; e = e->next)
                if (strcmp(segment, e->segment) == 0 && lwc_string_isequal(scheme, e->scheme, &match) == lwc_error_ok &&
                    match == true && e->port == port)
                    break;

            d = e ? urldb_add_path_fragment(e, fragment) : urldb_add_path_node(scheme, port, segment, fragment, d);
            break;
        }
...
static bool urldb_insert_cookie(struct cookie_internal_data *c, lwc_string *scheme, nsurl *url)
{
    ...
    if (c->domain[0] == '.') {
        h = urldb_search_find(urldb_get_search_tree(&(c->domain[1])), c->domain + 1);
        if (!h) {
        ...
        h = urldb_search_find(urldb_get_search_tree(c->domain), c->domain);
    ...
    for (d = p->cookies; d; d = d->next) {
        if (!strcmp(d->domain, c->domain) && !strcmp(d->path, c->path) && !strcmp(d->name, c->name))
            break;
    }
        ...
        if (p->cookies != NULL) {
            ...
            for (c = p->cookies; c != NULL; c = c->next) {
                ...
                fprintf(fp,
                    "%d\t%s\t%d\t%s\t%d\t%d\t%d\t%d\t%d\t%d\t"
                    "%s\t%s\t%d\t%s\t%s\t%s\n",
                    c->version, c->domain, c->domain_from_set, c->path, c->path_from_set, c->secure, c->http_only,
                    (int)c->expires, (int)c->last_used, c->no_destroy, c->name, c->value, c->value_was_quoted,
                    p->scheme ? lwc_string_data(p->scheme) : "unused", p->url ? nsurl_access(p->url) : "unused",
                    c->comment ? c->comment : "");
            }
        }
...
bool urldb_add_url(nsurl *url)
{
    ...
    /* Copy and merge path/query strings */
    if (nsurl_get(url, NSURL_PATH | NSURL_QUERY, &path_query, &len) != NSERROR_OK) {
        return false;
    }
...
void urldb_set_auth_details(nsurl *url, const char *realm, const char *auth)
{
    ...
    for (space = h->prot_space; space; space = space->next) {
        if (!strcmp(space->realm, realm) && lwc_string_isequal(space->scheme, p->scheme, &match) == lwc_error_ok &&
            match == true && space->port == p->port)
            break;
    }
...
bool urldb_set_hsts_policy(struct nsurl *url, const char *header)
{
    ...
    h->hsts.include_sub_domains = http_strict_transport_security_include_subdomains(sts);
    ...
    if (slash) {
        ...
        if (!h) {
            ...
            if (len <= 3 || strncasecmp(host, "www.", 4) != 0) {
                ...
                h = urldb_search_find(search_trees[ST_DN + 'w' - 'a'], buf);
                if (!h)
                    return;
            } else
                return;
        ...
        if (len <= 3 || strncasecmp(prefix, "www.", 4) != 0) {
            ...
            if (!urldb_iterate_partial_host(search_trees[ST_DN + 'w' - 'a'], buf, callback))
                return;
        }
    }
}
...
void urldb_iterate_entries(bool (*callback)(nsurl *url, const struct url_data *data))
{
    ...
    for (i = 0; i < NUM_SEARCH_TREES; i++) {
        if (!urldb_iterate_entries_host(search_trees[i], callback, NULL)) {
            break;
        }
    }
}
...
void urldb_iterate_cookies(bool (*callback)(const struct cookie_data *data))
{
    ...
    for (i = 0; i < NUM_SEARCH_TREES; i++) {
        if (!urldb_iterate_entries_host(search_trees[i], NULL, callback))
            break;
    }
}
...
bool urldb_set_cookie(const char *header, nsurl *url, nsurl *referer)
{
    ...
    if (referer) {
        ...
        if (lwc_string_isequal(host, rhost, &match) == lwc_error_ok && match == false) {
            const char *hptr;
            const char *rptr;
            const char *dot;
            ...
            if (urldb__host_is_ip_address(host_data) || urldb__host_is_ip_address(rhost_data)) {
    ...
    do {
        ...
        if (strcasecmp(lwc_string_data(host), c->domain) != 0) {
            ...
            if (strcasecmp(lwc_string_data(host) + (hlen - dlen), domain)) {
                urldb_free_cookie(c);
                goto error;
            }
...
char *urldb_get_cookie(nsurl *url, bool include_http_only)
{
    ...
    matched_cookies = malloc(matched_cookies_size * sizeof(struct cookie_internal_data *));
    if (!matched_cookies)
        return NULL;
...
#define GROW_MATCHED_COOKIES                                                                                           \
    do {                                                                                                               \
        if (count == matched_cookies_size) {                                                                           \
            ...
            temp = realloc(matched_cookies, (matched_cookies_size + 20) * sizeof(struct cookie_internal_data *));      \
                                                                                                                       \
            if (temp == NULL) {                                                                                        \
                free(ret);                                                                                             \
    ...
    if (*(p->segment) != '\0') {
        ...
        for (q = p->parent->children; q; q = q->next) {
            ...
            for (c = q->cookies; c; c = c->next) {
                ...
                if (c->secure && lwc_string_isequal(q->scheme, corestring_lwc_https, &match) && match == false)
    ...
    for (p = p->parent; p; p = p->parent) {
        ...
        for (c = p->cookies; c; c = c->next) {
            ...
            if (c->secure && lwc_string_isequal(p->scheme, corestring_lwc_https, &match) && match == false)
                /* Secure cookie for insecure server
                 * => ignore */
                continue;
    ...
    for (h = (const struct host_part *)p; h && h != &db_root; h = h->parent) {
        for (c = h->paths.cookies; c; c = c->next) {
            ...
            if (c->secure && lwc_string_isequal(scheme, corestring_lwc_https, &match) && match == false)
    ...
    for (i = 0; i < count; i++) {
        if (!urldb_concat_cookie(matched_cookies[i], version, &ret_used, &ret_alloc, &ret)) {
            free(ret);
            free(matched_cookies);
            return NULL;
        }
...
#define SKIP_T                                                                                                         \
    ...
    while (fgets(s, sizeof s, fp)) {
        char *p = s, *end = 0, *domain, *path, *name, *value, *scheme, *url, *comment;
        int version, domain_specified, path_specified, secure, http_only, no_destroy, value_quoted;
        ...
        if (!(c->name && c->value && c->comment && c->domain && c->path)) {
            urldb_free_cookie(c);
            break;
        }
        ...
        if (c->domain[0] != '.') {
            ...
            scheme_lwc = nsurl_get_component(url_nsurl, NSURL_SCHEME);
...
{
    FILE *fp;
    int cookie_file_version = max(loaded_cookie_file_version, COOKIE_FILE_VERSION);
