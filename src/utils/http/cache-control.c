...
http_directive_list_iterate(const http_directive *cur, lwc_string **name, lwc_string **value)
{
    if (cur == NULL)
        return NULL;
...
static uint32_t count(const http_directive *list, lwc_string *key)
{
    ...
    while (list != NULL) {
        if (lwc_string_caseless_isequal(key, list->name, &match) == lwc_error_ok && match) {
            count++;
        }
...
nserror http_parse_cache_control(const char *header_value, http_cache_control **result)
{
    ...
    if (*pos == ',') {
        error = http__item_list_parse(&pos, http__parse_directive, (http__item *)first, (http__item **)&directives);
        if (error != NSERROR_OK) {
            if (directives != NULL) {
    ...
    /* Find max-age */
    error = http_directive_list_find_item(directives, corestring_lwc_max_age, &value_str);
    ...
    /* Find no-cache */
    error = http_directive_list_find_item(directives, corestring_lwc_no_cache, &value_str);
    if (error == NSERROR_OK) {
        no_cache = true;
        if (value_str != NULL) {
            lwc_string_unref(value_str);
    ...
    /* Find no-store */
    error = http_directive_list_find_item(directives, corestring_lwc_no_store, &value_str);
    if (error == NSERROR_OK) {
        no_store = true;
        if (value_str != NULL) {
            lwc_string_unref(value_str);
