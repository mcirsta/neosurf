    ...
    do {
        ...
        if ((exc == DOM_NO_ERR) && (node_type == DOM_ELEMENT_NODE)) {
            ...
            if ((exc == DOM_NO_ERR) && (node_name != NULL)) {
                if (dom_string_caseless_lwc_isequal(node_name, element_name)) {
...
static bool dump_dom_element(dom_node *node, FILE *f, int depth)
{
    ...
    if (length == 5 && strncmp(string, "title", 5) == 0) {
        ...
        if (exc == DOM_NO_ERR && str != NULL) {
            fprintf(f, " $%.*s$", (int)dom_string_byte_length(str), dom_string_data(str));
    ...
    if (dump_dom_element_attribute(node, f, "id") == false || dump_dom_element_attribute(node, f, "class") == false) {
        /* Error occured */
        fprintf(f, "\n");
        return false;
    }
