...
bool nsurl_compare(const nsurl *url1, const nsurl *url2, nsurl_component parts)
{
    ...
    if (parts & NSURL_PATH) {
        nsurl__component_compare(url1->components.path, url2->components.path, &match);
    ...
    if (parts & NSURL_HOST) {
        nsurl__component_compare(url1->components.host, url2->components.host, &match);
    ...
    if (parts & NSURL_QUERY) {
        nsurl__component_compare(url1->components.query, url2->components.query, &match);
    ...
    if (parts & NSURL_SCHEME) {
        nsurl__component_compare(url1->components.scheme, url2->components.scheme, &match);
    ...
    if (parts & NSURL_USERNAME) {
        nsurl__component_compare(url1->components.username, url2->components.username, &match);
    ...
    if (parts & NSURL_PASSWORD) {
        nsurl__component_compare(url1->components.password, url2->components.password, &match);
    ...
    if (parts & NSURL_PORT) {
        nsurl__component_compare(url1->components.port, url2->components.port, &match);
    ...
    if (parts & NSURL_FRAGMENT) {
        nsurl__component_compare(url1->components.fragment, url2->components.fragment, &match);
...
lwc_string *nsurl_get_component(const nsurl *url, nsurl_component part)
{
    ...
    switch (part) {
    case NSURL_SCHEME:
        return (url->components.scheme != NULL) ? lwc_string_ref(url->components.scheme) : NULL;

    case NSURL_USERNAME:
        return (url->components.username != NULL) ? lwc_string_ref(url->components.username) : NULL;

    case NSURL_PASSWORD:
        return (url->components.password != NULL) ? lwc_string_ref(url->components.password) : NULL;

    case NSURL_HOST:
        return (url->components.host != NULL) ? lwc_string_ref(url->components.host) : NULL;

    case NSURL_PORT:
        return (url->components.port != NULL) ? lwc_string_ref(url->components.port) : NULL;

    case NSURL_PATH:
        return (url->components.path != NULL) ? lwc_string_ref(url->components.path) : NULL;

    case NSURL_QUERY:
        return (url->components.query != NULL) ? lwc_string_ref(url->components.query) : NULL;
...
nserror nsurl_get_utf8(const nsurl *url, char **url_s, size_t *url_l)
{
    ...
    err = idna_decode(lwc_string_data(host), lwc_string_length(host), &idna_host, &idna_host_len);
    if (err != NSERROR_OK) {
    ...
    err = nsurl_get(url, NSURL_SCHEME | NSURL_CREDENTIALS, &scheme, &scheme_len);
    if (err != NSERROR_OK) {
        goto cleanup;
    }

    err = nsurl_get(url, NSURL_PORT | NSURL_PATH | NSURL_QUERY | NSURL_FRAGMENT, &path, &path_len);
    if (err != NSERROR_OK) {
        goto cleanup;
    }
...
nserror nsurl_defragment(const nsurl *url, nsurl **no_frag)
{
    ...
    *no_frag = malloc(sizeof(nsurl) + length + 1); /* Add 1 for \0 */
    ...
    /* Copy components */
    (*no_frag)->components.scheme = nsurl__component_copy(url->components.scheme);
    (*no_frag)->components.username = nsurl__component_copy(url->components.username);
    (*no_frag)->components.password = nsurl__component_copy(url->components.password);
    (*no_frag)->components.host = nsurl__component_copy(url->components.host);
    (*no_frag)->components.port = nsurl__component_copy(url->components.port);
    (*no_frag)->components.path = nsurl__component_copy(url->components.path);
    (*no_frag)->components.query = nsurl__component_copy(url->components.query);
...
nserror nsurl_refragment(const nsurl *url, lwc_string *frag, nsurl **new_url)
{
    ...
    *pos = '\0';
    ...
    /* Copy components */
    (*new_url)->components.scheme = nsurl__component_copy(url->components.scheme);
    (*new_url)->components.username = nsurl__component_copy(url->components.username);
    (*new_url)->components.password = nsurl__component_copy(url->components.password);
    (*new_url)->components.host = nsurl__component_copy(url->components.host);
    (*new_url)->components.port = nsurl__component_copy(url->components.port);
    (*new_url)->components.path = nsurl__component_copy(url->components.path);
    (*new_url)->components.query = nsurl__component_copy(url->components.query);
...
nserror nsurl_replace_query(const nsurl *url, const char *query, nsurl **new_url)
{
    ...
    *pos = '\0';
    ...
    (*new_url)->components.fragment = nsurl__component_copy(url->components.fragment);
...
nserror nsurl_replace_scheme(const nsurl *url, lwc_string *scheme, nsurl **new_url)
{
    ...
    *new_url = malloc(sizeof(nsurl) + len + 1); /* Add 1 for \0 */
    ...
    memcpy(pos + scheme_len, url->string + url->length - base_len, base_len);
    pos[len] = '\0';
    ...
    if (lwc_string_caseless_isequal(scheme, corestring_lwc_http, &match) == lwc_error_ok && match == true) {
        (*new_url)->components.scheme_type = NSURL_SCHEME_HTTP;
    } else if (lwc_string_caseless_isequal(scheme, corestring_lwc_https, &match) == lwc_error_ok && match == true) {
        (*new_url)->components.scheme_type = NSURL_SCHEME_HTTPS;
    } else if (lwc_string_caseless_isequal(scheme, corestring_lwc_file, &match) == lwc_error_ok && match == true) {
        (*new_url)->components.scheme_type = NSURL_SCHEME_FILE;
    } else if (lwc_string_caseless_isequal(scheme, corestring_lwc_ftp, &match) == lwc_error_ok && match == true) {
        (*new_url)->components.scheme_type = NSURL_SCHEME_FTP;
    } else if (lwc_string_caseless_isequal(scheme, corestring_lwc_mailto, &match) == lwc_error_ok && match == true) {
...
nserror nsurl_nice(const nsurl *url, char **result, bool remove_extensions)
{
    ...
    *result = 0;
    ...
    if ((url->components.path != NULL) && (lwc_string_length(url->components.path) != 0) &&
        (lwc_string_isequal(url->components.path, corestring_lwc_slash_, &match) == lwc_error_ok) && (match == false)) {
        bool first = true;
        bool keep_looking;
        ...
        do {
            ...
            if (first) {
                if (strncasecmp("/default.", data + pos, SLEN("/default.")) == 0) {
                    keep_looking = true;

                } else if (strncasecmp("/index.", data + pos, 6) == 0) {
                    keep_looking = true;
                }
                first = false;
            }
        ...
        if (strncasecmp("default.", data + pos, 8) != 0 && strncasecmp("index.", data + pos, 6) != 0) {
            size_t end = pos;
...
nserror nsurl_parent(const nsurl *url, nsurl **new_url)
{
    ...
    if (old_path_len == 0) {
        ...
        if (old_path_len > 1) {
            ...
            /* Work back to next / */
            while (new_path_len > 0 && path[new_path_len - 1] != '/')
                new_path_len--;
        }
    }
    ...
    *new_url = malloc(sizeof(nsurl) + len + 1); /* Add 1 for \0 */
    ...
    if (old_path_len == 0) {
        ...
        if (lwc_intern_string(path, new_path_len, &lwc_path) != lwc_error_ok) {
            free(*new_url);
            return NSERROR_NOMEM;
        }
    }
