 ...
 */
static const char *const weekdays_short[NSC_TIME_WEEKDAY__COUNT] = {[NSC_TIME_WEEKDAY_SUN] = "Sun",
 ...
 */
static const char *const months[NSC_TIME_MONTH__COUNT] = {[NSC_TIME_MONTH_JAN] = "Jan",
...
static void nsc_gmtime_fallback(time_t t, int *year, int *month, int *day, int *hour, int *min, int *sec, int *wday)
{
...
const char *rfc1123_date(time_t t)
{
    ...
    snprintf(ret, sizeof ret, "%s, %02d %s %d %02d:%02d:%02d GMT", weekdays_short[wday], day, months[month], year, hour,
        min, sec);
 ...
 */
static const char *const weekdays_long[NSC_TIME_WEEKDAY__COUNT] = {[NSC_TIME_WEEKDAY_SUN] = "Sunday",
 ...
 */
static const int16_t timezone_mins[NSC_TIME_ZONE__COUNT] = {[NSC_TIME_ZONE_IDLE] = NSC_TIME_ZONE_OFFSET_IDLE,
 ...
 */
static const char *const timezones[NSC_TIME_ZONE__COUNT] = {[NSC_TIME_ZONE_IDLE] = "IDLE",
...
enum nsc_date_component_flags {
    ...
    NSC_COMPONENT_FLAGS__HAVE_YYYYMMDD = NSC_COMPONENT_FLAGS_HAVE_YEARS | NSC_COMPONENT_FLAGS_HAVE_MONTHS |
        NSC_COMPONENT_FLAGS_HAVE_DAYS,
    NSC_COMPONENT_FLAGS__HAVE_HHMMSS = NSC_COMPONENT_FLAGS_HAVE_HOURS | NSC_COMPONENT_FLAGS_HAVE_MINS |
    ...
    NSC_COMPONENT_FLAGS__HAVE_ALL = NSC_COMPONENT_FLAGS_HAVE_YEARS | NSC_COMPONENT_FLAGS_HAVE_MONTHS |
        ...
        NSC_COMPONENT_FLAGS_HAVE_SECS | NSC_COMPONENT_FLAGS_HAVE_TIMEZONE
};
 ...
 */
static inline bool time__parse_weekday(const char *str, size_t len, enum nsc_date_component_flags *flags)
{
    const char *const *names = (len == 3) ? weekdays_short : weekdays_long;
 ...
 */
static inline bool
time__parse_month(const char *str, size_t len, enum nsc_date_component_flags *flags, struct nsc_date_parse_ctx *ctx)
{
 ...
 */
static inline bool
time__parse_timezone(const char *str, size_t len, enum nsc_date_component_flags *flags, struct nsc_date_parse_ctx *ctx)
{
    ...
    for (uint32_t i = 0; i < NSC_TIME_ZONE__COUNT; i++) {
        if (ascii_strings_count_equal_caseless(timezones[i], str) == len) {
 ...
 */
static inline bool
time__parse_hh_mm_ss(const char *str, size_t *len, enum nsc_date_component_flags *flags, struct nsc_date_parse_ctx *ctx)
{
    size_t l;
 ...
 */
static inline bool
time__parse_number(const char *str, size_t len, enum nsc_date_component_flags *flags, struct nsc_date_parse_ctx *ctx)
{
    int value;
    ...
    case 4:
        if (!flags_chk(*flags, NSC_COMPONENT_FLAGS_HAVE_TIMEZONE)) {
            if (ascii_is_sign(ctx->prev) && value <= 1400) {
                ctx->timezone_offset_mins = value / 100 * 60 + value % 100;
                if (ctx->prev == '+') {
    ...
    case 2:
    case 1:
        if (!flags_chk(*flags, NSC_COMPONENT_FLAGS_HAVE_DAYS) && value > 0 && value <= 31) {
            ctx->day = value - 1;
            *flags |= NSC_COMPONENT_FLAGS_HAVE_DAYS;
            return true;
        }
        if (!flags_chk(*flags, NSC_COMPONENT_FLAGS_HAVE_YEARS)) {
            ctx->years = (value > 70) ? value + 1900 : value + 2000;
            *flags |= NSC_COMPONENT_FLAGS_HAVE_YEARS;
            return true;
        }
        break;
 ...
 */
static nserror
time__ctx_to_time_t(const struct nsc_date_parse_ctx *ctx, enum nsc_date_component_flags flags, time_t *time)
{
    enum {
    ...
    static const uint16_t month_offsets[NSC_TIME_MONTH__COUNT] = {[NSC_TIME_MONTH_JAN] = NSC_MONTH_OFF_JAN,
    ...
    int year = (ctx->month < NSC_TIME_MONTH_FEB) ? ctx->years - 1 : ctx->years;
    ...
    *time = (((((time_t)(total_days)) * 24) + ctx->hours) * 60 + mins) * 60 + ctx->secs;
    return NSERROR_OK;
}
...
static nserror time__get_date(const char *str, time_t *time)
{
    ...
    struct nsc_date_parse_ctx ctx = {
        .prev = '\0', .secs = 0, .mins = 0, .hours = 0, .day = 0, .month = 0, .years = 0, .timezone_offset_mins = 0};
    ...
    /* Parse */
    while (*str != '\0' && !flags_chk_all(flags, NSC_COMPONENT_FLAGS__HAVE_ALL)) {
        size_t len = 1;

        if (ascii_is_alpha(*str)) {
            len += ascii_count_alpha(str + 1);

            if (!time__parse_weekday(str, len, &flags) && !time__parse_month(str, len, &flags, &ctx) &&
            ...
            if (!time__parse_hh_mm_ss(str, &len, &flags, &ctx) && !time__parse_number(str, len, &flags, &ctx)) {
                return NSERROR_INVALID;
            }
        }
        ctx.prev = *str;
    ...
    if (ctx.secs > 60 || ctx.mins > 59 || ctx.hours > 23 || ctx.day > 31 || ctx.month > 11) {
        return NSERROR_INVALID;
    }
