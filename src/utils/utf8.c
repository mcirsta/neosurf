...
uint32_t utf8_to_ucs4(const char *s_in, size_t l)
{
    ...
    perror = parserutils_charset_utf8_to_ucs4((const uint8_t *)s_in, l, &ucs4, &len);
...
size_t utf8_char_byte_length(const char *s)
{
    ...
    perror = parserutils_charset_utf8_char_byte_length((const uint8_t *)s, &len);
...
static nserror get_cached_cd(const char *enc_from, const char *enc_to, iconv_t *cd_out)
{
    ...
    if (strncasecmp(last_cd.from, enc_from, sizeof(last_cd.from)) == 0 &&
        strncasecmp(last_cd.to, enc_to, sizeof(last_cd.to)) == 0 && last_cd.cd != 0) {
        *cd_out = last_cd.cd;
        return NSERROR_OK;
    }
 ...
 */
static nserror utf8_convert(
    const char *string, size_t slen, const char *from, const char *to, char **result_out, size_t *result_len_out)
{
    iconv_t cd;
...
nserror utf8_to_html(const char *string, const char *encname, size_t len, char **result_out)
{
    ...
    while (off < len) {
        /* Must escape '&', '<', and '>' */
        if (string[off] == '&' || string[off] == '<' || string[off] == '>') {
            if (off - prev_off > 0) {
                /* Emit chunk */
                in = string + prev_off;
                inlen = off - prev_off;
            ...
            esclen = snprintf(escape, sizeof(escape), "&#x%06x;", string[off]);
            pescape = escape;
