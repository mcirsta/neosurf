...
const char *filename_request(void)
{
    ...
    if (i == -1) {
        ...
        if (dir == NULL) {
            NSLOG(netsurf, INFO, "Failed to create a new directory.");
            return NULL;
        }
        i = 63;
    }
        ...
        if (*start == '/') {
            *start = '\0';
            ...
            if (ret != 0 && errno != EEXIST) {
                NSLOG(netsurf, INFO, "Failed to create directory \"%s\"", directory);
                free(directory);
                return false;
            }
...
bool filename_flush_directory(const char *folder, int depth)
{
    ...
    while ((entry = readdir(parent))) {
        ...
        written = snprintf(child, sizeof(child), "%s/%s", folder, entry->d_name);
        if (written == sizeof(child)) {
            child[sizeof(child) - 1] = '\0';
        }

#if (defined(HAVE_DIRFD) && defined(HAVE_FSTATAT))
        if (fstatat(dirfd(parent), entry->d_name, &statbuf, AT_SYMLINK_NOFOLLOW) == -1) {
#else
        if (stat(child, &statbuf) == -1) {
#endif
            NSLOG(netsurf, INFO, "Unable to stat %s: %s", child, strerror(errno));
            continue;
        }
        ...
        if (del == false && (entry->d_name[0] >= '0') && (entry->d_name[0] <= '6') && (entry->d_name[1] >= '0') &&
            ...
            if (number >= 0 && number <= 63) {
                if (depth == 3) {
                    ...
                    /* Find in dir list */
                    for (dir = root; dir; dir = dir->next) {
                        number = dir->numeric_prefix & prefix_mask;
                        if (number == prefix) {
                            /* In list: retain */
                            del = false;
                            break;
        ...
        if (del) {
            if (S_ISDIR(statbuf.st_mode)) {
                changed = (netsurf_recursive_rm(child) == NSERROR_OK);
            } else {
...
static struct directory *filename_create_directory(const char *prefix)
{
    ...
    if (prefix == NULL) {
        for (index = 0, old_dir = root; old_dir; index++, old_dir = old_dir->next) {
        ...
        sprintf(dir_prefix, "%.2i/%.2i/%.2i/", ((index >> 12) & 63), ((index >> 6) & 63), ((index >> 0) & 63));

        prefix = dir_prefix;
    } else {
        /* prefix format is always '01/23/45/' */
        index = ((prefix[7] + prefix[6] * 10 - START_PREFIX) | ((prefix[4] + prefix[3] * 10 - START_PREFIX) << 6) |
    ...
    if (prev_dir && strncmp(prev_dir->prefix, new_dir->prefix, 6) == 0) {
        new_dir->prefix[8] = '\0';
        sprintf(filename_directory, "%s/%s", TEMP_FILENAME_PREFIX, new_dir->prefix);
        new_dir->prefix[8] = '/';
        ...
        if (!is_dir(filename_directory)) {
             ...
             * structure normally. */
            NSLOG(netsurf, INFO, "Failed to create optimised structure '%s'", filename_directory);
        }
    }
    ...
    for (i = 0; i < 3 && *last_2; i++) {
        *last_1++ = *last_2++;
        ...
        if (*last_2) {
            ...
            if (!is_dir(filename_directory)) {
                if (nsmkdir(filename_directory, S_IRWXU)) {
                    NSLOG(netsurf, INFO, "Failed to create directory '%s'", filename_directory);
                    return NULL;
                }
            }
        }
