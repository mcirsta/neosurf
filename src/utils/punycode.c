...
enum punycode_status punycode_encode(size_t input_length_orig, const punycode_uint input[],
    const unsigned char case_flags[], size_t *output_length, char output[])
{
    ...
    while (h < input_length) {
        ...
        for (j = 0; j < input_length; ++j) {
            /* Punycode does not need to check whether input[j] is
             * basic: */
...
enum punycode_status punycode_decode(
    size_t input_length, const char input[], size_t *output_length, punycode_uint output[], unsigned char case_flags[])
{
