...
static bool strtooption(const char *value, struct nsoption_s *option)
{
    ...
    case OPTION_COLOUR: {
        ...
        if (sscanf(value, "%x", &tmp) == 1) {
            ...
            option->value.c = (((0x000000FF & rgbcolour) << 16) | ((0x0000FF00 & rgbcolour) << 0) |
...
static void nsoption_validate(struct nsoption_s *opts, struct nsoption_s *defs)
{
    ...
    for (cloop = NSOPTION_SYS_COLOUR_START; cloop <= NSOPTION_SYS_COLOUR_END; cloop++) {
    ...
    while (((opts[NSOPTION_curl_fetch_timeout].value.u * opts[NSOPTION_max_retried_fetches].value.u) > 60) &&
        (opts[NSOPTION_max_retried_fetches].value.u > 1))
...
static bool nsoption_is_set(const struct nsoption_s *opts, const struct nsoption_s *defs, const enum nsoption_e entry)
{
    ...
    case OPTION_STRING:
         ...
         */
        if (((defs[entry].value.s == NULL) && (opts[entry].value.s != NULL)) ||
            ((defs[entry].value.s != NULL) && (opts[entry].value.s == NULL)) ||
            ((defs[entry].value.s != opts[entry].value.s) && (strcmp(opts[entry].value.s, defs[entry].value.s) != 0))) {
            ret = true;
        }
        break;
    }
...
static size_t nsoption_output_value_file(struct nsoption_s *option, void *ctx)
{
    ...
    case OPTION_COLOUR:
        rgbcolour = (((0x000000FF & option->value.c) << 16) | ((0x0000FF00 & option->value.c) << 0) |
    ...
    case OPTION_STRING:
        slen = fprintf(
            fp, "%s:%s\n", option->key, ((option->value.s == NULL) || (*option->value.s == 0)) ? "" : option->value.s);
        break;
    }
...
static size_t nsoption_output_value_html(struct nsoption_s *option, size_t size, size_t pos, char *string)
{
    ...
    switch (option->type) {
    case OPTION_BOOL:
        slen = snprintf(string + pos, size - pos, "%s", option->value.b ? "true" : "false");
        break;

    case OPTION_INTEGER:
        slen = snprintf(string + pos, size - pos, "%i", option->value.i);
        break;

    case OPTION_UINT:
        slen = snprintf(string + pos, size - pos, "%u", option->value.u);
        break;
    ...
    case OPTION_COLOUR:
        ...
        slen = snprintf(string + pos, size - pos,
            ...
            "</span>",
            (unsigned int)rgbcolour, (unsigned int)rgbcolour, colour_to_bw_furthest(rgbcolour));
        break;

    case OPTION_STRING:
        if (option->value.s != NULL) {
            slen = snprintf(string + pos, size - pos, "%s", option->value.s);
        } else {
            slen = snprintf(string + pos, size - pos,
...
static size_t nsoption_output_value_text(struct nsoption_s *option, size_t size, size_t pos, char *string)
{
    ...
    switch (option->type) {
    case OPTION_BOOL:
        slen = snprintf(string + pos, size - pos, "%c", option->value.b ? '1' : '0');
        break;
 ...
 * exported interface documented in utils/nsoption.h
 */
nserror nsoption_generate(nsoption_generate_cb *generate_cb, void *generate_ctx, enum nsoption_generate_flags flags,
    struct nsoption_s *opts, struct nsoption_s *defs)
{
    ...
    for (entry = 0; entry < NSOPTION_LISTEND; entry++) {
        if (((flags & NSOPTION_GENERATE_CHANGED) != 0) && (nsoption_is_set(opts, defs, entry) == false)) {
            continue;
        }
...
nserror nsoption_write(const char *path, struct nsoption_s *opts, struct nsoption_s *defs)
{
    ...
    ret = nsoption_generate(nsoption_output_value_file, fp, NSOPTION_GENERATE_CHANGED, opts, defs);
...
nserror nsoption_dump(FILE *outf, struct nsoption_s *opts)
{
    ...
    return nsoption_generate(nsoption_output_value_file, outf, NSOPTION_GENERATE_ALL, opts, nsoptions_default);
}
