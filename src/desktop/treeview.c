 ...
 */
static struct treeview_resource treeview_res[TREE_RES_LAST] = {{"resource:icons/arrow-l.png", NULL, 0, false},
    {"resource:icons/content.png", NULL, 0, false}, {"resource:icons/directory.png", NULL, 0, false},
    {"resource:icons/directory2.png", NULL, 0, false}, {"resource:icons/search.png", NULL, 0, false}};
...
{
    return (tree->search.search == false) ? tree->root->height : tree->search.height;
}
 ...
 */
static inline void treeview__cw_invalidate_area(const struct treeview *tree, const struct rect *r)
{
    if (tree->cw_h != NULL) {
 ...
 */
static inline unsigned treeview__get_search_height(const treeview *tree)
{
    return (tree->flags & TREEVIEW_SEARCHABLE) ? tree_g.line_height : 0;
}
 ...
 */
static inline void treeview__cw_update_size(const struct treeview *tree, int width, int height)
{
    if (tree->cw_h != NULL) {
        guit->corewindow->set_extent(tree->cw_h, width, height + treeview__get_search_height(tree));
    }
}
 ...
 */
static inline void treeview__cw_scroll_top(const struct treeview *tree)
{
    struct rect r = {
        .x0 = 0,
        .y0 = 0,
 ...
 */
static inline void treeview__cw_drag_status(const struct treeview *tree, core_window_drag_status ds)
{
    if (tree->cw_h != NULL) {
 ...
 */
static inline struct treeview_text *treeview_get_text_for_field(treeview *tree, treeview_node *n, int i)
{
    if (i == 0) {
        return &n->text;
...
static inline treeview_node *treeview_node_next(treeview_node *node, bool full)
{
    ...
    if ((full || (node->flags & TV_NFLAGS_EXPANDED)) && node->children != NULL) {
        /* Next node is child */
        node = node->children;
    } else {
...
static treeview_node *treeview_y_node(treeview *tree, int target_y)
{
    ...
    while (n != NULL) {
        int h = (n->type == TREE_NODE_ENTRY) ? n->height : tree_g.line_height;
...
static int treeview_node_y(const treeview *tree, const treeview_node *node)
{
    ...
    while (n != NULL && n != node) {
        y += (n->type == TREE_NODE_ENTRY) ? n->height : tree_g.line_height;
 ...
 */
static inline void treeview__cw_scroll_to_node(const struct treeview *tree, const struct treeview_node *node)
{
    struct rect r = {
        .x0 = 0,
        .y0 = treeview_node_y(tree, node),
        .x1 = 1,
        .y1 = (((node != NULL) && (node->type == TREE_NODE_ENTRY)) ? node->height : tree_g.line_height),
    };
 ...
 */
static nserror treeview_walk_internal(treeview *tree, treeview_node *root, enum treeview_walk_mode mode,
    nserror (*callback_bwd)(treeview_node *n, void *ctx, bool *end),
    nserror (*callback_fwd)(treeview_node *n, void *ctx, bool *skip_children, bool *end), void *ctx)
{
    treeview_node *node, *child, *parent, *next_sibling;
    bool walking_search = (mode == TREEVIEW_WALK_MODE_DISPLAY && tree->search.search == true);
    ...
    child = (full || (node->flags & TV_NFLAGS_EXPANDED)) ? node->children : NULL;
    ...
    while (node != NULL) {
        ...
        if (child != NULL && !skip_children) {
            ...
            while (node != root && next_sibling == NULL) {
                ...
                if (callback_bwd != NULL && (entry || !walking_search)) {
                    /* Backwards callback */
        ...
        if (walking_search && (!entry || !(node->flags & TV_NFLAGS_MATCHED))) {
            continue;
        }
...
static nserror treeview__search_walk_cb(treeview_node *n, void *ctx, bool *skip_children, bool *end)
{
    ...
    for (int i = 0; i < sw->tree->n_fields; i++) {
        ...
        if (ef->flags & TREE_FLAG_SEARCHABLE) {
            if (strcasestr(entry->fields[i].value.data, sw->text) != NULL) {
                matched = true;
                break;
            }
        }
...
static nserror treeview__search(treeview *tree, const char *text, unsigned int len)
{
    ...
    err = treeview_walk_internal(
        tree, tree->root, TREEVIEW_WALK_MODE_LOGICAL_COMPLETE, NULL, treeview__search_walk_cb, &sw);
    if (err != NSERROR_OK) {
        return err;
    }
...
static void treeview_textarea_search_callback(void *data, struct textarea_msg *msg)
{
    ...
    case TEXTAREA_MSG_DRAG_REPORT:
        ...
        treeview__cw_drag_status(tree, treeview__get_cw_drag_type(tree));
        break;
    ...
    case TEXTAREA_MSG_TEXT_MODIFIED:
        ...
        treeview__search(tree, msg->data.modified.text, msg->data.modified.len - 1);
        break;
 ...
 */
static inline void
treeview_insert_node(treeview *tree, treeview_node *a, treeview_node *b, enum treeview_relationship rel)
{
    assert(a != NULL);
    ...
    if (a->children != NULL) {
        treeview_walk_internal(
            tree, a, TREEVIEW_WALK_MODE_LOGICAL_COMPLETE, NULL, treeview_set_inset_from_parent, NULL);
    }
    ...
    if (a->parent->flags & TV_NFLAGS_EXPANDED) {
         ...
         * affect layout */
        if (a->text.width == 0) {
            guit->layout->width(&plot_style_odd.text, a->text.data, a->text.len, &(a->text.width));
        }
...
nserror treeview_create_node_folder(treeview *tree, treeview_node **folder, treeview_node *relation,
    enum treeview_relationship rel, const struct treeview_field_data *field, void *data,
    treeview_node_options_flags flags)
{
    ...
    n->flags = (flags & TREE_OPTION_SPECIAL_DIR) ? TV_NFLAGS_SPECIAL : TV_NFLAGS_NONE;
...
/* Exported interface, documented in treeview.h */
nserror
treeview_update_node_folder(treeview *tree, treeview_node *folder, const struct treeview_field_data *field, void *data)
{
    bool match;
    ...
    assert(field != NULL);
    assert(
        lwc_string_isequal(tree->fields[tree->n_fields].field, field->field, &match) == lwc_error_ok && match == true);
    ...
    if (folder->parent->flags & TV_NFLAGS_EXPANDED) {
        ...
        guit->layout->width(&plot_style_odd.text, folder->text.data, folder->text.len, &(folder->text.width));
    } else {
...
/* Exported interface, documented in treeview.h */
nserror
treeview_update_node_entry(treeview *tree, treeview_node *entry, const struct treeview_field_data fields[], void *data)
{
    bool match;
    ...
    assert(lwc_string_isequal(tree->fields[0].field, fields[0].field, &match) == lwc_error_ok && match == true);
    ...
    if (entry->parent->flags & TV_NFLAGS_EXPANDED) {
        /* Text will be seen, get its width */
        guit->layout->width(&plot_style_odd.text, entry->text.data, entry->text.len, &(entry->text.width));
    } else {
        /* Just invalidate the width, since it's not needed now */
        entry->text.width = 0;
    }
    ...
    for (i = 1; i < tree->n_fields; i++) {
        ...
        assert(lwc_string_isequal(tree->fields[i].field, fields[i].field, &match) == lwc_error_ok && match == true);
        ...
        if (entry->flags & TV_NFLAGS_EXPANDED) {
            /* Text will be seen, get its width */
            guit->layout->width(&plot_style_odd.text, e->fields[i - 1].value.data, e->fields[i - 1].value.len,
...
/* Exported interface, documented in treeview.h */
nserror treeview_create_node_entry(treeview *tree, treeview_node **entry, treeview_node *relation,
    enum treeview_relationship rel, const struct treeview_field_data fields[], void *data,
    treeview_node_options_flags flags)
{
    bool match;
    ...
    e = malloc(sizeof(struct treeview_node_entry) + (tree->n_fields - 1) * sizeof(struct treeview_field));
    if (e == NULL) {
        return NSERROR_NOMEM;
    }
...
/* Exported interface, documented in treeview.h */
nserror treeview_walk(treeview *tree, treeview_node *root, treeview_walk_cb enter_cb, treeview_walk_cb leave_cb,
    void *ctx, enum treeview_node_type type)
{
    struct treeview_walk_ctx tw = {.enter_cb = enter_cb, .leave_cb = leave_cb, .ctx = ctx, .type = type};
    ...
    return treeview_walk_internal(tree, root, TREEVIEW_WALK_MODE_LOGICAL_COMPLETE,
        (leave_cb != NULL) ? treeview_walk_bwd_cb : NULL, (enter_cb != NULL) ? treeview_walk_fwd_cb : NULL, &tw);
}
...
static nserror treeview_delete_node_walk_cb(treeview_node *n, void *ctx, bool *end)
{
    ...
    if (treeview_unlink_node(n))
        nd->h_reduction += (n->type == TREE_NODE_ENTRY) ? n->height : tree_g.line_height;
    ...
    if (nd->tree->edit.textarea != NULL && nd->tree->edit.node == n) {
 ...
 */
static nserror
treeview_delete_node_internal(treeview *tree, treeview_node *n, bool interaction, treeview_node_options_flags flags)
{
    nserror err;
    treeview_node *p = n->parent;
    struct treeview_node_delete nd = {.tree = tree, .h_reduction = 0, .user_interaction = interaction};
    ...
    err = treeview_walk_internal(tree, n, TREEVIEW_WALK_MODE_LOGICAL_COMPLETE, treeview_delete_node_walk_cb, NULL, &nd);
    if (err != NSERROR_OK) {
        return err;
    }
    ...
    /* Inform front end of change in dimensions */
    if (tree->root != NULL && p != NULL && p->flags & TV_NFLAGS_EXPANDED && nd.h_reduction > 0 &&
...
static nserror treeview_delete_empty_nodes(treeview *tree, bool interaction)
{
    ...
    while (node != NULL) {
        ...
        if (child != NULL) {
            ...
            while (node->parent != NULL && next_sibling == NULL) {
                if (node->type == TREE_NODE_FOLDER && node->children == NULL) {
                    /* Delete node */
                    p = node->parent;
                    err = treeview_delete_node_walk_cb(node, &nd, &abort);
                    if (err != NSERROR_OK) {
                        return err;
                    }
 ...
 */
static struct textarea *treeview__create_textarea(treeview *tree, int width, int height, colour border,
    colour background, colour foreground, plot_font_style_t text, textarea_client_callback ta_callback)
{
...
/* Exported interface, documented in treeview.h */
nserror treeview_create(treeview **treeout, const struct treeview_callback_table *callbacks, int n_fields,
    struct treeview_field_desc fields[], struct core_window *cw, treeview_flags flags)
{
    treeview *tree;
    nserror error;
    int fldidx;
    ...
    for (fldidx = 0; fldidx < n_fields; fldidx++) {
        ...
        guit->layout->width(&plot_style_odd.text, f->value.data, f->value.len, &(f->value.width));
    ...
    if (flags & TREEVIEW_SEARCHABLE) {
        tree->search.textarea = treeview__create_textarea(tree, 600, tree_g.line_height,
            nscolours[NSCOLOUR_TEXT_INPUT_BG], nscolours[NSCOLOUR_TEXT_INPUT_BG], nscolours[NSCOLOUR_TEXT_INPUT_FG],
            plot_style_odd.text, treeview_textarea_search_callback);
...
nserror treeview_destroy(treeview *tree)
{
    ...
    /* Destroy nodes */
    treeview_delete_node_internal(tree, tree->root, false, TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
...
static nserror treeview_node_expand_internal(treeview *tree, treeview_node *node)
{
    ...
    case TREE_NODE_FOLDER:
        ...
        do {
            if (child->text.width == 0) {
                guit->layout->width(&plot_style_odd.text, child->text.data, child->text.len, &(child->text.width));
            }
    ...
    case TREE_NODE_ENTRY:
        ...
        for (i = 0; i < tree->n_fields - 1; i++) {
            ...
            if (e->fields[i].value.width == 0) {
                guit->layout->width(
                    &plot_style_odd.text, e->fields[i].value.data, e->fields[i].value.len, &(e->fields[i].value.width));
            }
    ...
    /* And node heights */
    for (struct treeview_node *n = node; (n != NULL) && (n->flags & TV_NFLAGS_EXPANDED); n = n->parent) {
        n->height += additional_height_entries + additional_height_folders;
    }

    if (tree->search.search && node->type == TREE_NODE_ENTRY && node->flags & TV_NFLAGS_MATCHED) {
...
static nserror treeview_node_contract_cb(treeview_node *n, void *ctx, bool *end)
{
    ...
    if ((n->flags & TV_NFLAGS_EXPANDED) == false || (n->type == TREE_NODE_FOLDER && data->only_entries)) {
        /* Nothing to do. */
        return NSERROR_OK;
    }
    ...
    for (struct treeview_node *node = n; (node != NULL) && (node->flags & TV_NFLAGS_EXPANDED); node = node->parent) {
...
static nserror treeview_node_contract_internal(treeview *tree, treeview_node *node)
{
    ...
    /* Contract children. */
    treeview_walk_internal(tree, node, TREEVIEW_WALK_MODE_LOGICAL_EXPANDED, treeview_node_contract_cb, NULL, &data);
...
nserror treeview_contract(treeview *tree, bool all)
{
    ...
    for (n = tree->root->children; n != NULL; n = n->next_sib) {
        ...
        /* Contract children. */
        treeview_walk_internal(tree, n, TREEVIEW_WALK_MODE_LOGICAL_EXPANDED, treeview_node_contract_cb, NULL, &data);
...
static nserror treeview_expand_cb(treeview_node *n, void *ctx, bool *skip_children, bool *end)
{
    ...
    if (n->flags & TV_NFLAGS_EXPANDED || (data->only_folders && n->type != TREE_NODE_FOLDER)) {
        /* Nothing to do. */
        return NSERROR_OK;
    }
 ...
 */
static void treeview_redraw_tree(treeview *tree, const int x, const int y, int *render_y_in_out, const struct rect *r,
    struct content_redraw_data *data, const struct redraw_context *ctx)
{
    ...
    while (node != NULL) {
        ...
        next = (node->flags & TV_NFLAGS_EXPANDED) ? node->children : NULL;
        ...
        if (next != NULL) {
            ...
            while (node != root && node->next_sib == NULL) {
                node = node->parent;
            }
        ...
        assert(node != NULL);
        assert(node != root);
        assert(node->type == TREE_NODE_FOLDER || node->type == TREE_NODE_ENTRY);
        ...
        if (tree->drag.type == TV_DRAG_SELECTION && (render_y + height >= sel_min && render_y < sel_max)) {
            invert_selection = true;
        } else {
            invert_selection = false;
        }
        if ((node->flags & TV_NFLAGS_SELECTED && !invert_selection) ||
            (!(node->flags & TV_NFLAGS_SELECTED) && invert_selection)) {
            bg_style = &style->sbg;
            text_style = &style->stext;
            infotext_style = &style->sitext;
            furniture = (node->flags & TV_NFLAGS_EXPANDED) ? style->furn[TREE_FURN_CONTRACT].sel
                                                           : style->furn[TREE_FURN_EXPAND].sel;
        } else {
            bg_style = &style->bg;
            text_style = &style->text;
            infotext_style = &style->itext;
            furniture = (node->flags & TV_NFLAGS_EXPANDED) ? style->furn[TREE_FURN_CONTRACT].bmp
        ...
        /* Render toggle */
        ctx->plot->bitmap(ctx, furniture, inset, render_y + tree_g.line_height / 4, style->furn[TREE_FURN_EXPAND].size,
            style->furn[TREE_FURN_EXPAND].size, bg_style->fill_colour, BITMAPF_NONE);
        ...
        if (treeview_res[res].ready) {
            ...
            data->y = render_y + ((tree_g.line_height - treeview_res[res].height + 1) / 2);
        ...
        ctx->plot->text(ctx, text_style, x0, render_y + baseline, node->text.data, node->text.len);
        ...
        if (node->type != TREE_NODE_ENTRY || !(node->flags & TV_NFLAGS_EXPANDED))
        ...
        for (i = 0; i < tree->n_fields - 1; i++) {
            ...
            if (ef->flags & TREE_FLAG_SHOW_NAME) {
                ...
                ctx->plot->text(ctx, infotext_style, x0 + max_width - ef->value.width - tree_g.step_width,
                    render_y + baseline, ef->value.data, ef->value.len);

                ctx->plot->text(ctx, infotext_style, x0 + max_width, render_y + baseline, entry->fields[i].value.data,
                    entry->fields[i].value.len);
            } else {
                ctx->plot->text(ctx, infotext_style, x0, render_y + baseline, entry->fields[i].value.data,
                    entry->fields[i].value.len);
            }
 ...
 * \param[in]     ctx       Current render context.
 */
static void treeview_redraw_search(treeview *tree, const int x, const int y, int *render_y_in_out, const struct rect *r,
    struct content_redraw_data *data, const struct redraw_context *ctx)
{
    struct treeview_node_style *style = &plot_style_odd;
    enum treeview_resource_id res = TREE_RES_CONTENT;
    ...
    while (node != NULL) {
        ...
        if (node->type == TREE_NODE_FOLDER || !(node->flags & TV_NFLAGS_MATCHED)) {
            continue;
        }
...
void treeview_redraw(treeview *tree, const int x, const int y, struct rect *clip, const struct redraw_context *ctx)
{
    ...
    if (tree->flags & TREEVIEW_SEARCHABLE) {
        if (render_y < r.y1) {
            ...
            if (treeview_res[icon].ready) {
                ...
                data.y = render_y + ((tree_g.line_height - treeview_res[icon].height + 1) / 2);
                data.background_colour = plot_style_even.bg.fill_colour;

                content_redraw(treeview_res[icon].c, &data, &r, &new_ctx);
            }

            textarea_redraw(tree->search.textarea, x + tree_g.window_padding + tree_g.icon_step, y,
                plot_style_even.bg.fill_colour, 1.0, &r, &new_ctx);
        }
        render_y += tree_g.line_height;
    }
    ...
    if (tree->search.search == true) {
        treeview_redraw_search(tree, x, y, &render_y, &r, &data, &new_ctx);
    } else {
    ...
    if ((tree->move.target_pos != TV_TARGET_NONE) && (treeview_res[TREE_RES_ARROW].ready)) {
        ...
        textarea_redraw(
            tree->edit.textarea, tree->edit.x + x, tree->edit.y + y, plot_style_even.bg.fill_colour, 1.0, &r, &new_ctx);
    }
...
static nserror treeview_node_selection_walk_cb(treeview_node *n, void *ctx, bool *skip_children, bool *end)
{
    ...
    case TREEVIEW_WALK_DELETE_SELECTION:
        if (n->flags & TV_NFLAGS_SELECTED) {
            err = treeview_delete_node_internal(sw->tree, n, true, TREE_OPTION_NONE);
            if (err != NSERROR_OK) {
                return err;
            }
            *skip_children = true;
    ...
    case TREEVIEW_WALK_PROPAGATE_SELECTION:
        if (n->parent != NULL && n->parent->flags & TV_NFLAGS_SELECTED && !(n->flags & TV_NFLAGS_SELECTED)) {
            n->flags ^= TV_NFLAGS_SELECTED;
            changed = true;
        }
        break;
    ...
    case TREEVIEW_WALK_COMMIT_SELECT_DRAG:
        if (sw->current_y >= sw->data.drag.sel_min && sw->current_y - height < sw->data.drag.sel_max) {
            n->flags ^= TV_NFLAGS_SELECTED;
        }
        return NSERROR_OK;
    ...
    case TREEVIEW_WALK_COPY_SELECTION:
        if (n->flags & TV_NFLAGS_SELECTED && n->type == TREE_NODE_ENTRY) {
            int i;
            char *temp;
            uint32_t len;
            const char *text;
            ...
            for (i = 0; i < sw->tree->n_fields; i++) {
                ...
                val = treeview_get_text_for_field(sw->tree, n, i);
                text = val->data;
                len = val->len;
...
bool treeview_has_selection(treeview *tree)
{
    ...
    treeview_walk_internal(tree, tree->root, TREEVIEW_WALK_MODE_DISPLAY, NULL, treeview_node_selection_walk_cb, &sw);
...
static void treeview_copy_selection(treeview *tree)
{
    ...
    err = treeview_walk_internal(
        tree, tree->root, TREEVIEW_WALK_MODE_DISPLAY, NULL, treeview_node_selection_walk_cb, &sw);
    if (err != NSERROR_OK) {
        return;
    }

    if (sw.data.copy.text != NULL) {
        guit->clipboard->set(sw.data.copy.text, sw.data.copy.len - 1, NULL, 0);
        free(sw.data.copy.text);
    }
}
...
static nserror treeview_node_launch_walk_fwd_cb(treeview_node *n, void *ctx, bool *skip_children, bool *end)
{
    ...
    } else if (n->type == TREE_NODE_ENTRY && (n->flags & TV_NFLAGS_SELECTED || lw->selected_depth > 0)) {
        struct treeview_node_msg msg;
...
static nserror treeview_launch_selection(treeview *tree)
{
    ...
    return treeview_walk_internal(tree, tree->root, TREEVIEW_WALK_MODE_LOGICAL_COMPLETE,
        treeview_node_launch_walk_bwd_cb, treeview_node_launch_walk_fwd_cb, &lw);
}


/* Exported interface, documented in treeview.h */
nserror
treeview_get_relation(treeview *tree, treeview_node **relation, enum treeview_relationship *rel, bool at_y, int y)
{
    treeview_node *n;
...
static bool treeview_keyboard_navigation(treeview *tree, uint32_t key, struct rect *rect)
{
    struct treeview_nav_state ns = {
        .tree = tree, .prev = NULL, .curr = NULL, .next = NULL, .last = NULL, .n_selected = 0, .prev_n_selected = 0};
    int search_height = treeview__get_search_height(tree);
     ...
     * state */
    treeview_walk_internal(tree, tree->root, TREEVIEW_WALK_MODE_DISPLAY, NULL, treeview_node_nav_cb, &ns);
    ...
    case NS_KEY_LEFT:
        ...
        if (ns.curr != NULL && ns.curr->parent != NULL && ns.curr->parent->type != TREE_NODE_ROOT) {
            /* Step to parent */
        ...
        if (ns.curr != NULL) {
            if (!(ns.curr->flags & TV_NFLAGS_EXPANDED)) {
                ...
                if (ns.curr->children != NULL) {
                    /* Step to first child */
                    ns.curr->children->flags |= TV_NFLAGS_SELECTED;
 ...
 * \return true iff redraw required
 */
static bool treeview_set_move_indicator(treeview *tree, bool need_redraw, treeview_node *target, int node_height,
    int node_y, int mouse_y, struct rect *rect)
{
    ...
    if (target->flags & TV_NFLAGS_SELECTED) {
        ...
        while (target->parent && target->parent->flags & TV_NFLAGS_SELECTED) {
            target = target->parent;
        }

        /* Find top adjacent selected sibling */
        while (target->prev_sib && target->prev_sib->flags & TV_NFLAGS_SELECTED) {
        ...
        case TREE_NODE_FOLDER:
            if (mouse_pos <= node_height / 4) {
                target_pos = TV_TARGET_ABOVE;
            } else if (mouse_pos <= (3 * node_height) / 4 || target->flags & TV_NFLAGS_EXPANDED) {
    ...
    if (target_pos == tree->move.target_pos && target == tree->move.target) {
        /* No change */
        return need_redraw;
    }
    ...
    node_y += (tree_g.line_height - treeview_res[TREE_RES_ARROW].height + 1) / 2;
 ...
 */
static bool
treeview_edit_node_at_point(treeview *tree, treeview_node *n, int node_y, int mouse_x, int mouse_y, struct rect *rect)
{
    ...
    if (n->type == TREE_NODE_ENTRY && n->height != tree_g.line_height) {
        ...
        for (i = 0; i < tree->n_fields - 1; i++) {
            ...
            if (mouse_y <= pos && (ef->flags & TREE_FLAG_ALLOW_EDIT)) {
    ...
    /* Create text area */
    tree->edit.textarea = treeview__create_textarea(
        tree, width, height, 0x000000, 0xffffff, 0x000000, plot_style_odd.text, treeview_textarea_callback);
    if (tree->edit.textarea == NULL) {
        return false;
    }
    ...
    textarea_mouse_action(
        tree->edit.textarea, BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_CLICK_1, mouse_x, tree_g.line_height / 2);
...
void treeview_edit_selection(treeview *tree)
{
    ...
    /* Edit node at y */
    redraw = treeview_edit_node_at_point(tree, n, y, 0, y + tree_g.line_height / 2, &rect);
...
static nserror treeview_node_mouse_action_cb(treeview_node *node, void *ctx, bool *skip_children, bool *end)
{
    ...
    bool click;
    int height;
    enum { TV_NODE_ACTION_NONE = 0, TV_NODE_ACTION_SELECTION = (1 << 0) } action = TV_NODE_ACTION_NONE;
    ...
    if (ma->y <= ma->current_y + tree_g.line_height) {
        ...
        if (ma->x >= inset - 1 && ma->x < inset + tree_g.step_width) {
            /* Over expansion toggle */
            part = TV_NODE_PART_TOGGLE;

        } else if (ma->x >= inset + tree_g.step_width &&
            ma->x < inset + tree_g.step_width + tree_g.icon_step + node->text.width) {
            /* On node */
            part = TV_NODE_PART_ON_NODE;
        }
    } else if (node->type == TREE_NODE_ENTRY && height > tree_g.line_height) {
        /* Expanded entries */
        int x = node->inset + tree_g.step_width + tree_g.icon_step;
        int y = ma->current_y + tree_g.line_height;
        int i;
        struct treeview_node_entry *entry = (struct treeview_node_entry *)node;
        for (i = 0; i < ma->tree->n_fields - 1; i++) {
            ...
            if (ef->flags & TREE_FLAG_SHOW_NAME) {
                ...
                if (ma->x >= x + max_width - ef->value.width - tree_g.step_width &&
                    ma->x < x + max_width - tree_g.step_width) {
                    /* On a field name */
                    part = TV_NODE_PART_ON_NODE;

                } else if (ma->x >= x + max_width && ma->x < x + max_width + entry->fields[i].value.width) {
                    /* On a field value */
                    part = TV_NODE_PART_ON_NODE;
                }
            } else {
                if (ma->x >= x && ma->x < x + entry->fields[i].value.width) {
                    /* On a field value */
                    part = TV_NODE_PART_ON_NODE;
                }
            }
    ...
    if (ma->mouse & (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_PRESS_2) && ma->tree->drag.type == TV_DRAG_NONE) {
    ...
    if (ma->tree->drag.type == TV_DRAG_NONE) {
        if (ma->mouse & BROWSER_MOUSE_DRAG_1 && ma->tree->drag.selected == false &&
            ma->tree->drag.part == TV_NODE_PART_NONE) {
            ma->tree->drag.type = TV_DRAG_SELECTION;
            treeview__cw_drag_status(ma->tree, CORE_WINDOW_DRAG_SELECTION);

        } else if (ma->tree->search.search == false && !(ma->tree->flags & TREEVIEW_NO_MOVES) &&
            ma->mouse & BROWSER_MOUSE_DRAG_1 &&
            (ma->tree->drag.selected == true || ma->tree->drag.part == TV_NODE_PART_ON_NODE)) {
            ma->tree->drag.type = TV_DRAG_MOVE;
            treeview__cw_drag_status(ma->tree, CORE_WINDOW_DRAG_MOVE);
        ...
        if (ma->tree->drag.start_node != NULL && ma->tree->drag.type == TV_DRAG_SELECTION) {
    ...
    case TV_DRAG_SELECTION: {
        ...
        int prev_y1 = ma->tree->drag.prev.node_y + ma->tree->drag.prev.node_h;

        r.y0 = (ma->current_y < ma->tree->drag.prev.node_y) ? ma->current_y : ma->tree->drag.prev.node_y;
    ...
    case TV_DRAG_MOVE:
        redraw |= treeview_set_move_indicator(ma->tree, redraw, node, height, ma->current_y, ma->y, &r);
        break;
    ...
    if (((node->type == TREE_NODE_FOLDER) && (ma->mouse & BROWSER_MOUSE_DOUBLE_CLICK) && click) ||
        (part == TV_NODE_PART_TOGGLE && click)) {
        int h = treeview__get_display_height(ma->tree) + ma->search_height;
        ...
        if (!redraw || r.y0 > ma->current_y)
            r.y0 = ma->current_y;
        if (h > treeview__get_display_height(ma->tree) + ma->search_height) {
            r.y1 = h;
        } else {
            r.y1 = treeview__get_display_height(ma->tree) + ma->search_height;
        }
        redraw = true;

    } else if ((node->type == TREE_NODE_ENTRY) && (ma->mouse & BROWSER_MOUSE_DOUBLE_CLICK) && click) {
        struct treeview_node_msg msg;
        msg.msg = TREE_MSG_NODE_LAUNCH;
        ...
        (ma->mouse & BROWSER_MOUSE_PRESS_1 && ma->mouse & BROWSER_MOUSE_MOD_2)) {
        /* Toggle selection of node */
        action |= TV_NODE_ACTION_SELECTION;

    } else if (ma->mouse & BROWSER_MOUSE_CLICK_1 && ma->mouse & (BROWSER_MOUSE_MOD_1 | BROWSER_MOUSE_MOD_3) &&
        part != TV_NODE_PART_TOGGLE) {
        ...
        /* Edit node */
        redraw |= treeview_edit_node_at_point(ma->tree, node, ma->current_y, ma->x, ma->y, &r);

    } else if (ma->mouse & BROWSER_MOUSE_PRESS_1 && !(ma->mouse & (BROWSER_MOUSE_MOD_1 | BROWSER_MOUSE_MOD_3)) &&
        !(node->flags & TV_NFLAGS_SELECTED) && part != TV_NODE_PART_TOGGLE) {
        /* Clear any existing selection */
        redraw |= treeview_clear_selection(ma->tree, &r);
...
void treeview_mouse_action(treeview *tree, browser_mouse_state mouse, int x, int y)
{
    ...
    if (tree->drag.type == TV_DRAG_TEXTAREA) {
        textarea_mouse_action(tree->edit.textarea, mouse, x - tree->edit.x, y - tree->edit.y);
        return;
        ...
        textarea_mouse_action(tree->search.textarea, mouse, x - tree_g.window_padding - tree_g.icon_size, y);
        return;
    } else if (mouse & (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_PRESS_2) && y < search_height &&
        tree->search.active == false) {
        tree->search.active = true;
        if (treeview_clear_selection(tree, &r)) {
            treeview__cw_invalidate_area(tree, &r);
        ...
        textarea_mouse_action(tree->search.textarea, mouse, x - tree_g.window_padding - tree_g.icon_size, y);
        return;
    } else if (mouse & (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_PRESS_2) && tree->search.active == true) {
    ...
    if (tree->edit.textarea != NULL) {
        ...
        if (ta_x > 0 && ta_x < tree->edit.w && ta_y > 0 && ta_y < tree->edit.h) {
            /* Inside textarea */
            textarea_mouse_action(tree->edit.textarea, mouse, ta_x, ta_y);
            return;
    ...
    if (y > treeview__get_display_height(tree) + search_height) {
        ...
        if (tree->drag.type == TV_DRAG_NONE) {
            if (mouse & BROWSER_MOUSE_DRAG_1 && tree->drag.selected == false && tree->drag.part == TV_NODE_PART_NONE) {
                tree->drag.type = TV_DRAG_SELECTION;
                treeview__cw_drag_status(tree, CORE_WINDOW_DRAG_SELECTION);
            } else if (mouse & BROWSER_MOUSE_DRAG_2) {
                tree->drag.type = TV_DRAG_SELECTION;
                treeview__cw_drag_status(tree, CORE_WINDOW_DRAG_SELECTION);
            }

            if (tree->drag.start_node != NULL && tree->drag.type == TV_DRAG_SELECTION) {
                tree->drag.start_node->flags ^= TV_NFLAGS_SELECTED;
            }
        }
        ...
        if (tree->drag.type == TV_DRAG_SELECTION) {
            ...
            int prev_y1 = tree->drag.prev.node_y + tree->drag.prev.node_h;
        ...
        treeview_walk_internal(tree, tree->root, TREEVIEW_WALK_MODE_DISPLAY, NULL, treeview_node_mouse_action_cb, &ma);
    }
}
...
static nserror treeview_init_furniture(void)
{
    ...
    plot_style_odd.furn[TREE_FURN_EXPAND].bmp = treeview_generate_triangle_bitmap(
        plot_style_odd.bg.fill_colour, plot_style_odd.itext.foreground, size);
    plot_style_odd.furn[TREE_FURN_EXPAND].sel = treeview_generate_triangle_bitmap(
        plot_style_odd.sbg.fill_colour, plot_style_odd.sitext.foreground, size);

    plot_style_even.furn[TREE_FURN_EXPAND].size = size;
    plot_style_even.furn[TREE_FURN_EXPAND].bmp = treeview_generate_triangle_bitmap(
        plot_style_even.bg.fill_colour, plot_style_even.itext.foreground, size);
    plot_style_even.furn[TREE_FURN_EXPAND].sel = treeview_generate_copy_bitmap(
        plot_style_odd.furn[TREE_FURN_EXPAND].sel, size);

    plot_style_odd.furn[TREE_FURN_CONTRACT].size = size;
    plot_style_odd.furn[TREE_FURN_CONTRACT].bmp = treeview_generate_rotate_bitmap(
        plot_style_odd.furn[TREE_FURN_EXPAND].bmp, size);
    plot_style_odd.furn[TREE_FURN_CONTRACT].sel = treeview_generate_rotate_bitmap(
        plot_style_odd.furn[TREE_FURN_EXPAND].sel, size);

    plot_style_even.furn[TREE_FURN_CONTRACT].size = size;
    plot_style_even.furn[TREE_FURN_CONTRACT].bmp = treeview_generate_rotate_bitmap(
        plot_style_even.furn[TREE_FURN_EXPAND].bmp, size);
    plot_style_even.furn[TREE_FURN_CONTRACT].sel = treeview_generate_rotate_bitmap(
        plot_style_even.furn[TREE_FURN_EXPAND].sel, size);

    if (plot_style_odd.furn[TREE_FURN_EXPAND].bmp == NULL || plot_style_odd.furn[TREE_FURN_EXPAND].sel == NULL ||
        plot_style_even.furn[TREE_FURN_EXPAND].bmp == NULL || plot_style_even.furn[TREE_FURN_EXPAND].sel == NULL ||
        plot_style_odd.furn[TREE_FURN_CONTRACT].bmp == NULL || plot_style_odd.furn[TREE_FURN_CONTRACT].sel == NULL ||
        plot_style_even.furn[TREE_FURN_CONTRACT].bmp == NULL || plot_style_even.furn[TREE_FURN_CONTRACT].sel == NULL)
        return NSERROR_NOMEM;
