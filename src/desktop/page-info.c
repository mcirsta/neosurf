...
static nserror page_info__measure_text_entry(struct page_info_text *pit)
{
    ...
    err = guit->layout->width(pit->style, pit->text, strlen(pit->text), &pit->width);
    if (err != NSERROR_OK) {
        return err;
    }
...
static nserror page_info__measure_text(struct page_info *pi)
{
    ...
    for (unsigned i = 0; i < PI_ENTRY__COUNT; i++) {
        ...
        case PAGE_INFO_ENTRY_TYPE_TEXT:
            ...
            if (i == PI_ENTRY_DOMAIN) {
                entry->u.text.padding_bottom = entry->u.text.height * 3 / 2;
            }
            break;

        case PAGE_INFO_ENTRY_TYPE_ITEM:
            err = page_info__measure_text_entry(&entry->u.item.item);
            if (err != NSERROR_OK) {
                return err;
            }
            err = page_info__measure_text_entry(&entry->u.item.detail);
            if (err != NSERROR_OK) {
                return err;
            }
    ...
    pi->window_padding = pi->entries[PI_ENTRY_DOMAIN].u.item.item.height / 2;
 ...
 */
static inline bool page_info__hide_entry(enum pi_entry entry, enum nsurl_scheme_type scheme)
{
    switch (entry) {
    case PI_ENTRY_CERT:
    ...
    case PI_ENTRY_COOKIES:
        if (scheme != NSURL_SCHEME_HTTP && scheme != NSURL_SCHEME_HTTPS) {
            return true;
        }
        break;
    default:
...
static nserror page_info__layout(struct page_info *pi)
{
    ...
    for (unsigned i = 0; i < PI_ENTRY__COUNT; i++) {
        ...
        case PAGE_INFO_ENTRY_TYPE_ITEM: {
            int full_width = entry->u.item.item.width + entry->u.item.detail.width;
 ...
 */
static nserror
page_info__redraw_text_entry(const struct page_info_text *pit, int x, int y, const struct redraw_context *ctx)
{
    int baseline = (pit->height * 3 + 2) / 4;

    ctx->plot->text(ctx, pit->style, x, y + baseline, pit->text, strlen(pit->text));
...
page_info_redraw(const struct page_info *pi, int x, int y, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    for (unsigned i = 0; i < PI_ENTRY__COUNT; i++) {
        ...
        switch (entry->type) {
        case PAGE_INFO_ENTRY_TYPE_TEXT:
            err = page_info__redraw_text_entry(&entry->u.text, cur_x, cur_y, &new_ctx);
            if (err != NSERROR_OK) {
                goto cleanup;
            }
            cur_y += entry->u.text.height;
        ...
        case PAGE_INFO_ENTRY_TYPE_ITEM:
            if (entry->u.item.hover) {
                r.y0 = cur_y;
                r.y1 = cur_y + entry->u.item.padding_top + entry->u.item.item.height + entry->u.item.padding_bottom;
                new_ctx.plot->rectangle(&new_ctx, &pi__hover, &r);
            }
            cur_y += entry->u.item.padding_top;
            err = page_info__redraw_text_entry(&entry->u.item.item, cur_x, cur_y, &new_ctx);
            if (err != NSERROR_OK) {
                goto cleanup;
            }
            cur_x += entry->u.item.item.width;
            err = page_info__redraw_text_entry(&entry->u.item.detail, cur_x, cur_y, &new_ctx);
            if (err != NSERROR_OK) {
                goto cleanup;
            }
            cur_y += entry->u.item.item.height;
...
static nserror page_info__handle_item_click(
    struct page_info *pi, enum browser_mouse_state mouse, enum pi_entry clicked, bool *did_something)
{
    nserror err;
    ...
    for (unsigned i = 0; i < PI_ENTRY__COUNT; i++) {
        ...
        case PAGE_INFO_ENTRY_TYPE_ITEM:
            ...
            if (y >= cur_y && y < cur_y + height) {
                hovering = true;
                err = page_info__handle_item_click(pi, mouse, i, did_something);
                if (err != NSERROR_OK) {
                    return err;
                }
            }
