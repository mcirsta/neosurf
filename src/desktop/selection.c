...
static nserror selection_redraw(struct selection *s, unsigned start_idx, unsigned end_idx)
{
    ...
    if (s->c->handler->textselection_redraw != NULL) {
        res = s->c->handler->textselection_redraw(s->c, start_idx, end_idx);
    } else {
...
static bool selection_copy(struct selection *s, struct selection_string *selstr)
{
    ...
    if (s->c->handler->textselection_copy != NULL) {
        res = s->c->handler->textselection_copy(s->c, s->start_idx, s->end_idx, selstr);
    } else {
        res = NSERROR_NOT_IMPLEMENTED;
    }
...
bool selection_string_append(
    const char *text, size_t length, bool space, plot_font_style_t *style, struct selection_string *sel_string)
{
    ...
    if (style != NULL) {
        ...
        new_styles = realloc(sel_string->styles, (sel_string->n_styles + 1) * sizeof(nsclipboard_styles));
        if (new_styles == NULL) {
            return false;
        }
        ...
        sel_string->styles[sel_string->n_styles].start = sel_string->length;
...
bool selection_click(struct selection *s, struct browser_window *top, browser_mouse_state mouse, unsigned idx)
{
    ...
    if (!pos && ((mouse & BROWSER_MOUSE_DRAG_1) || (modkeys && (mouse & BROWSER_MOUSE_DRAG_2)))) {
...
bool selection_copy_to_clipboard(struct selection *s)
{
    ...
    guit->clipboard->set(sel_string.buffer, sel_string.length, sel_string.styles, sel_string.n_styles);
