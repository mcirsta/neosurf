...
static nserror global_history_create_dir(enum global_history_folders f)
{
    ...
    err = treeview_create_node_folder(gh_ctx.tree, &gh_ctx.folders[f].folder, relation, rel, &gh_ctx.folders[f].data,
        &gh_ctx.folders[f],
        gh_ctx.built ? TREE_OPTION_NONE : TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
 ...
 * \return NSERROR_OK on success, appropriate error otherwise
 */
static inline nserror global_history_get_parent_treeview_node(treeview_node **parent, int slot)
{
    int folder_index;
...
{
    const char *title = (data->title != NULL) ? data->title : messages_get("NoTitle");
    char buffer[16];
    struct tm *lvtime;
...
static nserror global_history_entry_insert(struct global_history_entry *e, int slot)
{
    ...
    err = treeview_create_node_entry(gh_ctx.tree, &(e->entry), parent, TREE_REL_FIRST_CHILD, e->data, e,
        gh_ctx.built ? TREE_OPTION_NONE : TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
    if (err != NSERROR_OK) {
        return err;
    }
...
static bool global_history_add_entry(nsurl *url, const struct url_data *data)
{
    ...
    if (got_treeview == true) {
        ...
        if (e != NULL) {
            treeview_delete_node(gh_ctx.tree, e->entry, TREE_OPTION_SUPPRESS_REDRAW | TREE_OPTION_SUPPRESS_RESIZE);
        }
    }

    if (global_history_add_entry_internal(url, slot, data, got_treeview) != NSERROR_OK) {
        return false;
    }
...
static nserror global_history_initialise_entry_fields(void)
{
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &gh_ctx.fields[GH_TITLE].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &gh_ctx.fields[GH_URL].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &gh_ctx.fields[GH_LAST_VISIT].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &gh_ctx.fields[GH_VISITS].field) != lwc_error_ok) {
        goto error;
    }
...
struct treeview_callback_table gh_tree_cb_t = {.folder = global_history_tree_node_folder_cb,
...
nserror global_history_init(void *core_window_handle)
{
    ...
    err = treeview_create(&gh_ctx.tree, &gh_tree_cb_t, N_FIELDS, gh_ctx.fields, core_window_handle,
    ...
    err = treeview_node_expand(gh_ctx.tree, gh_ctx.folders[GH_TODAY].folder);
    if (err != NSERROR_OK) {
        return err;
    }
...
static nserror global_history_export_enter_cb(void *ctx, void *node_data, enum treeview_node_type type, bool *abort)
{
    ...
    if (type == TREE_NODE_ENTRY) {
        ...
        ret = utf8_to_html(e->data[GH_TITLE].value, "iso-8859-1", e->data[GH_TITLE].value_len, &t_text);
        if (ret != NSERROR_OK)
            return NSERROR_SAVE_FAILED;

        ret = utf8_to_html(e->data[GH_URL].value, "iso-8859-1", e->data[GH_URL].value_len, &u_text);
        if (ret != NSERROR_OK) {
            free(t_text);
            return NSERROR_SAVE_FAILED;
        }
