    ...
    scrollbar_client_callback client_callback, struct scrollbar **s)
{
    ...
    scrollbar->bar_len = (full_size == 0) ? 0 : ((well_length * visible_size) / full_size);
 ...
 */
static inline nserror scrollbar_rectangle(const struct redraw_context *ctx, struct rect *area, colour c, bool inset)
{
    struct rect line;
    nserror res;
 ...
 * Exported interface.  Documented in scrollbar.h
 */
nserror scrollbar_redraw(
    struct scrollbar *s, int x, int y, const struct rect *clip, float scale, const struct redraw_context *ctx)
{
    ...
    bar_pos = s->bar_pos;
    bar_c1 = (s->horizontal ? area.x0 : area.y0) + SCROLLBAR_WIDTH + s->bar_pos + s->bar_len - 1;
    ...
    if ((area.x1 < clip->x0) || (area.y1 < clip->y0) || (clip->x1 < area.x0) || (clip->y1 < area.y0)) {
        return NSERROR_OK;
    }
...
void scrollbar_set(struct scrollbar *s, int value, bool bar_pos)
{
    ...
    if (bar_pos) {
        ...
        s->offset = ((well_length - s->bar_len) < 1)
            ? 0
            : (((s->full_size - s->visible_size) * s->bar_pos) / (well_length - s->bar_len));
        ...
        s->bar_pos = (s->full_size < 1) ? 0 : ((well_length * s->offset) / s->full_size);
    }
...
void scrollbar_set_extents(struct scrollbar *s, int length, int visible_size, int full_size)
{
    ...
    if (length == s->length && visible_size == s->visible_size && full_size == s->full_size) {
        /* Nothing's changed. */
        return;
    }
...
static void scrollbar_drag_start_internal(struct scrollbar *s, int x, int y, bool content_drag, bool pair)
{
    ...
    if (pair && s->pair != NULL) {
        ...
        s->pair->drag_start_pos = (content_drag) ? s->pair->offset : s->pair->bar_pos;
...
scrollbar_mouse_status scrollbar_mouse_action(struct scrollbar *s, browser_mouse_state mouse, int x, int y)
{
    ...
    bool but1 = ((mouse & BROWSER_MOUSE_PRESS_1) ||
        ((mouse & BROWSER_MOUSE_HOLDING_1) && (mouse & BROWSER_MOUSE_DRAG_ON) && !s->dragging));
    bool but2 = ((mouse & BROWSER_MOUSE_PRESS_2) ||
        ((mouse & BROWSER_MOUSE_HOLDING_2) && (mouse & BROWSER_MOUSE_DRAG_ON) && !s->dragging));
    ...
    if (s->dragging) {
        ...
        if (val != 0) {
            scrollbar_set(s, s->drag_start_pos + val, !(s->drag_content));
        }
        if (s->pair_drag) {
    ...
    if (mouse & (BROWSER_MOUSE_DRAG_1 | BROWSER_MOUSE_DRAG_2) &&
        (val >= SCROLLBAR_WIDTH + s->bar_pos && val < SCROLLBAR_WIDTH + s->bar_pos + s->bar_len)) {
        /* The mouse event is a drag start on the scrollbar position
         * indication bar. */
        scrollbar_drag_start_internal(s, x, y, false, (mouse & BROWSER_MOUSE_DRAG_2) ? true : false);
    }
...
void scrollbar_mouse_drag_end(struct scrollbar *s, browser_mouse_state mouse, int x, int y)
{
    ...
    if (s->pair_drag) {
        ...
        if (val != 0) {
            scrollbar_set(s->pair, drag_start_pos + val, !(s->pair->drag_content));
        }
