...
static nserror cookie_manager_walk_cb(void *ctx, void *node_data, enum treeview_node_type type, bool *abort)
{
    ...
    if (type == TREE_NODE_ENTRY) {
        ...
        if (folder->data.value_len == tw->title_len && strcmp(tw->title, folder->data.value) == 0) {
            /* Found what we're looking for */
            tw->folder = folder;
            *abort = true;
        }
 ...
 */
static nserror
cookie_manager_find_entry(treeview_node *root, const char *title, size_t title_len, struct cookie_manager_entry **found)
{
    nserror err;
    struct treeview_walk_ctx tw = {.title = title, .title_len = title_len, .folder = NULL, .entry = NULL};

    err = treeview_walk(cm_ctx.tree, root, cookie_manager_walk_cb, NULL, &tw, TREE_NODE_ENTRY);
    if (err != NSERROR_OK)
        return err;
 ...
 * \return NSERROR_OK on success, appropriate error otherwise
 */
static nserror cookie_manager_find_folder(
    treeview_node *root, const char *title, size_t title_len, struct cookie_manager_folder **found)
{
    nserror err;
    struct treeview_walk_ctx tw = {.title = title, .title_len = title_len, .folder = NULL, .entry = NULL};

    err = treeview_walk(cm_ctx.tree, root, cookie_manager_walk_cb, NULL, &tw, TREE_NODE_FOLDER);
    if (err != NSERROR_OK)
        return err;
...
cookie_manager_field_builder(enum cookie_manager_field field, struct treeview_field_data *data, const char *value)
{
 ...
 * \return NSERROR_OK on success, appropriate error otherwise
 */
static inline nserror cookie_manager_field_builder_time(
    enum cookie_manager_field field, struct treeview_field_data *fdata, const time_t *value)
{
    struct tm *ftime;
    ...
    if ((ftime = localtime(value)) != NULL) {
        ...
        if (value != NULL) {
            fdata->value = value;
            fdata->value_len = strftime(value, vsize, "%a %b %e %H:%M:%S %Y", ftime);
        }
    }
...
static nserror cookie_manager_set_treeview_field_data(struct cookie_manager_entry *e, const struct cookie_data *data)
{
    ...
    /* Set the fields up */
    cookie_manager_field_builder(COOKIE_M_NAME, &e->data[COOKIE_M_NAME], strdup(data->name));
    cookie_manager_field_builder(COOKIE_M_CONTENT, &e->data[COOKIE_M_CONTENT], strdup(data->value));
    cookie_manager_field_builder(COOKIE_M_DOMAIN, &e->data[COOKIE_M_DOMAIN], strdup(data->domain));
    cookie_manager_field_builder(COOKIE_M_PATH, &e->data[COOKIE_M_PATH], strdup(data->path));

    /* Set the Expires date field */
    if (data->expires == -1) {
        cookie_manager_field_builder(
            COOKIE_M_EXPIRES, &e->data[COOKIE_M_EXPIRES], strdup(messages_get("CookieManagerSession")));
    } else {
        cookie_manager_field_builder_time(COOKIE_M_EXPIRES, &e->data[COOKIE_M_EXPIRES], &data->expires);
    }

    /* Set the Last used date field */
    cookie_manager_field_builder_time(COOKIE_M_LAST_USED, &e->data[COOKIE_M_LAST_USED], &data->last_used);
...
static nserror cookie_manager_create_cookie_node(struct cookie_manager_folder *parent, const struct cookie_data *data)
{
    ...
    err = treeview_create_node_entry(cm_ctx.tree, &(cookie->entry), parent->folder, TREE_REL_FIRST_CHILD, cookie->data,
        cookie, cm_ctx.built ? TREE_OPTION_NONE : TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
    if (err != NSERROR_OK) {
 ...
 */
static nserror
cookie_manager_create_domain_folder(struct cookie_manager_folder **folder, const struct cookie_data *data)
{
    nserror err;
    ...
    err = treeview_create_node_folder(cm_ctx.tree, &(f->folder), NULL, TREE_REL_FIRST_CHILD, &f->data, f,
        cm_ctx.built ? TREE_OPTION_NONE : TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
    if (err != NSERROR_OK) {
...
bool cookie_manager_add(const struct cookie_data *data)
{
    ...
    err = cookie_manager_find_folder(NULL, data->domain, strlen(data->domain), &parent);
    if (err != NSERROR_OK) {
        return false;
    }
    ...
    /* Create cookie node */
    err = cookie_manager_find_entry(parent->folder, data->name, strlen(data->name), &cookie);
    if (err != NSERROR_OK)
        return false;
...
static nserror cookie_manager_init_entry_fields(void)
{
    ...
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_NAME].field) != lwc_error_ok) {
        goto error;
    }
    ...
    cm_ctx.fields[COOKIE_M_DOMAIN].flags = TREE_FLAG_SHOW_NAME | TREE_FLAG_SEARCHABLE;
    label = "TreeviewLabelDomain";
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_DOMAIN].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_PATH].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_LAST_USED].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_RESTRICTIONS].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &cm_ctx.fields[COOKIE_M_VERSION].field) != lwc_error_ok) {
        goto error;
    }
...
static nserror cookie_manager_init_common_values(void)
{
    ...
    cookie_manager_field_builder(COOKIE_M_RESTRICTIONS, &cm_ctx.values[COOKIE_M_HTTPS], strdup(temp));

    temp = messages_get("CookieManagerSecure");
    cookie_manager_field_builder(COOKIE_M_RESTRICTIONS, &cm_ctx.values[COOKIE_M_SECURE], strdup(temp));

    temp = messages_get("CookieManagerHTTP");
    cookie_manager_field_builder(COOKIE_M_RESTRICTIONS, &cm_ctx.values[COOKIE_M_HTTP], strdup(temp));

    temp = messages_get("None");
    cookie_manager_field_builder(COOKIE_M_RESTRICTIONS, &cm_ctx.values[COOKIE_M_NONE], strdup(temp));

    /* Set the Cookie version text */
    assert(COOKIE_NETSCAPE == 0);
    temp = messages_get("TreeVersion0");
    cookie_manager_field_builder(COOKIE_M_VERSION, &cm_ctx.values[COOKIE_M_NETSCAPE], strdup(temp));

    assert(COOKIE_RFC2109 == 1);
    temp = messages_get("TreeVersion1");
    cookie_manager_field_builder(COOKIE_M_VERSION, &cm_ctx.values[COOKIE_M_RFC2109], strdup(temp));

    assert(COOKIE_RFC2965 == 2);
    temp = messages_get("TreeVersion2");
    cookie_manager_field_builder(COOKIE_M_VERSION, &cm_ctx.values[COOKIE_M_RFC2965], strdup(temp));
...
struct treeview_callback_table cm_tree_cb_t = {.folder = cookie_manager_tree_node_folder_cb,
...
nserror cookie_manager_init(void *core_window_handle)
{
    ...
    err = treeview_create(&cm_ctx.tree, &cm_tree_cb_t, COOKIE_M_N_FIELDS, cm_ctx.fields, core_window_handle,
        TREEVIEW_NO_MOVES | TREEVIEW_DEL_EMPTY_DIRS | TREEVIEW_SEARCHABLE);
    if (err != NSERROR_OK) {
        cm_ctx.tree = NULL;
        return err;
    }
