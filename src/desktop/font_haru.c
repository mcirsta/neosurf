...
static bool haru_nsfont_init(HPDF_Doc *pdf, HPDF_Page *page, const char *string, char **string_nt, int length);

static bool haru_nsfont_width(const plot_font_style_t *fstyle, const char *string, size_t length, int *width);

static bool haru_nsfont_position_in_string(
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x);

static bool haru_nsfont_split(
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x);

static float pdf_text_scale = DEFAULT_EXPORT_SCALE;

const struct font_functions haru_nsfont = {haru_nsfont_width, haru_nsfont_position_in_string, haru_nsfont_split};
...
{
    NSLOG(netsurf, INFO, "ERROR: in font_haru \n\terror_no=%x\n\tdetail_no=%d\n", (HPDF_UINT)error_no,
        (HPDF_UINT)detail_no);
 ...
 */
bool haru_nsfont_width(const plot_font_style_t *fstyle, const char *string, size_t length, int *width)
{
    HPDF_Doc pdf;
    HPDF_Page page;
    char *string_nt;
...
#ifdef FONT_HARU_DEBUG
    NSLOG(netsurf, INFO, "Measuring string: %s ; Calculated width: %f %i", string_nt, width_real, *width);
#endif
    free(string_nt);
    HPDF_Free(pdf);
    ...
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x)
{
    ...
    if (HPDF_Page_SetWidth(page, x) != HPDF_OK || !haru_nsfont_apply_style(fstyle, pdf, page, NULL, NULL)) {
        free(string_nt);
        HPDF_Free(pdf);
        return false;
    }
    ...
    offset = HPDF_Page_MeasureText(page, string_nt, x, HPDF_FALSE, &real_width);
...
#ifdef FONT_HARU_DEBUG
    NSLOG(netsurf, INFO, "Position in string: %s at x: %i; Calculated position: %i", string_nt, x, *char_offset);
#endif
    free(string_nt);
    HPDF_Free(pdf);
    ...
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x)
{
    ...
    offset = HPDF_Page_MeasureText(page, string_nt, x, HPDF_TRUE, &real_width);

#ifdef FONT_HARU_DEBUG
    NSLOG(netsurf, INFO, "Splitting string: %s for width: %i ; Calculated position: %i Calculated real_width: %f",
