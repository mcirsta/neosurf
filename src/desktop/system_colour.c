...
nserror ns_system_colour_init(void)
{
    ...
    for (ccount = 0; ccount < colour_list_len; ccount++) {
        ...
        if (lwc_intern_string(opt->key + SLEN("sys_colour_"), opt->key_len - SLEN("sys_colour_"),
...
nserror ns_system_colour_char(const char *name, colour *colour_out)
{
    ...
    for (ccount = 0; ccount < colour_list_len; ccount++) {
        if (strcmp(name, nsoptions[ccount + NSOPTION_SYS_COLOUR_START].key + SLEN("sys_colour_")) == 0) {
...
css_error ns_system_colour(void *pw, lwc_string *name, css_color *colour)
{
    ...
    for (ccount = 0; ccount < colour_list_len; ccount++) {
        if (lwc_string_caseless_isequal(name, colour_list[ccount], &match) == lwc_error_ok && match) {
