...
static bool browser_window_resolve_frame_dimension(
    struct browser_window *bw, struct browser_window *sibling, int x, int y, bool width, bool height);
...
void browser_window_handle_scrollbars(struct browser_window *bw)
{
    ...
    if (scroll_y) {
        ...
        if (bw->scroll_y == NULL) {
            ...
            if (scrollbar_create(false, length, c_height, visible, bw, browser_window_scroll_callback,
                    &(bw->scroll_y)) != NSERROR_OK) {
                return;
            }
        } else {
            /* update vertical scrollbar */
            scrollbar_set_extents(bw->scroll_y, length, visible, c_height);
        }
    }
    ...
    if (scroll_x) {
        ...
        if (bw->scroll_x == NULL) {
            ...
            if (scrollbar_create(true, length, c_width, visible, bw, browser_window_scroll_callback, &(bw->scroll_x)) !=
                NSERROR_OK) {
                return;
            }
        } else {
            /* update horizontal scrollbar */
            scrollbar_set_extents(bw->scroll_x, length, visible, c_width);
        }
    }
...
nserror browser_window_create_iframes(struct browser_window *bw)
{
    ...
    for (cur = iframe; cur; cur = cur->next) {
        ...
        if (cur->url) {
            ...
            ret = browser_window_navigate(window, cur->url, hlcache_handle_get_url(bw->current_content),
                BW_NAVIGATE_UNVERIFIABLE, NULL, NULL, bw->current_content);
        }
    }
...
static void browser_window_recalculate_frameset_internal(struct browser_window *bw)
{
    ...
    for (row = 0; row < bw->rows; row++) {
        ...
        for (col = 0; col < bw->cols; col++) {
            ...
            case FRAME_DIMENSION_PIXELS:
                widths[col][row] = window->frame_width.value * window->scale;
                if (window->border) {
                    if (col != 0)
                        widths[col][row] += 1;
                    if (col != bw->cols - 1)
            ...
            default:
                ...
                assert(window->frame_width.unit == FRAME_DIMENSION_PIXELS ||
                    window->frame_width.unit == FRAME_DIMENSION_PERCENT ||
                    window->frame_width.unit == FRAME_DIMENSION_RELATIVE);
                break;
            }
        ...
        if ((relative > 0) && (avail_width > 0)) {
            ...
            for (col = 0; col < bw->cols; col++) {
                if (col == bw->cols - 1) {
                    ...
                    widths[col][row] = widths[col][row] < 0 ? 0 : widths[col][row];
                } else {
                    /* Find size of cell adjustment */
                    size = (widths[col][row] * extent) / (bw_width - extent);
                    /* Modify cell */
                    widths[col][row] += size;
                    applied += size;
                }
    ...
    for (col = 0; col < bw->cols; col++) {
        ...
        for (row = 0; row < bw->rows; row++) {
            ...
            case FRAME_DIMENSION_PIXELS:
                heights[col][row] = window->frame_height.value * window->scale;
                if (window->border) {
                    if (row != 0)
                        heights[col][row] += 1;
                    if (row != bw->rows - 1)
                ...
                break;
            case FRAME_DIMENSION_PERCENT:
                heights[col][row] = bw_height * window->frame_height.value / 100;
                break;
            case FRAME_DIMENSION_RELATIVE:
                heights[col][row] = 0;
            ...
            default:
                /* unknown frame dimension unit */
                assert(window->frame_height.unit == FRAME_DIMENSION_PIXELS ||
                    window->frame_height.unit == FRAME_DIMENSION_PERCENT ||
                    window->frame_height.unit == FRAME_DIMENSION_RELATIVE);
                break;
            }
        ...
        if ((relative > 0) && (avail_height > 0)) {
            ...
            for (row = 0; row < bw->rows; row++) {
                if (row == bw->rows - 1) {
                    ...
                    heights[col][row] = heights[col][row] < 0 ? 0 : heights[col][row];
                } else {
                    /* Find size of cell adjustment */
                    size = (heights[col][row] * extent) / (bw_height - extent);
                    /* Modify cell */
                    heights[col][row] += size;
                    applied += size;
                }
    ...
    for (row = 0; row < bw->rows; row++) {
        ...
        for (col = 0; col < bw->cols; col++) {
            ...
            if (window->width != new_width || window->height != new_height) {
                ...
                browser_window_reformat(window, false, new_width * bw->scale, new_height * bw->scale);
...
static nserror browser_window_create_frameset_internal(struct browser_window *bw, struct content_html_frames *frameset)
{
    ...
    for (window = bw; window->parent; window = window->parent) {
        if (window->current_content && content_get_type(window->current_content) == CONTENT_HTML)
            break;
    }
    ...
    for (row = 0; row < bw->rows; row++) {
        for (col = 0; col < bw->cols; col++) {
            ...
            if (frame->url) {
                browser_window_navigate(window, frame->url, hlcache_handle_get_url(parent),
                    BW_NAVIGATE_HISTORY | BW_NAVIGATE_UNVERIFIABLE, NULL, NULL, parent);
            }
        }
    }
...
bool browser_window_resolve_frame_dimension(
    struct browser_window *bw, struct browser_window *sibling, int x, int y, bool width, bool height)
{
...
static bool browser_window_resize_frames(
    struct browser_window *bw, browser_mouse_state mouse, int x, int y, browser_pointer_shape *pointer)
{
    struct browser_window *parent;
    bool left, right, up, down;
    int i, resize_margin;

    if ((x < bw->x) || (x > bw->x + bw->width) || (y < bw->y) || (y > bw->y + bw->height))
        return false;
...
bool browser_window_frame_resize_start(
    struct browser_window *bw, browser_mouse_state mouse, int x, int y, browser_pointer_shape *pointer)
{
    ...
    return browser_window_resize_frames(root, mouse, x + offx, y + offy, pointer);
}
