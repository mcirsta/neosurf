...
static void textarea_normalise_text(struct textarea *ta, unsigned int b_start, unsigned int b_len)
{
    ...
    for (index = 0; index < b_len; index++) {
        if (ta->text.data[b_start + index] == '\r') {
            if (b_start + index + 1 <= ta->text.len && ta->text.data[b_start + index + 1] == '\n') {
                ta->text.len--;
                ta->text.utf8_len--;
                memmove(ta->text.data + b_start + index, ta->text.data + b_start + index + 1,
...
static bool textarea_scroll_visible(struct textarea *ta)
{
    ...
    if (ta->bar_x == NULL && ta->scroll_x != 0 && ta->flags & TEXTAREA_MULTILINE) {
    ...
    if (ta->flags & TEXTAREA_MULTILINE) {
        ...
        y0 = 0;
        y1 = ta->vis_height - 2 * ta->border_width - ta->pad_top - ta->pad_bottom;
...
static bool textarea_set_caret_internal(struct textarea *ta, int caret_b)
{
    ...
    if (ta->caret_pos.byte_off != -1 && ta->flags & TEXTAREA_INTERNAL_CARET) {
    ...
    if (caret_b != -1) {
        ...
        guit->layout->width(&ta->fstyle, ta->show->data + ta->lines[ta->caret_pos.line].b_start,
        ...
        if (!scrolled && ta->flags & TEXTAREA_INTERNAL_CARET) {
            ...
            y1 = min(y + ta->line_height, ta->vis_height);
        ...
        if (!(ta->flags & TEXTAREA_INTERNAL_CARET)) {
            ...
            struct rect cr = {.x0 = ta->border_width,
                .y0 = ta->border_width,
                .x1 = ta->vis_width - ta->border_width - ((ta->bar_y == NULL) ? 0 : SCROLLBAR_WIDTH),
                .y1 = ta->vis_height - ta->border_width - ((ta->bar_x == NULL) ? 0 : SCROLLBAR_WIDTH)};
            msg.ta = ta;
...
static bool textarea_select(struct textarea *ta, int b_start, int b_end, bool force_redraw)
{
    ...
    if (ta->sel_start == b_start && ta->sel_end == b_end && !force_redraw)
        return true;

    msg.ta = ta;
    msg.type = TEXTAREA_MSG_REDRAW_REQUEST;
    msg.data.redraw.x0 = ta->border_width;
    msg.data.redraw.x1 = ta->vis_width - ta->border_width - ((ta->bar_y == NULL) ? 0 : SCROLLBAR_WIDTH);

    if (force_redraw) {
        /* Asked to redraw everything */
        msg.data.redraw.y0 = ta->border_width;
        msg.data.redraw.y1 = ta->vis_height - ta->border_width - ((ta->bar_x == NULL) ? 0 : SCROLLBAR_WIDTH);
    } else {
        ...
        if (!pre_existing_selection) {
            ...
            b_low = (ta->sel_start < b_start) ? ta->sel_start : b_start;
            b_high = (ta->sel_end > b_end) ? ta->sel_end : b_end;
        ...
        msg.data.redraw.y0 = max(ta->border_width, ta->line_height * line_start + ta->text_y_offset - ta->scroll_y);
        msg.data.redraw.y1 = min(ta->vis_height - ta->border_width - ((ta->bar_x == NULL) ? 0 : SCROLLBAR_WIDTH),
            ta->line_height * line_end + ta->text_y_offset + ta->line_height - ta->scroll_y);
    }
...
static void textarea_scrollbar_callback(void *client_data, struct scrollbar_msg_data *scrollbar_data)
{
    ...
    case SCROLLBAR_MSG_MOVED:
        ...
        if (!(ta->flags & TEXTAREA_INTERNAL_CARET) && ta->sel_start < 0 && ta->caret_pos.byte_off >= 0) {
            /* Tell client where caret should be placed */
            ...
            if ((x >= cr.x0 && x < cr.x1) && (y + h >= cr.y0 && y < cr.y1)) {
...
static bool textarea_reflow_singleline(struct textarea *ta, size_t b_off, struct rect *r)
{
    ...
    int w = ta->vis_width - 2 * ta->border_width - ta->pad_left - ta->pad_right;
    ...
    if (ta->flags & TEXTAREA_PASSWORD && ta->text.utf8_len != ta->password.utf8_len) {
        ...
        for (c = 0; c < b_len; c += rep_len) {
            for (b = 0; b < rep_len; b++) {
                ta->password.data[c + b] = PASSWORD_REPLACEMENT[b];
            }
        }
    ...
    /* Measure new width */
    guit->layout->width(&ta->fstyle, ta->show->data, ta->show->len - 1, &x);

    /* Get width of retained text */
    if (b_off != ta->lines[0].b_length) {
        guit->layout->width(&ta->fstyle, ta->show->data, b_off, &retained_width);
    } else {
...
static bool textarea_reflow_multiline(struct textarea *ta, const size_t b_start, const int b_length, struct rect *r)
{
    ...
    scroll_lines = (ta->vis_height - 2 * ta->border_width - ta->pad_top - ta->pad_bottom) / ta->line_height;
    ...
    do {
        ...
        avail_width = ta->vis_width - 2 * ta->border_width - ta->pad_left - ta->pad_right;
        if (avail_width < 0)
            avail_width = 0;
        ...
        for (; len > 0; len -= b_off, text += b_off) {
            ...
            guit->layout->split(&ta->fstyle, text, para_end - text, avail_width, &b_off, &x);
            ...
            if (x > avail_width && ta->bar_x == NULL) {
                ...
                if (scrollbar_create(true, w, w, w, ta, textarea_scrollbar_callback, &(ta->bar_x)) != NSERROR_OK) {
                    return false;
                }
                if (ta->bar_y != NULL)
                    scrollbar_make_pair(ta->bar_x, ta->bar_y);
            ...
            if (line > ta->lines_alloc_size - 2) {
                ...
                ta->lines = temp;
                ta->lines_alloc_size = line + 2 + LINE_CHUNK_SIZE;
            }
            ...
            if (para_end == text + b_off && *para_end == '\n') {
                ...
                if (len - b_off == 0) {
                     ...
                     * add last line */
                    ta->lines[line].b_start = text + b_off - ta->text.data;
                    ta->lines[line].b_length = 0;
                    ta->lines[line++].width = x;
                }
        ...
        if (line > scroll_lines && ta->bar_y == NULL) {
            ...
            if (scrollbar_create(false, h, h, h, ta, textarea_scrollbar_callback, &(ta->bar_y)) != NSERROR_OK) {
                return false;
            }
            if (ta->bar_x != NULL)
                scrollbar_make_pair(ta->bar_x, ta->bar_y);
    ...
    h_extent += ta->pad_left + ta->pad_right - (ta->bar_y != NULL ? SCROLLBAR_WIDTH : 0);
    v_extent = line * ta->line_height + ta->pad_top + ta->pad_bottom - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0);

    if (ta->bar_x != NULL) {
        /* Set horizontal scrollbar extents */
        int w = ta->vis_width - 2 * ta->border_width - (ta->bar_y != NULL ? SCROLLBAR_WIDTH : 0);
    ...
    if (ta->bar_y != NULL) {
        ...
        int h = ta->vis_height - 2 * ta->border_width;
        scrollbar_set_extents(ta->bar_y, h, h - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0), v_extent);
    }
    ...
    if (ta->lines[start].b_start + ta->lines[start].b_length > b_start_line_end) {
        b_start_line_end = ta->lines[start].b_start + ta->lines[start].b_length;
    }
    ...
    r->y0 = max(r->y0, (signed)(ta->line_height * start + ta->text_y_offset - ta->scroll_y));

    /* Reduce redraw region to single line if possible */
    if ((skip_line || start == 0) && ta->lines[start].b_start + ta->lines[start].b_length >= b_start + b_length) {
        size_t b_line_end = ta->lines[start].b_start + ta->lines[start].b_length;
        text = ta->text.data + b_line_end;
        if (*text == '\0' || *text == '\n') {
            r->y1 = min(r->y1, (signed)(ta->line_height * (start + 1) + ta->text_y_offset - ta->scroll_y));
            if (b_start > ta->lines[start].b_start && b_start <= b_line_end) {
                /* Remove unchanged text at start of line
                 * from redraw region */
                int retained_width = 0;
                size_t retain_end = b_start - ta->lines[start].b_start;
                text = ta->text.data + ta->lines[start].b_start;

                guit->layout->width(&ta->fstyle, text, retain_end, &retained_width);

                r->x0 = max(r->x0, retained_width + ta->border_width + ta->pad_left - ta->scroll_x - 1);
            }
        }
    }
...
static size_t textarea_get_b_off_xy(struct textarea *ta, int x, int y, bool visible)
{
    ...
    x = x - ta->border_width - ta->pad_left + (visible ? ta->scroll_x : 0);
    y = y - ta->border_width - ta->pad_top + (visible ? ta->scroll_y : 0);
    ...
    /* Get byte position */
    guit->layout->position(
        &ta->fstyle, ta->show->data + ta->lines[line].b_start, ta->lines[line].b_length, x, &bpos, &x);
     ...
     */
    if (ta->flags & TEXTAREA_MULTILINE && ta->lines[line].b_length > 1 && bpos == (unsigned)ta->lines[line].b_length &&
        ta->show->data[ta->lines[line].b_start + ta->lines[line].b_length - 1] == ' ')
        bpos--;
 ...
 */
static bool
textarea_insert_text(struct textarea *ta, const char *text, size_t b_off, size_t b_len, int *byte_delta, struct rect *r)
{
    int char_delta;
    ...
    if (ta->flags & TEXTAREA_PASSWORD) {
        ...
        b_off = utf8_bounded_byte_length(ta->text.data, ta->text.len - 1, c_off);
    }
    ...
    memmove(ta->text.data + b_off + b_len, ta->text.data + b_off, ta->text.len - b_off);
    /* Insert new text */
 ...
 */
static inline void textarea_char_to_byte_offset(
    struct textarea_utf8 *text, unsigned int start, unsigned int end, size_t *b_start, size_t *b_end)
{
    size_t diff = end - start;
    /* find byte offset of replace start */
    for (*b_start = 0; start-- > 0; *b_start = utf8_next(text->data, text->len - 1, *b_start))
        ; /* do nothing */

    /* find byte length of replaced text */
    for (*b_end = *b_start; diff-- > 0; *b_end = utf8_next(text->data, text->len - 1, *b_end))
        ; /* do nothing */
}
 ...
 */
static bool textarea_replace_text_internal(struct textarea *ta, size_t b_start, size_t b_end, const char *rep,
    size_t rep_len, bool add_to_clipboard, int *byte_delta, struct rect *r)
{
    int char_delta;
    const size_t show_b_off = b_start;
    *byte_delta = 0;

    if ((ta->flags & TEXTAREA_READONLY) && !(rep == NULL && rep_len == 0 && add_to_clipboard))
    ...
    r->x1 = ta->vis_width - ta->border_width - ((ta->bar_y == NULL) ? 0 : SCROLLBAR_WIDTH);
    r->y1 = ta->vis_height - ta->border_width - ((ta->bar_x == NULL) ? 0 : SCROLLBAR_WIDTH);

    /* Early exit if just inserting */
    if (b_start == b_end && rep != NULL)
        return textarea_insert_text(ta, rep, b_start, rep_len, byte_delta, r);
    ...
    if (add_to_clipboard) {
        guit->clipboard->set(ta->show->data + b_start, b_end - b_start, NULL, 0);
    }
    ...
    if (ta->flags & TEXTAREA_PASSWORD) {
        ...
        c_end = c_start;
        c_end += utf8_bounded_length(ta->password.data + b_start, b_end - b_start);
        textarea_char_to_byte_offset(&ta->text, c_start, c_end, &b_start, &b_end);
    }
    ...
    if (rep_len + ta->text.len - (b_end - b_start) >= ta->text.alloc) {
        ...
        ta->text.data = temp;
        ta->text.alloc = rep_len + ta->text.len - (b_end - b_start) + TA_ALLOC_STEP;
    }

    /* Shift text following to new position */
    memmove(ta->text.data + b_start + rep_len, ta->text.data + b_end, ta->text.len - b_end);
...
static bool textarea_copy_to_undo_buffer(struct textarea *ta, size_t b_start, size_t b_end, size_t rep_len)
{
    ...
    if (undo->next_detail == 0)
        ...
        b_offset = undo->details[undo->next_detail - 1].b_start + undo->details[undo->next_detail - 1].b_limit;
    ...
    memcpy(undo->text.data + b_offset, ta->text.data + b_start, b_end - b_start);
 ...
 * password textarea case, they are for ta->password.
 */
static bool textarea_replace_text(struct textarea *ta, size_t b_start, size_t b_end, const char *rep, size_t rep_len,
    bool add_to_clipboard, int *byte_delta, struct rect *r)
{
    if (!(b_start != b_end && rep == NULL && add_to_clipboard) && !(ta->flags & TEXTAREA_PASSWORD)) {
        /* Not just copying to clipboard, and not a password field;
         * Sort out undo buffer. */
        if (textarea_copy_to_undo_buffer(ta, b_start, b_end, rep_len) == false)
            return false;
    }

    /* Replace the text in the textarea, and reflow it */
    if (textarea_replace_text_internal(ta, b_start, b_end, rep, rep_len, add_to_clipboard, byte_delta, r) == false) {
        return false;
    }
    ...
    if (!(b_start != b_end && rep == NULL && add_to_clipboard) && !(ta->flags & TEXTAREA_PASSWORD)) {
        /* Not just copying to clipboard, and not a password field;
         * Update UNDO buffer */
        ta->undo.details[ta->undo.next_detail].b_text_end = b_end + *byte_delta;
...
static bool textarea_undo(struct textarea *ta, bool forward, unsigned int *caret, struct rect *r)
{
    ...
    textarea_replace_text_internal(ta, detail->b_text_start, detail->b_text_end, ta->undo.text.data + detail->b_start,
        b_len, false, &byte_delta, r);
...
static bool textarea_drag_end(struct textarea *ta, browser_mouse_state mouse, int x, int y)
{
    ...
    case TEXTAREA_DRAG_SCROLLBAR:
        if (ta->drag_info.data.scrollbar == ta->bar_x) {
            x -= ta->border_width;
            y -= ta->vis_height - ta->border_width - SCROLLBAR_WIDTH;
        } else {
            x -= ta->vis_width - ta->border_width - SCROLLBAR_WIDTH;
            y -= ta->border_width;
        }
        scrollbar_mouse_drag_end(ta->drag_info.data.scrollbar, mouse, x, y);
        assert(ta->drag_info.type == TEXTAREA_DRAG_NONE);
...
static void textarea_setup_text_offsets(struct textarea *ta)
{
    ...
    if (ta->flags & TEXTAREA_MULTILINE) {
        ...
        text_y_offset_baseline += (ta->line_height * 3 + 2) / 4 + ta->pad_top;
    } else {
 ...
 */
static void
textarea_set_text_style(struct textarea *ta, const plot_font_style_t *fstyle, colour selected_text, colour selected_bg)
{
    ta->fstyle = *fstyle;
...
textarea_create(const textarea_flags flags, const textarea_setup *setup, textarea_client_callback callback, void *data)
{
    ...
    textarea_set_text_style(ret, &setup->text, setup->selected_text, setup->selected_bg);
...
bool textarea_drop_text(struct textarea *ta, const char *text, size_t text_length)
{
    ...
    if (ta->sel_start != -1) {
        if (!textarea_replace_text(ta, ta->sel_start, ta->sel_end, text, text_length, false, &byte_delta, &r))
            return false;

        caret_pos = ta->sel_end;
        ta->sel_start = ta->sel_end = -1;
    } else {
        if (!textarea_replace_text(ta, caret_pos, caret_pos, text, text_length, false, &byte_delta, &r))
            return false;
    }
...
bool textarea_set_caret(struct textarea *ta, int caret)
{
    ...
    if (caret < 0) {
        ...
        b_off = utf8_bounded_byte_length(ta->show->data, ta->show->len - 1, caret);
    ...
    const struct redraw_context *ctx)
{
    ...
    plot_style_t plot_style_fill_bg = {.stroke_type = PLOT_OP_TYPE_NONE,
        .stroke_width = 0,
    ...
    if (scale == 1.0) {
        ...
        if (r.x0 > x + ta->vis_width * scale || r.y0 > y + ta->vis_height * scale)
            return;
    }
    ...
    ctx->plot->clip(ctx, &r);
    if (ta->border_col != NS_TRANSPARENT && ta->border_width > 0) {
        /* Plot border */
        rect.x0 = x;
        rect.y0 = y;
    ...
    if (scale == 1.0) {
        ...
        if (r.y1 > y + ta->vis_height - ta->border_width - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0))
            r.y1 = y + ta->vis_height - ta->border_width - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0);
    } else {
        ...
        if (r.y1 > y + (ta->vis_height - ta->border_width - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0)) * scale)
            r.y1 = y + (ta->vis_height - ta->border_width - (ta->bar_x != NULL ? SCROLLBAR_WIDTH : 0)) * scale;
    }
    ...
    for (line = line0; (line <= line1) && (y + line * ta->line_height <= r.y1 + ta->scroll_y); line++) {
        ...
        if (ta->sel_end == -1 || ta->sel_end == ta->sel_start || sel_end < ta->lines[line].b_start ||
            sel_start > ta->lines[line].b_start + ta->lines[line].b_length) {
            /* Simple case; no selection on this line */
            fstyle = ta->fstyle;
            fstyle.size = fsize;

            ctx->plot->text(ctx, &fstyle, x + ta->border_width + ta->pad_left - ta->scroll_x,
                y + line_y + text_y_offset_baseline, ta->show->data + ta->lines[line].b_start,
            ...
            do {
                ...
                } else if (sel_start <= b_pos && sel_end > b_pos + b_len) {
                ...
                left = right;
                if (b_len_part != b_len) {
                    guit->layout->width(&fstyle, line_text, b_end, &right);
                } else {
                ...
                if (s.x1 <= left || s.x0 > right) {
                    /* Skip this span, it's outside the
                     * visible */
                    b_pos += b_len_part;
                    b_len -= b_len_part;
                    continue;
        ...
        if (line < ta->line_count - 1 &&
            ta->lines[line + 1].b_start != ta->lines[line].b_start + ta->lines[line].b_length)
            b_pos++;
    }

    if (ta->flags & TEXTAREA_INTERNAL_CARET && (ta->sel_end == -1 || ta->sel_start == ta->sel_end) &&
    ...
    if (ta->bar_x != NULL) {
        scrollbar_redraw(ta->bar_x, x / scale + ta->border_width,
            y / scale + ta->vis_height - ta->border_width - SCROLLBAR_WIDTH, clip, scale, ctx);
    }

    if (ta->bar_y != NULL) {
        scrollbar_redraw(ta->bar_y, x / scale + ta->vis_width - ta->border_width - SCROLLBAR_WIDTH,
            y / scale + ta->border_width, clip, scale, ctx);
    }
}
...
bool textarea_keypress(struct textarea *ta, uint32_t key)
{
    ...
    if (!(key <= 0x001F || (0x007F <= key && key <= 0x009F))) {
        ...
        if (ta->sel_start != -1) {
            if (!textarea_replace_text(ta, ta->sel_start, ta->sel_end, utf8, length, false, &byte_delta, &r))
                return false;
            ...
            if (!textarea_replace_text(ta, caret, caret, utf8, length, false, &byte_delta, &r))
                return false;
            redraw = true;
        }
        caret += byte_delta;
        ...
        case NS_KEY_COPY_SELECTION:
            if (ta->sel_start != -1) {
                if (!textarea_replace_text(ta, ta->sel_start, ta->sel_end, NULL, 0, true, &byte_delta, &r))
                    return false;
            }
            break;
        case NS_KEY_DELETE_LEFT:
            if (readonly)
                break;
            if (ta->sel_start != -1) {
                if (!textarea_replace_text(ta, ta->sel_start, ta->sel_end, "", 0, false, &byte_delta, &r))
                    return false;
                ...
                if (!textarea_replace_text(ta, b_off, caret, "", 0, false, &byte_delta, &r))
                    return false;
                redraw = true;
            }
            caret += byte_delta;
        ...
        case NS_KEY_DELETE_RIGHT:
            ...
            if (ta->sel_start != -1) {
                ...
                b_off = utf8_next(ta->show->data, ta->show->len - 1, caret);
                if (!textarea_replace_text(ta, caret, b_off, "", 0, false, &byte_delta, &r))
                    return false;
                caret = b_off;
                redraw = true;
            }
        ...
        case NS_KEY_PASTE: {
            ...
            if (ta->sel_start != -1) {
                if (!textarea_replace_text(
                        ta, ta->sel_start, ta->sel_end, clipboard, clipboard_length, false, &byte_delta, &r))
                    return false;
                ...
                if (!textarea_replace_text(ta, caret, caret, clipboard, clipboard_length, false, &byte_delta, &r))
                    return false;
                redraw = true;
            }
            caret += byte_delta;
        ...
        case NS_KEY_CUT_SELECTION:
            if (readonly)
                break;
            if (ta->sel_start != -1) {
                if (!textarea_replace_text(ta, ta->sel_start, ta->sel_end, "", 0, true, &byte_delta, &r))
                    return false;
        ...
        case NS_KEY_RIGHT:
            ...
            if (caret < ta->show->len - 1)
                caret = utf8_next(ta->show->data, ta->show->len - 1, caret);
            if (ta->sel_start != -1) {
                textarea_clear_selection(ta);
            }
            break;
        ...
        case NS_KEY_PAGE_UP:
            if (!(ta->flags & TEXTAREA_MULTILINE))
                break;
            y = ta->vis_height - 2 * ta->border_width - ta->pad_top - ta->pad_bottom - ta->line_height;
        ...
        case NS_KEY_LINE_END:
            ...
            caret = ta->lines[line].b_start + ta->lines[line].b_length;

            if (!(ta->flags & TEXTAREA_PASSWORD) && caret > 0 && ta->text.data[caret - 1] == ' ')
                caret--;
            if (ta->sel_start != -1) {
                textarea_clear_selection(ta);
            }
        ...
        case NS_KEY_DELETE_WORD_LEFT:
            ...
            if (!textarea_replace_text(ta, caret, caret_copy, "", 0, false, &byte_delta, &r))
                return false;
        ...
        case NS_KEY_DELETE_WORD_RIGHT:
            ...
            for (; caret_copy < ta->show->len - 1; caret_copy++) {
                if (strchr(sep, ta->show->data[caret_copy]) != NULL) {
                    break;
                }
            }
        ...
        case NS_KEY_DELETE_LINE:
            ...
            if (ta->sel_start != -1) {
                ...
                if (ta->lines[line].b_length != 0) {
                    ...
                    b_len = ta->lines[line].b_length;
                    if (!textarea_replace_text(ta, caret, caret + b_len, "", 0, false, &byte_delta, &r))
                        return false;
                    caret = caret + b_len;
                } else if (caret < ta->show->len - 1) {
                    /* Delete blank line */
                    if (!textarea_replace_text(ta, caret, caret + 1, "", 0, false, &byte_delta, &r))
                        return false;
                    caret++;
                }
                redraw = true;
        ...
        case NS_KEY_DELETE_LINE_START:
            ...
            if (ta->sel_start != -1) {
                ...
                if (!textarea_replace_text(ta, caret - ta->caret_pos.byte_off, caret, "", 0, false, &byte_delta, &r))
                    return false;
                redraw = true;
            }
            caret += byte_delta;
    ...
    if (bar_x != (ta->bar_x != NULL) || bar_y != (ta->bar_y != NULL) || h_extent != ta->h_extent ||
        v_extent != ta->v_extent) {
 ...
 */
static textarea_mouse_status
textarea_mouse_scrollbar_action(struct textarea *ta, browser_mouse_state mouse, int x, int y)
{
    ...
    if (ta->drag_info.type == TEXTAREA_DRAG_SCROLLBAR) {
        ...
        return (scrollbar_mouse_action(ta->drag_info.data.scrollbar, mouse, x, y) << 3);
    }
    ...
    if (ta->bar_x != NULL && ta->drag_info.type == TEXTAREA_DRAG_NONE) {
        ...
        sl = ta->vis_width - 2 * ta->border_width - (ta->bar_y != NULL ? SCROLLBAR_WIDTH : 0);

        if (sx >= 0 && sy >= 0 && sx < sl && sy < SCROLLBAR_WIDTH) {
            return (scrollbar_mouse_action(ta->bar_x, mouse, sx, sy) << 3);
        }
    }
    ...
    if (ta->bar_y != NULL && ta->drag_info.type == TEXTAREA_DRAG_NONE) {
        ...
        if (sx >= 0 && sy >= 0 && sx < SCROLLBAR_WIDTH && sy < sl) {
            return (scrollbar_mouse_action(ta->bar_y, mouse, sx, sy) << 3);
        }
    }
...
textarea_mouse_status textarea_mouse_action(struct textarea *ta, browser_mouse_state mouse, int x, int y)
{
    ...
    if (ta->drag_info.type != TEXTAREA_DRAG_NONE && mouse == BROWSER_MOUSE_HOVER) {
    ...
    if ((x >= ta->vis_width || y >= ta->vis_height) && ta->drag_info.type == TEXTAREA_DRAG_NONE &&
    ...
    if (mouse & BROWSER_MOUSE_DOUBLE_CLICK) {
        ...
        if (ta->sel_start != -1) {
            /* Adjust selection */
            b_start = (ta->sel_end - ta->sel_start) / 2 + ta->sel_start;
            b_start = ((unsigned)b_start > b_off) ? ta->sel_end : ta->sel_start;
    ...
    } else if (mouse & (BROWSER_MOUSE_HOLDING_1 | BROWSER_MOUSE_HOLDING_2) &&
        ta->drag_info.type == TEXTAREA_DRAG_SELECTION) {
        /* Selection track */
        int scrx = 0;
        int scry = 0;
        ...
        w = ta->vis_width - ta->border_width - ((ta->bar_y == NULL) ? 0 : SCROLLBAR_WIDTH);
        h = ta->vis_height - ta->border_width - ((ta->bar_x == NULL) ? 0 : SCROLLBAR_WIDTH);
    ...
    int right, int bottom, int left)
{
    ...
    textarea_set_text_style(ta, fstyle, ta->sel_fstyle.foreground, ta->sel_fstyle.background);
...
bool textarea_scroll(struct textarea *ta, int scrx, int scry)
{
    ...
    if (ta->flags & TEXTAREA_MULTILINE) {
        ...
        if (ta->bar_x != NULL && scrx != 0 && scrollbar_scroll(ta->bar_x, scrx))
            handled_scroll = true;
        if (ta->bar_y != NULL && scry != 0 && scrollbar_scroll(ta->bar_y, scry))
            handled_scroll = true;
