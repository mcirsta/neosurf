...
static struct history_entry *browser_window_history__clone_entry(struct history *history, struct history_entry *entry)
{
    ...
    if (entry->page.bitmap == NULL) {
        ...
        new_entry->page.bitmap = guit->bitmap->create(LOCAL_HISTORY_WIDTH, LOCAL_HISTORY_HEIGHT, BITMAP_OPAQUE);
...
static int browser_window_history__layout_subtree(struct history *history, struct history_entry *entry, int x, int y)
{
    ...
    for (child = entry->forward; child; child = child->next) {
        y1 = browser_window_history__layout_subtree(
            history, child, x + LOCAL_HISTORY_WIDTH + LOCAL_HISTORY_RIGHT_MARGIN, y1);
        if (child->next)
...
static void browser_window_history__layout(struct history *history)
{
    ...
    if (history->start)
        history->height = browser_window_history__layout_subtree(
            history, history->start, LOCAL_HISTORY_RIGHT_MARGIN / 2, LOCAL_HISTORY_BOTTOM_MARGIN / 2);
    else
        history->height = 0;
 ...
 */
static bool browser_window_history__enumerate_entry(const struct browser_window *bw, const struct history_entry *entry,
    browser_window_history_enumerate_cb cb, void *ud)
{
    const struct history_entry *child;

    if (!cb(bw, entry->x, entry->y, entry->x + LOCAL_HISTORY_WIDTH, entry->y + LOCAL_HISTORY_HEIGHT, entry, ud))
        return false;
...
nserror browser_window_history_clone(const struct browser_window *existing, struct browser_window *clone)
{
    ...
    if (existing == NULL || existing->history == NULL || existing->history->start == NULL)
...
nserror browser_window_history_update(struct browser_window *bw, struct hlcache_handle *content)
{
    ...
    if ((bw->window != NULL) && guit->window->get_scroll(bw->window, &sx, &sy)) {
...
nserror browser_window_history_back(struct browser_window *bw, bool new_window)
{
    if (bw != NULL && bw->internal_nav) {
        ...
        if (bw->current_parameters.url != NULL) {
             ...
             */
            return browser_window_navigate(bw, corestring_nsurl_about_blank, NULL, /* Referer */
                BW_NAVIGATE_HISTORY, NULL, /* Post */
                NULL, /* Post */
                NULL /* parent fetch */);
        }
    }

    if (!bw || !bw->history || !bw->history->current || !bw->history->current->back) {
        return NSERROR_BAD_PARAMETER;
    }
    return browser_window_history_go(bw, bw->history->current->back, new_window);
}
...
{
    if (!bw || !bw->history || !bw->history->current || !bw->history->current->forward_pref) {
        return NSERROR_BAD_PARAMETER;
    }
    return browser_window_history_go(bw, bw->history->current->forward_pref, new_window);
}
...
{
    return (bw && bw->history && bw->history->current && bw->history->current->back);
}
...
{
    return (bw && bw->history && bw->history->current && bw->history->current->forward_pref);
}
...
nserror browser_window_history_go(struct browser_window *bw, struct history_entry *entry, bool new_window)
{
    ...
    if (entry->page.frag_id) {
        error = nsurl_refragment(entry->page.url, entry->page.frag_id, &url);
    ...
    if (new_window) {
        ...
        error = browser_window_create(BW_CREATE_CLONE, url, NULL, bw, NULL);
        ...
        history->current = entry;
        error = browser_window_navigate(bw, url, NULL, BW_NAVIGATE_NO_TERMINAL_HISTORY_UPDATE, NULL, NULL, NULL);
    }
    ...
    const struct browser_window *bw, browser_window_history_enumerate_cb cb, void *user_data)
{
    ...
    for (; e != NULL; e = e->forward_pref) {
        if (!cb(bw, e->x, e->y, e->x + LOCAL_HISTORY_WIDTH, e->y + LOCAL_HISTORY_HEIGHT, e, user_data))
            break;
    }
}
    ...
    const struct browser_window *bw, browser_window_history_enumerate_cb cb, void *user_data)
{
    if (bw == NULL || bw->history == NULL)
        return;
    browser_window_history__enumerate_entry(bw, bw->history->start, cb, user_data);
}
