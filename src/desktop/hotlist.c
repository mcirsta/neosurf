...
static nserror hotlist_schedule_save(void)
{
    if (hl_ctx.save_scheduled == false && hl_ctx.save_path != NULL) {
        nserror err = guit->misc->schedule(10 * 1000, hotlist_schedule_save_cb, NULL);
        if (err != NSERROR_OK) {
            return err;
        }
 ...
 */
static nserror
hotlist_create_treeview_field_data(struct hotlist_entry *e, const char *title, const struct url_data *data)
{
    nserror err;
    ...
    if (title == NULL) {
        ...
        title = (data->title != NULL) ? data->title : nsurl_access(e->url);
    }
    ...
    e->data[HL_TITLE].value_len = (e->data[HL_TITLE].value != NULL) ? strlen(title) : 0;
...
static nserror hotlist_entry_insert(struct hotlist_entry *e, treeview_node *relation, enum treeview_relationship rel)
{
    ...
    err = treeview_create_node_entry(hl_ctx.tree, &(e->entry), relation, rel, e->data, e,
        hl_ctx.built ? TREE_OPTION_NONE : TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW);
    if (err != NSERROR_OK) {
        return err;
    }
 ...
 */
static nserror
hotlist_create_entry(nsurl *url, const char *title, const struct url_data *data, struct hotlist_entry **entry)
{
    nserror err;
 ...
 * \return NSERROR_OK on success, or appropriate error otherwise
 */
static nserror hotlist_add_entry_internal(nsurl *url, const char *title, const struct url_data *data,
    treeview_node *relation, enum treeview_relationship rel, treeview_node **entry)
{
    nserror err;
    struct hotlist_entry *e;
 ...
 * \return NSERROR_OK on success, or appropriate error otherwise
 */
static nserror hotlist_add_folder_internal(const char *title, treeview_node *relation, enum treeview_relationship rel,
    struct hotlist_folder **folder, bool default_folder)
{
    ...
    if (!hl_ctx.built)
        flags |= TREE_OPTION_SUPPRESS_RESIZE | TREE_OPTION_SUPPRESS_REDRAW;
    if (default_folder)
        flags |= TREE_OPTION_SPECIAL_DIR;

    err = treeview_create_node_folder(hl_ctx.tree, &n, relation, rel, &f->data, f, flags);
    if (err != NSERROR_OK) {
...
static nserror hotlist_tree_node_folder_cb(struct treeview_node_msg msg, void *data)
{
    ...
    case TREE_MSG_NODE_EDIT:
        if (lwc_string_isequal(hl_ctx.fields[HL_FOLDER].field, msg.data.node_edit.field, &match) == lwc_error_ok &&
            match == true && msg.data.node_edit.text != NULL && msg.data.node_edit.text[0] != '\0') {
            ...
            if (f->data.value == NULL) {
                ...
                treeview_update_node_folder(hl_ctx.tree, f->folder, &f->data, f);
                free((void *)old_text);
            }
        }
        break;
...
static nserror hotlist_tree_node_entry_cb(struct treeview_node_msg msg, void *data)
{
    ...
    case TREE_MSG_NODE_EDIT:
        if (lwc_string_isequal(hl_ctx.fields[HL_TITLE].field, msg.data.node_edit.field, &match) == lwc_error_ok &&
            match == true && msg.data.node_edit.text != NULL && msg.data.node_edit.text[0] != '\0') {
            /* Requst to change the entry title text */
            old_text = e->data[HL_TITLE].value;
            e->data[HL_TITLE].value = strdup(msg.data.node_edit.text);

            if (e->data[HL_TITLE].value == NULL) {
                e->data[HL_TITLE].value = old_text;
            } else {
                e->data[HL_TITLE].value_len = strlen(e->data[HL_TITLE].value);
                treeview_update_node_entry(hl_ctx.tree, e->entry, e->data, e);
                free((void *)old_text);
            }

            err = hotlist_schedule_save();

        } else if (lwc_string_isequal(hl_ctx.fields[HL_URL].field, msg.data.node_edit.field, &match) == lwc_error_ok &&
            match == true && msg.data.node_edit.text != NULL && msg.data.node_edit.text[0] != '\0') {
...
struct treeview_callback_table hl_tree_cb_t = {.folder = hotlist_tree_node_folder_cb,
...
static nserror hotlist_load_directory(dom_node *ul, hotlist_load_ctx *ctx)
{
    ...
    return libdom_iterate_child_elements(ul, hotlist_load_directory_cb, ctx);
}
...
nserror hotlist_load_directory_cb(dom_node *node, void *ctx)
{
    ...
    } else if (current_ctx->last_was_h4 && dom_string_caseless_lwc_isequal(name, corestring_lwc_ul)) {
...
static nserror hotlist_generate(void)
{
    ...
    for (i = n_entries - 1; i >= 0; i--) {
        ...
        /* Build the node */
        err = hotlist_add_entry_internal(url, title, NULL, f->folder, TREE_REL_FIRST_CHILD, &e);
        nsurl_unref(url);
...
static nserror hotlist_export_enter_cb(void *ctx, void *node_data, enum treeview_node_type type, bool *abort)
{
    ...
    if (type == TREE_NODE_ENTRY) {
        ...
        ret = utf8_to_html(e->data[HL_TITLE].value, "iso-8859-1", e->data[HL_TITLE].value_len, &t_text);
        if (ret != NSERROR_OK)
            return NSERROR_SAVE_FAILED;

        ret = utf8_to_html(e->data[HL_URL].value, "iso-8859-1", e->data[HL_URL].value_len, &u_text);
        if (ret != NSERROR_OK) {
            free(t_text);
            return NSERROR_SAVE_FAILED;
        }
        ...
        ret = utf8_to_html(f->data.value, "iso-8859-1", f->data.value_len, &f_text);
        if (ret != NSERROR_OK)
            return NSERROR_SAVE_FAILED;
    ...
    void *ctx, hotlist_folder_enter_cb enter_cb, hotlist_address_cb address_cb, hotlist_folder_leave_cb leave_cb)
{
    ...
    err = treeview_walk(hl_ctx.tree, NULL, hotlist_iterate_enter_cb, hotlist_iterate_leave_cb, &data,
...
static nserror hotlist_initialise_entry_fields(void)
{
    ...
    hl_ctx.fields[HL_TITLE].flags = TREE_FLAG_DEFAULT | TREE_FLAG_ALLOW_EDIT;
    label = "TreeviewLabelTitle";
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &hl_ctx.fields[HL_TITLE].field) != lwc_error_ok) {
        goto error;
    }

    hl_ctx.fields[HL_URL].flags = TREE_FLAG_ALLOW_EDIT | TREE_FLAG_COPY_TEXT | TREE_FLAG_SEARCHABLE;
    label = "TreeviewLabelURL";
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &hl_ctx.fields[HL_URL].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &hl_ctx.fields[HL_LAST_VISIT].field) != lwc_error_ok) {
        goto error;
    }
    ...
    label = messages_get(label);
    if (lwc_intern_string(label, strlen(label), &hl_ctx.fields[HL_VISITS].field) != lwc_error_ok) {
        goto error;
    }

    hl_ctx.fields[HL_FOLDER].flags = TREE_FLAG_DEFAULT | TREE_FLAG_ALLOW_EDIT;
...
nserror hotlist_init(const char *load_path, const char *save_path)
{
    ...
    err = treeview_create(&hl_ctx.tree, &hl_tree_cb_t, HL_N_FIELDS, hl_ctx.fields, NULL, TREEVIEW_SEARCHABLE);
    if (err != NSERROR_OK) {
        free(hl_ctx.save_path);
        hl_ctx.tree = NULL;
        return err;
...
nserror hotlist_add_url(nsurl *url)
{
    ...
    if (hl_ctx.default_folder == NULL) {
        ...
        struct hotlist_folder *f;
        err = hotlist_add_folder_internal(temp, NULL, TREE_REL_FIRST_CHILD, &f, true);
        if (err != NSERROR_OK)
            return err;
    ...
    err = hotlist_add_entry_internal(url, NULL, NULL, hl_ctx.default_folder->folder, TREE_REL_FIRST_CHILD, &entry);
    if (err != NSERROR_OK)
        return err;
...
bool hotlist_has_url(nsurl *url)
{
    ...
    err = treeview_walk(hl_ctx.tree, NULL, hotlist_has_url_walk_cb, NULL, &tw, TREE_NODE_ENTRY);
    if (err != NSERROR_OK)
        return false;
...
static nserror hotlist_remove_url_walk_cb(void *ctx, void *node_data, enum treeview_node_type type, bool *abort)
{
    ...
    if (type == TREE_NODE_ENTRY) {
        ...
        if (nsurl_compare(e->url, tw->url, NSURL_COMPLETE) == true) {
            ...
            treeview_delete_node(hl_ctx.tree, e->entry, TREE_OPTION_NONE);
        }
    }
...
void hotlist_remove_url(nsurl *url)
{
    ...
    err = treeview_walk(hl_ctx.tree, NULL, NULL, hotlist_remove_url_walk_cb, &tw, TREE_NODE_ENTRY);
    if (err != NSERROR_OK)
        return;
...
static nserror hotlist_update_url_walk_cb(void *ctx, void *node_data, enum treeview_node_type type, bool *abort)
{
    ...
    if (nsurl_compare(e->url, tw->url, NSURL_COMPLETE) == true) {
        ...
        err = treeview_update_node_entry(hl_ctx.tree, e->entry, e->data, e);
        if (err != NSERROR_OK)
            return err;
    }
...
void hotlist_update_url(nsurl *url)
{
    ...
    err = treeview_walk(hl_ctx.tree, NULL, hotlist_update_url_walk_cb, NULL, &tw, TREE_NODE_ENTRY);
    if (err != NSERROR_OK)
        return;
...
nserror hotlist_add_entry(nsurl *url, const char *title, bool at_y, int y)
{
    ...
    err = hotlist_add_entry_internal(url, title, NULL, relation, rel, &entry);
    if (err != NSERROR_OK) {
        nsurl_unref(url);
        return err;
    }
