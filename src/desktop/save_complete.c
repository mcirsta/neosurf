...
typedef enum { EVENT_ENTER, EVENT_LEAVE } save_complete_event_type;


static nserror save_complete_save_html(save_complete_ctx *ctx, struct hlcache_handle *c, bool index);
static nserror
save_complete_save_imported_sheets(save_complete_ctx *ctx, struct nscss_import *imports, uint32_t import_count);
...
static struct hlcache_handle *save_complete_ctx_find_content(save_complete_ctx *ctx, const nsurl *url)
{
    ...
    for (entry = ctx->list; entry != NULL; entry = entry->next) {
        if (nsurl_compare(url, hlcache_handle_get_url(entry->content), NSURL_COMPLETE)) {
...
static nserror save_complete_save_buffer(
    save_complete_ctx *ctx, const char *leafname, const uint8_t *data, size_t data_len, lwc_string *mime_type)
{
    nserror ret;
    FILE *fp;
    char *fname = NULL;
 ...
 */
static int
snregexec(regex_t *preg, const char *string, size_t stringlen, size_t nmatch, regmatch_t pmatch[], int eflags)
{
    char *strbuf;
    int matches;
 ...
 */
static uint8_t *save_complete_rewrite_stylesheet_urls(
    save_complete_ctx *ctx, const uint8_t *source, size_t size, const nsurl *base, size_t *osize)
{
    uint8_t *rewritten;
    ...
    for (offset = 0; (SLEN("@import") < size) && (offset <= (size - SLEN("@import"))); offset++) {
        if (source[offset] == '@' && ascii_to_lower(source[offset + 1]) == 'i' &&
            ascii_to_lower(source[offset + 2]) == 'm' && ascii_to_lower(source[offset + 3]) == 'p' &&
            ascii_to_lower(source[offset + 4]) == 'o' && ascii_to_lower(source[offset + 5]) == 'r' &&
    ...
    *osize = 0;
    ...
    while (offset < size) {
        ...
        m = snregexec(&save_complete_import_re, (const char *)source + offset, size - offset, 11, match, 0);
        if (m)
            break;
        ...
        import_url_copy = strndup((const char *)import_url, import_url_len);
...
static nserror save_complete_save_stylesheet(save_complete_ctx *ctx, hlcache_handle *css)
{
    ...
    if (save_complete_ctx_find_content(ctx, hlcache_handle_get_url(css)) != NULL) {
        return NSERROR_OK;
    }
    ...
    result = save_complete_save_imported_sheets(ctx, imports, import_count);
    if (result != NSERROR_OK) {
        return result;
    }

    css_data = content_get_source_data(css, &css_size);
    source = save_complete_rewrite_stylesheet_urls(ctx, css_data, css_size, hlcache_handle_get_url(css), &source_len);
    if (source == NULL) {
        return NSERROR_NOMEM;
    }
    ...
    result = save_complete_save_buffer(ctx, filename, source, source_len, type);
...
static nserror save_complete_save_html_object(save_complete_ctx *ctx, hlcache_handle *obj)
{
    ...
    if (save_complete_ctx_find_content(ctx, hlcache_handle_get_url(obj)) != NULL) {
        return NSERROR_OK;
    }
...
static bool save_complete_libdom_treewalk(
    dom_node *root, bool (*callback)(dom_node *node, save_complete_event_type event_type, void *ctx), void *ctx)
{
    dom_node *node;
...
static bool save_complete_rewrite_url_value(save_complete_ctx *ctx, const char *value, size_t value_len)
{
    ...
    if (url != NULL) {
        ...
        if (content != NULL) {
            ...
            /* no match found */
            error = utf8_to_html(nsurl_access(url), "UTF-8", nsurl_length(url), &escaped);
            nsurl_unref(url);
...
static bool save_complete_handle_attr_value(
    save_complete_ctx *ctx, dom_string *node_name, dom_string *attr_name, dom_string *attr_value)
{
     ...
     */
    /* 1 */
    if (name_len == SLEN("data") && strncasecmp(name_data, "data", name_len) == 0) {
        if (node_len == SLEN("object") && strncasecmp(node_data, "object", node_len) == 0) {
            return save_complete_rewrite_url_value(ctx, value_data, value_len);
        } else {
            return save_complete_write_value(ctx, value_data, value_len);
        }
    }
    /* 2 */
    else if (name_len == SLEN("href") && strncasecmp(name_data, "href", name_len) == 0) {
        if ((node_len == SLEN("a") && strncasecmp(node_data, "a", node_len) == 0) ||
            (node_len == SLEN("area") && strncasecmp(node_data, "area", node_len) == 0) ||
            (node_len == SLEN("link") && strncasecmp(node_data, "link", node_len) == 0)) {
            return save_complete_rewrite_url_value(ctx, value_data, value_len);
        } else {
            return save_complete_write_value(ctx, value_data, value_len);
        }
    ...
    /* 3 */
    else if (name_len == SLEN("src") && strncasecmp(name_data, "src", name_len) == 0) {
        if ((node_len == SLEN("frame") && strncasecmp(node_data, "frame", node_len) == 0) ||
            (node_len == SLEN("iframe") && strncasecmp(node_data, "iframe", node_len) == 0) ||
            (node_len == SLEN("input") && strncasecmp(node_data, "input", node_len) == 0) ||
            (node_len == SLEN("img") && strncasecmp(node_data, "img", node_len) == 0) ||
            (node_len == SLEN("script") && strncasecmp(node_data, "script", node_len) == 0)) {
            return save_complete_rewrite_url_value(ctx, value_data, value_len);
        } else {
            return save_complete_write_value(ctx, value_data, value_len);
        }
    ...
    /* 4 */
    else if (name_len == SLEN("background") && strncasecmp(name_data, "background", name_len) == 0) {
        return save_complete_rewrite_url_value(ctx, value_data, value_len);
    } else {
        return save_complete_write_value(ctx, value_data, value_len);
    }
...
static bool save_complete_handle_attr(save_complete_ctx *ctx, dom_string *node_name, dom_attr *attr)
{
    ...
    if (value != NULL) {
        fputc('=', ctx->fp);
        if (save_complete_handle_attr_value(ctx, node_name, name, value) == false) {
...
static bool save_complete_handle_element(save_complete_ctx *ctx, dom_node *node, save_complete_event_type event_type)
{
    ...
    if ((name_len == SLEN("base")) && (strncasecmp(name_data, "base", name_len) == 0)) {
        /* Elide BASE elements from the output */
        process = false;
    } else if ((name_len == SLEN("meta")) && (strncasecmp(name_data, "meta", name_len) == 0)) {
        ...
        if (event_type == EVENT_LEAVE) {
            ...
            /* Elide meta charsets */
            dom_string *value;
            error = dom_element_get_attribute(node, corestring_dom_http_equiv, &value);
            if (error != DOM_NO_ERR) {
                dom_string_unref(name);
                return false;
            }
            ...
            if (value != NULL) {
                if (dom_string_length(value) == SLEN("Content-Type") &&
                    strncasecmp(dom_string_data(value), "Content-Type", SLEN("Content-Type")) == 0)
                    process = false;
                ...
                error = dom_element_has_attribute(node, corestring_dom_charset, &yes);
                if (error != DOM_NO_ERR) {
                    dom_string_unref(name);
                    return false;
                }
        ...
        ((name_len == SLEN("link") && strncasecmp(name_data, "link", name_len) == 0))) {
    ...
    if (event_type == EVENT_ENTER && name_len == SLEN("style") && strncasecmp(name_data, "style", name_len) == 0) {
        dom_string *content;
        ...
        if (content != NULL) {
            ...
            rewritten = save_complete_rewrite_stylesheet_urls(
                ctx, (const uint8_t *)dom_string_data(content), dom_string_byte_length(content), ctx->base, &len);
            if (rewritten == NULL) {
    ...
    } else if (event_type == EVENT_ENTER && name_len == SLEN("head") && strncasecmp(name_data, "head", name_len) == 0) {
    ...
    if (type == DOM_ELEMENT_NODE) {
        ...
        if (ctx->iter_state != STATE_IN_STYLE) {
            ...
            if (text != NULL) {
                ...
                ret = utf8_to_html(text_data, "UTF-8", text_len, &escaped);
                if (ret != NSERROR_OK)
                    return false;

                fwrite(escaped, sizeof(*escaped), strlen(escaped), ctx->fp);
        ...
        if (name != NULL) {
            ...
            if (name_len > 0)
                fprintf(ctx->fp, " PUBLIC \"%.*s\"", (int)name_len, name_data);
        ...
        if (name != NULL) {
            ...
            if (name_len > 0)
                fprintf(ctx->fp, " \"%.*s\"", (int)name_len, name_data);
...
static nserror save_complete_save_html_document(save_complete_ctx *ctx, hlcache_handle *c, bool index)
{
    ...
    if (save_complete_libdom_treewalk((dom_node *)doc, save_complete_node_handler, ctx) == false) {
        free(fname);
        fclose(fp);
        return NSERROR_NOMEM;
    }
...
static nserror save_complete_inventory(save_complete_ctx *ctx)
{
    ...
    for (entry = ctx->list; entry != NULL; entry = entry->next) {
        fprintf(fp, "%p %s\n", entry->content, nsurl_access(hlcache_handle_get_url(entry->content)));
    }
