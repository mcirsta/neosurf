 ...
 */
static inline void browser_window_get_scrollbar_pos(struct browser_window *bw, bool horizontal, int *x, int *y)
{
    if (horizontal) {
        *x = 0;
...
static void browser_window_set_selection(struct browser_window *bw, bool selection, bool read_only)
{
    ...
    if (bw != top->selection.bw && top->selection.bw != NULL && top->selection.bw->current_content != NULL) {
        /* clear old selection */
 ...
 */
static nserror
browser_window__get_contextual_content(struct browser_window *bw, int x, int y, struct browser_window_features *data)
{
    ...
    if (bw->children) {
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            if ((x < bwc->x) || (bwc->x + bwc->width < x) || (y < bwc->y) || (bwc->y + bwc->height < y)) {
                continue;
            }

            /* Pass request into this child */
            return browser_window__get_contextual_content(bwc, (x - bwc->x), (y - bwc->y), data);
        }
        ...
        ret = content_get_contextual_content(bw->current_content, x, y, data);
 ...
 */
static nserror browser_window_download(struct browser_window *bw, nsurl *url, nsurl *nsref, uint32_t fetch_flags,
    bool fetch_is_post, llcache_post_data *post)
{
    llcache_handle *l;
    ...
    error = llcache_handle_retrieve(url, fetch_flags, nsref, fetch_is_post ? post : NULL, NULL, NULL, &l);
 ...
 */
static nserror
browser_window_update_favicon(hlcache_handle *c, struct browser_window *bw, struct content_rfc5988_link *link)
{
    nsurl *nsref = NULL;
    nsurl *nsurl;
    nserror res;
    ...
    if (link == NULL) {
        ...
        if ((lwc_string_caseless_isequal(scheme, corestring_lwc_http, &match) == lwc_error_ok && match) ||
            (lwc_string_caseless_isequal(scheme, corestring_lwc_https, &match) == lwc_error_ok && match)) {
...
static void browser_window_refresh(void *p)
{
    ...
    browser_window_navigate(bw, refresh, url, flags, NULL, NULL, parent);
}
...
static bool frag_scroll(struct browser_window *bw)
{
    ...
    if (!html_get_id_offset(bw->current_content, bw->frag_id, &rect.x0, &rect.y0)) {
        return false;
    }
    ...
    if (browser_window_set_scroll(bw, &rect) == NSERROR_OK) {
        if (bw->current_content != NULL && bw->history != NULL && bw->history->current != NULL) {
...
{
    static const struct rect zrect = {.x0 = 0, .y0 = 0, .x1 = 0, .y1 = 0};
    ...
    case BROWSER_WINDOW_NORMAL:
        ...
        guit->window->set_title(bw->window, content_get_title(bw->current_content));
...
static nserror browser_window_content_ready(struct browser_window *bw)
{
    ...
    if (bw->history_add && bw->history && !bw->internal_nav) {
        ...
        if (urldb_add_url(url)) {
            ...
            urldb_set_url_content_type(url, content_get_type(bw->current_content));
 ...
 */
static nserror browser_window__handle_userpass_response(
    nsurl *url, const char *realm, const char *username, const char *password, void *pw)
{
    struct browser_window *bw = (struct browser_window *)pw;
    char *userpass;
    nserror err;
...
static nserror browser_window__handle_login(struct browser_window *bw, const char *realm, nsurl *url)
{
    ...
    err = browser_window__unpack_userpass(urldb_get_auth_details(url, realm), &username, &password);
    if (err != NSERROR_OK) {
        goto out;
    }
    ...
    err = fetch_multipart_data_new_kv(&params.post_multipart, "siteurl", nsurl_access(url));
    if (err != NSERROR_OK) {
        goto out;
    }

    err = fetch_multipart_data_new_kv(&params.post_multipart, "realm", realm);
    if (err != NSERROR_OK) {
        goto out;
    }

    err = fetch_multipart_data_new_kv(&params.post_multipart, "username", username);
    if (err != NSERROR_OK) {
        goto out;
    }

    err = fetch_multipart_data_new_kv(&params.post_multipart, "password", password);
    if (err != NSERROR_OK) {
        goto out;
    }
    ...
    err = guit->misc->login(url, realm, username, password, browser_window__handle_userpass_response, bw);
...
static nserror browser_window__handle_bad_certs(struct browser_window *bw, nsurl *url)
{
    ...
    if (bw->loading_cert_chain != NULL) {
        ...
        err = fetch_multipart_data_new_kv(&params.post_multipart, "chainurl", nsurl_access(chainurl));
        if (err != NSERROR_OK) {
            goto out;
        }
    }

    err = fetch_multipart_data_new_kv(&params.post_multipart, "reason", reason);
    if (err != NSERROR_OK) {
        goto out;
    }
 ...
 */
static inline nserror browser_window_refresh_url_bar_internal(struct browser_window *bw, nsurl *url)
{
    assert(bw);
    assert(url);
...
static nserror browser_window_callback(hlcache_handle *c, const hlcache_event *event, void *pw)
{
    ...
    case CONTENT_MSG_LOG:
        browser_window_console_log(
            bw, event->data.log.src, event->data.log.msg, event->data.log.msglen, event->data.log.flags);
        break;
    ...
    case CONTENT_MSG_STATUS:
        if (event->data.explicit_status_text == NULL) {
            ...
            if (bw->loading_content != NULL) {
                ...
                status = content_get_status_message(bw->loading_content);
            }
            ...
            browser_window_set_status(bw, event->data.explicit_status_text);
        }
        break;
    ...
    case CONTENT_MSG_LINK: /* content has an rfc5988 link element */
    {
        ...
        if ((lwc_string_caseless_isequal(event->data.rfc5988_link->rel, corestring_lwc_icon, &match) == lwc_error_ok &&
                match) ||
            (lwc_string_caseless_isequal(event->data.rfc5988_link->rel, corestring_lwc_shortcut_icon, &match) ==
                    lwc_error_ok &&
                match)) {
    ...
    case CONTENT_MSG_DRAGSAVE: {
        ...
        case CONTENT_SAVE_ORIG:
            guit->window->drag_save_object(root->window, save, GUI_SAVE_OBJECT_ORIG);
            break;

        case CONTENT_SAVE_NATIVE:
            guit->window->drag_save_object(root->window, save, GUI_SAVE_OBJECT_NATIVE);
            break;

        case CONTENT_SAVE_COMPLETE:
            guit->window->drag_save_object(root->window, save, GUI_SAVE_COMPLETE);
            break;

        case CONTENT_SAVE_SOURCE:
            guit->window->drag_save_object(root->window, save, GUI_SAVE_SOURCE);
            break;
        }
    } break;
    ...
    case CONTENT_MSG_SAVELINK: {
        ...
        struct browser_window *root = browser_window_get_root(bw);
        guit->window->save_link(root->window, event->data.savelink.url, event->data.savelink.title);
    } break;
    ...
    case CONTENT_MSG_CARET:
        ...
        case CONTENT_CARET_SET_POS:
            browser_window_place_caret(bw, event->data.caret.pos.x, event->data.caret.pos.y,
    ...
    case CONTENT_MSG_SELECTMENU:
        if (event->data.select_menu.gadget->type == GADGET_SELECT) {
            struct browser_window *root = browser_window_get_root(bw);
            guit->window->create_form_select_menu(root->window, event->data.select_menu.gadget);
        }
    ...
    case CONTENT_MSG_GADGETCLICK:
        if (event->data.gadget_click.gadget->type == GADGET_FILE) {
            struct browser_window *root = browser_window_get_root(bw);
            guit->window->file_gadget_open(root->window, c, event->data.gadget_click.gadget);
        }
    ...
    case CONTENT_MSG_TEXTSEARCH:
        ...
        case CONTENT_TEXTSEARCH_FIND:
            guit->search->hourglass(event->data.textsearch.state, event->data.textsearch.ctx);
            break;

        case CONTENT_TEXTSEARCH_MATCH:
            guit->search->status(event->data.textsearch.state, event->data.textsearch.ctx);
            break;

        case CONTENT_TEXTSEARCH_BACK:
            guit->search->back_state(event->data.textsearch.state, event->data.textsearch.ctx);
            break;

        case CONTENT_TEXTSEARCH_FORWARD:
            guit->search->forward_state(event->data.textsearch.state, event->data.textsearch.ctx);
            break;

        case CONTENT_TEXTSEARCH_RECENT:
            guit->search->add_recent(event->data.textsearch.string, event->data.textsearch.ctx);
 ...
 */
static void browser_window_find_target_internal(struct browser_window *bw, const char *target, int depth,
    struct browser_window *page, int *rdepth, struct browser_window **bw_target)
{
    int i;
    ...
    if (bw->children != NULL) {
        for (i = 0; i < (bw->cols * bw->rows); i++) {
            ...
            if (bw->children[i].children)
                browser_window_find_target_internal(&bw->children[i], target, depth, page, rdepth, bw_target);
        }
    }

    if (bw->iframes != NULL) {
        for (i = 0; i < bw->iframe_count; i++) {
            browser_window_find_target_internal(&bw->iframes[i], target, depth, page, rdepth, bw_target);
        }
    }
}
...
static void browser_window_mouse_click_internal(struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    ...
    if (bw->children) {
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            if ((x < child->x) || (y < child->y) || (child->x + child->width < x) || (child->y + child->height < y)) {
             ...
             * on to child. */
            browser_window_mouse_click_internal(child, mouse, x - child->x + scrollbar_get_offset(child->scroll_x),
    ...
    if (bw->scroll_x != NULL) {
        ...
        if (scr_x > 0 && scr_x < get_horz_scrollbar_len(bw) && scr_y > 0 && scr_y < SCROLLBAR_WIDTH) {
            status = scrollbar_mouse_status_to_message(scrollbar_mouse_action(bw->scroll_x, mouse, scr_x, scr_y));
    ...
    if (bw->scroll_y != NULL) {
        ...
        if (scr_y > 0 && scr_y < get_vert_scrollbar_len(bw) && scr_x > 0 && scr_x < SCROLLBAR_WIDTH) {
            status = scrollbar_mouse_status_to_message(scrollbar_mouse_action(bw->scroll_y, mouse, scr_x, scr_y));
            pointer = BROWSER_POINTER_DEFAULT;
    ...
    default:
        if (mouse & BROWSER_MOUSE_MOD_2) {
            if (mouse & BROWSER_MOUSE_DRAG_2) {
                guit->window->drag_save_object(bw->window, c, GUI_SAVE_OBJECT_NATIVE);
            } else if (mouse & BROWSER_MOUSE_DRAG_1) {
                guit->window->drag_save_object(bw->window, c, GUI_SAVE_OBJECT_ORIG);
            }
...
static void browser_window_mouse_track_internal(struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    ...
    if (bw->window != NULL && bw->drag.window && bw != bw->drag.window) {
        ...
        if (drag_bw->browser_window_type == BROWSER_WINDOW_FRAME) {
            browser_window_mouse_track_internal(drag_bw, mouse, x - off_x, y - off_y);

        } else if (drag_bw->browser_window_type == BROWSER_WINDOW_IFRAME) {
            browser_window_mouse_track_internal(drag_bw, mouse, x - off_x / bw->scale, y - off_y / bw->scale);
        }
        return;
    }
    ...
    if (bw->children) {
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            browser_window_mouse_track_internal(child, mouse, x - child->x + scrollbar_get_offset(child->scroll_x),
                y - child->y + scrollbar_get_offset(child->scroll_y));
    ...
    if (bw->scroll_x != NULL && bw->drag.type != DRAGGING_SCR_Y) {
        ...
        if ((bw->drag.type == DRAGGING_SCR_X) ||
            (scr_x > 0 && scr_x < get_horz_scrollbar_len(bw) && scr_y > 0 && scr_y < SCROLLBAR_WIDTH &&
                bw->drag.type == DRAGGING_NONE)) {
    ...
    if (bw->scroll_y != NULL) {
        ...
        if ((bw->drag.type == DRAGGING_SCR_Y) ||
            (scr_y > 0 && scr_y < get_vert_scrollbar_len(bw) && scr_x > 0 && scr_x < SCROLLBAR_WIDTH &&
                bw->drag.type == DRAGGING_NONE)) {
            /* Start a scrollbar drag, or continue existing drag */
            status = scrollbar_mouse_status_to_message(scrollbar_mouse_action(bw->scroll_y, mouse, scr_x, scr_y));
            pointer = BROWSER_POINTER_DEFAULT;
...
static bool browser_window_scroll_at_point_internal(struct browser_window *bw, int x, int y, int scrx, int scry)
{
    ...
    if (bw->children) {
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            /* Pass request into this child */
            return browser_window_scroll_at_point_internal(bwc, (x - bwc->x), (y - bwc->y), scrx, scry);
        }
    }

    /* Try to scroll any current content */
    if (bw->current_content != NULL && content_scroll_at_point(bw->current_content, x, y, scrx, scry) == true) {
        /* Scroll handled by current content */
        return true;
    }

    /* Try to scroll this window. */
    return (int)scrollbar_scroll(bw->scroll_y, scry) | (int)scrollbar_scroll(bw->scroll_x, scrx);
}
...
static bool browser_window_drop_file_at_point_internal(struct browser_window *bw, int x, int y, char *file)
{
    ...
    if (bw->children) {
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            /* Pass request into this child */
            return browser_window_drop_file_at_point_internal(bwc, (x - bwc->x), (y - bwc->y), file);
        }
    }

    /* Pass file drop on to any content */
    if (bw->current_content != NULL) {
        return content_drop_file_at_point(bw->current_content, x, y, file);
    }
    ...
    struct browser_window *bw, int x, int y, const struct rect *clip, const struct redraw_context *ctx)
{
    ...
    if (bw->children) {
        ...
        if (bw->window != NULL) {
            ...
            plot_ok &= (new_ctx.plot->rectangle(ctx, plot_style_fill_white, clip) == NSERROR_OK);
        }
        ...
        for (cur_child = 0; cur_child < children; cur_child++) {
            ...
            content_clip.x1 = content_clip.x0 + child->width * child->scale;
            content_clip.y1 = content_clip.y0 + child->height * child->scale;
            ...
            if (content_clip.x0 >= content_clip.x1 || content_clip.y0 >= content_clip.y1)
                continue;

            /* Redraw frame */
            plot_ok &= browser_window_redraw(child, x + child->x, y + child->y, &content_clip, &new_ctx);
        }
...

    {
        ...
        NSLOG(layout, INFO,
            "bw redraw baseline: bw %p pos_x %d pos_y %d scale %f root_w %d root_h %d content_clip x0 %d x1 %d", bw,
            pos_x, pos_y, bw->scale, root_w, root_h, clip->x0, clip->x1);
    }
    ...
    if (!bw->window) {
        int x0 = x * bw->scale;
        int y0 = y * bw->scale;
        int x1 = (x + bw->width - ((bw->scroll_y != NULL) ? SCROLLBAR_WIDTH : 0)) * bw->scale;
        int y1 = (y + bw->height - ((bw->scroll_x != NULL) ? SCROLLBAR_WIDTH : 0)) * bw->scale;

        if (content_clip.x0 < x0)
            content_clip.x0 = x0;
        if (content_clip.y0 < y0)
            content_clip.y0 = y0;
        if (x1 < content_clip.x1)
            content_clip.x1 = x1;
        if (y1 < content_clip.y1)
            content_clip.y1 = y1;
    }

    /* Render the content */
    plot_ok &= content_redraw(bw->current_content, &data, &content_clip, &new_ctx);
    ...
    if (!bw->window) {
        ...
        int off_x, off_y;
        if (bw->scroll_x != NULL) {
            browser_window_get_scrollbar_pos(bw, true, &off_x, &off_y);
            res = scrollbar_redraw(bw->scroll_x, x + off_x, y + off_y, clip, bw->scale, &new_ctx);
            if (res != NSERROR_OK) {
                plot_ok = false;
            }
        }
        if (bw->scroll_y != NULL) {
            browser_window_get_scrollbar_pos(bw, false, &off_x, &off_y);
            res = scrollbar_redraw(bw->scroll_y, x + off_x, y + off_y, clip, bw->scale, &new_ctx);
            if (res != NSERROR_OK) {
                plot_ok = false;
            }
        }
...
char *browser_window_get_selection(struct browser_window *bw)
{
    ...
    if (bw->selection.bw == NULL || bw->selection.bw->current_content == NULL)
        return NULL;
...
nserror browser_window_get_features(struct browser_window *bw, int x, int y, struct browser_window_features *data)
{
    ...
    return browser_window__get_contextual_content(bw, x / bw->scale, y / bw->scale, data);
}
...
{
    return browser_window_scroll_at_point_internal(bw, x / bw->scale, y / bw->scale, scrx, scry);
}
...
{
    return browser_window_drop_file_at_point_internal(bw, x / bw->scale, y / bw->scale, file);
}
    ...
    struct browser_window *existing, struct browser_window **bw)
{
    ...
    ret->window = guit->window->create(ret, (existing != NULL) ? existing->window : NULL, gw_flags);
    ...
    if (url != NULL) {
        ...
        browser_window_navigate(ret, url, referrer, nav_flags, NULL, NULL, NULL);
    }
...
nserror browser_window_initialise_common(
    enum browser_window_create_flags flags, struct browser_window *bw, const struct browser_window *existing)
{
    nserror err;
    assert(bw);
...
nserror browser_window_refresh_url_bar(struct browser_window *bw)
{
    ...
    if (bw->current_content == NULL) {
        ...
        ret = browser_window_refresh_url_bar_internal(bw, corestring_nsurl_about_blank);
        ...
        ret = nsurl_refragment(url, bw->frag_id, &display_url);
        if (ret == NSERROR_OK) {
            ret = browser_window_refresh_url_bar_internal(bw, display_url);
...
/* exported interface documented in neosurf/browser_window.h */
nserror browser_window_navigate(struct browser_window *bw, nsurl *url, nsurl *referrer,
    ...
    hlcache_handle *parent)
{
    ...
    if (is_internal && !(flags & BW_NAVIGATE_INTERNAL)) {
     ...
     */
    if (bw->current_content != NULL && bw->history != NULL && bw->history->current != NULL && !is_internal &&
        !(flags & BW_NAVIGATE_NO_TERMINAL_HISTORY_UPDATE)) {
        browser_window_history_update(bw, bw->current_content);
    }
    ...
    if ((flags & BW_NAVIGATE_DOWNLOAD) != 0) {
        error = browser_window_download(bw, url, referrer, fetch_flags, fetch_is_post, &post);
        nsurl_unref(url);
        if (referrer != NULL) {
    ...
    if (nsurl_has_component(url, NSURL_FRAGMENT)) {
        ...
        if ((bw->current_content != NULL) && (hlcache_handle_get_url(bw->current_content) != NULL)) {
            same_url = nsurl_compare(url, hlcache_handle_get_url(bw->current_content), NSURL_COMPLETE);
        }

        /* if we're simply moving to another ID on the same page,
         * don't bother to fetch, just update the window.
         */
        if ((same_url) && (fetch_is_post == false) && (nsurl_has_component(url, NSURL_QUERY) == false)) {
            nsurl_unref(url);
    ...
    if (is_internal) {
        ...
        /* At this point, we're navigating, so store the fetch
         * parameters */
        browser_window__free_fetch_parameters(&bw->loading_parameters);
...
static nserror navigate_internal_query_auth(struct browser_window *bw, struct browser_fetch_parameters *params)
{
    ...
    if (username == NULL || password == NULL || realm == NULL || siteurl == NULL) {
 ...
 */
nserror browser_window__navigate_internal(struct browser_window *bw, struct browser_fetch_parameters *params)
{
    lwc_string *scheme, *path;
...
bool browser_window_up_available(struct browser_window *bw)
{
    ...
    if (bw != NULL && bw->current_content != NULL) {
        nsurl *parent;
        nserror err;
        err = nsurl_parent(hlcache_handle_get_url(bw->current_content), &parent);
        if (err == NSERROR_OK) {
            result = nsurl_compare(hlcache_handle_get_url(bw->current_content), parent, NSURL_COMPLETE) == false;
            nsurl_unref(parent);
        }
    }
...
nserror browser_window_navigate_up(struct browser_window *bw, bool new_window)
{
    ...
    if (new_window) {
        err = browser_window_create(BW_CREATE_CLONE, parent, NULL, bw, NULL);
    } else {
        err = browser_window_navigate(bw, parent, NULL, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
    }
...
nserror browser_window_get_url(struct browser_window *bw, bool fragment, nsurl **url_out)
{
    ...
    if (!fragment || bw->frag_id == NULL || bw->loading_content != NULL) {
        ...
        err = nsurl_refragment(browser_window_access_url(bw), bw->frag_id, &url);
        if (err != NSERROR_OK) {
            return err;
        }
    }
...
void browser_window_stop(struct browser_window *bw)
{
    ...
    if (bw->current_content != NULL && content_get_status(bw->current_content) != CONTENT_STATUS_DONE) {
        nserror error;
        assert(content_get_status(bw->current_content) == CONTENT_STATUS_READY);
...
nserror browser_window_reload(struct browser_window *bw, bool all)
{
    ...
    return browser_window_navigate(bw, reload_url, NULL, BW_NAVIGATE_NONE, NULL, NULL, NULL);
}
...
void browser_window_set_pointer(struct browser_window *bw, browser_pointer_shape shape)
{
    ...
    loading = ((bw->loading_content != NULL) ||
        ((bw->current_content != NULL) && (content_get_status(bw->current_content) == CONTENT_STATUS_READY)));
...
nserror browser_window_set_scale(struct browser_window *bw, float scale, bool absolute)
{
    ...
    if (!absolute) {
        ...
        if ((scale + bw->scale) > (1.01 - scale) && (scale + bw->scale) < (0.99 + scale)) {
            scale = 1.0;
        } else {
            scale += bw->scale;
        }
...
browser_window_find_target(struct browser_window *bw, const char *target, browser_mouse_state mouse)
{
     ...
     */
    if ((!(mouse & BROWSER_MOUSE_CLICK_2)) && (!((mouse & BROWSER_MOUSE_CLICK_2) && (mouse & BROWSER_MOUSE_MOD_2))) &&
    ...
    if (((nsoption_bool(button_2_tab)) && (mouse & BROWSER_MOUSE_CLICK_2)) ||
        ((!nsoption_bool(button_2_tab)) && ((mouse & BROWSER_MOUSE_CLICK_1) && (mouse & BROWSER_MOUSE_MOD_2))) ||
        ((nsoption_bool(button_2_tab)) && (!strcasecmp(target, "_blank")))) {
        /* open in new tab if:
        ...
        return bw_target;
    } else if (((!nsoption_bool(button_2_tab)) && (mouse & BROWSER_MOUSE_CLICK_2)) ||
        ((nsoption_bool(button_2_tab)) && ((mouse & BROWSER_MOUSE_CLICK_1) && (mouse & BROWSER_MOUSE_MOD_2))) ||
        ((!nsoption_bool(button_2_tab)) && (!strcasecmp(target, "_blank")))) {
...
{
    browser_window_mouse_track_internal(bw, mouse, x / bw->scale, y / bw->scale);
}

/* exported interface documented in neosurf/browser_window.h */
void browser_window_mouse_click(struct browser_window *bw, browser_mouse_state mouse, int x, int y)
{
    browser_window_mouse_click_internal(bw, mouse, x / bw->scale, y / bw->scale);
}
...
void browser_window_page_drag_start(struct browser_window *bw, int x, int y)
{
    ...
    if (bw->window != NULL) {
        /* Front end window */
        guit->window->get_scroll(bw->window, &bw->drag.start_scroll_x, &bw->drag.start_scroll_y);
...
bool browser_window_stop_available(struct browser_window *bw)
{
    return (bw &&
        (bw->loading_content ||
            (bw->current_content && (content_get_status(bw->current_content) != CONTENT_STATUS_DONE))));
}
...
browser_window_page_info_state browser_window_get_page_info_state(const struct browser_window *bw)
{
    ...
    scheme = nsurl_get_component(hlcache_handle_get_url(bw->current_content), NSURL_SCHEME);
...
nserror browser_window_show_certificates(struct browser_window *bw)
{
    ...
    if (res == NSERROR_OK) {
        res = browser_window_create(BW_CREATE_HISTORY | BW_CREATE_FOREGROUND | BW_CREATE_TAB, url, NULL, bw, NULL);
