 ...
 */
static nserror parse_providers(
    char *providersd, size_t providers_size, struct search_provider **providers_out, size_t *providers_count)
{
...
nserror search_web_get_provider_bitmap(struct bitmap **bitmap_out)
{
    ...
    if ((ico_bitmap == NULL) && (search_web_ctx.default_ico_handle != NULL)) {
...
static nserror default_ico_callback(hlcache_handle *ico, const hlcache_event *event, void *pw)
{
    ...
    case CONTENT_MSG_DONE:
        ...
        if (ctx->providers[search_web_ctx.current].ico_handle == NULL) {
            guit->search_web->provider_update(ctx->providers[search_web_ctx.current].name, content_get_bitmap(ico));
        }
        break;
...
nserror search_web_init(const char *provider_fname)
{
    ...
    ret = parse_providers(providers, providers_size, &search_web_ctx.providers, &search_web_ctx.providers_count);
    if (ret != NSERROR_OK) {
        free(providers);
        return ret;
    }
