 ...
 */
static nserror redraw_entry(struct history *history, struct history_entry *entry, struct history_entry *cursor,
    struct rect *clip, int x, int y, const struct redraw_context *ctx)
{
    size_t char_offset;
    int actual_x;
    ...
    if (entry->page.bitmap != NULL) {
        res = ctx->plot->bitmap(ctx, entry->page.bitmap, entry->x + x, entry->y + y, LOCAL_HISTORY_WIDTH,
            LOCAL_HISTORY_HEIGHT, 0xffffff, 0);
        if (res != NSERROR_OK) {
            return res;
        }
    }
    ...
    res = guit->layout->position(
        plot_style_font, entry->page.title, strlen(entry->page.title), LOCAL_HISTORY_WIDTH, &char_offset, &actual_x);
    if (res != NSERROR_OK) {
        return res;
    }

    res = ctx->plot->text(
        ctx, pfstyle, entry->x + x, entry->y + LOCAL_HISTORY_HEIGHT + 12 + y, entry->page.title, char_offset);
    if (res != NSERROR_OK) {
        return res;
    }
...
static struct history_entry *find_entry_position(struct history_entry *entry, int x, int y)
{
    ...
    if ((entry->x <= x) && (x <= entry->x + LOCAL_HISTORY_WIDTH) && (entry->y <= y) &&
...
nserror local_history_scroll_to_cursor(struct local_history_session *session)
{
    ...
    cursor.x1 = cursor.x0 + LOCAL_HISTORY_WIDTH + LOCAL_HISTORY_RIGHT_MARGIN / 2;
    cursor.y1 = cursor.y0 + LOCAL_HISTORY_HEIGHT + LOCAL_HISTORY_BOTTOM_MARGIN / 2;
...
/* exported interface documented in desktop/local_history.h */
nserror local_history_redraw(
    struct local_history_session *session, int x, int y, struct rect *clip, const struct redraw_context *ctx)
{
    struct rect r = {
        .x0 = clip->x0 + x,
        .y0 = clip->y0 + y,
    ...
    return redraw_entry(session->bw->history, session->bw->history->start, session->cursor, clip, x, y, ctx);
}
...
bool local_history_keypress(struct local_history_session *session, uint32_t key)
{
    ...
    case NS_KEY_CR:
        ...
        if (session->cursor != session->bw->history->current) {
            browser_window_history_go(session->bw, session->cursor, false);
    ...
    case NS_KEY_DOWN:
        ...
        if (session->cursor->next != NULL) {
            ...
            _local_history_find_branch_point(session->cursor, &branch_point);
    ...
    case NS_KEY_UP:
        ...
        if (session->cursor->back != NULL) {
            ...
            while (ent != session->cursor && ent->next != NULL && ent->next != session->cursor) {
                ent = ent->next;
            }
...
nserror local_history_set(struct local_history_session *session, struct browser_window *bw)
{
    ...
    if (bw != NULL) {
        ...
        guit->corewindow->set_extent(
            session->core_window_handle, session->bw->history->width, session->bw->history->height);
        local_history_scroll_to_cursor(session);
    }
