...
struct plotter_table {
     ...
     * \return NSERROR_OK on success else error code.
     */
    nserror (*bitmap)(const struct redraw_context *ctx, struct bitmap *bitmap, int x, int y, int width, int height,
     ...
     * \return NSERROR_OK on success else error code.
     */
    nserror (*text)(const struct redraw_context *ctx, const plot_font_style_t *fstyle, int x, int y, const char *text,
        size_t length);
