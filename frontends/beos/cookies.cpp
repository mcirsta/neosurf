...
{
public:
    CookieDateColumn(const char *title, float width) : BDateColumn(title, width, width / 2, width * 2)
    {
    }
...
{
public:
    CookieRow(BColumnListView *list, struct cookie_data &cookie) : BRow(), fCookie(cookie)
    {
        list->AddRow(this);
...
CookieWindow::CookieWindow(BRect frame)
    : BWindow(frame, "Cookie manager", B_TITLED_WINDOW, B_NORMAL_WINDOW_FEEL,
          B_AUTO_UPDATE_SIZE_LIMITS | B_ASYNCHRONOUS_CONTROLS)
{
    ...
    fCookies = new BColumnListView("cookie list", B_WILL_DRAW, B_FANCY_BORDER, false);

    float em = fCookies->StringWidth("M");
    float flagsLength = fCookies->StringWidth("Mhttps hostOnly" B_UTF8_ELLIPSIS);

    fCookies->AddColumn(new BStringColumn("Name", 20 * em, 10 * em, 50 * em, 0), 0);
    fCookies->AddColumn(new BStringColumn("Path", 10 * em, 10 * em, 50 * em, 0), 1);
    fCookies->AddColumn(new CookieDateColumn("Expiration", fCookies->StringWidth("88/88/8888 88:88:88 AM")), 2);
    fCookies->AddColumn(new BStringColumn("Value", 20 * em, 10 * em, 50 * em, 0), 3);
    fCookies->AddColumn(new BStringColumn("Flags", flagsLength, flagsLength, flagsLength, 0), 4);
    ...
    root->AddItem(BGroupLayoutBuilder(B_VERTICAL, B_USE_DEFAULT_SPACING)
                      ...
                      .SetInsets(5, 5, 5, 5)
                      .AddGlue()
                      .Add(new BButton("delete", "Delete", new BMessage(COOKIE_DELETE))),
        3);
...
BStringItem *CookieWindow::_AddDomain(BString domain, bool fake)
{
    ...
    if (sibling) {
        ...
        fDomains->AddItem(
            domainItem, fDomains->FullListIndexOf(sibling) + fDomains->CountItemsUnder(sibling, false) + 1);
    } else {
