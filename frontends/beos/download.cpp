      ...
      success(false)
{
    ...
    BRect rect = Bounds();
    rect.InsetBy(3, 3);
    bar = new BStatusBar(rect, "progress", download_context_get_filename(ctx), buffer);
...
void NSDownloadWindow::MessageReceived(BMessage *message)
{
    ...
    case B_SAVE_REQUESTED: {
        ...
        BFile *storage = new BFile(&dir, name, B_WRITE_ONLY | B_CREATE_FILE | B_ERASE_FILE);
        dw->storageLock->Lock();
...
static struct gui_download_window *gui_download_window_create(download_context *ctx, struct gui_window *parent)
{
    ...
    BFilePanel *panel = new BFilePanel(B_SAVE_PANEL, new BMessenger(download->window), NULL, 0, false);
