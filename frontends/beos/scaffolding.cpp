...
{
public:
    NSIconTextControl(BRect frame, const char *name, const char *label, const char *initialText, BMessage *message,
...
NSIconTextControl::NSIconTextControl(BRect frame, const char *name, const char *label, const char *initialText,
    BMessage *message, uint32 resizeMode, uint32 flags)
    : BTextControl(frame, name, label, initialText, message, resizeMode, flags), fIconOffset(0, 0), fIconBitmap(NULL)
{
    BRect r(Bounds());
    fIconFrame = r;
    ...
    fIconFrame.OffsetBy(
        (int32)((r.IntegerHeight() - ICON_WIDTH + 3) / 2), (int32)((r.IntegerHeight() - ICON_WIDTH + 1) / 2));
    FixupTextRect();
}
...
void NSResizeKnob::MouseMoved(BPoint where, uint32 code, const BMessage *dragMessage)
{
    if (fOffset.x >= 0) {
...
NSThrobber::NSThrobber(BRect frame)
    : BView(frame, "NSThrobber", B_FOLLOW_TOP | B_FOLLOW_RIGHT, B_WILL_DRAW), fBitmap(NULL)
{
}
...
BArchivable *NSBaseView::Instantiate(BMessage *archive)
{
    if (!validate_instantiation(archive, "NSBaseView"))
        return NULL;
    const char *url;
    if (archive->FindString("url", &url) < B_OK || url == NULL || strlen(url) == 0) {
        url = "about:";
    }
...
void nsbeos_scaffolding_dispatch_event(nsbeos_scaffolding *scaffold, BMessage *message)
{
        ...
        for (i = 0; message->FindRef("refs", i, &ref) >= B_OK; i++) {
            ...
            if (error == NSERROR_OK) {
                if (/*message->WasDropped() &&*/ i == 0) {
                    ...
                    error = browser_window_create(BW_CREATE_CLONE, nsurl, NULL, bw, NULL);
                }
                nsurl_unref(nsurl);
            }
            if (error != NSERROR_OK) {
    ...
    case B_NETPOSITIVE_OPEN_URL: {
        ...
        if (error != NSERROR_OK) {
            ...
            browser_window_navigate(bw, nsurl, NULL,
                (browser_window_nav_flags)(BW_NAVIGATE_HISTORY | BW_NAVIGATE_UNVERIFIABLE), NULL, NULL, NULL);
            nsurl_unref(nsurl);
        }
        break;
    }
    ...
    case 'home': {
        ...
        if (error != NSERROR_OK) {
            ...
            browser_window_navigate(bw, url, NULL, (browser_window_nav_flags)(BW_NAVIGATE_HISTORY), NULL, NULL, NULL);
            nsurl_unref(url);
        }
        break;
    }
    ...
    case 'sear': {
        ...
        ret = search_web_omni(text.String(), SEARCH_WEB_OMNI_SEARCHONLY, &url);
        if (ret == NSERROR_OK) {
            ret = browser_window_create(
                (browser_window_create_flags)(BW_CREATE_HISTORY | BW_CREATE_TAB), url, NULL, bw, NULL);
            nsurl_unref(url);
        }
    ...
    case HELP_OPEN_ABOUT: {
        ...
        if (nserr == NSERROR_OK) {
            nserr = browser_window_navigate(
                bw, url, NULL, (browser_window_nav_flags)(BW_NAVIGATE_HISTORY), NULL, NULL, NULL);
            nsurl_unref(url);
        }
        if (nserr != NSERROR_OK) {
...
BBitmapButton::BBitmapButton(BRect rect, const char *name, const char *label, BMessage *message)
...
nsbeos_scaffolding *nsbeos_new_scaffolding(struct gui_window *toplevel)
{
    ...
    message->AddPointer("scaffolding", g);
    g->url_bar = new NSIconTextControl(rect, "url_bar", "", "", message, B_FOLLOW_LEFT_RIGHT | B_FOLLOW_TOP);
    ...
    message->AddPointer("scaffolding", g);
    g->search_bar = new BTextControl(
        rect, "search_bar", "", "Search" B_UTF8_ELLIPSIS, message, B_FOLLOW_RIGHT | B_FOLLOW_TOP);
