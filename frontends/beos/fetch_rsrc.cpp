...
static void *fetch_rsrc_setup(struct fetch *parent_fetch, nsurl *url, bool only_2xx, bool downgrade_tls,
    const char *post_urlenc, const struct fetch_multipart_data *post_multipart, const char **headers)
{
...
static bool fetch_rsrc_process(struct fetch_rsrc_context *c)
{
    ...
    if (c->mimetype == NULL) {
        msg.type = FETCH_ERROR;
        msg.data.error = "Unable to allocate memory for mimetype in rsrc: URL";
        fetch_rsrc_send_callback(&msg, c);
        return false;
    }
...
static void fetch_rsrc_poll(lwc_string *scheme)
{
    ...
    do {
        ...
        if (!c->aborted && fetch_rsrc_process(c) == true) {
             ...
             */
            snprintf(header, sizeof header, "Content-Type: %s", c->mimetype);
...
void fetch_rsrc_register(void)
{
    ...
    const struct fetcher_operation_table fetcher_ops_rsrc = {fetch_rsrc_initialise, fetch_rsrc_can_fetch,
        fetch_rsrc_setup, fetch_rsrc_start, fetch_rsrc_abort, fetch_rsrc_free, fetch_rsrc_poll, NULL,
        fetch_rsrc_finalise};
