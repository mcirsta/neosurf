...
static void
nsbeos_window_expose_event(BView *view, gui_window *g, BMessage *message);
static void
nsbeos_window_keypress_event(BView *view, gui_window *g, BMessage *event);
static void
nsbeos_window_resize_event(BView *view, gui_window *g, BMessage *event);
static void
nsbeos_window_moved_event(BView *view, gui_window *g, BMessage *event);
/* Other useful bits */
...
void nsbeos_dispatch_event(BMessage *message)
{
	...
	struct gui_window *y;
	for (y = window_list; y && scaffold && y->scaffold != scaffold;
	     y = y->next)
		continue;
	...
	case B_MOUSE_DOWN: {
		...
		if (buttons & B_SECONDARY_MOUSE_BUTTON) {
			...
			nsbeos_scaffolding_popup_menu(
				gui->scaffold, gui->bw, where, screenWhere);
			break;
		}
		...
		if (buttons &
		    B_TERTIARY_MOUSE_BUTTON) /* 3 == middle button on BeOS */
			gui->mouse.state = BROWSER_MOUSE_PRESS_2;
...
void nsbeos_redraw_caret(struct gui_window *g)
{
	...
	g->view->Invalidate(
		BRect(g->caretx, g->carety, g->caretx, g->carety + g->careth));
...
beos_window_invalidate_area(struct gui_window *g, const struct rect *rect)
{
	...
	if (rect != NULL) {
		// XXX +1 ??
		g->view->Invalidate(
			BRect(rect->x0, rect->y0, rect->x1 - 1, rect->y1 - 1));
	} else {
		g->view->Invalidate();
	}
...
static void gui_window_remove_caret(struct gui_window *g)
{
	...
	g->view->Invalidate(
		BRect(g->caretx, g->carety, g->caretx, g->carety + oh));
			      ...
			      int n_styles)
{
	...
	if (be_clipboard->Lock()) {
		...
		if (clip) {
			clip->AddData(
				"text/plain", B_MIME_TYPE, buffer, length);

			int arraySize = sizeof(text_run_array) +
					n_styles * sizeof(text_run);
			text_run_array *array = (text_run_array *)malloc(
				arraySize);
			...
			for (int i = 0; i < n_styles; i++) {
				...
				array->runs[i].color = nsbeos_rgb_colour(
					styles[i].style.foreground);
			}
