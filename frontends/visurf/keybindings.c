...
uint32_t xkb_state_mask(struct xkb_state *state)
{
    ...
    for (size_t i = 0; i < sizeof(mod_map) / sizeof(mod_map[0]); ++i) {
        if (xkb_state_mod_name_is_active(state, mod_map[i].xkb, XKB_STATE_MODS_EFFECTIVE)) {
            mask |= mod_map[i].mask;
        }
    }
    return mask;
...
nserror nsvi_bindings_new(struct nsvi_bindings *state, const char *desc, const char *cmd)
{
    ...
    if (state->nbindings + 1 >= state->zbindings) {
        if (state->zbindings == 0) {
            ...
            state->zbindings *= 2;
            struct nsvi_binding *new = realloc(state->bindings, state->zbindings * sizeof(struct nsvi_binding));
            assert(new);
            state->bindings = new;
        }
    }
    ...
    memmove(&state->bindings[i + 1], &state->bindings[i], (state->nbindings - i) * sizeof(struct nsvi_binding));
...
nserror nsvi_bindings_remove(struct nsvi_bindings *state, const char *desc)
{
    ...
    for (size_t i = 0; i < state->nbindings; i++) {
        ...
        if (!a && !b) {
            ...
            memmove(
                &state->bindings[i], &state->bindings[i + 1], (state->nbindings - i - 1) * sizeof(struct nsvi_binding));
            i--;
            state->nbindings--;
        }
    }
...
static enum binding_state get_binding_state(struct nsvi_bindings *state, struct nsvi_binding *binding)
{
...
void nsvi_bindings_handle(struct nsvi_bindings *state, xkb_keycode_t keycode, bool pressed)
{
