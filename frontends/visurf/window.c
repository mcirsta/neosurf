...
static int draw_tabs(struct nsvi_window *win, struct pool_buffer *buf)
{
    ...
    for (size_t i = 0; i < win->ntab; ++i) {
        ...
        cairo_rectangle(buf->cairo, x + config.margin, config.margin, config.tabs.throbber.width, height);
        cairo_fill(buf->cairo);

        cairo_save(buf->cairo);
        cairo_rectangle(buf->cairo, x, config.margin, tabwidth - config.margin * 2, height);
        ...
        cairo_move_to(buf->cairo, x + (config.margin * 2) + config.tabs.throbber.width, config.margin);
...
static char *keybuf_to_str(struct nsvi_bindings *state)
{
    ...
    for (size_t i = 0; i < state->nbuf; i++) {
        ...
        if (state->buffer[i].mask != 0) {
            ...
            buf[idx++] = '-';
            buf[idx++] = '\0';
            int ret = snprintf(&keys[bufidx + inc], bufsz - bufidx, "%s", buf);
            assert(ret >= 0);
        ...
        int ret = xkb_keysym_to_utf8(state->buffer[i].keysym, &keys[bufidx + inc], bufsz - bufidx);
        if (ret < 0) {
            goto expand;
        }
        if (ret == 0) {
...
static int draw_normal_statusbar(struct nsvi_window *win, struct pool_buffer *buf)
{
    ...
    cairo_rectangle(buf->cairo, 0, win->height - (config.margin * 2 + height), win->width, config.margin * 2 + height);
    cairo_fill(buf->cairo);
    ...
    cairo_move_to(buf->cairo, win->width - width - config.margin, win->height - height - config.margin);
    pango_printf(buf->cairo, config.font, "%s", status);
    free(status);
    return height + config.margin * 2;
}

static int draw_insert_statusbar(struct nsvi_window *win, struct pool_buffer *buf)
{
    int width, height;
...
static int draw_exline_completions(struct nsvi_window *win, struct pool_buffer *buf, int bottom)
{
    ...
    for (size_t i = 0; i < exline->ncomp; ++i, ++nitems) {
        ...
        get_text_size(buf->cairo, config.font, &itemwidth, &itemheight, NULL, "%s", exline->comps[i]);
    ...
    cairo_rectangle(buf->cairo, 0, bottom - (config.margin * 2 + height), win->width, config.margin * 2 + height);
    cairo_fill(buf->cairo);
...
static int draw_exline_statusbar(struct nsvi_window *win, struct pool_buffer *buf)
{
    struct exline_state *exline = &win->exline;
    ...
    cairo_move_to(buf->cairo, config.margin + width, win->height - height - config.margin);
    cairo_line_to(buf->cairo, config.margin + width, win->height - config.margin);
...
static int draw_follow_statusbar(struct nsvi_window *win, struct pool_buffer *buf)
{
    int width, height;
    get_text_size(buf->cairo, config.font, &width, &height, NULL, "Choose a link to follow...");
...
static int draw_error_statusbar(struct nsvi_window *win, struct pool_buffer *buf)
{
    int width, height;
...
static void draw_hints(struct nsvi_window *win, struct pool_buffer *buf)
{
    ...
    for (size_t i = 0; i < follow->nhint; ++i) {
        ...
        size_t nlike = 0;
        for (; nlike < follow->hintln && nlike < strlen(hint->hint); ++nlike) {
        ...
        int width, height;
        get_text_size(buf->cairo, config.font, &width, &height, NULL, "%s", hint->hint);
        ...
        cairo_rectangle(buf->cairo, x - config.hints.border.width, y - config.hints.border.width,
        ...
        cairo_rectangle(buf->cairo, x, y, width + config.margin * 2, height + config.margin * 2);
        cairo_fill(buf->cairo);
...
static bool draw_frame(struct nsvi_window *win)
{
    ...
    struct redraw_context ctx = {.interactive = true, .background_images = true, .plot = &nsvi_plotters};
    ...
    if (gw->caret.enabled) {
        ...
        int x = gw->caret.x + gw->sx, y = win->tab_height + gw->caret.y + gw->sy;
...
static void nsvi_window_set_cursor(struct nsvi_window *win)
{
    ...
    if (!win->mouse.cursor_surface) {
        win->mouse.cursor_surface = wl_compositor_create_surface(win->state->wl_compositor);
    }
    ...
    wl_pointer_set_cursor(
        win->state->wl_pointer, win->mouse.serial, win->mouse.cursor_surface, image->hotspot_x, image->hotspot_y);
    wl_surface_attach(win->mouse.cursor_surface, buffer, 0, 0);
    wl_surface_damage_buffer(win->mouse.cursor_surface, 0, 0, INT32_MAX, INT32_MAX);
...
static struct nsvi_window *nsvi_window_create(void)
{
    ...
    win->wl_surface = wl_compositor_create_surface(global_state->wl_compositor);
    wl_surface_add_listener(win->wl_surface, &wl_surface_listener, win);
    win->xdg_surface = xdg_wm_base_get_xdg_surface(global_state->xdg_wm_base, win->wl_surface);
    ...
    if (global_state->xdg_decoration_manager != NULL) {
        win->xdg_toplevel_decoration = zxdg_decoration_manager_v1_get_toplevel_decoration(
            global_state->xdg_decoration_manager, win->xdg_toplevel);
...
static void nsvi_window_add_tab(struct nsvi_window *win, struct gui_window *gw, bool background)
{
    gw->window = win;
    ...
    if (!win->tabs) {
        ...
        win->tabs = realloc(win->tabs, (win->ntab + 1) * sizeof(struct gui_window *));
        assert(win->tabs);
    }
    memmove(&win->tabs[i + 1], &win->tabs[i], sizeof(struct gui_window *) * (win->ntab - i));
    ++win->ntab;
    win->tabs[i] = gw;
    if (!background) {
        win->tab = i;
...
nsvi_gui_window_create(struct browser_window *bw, struct gui_window *existing, gui_window_create_flags flags)
{
...
static void nsvi_gui_window_destroy(struct gui_window *gw)
{
    ...
    memmove(&win->tabs[i], &win->tabs[i + 1], sizeof(struct gui_window *) * (win->ntab - (i + 1)));
    free(gw->search);
    free(gw);
...
static nserror nsvi_window_invalidate(struct gui_window *gw, const struct rect *rect)
{
    struct nsvi_window *win = gw->window;
...
static nserror nsvi_window_set_scroll(struct gui_window *gw, const struct rect *rect)
{
    struct nsvi_window *win = gw->window;
...
static nserror nsvi_window_get_dimensions(struct gui_window *gw, int *width, int *height)
{
    struct nsvi_window *win = gw->window;
    *width = win->width;
...
static nserror nsvi_window_event(struct gui_window *gw, enum gui_window_event event)
{
    struct nsvi_window *win = gw->window;
...
static void nsvi_window_set_pointer(struct gui_window *gw, enum gui_pointer_shape shape)
{
    struct nsvi_window *win = gw->window;
    if (win->tabs[win->tab] != gw) {
...
static void nsvi_window_place_caret(struct gui_window *gw, int x, int y, int height, const struct rect *clip)
{
    gw->caret.x = x;
    gw->caret.y = y;
...
static struct nsurl *nsvi_window_url_filter(struct gui_window *gw, struct nsurl *url)
{
    struct nsvi_window *win = gw->window;
...
static void nsvi_window_handle_normal_key(struct nsvi_window *win, xkb_keycode_t keycode, bool pressed)
{
    if (!pressed) {
        return;
    }
    xkb_keysym_t sym = xkb_state_key_get_one_sym(win->state->xkb_state, keycode);
    uint32_t mask = xkb_state_mask(win->state->xkb_state);
    uint32_t codepoint = xkb_state_key_get_utf32(win->state->xkb_state, keycode);
...
static void exline_delete_backward(struct exline_state *exline, size_t size)
{
    ...
    memmove(&exline->cmd[exline->index - size], &exline->cmd[exline->index], (exline->cmdln - exline->index) + size);
    exline->index -= size;
    exline->cmdln -= size;
}
...
static void nsvi_window_exline_paste(struct nsvi_window *win)
{
    ...
    while (true) {
        ...
        memmove(&exline->cmd[exline->index + n], &exline->cmd[exline->index], (exline->cmdln - exline->index) + 1);
...
static void nsvi_window_handle_exline_key(struct nsvi_window *win, xkb_keycode_t keycode, bool pressed)
{
    if (!pressed) {
        return;
    }
    ...
    if (mask == 0) {
        ...
        case XKB_KEY_Delete:
            if (exline->index != exline->cmdln) {
                memmove(
                    &exline->cmd[exline->index], &exline->cmd[exline->index + 1], (exline->cmdln - exline->index) + 1);
                --exline->cmdln;
                request_frame(win);
            }
            return;
        ...
        case XKB_KEY_Delete:
        case XKB_KEY_w:
            exline_delete_backward(exline, exline->index - exline_next_word_start_backward(exline));
            request_frame(win);
            return;
        case XKB_KEY_a:
            exline->index = 0;
    ...
    memmove(&exline->cmd[exline->index + sz], &exline->cmd[exline->index], (exline->cmdln - exline->index) + 1);
    char buf[9];
...
static void nsvi_window_follow_hint(struct nsvi_window *win, struct link_hint *hint)
{
    ...
    if (gadget) {
        ...
        case GADGET_CHECKBOX:
            ...
            dom_html_input_element_set_checked((dom_html_input_element *)(gadget->node), gadget->selected);
            break;
        case GADGET_RADIO:
        ...
        case GADGET_IMAGE:
        case GADGET_SUBMIT:
            error = form_submit(content_get_url((struct content *)gadget->html), gw->bw, gadget->form, gadget);
            break;
        case GADGET_TEXTBOX:
        case GADGET_PASSWORD:
        case GADGET_TEXTAREA:
            html_set_focus(gadget->html, HTML_FOCUS_TEXTAREA, (union html_focus_owner)hint->node, false, 0, 0, 0, NULL);
            textarea_set_caret(gadget->data.text.ta, textarea_get_text(gadget->data.text.ta, NULL, 0) - 1);
            break;
        case GADGET_HIDDEN:
            /* not possible */
            break;
        ...
        case FOLLOW_OPEN:
            error = browser_window_navigate(gw->bw, url, NULL, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
            break;
        ...
        case FOLLOW_OPEN_WINDOW:
            ...
            break;
        case FOLLOW_YANK:
            nsvi_set_clipboard(win->state, "text/plain", nsurl_access(url), strlen(nsurl_access(url)));
            break;
...
static void nsvi_window_handle_follow_key(struct nsvi_window *win, xkb_keycode_t keycode, bool pressed)
{
    if (!pressed) {
        return;
    }
    ...
    int sz = xkb_state_key_get_utf8(win->state->xkb_state, keycode, NULL, 0);
    xkb_state_key_get_utf8(win->state->xkb_state, keycode, &follow->hintbuf[follow->hintln], follow->needed + 1);
    ...
    for (size_t i = 0; i < follow->nhint; ++i) {
        ...
        if (strncmp(follow->hints[i].hint, follow->hintbuf, follow->hintln + 1) == 0) {
            partial = true;
        }
    }
...
void nsvi_window_key_event(struct nsvi_window *win, xkb_keycode_t keycode, bool pressed)
{
    struct nsvi_state *state = win->state;
    switch (win->mode) {
    case INSERT:
...
static bool nsvi_window_browser_coords(
    struct nsvi_window *win, struct gui_window *gw, int surface_x, int surface_y, int *browser_x, int *browser_y)
{
    *browser_x = surface_x - gw->sx;
    *browser_y = surface_y - gw->sy - win->tab_height;
    if (surface_y < win->tab_height || surface_y >= (win->height - win->status_height)) {
        return false;
    }
    return true;
}

static void nsvi_window_pointer_motion_event(struct nsvi_window *win, struct pointer_event *event)
{
    struct gui_window *gw = win->tabs[win->tab];
    ...
    int bx, by;
    int bpx, bpy;
    if (!nsvi_window_browser_coords(win, gw, pointer_x, pointer_y, &bx, &by)) {
        goto exit;
    }
    nsvi_window_browser_coords(win, gw, pointer_x, pointer_y, &bpx, &bpy);
    if (win->mouse.state & BROWSER_MOUSE_PRESS_1) {
        // Start button 1 drag
        browser_window_mouse_click(gw->bw, BROWSER_MOUSE_DRAG_1, bpx, bpy);
        ...
        // Start button 2 drag
        browser_window_mouse_click(gw->bw, BROWSER_MOUSE_DRAG_2, bpx, bpy);
...
static void nsvi_window_pointer_button_press_event(struct nsvi_window *win, struct pointer_event *event)

{
    ...
    if (win->mouse.pressed_y <= win->tab_height) {
        ...
        if (event->button == BTN_MIDDLE) {
            nsurl *url;
            if (browser_window_get_url(win->tabs[tab]->bw, true, &url) == NSERROR_OK) {
    ...
    int bx, by;
    if (!nsvi_window_browser_coords(win, gw, win->mouse.pointer_x, win->mouse.pointer_y, &bx, &by)) {
        return;
    }
    browser_window_mouse_click(gw->bw, win->mouse.state, bx, by);
}

static void nsvi_window_pointer_button_release_event(struct nsvi_window *win, struct pointer_event *event)
{
    struct gui_window *gw = win->tabs[win->tab];
...
void nsvi_window_pointer_event(struct nsvi_window *win, struct pointer_event *event)
{
    struct gui_window *gw = win->tabs[win->tab];
    ...
    if (event->event_mask & POINTER_EVENT_ENTER || event->event_mask & POINTER_EVENT_MOTION) {
        nsvi_window_pointer_motion_event(win, event);
    }
    int bx, by;
    bool bounds = nsvi_window_browser_coords(win, gw, win->mouse.pointer_x, win->mouse.pointer_y, &bx, &by);
...
static void nsvi_window_touch_scroll(struct nsvi_window *win, struct touch_event *event)
{
    struct nsvi_window_touch *touch = &win->touch;
    struct touch_point *point = get_touch_point(win->state, touch->scroll.id);
    assert(point && point->valid);
    if (point->event_mask & TOUCH_EVENT_UP || point->event_mask & TOUCH_EVENT_CANCEL) {
    ...
    int bx, by;
    struct gui_window *gw = win->tabs[win->tab];
    bool bounds = nsvi_window_browser_coords(win, gw, touch->down_x, touch->down_y, &bx, &by);
    if (!bounds) {
        return;
    }
    int sx = wl_fixed_to_int(point->surface_x), sy = wl_fixed_to_int(point->surface_y);
...
static void nsvi_window_touch_drag(struct nsvi_window *win, struct touch_event *event)
{
    struct nsvi_window_touch *touch = &win->touch;
    struct touch_point *point = get_touch_point(win->state, touch->scroll.id);
    assert(point && point->valid);
    ...
    if (point->event_mask & TOUCH_EVENT_UP || point->event_mask & TOUCH_EVENT_CANCEL) {
        ...
        browser_window_mouse_track(gw->bw, BROWSER_MOUSE_DRAG_ON | BROWSER_MOUSE_HOLDING_1, bx, by);
    }
}

static void nsvi_window_touch_zoom(struct nsvi_window *win, struct touch_event *event)
{
    // TODO
}

static void nsvi_window_touch_pending(struct nsvi_window *win, struct touch_event *event)
{
    ...
    int bx, by;
    struct gui_window *gw = win->tabs[win->tab];
    bool bounds = nsvi_window_browser_coords(win, gw, win->touch.down_x, win->touch.down_y, &bx, &by);
    ...
    case 1:
        ...
        if (points[0]->event_mask & TOUCH_EVENT_UP || points[0]->event_mask & TOUCH_EVENT_CANCEL) {
            // Tap interaction?
            if (diff < 1000 && length < 25) {
                if (bounds) {
                    browser_window_mouse_click(gw->bw, BROWSER_MOUSE_PRESS_1, bx, by);
                    browser_window_mouse_click(gw->bw, BROWSER_MOUSE_CLICK_1, bx, by);
                    browser_window_mouse_track(gw->bw, 0, bx, by);
                } else {
        ...
        if (length > 25) {
            ...
            browser_window_mouse_click(gw->bw, BROWSER_MOUSE_PRESS_1, bx, by);
            browser_window_mouse_click(gw->bw, BROWSER_MOUSE_DRAG_1, bx, by);
            browser_window_mouse_track(gw->bw, BROWSER_MOUSE_DRAG_ON | BROWSER_MOUSE_HOLDING_1, bx, by);
        }
        break;
    case 2:
