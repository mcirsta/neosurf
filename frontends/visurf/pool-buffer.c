...
{
    static const char template[] = "visurf-XXXXXX";
...
create_buffer(struct wl_shm *shm, struct pool_buffer *buf, int32_t width, int32_t height, uint32_t format)
{
    ...
    buf->layout = pango_cairo_create_layout(buf->cairo);
...
struct pool_buffer *
get_next_buffer(struct wl_shm *shm, struct pool_buffer pool[static 2], uint32_t width, uint32_t height, int scale)
{
