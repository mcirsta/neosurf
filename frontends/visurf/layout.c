...
static nserror nsfont_width(const plot_font_style_t *fstyle, const char *string, size_t length, int *width)
{
    ...
    PangoLayout *layout = activebuffer->layout;
    ...
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x)
{
    ...
    if (pango_layout_xy_to_index(layout, x * PANGO_SCALE, 0, &index, 0) == FALSE) {
        index = length;
    }
    ...
    const plot_font_style_t *fstyle, const char *string, size_t length, int x, size_t *char_offset, int *actual_x)
{
    ...
    layout = pango_layout_new(activebuffer->pango);
...
nserror nsfont_paint(int x, int y, const char *string, size_t length, const plot_font_style_t *fstyle)
{
    ...
    cairo_move_to(activebuffer->cairo, x, y);
    nsvi_set_colour(fstyle->foreground);
    pango_cairo_show_layout_line(activebuffer->cairo, line);
...
struct gui_layout_table vi_layout_table = {
    .width = nsfont_width,
    .position = nsfont_position_in_string,
    .split = nsfont_split,
};

static PangoLayout *get_chrome_layout(cairo_t *cairo, const char *font, const char *text)
{
...
void get_text_size(cairo_t *cairo, const char *font, int *width, int *height, int *baseline, const char *fmt, ...)
{
    va_list args;
    va_start(args, fmt);
