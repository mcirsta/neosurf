...
void nsvi_set_colour(colour c)
{
    cairo_set_source_rgba(
        activebuffer->cairo, (c & 0xff) / 255.0, ((c & 0xff00) >> 8) / 255.0, ((c & 0xff0000) >> 16) / 255.0, 1.0);
}
...
static inline void nsvi_set_line_width(plot_style_fixed width)
{
    if (width == 0) {
        ...
        cairo_set_line_width(activebuffer->cairo, plot_style_fixed_to_double(width));
    }
}

static nserror nsvi_plot_clip(const struct redraw_context *ctx, const struct rect *clip)
{
    cairo_reset_clip(activebuffer->cairo);
    cairo_rectangle(activebuffer->cairo, clip->x0, clip->y0, clip->x1 - clip->x0, clip->y1 - clip->y0);
    ...
    const struct redraw_context *ctx, const plot_style_t *style, int x, int y, int radius, int angle1, int angle2)
{
    ...
    cairo_set_line_width(activebuffer->cairo, 1);
    cairo_arc(activebuffer->cairo, x, y, radius, (angle1 + 90) * (M_PI / 180), (angle2 + 90) * (M_PI / 180));
    cairo_stroke(activebuffer->cairo);

    return NSERROR_OK;
}

static nserror nsvi_plot_disc(const struct redraw_context *ctx, const plot_style_t *style, int x, int y, int radius)
{
...
static nserror nsvi_plot_line(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *line)
{
    nsvi_set_colour(style->stroke_colour);
     ...
     * between pixels
     */
    cairo_move_to(activebuffer->cairo, (line->x0 == line->x1) ? line->x0 + 0.5 : line->x0,
        (line->y0 == line->y1) ? line->y0 + 0.5 : line->y0);
    cairo_line_to(activebuffer->cairo, (line->x0 == line->x1) ? line->x1 + 0.5 : line->x1,
...
static nserror nsvi_plot_rectangle(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *rect)
{
    if (style->fill_type != PLOT_OP_TYPE_NONE) {
        nsvi_set_colour(style->fill_colour);
        nsvi_set_solid();
        ...
        cairo_set_line_width(activebuffer->cairo, 0);
        cairo_rectangle(activebuffer->cairo, rect->x0, rect->y0, rect->x1 - rect->x0, rect->y1 - rect->y0);
        cairo_fill(activebuffer->cairo);
        cairo_stroke(activebuffer->cairo);
    }
    ...
    if (style->stroke_type != PLOT_OP_TYPE_NONE) {
        ...
        cairo_rectangle(activebuffer->cairo, rect->x0 + 0.5, rect->y0 + 0.5, rect->x1 - rect->x0, rect->y1 - rect->y0);
        cairo_stroke(activebuffer->cairo);
    }
    return NSERROR_OK;
}
...
static nserror
nsvi_plot_polygon(const struct redraw_context *ctx, const plot_style_t *style, const int *p, unsigned int n)
{
    unsigned int i;
...
static nserror nsvi_plot_path(const struct redraw_context *ctx, const plot_style_t *pstyle, const float *p,
    unsigned int n, const float transform[6])
{
    ...
    for (i = 0; i < n;) {
        if (p[i] == PLOTTER_PATH_MOVE) {
            ...
            cairo_curve_to(activebuffer->cairo, p[i + 1], p[i + 2], p[i + 3], p[i + 4], p[i + 5], p[i + 6]);
            i += 7;
        } else {
...
static nserror nsvi_plot_bitmap(const struct redraw_context *ctx, struct bitmap *bitmap, int x, int y, int width,
    int height, colour bg, bitmap_flags_t flags)
{
    ...
    if ((img_width == width) && (img_height == height)) {
        ...
        if (repeat_x || repeat_y) {
            cairo_pattern_set_extend(cairo_get_source(activebuffer->cairo), CAIRO_EXTEND_REPEAT);
        }

        /* Render the bitmap */
        cairo_rectangle(
            activebuffer->cairo, cliprect_bitmap.x0, cliprect_bitmap.y0, cliprect_bitmap.x1, cliprect_bitmap.y1);
        cairo_fill(activebuffer->cairo);
    } else {
        /* Scaled rendering */
        ...
        cairo_set_source_surface(activebuffer->cairo, img_surface, x / scale_x, y / scale_y);
        ...
        /* Render the bitmap */
        cairo_rectangle(activebuffer->cairo, cliprect_bitmap.x0 / scale_x, cliprect_bitmap.y0 / scale_y,
            cliprect_bitmap.x1 / scale_x, cliprect_bitmap.y1 / scale_y);
        cairo_fill(activebuffer->cairo);
...
static nserror nsvi_plot_text(const struct redraw_context *ctx, const struct plot_font_style *fstyle, int x, int y,
...
/** GTK plotter table */
const struct plotter_table nsvi_plotters = {.clip = nsvi_plot_clip,
    .arc = nsvi_plot_arc,
    .disc = nsvi_plot_disc,
    .line = nsvi_plot_line,
