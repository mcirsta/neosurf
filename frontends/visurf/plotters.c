...
static nserror nsvi_plot_arc(const struct redraw_context *ctx,
...
static nserror nsvi_plot_disc(const struct redraw_context *ctx,
			      const plot_style_t *style,
			      int x,
			      int y,
			      int radius)
...
static nserror nsvi_plot_line(const struct redraw_context *ctx,
			      const plot_style_t *style,
...
static nserror nsvi_plot_rectangle(const struct redraw_context *ctx,
				   const plot_style_t *style,
...
static nserror nsvi_plot_polygon(const struct redraw_context *ctx,
				 const plot_style_t *style,
				 const int *p,
				 unsigned int n)
{
...
static nserror nsvi_plot_path(const struct redraw_context *ctx,
			      ...
			      const float transform[6])
{
	...
	for (i = 0; i < n;) {
		if (p[i] == PLOTTER_PATH_MOVE) {
			...
			cairo_curve_to(activebuffer->cairo,
				       p[i + 1],
				       p[i + 2],
				       p[i + 3],
				       p[i + 4],
...
static nserror nsvi_plot_bitmap(const struct redraw_context *ctx,
				...
				bitmap_flags_t flags)
{
	...
	if ((img_width == width) && (img_height == height)) {
		...
		cairo_set_source_surface(
			activebuffer->cairo, img_surface, x, y);
		...
		cairo_set_source_surface(activebuffer->cairo,
					 img_surface,
					 x / scale_x,
					 y / scale_y);
...
static nserror nsvi_plot_text(const struct redraw_context *ctx,
