...
static int cmd_exec(struct nsvi_state *state, int argc, char *argv[])
{
    ...
    if (pid == 0) {
        ...
        NSLOG(neosurf, ERROR, "exec: execvp failed: %s", strerror(errno));
        exit(1);
    }
...
static void follow_collect_hints(struct gui_window *win, struct box *node)
{
    ...
    if ((node->href || node->gadget) && node->width > 0 && node->height > 0) {
        for (size_t i = 0; i < follow->nhint; i++) {
            if (node->href == follow->hints[i].node->href && node->href != NULL) {
                goto end;
            }
        }
        if (!follow->hints) {
            ...
            follow->zhint *= 2;
            follow->hints = realloc(follow->hints, follow->zhint * sizeof(struct link_hint));
        }
...
static void follow_label_hints(struct gui_window *win)
{
    ...
    if (needed > 1) {
        for (size_t serial = 0; serial < nshort; ++serial) {
            follow->hints[i++].hint = mkhint(serial, alphabet, needed - 1);
        }
    }
...
static int cmd_page(struct nsvi_state *state, int argc, char *argv[])
{
    ...
    struct content_rfc5988_link *link = hlcache_handle_get_content(handle)->links;
    bool rel_match = true;
    int ret = 0;
    ...
    while (link != NULL) {
        if (lwc_string_caseless_isequal(link->rel, rel, &rel_match) == lwc_error_ok && rel_match) {
            browser_window_navigate(bw, link->href, NULL, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
            request_frame(win);
            goto exit;
        }
        link = link->next;
...
static int cmd_search(struct nsvi_state *state, int argc, char *argv[])
{
    ...
    browser_window_search(win->tabs[win->tab]->bw, NULL, gw->search_flags, gw->search);
    return 0;
}
...
static int cmd_undo(struct nsvi_state *state, int argc, char *argv[])
{
    ...
    case UNDO_TAB:
        ...
        error = browser_window_create(
            BW_CREATE_TAB | BW_CREATE_FOREGROUND | BW_CREATE_HISTORY, undo->urls[0], NULL, gw->bw, NULL);
        if (error != NSERROR_OK) {
...
static void populate_env(struct nsvi_state *state)
{
    ...
    if (err == NSERROR_OK) {
        ...
        char *part;
        size_t sz;
        for (size_t i = 0; i < sizeof(urlparts) / sizeof(urlparts[0]); ++i) {
...
void nsvi_command(void *user, const char *cmd)
{
    ...
    struct command *command = bsearch(
        argv[0], &commands, sizeof(commands) / sizeof(commands[0]), sizeof(commands[0]), cmd_compar);
    if (!command) {
        char *error_message;
        if (argv[0] == NULL) {
...
int nsvi_command_source(void *user, const char *filepath)
{
    ...
    if (f == NULL) {
        NSLOG(neosurf, ERROR, "source: unable to open file at '%s'", filepath);
        return 1;
    }
    char *line = NULL;
    size_t size = 0;
