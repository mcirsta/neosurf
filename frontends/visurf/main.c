...
static nserror nsvi_schedule_remove(void (*callback)(void *p), void *p)
{
...
static nserror nsvi_misc_schedule(int ms, void (*callback)(void *p), void *p)
{
...
static void wl_data_source_target(void *data, struct wl_data_source *wl_data_source, const char *mime_type)
{
    // Not used
}

static void wl_data_source_send(void *data, struct wl_data_source *wl_data_source, const char *mime_type, int32_t fd)
{
...
static void wl_data_source_dnd_drop_performed(void *data, struct wl_data_source *wl_data_source)
{
    // Not used
}

static void wl_data_source_dnd_finished(void *data, struct wl_data_source *wl_data_source)
{
    // Not used
}

static void wl_data_source_action(void *data, struct wl_data_source *wl_data_source, uint32_t dnd_action)
{
    // Not used
}
...
void nsvi_set_clipboard(struct nsvi_state *state, const char *type, const char *data, size_t len)
{
    ...
    struct wl_data_source *source = wl_data_device_manager_create_data_source(state->wl_data_device_manager);
    ...
    wl_data_device_set_selection(state->wl_data_device, source, win->keyboard_enter_serial);
}

static void nsvi_clipboard_get(char **buffer, size_t *length)
{
...
static void nsvi_clipboard_set(const char *buffer, size_t length, nsclipboard_styles styles[], int n_styles)
{
...
static void wl_pointer_enter(void *data, struct wl_pointer *wl_pointer, uint32_t serial, struct wl_surface *surface,
    wl_fixed_t surface_x, wl_fixed_t surface_y)
{
    ...
    state->pointer_event.surface_x = surface_x, state->pointer_event.surface_y = surface_y;
}
    ...
    void *data, struct wl_pointer *wl_pointer, uint32_t serial, uint32_t time, uint32_t button, uint32_t state)
{
    ...
    vistate->pointer_event.button = button, vistate->pointer_event.state = state;
}
    ...
    void *data, struct wl_keyboard *wl_keyboard, uint32_t serial, uint32_t time, uint32_t key, uint32_t state)
{
    ...
    if (pressed && vistate->repeat_rate != 0 && xkb_keymap_key_repeats(vistate->xkb_keymap, keycode)) {
...
static void wl_keyboard_modifiers(void *data, struct wl_keyboard *wl_keyboard, uint32_t serial, uint32_t mods_depressed,
...
static void wl_touch_down(void *data, struct wl_touch *wl_touch, uint32_t serial, uint32_t time,
...
static void
wl_touch_motion(void *data, struct wl_touch *wl_touch, uint32_t time, int32_t id, wl_fixed_t x, wl_fixed_t y)
{
    struct nsvi_state *state = data;
    struct touch_point *point = get_touch_point(state, id);
    if (point == NULL) {
...
static void wl_output_geometry(void *data, struct wl_output *wl_output, int32_t x, int32_t y, int32_t physical_width,
...
static void wl_data_offer_source_actions(void *data, struct wl_data_offer *offer, uint32_t source_actions)
{
...
static void wl_data_device_data_offer(void *data, struct wl_data_device *data_device, struct wl_data_offer *offer)
{
    wl_data_offer_add_listener(offer, &wl_data_offer_listener, data);
}

static void wl_data_device_enter(void *data, struct wl_data_device *wl_data_device, uint32_t serial,
    struct wl_surface *surface, wl_fixed_t x, wl_fixed_t y, struct wl_data_offer *id)
{
    // TODO
}

static void wl_data_device_leave(void *data, struct wl_data_device *wl_data_device)
{
    // TODO
}

static void
wl_data_device_motion(void *data, struct wl_data_device *wl_data_device, uint32_t time, wl_fixed_t x, wl_fixed_t y)
{
    // TODO
}

static void wl_data_device_drop(void *data, struct wl_data_device *wl_data_device)
{
    // TODO
}

static void wl_data_device_selection(void *data, struct wl_data_device *wl_data_device, struct wl_data_offer *id)
{
    struct nsvi_state *state = data;
...
registry_global(void *data, struct wl_registry *wl_registry, uint32_t name, const char *interface, uint32_t version)
{
    ...
    if (strcmp(interface, wl_shm_interface.name) == 0) {
        ...
        struct nsvi_output *output = calloc(1, sizeof(struct nsvi_output));
        output->wl_output = wl_registry_bind(wl_registry, name, &wl_output_interface, 3);
...
static char **nsvi_init_resource(const char *resource_path)
{
...
static void nsvi_init_bindings(struct nsvi_state *state, struct nsvi_bindings *bindings)
{
...
static void nsvi_load_config(struct nsvi_state *state)
{
...
nserror nsvi_filter_url(struct nsvi_filter *filter, const char *in, nsurl **out)
{
    nsurl *new;
    nserror err;
    if (!filter->in) {
...
int main(int argc, char *argv[])
{
    ...
    if (state.wl_data_device_manager && state.wl_seat) {
        state.wl_data_device = wl_data_device_manager_get_data_device(state.wl_data_device_manager, state.wl_seat);
    ...
    while (!state.quit) {
        ...
        if (next) {
            ...
            if (timeout <= 0) {
                ...
                /* If less than 1ms remains, wait at least 1ms
        ...
        for (int fd = 0; fd <= max_fd; ++fd) {
            bool present = FD_ISSET(fd, &read_fd) || FD_ISSET(fd, &write_fd) || FD_ISSET(fd, &except_fd);
            if (present) {
                ++nfetch;
            }
        }
        if (state.nfd < nfetch + 1) {
            state.nfd = nfetch + 1;
            state.fds = realloc(state.fds, sizeof(struct pollfd) * state.nfd);
        }
        int i = 1;
