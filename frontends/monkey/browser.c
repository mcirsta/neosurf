...
gui_window_create(struct browser_window *bw, struct gui_window *existing, gui_window_create_flags flags)
{
    ...
    moutf(MOUT_WINDOW, "NEW WIN %u FOR %p EXISTING %p NEWTAB %s CLONE %s", ret->win_num, bw, existing,
        flags & GW_CREATE_TAB ? "TRUE" : "FALSE", flags & GW_CREATE_CLONE ? "TRUE" : "FALSE");
    moutf(MOUT_WINDOW, "SIZE WIN %u WIDTH %d HEIGHT %d", ret->win_num, ret->width, ret->height);
...
static nserror gui_window_get_dimensions(struct gui_window *g, int *width, int *height)
{
    ...
    *height = g->height;
    ...
    moutf(MOUT_WINDOW, "GET_DIMENSIONS WIN %u WIDTH %d HEIGHT %d", g->win_num, *width, *height);
...
static nserror gui_window_set_scroll(struct gui_window *gw, const struct rect *rect)
{
    ...
    moutf(MOUT_WINDOW, "SET_SCROLL WIN %u X %d Y %d", gw->win_num, rect->x0, rect->y0);
    return NSERROR_OK;
}
...
{
    if (rect != NULL) {
        moutf(MOUT_WINDOW, "INVALIDATE_AREA WIN %u X %d Y %d WIDTH %d HEIGHT %d", gw->win_num, rect->x0, rect->y0,
...
static void gui_window_update_extent(struct gui_window *g)
{
    ...
    moutf(MOUT_WINDOW, "UPDATE_EXTENT WIN %u WIDTH %d HEIGHT %d", g->win_num, width, height);
}
...
static void gui_window_set_pointer(struct gui_window *g, gui_pointer_shape shape)
{
    ...
    moutf(MOUT_WINDOW, "SET_POINTER WIN %u POINTER %s", g->win_num, ptr_name);
}

static nserror gui_window_set_url(struct gui_window *g, nsurl *url)
{
    moutf(MOUT_WINDOW, "SET_URL WIN %u URL %s", g->win_num, nsurl_access(url));
    return NSERROR_OK;
}

static bool gui_window_get_scroll(struct gui_window *g, int *sx, int *sy)
{
    moutf(MOUT_WINDOW, "GET_SCROLL WIN %u X %d Y %d", g->win_num, g->scrollx, g->scrolly);
    *sx = g->scrollx;
    *sy = g->scrolly;
    return true;
}
...
{
    moutf(MOUT_WINDOW, "PLACE_CARET WIN %u X %d Y %d HEIGHT %d", g->win_num, x, y, height);
}
...
{
    moutf(MOUT_WINDOW, "SAVE_LINK WIN %u URL %s TITLE %s", g->win_num, nsurl_access(url), title);
    return NSERROR_OK;
}

static void gui_window_console_log(struct gui_window *g, browser_window_console_source src, const char *msg,
    size_t msglen, browser_window_console_flags flags)
{
    ...
    moutf(MOUT_WINDOW, "CONSOLE_LOG WIN %u SOURCE %s %sFOLDABLE %s %.*s", g->win_num, src_text, foldable ? "" : "NOT-",
        level_text, (int)msglen, msg);
}
...
static void monkey_window_handle_go(int argc, char **argv)
{
    ...
    if (error == NSERROR_OK) {
        ...
        if (error == NSERROR_OK) {
            error = browser_window_navigate(gw->bw, url, ref_url, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
            if (ref_url != NULL) {
                nsurl_unref(ref_url);
            }
        }
