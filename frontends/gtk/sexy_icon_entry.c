...
#define IS_VALID_ICON_ENTRY_POSITION(pos) ((pos) == SEXY_ICON_ENTRY_PRIMARY || (pos) == SEXY_ICON_ENTRY_SECONDARY)
...
G_DEFINE_TYPE_EXTENDED(SexyIconEntry, sexy_icon_entry, GTK_TYPE_ENTRY, 0,
...
void sexy_icon_entry_class_init(SexyIconEntryClass *klass)
{
    ...
    signals[ICON_PRESSED] = g_signal_new("icon_pressed", G_TYPE_FROM_CLASS(gobject_class),
        G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(SexyIconEntryClass, icon_pressed), NULL, NULL, NULL,
     ...
     * g_signal_new() changed from marshal type to NULL - so there may
     * well be no working signal */
    signals[ICON_RELEASED] = g_signal_new("icon_released", G_TYPE_FROM_CLASS(gobject_class),
        G_SIGNAL_RUN_LAST | G_SIGNAL_ACTION, G_STRUCT_OFFSET(SexyIconEntryClass, icon_released), NULL, NULL, NULL,
        G_TYPE_NONE, 2, G_TYPE_INT, G_TYPE_INT);
}
...
static void get_icon_allocation(SexyIconEntry *icon_entry, gboolean left, GtkAllocation *widget_alloc,
    GtkAllocation *text_area_alloc, GtkAllocation *allocation, SexyIconEntryPosition *icon_pos)
{
    gboolean rtl;
...
static void sexy_icon_entry_realize(GtkWidget *widget)
{
    ...
    for (i = 0; i < MAX_ICONS; i++) {
        ...
        gdk_window_set_background(icon_info->window, &widget->style->base[nsgtk_widget_get_state(widget)]);
    }
...
static void place_windows(SexyIconEntry *icon_entry, GtkAllocation *widget_alloc)
{
    ...
    gdk_window_move_resize(icon_entry->priv->icons[left_icon_pos].window, left_icon_alloc.x, left_icon_alloc.y,
        left_icon_alloc.width, left_icon_alloc.height);

    gdk_window_move_resize(icon_entry->priv->icons[right_icon_pos].window, right_icon_alloc.x, right_icon_alloc.y,
        right_icon_alloc.width, right_icon_alloc.height);

    gdk_window_move_resize(GTK_ENTRY(icon_entry)->text_area, text_area_alloc.x, text_area_alloc.y,
...
static void draw_icon(GtkWidget *widget, SexyIconEntryPosition icon_pos)
{
    ...
    gdk_draw_pixbuf(
        icon_info->window, widget->style->black_gc, pixbuf, 0, 0, x, y, -1, -1, GDK_RGB_DITHER_NORMAL, 0, 0);
    ...
    if (nsgtk_widget_is_drawable(widget)) {
        ...
        for (i = 0; i < MAX_ICONS && !found; i++) {
            ...
            if (event->window == icon_info->window) {
                ...
                gtk_paint_flat_box(widget->style, icon_info->window, nsgtk_widget_get_state(widget), GTK_SHADOW_NONE,
...
static void update_icon(GObject *obj, GParamSpec *param, SexyIconEntry *entry)
{
    if (param != NULL) {
        ...
        if (strcmp(name, "pixbuf") && strcmp(name, "stock") && strcmp(name, "image") && strcmp(name, "pixmap") &&
...
static gint sexy_icon_entry_button_release(GtkWidget *widget, GdkEventButton *event)
{
    ...
    for (i = 0; i < MAX_ICONS; i++) {
        ...
        if (event->window == icon_window) {
            ...
            if (event->button == 1 && sexy_icon_entry_get_icon_highlight(entry, i) && event->x >= 0 && event->y >= 0 &&
...
void sexy_icon_entry_set_icon(SexyIconEntry *entry, SexyIconEntryPosition icon_pos, GtkImage *icon)
{
    ...
    if (icon_pos == SEXY_ICON_ENTRY_SECONDARY && entry->priv->icon_released_id != 0) {
...
void sexy_icon_entry_add_clear_button(SexyIconEntry *icon_entry)
{
    ...
    icon = nsgtk_image_new_from_stock(NSGTK_STOCK_CLEAR, GTK_ICON_SIZE_MENU);
    gtk_widget_show(icon);
    sexy_icon_entry_set_icon(SEXY_ICON_ENTRY(icon_entry), SEXY_ICON_ENTRY_SECONDARY, GTK_IMAGE(icon));
    sexy_icon_entry_set_icon_highlight(SEXY_ICON_ENTRY(icon_entry), SEXY_ICON_ENTRY_SECONDARY, TRUE);

    if (icon_entry->priv->icon_released_id != 0) {
        g_signal_handler_disconnect(icon_entry, icon_entry->priv->icon_released_id);
    }

    icon_entry->priv->icon_released_id = g_signal_connect(
