					 ...
					 const gchar *id)
{
	...
	GtkImage *image;
	image = GTK_IMAGE(
		gtk_image_new_from_stock(id, GTK_ICON_SIZE_LARGE_TOOLBAR));
...
static void
nsgtk_layout_set_adjustment_step_increment(GtkAdjustment *adj, int value)
{
...
nsgtk_style_context_get_font(GtkStyleContext *style, GtkStateFlags state)
{
	...
	gtk_style_context_get(
		style, state, GTK_STYLE_PROPERTY_FONT, &fontdesc, NULL);
	return fontdesc;
#else
#if GTK_CHECK_VERSION(3, 0, 0)
...
#else
	gtk_menu_popup(
		menu, NULL, NULL, NULL, NULL, 0, gtk_get_current_event_time());
#endif
}
				      ...
				      GError **error)
{
	...
	ret = gtk_builder_add_from_string(
		builder, buffer, buffer_length, error);
