...
#endif
    cairo_set_source_rgba(gtk_print_current_cr, r / 255.0, g / 255.0, b / 255.0, 1.0);
}
...
static inline void nsgtk_set_line_width(plot_style_fixed width)
{
    if (width == 0) {
        ...
        cairo_set_line_width(gtk_print_current_cr, plot_style_fixed_to_double(width));
    }
}
 ...
 * \return NSERROR_OK on success else error code.
 */
static nserror nsgtk_print_plot_arc(
    const struct redraw_context *ctx, const plot_style_t *style, int x, int y, int radius, int angle1, int angle2)
{
    ...
    cairo_set_line_width(gtk_print_current_cr, 1);
    cairo_arc(gtk_print_current_cr, x, y, radius, (angle1 + 90) * (M_PI / 180), (angle2 + 90) * (M_PI / 180));
 ...
 */
static nserror
nsgtk_print_plot_disc(const struct redraw_context *ctx, const plot_style_t *style, int x, int y, int radius)
{
 ...
 */
static nserror
nsgtk_print_plot_line(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *line)
{
    nsgtk_print_set_colour(style->stroke_colour);
...
nsgtk_print_plot_rectangle(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *rect)
{
    ...
    if (style->stroke_type != PLOT_OP_TYPE_NONE) {
        ...
        cairo_rectangle(gtk_print_current_cr, rect->x0, rect->y0, rect->x1 - rect->x0, rect->y1 - rect->y0);
...
static nserror
nsgtk_print_plot_polygon(const struct redraw_context *ctx, const plot_style_t *style, const int *p, unsigned int n)
{
    unsigned int i;
 ...
 * \return NSERROR_OK on success else error code.
 */
static nserror nsgtk_print_plot_bitmap(const struct redraw_context *ctx, struct bitmap *bitmap, int x, int y, int width,
...
static nserror nsgtk_print_plot_text(const struct redraw_context *ctx, const struct plot_font_style *fstyle, int x,
...
static const struct printer gtk_printer = {&nsgtk_print_plotters, gtk_print_begin, gtk_print_next_page, gtk_print_end};
...
void gtk_print_signal_begin_print(GtkPrintOperation *operation, GtkPrintContext *context, gpointer user_data)
{
    ...
    if (print_set_up(content_to_print, &gtk_printer, settings, &height_to_print) == false) {
        ...
        height_on_page = height_on_page - FIXTOFLT(FSUB(settings->margins[MARGINTOP], settings->margins[MARGINBOTTOM]));
