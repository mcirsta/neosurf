...
static inline void nsgtk_print_set_colour(colour c)
{
	...
	gdk_colormap_alloc_color(
		gdk_colormap_get_system(), &colour, true, true);
#endif
	cairo_set_source_rgba(
		gtk_print_current_cr, r / 255.0, g / 255.0, b / 255.0, 1.0);
}
				    ...
				    int angle2)
{
	...
	cairo_set_line_width(gtk_print_current_cr, 1);
	cairo_arc(gtk_print_current_cr,
		  x,
		  y,
		  radius,
		  (angle1 + 90) * (M_PI / 180),
				    ...
				    colour bg)
{
	...
	if (width == 0 || height == 0)
		/* Nothing to plot */
		return true;
	if ((x > (cliprect.x + cliprect.width)) || ((x + width) < cliprect.x) ||
	...
	if ((bmwidth == width) && (bmheight == height)) {
		...
		cairo_rectangle(
			gtk_print_current_cr, dsrcx, dsrcy, dwidth, dheight);
		cairo_fill(gtk_print_current_cr);

	} else {
		/* Bitmap is scaled */
		if ((bitmap->scsurface != NULL) &&
		    ((cairo_image_surface_get_width(bitmap->scsurface) !=
		      width) ||
		     (cairo_image_surface_get_height(bitmap->scsurface) !=
		      height))) {
		...
		if (bitmap->scsurface == NULL) {
			bitmap->scsurface = cairo_surface_create_similar(
				bmsurface,
		...
		cairo_set_source_surface(
			gtk_print_current_cr, bitmap->scsurface, x, y);
		cairo_rectangle(
			gtk_print_current_cr, dsrcx, dsrcy, dwidth, dheight);
		cairo_fill(gtk_print_current_cr);
	}
				       ...
				       bitmap_flags_t flags)
{
	...
	while (doneheight < (cliprect.y + cliprect.height)) {
		...
		while (donewidth < (cliprect.x + cliprect.width)) {
			nsgtk_print_plot_pixbuf(donewidth,
						doneheight,
						width,
						height,
						bitmap,
...
static const struct printer gtk_printer = {&nsgtk_print_plotters,
					   gtk_print_begin,
					   gtk_print_next_page,
					   gtk_print_end};
				  ...
				  gpointer user_data)
{
	...
	if (print_set_up(content_to_print,
			 &gtk_printer,
			 settings,
			 &height_to_print) == false) {
