...
#define TOGGLEBUTTON_SIGNALS(WIDGET, OPTION)                                                                           \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_toggled(GtkToggleButton *togglebutton, struct ppref *priv);      \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_toggled(GtkToggleButton *togglebutton, struct ppref *priv)       \
    {                                                                                                                  \
        nsoption_set_bool(OPTION, gtk_toggle_button_get_active(togglebutton));                                         \
    }                                                                                                                  \
                                                                                                                       \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv);                  \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv)                   \
    {                                                                                                                  \
        gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), nsoption_bool(OPTION));                                \
    }

#define SPINBUTTON_SIGNALS(WIDGET, OPTION, MULTIPLIER)                                                                 \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_valuechanged(GtkSpinButton *spinbutton, struct ppref *priv);     \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_valuechanged(GtkSpinButton *spinbutton, struct ppref *priv)      \
    {                                                                                                                  \
        nsoption_set_int(OPTION, round(gtk_spin_button_get_value(spinbutton) * MULTIPLIER));                           \
    }                                                                                                                  \
                                                                                                                       \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv);                  \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv)                   \
    {                                                                                                                  \
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(widget), ((gdouble)nsoption_int(OPTION)) / MULTIPLIER);              \
    }
...
#define SPINBUTTON_UINT_SIGNALS(WIDGET, OPTION, MULTIPLIER)                                                            \
    ...
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_valuechanged(GtkSpinButton *spinbutton, struct ppref *priv)      \
    {                                                                                                                  \
        nsoption_set_uint(OPTION, round(gtk_spin_button_get_value(spinbutton) * MULTIPLIER));                          \
    }                                                                                                                  \
                                                                                                                       \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv);                  \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv)                   \
    {                                                                                                                  \
        gtk_spin_button_set_value(GTK_SPIN_BUTTON(widget), ((gdouble)nsoption_uint(OPTION)) / MULTIPLIER);             \
    }

#define ENTRY_SIGNALS(WIDGET, OPTION)                                                                                  \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_changed(GtkEditable *editable, struct ppref *priv);              \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_changed(GtkEditable *editable, struct ppref *priv)               \
    {                                                                                                                  \
        nsoption_set_charp(OPTION, strdup(gtk_entry_get_text(GTK_ENTRY(editable))));                                   \
    }                                                                                                                  \
                                                                                                                       \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv);                  \
    G_MODULE_EXPORT void nsgtk_preferences_##WIDGET##_realize(GtkWidget *widget, struct ppref *priv)                   \
...
nsgtk_preferences_fileChooserDownloads_selectionchanged(GtkFileChooser *chooser, struct ppref *priv);
...
G_MODULE_EXPORT void nsgtk_preferences_comboProxyType_realize(GtkWidget *widget, struct ppref *priv)
{
    ...
    if (nsoption_bool(http_proxy) == true) {
        ...
        if (nsoption_charp(http_proxy_host) == NULL) {
            ...
            ((nsoption_charp(http_proxy_auth_user) == NULL) || (nsoption_charp(http_proxy_auth_pass) == NULL))) {
            /* authentication selected with empty credentials, turn
             * proxy off */
            proxytype = 0;
        }
    }
...
G_MODULE_EXPORT void nsgtk_preferences_comboDefault_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), nsoption_int(font_default));
}
 ...
 */
static nserror comboboxLanguage_add_from_data(
    GtkListStore *liststore, GtkComboBox *combobox, const char *accept_language, const uint8_t *data, size_t data_size)
{
 ...
 */
static nserror comboboxLanguage_add_from_file(
    GtkListStore *liststore, GtkComboBox *combobox, const char *accept_language, const char *file_location)
{
    int active_language = -1;
    GtkTreeIter iter;
    int combo_row_count = 0;
...
G_MODULE_EXPORT void nsgtk_preferences_comboboxLanguage_realize(GtkWidget *widget, struct ppref *priv)
{
    ...
    if (res == NSERROR_OK) {
        res = comboboxLanguage_add_from_data(
            priv->content_language, GTK_COMBO_BOX(widget), accept_language, data, data_size);
    } else {
        /* attempt to read languages from file */
        res = nsgtk_path_from_resname("languages", &languages_file);
        if (res == NSERROR_OK) {
            res = comboboxLanguage_add_from_file(
                priv->content_language, GTK_COMBO_BOX(widget), accept_language, languages_file);
        }
    }
    if (res != NSERROR_OK) {
...
G_MODULE_EXPORT void nsgtk_preferences_checkShowSingleTab_toggled(GtkToggleButton *togglebutton, struct ppref *priv)
{
    nsoption_set_bool(show_single_tab, gtk_toggle_button_get_active(togglebutton));
    nsgtk_window_update_all();
}

G_MODULE_EXPORT void nsgtk_preferences_checkShowSingleTab_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(widget), nsoption_bool(show_single_tab));
}
...
G_MODULE_EXPORT void nsgtk_preferences_comboTabPosition_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), nsoption_int(position_tab));
}
...
G_MODULE_EXPORT void nsgtk_preferences_comboDeveloperView_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), nsoption_int(developer_view));
}
...
G_MODULE_EXPORT void nsgtk_preferences_comboButtonType_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_combo_box_set_active(GTK_COMBO_BOX(widget), nsoption_int(button_type) - 1);
}
...
G_MODULE_EXPORT void nsgtk_preferences_comboSearch_changed(GtkComboBox *widget, struct ppref *priv)
{
    ...
    if ((search_web_iterate_providers(-1, &defprovider) != -1) && (strcmp(provider, defprovider) == 0)) {
        free(provider);
        /* use default option */
        provider = NULL;
    }
...
G_MODULE_EXPORT void nsgtk_preferences_comboSearch_realize(GtkWidget *widget, struct ppref *priv)
{
    ...
    while (iter != -1) {
        gtk_list_store_insert_with_values(priv->search_providers, NULL, -1, 0, name, -1);
...
G_MODULE_EXPORT void nsgtk_preferences_fileChooserDownloads_realize(GtkWidget *widget, struct ppref *priv)
{
    gtk_file_chooser_set_current_folder(GTK_FILE_CHOOSER(widget), nsoption_charp(downloads_directory));
}
