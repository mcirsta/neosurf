...
void nsgtk_about_dialog_init(GtkWindow *parent)
{
    ...
    if (pixbufs != NULL) {
        ...
        image = nsgtk_image_new_from_pixbuf_icon(GDK_PIXBUF(g_list_nth_data(pixbufs, 0)), GTK_ICON_SIZE_DIALOG);
        g_list_free(pixbufs);
    ...
    g_signal_connect_swapped(dialog, "response", G_CALLBACK(nsgtk_about_dialog_response), dialog);
