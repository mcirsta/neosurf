...
void nsgtk_set_colour(colour c)
{
    cairo_set_source_rgba(
        current_cr, (c & 0xff) / 255.0, ((c & 0xff00) >> 8) / 255.0, ((c & 0xff0000) >> 16) / 255.0, 1.0);
}
...
static inline void nsgtk_set_line_width(plot_style_fixed width)
{
    if (width == 0) {
        ...
        cairo_set_line_width(current_cr, plot_style_fixed_to_double(width));
    }
}
    ...
    const struct redraw_context *ctx, const plot_style_t *style, int x, int y, int radius, int angle1, int angle2)
{
    ...
    cairo_set_line_width(current_cr, 1);
    cairo_arc(current_cr, x, y, radius, (angle1 + 90) * (M_PI / 180), (angle2 + 90) * (M_PI / 180));
...
static nserror nsgtk_plot_line(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *line)
{
     ...
     * between pixels
     */
    cairo_move_to(current_cr, (line->x0 == line->x1) ? line->x0 + 0.5 : line->x0,
        (line->y0 == line->y1) ? line->y0 + 0.5 : line->y0);
    cairo_line_to(current_cr, (line->x0 == line->x1) ? line->x1 + 0.5 : line->x1,
 ...
 */
static nserror
nsgtk_plot_rectangle(const struct redraw_context *ctx, const plot_style_t *style, const struct rect *rect)
{
    if (style->fill_type != PLOT_OP_TYPE_NONE) {
        nsgtk_set_colour(style->fill_colour);
        nsgtk_set_solid();
        ...
        cairo_set_line_width(current_cr, 0);
        cairo_rectangle(current_cr, rect->x0, rect->y0, rect->x1 - rect->x0, rect->y1 - rect->y0);
        cairo_fill(current_cr);
        cairo_stroke(current_cr);
    }
    ...
    if (style->stroke_type != PLOT_OP_TYPE_NONE) {
        ...
        cairo_rectangle(current_cr, rect->x0 + 0.5, rect->y0 + 0.5, rect->x1 - rect->x0, rect->y1 - rect->y0);
        cairo_stroke(current_cr);
    }
    return NSERROR_OK;
}
 ...
 */
static nserror
nsgtk_plot_polygon(const struct redraw_context *ctx, const plot_style_t *style, const int *p, unsigned int n)
{
    unsigned int i;
 ...
 * \return NSERROR_OK on success else error code.
 */
static nserror nsgtk_plot_path(const struct redraw_context *ctx, const plot_style_t *pstyle, const float *p,
 ...
 * \return NSERROR_OK on success else error code.
 */
static nserror nsgtk_plot_bitmap(const struct redraw_context *ctx, struct bitmap *bitmap, int x, int y, int width,
    int height, colour bg, bitmap_flags_t flags)
{
    ...
    if ((img_width == width) && (img_height == height)) {
        ...
        if (repeat_x || repeat_y) {
            cairo_pattern_set_extend(cairo_get_source(current_cr), CAIRO_EXTEND_REPEAT);
        }
 ...
 * \return NSERROR_OK on success else error code.
 */
static nserror nsgtk_plot_text(const struct redraw_context *ctx, const struct plot_font_style *fstyle, int x, int y,
