...
static gboolean nsgtk_cw_button_release_event(GtkWidget *widget, GdkEventButton *event, gpointer g)
{
    ...
    if (mouse->state & BROWSER_MOUSE_DOUBLE_CLICK) {
        if (mouse->state & BROWSER_MOUSE_PRESS_1) {
            ...
            mouse->state ^= (BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_CLICK_2 | BROWSER_MOUSE_DOUBLE_CLICK);
        }
        ...
        mouse->state ^= (BROWSER_MOUSE_HOLDING_1 | BROWSER_MOUSE_DRAG_ON);
        was_drag = true;
    } else if (mouse->state & BROWSER_MOUSE_HOLDING_2) {
        mouse->state ^= (BROWSER_MOUSE_HOLDING_2 | BROWSER_MOUSE_DRAG_ON);
        was_drag = true;
    }

    /* Handle modifiers being removed */
    if ((mouse->state & BROWSER_MOUSE_MOD_1) && !(event->state & GDK_SHIFT_MASK)) {
        mouse->state ^= BROWSER_MOUSE_MOD_1;
    }
    if ((mouse->state & BROWSER_MOUSE_MOD_2) && !(event->state & GDK_CONTROL_MASK)) {
        mouse->state ^= BROWSER_MOUSE_MOD_2;
    }
    if ((mouse->state & BROWSER_MOUSE_MOD_3) && !(event->state & GDK_MOD1_MASK)) {
        mouse->state ^= BROWSER_MOUSE_MOD_3;
    }

    /* end drag with modifiers */
    if (was_drag && (mouse->state & (BROWSER_MOUSE_MOD_1 | BROWSER_MOUSE_MOD_2 | BROWSER_MOUSE_MOD_3))) {
...
static gboolean nsgtk_cw_motion_notify_event(GtkWidget *widget, GdkEventMotion *event, gpointer g)
{
    ...
    if (mouse->pressed == false) {
        nsgtk_cw->mouse(nsgtk_cw, BROWSER_MOUSE_HOVER, event->x, event->y);
        return TRUE;
    }

    if ((fabs(event->x - mouse->last_x) < 5.0) && (fabs(event->y - mouse->last_y) < 5.0)) {
    ...
    if (mouse->state & BROWSER_MOUSE_PRESS_1) {
        /* Start button 1 drag */
        nsgtk_cw->mouse(nsgtk_cw, BROWSER_MOUSE_DRAG_1, mouse->pressed_x, mouse->pressed_y);

        /* Replace PRESS with HOLDING and declare drag in progress */
        mouse->state ^= (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_HOLDING_1);
        mouse->state |= BROWSER_MOUSE_DRAG_ON;

    } else if (mouse->state & BROWSER_MOUSE_PRESS_2) {
        /* Start button 2s drag */
        nsgtk_cw->mouse(nsgtk_cw, BROWSER_MOUSE_DRAG_2, mouse->pressed_x, mouse->pressed_y);

        /* Replace PRESS with HOLDING and declare drag in progress */
        mouse->state ^= (BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_HOLDING_2);
        mouse->state |= BROWSER_MOUSE_DRAG_ON;
        ...
        if (mouse->state & (BROWSER_MOUSE_HOLDING_1 | BROWSER_MOUSE_HOLDING_2)) {
            nsgtk_cw->mouse(nsgtk_cw, mouse->state, event->x, event->y);
        }
    }
...
static nserror nsgtk_cw_key(struct nsgtk_corewindow *nsgtk_cw, uint32_t nskey)
{
    ...
    case NS_KEY_LEFT:
        scroll = hscroll;
        value = gtk_adjustment_get_value(scroll) - nsgtk_adjustment_get_step_increment(scroll);
        if (value < nsgtk_adjustment_get_lower(scroll))
            value = nsgtk_adjustment_get_lower(scroll);
        break;
    ...
    case NS_KEY_PAGE_UP:
        scroll = vscroll;
        value = gtk_adjustment_get_value(scroll) - nsgtk_adjustment_get_page_increment(scroll);
 ...
 */
static gboolean nsgtk_cw_draw_event(GtkWidget *widget, cairo_t *cr, gpointer data)
{
...
static nserror nsgtk_cw_update_size(struct core_window *cw, int width, int height)
{
    ...
    gtk_widget_set_size_request(GTK_WIDGET(nsgtk_cw->drawing_area), width, height);
...
static nserror nsgtk_cw_get_window_dimensions(const struct core_window *cw, int *width, int *height)
{
    ...
    if (nsgtk_cw->scrolled != NULL) {
        ...
        *height = page;
        ...
        gtk_widget_get_allocation(GTK_WIDGET(nsgtk_cw->drawing_area), &allocation);
 ...
 */
static struct core_window_table nsgtk_cw_cb_table = {.invalidate = nsgtk_cw_invalidate_area,
...
nserror nsgtk_corewindow_init(struct nsgtk_corewindow *nsgtk_cw)
{
    ...
    gtk_im_context_set_client_window(
        nsgtk_cw->input_method, gtk_widget_get_parent_window(GTK_WIDGET(nsgtk_cw->drawing_area)));
    gtk_im_context_set_use_preedit(nsgtk_cw->input_method, FALSE);

    g_signal_connect(G_OBJECT(nsgtk_cw->input_method), "commit", G_CALLBACK(nsgtk_cw_input_method_commit), nsgtk_cw);

    nsgtk_connect_draw_event(GTK_WIDGET(nsgtk_cw->drawing_area), G_CALLBACK(nsgtk_cw_draw_event), nsgtk_cw);

    g_signal_connect(
        G_OBJECT(nsgtk_cw->drawing_area), "button-press-event", G_CALLBACK(nsgtk_cw_button_press_event), nsgtk_cw);

    g_signal_connect(
        G_OBJECT(nsgtk_cw->drawing_area), "button-release-event", G_CALLBACK(nsgtk_cw_button_release_event), nsgtk_cw);

    g_signal_connect(
        G_OBJECT(nsgtk_cw->drawing_area), "motion-notify-event", G_CALLBACK(nsgtk_cw_motion_notify_event), nsgtk_cw);

    g_signal_connect(
        G_OBJECT(nsgtk_cw->drawing_area), "key-press-event", G_CALLBACK(nsgtk_cw_keypress_event), nsgtk_cw);

    g_signal_connect(
        G_OBJECT(nsgtk_cw->drawing_area), "key-release-event", G_CALLBACK(nsgtk_cw_keyrelease_event), nsgtk_cw);

    nsgtk_widget_override_background_color(
        GTK_WIDGET(nsgtk_cw->drawing_area), GTK_STATE_FLAG_NORMAL, 0, 0xffff, 0xffff, 0xffff);
