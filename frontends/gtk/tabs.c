...
static void nsgtk_tab_update_size(GtkWidget *hbox, GtkStyle *previous_style, GtkWidget *close_button)
{
    ...
    metrics = pango_context_get_metrics(
        context, nsgtk_style_context_get_font(style, state), pango_context_get_language(context));

    char_width = pango_font_metrics_get_approximate_digit_width(metrics);
    pango_font_metrics_unref(metrics);

    nsgtk_icon_size_lookup_for_settings(gtk_widget_get_settings(hbox), GTK_ICON_SIZE_MENU, &w, &h);
...
static GtkWidget *nsgtk_tab_label_setup(GtkWidget *page, const char *title, GdkPixbuf *icon_pixbuf)
{
    ...
    g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy), page);

    g_signal_connect(hbox, "style-set", G_CALLBACK(nsgtk_tab_update_size), button);

    g_signal_connect(ebox, "button-release-event", G_CALLBACK(nsgtk_tab_button_release), page);
...
static void nsgtk_tab_page_reordered(GtkNotebook *notebook, GtkWidget *child, guint page_num, gpointer user_data)
{
    ...
    if (((gint)page_num == (pages - 1)) && (child != addpage)) {
        /* moved tab to end */
...
nserror nsgtk_notebook_create(GtkBuilder *builder, GtkNotebook **notebook_out)
{
    ...
    g_signal_connect(notebook, "switch-page", G_CALLBACK(nsgtk_tab_switch_page), NULL);
    g_signal_connect_after(notebook, "switch-page", G_CALLBACK(nsgtk_tab_switch_page_after), NULL);
    g_signal_connect(notebook, "page-removed", G_CALLBACK(nsgtk_tab_visibility_update), NULL);
    g_signal_connect(notebook, "page-added", G_CALLBACK(nsgtk_tab_visibility_update), NULL);
    g_signal_connect(notebook, "page-reordered", G_CALLBACK(nsgtk_tab_page_reordered), NULL);
...
/* exported interface documented in gtk/tabs.h */
void nsgtk_tab_add(
    struct gui_window *gw, GtkWidget *tab_contents, bool background, const char *title, GdkPixbuf *icon_pixbuf)
{
    GtkNotebook *notebook;
