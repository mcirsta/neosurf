 ...
 */
static nserror
nsgtk_local_history_mouse(struct nsgtk_corewindow *nsgtk_cw, browser_mouse_state mouse_state, int x, int y)
{
...
static nserror nsgtk_local_history_init(struct browser_window *bw, struct nsgtk_local_history_window **win_out)
{
    ...
    ncwin->core.scrolled = GTK_SCROLLED_WINDOW(gtk_builder_get_object(ncwin->builder, "HistoryScrolled"));

    ncwin->core.drawing_area = GTK_DRAWING_AREA(gtk_builder_get_object(ncwin->builder, "HistoryDrawingArea"));
    ...
    g_signal_connect(G_OBJECT(ncwin->wnd), "grab-broken-event", G_CALLBACK(gtk_widget_hide_on_delete), ncwin);
    /* Handle button press events */
    g_signal_connect(G_OBJECT(ncwin->wnd), "button-press-event", G_CALLBACK(gtk_widget_hide_on_delete), ncwin);
    ...
    res = local_history_init((struct core_window *)ncwin, bw, &ncwin->session);
    if (res != NSERROR_OK) {
        free(ncwin);
        return res;
    }
...
nserror nsgtk_local_history_present(GtkWindow *parent, struct browser_window *bw)
{
    ...
    if (res == NSERROR_OK) {
        gtk_window_group_add_window(gtk_window_get_group(parent), local_history_window->wnd);
        gtk_window_set_transient_for(local_history_window->wnd, parent);
        gtk_window_set_screen(local_history_window->wnd, gtk_widget_get_screen(GTK_WIDGET(parent)));
         ...
         */
        res = local_history_get_size(local_history_window->session, &width, &height);
        if (width > prnt_width) {
            width = prnt_width;
        }
