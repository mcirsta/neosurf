...
static gboolean nsgtk_window_motion_notify_event(GtkWidget *widget, GdkEventMotion *event, gpointer data)
{
    ...
    if ((fabs(event->x - g->last_x) < 5.0) && (fabs(event->y - g->last_y) < 5.0)) {
    ...
    if (g->mouse.state & BROWSER_MOUSE_PRESS_1) {
        /* Start button 1 drag */
        browser_window_mouse_click(g->bw, BROWSER_MOUSE_DRAG_1, g->mouse.pressed_x, g->mouse.pressed_y);

        /* Replace PRESS with HOLDING and declare drag in progress */
        g->mouse.state ^= (BROWSER_MOUSE_PRESS_1 | BROWSER_MOUSE_HOLDING_1);
        g->mouse.state |= BROWSER_MOUSE_DRAG_ON;
    } else if (g->mouse.state & BROWSER_MOUSE_PRESS_2) {
        /* Start button 2 drag */
        browser_window_mouse_click(g->bw, BROWSER_MOUSE_DRAG_2, g->mouse.pressed_x, g->mouse.pressed_y);

        /* Replace PRESS with HOLDING and declare drag in progress */
        g->mouse.state ^= (BROWSER_MOUSE_PRESS_2 | BROWSER_MOUSE_HOLDING_2);
        g->mouse.state |= BROWSER_MOUSE_DRAG_ON;
    }
...
static gboolean nsgtk_window_button_press_event(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    ...
    browser_window_mouse_click(g->bw, g->mouse.state, g->mouse.pressed_x, g->mouse.pressed_y);
...
static gboolean nsgtk_window_button_release_event(GtkWidget *widget, GdkEventButton *event, gpointer data)
{
    ...
    /* Handle modifiers being removed */
    if (g->mouse.state & BROWSER_MOUSE_MOD_1 && !(event->state & GDK_SHIFT_MASK)) {
        g->mouse.state ^= BROWSER_MOUSE_MOD_1;
    }
    if (g->mouse.state & BROWSER_MOUSE_MOD_2 && !(event->state & GDK_CONTROL_MASK)) {
        g->mouse.state ^= BROWSER_MOUSE_MOD_2;
    }
    if (g->mouse.state & BROWSER_MOUSE_MOD_3 && !(event->state & GDK_MOD1_MASK)) {
        g->mouse.state ^= BROWSER_MOUSE_MOD_3;
    }

    if (g->mouse.state &
        (BROWSER_MOUSE_CLICK_1 | BROWSER_MOUSE_CLICK_2 | BROWSER_MOUSE_CLICK_3 | BROWSER_MOUSE_CLICK_4 |
...
static gboolean nsgtk_window_scroll_event(GtkWidget *widget, GdkEventScroll *event, gpointer data)
{
    ...
    if (browser_window_scroll_at_point(g->bw, event->x, event->y, deltax, deltay) != true) {
...
static gboolean nsgtk_window_keypress_event(GtkWidget *widget, GdkEventKey *event, gpointer data)
{
    ...
    case GDK_KEY(KP_Left):
        value = gtk_adjustment_get_value(hscroll) - nsgtk_adjustment_get_step_increment(hscroll);
    ...
    case GDK_KEY(Up):
    case GDK_KEY(KP_Up):
        value = gtk_adjustment_get_value(vscroll) - nsgtk_adjustment_get_step_increment(vscroll);
    ...
    case GDK_KEY(KP_Page_Up):
        value = gtk_adjustment_get_value(vscroll) - nsgtk_adjustment_get_page_increment(vscroll);
...
static void nsgtk_paned_notify__position(GObject *gobject, GParamSpec *pspec, gpointer data)
{
    ...
    if (g->paned_sized == false) {
        g->paned_sized = true;
        gtk_paned_set_position(g->paned, (nsoption_int(toolbar_status_size) * pane_alloc.width) / 10000);
        return;
    }

    nsoption_set_int(toolbar_status_size, ((gtk_paned_get_position(g->paned) * 10000) / (pane_alloc.width - 1)));
}
...
{
    gtk_paned_set_position(GTK_PANED(widget), (nsoption_int(toolbar_status_size) * allocation->width) / 10000);
...
gui_window_create(struct browser_window *bw, struct gui_window *existing, gui_window_create_flags flags)
{
    ...
    /* create toolbar */
    res = nsgtk_toolbar_create(tab_builder, bw_from_gw, g, !!(flags & GW_CREATE_FOCUS_LOCATION), &g->toolbar);
    if (res != NSERROR_OK) {
        free(g);
        g_object_unref(tab_builder);
        return NULL;
    ...
    gtk_widget_add_events(GTK_WIDGET(g->layout),
        GDK_EXPOSURE_MASK | GDK_LEAVE_NOTIFY_MASK | GDK_BUTTON_PRESS_MASK | GDK_BUTTON_RELEASE_MASK |
    ...
    nsgtk_widget_override_background_color(GTK_WIDGET(g->layout), GTK_STATE_FLAG_NORMAL, 0, 0xffff, 0xffff, 0xffff);
...
#define CONNECT(obj, sig, callback, ptr) g_signal_connect(G_OBJECT(obj), (sig), G_CALLBACK(callback), (ptr))
    ...
    /* layout signals */
    CONNECT(g->layout, "motion-notify-event", nsgtk_window_motion_notify_event, g);
    CONNECT(g->layout, "button-press-event", nsgtk_window_button_press_event, g);
    CONNECT(g->layout, "button-release-event", nsgtk_window_button_release_event, g);
    CONNECT(g->layout, "key-press-event", nsgtk_window_keypress_event, g);
    CONNECT(g->layout, "key-release-event", nsgtk_window_keyrelease_event, g);
    ...
    /* input method */
    gtk_im_context_set_client_window(g->input_method, nsgtk_layout_get_bin_window(g->layout));
...
static void nsgtk_redraw_caret(struct gui_window *g)
{
    ...
    gtk_widget_queue_draw_area(GTK_WIDGET(g->layout), g->caretx - sx, g->carety - sy, 1, g->careth + 1);
}
...
static void gui_window_remove_caret(struct gui_window *g)
{
    ...
    gtk_widget_queue_draw_area(GTK_WIDGET(g->layout), g->caretx - sx, g->carety - sy, 1, oh + 1);
}
...
static void gui_window_set_pointer(struct gui_window *g, gui_pointer_shape shape)
{
    ...
    if (!nullcursor)
        cursor = gdk_cursor_new_for_display(gtk_widget_get_display(GTK_WIDGET(g->layout)), cursortype);
    gdk_window_set_cursor(nsgtk_widget_get_window(GTK_WIDGET(g->layout)), cursor);
...
static void gui_window_create_form_select_menu(struct gui_window *g, struct form_control *control)
{
    ...
    while (option != NULL) {
         ...
         */
        g_signal_connect(menu_item, "toggled", G_CALLBACK(nsgtk_select_menu_clicked), (gpointer)item);
...
static void gui_window_file_gadget_open(struct gui_window *g, struct hlcache_handle *hl, struct form_control *gadget)
{
    ...
    dialog = gtk_file_chooser_dialog_new("Select File", nsgtk_scaffolding_window(g->scaffold),
