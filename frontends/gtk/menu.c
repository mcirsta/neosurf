...
static bool nsgtk_menu_add_image_item(GtkMenu *menu, GtkWidget **item_out, const char *message, GtkAccelGroup *group)
{
    ...
    if (accelerator_desc != NULL) {
        ...
        if (key > 0) {
            gtk_widget_add_accelerator(item, "activate", group, key, mod, GTK_ACCEL_VISIBLE);
        }
    }
...
#define CHECK_ITEM(p, q, r, s)                                                                                         \
    do {                                                                                                               \
        s->q##_menuitem = GTK_CHECK_MENU_ITEM(gtk_check_menu_item_new_with_mnemonic(messages_get(#r)));                \
        if ((s->q##_menuitem != NULL) && (s->p##_menu != NULL)) {                                                      \
            gtk_menu_shell_append(GTK_MENU_SHELL(s->p##_menu), GTK_WIDGET(s->q##_menuitem));                           \
...
#define SET_SUBMENU(q, r)                                                                                              \
    do {                                                                                                               \
        ...
        if ((r->q##_submenu != NULL) && (r->q##_submenu->q##_menu != NULL) && (r->q##_menuitem != NULL)) {             \
            gtk_menu_item_set_submenu(GTK_MENU_ITEM(r->q##_menuitem), GTK_WIDGET(r->q##_submenu->q##_menu));           \
        }                                                                                                              \
    } while (0)
...
{
    struct nsgtk_scaleview_submenu *ret = malloc(sizeof(struct nsgtk_scaleview_submenu));
    if (ret == NULL) {
        nsgtk_warning(messages_get("NoMemory"), 0);
        return NULL;
    }
...
{
    struct nsgtk_developer_submenu *dmenu = malloc(sizeof(struct nsgtk_developer_submenu));
    if (dmenu == NULL) {
        nsgtk_warning(messages_get("NoMemory"), 0);
        return NULL;
    }
