...
static struct menu_events viewdata_menu_events[] = {MENUEVENT(viewdata_save_as), MENUEVENT(viewdata_print),
    MENUEVENT(viewdata_close), MENUEVENT(viewdata_select_all), MENUEVENT(viewdata_cut), MENUEVENT(viewdata_copy),
    MENUEVENT(viewdata_paste), MENUEVENT(viewdata_delete), MENUEVENT(viewdata_zoom_in), MENUEVENT(viewdata_zoom_out),
    MENUEVENT(viewdata_zoom_normal), MENUEVENT(viewdata_about), {NULL, NULL}};
...
static void nsgtk_viewdata_file_save(GtkWindow *parent, const char *filename, const char *data, size_t data_size)
{
    ...
    notif = gtk_dialog_new_with_buttons(
        messages_get("gtkSaveFailedTitle"), parent, GTK_DIALOG_MODAL, NSGTK_STOCK_OK, GTK_RESPONSE_NONE, NULL);

    g_signal_connect_swapped(notif, "response", G_CALLBACK(gtk_widget_destroy), notif);
...
gboolean nsgtk_on_viewdata_save_as_activate(GtkMenuItem *widget, gpointer g)
{
    ...
    fc = gtk_file_chooser_dialog_new(messages_get("gtkSaveFile"), nsg->window, GTK_FILE_CHOOSER_ACTION_SAVE,
...
static void nsgtk_viewdata_update_zoomlevel(gpointer g)
{
    ...
    while (nsg) {
        if (nsg->gv) {
            ...
            gtk_text_buffer_remove_all_tags(GTK_TEXT_BUFFER(buf), &start, &end);
            gtk_text_buffer_apply_tag(GTK_TEXT_BUFFER(buf), GTK_TEXT_TAG(tag), &start, &end);
        }
        nsg = nsg->next;
    }
}
...
static nserror window_init(const char *title, const char *filename, char *ndata, size_t ndata_len)
{
    ...
    g_signal_connect(G_OBJECT(window), "destroy", G_CALLBACK(nsgtk_viewdata_destroy_event), newctx);
    g_signal_connect(G_OBJECT(window), "delete-event", G_CALLBACK(nsgtk_viewdata_delete_event), newctx);

    dataview = GTK_TEXT_VIEW(gtk_builder_get_object(newctx->builder, "viewdata_view"));
...
static char **xdg_data_strvec(void)
{
    ...
    if ((xdg_data_dirs == NULL) || (*xdg_data_dirs == 0) || (strlen(xdg_data_dirs) > 4096)) {
        xdg_data_dirs = "/usr/local/share/:/usr/share/";
    }

    xdg_data_home = getenv("XDG_DATA_HOME");
    if ((xdg_data_home == NULL) || (*xdg_data_home == 0) || (strlen(xdg_data_home) > 4096)) {
        /* $XDG_DATA_HOME is empty use $HOME/.local/share */

        home_dir = getenv("HOME");
        if ((home_dir == NULL) || (*home_dir == 0) || (strlen(home_dir) > 4096)) {
            ...
            snprintf(xdg_data_path, xdg_data_size, "%s/.local/share/:%s", home_dir, xdg_data_dirs);
        }
    } else {
...
static nserror editor_init_fname(const char *title, const char *leafname, const char *fname)
{
    ...
    while (xdg_data_vec[veci] != NULL) {
        def_app_desktop = xdg_get_default_app(xdg_data_vec[veci], "text/plain");
    ...
    if (g_spawn_async(NULL, argv, NULL, G_SPAWN_SEARCH_PATH, NULL, NULL, NULL, NULL) != TRUE) {
        return NSERROR_NOT_FOUND;
    }
