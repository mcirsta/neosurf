 ...
 * urlbar constructor
 */
NS_URLBar::NS_URLBar(QWidget *parent, NS_Actions *actions, struct browser_window *bw) : QToolBar(parent), m_bw(bw)
{
    ...
    m_input->addAction(actions->m_reset_page_scale, QLineEdit::TrailingPosition);
#endif
    addWidget(m_input);
    ...
    connect(m_input, &QLineEdit::returnPressed, this, &NS_URLBar::input_pressed);
}
...
void NS_URLBar::input_pressed()
{
    ...
    if (res == NSERROR_OK) {
        res = browser_window_navigate(this->m_bw, url, NULL, BW_NAVIGATE_HISTORY, NULL, NULL, NULL);
        nsurl_unref(url);
    }
}
...
nserror NS_URLBar::set_url(struct nsurl *url)
{
    ...
    if (nsurl_get_utf8(url, &idn_url_s, &idn_url_l) == NSERROR_OK) {
        ...
        m_input->setText(QString::fromUtf8(nsurl_access(url), nsurl_length(url)));
    }
