...
static char *accept_language_from_selection(const QList<QByteArray> &selected)
{
    ...
    if (alang != NULL) {
        ...
        for (idx = 0; idx < selected.size(); idx++) {
            ...
            outlen = snprintf(
                cur, totallen, "%.*s;q=%.1f, ", (int)selected.at(idx).size(), selected.at(idx).constData(), quality);
            cur += outlen;
            totallen -= outlen;
        }
        cur -= 2;
...
void HomeSettings::categoryApply()
{
    if (m_homeurl->isModified()) {
        nsoption_set_charp(homepage_url, strdup(m_homeurl->text().toStdString().c_str()));
    }
}

HomeSettings::HomeSettings(QWidget *parent = nullptr) : AbstractSettingsCategory(parent), m_homeurl(new QLineEdit)
{
...
AppearanceSettings::AppearanceSettings(QWidget *parent = nullptr)
    : AbstractSettingsCategory(parent), m_opentab(new QCheckBox), m_switchnew(new QCheckBox),
      m_colour_selection(new QComboBox), m_zoom(new QComboBox)
{
...
void LanguageSettings::categoryRealize()
{
    ...
    for (int idx = 0; idx < split.size(); idx++) {
        ...
        for (langlen = 0; langlen < ent.size(); langlen++) {
            if ((ent[langlen] < 'a' || ent[langlen] > 'z') && (ent[langlen] < 'A' || ent[langlen] > 'Z') &&
                (ent[langlen] < '0' || ent[langlen] > '9') && (ent[langlen] != '-')) {
                break;
            }
        }
...
SearchSettings::SearchSettings(QWidget *parent = nullptr) : AbstractSettingsCategory(parent), m_provider(new QComboBox)
{
    int iter;
    const char *name;
...
PrivacySettings::PrivacySettings(QWidget *parent = nullptr)
    : AbstractSettingsCategory(parent), m_preventpopups(new QCheckBox), m_hideadverts(new QCheckBox),
      m_enablednt(new QCheckBox), m_enablereferral(new QCheckBox)
{
...
{
    nsoption_set_bool(disable_popups, (m_preventpopups->checkState() == Qt::Checked));

    nsoption_set_bool(block_advertisements, (m_hideadverts->checkState() == Qt::Checked));

    nsoption_set_bool(do_not_track, (m_enablednt->checkState() == Qt::Checked));

    nsoption_set_bool(send_referer, (m_enablereferral->checkState() == Qt::Checked));
}
...
void NetworkSettings::categoryApply()
{
    ...
    if (access == 0) {
        ...
        if (m_proxy_host->isModified()) {
            nsoption_set_charp(http_proxy_host, strdup(m_proxy_host->text().toStdString().c_str()));
        }
        nsoption_set_int(http_proxy_port, m_proxy_port->value());
        if (m_proxy_auth_user->isModified()) {
            nsoption_set_charp(http_proxy_auth_user, strdup(m_proxy_auth_user->text().toStdString().c_str()));
        }
        if (access == 1) {
            ...
            if (m_proxy_auth_pass->isModified()) {
                nsoption_set_charp(http_proxy_auth_pass, strdup(m_proxy_auth_pass->text().toStdString().c_str()));
            }
            if (m_proxy_noproxy->isModified()) {
                nsoption_set_charp(http_proxy_noproxy, strdup(m_proxy_noproxy->text().toStdString().c_str()));
            }
        }
    }
...
NetworkSettings::NetworkSettings(QWidget *parent = nullptr)
    : AbstractSettingsCategory(parent), m_proxylayout(new QFormLayout), m_hostport(new QWidget),
      m_proxy_access(new QComboBox), m_proxy_host(new QLineEdit), m_proxy_port(new QSpinBox),
      m_proxy_auth_user(new QLineEdit), m_proxy_auth_pass(new QLineEdit), m_proxy_noproxy(new QLineEdit)
{
    // Proxy

    connect(m_proxy_access, &QComboBox::currentIndexChanged, this, &NetworkSettings::proxy_access_changed);
...
NS_Settings::NS_Settings(QWidget *parent)
    : QWidget(parent), m_headerlabel(new QLabel), m_categorylist(new FirstListWidget), m_categories(new QStackedWidget),
      m_buttonbox(new QDialogButtonBox(QDialogButtonBox::Ok | QDialogButtonBox::Cancel | QDialogButtonBox::Apply))
{
    connect(m_categorylist, &QListWidget::currentRowChanged, m_categories, &QStackedWidget::setCurrentIndex);

    connect(m_categorylist, &QListWidget::currentTextChanged, this, &NS_Settings::categorychanged_slot);

    connect(m_buttonbox, &QDialogButtonBox::clicked, this, &NS_Settings::clicked_slot);
...
void NS_Settings::showEvent(QShowEvent *event)
{
    for (int idx = 0; idx < m_categories->count(); idx++) {
        dynamic_cast<AbstractSettingsCategory *>(m_categories->widget(idx))->categoryRealize();
    }
}
...
void NS_Settings::clicked_slot(QAbstractButton *button)
{
    ...
    if ((role == QDialogButtonBox::AcceptRole) || (role == QDialogButtonBox::ApplyRole)) {
        for (int idx = 0; idx < m_categories->count(); idx++) {
            dynamic_cast<AbstractSettingsCategory *>(m_categories->widget(idx))->categoryApply();
        }
        NS_Application::instance()->nsOptionPersist();
    }
    if ((role == QDialogButtonBox::RejectRole) || (role == QDialogButtonBox::AcceptRole)) {
        hide();
    }
}
