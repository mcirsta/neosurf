...
void NS_Application::nsOptionFromPalette(struct nsoption_s *opts)
{
    ...
    */
    struct {
        ...
        {QPalette::Active, QPalette::Highlight, /* this could be Accent (qt 6.6) */
            NSOPTION_sys_colour_AccentColor},
        {QPalette::Active, QPalette::HighlightedText, NSOPTION_sys_colour_AccentColorText},
        {QPalette::Active, QPalette::BrightText, NSOPTION_sys_colour_ActiveText},
        {QPalette::Active, QPalette::Light, NSOPTION_sys_colour_ButtonBorder},
        {QPalette::Active, QPalette::Button, NSOPTION_sys_colour_ButtonFace},
        {QPalette::Active, QPalette::ButtonText, NSOPTION_sys_colour_ButtonText},
        {QPalette::Active, QPalette::Window, NSOPTION_sys_colour_Canvas},
        {QPalette::Active, QPalette::WindowText, NSOPTION_sys_colour_CanvasText},
        {QPalette::Active, QPalette::Base, NSOPTION_sys_colour_Field},
        {QPalette::Active, QPalette::Text, NSOPTION_sys_colour_FieldText},
        {QPalette::Disabled, QPalette::Text, NSOPTION_sys_colour_GrayText},
        {QPalette::Active, QPalette::Highlight, NSOPTION_sys_colour_Highlight},
        {QPalette::Active, QPalette::HighlightedText, NSOPTION_sys_colour_HighlightText},
        {QPalette::Active, QPalette::Link, NSOPTION_sys_colour_LinkText},
        {QPalette::Active, QPalette::Highlight, NSOPTION_sys_colour_Mark},
        {QPalette::Active, QPalette::HighlightedText, NSOPTION_sys_colour_MarkText},
        {QPalette::Active, QPalette::AlternateBase, NSOPTION_sys_colour_SelectedItem},
        {QPalette::Active, QPalette::BrightText, NSOPTION_sys_colour_SelectedItemText},
        {QPalette::Active, QPalette::LinkVisited, NSOPTION_sys_colour_VisitedText},
        {QPalette::Active, QPalette::NoRole, NSOPTION_LISTEND},
    };
    const QPalette palette;
    int idx;
    int r, g, b;
    for (idx = 0; entries[idx].option != NSOPTION_LISTEND; idx++) {
        palette.color(entries[idx].group, entries[idx].role).getRgb(&r, &g, &b);
        opts[entries[idx].option].value.c = ((b & 0xff) << 16) | ((g & 0xff) << 8) | (r & 0xff);
    }
}
        ...
        if (settings.contains(option->key)) {
            ...
            case OPTION_STRING:
                nsoption_set_tbl_charp(
                    nsoptions, (enum nsoption_e)entry, strdup(settings.value(option->key).toString().toUtf8()));
                break;
            }
        }
    }
...
static size_t set_qtsetting(struct nsoption_s *option, void *ctx)
{
    ...
    case OPTION_STRING:
        settings->setValue(
            option->key, QString(((option->value.s == NULL) || (*option->value.s == 0)) ? "" : option->value.s));
        break;
    }
...
void NS_Application::nsOptionPersist()
{
    QSettings settings;
    settings.clear();
    nsoption_generate(set_qtsetting, &settings, NSOPTION_GENERATE_CHANGED, NULL, NULL);
}
...
void NS_Application::nsOptionUpdate()
{
    ...
    case 0: {
        // automaticaly select
        const QPalette palette;
        const bool dark_mode = palette.base().color().lightness() < palette.windowText().color().lightness();
      ...
      m_local_history_window(nullptr), m_global_history_window(nullptr), m_cookies_window(nullptr)
{
    ...
    res = messages_add_from_inline((uint8_t *)messages_data.data(), messages_data.size());
...
{
    if ((m_schedule_timer->isActive() == false) || (m_schedule_timer->remainingTime() > ms)) {
        m_schedule_timer->start(ms);
    }
}
...
nserror NS_Application::create_browser_widget(struct browser_window *existing, bool intab)
{
    nsurl *url = NULL;
...
nserror
NS_Application::create_browser_widget(struct hlcache_handle *hlchandle, struct browser_window *existing, bool intab)
{
    if (hlchandle == NULL) {
        return NSERROR_BAD_PARAMETER;
    }
...
nserror NS_Application::create_browser_widget(nsurl *url, struct browser_window *existing, bool intab)
{
    ...
    if (res == NSERROR_OK) {
        res = browser_window_create((enum browser_window_create_flags)flags, url, NULL, existing, NULL);
    }
