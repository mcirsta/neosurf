...
NS_Actions::NS_Actions(QWidget *parent, struct browser_window *bw)
    : QObject(parent),
      m_back(new QAction(parent->style()->standardIcon(QStyle::SP_ArrowLeft), messages_get("Back"), parent)),
      m_forward(new QAction(parent->style()->standardIcon(QStyle::SP_ArrowRight), messages_get("Forward"), parent)),
      m_stop_reload(new QAction(parent)), m_settings(new QAction(messages_get("Settings"), parent)),
      m_bookmarks(new QAction(messages_get("ManageBookmarks"), parent)),
      m_add_edit_bookmark(new QAction(QIcon(":/icons/hotlist-add.png"), messages_get("AddBookmark"), parent)),
      m_local_history(new QAction(QIcon(":/local-history.png"), messages_get("HistLocalNS"), parent)),
      m_global_history(new QAction(messages_get("HistGlobalNS"), parent)),
      m_cookies(new QAction(messages_get("ShowCookiesNS"), parent)),
      m_page_info(new QAction(QIcon(":/icons/page-info-internal.png"), tr("Page Info"), parent)),
      ...
      m_newwindow(new QAction(messages_get("NewWindowNS"), parent)), m_quit(new QAction(messages_get("Quit"), parent)),
      ...
      m_sel_search(new QAction(messages_get("SearchWeb"), parent)), m_bw(bw), m_marked(false),
      m_pistate(PAGE_STATE_INTERNAL), m_link(nullptr), m_object(nullptr), m_selection(nullptr)
{
    /* shortcuts */
    ...
    connect(m_forward, &QAction::triggered, this, &NS_Actions::forward_slot);

    connect(m_stop_reload, &QAction::triggered, this, &NS_Actions::stop_reload_slot);

    connect(m_settings, &QAction::triggered, this, &NS_Actions::settings_slot);

    connect(m_bookmarks, &QAction::triggered, this, &NS_Actions::bookmarks_slot);

    connect(m_add_edit_bookmark, &QAction::triggered, this, &NS_Actions::add_edit_bookmark_slot);

    connect(m_local_history, &QAction::triggered, this, &NS_Actions::local_history_slot);

    connect(m_global_history, &QAction::triggered, this, &NS_Actions::global_history_slot);

    connect(m_cookies, &QAction::triggered, this, &NS_Actions::cookies_slot);

    connect(m_page_info, &QAction::triggered, this, &NS_Actions::page_info_slot);

    connect(m_page_scale, &QAction::triggered, this, &NS_Actions::reset_page_scale_slot);

    connect(m_reset_page_scale, &QAction::triggered, this, &NS_Actions::reset_page_scale_slot);

    connect(m_reduce_page_scale, &QAction::triggered, this, &NS_Actions::reduce_page_scale_slot);

    connect(m_increase_page_scale, &QAction::triggered, this, &NS_Actions::increase_page_scale_slot);

    connect(m_newtab, &QAction::triggered, this, &NS_Actions::newtab_slot);

    connect(m_newwindow, &QAction::triggered, this, &NS_Actions::newwindow_slot);

    connect(m_quit, &QAction::triggered, this, &NS_Actions::quit_slot);

    connect(m_page_save, &QAction::triggered, this, &NS_Actions::page_save_slot);

    connect(m_page_source, &QAction::triggered, this, &NS_Actions::page_source_slot);

    connect(m_debug_render, &QAction::triggered, this, &NS_Actions::debug_render_slot);

    connect(m_debug_box_tree, &QAction::triggered, this, &NS_Actions::debug_box_tree_slot);
    ...
    connect(m_link_new_window, &QAction::triggered, this, &NS_Actions::link_new_window_slot);

    connect(m_link_bookmark, &QAction::triggered, this, &NS_Actions::link_bookmark_slot);

    connect(m_link_save, &QAction::triggered, this, &NS_Actions::link_save_slot);

    connect(m_link_copy, &QAction::triggered, this, &NS_Actions::link_copy_slot);

    connect(m_img_new_tab, &QAction::triggered, this, &NS_Actions::img_new_tab_slot);

    connect(m_img_save, &QAction::triggered, this, &NS_Actions::img_save_slot);

    /* image link copy uses object copy slot */
    connect(m_img_copy, &QAction::triggered, this, &NS_Actions::obj_copy_slot);

    connect(m_obj_save, &QAction::triggered, this, &NS_Actions::obj_save_slot);

    connect(m_obj_copy, &QAction::triggered, this, &NS_Actions::obj_copy_slot);

    connect(m_sel_copy, &QAction::triggered, this, &NS_Actions::sel_copy_slot);

    connect(m_sel_search, &QAction::triggered, this, &NS_Actions::sel_search_slot);
...
static const float page_scales[PAGE_SCALE_COUNT] = {
    0.33, 0.50, 0.67, 0.75, 0.80, 0.90, 1.00, 1.10, 1.20, 1.33, 1.50, 1.70, 2.00, 2.40, 3.00, 4.00, 5.00};
...
void NS_Actions::sel_search_slot(bool checked)
{
    ...
    if (res == NSERROR_OK) {
        res = browser_window_create(
            (enum browser_window_create_flags)(BW_CREATE_HISTORY | BW_CREATE_TAB | BW_CREATE_FOREGROUND), url, NULL,
            m_bw, NULL);
        nsurl_unref(url);
    }
    if (res != NSERROR_OK) {
