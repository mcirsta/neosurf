...
NS_ListSelection::NS_ListSelection(QWidget *parent)
    : QWidget(parent), m_source(new FirstListWidget), m_selected(new FirstListWidget), m_add(new QToolButton),
      m_rem(new QToolButton), m_sel_up(new QToolButton), m_sel_down(new QToolButton)
{
    m_count = 0;
    ...
    connect(m_add, &QAbstractButton::clicked, this, &NS_ListSelection::addtoselection);

    m_rem->setIcon(style()->standardIcon(QStyle::SP_ArrowLeft));
    m_rem->setAccessibleName("Remove");
    connect(m_rem, &QAbstractButton::clicked, this, &NS_ListSelection::remfromselection);

    m_sel_up->setIcon(style()->standardIcon(QStyle::SP_ArrowUp));
    m_sel_up->setAccessibleName("Move up");
    connect(m_sel_up, &QAbstractButton::clicked, this, &NS_ListSelection::selectionup);

    m_sel_down->setIcon(style()->standardIcon(QStyle::SP_ArrowDown));
    m_sel_down->setAccessibleName("Move down");
    connect(m_sel_down, &QAbstractButton::clicked, this, &NS_ListSelection::selectiondown);
