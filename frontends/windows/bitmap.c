...
static void *win32_bitmap_create(int width, int height, enum gui_bitmap_flags flags)
{
	...
	NSLOG(neosurf, INFO, "width %d, height %d, flags %u", width, height,
	      (unsigned)flags);
...
static void bitmap_set_opaque(void *bitmap, bool opaque)
{
	...
	NSLOG(neosurf, INFO, "setting bitmap %p to %s", bm,
...
bitmap_render(struct bitmap *bitmap, struct hlcache_handle *content)
{
	...
	NSLOG(neosurf, INFO, "bitmap %p for content %p width %d, height %d",
