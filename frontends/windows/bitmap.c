 ...
 */
static void *
win32_bitmap_create(int width, int height, enum gui_bitmap_flags flags)
{
...
struct bitmap *bitmap_scale(struct bitmap *prescale, int width, int height)
{
	...
	for (i = 0; i < height; i++) {
		...
		for (ii = 0; ii < width; ii++) {
			retpixdata[v + ii] =
				inpixdata[vv + (int)((ii * prescale->width) /
						     width)];
		}
	}
	return ret;
...
nserror win32_bitmap_ensure_scaled(struct bitmap *bm, int width, int height)
{
	...
	if (bm->scaled_windib != NULL && bm->scaled_pbmi != NULL &&
	    bm->scaled_pixdata != NULL && bm->scaled_width == width &&
...
bitmap_render(struct bitmap *bitmap, struct hlcache_handle *content)
{
	...
	fsbitmap = win32_bitmap_create(width,
				       height,
