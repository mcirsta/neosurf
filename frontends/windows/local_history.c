 ...
 */
static nserror
nsw32_local_history_mouse(struct nsw32_corewindow *nsw32_cw, browser_mouse_state mouse_state, int x, int y)
{
    struct nsw32_local_history_window *lhw;
 ...
 */
static nserror
nsw32_local_history_init(HINSTANCE hInstance, struct browser_window *bw, struct nsw32_local_history_window **win_out)
{
...
nserror nsw32_local_history_present(HWND hWndParent, struct browser_window *bw)
{
    ...
    if (res == NSERROR_OK) {
        ...
        SetWindowPos(local_history_window->core.hWnd, HWND_TOP, parentr.left + (margin / 2), parentr.top + (margin / 2),
